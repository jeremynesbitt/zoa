! TODO:  This file should go away.  Still being used by lens editor but
...
module hl_zoa_tree_tmp
  ...
  use gtk, only: gtk_cell_layout_get_cells,&
       ...
       & TRUE, FALSE, g_signal_connect, gtk_combo_box_new_with_model_and_entry, &
       & gtk_combo_box_set_entry_text_column, gtk_combo_box_set_active
  ...
  use g, only: g_list_foreach, g_list_free, g_list_length, g_list_nth,&
       ...
       & g_value_take_object, g_object_ref, g_value_set_static_string
...
contains
  ...
  function hl_gtk_tree_new(scroll, ncols, types, changed, data, multiple,&
       ...
       & edited_combo, data_edited_combo, changed_combo, data_changed_combo, &
       & toggled_radio, data_toggled_radio,  &
       & hscroll_policy, vscroll_policy, valsArray, refsArray) result(tree)
    ...
    ! if (.not.present(valsArray)) then
    !   !allocate(character(len=4) :: valsArray(1))
    !   !allocate(refsArray(1))
    !    valsArray(1) = "TMP"
    !    refsArray(1) = 0
    ! end if
    PRINT *, "tree_new valsArray is ", valsArray
    PRINT *, "tree_new refsArray is ", refsArray
  ...
  subroutine hl_gtk_list_tree_add_column(icol, view, is_list, type, &
       ...
       & data_toggled_radio, valsArray, refsArray)
 ...
 !+
  ...
  subroutine hl_gtk_list_tree_combo_model_attach(renderer, valsArray, refsArray, cmodel, colno)
    type(c_ptr), intent(in) :: renderer
    character(kind=c_char, len=*),intent(in), dimension(:) :: valsArray
    integer(c_int), intent(in), dimension(:) :: refsArray
