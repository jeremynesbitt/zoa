...
!
   CHARACTER WQLOCAL*8
!
   real(real64) W2LOCAL,CLEAR
!
   COMMON/LOCALWQ/WQLOCAL,W2LOCAL
!
   real(real64) X(1:96),NWN1,NWN2,NWN3,NWN4,JPX,TRAYY,TRAYX ,JPY,JPUX,JPUY,JPCX,JPCY,JPUCX,JPUCY,VXLO,VXHI,VYLO,VYHI ,V1,V2,VALVAL,OPDWT,GREYOP,RPOINT,FPOINT,RUN_OPT_MAC
!
   COMMON/WTOPD/OPDWT,GREYOP
!
   INTEGER IAUTO,ISFI,ITYP,SF,IV,PREDEFI,I,NF,IIA,JIA,SF1 ,K,II,WVNUMOP,ERROR,ISURF,CLRTYP,n
!
   COMMON/AUTOI/IAUTO
!
   real(real64) WV,VALUE,CONSUM,DELLELL,DELLELL1,DELLELL2
   INTEGER NUM5,ORI
   COMMON/GV/VALUE,NUM5
!
   COMMON/PRCOM/WV,ITYP
!
   real(real64) OLDREG9,W1A,TEMPR1,TEMPR2,TEMPSUM,TEMPDIF ,INDEX,EDGTHK,VNUM,PARTL,V,DISP,INTV,W1B,XCJ,YCJ,ZCJ,DIAX,DIAY ,ENDIST,EXDIST,AAA
!
...
!
   COMMON/OPOPMP/OPMAP
!
   real(real64) INV,COSARG,OPVALUE,DUMMY,EFLY,EFLX
!
...
!     SET MSG TO FALSE
            ...
            ELSE
               ...
               IF(INT(OPERND(I,9)).EQ.1)  call sysConfig%setSpectralWeights(1,1.0D0)
               IF(INT(OPERND(I,9)).EQ.2)  call sysConfig%setSpectralWeights(2,1.0D0)
               IF(INT(OPERND(I,9)).EQ.3)  call sysConfig%setSpectralWeights(3,1.0D0)
               IF(INT(OPERND(I,9)).EQ.4)  call sysConfig%setSpectralWeights(4,1.0D0)
               IF(INT(OPERND(I,9)).EQ.5)  call sysConfig%setSpectralWeights(5,1.0D0)
               IF(INT(OPERND(I,9)).EQ.6)  call sysConfig%setSpectralWeights(6,1.0D0)
               IF(INT(OPERND(I,9)).EQ.7)  call sysConfig%setSpectralWeights(7,1.0D0)
               IF(INT(OPERND(I,9)).EQ.8)  call sysConfig%setSpectralWeights(8,1.0D0)
               IF(INT(OPERND(I,9)).EQ.9)  call sysConfig%setSpectralWeights(9,1.0D0)
               IF(INT(OPERND(I,9)).EQ.10) call sysConfig%setSpectralWeights(10,1.0D0)
               OLDIF=LDIF
               LDIF=.FALSE.
               CALL SPOT1(2)
               LDIF=OLDIF
...
FUNCTION RUN_OPT_MAC(I)
   ...
   IMPLICIT NONE
   INTEGER I,OLDOUT1
   real(real64) RUN_OPT_MAC
   COMMON/OLDOUTT/OLDOUT1
   OLDOUT1=OUT
   OUT=98
      CALL KDP_EXEC('MACROOPT')
   OUT=OLDOUT1
   RUN_OPT_MAC=GPREG(I)
   RETURN
END
