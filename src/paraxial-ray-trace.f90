...
module paraxial_ray_trace_test
    ...
    !       PICY'(1)
            ...
            PRINT *, "PXTRAY(1:8,1) is ", PXTRAY(1:8,1)
    !
...
function traNextSurf(lastSurf, surfIdx, lambdaIdx, useXZPlane) result(nextSurf)
...
! Ideal lenses will be taken into account here
    ...
    if(present(useXZPlane) .EQV. .FALSE.) useXZPlane = .FALSE.

    nextSurf(1) = lastSurf(1)+ldm%getSurfThi(surfIdx-1)*lastSurf(2)
    curv = ldm%getSurfCurv(surfIdx, useXZPlane)
