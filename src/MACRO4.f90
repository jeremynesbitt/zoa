!       FOURTH FILE OF MACRO FILES

! SUB MACOUT.FOR
SUBROUTINE MACOUT
   use zoa_file_handler
!
   use DATMAI
   IMPLICIT NONE
!
   LOGICAL EXST,EXISJK
!
!       THIS SUBROUTINE IS CALLED BY MMFL AND
!       CAUSES THE ITH MACRO TO BE OUTPUT
!       TO THE DEFAULT OUTPUT DEVICE IN THE SAME
!       FORMAT AS IF IT WERE BEING INPUT FROM THE KEYBOARD
!
   INTEGER OCC,MCLOC1,IMCEN1,LENG,NF,&
   &K,L,I,rec30, uMFILE
!
   CHARACTER MCNAM1*8,AN1*23,AN2*23,AN3*23,AN4*23,&
   &AN5*23,BN1*11,BN2*11,BN3*11,BN4*11,BN5*11,AAN1*1,AAN2*1,&
   &AAN3*1,AAN4*1,AAN5*1,BBBN1*1,BBBN2*1,BBBN3*1,BBBN4*1,BBBN5*1,&
   &LINE1*80,LINE2*140,FILNAM*10,CM*1,STAMP*20,BLIN80*80,&
   &BLIN13*140
!
   COMMON/NEFER/NF
!
   COMMON/COMNAM/FILNAM
!
   INCLUDE 'DATMAC.INC'
   BLIN80=AA//AA//AA//AA
   BLIN13=AA//AA//AA//AA//AA//AA//AA
!
!       OPEN UNIT 20 FOR I/O
   CM=','
!
!       ***************************************************************
   EXISJK=.FALSE.
   INQUIRE(FILE=trim(LIBMAC)//'MAC.DAT',EXIST=EXISJK)
   IF(.NOT.EXISJK) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MACRO FILES DO NOT YET EXIST'//'\n'//&
      & 'TO INITIALIZE IT, USE "IMF" AND "PROCEED"', 1)
      RETURN
   END IF
!       ***************************************************************
   OPEN(UNIT=20,ACCESS='DIRECT',FILE=trim(LIBMAC)//'MAC.DAT',FORM=&
   &'UNFORMATTED',RECL=(40*NRECL),STATUS='UNKNOWN')
!
   READ(UNIT=20,REC=NF,ERR=10) MCNAM1,MCLOC1,IMCEN1,OCC,STAMP
!
!       MCNAN1 IS THE MACRO NAME, MCLOC1 IS THE MACRO LOCK,
!       IMCEN1 IS THE MACRO LENGTH IN LINES, AND OCC IS A FLAG
!       =0 IF THE RECORD REPRESENTS A BLANK MACRO AND =1
!       FOR A NON-BLANK MACRO.
!
!       MACRO RECORDS (MAXIMUM OF MAXLL NON-BLANK RECL=55*NRECL RECORDS)
!       ARE USED TO STORE A MACRO.
!
   CALL MACFIL
   EXST=.FALSE.
   INQUIRE(FILE=trim(LIBMAC)//FILNAM,EXIST=EXST)
   IF(.NOT.EXST) THEN
      OUTLYNE=&
      &'MACRO BODY FILE '//FILNAM//' MISSING'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'MACRO FILE DAMAGED, SEE REFERENCE MANUAL FOR'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'REPAIR PROCEDURE'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   !CALL READMACROFILE(trim(LIBMAC)//FILNAM)
   uMFILE = open_macro_file(FILNAM)

!
!       NOW DETERMINE THE LOCATION OF THE MACRO BODY.
!
!       NOW READ THE BODY OF THE MACRO STARTING AT RECORD 1
!
   DO 30 K=1,(IMCEN1)
      L=K-1
      READ(UNIT=30,REC=K) MACCW(L),MACQW(L),MACSTR(L),MACNW(1,L),&
      &MACNW(2,L),MACNW(3,L),MACNW(4,L),MACNW(5,L),MACSTA(1,L),&
      &MACSTA(2,L),MACSTA(3,L),MACSTA(4,L),MACSTA(5,L),MACSTA(6,L),&
      &MACSTA(7,L),MACSTA(8,L),MACSTA(9,L),MACSTA(10,L),MACSTA(11,L),&
      &MACSTA(12,L),MACSTA(13,L),MACSTA(14,L),MACSTA(15,L),MACSTA(16,L)&
      &,MACSTA(17,L),MACSTA(18,L),MACSTA(19,L),MACSTA(20,L)
!
30 CONTINUE
   CALL CLOSE_FILE(30,1)
   CALL CLOSE_FILE(20,1)
!
!       THE MACRO ARRAYS ARE NOW LOADED. NOW REFORMAT AND OUTPUT.
!
!       THE OUTPUT WILL HAVE IMCEN1 LINES IN IT.
!
   DO I=0,(IMCEN1-1)
!
!       CHECK FOR PRESECE OF A QUALIFIER
      IF(MACSTA(5,I).EQ.1) THEN
!       THERE IS A QUALIFIER WORD
!       PRODUCE OUTPUT
!
!       IF OUT = 6 (SCREEN) OR OUT = 7 (PRINTER)
!       THEN WRITE COMPACT 80 COLUMN OUTPUT.
!
!       FOR ED,CR,AND PU WRITE EXTENDED COMPLETE OUTPUT
!
         IF(OUT.EQ.8.OR.OUT.EQ.9.OR.OUT.EQ.10.OR.OUT.EQ.97) THEN
            LINE2=MACCW(I)//CHAR(32)//MACQW(I)
         ELSE
            LINE1=MACCW(I)//CHAR(32)//MACQW(I)
         END IF
!       THERE IS NO STRING,THERE ARE NUMERIC WORDS AND A QUALIFIER.
!       BEFORE WRITING THE NUMERIC DATA,IT MUST BE CONVERTED
!       INTO A STRING VARIABLE. OTHERWISE HANDELING THE DEFAULT
!       VALUES BECOMES AN S.O.B..
!       CONVERT THE NUMERIC VALES TO STRING VALUES WITH
!       SUBROUTINE NWTOAW.  THE CALL IS :
!
!               CALL NWTOAW(N1,N2,N3,N4,N5,AN1,AN2,AN3,AN4,AN5,
!     1  BN1,BN2,BN3,BN4,BN5)
!       THW RETURNED VALUES ARE AN1,AN2,AN3,AN4,AN5
!
         CALL NWTOAW(MACNW(1,I),MACNW(2,I),MACNW(3,I),MACNW(4,I)&
         &,MACNW(5,I),AN1,AN2,AN3,AN4,AN5,BN1,BN2,BN3,BN4,BN5)
!
!       DEAL WITH THE DEFAULT VALUES
         AAN1=','
         AAN2=','
         AAN3=','
         AAN4=','
         AAN5=','
         BBBN1=','
         BBBN2=','
         BBBN3=','
         BBBN4=','
         BBBN5=','
!
!       PRODUCE OUTPUT. WRITE NUMERIC WORDS EVEN IF ALL ZERO.
!
         IF(OUT.EQ.8.OR.OUT.EQ.9.OR.OUT.EQ.10.OR.OUT.EQ.97) THEN
            LINE2=MACCW(I)//CHAR(32)//MACQW(I)//CM
            IF(MACSTA(12,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN1
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN1//CM
            END IF
            IF(MACSTA(13,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN2
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN2//CM
            END IF
            IF(MACSTA(14,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN3
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN3//CM
            END IF
            IF(MACSTA(15,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN4
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN4//CM
            END IF
            IF(MACSTA(16,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN5
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN5//CM
            END IF
         ELSE
            LINE1=MACCW(I)//CHAR(32)//MACQW(I)//CM
            IF(MACSTA(12,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN1
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN1//CM
            END IF
            IF(MACSTA(13,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN2
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN2//CM
            END IF
            IF(MACSTA(14,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN3
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN3//CM
            END IF
            IF(MACSTA(15,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN4
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN4//CM
            END IF
            IF(MACSTA(16,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN5
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN5//CM
            END IF
         END IF
      ELSE
!       NO QUALIFIER WORD
!       THERE IS NUMERIC INPUT.
!       BEFORE WRITING THE NUMERIC DATA,IT MUST BE CONVERTED
!       INTO A STRING VARIABLE. OTHERWISE HANDELING THE DEFAULT
!       VALUES BECOMES AN S.O.B..
!       CONVERT THE NUMERIC VALES TO STRING VALUES WITH
!       SUBROUTINE NWTOAW.  THE CALL IS :
!
!               CALL NWTOAW(N1,N2,N3,N4,N5,AN1,AN2,AN3,AN4,AN5)
!       THW RETURNED VALUES ARE AN1,AN2,AN3,AN4,AN5,BN1,BN2,BN3,
!     1  BN4,BN5
!
         CALL NWTOAW(MACNW(1,I),MACNW(2,I),MACNW(3,I),MACNW(4,I)&
         &,MACNW(5,I),AN1,AN2,AN3,AN4,AN5,BN1,BN2,BN3,BN4,BN5)
!
!       DEAL WITH THE DEFAULT VALUES
!       AAN1=','
         AAN2=','
         AAN3=','
         AAN4=','
         AAN5=','
         BBBN1=','
         BBBN2=','
         BBBN3=','
         BBBN4=','
         BBBN5=','
!
!       PRODUCE OUTPUT. WRITE NUMERIC WORDS EVEN IF ALL ZERO.
         IF(OUT.EQ.8.OR.OUT.EQ.9.OR.OUT.EQ.10.OR.OUT.EQ.97) THEN
            LINE2=MACCW(I)//CM
            IF(MACSTA(12,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN1
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN1//CM
            END IF
            IF(MACSTA(13,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN2
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN2//CM
            END IF
            IF(MACSTA(14,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN3
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN3//CM
            END IF
            IF(MACSTA(15,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN4
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN4//CM
            END IF
            IF(MACSTA(16,I).EQ.1) THEN
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AAN5
            ELSE
               CALL LTH140(LINE2,LENG)
               LINE2=LINE2(1:LENG)//AN5//CM
            END IF
         ELSE
            LINE1=MACCW(I)//CM
            IF(MACSTA(12,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN1
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN1//CM
            END IF
            IF(MACSTA(13,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN2
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN2//CM
            END IF
            IF(MACSTA(14,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN3
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN3//CM
            END IF
            IF(MACSTA(15,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN4
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN4//CM
            END IF
            IF(MACSTA(16,I).EQ.1) THEN
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BBBN5
            ELSE
               CALL LTH80(LINE1,LENG)
               LINE1=LINE1(1:LENG)//BN5//CM
            END IF
         END IF
      END IF
!       NOW CHECK FOR PRESENCE OF A STRING
      IF(MACSTA(6,I).EQ.1) THEN
         IF(OUT.EQ.8.OR.OUT.EQ.9.OR.OUT.EQ.10.OR.OUT.EQ.97) THEN
            LINE2=TRIM(LINE2)//TRIM(MACSTR(I))
         ELSE
            LINE1=TRIM(LINE1)//TRIM(MACSTR(I))
         END IF
      END IF
!
      IF(OUT.EQ.7.OR.OUT.EQ.6) THEN
         IF(LINE1.NE.BLIN80) CALL NO_BLANK(LINE1,79)
         IF(LINE1.NE.BLIN80) CALL NO_ZEROS(LINE1,79)
         IF(LINE1.NE.BLIN80) WRITE(OUTLYNE,*) TRIM(LINE1)
         IF(LINE1.NE.BLIN80) CALL SHOWIT(0)
      END IF
      IF(OUT.EQ.8.OR.OUT.EQ.9.OR.OUT.EQ.10.OR.OUT.EQ.97) THEN
         IF(LINE2.NE.BLIN13) CALL NO_BLANK(LINE2,140)
         IF(LINE2.NE.BLIN13) CALL NO_ZEROS(LINE2,139)
         IF(LINE2.NE.BLIN13) WRITE(OUTLYNE,*) TRIM(LINE2)
         IF(LINE2.NE.BLIN13) CALL SHOWIT(2)
      END IF
   END DO
!
   CALL CLOSE_FILE(20,1)
   RETURN
10 RETURN
END
SUBROUTINE NO_BLANK(STRING,N)
   INTEGER N,I,J
   CHARACTER STRING*(*)
!     STRIP LEADING BLANKS

   DO I=1,N
      IF(STRING(1:1).EQ.' ')&
      &STRING(1:N)=STRING(2:140)//' '
   END DO
!     IS LINE BLANK? IF YES RETURN
   IF(STRING(1:1).EQ.' ') RETURN
   J=2
   DO I=1,N
      IF(STRING(J:J+1).EQ.'  ') THEN
         STRING(1:N)=STRING(1:J)//STRING(J+2:N)//' '
      ELSE
         J=J+1
      END IF
   END DO
   J=2
   DO I=1,N-1
      IF(STRING(J:J+1).EQ.' ,') THEN
         STRING(1:N)=STRING(1:J-1)//STRING(J+1:N)//' '
      ELSE
         J=J+1
      END IF
   END DO
   J=2
   DO I=1,N-2
      IF(STRING(J:J+1).EQ.', ') THEN
         STRING(1:N)=STRING(1:J)//STRING(J+2:N)//' '
      ELSE
         J=J+1
      END IF
   END DO
   RETURN
END
SUBROUTINE ONE_BLANK(STRING,N)
   INTEGER N,I,J
   CHARACTER STRING*(*)
!     STRIP LEADING BLANKS

   DO I=1,N
      IF(STRING(1:1).EQ.' ')&
      &STRING(1:N)=STRING(2:140)//' '
   END DO
!     IS LINE BLANK? IF YES RETURN
   IF(STRING(1:1).EQ.' ') RETURN
   J=2
   DO I=1,N
      IF(STRING(J:J+1).EQ.'  ') THEN
         STRING(1:N)=STRING(1:J-1)//STRING(J+1:N)//' '
      ELSE
         J=J+1
      END IF
   END DO
   RETURN
END
SUBROUTINE NO_ZEROS(STRING,N)
   use DATMAI
   INTEGER N,I,J
   CHARACTER STRING*(*)
!     STRIP LEADING BLANKS

20 CONTINUE
   J=1
   DO I=1,N
      IF(N.EQ.79.AND.J.GE.77) GO TO 30
      IF(N.EQ.139.AND.J.GE.137) GO TO 30
      IF(STRING(J:J+2).EQ.'00D'.OR.STRING(J:J+2).EQ.'00E'.OR.&
      &STRING(J:J+2).EQ.'00,'.OR.STRING(J:J+2).EQ.'00 ') THEN
         STRING(1:N)=STRING(1:J-1)//STRING(J+1:N)//' '
         IF(STRING(J:J+1).EQ.' 0')&
         &STRING(1:N)=STRING(1:J-1)//STRING(J+1:N)//' '
         GO TO 20
      ELSE
         J=J+1
         IF(N.EQ.79.AND.J.GE.77) GO TO 30
         IF(N.EQ.139.AND.J.GE.137) GO TO 30
      END IF
   END DO
30 CONTINUE
   RETURN
END
! SUB NEWMAC.FOR
SUBROUTINE NEWMAC
!
   use DATMAI
   IMPLICIT NONE
!
   CHARACTER STAMP*20
!
   LOGICAL EXISJK
!
   INTEGER &
   &I,J,K,MAXLL,KKKEY,CCODE
!
   COMMON/CODECC/CCODE,KKKEY
!
   REAL*8 DWW1
!
!       THIS IS THE SUBROUTINE RESPONSIBLE FOR CREATION OF A
!       NEW MACRO. SUBROUTINE EOM IS THE ONLY WAY BACK TO THE
!       MAIN PROGRAM CMD LEVEL. WQ IS THE NEW MACRO NAME.
!       MACRO NAMES MUST NOT BE THE SAME AS PROGRAM COMMAND
!       NAMES AS IF THEY ARE, THE MACRO WILL NEVER BE ABLE
!       TO BE EXECUTED OR DELETED WITHOUT EXTERNAL INTERVENTION.
!
   CHARACTER MNAME*8

   INTEGER M1,M2,M3,NEXTM3
!
!       MACCW,MACQW,AND MACSTR STORE THE COMMAND,QUALIFIER,AND
!       ALPHA-NUMERIC STRING.
!
!       MACNW STORES THE 5 NUMERIC WORDS
!
   INTEGER IMAC
!
!       MACSTA STORE UP TO 20 STATUS INDICATORS
!
!       ALL STORAGE IS FOR MACROS UP TO MAXLL LINES
!
   COMMON/IMAC/IMAC
!
   INCLUDE 'DATMAC.INC'
!
!
   call does_mac_dat_exist(EXISJK)

   IF(.NOT.EXISJK) THEN

      F1=1
      F2=0
   END IF

!
   CALL MAXLNN(WQ,MAXLL,MAXLIN)
   I=(MAXLL-1)
   MACCW(0:I)=BB
   MACQW(0:I)=BB
   MACSTR(0:I)=AA//AA//AA//AA
   MACSTA(1:20,0:I)=0
   MACNW(1:5,0:I)=0.0D0

!       OPEN UNIT 20 FOR I/O
!
!       CHECK FOR A BLANK MACRO NAME, IF FOUND SEND MESSAGE
!       AND RETURN
   IF(WQ.EQ.'        ') THEN
      OUTLYNE=&
      &'"MACRO" MUST BE FOLLOWED BY A NON-BLANK MACRO NAME'
      CALL SHOWIT(1)
      OUTLYNE='RETURNED TO CMD LEVEL'
      CALL SHOWIT(1)
      F1=1
      F2=0
      CALL MACFAL
      RETURN
   ELSE
!       NOT A BLANK NAME
   END IF
!
!       ***************************************************************
   OPEN(UNIT=20,ACCESS='DIRECT',FILE=LIBMAC//'MAC.DAT',FORM=&
   &'UNFORMATTED',RECL=(40*NRECL),STATUS='UNKNOWN')
   J=0
   M3=0
   DO 10 I=1,MAXMAC
      READ(UNIT=20,REC=I,ERR=10) MNAME,M1,M2,NEXTM3,STAMP
      M3=M3+NEXTM3
      IF(WQ.EQ.MNAME) J=J+1
10 CONTINUE
!               CLOSE UNIT 20
   CALL CLOSE_FILE(20,1)
   IF(J.GT.0) THEN
!     OVERWRITE
      F16=1
      F1=1
      F2=0
      CCODE=1
      KKKEY=M1
      CALL MACFAL
      RETURN
   ELSE
   END IF
!       CHECK FOR NO ROOM IN THE DIRECTORY
   IF(M3.GE.MAXMAC) THEN
!       NO MORE ROOM
      OUTLYNE='MACRO DIRECTORY FULL. RETURNED TO CMD LEVEL'
      CALL SHOWIT(1)
   END IF
!
   IMAC=0
!
!       IMAC IS THE COUNTER FOR MACRO LINES. IT IS INITIALIZED
!       HERE TO 0 AND IS PASSED IN MACRO COMMON
!
!       DURING MACRO CREATION, THE CMD LEVEL IS DISABLED
!       AND THE MACRO INPUT LEVEL IN ENABLED.
!
!       DURING MACRO INPUT, EACH PROGRAM INSTRUCTION
!       ENTERED IS PROCESSED BY SUBROUTINE PROCESS
!       THEN PASSED BY SUBROUTINE CONTROL TO SUBROUTINE
!       MACIN WHERE EACH COMMAND IS
!       STORED IN THE MACRO CREATION ARRAYS.
!       WHEN EOM IS ENTERED, THE MACRO CREATION ARRAYS
!       ARE WRITTEN TO THE MACRO FILE MAC.DAT.
!       THE RECORDS ARE WRITTEN DIRECT AND UNFORMATTED.
!       A DIRECTORY OF MACRO NAMES IS WRITTEN IN THE FIRST
!       MAXMAC ENRTIES OF THIS MACRO FILE.
!
   DWW1=W1
   CALL MACIN(DWW1)
   RETURN
END
SUBROUTINE MACARRAY_LOAD(NUMINLIST, MACARRAY)
   use GLOBALS
   use DATMAI
   IMPLICIT NONE
!       ARRAY TO SUPPORT MACRO GUI


   CHARACTER(len=8), dimension(1024), intent(inout) :: MACARRAY
   INTEGER, intent(inout) :: NUMINLIST

   INCLUDE 'DATMAC.INC'
   INTEGER OLDOUT,OLDIN,I,J, rec9
   CHARACTER*132 LINE
   MACARRAY(1:1024)='        '
   CALL CLOSE_FILE(9,0)
   OLDOUT=OUT
   OLDIN=IN
   SAVE_KDP(24)=SAVEINPT(24)
   INPUT='OUT ED'
   CALL PROCES
   INPUT='MFLN'
   CALL PROCES
   REST_KDP(24)=RESTINPT(24)
   OUT=OLDOUT
   IN=OLDIN
   CALL CLOSE_FILE(9,1)
   inquire(iolength=rec9) LINE
   OPEN(UNIT=9,ACCESS='SEQUENTIAL',BLANK='NULL'&
   &,FORM='FORMATTED',RECL=rec9,&
   &FILE=trim(basePath)//'EDITTEXT.DAT',STATUS='UNKNOWN')
   REWIND(9)
   I=1
1  READ(9,25,ERR=90,END=90) LINE
   !PRINT *, "LINE is ", LINE
   IF(I.GE.3) MACARRAY(I-2)=LINE(1:8)
   I=I+1
   GO TO 1
90 CONTINUE
   CALL CLOSE_FILE(9,0)
25 FORMAT(A132)
   NUMINLIST=I-2
   !PRINT *, "MACARRAY IS ", MACARRAY
   !PRINT *, "NUMINLIST IS ", NUMINLIST
   RETURN
END

subroutine does_mac_dat_exist(boolResult)
   use DATMAI

   logical :: boolResult


   INQUIRE(FILE=trim(LIBMAC)//'MAC.DAT',EXIST=boolResult)
   IF(.NOT.boolResult) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MACRO FILES DO NOT YET EXIST'//'\n'//&
      & 'TO INITIALIZE IT, USE "IMF" AND "PROCEED"', 1)
      RETURN
   END IF

end subroutine
