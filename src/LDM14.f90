...
!
      IF(SURF.EQ.0) THEN
         OUTLYNE=&
         &'REFLECTIVE SURFACE NOT VALID ON OBJECT SURFACE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
...
!     WC IS 'IDEAL'
   ...
   IF(WC.EQ.'INDEX'.OR.WC.EQ.'VNUM'.OR.WC.EQ.'DPART') THEN
      ...
      ELSE
         IF((SURF-1).LT.0) THEN
            OUTLYNE=&
            &'"INDEX", "VNUM" AND "DPART"'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'COMMANDS WORK ON THE PREVIOUS SURFACE IN LENS'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'INPUT MODE BUT THERE ARE NO SURFACES AS YET'
            OUTLYNE='NO VALUE TO DISPLAY'
...
!
   ...
   IF(WC.EQ.'PCW') THEN
      VA1=1
      VAL=&
      &'CURRENT PRIMARY WAVELENGTH PAIR NUMBERS ARE:'
      V1=1
      VALUE1=SYSTEM(7)
      V2=1
      VALUE2=SYSTEM(8)
   ...
   IF(WC.EQ.'SCW') THEN
      VA1=1
      VAL=&
      &'CURRENT SECONDARY WAVELENGTH PAIR NUMBERS ARE:'
      V1=1
      VALUE1=SYSTEM(9)
      V1=2
      VALUE1=SYSTEM(10)
...
26 FORMAT(&
   &'NOTE: VALUE #6 FOR "CLAP" OR "COBS" IS THE "TILT" VALUE')
...
SUBROUTINE ROO
   ...
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"ROO" SETS UP A SINGLE SURFACE OPTICAL ROOF'
      CALL SHOWIT(1)
      RETURN
   END IF
...
SUBROUTINE CCR
   ...
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"CCR" SETS UP A SINGLE SURFACE CORNER CUBE'
      CALL SHOWIT(1)
      RETURN
   END IF
...
!     THIS DISPLAYS ROOF AND CCR DATA IN THE SEQUENTIAL DATABASE.
   ...
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"SHOWNSS" LISTS ALL ROOFS AND CORNER CUBES IN'
      CALL SHOWIT(1)
      OUTLYNE=&
...
!
   ...
   IF(SQ.EQ.0) THEN
      IF(WC.EQ.'GLASS')&
      &OUTLYNE='"GLASS" REQUIRES EXPLICIT QUALIFIER INPUT'
      IF(WC.EQ.'MODEL')&
      &OUTLYNE='"MODEL" REQUIRES EXPLICIT QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
SUBROUTINE RAYERROR
   ...
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"RAYERROR" SETS A RAYDOM RAY/SURFACE ANGLE ERROR'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
