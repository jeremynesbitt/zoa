...
!
   IF(SQ.EQ.1.OR.SST.EQ.1)THEN
      ...
      & '"COATING" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
!
   ELSE
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"COATING" TAKES NO NUMERIC WORD #2 TO #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
!
   ELSE
   ...
   IF(S1.EQ.0)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"COATING" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
!
   ELSE
   ...
   IF(W1.LT.0.0D0.OR.W1.GT.1000.0D0)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"COATING" REQUIRES NUMERIC WORD #1 INPUT'//'\n'//&
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"REFL","REFLTIR" AND "REFLTIRO" TAKE NO EXPLICIT INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
      IF(SURF.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'OBJECT SURFACE CAN NOT BE A REFLECTOR'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(SURF.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL('REFLECTIVE SURFACE NOT VALID ON OBJECT SURFACE', 1)
         RETURN
      END IF
!
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.SN.EQ.1)THEN
      CALL REPORT_ERROR_AND_FAIL('"AIR" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
!
   ELSE
   END IF
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.SN.EQ.1)THEN
      CALL REPORT_ERROR_AND_FAIL('"PERFECT" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
!
   ELSE
   END IF
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"IDEAL" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"IDEAL" ONLY TAKES NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
   IF(S1.EQ.0)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"IDEAL" REQUIRES EXPLICIT WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
   IF(W1.EQ.0.0D0)THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"IDEAL" REQUIRES NON-ZERO NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
...
!
   ...
   use mod_surface
   use DATMAI
   use mod_system, only: sys_astop, sys_autofunc, sys_bdx, sys_bdy, &
      & sys_fno_val_set, sys_fno_val_x, sys_fno_val_y, sys_mode, sys_na_set, &
      & sys_naox, sys_naoy, sys_pxim, sys_pxim_fang_set, sys_pyim, &
      & sys_pyim_fang_set, sys_ref_orient, sys_ref_surf, sys_rxim, &
      & sys_rxim_fang_set, sys_ryim, sys_ryim_fang_set, sys_sax, sys_say, &
      & sys_sax_float, sys_say_float, sys_scx, &
   ...
   IF(WC.NE.'IDEAL') THEN
      ...
      IF(WC.EQ.'N1'.OR.WC.EQ.'N2'.OR.WC.EQ.'N3'.OR.WC.EQ.'N4'.OR.WC.EQ.'N5'.OR.WC.EQ.'N6'.OR.WC.EQ.'N7'.OR.WC.EQ.'N8'.OR.WC.EQ.'N9'.OR.WC.EQ.'N10') THEN
         IF((SURF-1).LT.0) THEN
            ...
            CALL REPORT_ERROR_AND_FAIL('NO VALUE TO DISPLAY', 1)
            RETURN
         END IF
         IF(WC.EQ.'N1') THEN
            VA1=1
   ...
   IF(WC.EQ.'MAGX') THEN
      DO KK=I,J-1
         IF(surf_solve_flag(KK).NE.0.0D0) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & 'ALL SOLVES FROM'//'\n'//&
            & 'SURFACE (I) TO (J-1) MUST BE REMOVED'//'\n'//&
            & 'MAGNIFICATION ADJUSTMENT NOT PERFORMED'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END DO
...
!     NOW SET DISTANCES
      ...
      IF(EFLX.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'EFLX IS ZERO'//'\n'//&
         & 'MAGNIFICATION ADJUSTMENT NOT PERFORMED', 1)
         RETURN
      END IF
      OBJ=((MAG+1.0D0)*EFLX)/MAG
      IF(OBJ.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'OBJECT DISTANCE IS ZERO'//'\n'//&
         & 'MAGNIFICATION ADJUSTMENT NOT PERFORMED', 1)
         RETURN
      END IF
      ...
      IF(ARG.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'IMAGE DISTANCE DISTANCE IS INFINITE'//'\n'//&
         & 'MAGNIFICATION ADJUSTMENT NOT PERFORMED', 1)
         RETURN
      END IF
      IMG=1.0D0/ARG
...
SUBROUTINE ROO
   ...
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"ROO" TAKES NO QUALIFIER, STRING OR NUMERIC WORD #3 TO #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.LE.0.0D0.OR.S1.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"ROO" REQUIRES AN EXPLICIT, POSITIVE APEX HEIGHT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
SUBROUTINE CCR
   ...
   IF(S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CCR" TAKES NO NUMERIC WORK #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.LE.0.0D0.OR.S1.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CCR" REQUIRES AN EXPLICIT, POSITIVE APEX HEIGHT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF2.EQ.1) W2=0.0D0
...
!     THIS DISPLAYS ROOF AND CCR DATA IN THE SEQUENTIAL DATABASE.
   ...
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"NSSRTG" TAKES NO NUMERIC, QUALIFIER OR STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   NONSS=.TRUE.
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "DUMOUT" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "DUMOUT" TAKES EITHER QUALIFIER OR'//'\n'//&
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"DUMOUT" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO NODUM DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "FOOTBLOK" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "FOOTBLOK" TAKES EITHER QUALIFIER OR'//'\n'//&
      & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"FOOTBLOK" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO FOOTBLOK DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "PIVAXIS" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "PIVAXIS" TAKES EITHER QUALIFIER OR'//'\n'//&
      & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"PIVAXIS" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO PIVAXIS DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "SURTYPE" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "SURTYPE" TAKES EITHER QUALIFIER OR'//'\n'//&
      & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"SURTYPE" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
...
!
   IF(WC.EQ.'MODEL'.AND.S4.EQ.1.OR.WC.EQ.'MODEL'.AND.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MODEL" TAKES NO NUMERIC WORD #3 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 0)
      RETURN
   END IF
   IF(WC.NE.'MODEL') THEN
   ...
   IF(WC.EQ.'GLASS') THEN
      IF(W1.EQ.0.0D0.OR.W2.EQ.0.0D0.OR.W3.EQ.0.0D0.OR.W4.EQ.0.0D0 .OR.W5.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GLASS" TAKES NO ZERO VALUES FOR REFRACTIVE INDICES'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WC.NE.'MODEL') THEN
      IF(W1.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MODEL" TAKES NO ZERO VALUES FOR THE "INDEX" VALUE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   ...
   IF(WC.EQ.'MODEL') THEN
      IF(W1.LE.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MODEL" REQUIRES A NON-ZERO, POSITIVE INPUT INDEX VALUE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
