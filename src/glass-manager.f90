...
contains
 ...
 function calcSpecialIndex(self, lambda) result(n)
  ...
  case('ACRYLIC')
    ...
    X(1:NUM) = (/0.36501D0,0.40466D0,0.43484D0 ,0.47999D0,0.48613D0,0.54607D0,0.58756D0,0.58929D0,0.64385D0 ,0.65627D0,0.70652D0,0.85211D0,1.01398D0/)

    Y(1:NUM) = (/1.513613D0,1.506607D0,1.502557D0 ,1.498258D0,1.497760D0,1.493795D0,1.491757D0,1.491681D0 ,1.489603D0,1.489201D0,1.487552D0,1.484965D0,1.483115D0/)   
  ...
  case('POLYSTYRENE')
    ...
    Y(1:NUM) = (/1.643126D0,1.625341D0,1.615466D0 ,1.605241D0,1.604079D0,1.595010D0,1.590481D0,1.590315D0 ,1.585808D0,1.584949D0,1.581954D0,1.576196D0,1.572553D0/)    
  ...
  case('POLYCARBONATE')
    ...
    Y(1:NUM) = (/1.643231D0,1.622447D0,1.611519D0 ,1.600654D0,1.599439D0,1.590081D0,1.585470D0,1.585302D0 ,1.580734D0,1.579864D0,1.576831D0,1.570981D0,1.567248D0/)  
  ...
  case('SAN')
    ...
    Y(1:NUM) = (/1.612490D0,1.597075D0,1.588640D0 ,1.579985D0,1.579000D0,1.571300D0,1.567400D0,1.567298D0 ,1.563438D0,1.562700D0,1.560119D0,1.555108D0,1.551870D0/)
  ...
  case('SILICON')
    if(lambda == 0.0) then
      ...
      n = 3.41696D0+(0.138497D0/((lambda**2)-0.028D0)) +(0.013924D0/(((lambda**2)-0.028D0)**2))-(0.0000209D0*(lambda**2)) +(0.000000148D0*(lambda**4))
    end if
  ...
  case('SILICA')
    if(lambda == 0.0) then
      ...
      n = DSQRT(1.0D0+ ((0.6961663*(lambda**2))/((lambda**2)-((0.0684043)**2)))+ ((0.4079426*(lambda**2))/((lambda**2)-((0.1162414)**2)))+ ((0.8974794*(lambda**2))/((lambda**2)-((9.896161)**2))) )
    end if    
...
!     DIAMOND INTERPOLATION FORMULA
      n= 2.37837D0+ ((1.18897E-2)*(1.0D0/((lambda**2)-0.028D0)))+ ((-1.0083E-4)*((1.0D0/((lambda**2)-0.028D0))**2))+ ((-2.3676E-5)*(lambda**2))+ ((3.24263E-8)*(lambda**4))    
    end if      
...
!     LiF INTERPOLATION FORMULA
      n = 1.38761D0+(0.001796D0/((lambda**2)-0.028D0)) +(0.000041D0/(((lambda**2)-0.028D0)**2))-(0.0023045D0*(lambda**2)) -(0.00000557D0*(lambda**4))
    end if         
...
!     
      n =  DSQRT( 2.361323D0-(3.11497D-4*(lambda**2))-(5.8613D-8*(lambda**4))+ (0.007676D0/(lambda**2))+(0.0156569D0/((lambda**2)-0.0324D0)))
    end if         
...
!     
      n =  DSQRT( ((1.023798D0*(lambda**2))/((lambda**2)-(0.00377588D0)))+ ((1.058264D0*(lambda**2))/((lambda**2)-(0.01225440D0)))+ ((5.280792D0*(lambda**2))/((lambda**2)-(321.361600D0)))+1.0D0)
    end if         
...
!     DATA FOR H2O
      ...
      X(1:NUM) = (/0.1829D0,0.20255D0,0.25020D0,0.30822D0,  10.35871D0,0.40466D0,0.44715D0,0.50157D0,0.54607D0,0.58926D0,  20.65628D0,0.70652D0,0.76820D0,0.808D0,0.871D0,0.943D0,1.028D0,  31.130D0,1.256D0,1.617D0,1.968D0/)

      Y(1:NUM) = (/1.46379D0,1.41993D0,1.37734D0,1.35671D0,  11.34795D0,1.342724D0,1.339423D0,1.336363D0,1.334466D0,1.332988D0,  21.331151D0,1.330019D0,1.32890D0,1.3286D0,1.3273D0,1.3262D0,  31.3250D0,1.3234D0,1.3215D0,1.3149D0,1.3078D0/)
 ...
 function calcPN(self, lambda) result(n)
  ...
  real(kind=real64) :: n
   ...
   n = DSQRT(self%coeffs(1)+    (self%coeffs(2)*(lambda**2))+   (self%coeffs(3)*(1.0D0/(lambda**2)))+    (self%coeffs(4)*(1.0D0/(lambda**4)))+    (self%coeffs(5)*(1.0D0/(lambda**6)))+    (self%coeffs(6)*(1.0D0/(lambda**8))))
 ...
 function calcPNSC(self, lambda) result(n)
  ...
  n = DSQRT(((self%coeffs(1)*(lambda**2))/((lambda**2)-self%coeffs(4)))+   ((self%coeffs(2)*(lambda**2))/((lambda**2)-self%coeffs(5)))+   ((self%coeffs(3)*(lambda**2))/((lambda**2)-self%coeffs(6)))+1.0D0)
