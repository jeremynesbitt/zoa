...
!
   ...
   RETURN
666 CONTINUE
   WRITE(OUTLYNE,*)'An error was encountered in closing the file:',TRIM(FILE_NAME),' with status = "DELETE"'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)'Try deleting the file from the operating system level',' before proceeding.'
   CALL SHOWIT(1)
   CALL MACFAL
   RETURN
777 CONTINUE
   WRITE(OUTLYNE,*)'An error was encountered in closing the file:',TRIM(FILE_NAME),' with status = "KEEP"'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)'Try deleting the file from the operating system level',' before proceeding.'
   CALL SHOWIT(1)
   CALL MACFAL
...
100 FORMAT('ELAPSED TIME SINCE TIMER RESET = (',G11.4,') SECONDS')
   CALL SHOWIT(0)
   RETURN
END
SUBROUTINE SETTIM
...
!       CHECK SYNTAX
   IF(STI.EQ.1) THEN
      IF(NODRAW)OUTLYNE='DRAWING TO THE SCREEN IS CURRENTLY TURNED "OFF"'
      IF(.NOT.NODRAW)OUTLYNE='DRAWING TO THE SCREEN IS CURRENTLY TURNED "ON"'
      CALL SHOWIT(1)
      RETURN
   ELSE
   END IF
...
!       CHECK SYNTAX
   IF(STI.EQ.1) THEN
      IF(NOWMF)OUTLYNE='DRAWING TO PRG.WMF IS CURENTLY TURNED "OFF"'
      IF(.NOT.NOWMF)OUTLYNE='DRAWING TO PRG.WMF IS CURRENTLY TURNED "ON"'
      CALL SHOWIT(1)
      RETURN
   ELSE
   END IF
...
!
   CHARACTER C*1,WS11*80,BL140*140,GRFILN*12,CEELINE*14 ,DRWNAM*11,MYSTR1*22,MYSTR2*20,MYSTR3*21,JK_TAG*2
!
...
!     MESSAGE THAT NO FILE EXISTS TO DRAW
      ...
      RETURN
   END IF
   IF(JK_TAG.EQ.'01') OUTLYNE='MONOCHROME GRAPHICS BEING SENT TO WINDOWS PRINT MANAGER...'
   IF(JK_TAG.EQ.'02') OUTLYNE='COLOR GRAPHICS BEING SENT TO WINDOWS PRINT MANAGER...'
   IF(JK_TAG.EQ.'03') OUTLYNE='MONOCHROME GRAPHICS BEING SENT WINDOWS METAFILE '//GRFILN
   IF(JK_TAG.EQ.'13') OUTLYNE='MONOCHROME GRAPHICS BEING SENT ENHANCED WINDOWS METAFILE '//GRFILN
   IF(JK_TAG.EQ.'23') OUTLYNE='MONOCHROME GRAPHICS BEING SENT ALDUS PLACEABLE WINDOWS'//' METAFILE '//GRFILN
   IF(JK_TAG.EQ.'04') OUTLYNE='COLOR GRAPHICS BEING SENT WINDOWS METAFILE '//GRFILN
   IF(JK_TAG.EQ.'14') OUTLYNE='COLOR GRAPHICS BEING SENT ENHANCED WINDOWS METAFILE '//GRFILN
   IF(JK_TAG.EQ.'24') OUTLYNE='COLOR GRAPHICS BEING SENT ALDUS PLACEABLE WINDOWS'//' METAFILE '//GRFILN
   IF(JK_TAG.EQ.'06') OUTLYNE='MONOCHROME GRAPHICS BEING SENT TO PCX FILE '//GRFILN
   IF(JK_TAG.EQ.'07') OUTLYNE='COLOR GRAPHICS BEING SENT TO PCX FILE '//GRFILN
   IF(JK_TAG.EQ.'08') OUTLYNE='MONOCHROME GRAPHICS BEING SENT TO BMP FILE '//GRFILN
   IF(JK_TAG.EQ.'09') OUTLYNE='COLOR GRAPHICS BEING SENT TO BMP FILE '//GRFILN
   IF(JK_TAG.EQ.'18') OUTLYNE='MONOCHROME GRAPHICS BEING SENT TO COMPRESSED BMP FILE '//GRFILN
   IF(JK_TAG.EQ.'19') OUTLYNE='COLOR GRAPHICS BEING SENT TO COMPRESSED BMP FILE '//GRFILN
   IF(JK_TAG.EQ.'31') OUTLYNE='MONOCHROME GRAPHICS BEING SENT TO EPS FILE '//GRFILN
   IF(JK_TAG.EQ.'32') OUTLYNE='COLOR GRAPHICS BEING SENT TO EPS FILE '//GRFILN
   CALL SHOWIT(1)
   IF(OPEN28) CALL MY_ENDPLT
   CALL CLOSE_FILE(28,1)
...
5  CONTINUE
   ...
   PRINT *, CEELINE
   IF(EXIS28.AND..NOT.OPEN28)CALL RUN_WPLOT(CEELINE)
...
!       PROCEED
   END IF
!       ***************************************************************
   OPEN(UNIT=24,ACCESS='DIRECT',FILE=LIBPLO//'PLIB.DAT',FORM='UNFORMATTED',RECL=(90*NRECL),STATUS='UNKNOWN')
   J=0
   K=0
   DO 10 I=1,999
...
!
   DO I=1,10
      IF(I.EQ.1) THEN
         WAVER=REAL(sys_wavelength(1))
         WAITR=REAL(sys_wl_weight(1))
      END IF
      IF(I.EQ.2) THEN
         WAVER=REAL(sys_wavelength(2))
         WAITR=REAL(sys_wl_weight(2))
      END IF
      IF(I.EQ.3) THEN
         WAVER=REAL(sys_wavelength(3))
         WAITR=REAL(sys_wl_weight(3))
      END IF
      IF(I.EQ.4) THEN
         WAVER=REAL(sys_wavelength(4))
         WAITR=REAL(sys_wl_weight(4))
      END IF
      IF(I.EQ.5) THEN
         WAVER=REAL(sys_wavelength(5))
         WAITR=REAL(sys_wl_weight(5))
      END IF
      IF(I.EQ.6) THEN
         WAVER=REAL(sys_wavelength(6))
         WAITR=REAL(sys_wl_weight(6))
      END IF
      IF(I.EQ.7) THEN
         WAVER=REAL(sys_wavelength(7))
         WAITR=REAL(sys_wl_weight(7))
      END IF
      IF(I.EQ.8) THEN
         WAVER=REAL(sys_wavelength(8))
         WAITR=REAL(sys_wl_weight(8))
      END IF
      IF(I.EQ.9) THEN
         WAVER=REAL(sys_wavelength(9))
         WAITR=REAL(sys_wl_weight(9))
      END IF
      IF(I.EQ.10) THEN
         WAVER=REAL(sys_wavelength(10))
         WAITR=REAL(sys_wl_weight(10))
      END IF
...
!     AND WRITE THE STRING
      ...
      IF(WAVER.NE.0.0) THEN
         IF(WAVER.GT.99.9) THEN
            ...
            CALL MY_JUSTSTRING(IIX+1300,IIY-IIIY,NNTT3(1:1),NT1ANG,NT1SIZ,3,3)
            CALL MY_SETFONT(1,0)
            CALL MY_SETCHARASPECT(1.25,1.25)
         END IF
         IF(WAVER.LE.99.9) THEN
            ...
            CALL MY_JUSTSTRING(IIX+1200,IIY-IIIY,NNTT3(1:1),NT1ANG,NT1SIZ,3,3)
            CALL MY_SETFONT(1,0)
            CALL MY_SETCHARASPECT(1.25,1.25)
         END IF
      END IF
      CALL MY_PLOT(IIX+1500,IIY-IIIY,0,0,0,10000,0,7000)
      CALL MY_JUSTSTRING(IIX+1500,IIY-IIIY,NNTT2(1:16),NT1ANG,NT1SIZ,3,3)
      CALL MY_SETCHARASPECT(1.5,1.5)
!
   END DO
!
...
!
   ...
   DO N=1,999
      ...
      IF(EXISJK) THEN
         IF(.NOT.OPENJK) OPEN(UNIT=77 ,FILE=LIBPLO//FN,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=(NRECL*42),STATUS='UNKNOWN')
...
!       CHECK SYNTAX
      IF(SST.EQ.1) THEN
         OUTLYNE='"PLOT SYMBOL" TAKES NO STRING INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(S5.EQ.1) THEN
         OUTLYNE='"PLOT SYMBOL" TAKES NO NUMERIC WORD #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
874      FORMAT('CURRENT SYMBOL IS ',A15,1X,'SYMBOL NUMBER = ',I1)
      ...
      IF(INT(W1).LT.1.OR.INT(W1).GT.10) THEN
         OUTLYNE='VALID SYMBOL NUMBERS RANGE FROM 1 TO 10'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(INT(W2).LT.1.OR.INT(W2).GT.9) THEN
         OUTLYNE='VALID SYMBOL SIZES RANGE FROM 1 TO 9'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(DF3.EQ.1.OR.DF4.EQ.1) THEN
         OUTLYNE='X AND Y-COORDINATES MUST BE ENTERED IN NUMERIC WORDS 3 AND 4'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     NO QUERRY
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1) THEN
         OUTLYNE='"COLORSET" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(WQ.NE.'RESET') THEN
         IF(S1.EQ.0.OR.SQ.EQ.0) THEN
            OUTLYNE='"COLORSET" REQUIRES EXPLICIT QUALIFIER AND'
            CALL SHOWIT(1)
            OUTLYNE='EXPLICIT NUMERIC WORD #1 INPUT'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
      ...
      ELSE
      END IF
      IF(WQ.NE.'RAYS'.AND.WQ.NE.'CLAP'.AND.WQ.NE.'COBS'.AND.WQ.NE.'EDGE'.AND.WQ.NE.'PROF'.AND.WQ.NE.'AXIS'.AND.WQ.NE.'GBAC'.AND.WQ.NE.'WAV1'.AND.WQ.NE.'WAV2'.AND.WQ.NE.'WAV3'.AND.WQ.NE.'WAV4'.AND.WQ.NE.'WAV5'.AND.WQ.NE.'FRAM'.AND.WQ.NE.'LABL'.AND.WQ.NE.'SPEC'.AND.WQ.NE.'PEN'.AND.WQ.NE.'WAV6'.AND.WQ.NE.'WAV7'.AND.WQ.NE.'WAV8'.AND.WQ.NE.'WAV9'.AND.WQ.NE.'WAV10'.AND.WQ.NE.'FILL')THEN
         OUTLYNE='INVALID QUALIFIER USED WITH "COLORSET"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(INT(W1).LT.-1.OR.INT(W1).GT.16) THEN
         OUTLYNE='NUMERIC COLOR VALUES MUST RANGE FROM -1 TO 15'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      IF(WQ.EQ.'GBAC')  THEN
         IF(W1.LT.-1.0D0.OR.W1.GT.15.0D0) THEN
            OUTLYNE='"COLORSET GBAC" ONLY USES COLOR NUMBERS -1 TO 15'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!     IS SQ=1 ?
      IF(SQ.EQ.1) THEN
         IF(WQ.NE.'RAYS'.AND.WQ.NE.'CLAP'.AND.WQ.NE.'COBS'.AND.WQ.NE.'EDGE'.AND.WQ.NE.'PROF'.AND.WQ.NE.'AXIS'.AND.WQ.NE.'GBAC'.AND.WQ.NE.'WAV1'.AND.WQ.NE.'WAV2'.AND.WQ.NE.'WAV3'.AND.WQ.NE.'WAV4'.AND.WQ.NE.'WAV5'.AND.WQ.NE.'FRAM'.AND.WQ.NE.'LABL'.AND.WQ.NE.'WAV6'.AND.WQ.NE.'WAV7'.AND.WQ.NE.'WAV8'.AND.WQ.NE.'WAV9'.AND.WQ.NE.'WAV10'.AND.WQ.NE.'SPEC'.AND.WQ.NE.'PEN'.AND.WQ.NE.'FILL')THEN
            OUTLYNE='INVALID QUALIFIER USED WITH "COLORSET" QUERRY'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!                       DEFINE VARIABLES
!
   CHARACTER DATA*80,WSS1*80,PPPLI*60,STRINGER*1,LLIP*80,DDATE*10,TTIME*8,AN*3,NEUTLINE*42 ,FN*10,NM*8,WSS*80,TTTIM*8,DDDAT*10 ,DRWNAM*11,JK_Fromfile*80,JK_Tofile*80
!
   COMMON/DRWTAG/DRWNAM
!
...
!
   REAL JJ1,JJ2
!
   INTEGER SING,LCNT,I,II,J,N,NIN,PFCOUNTER,I1,I2,I3,I4,I5,I6,I7,I8,ALLOERR,JJ, fNeut
!
...
!
   IF(WQ.EQ.'P') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PLIB P" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       INPUT OK
      END IF
      IF(W1.LT.1.0.AND.DF1.NE.1.OR.W1.GT.999.0.AND.DF1.NE.1) THEN
      ...
      ELSE
      END IF
      IF(W2.LT.2.0.AND.DF2.NE.1.OR.W2.GT.999.0.AND.DF2.NE.1) THEN
...
!     WQ.NOT.'PUT'
   ...
   IF(WQ.EQ.'PUT') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PLIB PUT" ONLY TAKES NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(WQ.EQ.'GET') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PLIB GET" ONLY TAKES NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(WQ.EQ.'DELK') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PLIB DEL" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
98    CONTINUE
      ...
      IF(J.GE.MAXNEUTRAL/2)CALL RESIZE_NEUT
...
!       DUMP THE LIBRARY FILE
            ...
            INQUIRE(FILE=LIBPLO//FN,EXIST=EXISJK)
            IF(EXISJK) THEN
               OPEN(UNIT=77 ,FILE=LIBPLO//FN,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=(NRECL*42),STATUS='UNKNOWN')
               CLOSE(77,STATUS='DELETE')
            ELSE
            END IF
...
!       DUMP THE LIBRARY FILE
            ...
            INQUIRE(FILE=LIBPLO//FN,EXIST=EXISJK)
            IF(EXISJK) THEN
               OPEN(UNIT=76 ,FILE=LIBPLO//FN,FORM='UNFORMATTED',ACCESS='DIRECT',RECL=(NRECL*42),STATUS='UNKNOWN')
...
!      when textView is c_null_ptr, i.e. the KDPDUMP sink).
      if (.not. ioConfig%dumpText) then
         call zoa_emit(trim(OUTLYNE),"black")
      end if
      !call pending_events()
   END IF
...
!     CHANGING CONFIGS, NO SCREEN OUTPUT
      IF(I.EQ.0.AND.OUT.EQ.6.OR.I.EQ.2.AND.OUT.EQ.6.OR.I.EQ.0.AND.OUT.EQ.69.OR.I.EQ.2.AND.OUT.EQ.69.OR.I.EQ.1.OR.I.EQ.4.OR.I.EQ.5.OR.I.EQ.10.AND.OUT.EQ.6 .OR.I.EQ.10.AND.OUT.EQ.6) RETURN
   END IF
!
   IF(I.EQ.20) THEN
...
SUBROUTINE OPEN_NEUTRAL_DAT(fileID)
   ...
   OPEN(fileID ,FILE=trim(basePath)//'NEUTRAL.DAT',FORM='UNFORMATTED',ACCESS='DIRECT',RECL=recLength,STATUS='UNKNOWN')
