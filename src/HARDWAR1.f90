...
!       CHECK SYNTAX
   ...
   IF(.NOT.DEVOK) THEN
      ...
      & 'INVALID QUALIFIER WORD USED WITH "GRAOUT"'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
...
!
   IF(DRWNAM(1:11).EQ.'NEUTRAL.DAT') THEN
      ...
      IF(.NOT.EXIS28.OR..NOT.PLEXIS) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO PLOTFILE EXISTS TO BE PLOTTED'//'\n'//&
         & 'NO PLOT WILL BE MADE', 1)
         RETURN
      ELSE
      END IF
   ELSE
...
!       ***************************************************************
   ...
   IF(.NOT.EXISJK) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT LIBRARY DOES NOT YET EXIST'//'\n'//&
      & 'TO INITIALIZE IT, USE "IPF" AND "PROCEED"', 1)
      RETURN
   ELSE
!       PROCEED
   END IF
...
!     CASE OF "COLORSET ?"
      IF(SQ.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL('"COLORSET" QUERRY REQUIRES A VALID QALIFIER WORD', 1)
         RETURN
      ELSE
!     SQ NOT 0
      END IF
...
!     FILE DOES NOT EXIST
      IF(.NOT.EXIS28) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO PLOTFILE EXISTS TO BE FILED'//'\n'//&
         & 'NO "LIB PUT" CAN BE DONE', 1)
         RETURN
      ELSE
      END IF
89    CONTINUE
...
!       IS NW1 DEFAULT
      IF(DF1.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLIB GET" REQUIRES EXPLICIT NUMERIC INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
      END IF
...
!       IS NW1 AND NW2 DEFAULT
      IF(DF1.EQ.1.AND.DF2.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLIB DEL" REQUIRES EXPLICIT NUMERIC INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
      END IF
...
!       IF YOU GOT HERE, LIBRARY WAS FULL
         CALL REPORT_ERROR_AND_FAIL('PLOT LIBRARY FULL, PLOT NOT FILED', 1)
         RETURN
11       CONTINUE
...
100         FORMAT('LIBRARY FILE NO. ',I3,' OCCUPIED')
            CALL SHOWIT(1)
            CALL REPORT_ERROR_AND_FAIL('PLOT NOT FILED', 1)
            RETURN
         ELSE
!       PROCEED, SPOT EMPTY
         END IF
