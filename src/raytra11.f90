...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"SAVERAY" SAVES EXISTING RAY AND REFERENCE RAY DATA FOR'
      CALL SHOWIT(1)
      OUTLYNE='LATER USE.'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       CHECK FOR INPUT
   ...
   IF(REFEXT.AND..NOT.RAYEXT) THEN
      OUTLYNE='ONLY REFERENCE RAY DATA EXISTS TO SAVE WITH "SAVERAY"'
      CALL SHOWIT(1)
   END IF
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"RESTRAY" RESTORES SAVED RAY AND REFERENCE RAY DATA'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!       CHECK FOR INPUT
   ...
   IF(SREFEXT.AND..NOT.SRAYEXT) THEN
      OUTLYNE='ONLY REFERENCE RAY DATA EXISTS TO RESTORE WITH "RESTRAY"'
      CALL SHOWIT(1)
   END IF
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"CLEARRAY" CLEARS SAVED RAY AND REFERENCE RAY DATA MEMORY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!       CHECK FOR INPUT
   ...
   IF(.NOT.SREFEXT.AND..NOT.SRAYEXT) THEN
      ...
      RETURN
   END IF
   OUTLYNE='SAVED RAY AND REFERENCE RAY DATA MEMORY CLEARED'
   CALL SHOWIT(1)
   CALL LASTRAY(3)
   RETURN
END
...
11    FORMAT('USEOLREF IS CURRENTLY TURNED "ON"')
   ...
   IF(WQ.EQ.'ON'.OR.WQ.EQ.'OFF') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE='"USEOLREF ("ON" OR "OFF")'//' TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       CHECK FOR STRING INPUT
   ...
   IF(WQ.EQ.'OFFSET') THEN
      IF(S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"AIMRAY OFFSET"'//' TAKES NO NUMERIC WORD #4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   IF(WQ.EQ.'ON'.OR.WQ.EQ.'OFF') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE='"AIMRAY ("ON" OR "OFF")'//' TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
14    FORMAT('CURRENT RAY AIMING Z-OFFSET = ',D23.15,' LENS UNITS')
   ...
   IF(WQ.EQ.'OFFSET') THEN
      ...
      call set_sys_aim_offset_x(W1)
      call set_sys_aim_offset_y(W2)
      call set_sys_aim_offset_z(W3)
...
!       CHECK FOR STRING INPUT
   ...
   IF(WQ.EQ.'ON'.OR.WQ.EQ.'OFF') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE='"AIMAPL ("ON" OR "OFF")'//' TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     STI=1
   END IF
   IF(STI.EQ.1) THEN
      IF(sys_aplanatic().EQ.0.0D0) WRITE(OUTLYNE,10)
      IF(sys_aplanatic().EQ.1.0D0) WRITE(OUTLYNE,11)
      CALL SHOWIT(0)
...
!
   IF(STI.EQ.1) THEN
      IF(sys_reverse().EQ.0.0D0) WRITE(OUTLYNE,10)
      IF(sys_reverse().EQ.1.0D0) WRITE(OUTLYNE,11)
      CALL SHOWIT(0)
...
11    FORMAT('"MACFAIL" IS CURRENTLY OFF')
      RETURN
   END IF
   IF(WQ.EQ.'OFF')MACFAILURE=.FALSE.
   IF(WQ.EQ.'ON')MACFAILURE=.TRUE.
   RETURN
END
! SUB OBJLEV.FOR
SUBROUTINE OBJLEV
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      ...
      RETURN
   END IF
   IF(WQ.EQ.'ON'.OR.WQ.EQ.'OFF'.OR.WQ.EQ.'WRITE'.OR.WQ.EQ.'SWRITE') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE='"RHIST ("ON" OR "OFF" OR "WRITE" OR "SWRITE")'//' TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   END IF
   IF(STI.EQ.0) THEN
      IF(WQ.NE.'ON'.AND.WQ.NE.'OFF'.AND.WQ.NE.'WRITE'.AND.WQ.NE.'SWRITE') THEN
         OUTLYNE='"ON", "YES", "NO", "OFF", "WRITE" AND "SWRITE"'//'ARE THE ONLY'
         CALL SHOWIT(1)
         OUTLYNE='VALID QUALIFIERS USED WITH "'//WC(1:6)//'"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
...
121   FORMAT(I10,I4,I10)
      OUTLYNE='LONG LIST RAY HISTORIES HAVE BEEN WRITTEN TO RAYHIST.DAT'
      CALL SHOWIT(1)
   ...
   IF(WQ.EQ.'SWRITE'.AND.RAY_HIST_FLAG) THEN
      ...
      DO K=1,RAY_HIST_NUM
         DO J=1,INT(sys_last_surf())
            WRITE(UNIT=48,FMT=13) J,K,RHIST(1,K,J),RHIST(2,K,J),RHIST(3,K,J),RHIST(9,K,J),RHIST(25,K,J),RHIST(39,K,J),RHIST(87,K,J),RHIST(88,K,J),RHIST(89,K,J),RHIST(90,K,J),RHIST(91,K,J),RHIST(92,K,J),RHIST(93,K,J)
         END DO
      END DO
      LONGHIST=.FALSE.
      SHORTHIST=.TRUE.
...
13    FORMAT(I3,I10,14(1X,D23.15))
      OUTLYNE='SHORT LIST RAY HISTORIES HAVE BEEN WRITTEN TO RAYHIST.DAT'
      CALL SHOWIT(1)
      CALL CLOSE_FILE(48,1)
   END IF
   RETURN
...
!       CREATE THE RHFOOT.DAT FILE FOR THE DESIGNATED SURFACE
   ...
   DO I=1,INTEN_NUM
      IF(TEMPINTEN(5,I).GT.0.0D0)WRITE(UNIT=103,FMT=*) TEMPINTEN(1:2,I)
   END DO
   CALL CLOSE_FILE(103,1)
!       NOW ALLOCATE THE ARRAYS USED TO PROCESS THE INTENSITIES
!
   ALLOCATE(INTEN_YARRAY(1:INTENNUM_PIXELS,1:INTENNUM_PIXELS),STAT=ALLOERR)
   ALLOCATE(INTEN_XARRAY(1:INTENNUM_PIXELS,1:INTENNUM_PIXELS),STAT=ALLOERR)
   ALLOCATE(INTEN_ARRAY(1:INTENNUM_PIXELS,1:INTENNUM_PIXELS,1:10),STAT=ALLOERR)
...
!       A SCENE.DAT FILE EXISTS
      ...
      IF(NUMFOB.NE.FOBNUMBER) THEN
         WRITE(OUTLYNE,*)'THE NUMBER OF ENTRIES IN THE SCENE.DAT FILE DOES NOT MATCH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'THE NUMBER OF FOBS IN THE SHORT RAYHIST.DAT FILE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'INTENSITY CALCULATION ABORTED'
         CALL SHOWIT(1)
         DEALLOCATE(TEMPHIST,TEMPINTEN,INTEN_ARRAY,INTEN_XARRAY,INTEN_YARRAY ,STAT=ALLOERR)
         CALL CLOSE_FILE(48,1)
         CALL CLOSE_FILE(102,1)
         CALL MACFAL
         RETURN
...
!       SAVE RAW INTENSITY SUM I IMES COS(AOI)
            INTEN_ARRAY(IX,IY,2)=INTEN_ARRAY(IX,IY,2)+TEMPINTEN(5,I)*(TEMPINTEN(4,I))
!       SAVE RAW AOI SUM
            INTEN_ARRAY(IX,IY,3)=INTEN_ARRAY(IX,IY,3)+DACOS(TEMPINTEN(4,I))
!       SAVE RAW X-AOI SUM
            INTEN_ARRAY(IX,IY,4)=INTEN_ARRAY(IX,IY,4)+(DACOS(TEMPINTEN(4,I))*DSIN(TEMPINTEN(6,I)))
!       SAVE RAW Y-AOI SUM
            INTEN_ARRAY(IX,IY,5)=INTEN_ARRAY(IX,IY,5)+(DACOS(TEMPINTEN(4,I))*DCOS(TEMPINTEN(6,I)))
...
!       WRITE THE INTEN FILE
   ...
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)'NUMBER OF FOBS IN RAY HISTORY REPORT = ',FOBNUMBER
   CALL SHOWIT(0)
   DO I=1,FOBNUMBER
...
!       ONLY CONSIDER RAYS WITH ENERGY IN THEM
      ...
      WRITE(OUTLYNE,*)'           FOR FOB# = ',I
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'     TOTAL INTENSITY = ',ITOTINTEN
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'TOTAL NUMBER OF RAYS = ',ITOTRAYS
      WRITE(OUTLYNE,*)'       P-V INTENSITY = ',IPTOV
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'            '
      CALL SHOWIT(1)
   END DO
...
141 FORMAT(I4,I10,1X,D23.15)
   ...
   DEALLOCATE(TEMPHIST,TEMPINTEN,SCENE_ARRAY,STAT=ALLOERR)
   RETURN
13 FORMAT(I3,I10,14(1X,D23.15))
131 FORMAT(I10,I4,I10)
END
...
!       CHECK FOR STRING INPUT
   ...
   IF(WQ.EQ.'ON') THEN
      IF(DF1.EQ.1.OR.DF2.EQ.1.OR.DF3.EQ.1.OR.DF4.EQ.1.OR.DF5.EQ.1) THEN
         OUTLYNE='"SCREEN ON" REQUIRES EXPLICIT NUMERIC WORD 1,2,3,4 AND 5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   IF(WQ.EQ.'OFF') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE='"SCREEN OFF" TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       NUMERIC WORD 1
   ...
   END IF
   IF(WQ.EQ.'OFF') THEN
      call set_sys_screen(0.0D0)
      call set_sys_screen_surf(0.0D0)
      call set_sys_screen_d(0.0D0)
      call set_sys_screen_h(0.0D0)
      call set_sys_screen_s(0.0D0)
      SYSTEM(108)=0.0D0
   END IF
   IF(WQ.EQ.'ON') THEN
      call set_sys_screen(1.0D0)
      call set_sys_screen_surf(W1)
      call set_sys_screen_d(W2)
      call set_sys_screen_h(W3)
      call set_sys_screen_s(W4)
      SYSTEM(108)=W5
   END IF
   RETURN
...
!       CHECK FOR STRING INPUT
   ...
   IF(WQ.EQ.'ON'.OR.WQ.EQ.'OFF') THEN
      IF(SN.EQ.1) THEN
         OUTLYNE=WC(1:8)//' ("ON" OR "OFF")'//' TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     STI=1
   ...
   IF(STI.EQ.1) THEN
      IF(WC.EQ.'FLIPREFX') THEN
         IF(sys_flip_ref_x().EQ.0.0D0) WRITE(OUTLYNE,10)
         IF(sys_flip_ref_x().EQ.1.0D0) WRITE(OUTLYNE,11)
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'FLIPREFY') THEN
         IF(sys_flip_ref_y().EQ.0.0D0) WRITE(OUTLYNE,12)
         IF(sys_flip_ref_y().EQ.1.0D0) WRITE(OUTLYNE,13)
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WC.EQ.'FLIPREFX') THEN
      IF(WQ.EQ.'OFF') THEN
         call set_sys_flip_ref_x(0.0D0)
      END IF
      IF(WQ.EQ.'ON') THEN
         call set_sys_flip_ref_x(1.0D0)
      END IF
   END IF
   IF(WC.EQ.'FLIPREFY') THEN
      IF(WQ.EQ.'OFF') THEN
         call set_sys_flip_ref_y(0.0D0)
      END IF
      IF(WQ.EQ.'ON') THEN
         call set_sys_flip_ref_y(1.0D0)
      END IF
   END IF
