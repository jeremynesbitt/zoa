!       NINTH SET OF OPTIMIZATION ROUTINES
...
!     HERE IS WHERE DATA IS STORED
!
!     DO FOCRIT OUTPUT HERE
      WRITE(OUTLYNE,*)'FOCRIT'
      CALL SHOWIT(10)
      DO I=1,MAXFOCRIT
!     THE OPERAND AND FUNCTION NAME AND NW1 AND NW2
...
!     SYNTAX IS MACDMP (MACNAME), 0(DEFAULT) OR 1 OR 2
!     0 SAVES VARIABLES AND MERIT
...
!
!
!       THIS IS SUBROUTINE OPDMP. THIS IS THE SUBROUTINE WHICH
!       SAVES VARIABLES AND MERIT DEFINITIONS WITH THE LENS IN
!       CURLENS AND IN LENS LIBRARIES
!
!
!     DOING BOTH VARIABLES AND MERIT DEFINITIONS
!
   IF(VBCNT.NE.0) THEN
!     DO VARAIBLES OUTPUT HERE
      OLDCFGVB=1
      WRITE(OUTLYNE,*)'VARIABLE'
      CALL SHOWIT(10)
      DO I=1,VBCNT
!     FIRST THE "CFG" COMMAND AS NEEDED
...
11       FORMAT(A8,',',D15.7,',',D15.7,',',D15.7,',',D15.7,',',D15.7)
   ...
   END IF
   IF(OPCNT.NE.0) THEN
!     DO MERIT OUTPUT HERE
!     OUTPUT CMD LEVEL SETUP COMMANDS FIRST
      IF(OPSPDTYPE.EQ.1) WRITE(OUTLYNE,21)
      IF(OPSPDTYPE.EQ.2) WRITE(OUTLYNE,22)
      IF(OPSPDTYPE.EQ.3) WRITE(OUTLYNE,23)
      CALL SHOWIT(10)
...
1000        FORMAT('CFG,',I2)
         END IF
!     NEXT THE MODE COMMAND AS NEEDED
...
!       HANDLES TVAR INPUT AND TVAR UPDATE COMMANDS AND TVAR
!       OUTPUT COMMANDS AT THE CMD LEVEL
!
!       THE ARRAY VARBLL STORES TVAR INFORMATION
...
!
!       TVAR USES THE SECOND HALF OF VARBLL(I,J)
!
...
!       J=7  > ENDING SURFACE NUMBER FOR COMPUND VARIABLE
!       J=8  > VARIABLE DELTA VALUE
!       J=9  > X PIVOT FOR COMPOUND VARIABLE
!       J=10 > Y PIVOT FOR COMPOUND VARIABLE
!       J=11 > Z PIVOT FOR COMPOUND VARIABLE
!       J=12 > ALTERNATE ROLL SURFACE
!       J=13 > ORIGINAL VALUE
!       J=14 > IS NOT USED
!       J=15 > IS NOT USED
!       J=16 > IS NOT USED
...
!
   ...
   END IF
!
!       'TVB' OUTPUT VARIABLE DATA FROM INSIDE
   ...
   &WC.EQ.'BTILTA'.OR.WC.EQ.'BTILTB'.OR.WC.EQ.'BTILTG') THEN
      IF(WQ.EQ.'PIVOT') THEN
         IF(S4.EQ.1.OR.S5.EQ.1) THEN
            ...
            RETURN
         END IF
!     QUALIFIER INPUT OK, PROCESS AND RETURN
...
!     NW2 IS DELTA VALUE
      ...
      IF(DF2.EQ.1) THEN
         DFDELTT=1
!
!     CAL MAX REF AP HT FOR ASPHERIC DELTTA SETTINGS
!
...
!
!     THE DEFAULT DELTTA FOR CV AND CVR MUST BE ADJUSTED
!
!     DEFAULT INPUT
...
!     DO ROLLY
      ...
      END IF
   ELSE
!     DROP THROUGH AND DO VARIABLES WITH TWO NUMERIC WORDS
      VBJK=TVBCNT+MAXCMP
      VARNAM(VBJK+1)=WC
      VARABL(VBJK+1,1) =DBLE(VALT)
      VARABL(VBJK+1,2) =0.0D0
...
!     FOR CV.RD,CVTOR,RDTOR, CHECK FOR FLAT SURFACES
   ...
   IF(VALT.EQ.136) THEN
      ...
      END IF
   END IF
!     DO CV AND RD OR RD_FR OR CV_FR
...
!
   TNUM=TVBCNT
!
!     THIS READS THE CURRENT TVAR LIST AND RE-SUBMITS THE COMMANDS
...
!     BUILD A LINES AND ISSUE TO CONTRO
            SAVE_KDP(1)=SAVEINPT(1)
            WC=VNA
!     ONE SURFACE NUMBER AND A VALUE
            W1=JKVAR(I,3)
            W2=JKVAR(I,8)
            S1=1
            S2=1
         ...
         ELSE
            SAVE_KDP(1)=SAVEINPT(1)
            WC=VNA
!     TWO SURFACE NUMBERS AND A VALUE
            W1=JKVAR(I,3)
            W2=JKVAR(I,7)
            W3=JKVAR(I,8)
            S1=1
