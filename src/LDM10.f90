...
!
   real(real64) SYSA,ALENA,AIPOLYX,AIPOLYY,SLVA,PIKA,FT01A,MULTCLAPA,MULTCOBSA
!
...
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO SOLVES EXIST'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
...
!
   LOGICAL GONOGO
!
   real(real64) A,B,LM1,DUMA,DUMB,DUML,LA1,LA2,LA3,LA4,LA5,DSGN ,LA6,LA7,LA8,LA9,LA10
!
   EXTERNAL DSGN
!
!
...
SUBROUTINE ANGLECALC(ALPHA,BETA,LL1,MM1,NN1)
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ALPHA,BETA,LL1,MM1,NN1,D31,D32,D33,COSB,SINB
!
!
...
SUBROUTINE NEWANGLES(AEEA,BEEB,CEEC,LX,MX,NX,LY,MY,NY,LZ,MZ,NZ)
   ...
   IMPLICIT NONE
   INTEGER SINB
   real(real64) D11,D12,D13,LX,MX,NX,LY,MY,NY,LZ,MZ,NZ
   real(real64) D21,D22,D23
   real(real64) D31,D32,D33,BEEB,AEEA,CEEC,COSB
!
!
   D11=LX
   D12=MX
...
!
   INTEGER I,J,K
!
   real(real64) EFLY,EFLX,BFD ,FNY,FNX,LENGTH,GIHY,GIHX,SYSP11,OAL,TMAGY,TMAGX,ERELY,ERELX,EPY,EPX,AMAGY,AMAGX
!
!
   IF(WC.EQ.'OCDY') THEN
...
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO PIKUPS EXIST'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
