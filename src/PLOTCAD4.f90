!       FOURTH FILE OF PLOT/CAD ROUTINES
...
SUBROUTINE PLTCHRSH
   ...
   LOGICAL TABEXIST
   real(real64) SHIFT_TABLE(1:10,1:6),MINSHIFT,MAXSHIFT,MINWAVE,MAXWAVE
   real(real64) DNUMWAVE
   COMMON/TABLESHIFT/SHIFT_TABLE,TABEXIST
   INTEGER NUMWAVE,I,PLV
   COMMON/WAVEMAX/NUMWAVE
   PLV=1
      CALL KDP_EXEC('CHRSHIFT ACC')
   DNUMWAVE=DBLE(NUMWAVE)
   IF(WQ(1:1).EQ.'X') PLV=2
!
!
!     THIS PLOTS CHROMATIC FOCUS SHIFT DATA
   IF(is_command_query()) THEN
      OUTLYNE=&
      &'"PLTCHRSH" PLOTS XZ OR YZ-PLANE CHROMATIC FOCAL SHIFT DATA'
      CALL SHOWIT(1)
   ...
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE=&
      &'"PLTCHRSH" TAKES NO STRING OR NUMERIC WORD INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(SQ.EQ.1.AND.WQ.NE.'X') THEN
      OUTLYNE=&
      &'"PLTCHRSH" ONLY TAKES "X" AS OPTIONAL QUALIFIER INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(.NOT.TABEXIST) THEN
      OUTLYNE=&
      &'A CHROMATIC FOCUS SHIFT TABLE COULD NOT BE GENERATED'
      CALL SHOWIT(1)
...
20 FORMAT(G11.4)
      ...
      CALL KDP_EXEC('PLOT UXAXIS '//AXMIN //AXMAX)
      CALL KDP_EXEC('PLOT UYAXIS '//AYMIN //AYMAX)
      CALL KDP_EXEC('PLOT UYLINE 0')
      CALL KDP_EXEC('PLOT UYAXISLB WAVELENGTH IN MICRONS')
...
!     AFOCAL
   ...
   IF(PLV.EQ.2) THEN
      SAVE_KDP(1)=SAVEINPT(1)
!
!     REGISTERS USED ARE 1 THROUGH NUMWAVE FOR THE FOCUS SHIFTS
!     AND                11 THROU  10+NUMWAVE FOR THE WAVELENGTHS
!     WE DO THIS FIRST FOR THE XZ-PLANE, THEN THE YZ-PLANE
!
!     XZ-PLAME MARGINAL
      DO I=1,NUMWAVE
...
SUBROUTINE CHRSHIFT
   ...
   INTEGER NUMWAVE
   COMMON/WAVEMAX/NUMWAVE
   real(real64) SHIFT_TABLE(1:10,1:6),LAM_TABLE(1:10,1:2)
   real(real64) JK_A(1:10),JK_1(1:10),JK_2(1:10),ZERO(1:4)
   real(real64) SPY,SPUY,SPCY,SPUCY
   real(real64) SPX,SPUX,SPCX,SPUCX
   real(real64) SCWPY,SCWPUY,SCWPCY,SCWPUCY
   real(real64) SCWPX,SCWPUX,SCWPCX,SCWPUCX
   COMMON/TABLESHIFT/SHIFT_TABLE,TABEXIST
   ...
   IF(is_command_query()) THEN
      OUTLYNE=&
      &'"CHRSHIFT" DISPLAYS CHROMATIC FOCUS SHIFT DATA'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   ...
   IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE=&
      &'"CHRSHIFT" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(WVCOUNT.LE.1) THEN
      WRITE(OUTLYNE,*)&
      &'MORE THAN ONE ACTIVE WAVELENGTH MUST EXIST FOR "CRSHIFT"'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'TO WORK.'
      CALL SHOWIT(1)
...
!     INCREASING WAVELENGTH IN MICRONS
   DO I=1,NUMWAVE
...
22    FORMAT('WAVELENGTH',3X,'MARG. Y-SHIFT',3X,'CHIEF Y-SHIFT'&
      &,3X,'MARG. X-SHIFT',3X,'CHIEF X-SHIFT')
      WRITE(OUTLYNE,22)
      CALL SHOWIT(0)
29    FORMAT(' (MICRONS)',2X,'(AXIAL MOTION)',3X,'(TRANSVERSE) '&
      &,2X,'(AXIAL MOTION)',3X,'(TRANSVERSE) ')
      WRITE(OUTLYNE,29)
      CALL SHOWIT(0)
23    FORMAT(1X)
   END IF
...
!     CONVERT TO MILLIRADIANS
      ...
      IF(.NOT.NOPRINT) THEN
         ...
         &,SHIFT_TABLE(I,4),SHIFT_TABLE(I,5)&
         &,SHIFT_TABLE(I,6)
         CALL SHOWIT(0)
      END IF
   END DO
...
!
   ...
   &NUMT2,NUMT3,NUMT4,NUMT5,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10 &
   &,NUMTOTAL,NUMT0
!
   real(real64) WEIS,REFHT,SUMSIG,WEI(1:10),LAM(1:10)&
   &,SUML2,SUML4
!
   real(real64) ARG,PIT,PEAK,MEAN,MEAN2
!
   real(real64) SUMONE(0:10),SUMTWO(0:10),OPPIT(0:10),&
   &SIG,OPPEAK(0:10),TOT
!
   PTOVOPD(0:10)=0.0D0
   SUMONE(0:10)=0.0D0
...
!
   ...
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(WC//' TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!     OPEN AND LOAD THE APPROPRIATE CAPFN DATA AND THEN PLOT IT
   IF(.NOT.CPFNEXT) THEN
      OUTLYNE=&
      &'NO CAPFN EXISTS'
      CALL SHOWIT(1)
...
!     CALC SUMSIG
      ...
      DO I=1,10
         ...
         &LAM(I).NE.0.0D0)&
         &SUMSIG=SUMSIG+((WEI(I)*(RMSOP(I)**2))/(LAM(I)**2))
      END DO
   END IF
   SIG=0.0D0
   IF(SUML2.NE.0.0D0.AND.SUMSIG.NE.0.0D0)&
   &RMSOPD=DSQRT(SUMSIG/SUML2)
   RMSOP(0)=RMSOPD
...
!       THIS PASSES THE SOLUTION OF THE LEAST SQUARES PROBLEM
   ...
   LOGICAL ERRR
!
   real(real64) ACCUM(1:96,1:96),REFHT,WW,TERM1,TERM2,&
   &WAVEX,A,CCOL(1:96),X(1:96),SUML2,SUML4,WEIS,&
...
!     LOAD DSPOTT(*,ID) WITH DSPOT
   ...
   DLLZX=&
   &(DLLZ*2.0D0)/((ALENS(WWVN,NEWIMG-1)*&
   &DSIN(DATAN(1.0D0/(2.0D0*RBFNX))))**2)
   DLLZY=&
   &(DLLZ*2.0D0)/((ALENS(WWVN,NEWIMG-1)*&
   &DSIN(DATAN(1.0D0/(2.0D0*RBFNY))))**2)
...
!
   ...
   IF(SST.EQ.1) THEN
      ...
      & '"PLOT CAPFN(OPD OR APD)" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WQ.EQ.'CAPFNOPD') THEN
      IF(S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT CAPFNOPD" TAKES NO NUMERIC WORD #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WQ.EQ.'CAPFNOPD') THEN
      IF(DF3.EQ.1.AND.DF4.EQ.0.OR.DF3.EQ.0.AND.DF4.EQ.1) THEN
         ...
         CALL SHOWIT(1)
         OUTLYNE=&
         &'EITHER USE NO EXPLICIT NUMERIC WORD #3 AND #4 (AUTO Z-SCALING)'
         CALL SHOWIT(1)
         OUTLYNE=&
         &'OR USE BOTH NUMERIC WORD #3 (Z-MAX) AND NUMERIC WORD #4 (Z-MIN)'
         CALL SHOWIT(1)
      ...
      IF(DF3.EQ.0.AND.DF4.EQ.0) THEN
         ...
         ELSE
            ...
            CALL SHOWIT(1)
            OUTLYNE=&
            &'NUMERIC WORD #3 MUST BE LESS THAN NUMERIC WORK #4'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'WHEN BOTH ARE ISSUED DURING CAPFNOPD PLOTTING'
            CALL SHOWIT(1)
            CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
            RETURN
         END IF
   ...
   IF(WQ.EQ.'CAPFNAPD') THEN
      IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT CAPFNAPD" TAKES NO NUMERIC WORD'//'\n'//&
         & '#3, #4 OR #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   ...
   real(real64) XTEST1,XTEST2,YTEST1,YTEST2,SLOPE
!
...
!
   ...
   real(real64) XV,YV,RANGE
   real(real64) XX,YY,XY2
   DIMENSION XX(:),YY(:),XY2(:)
   REAL F_JK,X_JK,Y_JK
...
!
   ...
   IMPLICIT NONE
!
!       THIS ROUTINE DOES PLOT AXIS AND PLOT NOAXIS AT THE CMD LEVEL
!
!
!       PLOT AXIS
!
...
!       CHECK SYNTAX
      IF(SST.EQ.1.OR.SN.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT AXIS" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      PLTAXS=.TRUE.
      ...
      IF(is_command_query()) THEN
         ...
         IF(.NOT.PLTAXS)&
         &OUTLYNE= 'AXIS PLOTTING IS NOT CURRENTLY ENABLED'
         CALL SHOWIT(1)
         RETURN
      END IF
      RETURN
...
!
   ...
   IMPLICIT NONE
!
!       THIS ROUTINE DOES THE "PLOT ACC" COMMAND AT THE CMD LEVEL
!       THE SIZE AND ANGLE ARE RESET USING "PLOT CHNOTE"
!
...
!       CHECK SYNTAX
   IF(S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT ACC" TAKES NO NUMERIC WORD #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
   IF(W3.LT.0.0D0.OR.W3.GT.10.0D0) THEN
      OUTLYNE=&
      &'VALID NUMERIC WORD #3 VALUES ARE: 1,2,3,4,5,6,7,8,9 OR 10'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"PLOT ACC" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
   END IF
!       STI
...
!       NOT ?, DO THE PLOTTING
   END IF
!       DEFAULT VALUE DETERMINATIONS
!       IF W1 IS DEFAULT, USE THE CURRENT CURRENT X PEN POS
   IF(DF1.EQ.1) W1=DBLE(PENPOSX)
!       IF W2 IS DEFAULT, USE THE CURRENT CURRENT Y PEN POS
   IF(DF2.EQ.1) W2=DBLE(PENPOSY)
!       IF W3 IS DEFAULT, W3=3.0D0
   IF(DF3.EQ.1) W3=3.0D0
!       IF W4 IS DEFAULT, SET INTEG FALSE
...
!
!       HERE IS WHERE THE ACCUMULATOR CONTENTS ARE PLOTTED
!
   COLPAS=COLLBL
   CALL MY_COLTYP(COLPAS)
   IIX=INT(W1)
...
!       COMMAND "PLOT" AT THE SPECTR LEVEL
!
...
!     SET LETTER SIZE AND ANGLE
   ...
   NT1ANG=0
   CALL MY_SETCHARASPECT(1.5,1.5)
!       LOWER AND UPPER LIMITS OF WAVELENGTH ARE
   LAMB1=TABLE(1,2)
   LAMB2=TABLE(CTAB,2)
!       CTAB IS THE MAXIMUM NUMBER OF ENTRIES IN THE TABLE ARRAYS
!       PROCEED
!
   DO 10 I = 1, CTAB
      X(I)  = REAL(TABLE(I,2))
...
!
!       LAMB1 IS THE SHORTEST WAVELENGTH, LLIM WILL BE THE NEXT
!       LOWEST INTEGER VALUE OF WAVELENGTH IN MICRONS
   LLIM=REAL(LAMB1)
   IF(LLIM.LT.0) LLIM=0
!       AND THE UPPER LIMIT IS CONVERSELY:
   ULIM=REAL(LAMB2)
!       MAX FUNCTIONAL VALUE IS:
   UFUNC=TABLE(1,3)
   DO I=2,CTAB
      IF(TABLE(I,3).GE.UFUNC) UFUNC=TABLE(I,3)
   END DO
!       MIN FUNCTIONAL VALUE IS:
   LFUNC=TABLE(1,3)
   DO I=2,CTAB
      IF(TABLE(I,3).LE.LFUNC) LFUNC=TABLE(I,3)
   END DO
!       DEFAULT OVERRIDES DONE HERE
!
...
!
   ...
   IF(ULIM.EQ.LLIM) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'UPPER AND LOWER WAVELENGTH VALUES ARE EQUAL'//'\n'//&
...
!
   real(real64) XCENTER,YCENTER,A,X10,X20,X30,X40 &
   &,Y10,Y20,Y30,Y40,X11,X21,X31,X41,Y11,Y21,Y31,Y41,RANGE &
   &,PLANG,CENTXX,CENTYY,PII2
!
   INTEGER COLPAS,I,XPOINT,YPOINT,IXI,IYI
!
   real(real64) JA,JB,XUP,XLO,YUP,YLO,MAXX,MAXY
!
...
!
   CALL PLOTBOX
!
!     LIFT PEN, MOVE TO INNER WINDOW START
!
   COLPAS=COLLBL
   CALL MY_COLTYP(COLPAS)
!     SPOT DIAGRAM HEADING
   IF(WQ.EQ.'SUM') THEN
   ...
   IF(.NOT.SPOTSSI) THEN
      ...
      IF(MAXX.GT.MAXY) THEN
         ...
         ELSE
            ...
            &SPDSSIVAL=(MAXX+(2.0D0*DABS(CENTXX-REFRY(11,NEWIMG))))/3.6D0
            IF(REFRY(11,NEWIMG).GT.PII)&
            &SPDSSIVAL=(MAXX+(2.0D0*DABS(CENTXX-(REFRY(11,NEWIMG)-PII2)&
            &)))/3.6D0
         END IF
      ELSE
         IF(sys_mode().LE.2.0D0) THEN
         ...
         ELSE
            ...
            &SPDSSIVAL=(MAXY+(2.0D0*DABS(CENTYY-REFRY(12,NEWIMG))))/3.6D0
            IF(REFRY(12,NEWIMG).GT.PII)&
            &SPDSSIVAL=(MAXY+(2.0D0*DABS(CENTYY-(REFRY(12,NEWIMG)-PII2)&
            &)))/3.6D0
         END IF
      END IF
   ELSE
...
!     CIRCULAR DETECTOR
      ...
      DO I=0,720
         ...
         &*1000.0D0)/SPDSSIVAL)+5000
         YPOINT=INT(((((DETDIAM/2.0D0)*DSIN(PLANG))+YCENTER)&
         &*1000.0D0)/SPDSSIVAL)+3500
         COLPAS=COLFRM
         CALL MY_COLTYP(COLPAS)
         IF(I.EQ.0)&
         &CALL MY_PLOT(XPOINT,YPOINT,0,0,3000,7000,1500,5500)
         IF(I.GT.0)&
...
!     NOW PLOT THE SPOT DATA
   ...
   DO I=2,ITOT
      IF(SPREC.EQ.61) THEN
!     LOAD DSPOT(*) WITH DSPOTT(*,ID) DATA
         ID=I-1
         CALL SPOTIT(4)
      END IF
      IF(SPREC.EQ.32)&
...
!     PLOT CENT LOCATION
      ...
      ELSE
         IF(REFRY(11,NEWIMG).LE.PII)&
         &XPOINT=INT(((CENTXX-REFRY(11,NEWIMG))*1000.0D0)/SPDSSIVAL)+5000
         IF(REFRY(12,NEWIMG).LE.PII)&
         &YPOINT=INT(((CENTYY-REFRY(12,NEWIMG))*1000.0D0)/SPDSSIVAL)+3500
         IF(REFRY(11,NEWIMG).GT.PII)&
         &XPOINT=INT(((CENTXX-(REFRY(11,NEWIMG)-PII2))&
         &*1000.0D0)/SPDSSIVAL)+5000
         IF(REFRY(12,NEWIMG).GT.PII)&
         &YPOINT=INT(((CENTYY-(REFRY(12,NEWIMG)-PII2))&
         &*1000.0D0)/SPDSSIVAL)+3500
      END IF
...
!     PLOT CHIEF RAY LOCATION
      ...
      ELSE
         IF(REFRY(11,NEWIMG).LE.PII)&
         &XPOINT=INT(((REFRY(11,NEWIMG)-CENTXX)*1000.0D0)/SPDSSIVAL)+5000
         IF(REFRY(12,NEWIMG).LE.PII)&
         &YPOINT=INT(((REFRY(12,NEWIMG)-CENTYY)*1000.0D0)/SPDSSIVAL)+3500
         IF(REFRY(11,NEWIMG).GT.PII)&
         &XPOINT=INT((((REFRY(11,NEWIMG)-PII2)-CENTXX)&
         &*1000.0D0)/SPDSSIVAL)+5000
         IF(REFRY(12,NEWIMG).GT.PII)&
         &YPOINT=INT((((REFRY(12,NEWIMG)-PII2)-CENTYY)&
         &*1000.0D0)/SPDSSIVAL)+3500
      END IF
      CALL MY_PLOT(3000,YPOINT,0,2,3000,7000,1500,5500)
      CALL MY_PLOT(7000,YPOINT,1,2,3000,7000,1500,5500)
...
!
   IF(WQ.NE.'SUM') THEN
!     DO THE PLOTTING OF THE CENTROID POSITIONS
!     AT X=200, Y=6500
      NT1SIZ=1
      NT1ANG=0
      IF(sys_mode().EQ.1.0D0.OR.sys_mode().EQ.2.0D0) THEN
...
!
   ...
   RETURN
END
!     THE FOLLOWING ARE FOR LSF AND FIELD ABERRATION PLOTS
...
!
   real(real64) Y2,XX,YY,XDEL,XXX,YYY
!
...
!
   ...
   &,XX(1:FEND),YY(1:FEND),STAT=ALLOERR)
   COLPAS=COLDEF
   CALL MY_COLTYP(COLPAS)
   DO I=1,FEND
      XX(I)=DBLE(X(I))
...
!
   ...
   &,XX(1:SPNUM),YY(1:SPNUM),XXX(1:SPNUM),YYY(1:SPNUM)&
   &,STAT=ALLOERR)
   COLPAS=COLDEF
   CALL MY_COLTYP(COLPAS)
   DO I=1,FEND
...
!       PLOT THE AXES
   ...
   IF(MTFGRIDS) THEN
!       PLOT THE VERTICAL GRID LINES (11 OF THEM FROM 0 TO 10)
...
!       PLOT THE HORIZONTAL TICS (11 OF THEM FROM 0 TO 10)
      ...
      CALL MY_PLOT(8000,5600,1,1,-10,10010,-10,7010)
   END IF
!       PLOT THE VERTICAL TICS (11 OF THEM FROM 0 TO 10)
...
SUBROUTINE PLOTVVAL(LFUNC,DELF1)
   ...
   INTEGER COLPAS,IB
!
!       PLOT THE VERTICAL AXIS VALUE LABELS
   COLPAS=COLLBL
   CALL MY_COLTYP(COLPAS)
   NUMJIM=LFUNC
   NUMBER=NUMJIM
...
SUBROUTINE PLOTHVAL(LLIM,DELX1)
   ...
   COLPAS=COLLBL
   CALL MY_COLTYP(COLPAS)
!     PLOT THE HORIZONTAL AXIS VALUE LABELS
   NUMJIM=LLIM
   NUMBER=NUMJIM
   IF(NUMBER.LT.0.00001) NUMBER=0.0
   CALL HMAT(DBLE(NUMBER),3,ANUMBER,.FALSE.,IB)
...
!       PLOT THE HORIZONTAL GRIDS (11 OF THEM FROM 0 TO 10)
...
!
!       PLOT THE HORIZONTAL TICS (5 OF THEM FROM 0 TO 10)
...
!     SET LETTER SIZE AND ANGLE
   ...
   &CT.EQ.11.OR.CT.EQ.13.OR.CT.EQ.15.OR.CT.EQ.17.OR.CT.EQ.19)&
   &BTYPE=' (VERT).'
...
!     SET LETTER SIZE AND ANGLE
   ...
   &CT.EQ.10.OR.CT.EQ.12.OR.CT.EQ.14.OR.CT.EQ.16.OR.&
   &CT.EQ.18.OR.CT.EQ.20)&
   &BTYPE=' (VERT.)'
   WRITE(B,101) FLDCODE(1,J)
   READ(B,200) CRANGE1
   WRITE(B,101) FLDCODE(2,J)
...
200 FORMAT(A9)
   ...
   &'DIFFRACTION LIMIT HORIZONTAL BARS    '
   IF(J.EQ.0.AND.CT.EQ.2) STRING=&
   &'DIFFRACTION LIMIT VERTICAL BARS      '
   CALL MY_JUSTSTRING(IX-350,IY-10,STRING(1:37),NT1ANG,NT1SIZ,3,3)
   RETURN
END
...
!
   ...
   ALLOCATE(IX(1:SPNUM),IY(1:SPNUM),Y2(1:FEND)&
   &,XX(1:SPNUM),YY(1:SPNUM),XXX(1:SPNUM),YYY(1:SPNUM),STAT=ALLOERR)
   DO I=1,22
      DONE(I)=.FALSE.
   END DO
   DO I=1,FEND
...
!     SET LETTER SIZE AND ANGLE
   ...
   &CT.EQ.13.OR.CT.EQ.15.OR.CT.EQ.17.OR.CT.EQ.19)&
   &BTYPE=' (VERT).'
...
!
   ...
   &,XX(1:SPNUM),YY(1:SPNUM),XXX(1:SPNUM),YYY(1:SPNUM),STAT=ALLOERR)
!
!     CT REPRESENTS 5 POINTS ON THE HORIZINTAL AXIS
!
   DO I=1,20
      DONE(I)=.FALSE.
   END DO
...
!
   REAL X,Y,LLIM,ULIM,LFUNC,UFUNC
!
   real(real64) Y2,XX,YY,XDEL,XXX,YYY,MAXFREQ
!
   INTEGER FEND,I,IX,IY,ALLOERR,SPNUM,LT,CT,J
!
   LOGICAL DONE(1:22)
...
!
   ...
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOTCON CAPFN(OPD OR APD)" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WQ.EQ.'CAPFNOPD') THEN
      IF(S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOTCON CAPFNOPD" TAKES NO NUMERIC WORD #4 or #5'//'\n'//&
         & '#4 OR #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   ...
   END IF
!
   ! JN - do plot with separate routine, but want to use this
   ! routine to load proper wavelength so comment out.
...
!
   ...
   END IF
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"PLOT UXLINE" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S4.EQ.1.OR.S3.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXLINE" ONLY TAKES NUMERIC WORDS #1 AND #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   ...
   IF(.NOT.USCALEX) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXLINE" REQUIRES THE X-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.USCALEY) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXLINE" REQUIRES THE Y-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!
   ...
   END IF
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"PLOT UYLINE" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S4.EQ.1.OR.S3.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYLINE" ONLY TAKES NUMERIC WORDS #1 AND #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF2.EQ.1) LNTYPE=0
   ...
   IF(.NOT.USCALEX) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYLINE" REQUIRES THE X-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.USCALEY) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYLINE" REQUIRES THE Y-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   COLPAS=COLPEN
...
SUBROUTINE PLOTUXAXIS(XLOWER,XUPPER)
   ...
   IF(SST.EQ.1) THEN
      OUTLYNE=&
      &'"PLOT UXAXIS" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXAXIS" REQUIRES EXPLICIT NUMERIC WORD #1'//'\n'//&
      & 'AND #2 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W2.LE.W1.AND.DF5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXAXIS" REQUIRES NUMERIC WORD #1'//'\n'//&
      & 'TO BE NUMERICALLY LESS THAN NUMERIC WORD #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       PLOT THE HORIZONTAL TICS (UUDX+1 OF THEM FROM 0 TO UUDX)
   IX=0
...
SUBROUTINE PLOTUYAXIS(YLOWER,YUPPER)
   ...
   IF(SST.EQ.1) THEN
      OUTLYNE=&
      &'"PLOT UYAXIS" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W2.LE.W1.AND.DF5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYAXIS" REQUIRES NUMERIC WORD #1'//'\n'//&
      & 'TO BE NUMERICALLY LESS THAN NUMERIC WORD #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYAXIS" REQUIRES EXPLICIT NUMERIC WORD #1'//'\n'//&
      & 'AND #2 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       PLOT THE VERTICAL TICS (UUDY+1 OF THEM FROM 0 TO UUDY)
   IY=0
...
SUBROUTINE PLOTUXAXRNG(XLOWER,XUPPER)
   ...
   IF(S3.EQ.1.OR.SST.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"PLOT UXAXRNG" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYAXRNG" REQUIRES EXPLICIT NUMERIC WORD #1'//'\n'//&
      & 'AND #2 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W2.LE.W1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXAXRNG" REQUIRES NUMERIC WORD #1'//'\n'//&
      & 'TO BE NUMERICALLY LESS THAN NUMERIC WORD #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!     SET THE STARTING AND ENDING POINTS OF THE X-DIRECTION
!     FOR THE USER-DEFINED PLOT
...
SUBROUTINE PLOTUYAXRNG(YLOWER,YUPPER)
   ...
   IF(S3.EQ.1.OR.SST.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"PLOT UYAXRNG" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   ...
   IF(W2.LE.W1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYAXRNG" REQUIRES NUMERIC WORD #1'//'\n'//&
      & 'TO BE NUMERICALLY LESS THAN NUMERIC WORD #2'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!     SET THE STARTING AND ENDING POINTS OF THE Y-DIRECTION
!     FOR THE USER-DEFINED PLOT
   YLOWER=YLOWER*1000
   YUPPER=YUPPER*1000
   UIYSTART=INT(YLOWER)
...
SUBROUTINE PLOTUXAXISLB(NAME,N)
   ...
   IF(SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UXAXISLB" ONLY TAKES STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
SUBROUTINE PLOTUYAXISLB(NAME,N)
   ...
   IF(SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UYAXISLB" ONLY TAKES STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       PLOT THE VERTICAL AXIS NAME
   ...
   &UIYSTART+((UIYSTOP-UIYSTART)/2),NAME(1:N),90,2,2,3)
   RETURN
END
...
!
   ...
   END IF
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"PLOT UPLOT" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S5.EQ.1) LNTYPE=0
   ...
   IF(DF1.EQ.1.OR.DF2.EQ.1.OR.DF3.EQ.1.OR.DF4.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UPLOT" REQUIRES EXPLICIT'//'\n'//&
      & '"NUMERIC WORD #1 TO #4 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W2.LE.W1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UPLOT" REQUIRES NUMERIC WORD #2 TO BE GREATER'//'\n'//&
      & '"THAN NUMERIC WORD #1'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W4.LE.W3) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UPLOT" REQUIRES NUMERIC WORD #4 TO BE GREATER'//'\n'//&
      & '"THAN NUMERIC WORD #3'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.USCALEX) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UPLOT" REQUIRES THE X-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.USCALEY) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT UPLOT" REQUIRES THE Y-AXIS TO BE PLOTTED FIRST'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   LXREG=INT(W1)
   ...
   &W2.LT.1.0D0.OR.W2.GT.MAXREG.OR.&
   &W3.LT.1.0D0.OR.W3.GT.MAXREG.OR.&
   &W4.LT.1.0D0.OR.W4.GT.MAXREG) THEN
      OUTLYNE='GENERAL PURPOSE STORAGE REGISTER NUMBER OUT OF RANGE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'VALID VALUES FROM 1 TO ',MAXREG,' FOR EACH ENTRY'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
