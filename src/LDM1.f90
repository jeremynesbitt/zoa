...
!       A FLOATING POINT REPRESENTATION IS STORED IN sys_last_surf()
...
!                       WV5=0.70652
!
!       WV1 THROUGH WV5 ARE STORED IN sys_wavelength(1) THROUGH
!       sys_wavelength(5) IN THE SYSTEM ARRAY.
!
!       SET THE DEFAULT WAVELENGTH VALUES.
!
   call set_sys_wavelength(1,0.58756D0)
   call set_sys_wavelength(2,0.48613D0)
   call set_sys_wavelength(3,0.65627D0)
   call set_sys_wavelength(4,0.43584D0)
   call set_sys_wavelength(5,0.70652D0)
!     SET WAVELENGTHS 6 TO 10 TO ZERO AND WEIGHTS TO ZERO
   SYSTEM(71:80)=0.0D0
   call set_sys_wv(1,0.58756D0)
   call set_sys_wv(2,0.48613D0)
   call set_sys_wv(3,0.65627D0)
   call set_sys_wv(4,0.43584D0)
   call set_sys_wv(5,0.70652D0)
!     SET WAVELENGTHS 6 TO 10 TO ZERO AND WEIGHTS TO ZERO
...
!       SYSTEM(21 THROUGH24)
   SYSTEM(21:24)=0.0D0
!
!       sys_xz_data() TRACKS IF SCX/SCX FANG OR SAX HAVE BEEN ENTERED
!       EXPLICITLY.
!       IT IS ASSUMED THAT THEY HAVE NOT IN WHICH CASE
!       sys_xz_data()=0.0, PUPIL AND FIELD ARE CIRCULAR
!
   call set_sys_xz_data(0.0D0)
!
...
!       TRACKER BY THE VALUE IN sys_ref_surf()
!
   call set_sys_ref_surf(1.0D0)
!
!       THE DEFAULT APERTURE STOP CONDITION IS "NO"
!       AND IS INDICATED BY THE VALUE IN sys_astop() OF -99
!       THIS IS THE DEFAULT. WHEN ASTOP NO IS ISSUED, sys_astop()
...
!
!       THE ASTOP ADJUSTMENT IS TRACKED IN sys_astop_adj()
!                       DEFAULT IS NO ADJUSTMENT = 0.0
!       EN ADJUST = sys_astop_adj()=1.0
!       EX ADJUSTMENT = sys_astop_adj()=-1.0
!
   call set_sys_astop_adj(0.0D0)
!
!
!       THE LENS EVALUATION MODE (FOCAL,UFOCAL,AFOCAL OR UAFOCAL)
!       STATUS IS STORED IN sys_mode() AS 1.0,2.0,3.0 OR 4.0
!       RESPECTIVELY. FOCAL IS THE DEFAULT MODE SET HERE.
!
   call set_sys_mode(1.0D0)
!
!       SET sys_curr_cfg() TO ITS DEFAULT OF 1.00
!
   sys_curr_cfg()=1.00D0
!       SET sys_high_cfg() TO ITS DEFAULT OF 1.00
!
   call set_sys_high_cfg(1.00D0)
!
!
!       NOTE:
...
!       CHECK SYNTAX
      IF(SST.EQ.1) THEN
         OUTLYNE='"PLOT "'//WQ(1:6)//'" TAKES NO STRING INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PLOT "'//WQ(1:6)//'" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(DF1.EQ.1) THEN
         OUTLYNE='"PLOT "'//WQ(1:6)//' REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"UPDATE LENS" TAKES NO NUMERIC OR STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(WC.EQ.'TSD') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"TSD" TAKES NO STRING OR QUALIFIER'
         CALL SHOWIT(1)
      ...
      IF(SQ.EQ.1.AND.SN.EQ.1) THEN
         OUTLYNE='"TSD" TAKES EITHER QUALIFIER OR NUMERIC INPUT, NOT BOTH'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(SQ.EQ.1.AND.WQ.NE.'ALL     ') THEN
         OUTLYNE='"TSD" ONLY ACCEPTS "ALL" AS A VAILID QUALIFIER'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(DF1.EQ.1.AND.DF2.EQ.0.OR.DF1.EQ.0.AND.DF2.EQ.1) THEN
         OUTLYNE='"TSD" USES EITHER TWO OR ZERO NUMERIC WORDS OR QUALIFIER INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(SST.EQ.1) THEN
      OUTLYNE='"'//WQ(1:2)//'" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:2)//'" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(SQ.EQ.1.AND.F5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:2)//'" TAKES NO QUALIFIER WORD IN LENS INPUT MODE'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(F6.EQ.1) THEN
      IF(WQ.NE.'        '.AND.WQ.NE.'DELT'.AND.WQ.NE.'CENT') THEN
         OUTLYNE='INVALID QUALIFIER WORD USED WITH "'//WQ(1:2)//'"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(SST.EQ.1) THEN
      OUTLYNE='"'//WQ(1:4)//'" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:4)//'" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1.AND.F5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:4)//'" TAKES NO QUALIFIER WORD IN LENS INPUT MODE'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(F6.EQ.1) THEN
      IF(WQ.NE.'        '.AND.WQ.NE.'DELT'.AND.WQ.NE.'CENT') THEN
         OUTLYNE='INVALID QUALIFIER WORD USED WITH "'//WQ(1:4)//'"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(SST.EQ.1) THEN
      OUTLYNE='"'//WQ(1:3)//'" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:3)//'" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1.AND.F5.EQ.1) THEN
      OUTLYNE='"'//WQ(1:3)//'" TAKES NO QUALIFIER WORD IN LENS INPUT MODE'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(F6.EQ.1) THEN
      IF(WQ.NE.'        '.AND.WQ.NE.'DELT'.AND.WQ.NE.'CENT') THEN
         OUTLYNE='INVALID QUALIFIER WORD USED WITH "'//WQ(1:3)//'"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   ...
   IF(WC.NE.'GRT') THEN
      IF(surf_diffraction_flag(SURF).NE.1.0D0) THEN
         OUTLYNE='"'//WQ(1:3)//'" REQUIRES THE SURFACE TO BE A LINEAR GRATING'
         CALL SHOWIT(1)
         OUTLYNE='NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   ELSE
      IF(SN.EQ.1) THEN
         OUTLYNE='"'//WQ(1:3)//'" TAKES NO NUMERIC INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1) THEN
      OUTLYNE='"'//WQ(1:3)//'" TAKES NO QUALIFIER WORD INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"'//WQ(1:4)//'" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(surf_diffraction_flag(SURF).NE.1.0D0) THEN
      OUTLYNE='NO DIFFRACTION GRATING TO DELETE ON THE CURRENT SURFACE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
      OUTLYNE='"'//WQ(1:3)//'" TAKES NO QUALIFIER OR STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
