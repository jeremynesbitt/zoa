...
!       CHECK SYNTAX
      IF(SN.EQ.1 .OR.SST.EQ.1.AND.STI.NE.1) THEN
   ...
   IF(WQ.NE.'NEW') THEN
      IF(DEVTYP.NE.2) THEN
         OUTLYNE='"DXF NEW" MUST BE ISSUED BEFORE PLOTTING CAN PROCEED'
         CALL SHOWIT(1)
      ...
      END IF
   END IF
   IF(WQ.EQ.'PROFY'.OR.WQ.EQ.'PROFX'.OR.WQ.EQ.'CLAP'.OR.WQ .EQ.'EDGEY'.OR.WQ.EQ.'EDGEX'.OR.WQ.EQ.'PROF'.OR.WQ.EQ.'VERTLINE') THEN
      COATSET=.FALSE.
...
!       CHECK SYNTAX
      IF(SST.EQ.1) THEN
         OUTLYNE='"DXF CLAP" TAKES NO STRING INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"DXF CLAP" ONLY TAKES NUMERIC WORDS #1, #2 and #3 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(DF3.EQ.0.AND.W3.LE.0.0D0.OR.DF3.EQ.0.AND.W3.GT.1.0D0) THEN
         OUTLYNE='THE SCALING FACTOR IN NUMERIC WORD #3 MUST BE GREATER'
         CALL SHOWIT(1)
         OUTLYNE='THAN ZERO AND LESS THAN OR EQUAL TO 1.0'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       NSSSURFS
   IF(WQ.EQ.'NSSSURFS') THEN
      IF(NEXISTN) THEN
         ...
         RETURN
      ELSE
         OUTLYNE='NO NSS DATABASE EXISTS, NO NSS SURFACES CAN BE ADDED'
         CALL MACFAL
         RETURN
      END IF
   END IF
...
!       NSSRAYS
   IF(WQ.EQ.'NSSRAYS') THEN
      ...
      ELSE
         IF(.NOT.NEXISTN) THEN
            OUTLYNE='NO NSS DATABASE EXISTS, NO NSS RAYS CAN BE ADDED'
            CALL SHOWIT(1)
         END IF
         IF(.NOT.EXIS92) THEN
            OUTLYNE='NO NSS RAY HISTORY FILE EXISTS, NO NSS RAYS CAN BE ADDED'
            CALL SHOWIT(1)
         END IF
         CALL MACFAL
         RETURN
...
SUBROUTINE DXFLAYER
   ...
   IF(SN.EQ.1) THEN
      ...
      RETURN
   END IF
   IF(WS(1:1).EQ.' '.OR.WS(2:2).EQ.' '.OR.WS(3:3).EQ.' '.OR.WS(4:4).EQ.' '.OR.WS(5:5).EQ.' '.OR.WS(6:6).EQ.' '.OR.WS(7:7).EQ.' '.OR.WS(8:8).EQ.' ') THEN
      OUTLYNE='DXF LAYER NAME NAME MUST BE EXACTLY 8 NON-BLANK CHARACTERS'
      CALL SHOWIT(1)
      OUTLYNE='IN LENGTH'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
...
SUBROUTINE GLBSURF
   ...
   END IF
   IF(DF1.EQ.1) THEN
      OUTLYNE='"DXF GLBSURF" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(W1.LT.0.0D0.OR.W1.GT.sys_last_surf()) THEN
      OUTLYNE='DXF GLOBAL SURFACE NUMBER BEYOND LEGAL BOUNDS'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       CHECK SYNTAX
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"DXF LINE" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   ALLOCATE(DXF_X(1:MAXSUR+1),DXF_Y(1:MAXSUR+1),DXF_Z(1:MAXSUR+1),STAT=ALLOERR)
   DO I=1,2
      IF(I.EQ.1) THEN
         DXF_X(1)=REAL(LX1)
         DXF_Y(1)=REAL(LY1)
...
!
   REAL*8 X,Y,Z,XN,YN,ZN,ROT1X,ROT1Z,ROT2Y,XX1,XX2,YY1,ROT2Z,AX,AY,AZ,AALF,APHI,YMAXI,YMINI,XMAXI,XMINI,RANGE ,XNEW,YNEW,LKG,VIEPH,VIEAL,Z1,YY2,AX1,AX2,AY1,AY2 ,X00,Y00,Z0,LX0,LY0,LZ0,ACALL1,ACALL2,XM,YM,ZCORR ,X1,X2,Y1,Y2,SLOPE,DELXX,DELYY,MX0,MY0,MZ0,NX0,NY0,NZ0
!
   LOGICAL ALT,VERT,INSIT,INSIDEIT,YESONE1,YESONE2,YESONE3,YESONE4 ,YESONE5,YESONE6,VIS
!
...
!
   REAL*8 XMIN,YMIN,XMAX,YMAX,ZA,ZB,ZM,ZDELZ,XMINO,YMINO,XMAXO,YMAXO,DRAPRO,THETA,FRACRAD,ZDELZ1 ,TEMP,YMIN2,XMIN2,YMAX2,XMAX2,CALL1,CALL2
!
...
!       CHECK SYNTAX
   IF(SST.EQ.1) THEN
      OUTLYNE='"DXF PROF" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"DXF PROF" ONLY TAKES NUMERIC WORDS #1, #2 AND #3 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     SET VISIBILITY FACTOR
      VIS(I)=.TRUE.
      IF(DUMMMY(I).AND.surf_clap_type(I) == 0.AND.surf_coat_type(I) == 0.AND.I.NE.STASUR.AND.I.NE.STPSUR)VIS(I)=.FALSE.
   END DO
!
   IF(.NOT.DXFEXIS) DXFEXIS=.TRUE.
!
...
!       CHECK SYNTAX
   IF(SST.EQ.1) THEN
      OUTLYNE='"DXF RAY" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"DXF RAY" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     IF THE CURRENT SURFACE IS A TRUE DUMMY
      IF(DUM(I).AND.surf_special_type(I) == 0 .AND.I.NE.NEWIMG) GLVIRT(I)=.TRUE.
...
!       CHECK SYNTAX
   IF(SST.EQ.1) THEN
      OUTLYNE='"PLOT VERTLINE" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"PLOT VERTLINE" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       CHECK SYNTAX
   IF(SST.EQ.1) THEN
      IF(WQ.EQ.'EDGEX')OUTLYNE='"DXF EDGEX" TAKES NO STRING INPUT'
      IF(WQ.EQ.'EDGEY')OUTLYNE='"DXF EDGEY" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      IF(WQ.EQ.'EDGEX')OUTLYNE='"DXF EDGEX" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
      IF(WQ.EQ.'EDGEY')OUTLYNE='"DXF EDGEY" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   REAL*8 X,Y,Z,XN,YN,ZN,ROT1X,ROT1Z,ROT2Y,MDX,MDY,SFI,ROT2Z,AX,AY,AZ,AALF,APHI,YMAXI,YMINI,XMAXI,XMINI ,XNEW,YNEW,LKG,VIEPH,VIEAL,Z1,ANGLE,AN2 ,X00,Y00,Z0,LX0,LY0,LZ0,ZCORR,ZDELZ ,X1,Y1,MX0,MY0,MZ0,NX0,NY0,NZ0,XID,YID,GAMGAM
!
...
!     OF THE CLEAR APERTURE
            III=II
!
            CALL CAO1(X,Y,ANGLE,III,AN2,JJ,XID,YID,IIRUN,CLPTYPE,ZDELZ ,MDX,MDY,GAMGAM)
!
...
!     PUT THE CALLS TO THE DXF ROUTINES HERE FOR ALL THE SURFACES
      IOP=1
      IF(DUMMMY(II).AND.surf_clap_type(II) == 0.AND.II.NE.STASUR.AND.II.NE.STPSUR) THEN
      ELSE
         DO L=1,2
            P=1
            DO I=0,180
