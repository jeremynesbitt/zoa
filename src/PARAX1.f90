...
!       THE VALUE OF sys_scy_y1() NEEDS TO BE
!       REFINED.
!
!       THE FIRST STEP IS TO PERFORM THE PARAXIAL RAY TRACE
...
60       CONTINUE
         IF(TMP15A.EQ.TMP15B) THEN
            ...
            RETURN
         ELSE
            call set_sys_scy_y1(((-.1*TMP15A)/(TMP15B-TMP15A))+sys_scy_y1())
         END IF
!
!       NOW TRACE FROM THE OBJECT SURFACE TO THE ASTOP SURFACE
!       USING THIS VALUE OF sys_scy_y1()
...
!       IF NO ASTOP IS DEFINED, sys_scy_y1() IS USED AS IT WAS STORED
!       DURING LENS INPUT.
!******************************************************************************
!       TRACE FROM THE OBJECT TO THE IMAGE, PROPERLY HANDLING ALL PIKUPS
!       AND SOLVES ALONG THE WAY. THEN PROCEED. NO SOLVES OCCUR ON SURFACES
...
!       PCX(AT ASTOP FOR sys_scx_x1()=0.1) IS CALLED TMP17B
!
!       sys_scx_x1()=((-.1*TMP17A)/(TMP17B-TMP17A))+sys_scx_x1()
!
      IF(sys_astop().GT.0.0D0 .AND.sys_telecentric().EQ.0.0D0) THEN
!
!       RECALCULATE THE CORRECT VALUE OF sys_scx_x1()
...
600      CONTINUE
         IF(TMP17A.EQ.TMP17B) THEN
            ...
            RETURN
         ELSE
            call set_sys_scx_x1(((-.1*TMP17A)/(TMP17B-TMP17A))+sys_scx_x1())
         END IF
!
!       NOW TRACE FROM THE OBJECT SURFACE TO THE ASTOP SURFACE
!       USING THIS VALUE OF sys_scx_x1()
...
!
!       NO ASTOP WAS DEFINED OR TEL ON.
!       IF NO ASTOP IS DEFINED, sys_scx_x1() IS USED AS IT WAS STORED
!       DURING LENS INPUT.
!******************************************************************************
