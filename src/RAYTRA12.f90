...
!
   REAL*8 X,Y,Z,L,M,N,SNINDX,LOLD,MOLD,NOLD,WPAS ,D21,D22,LS,SNIND2,YL,YM,YN,OXSTRT,OYSTRT,OLDCX,OLDCY,D11,D12,A10,STX,STY,STZ,XAHT,YAHT,NXAHT,NYAHT,MAG,LSTART,MSTART,NSTART,YANG,XANG,X1LAST,LARGE,WHT1,WHT2,Y1LAST,X1ONE,Y1ONE,RXLAST,RYLAST,RXONE,RYONE,DDELX,DDELY,YYANG,XXANG,MF1,MF2,DET,TARX,TARY,XFOBB0,YFOBB0,ZFOBB0 ,ANGJK1,ANGJK2,XC1,YC1,ZC1,WWWW1,WWWW2,WWWW3,XCHIEF_TARGET ,ZSAG,SCLFACY,SCLFACX,AWW1,AWW2,YCHIEF_TARGET,RAT,CX,CY ,CX1LAST,CY1LAST,CX1ONE,CY1ONE,CRXLAST,CRYLAST,CRXONE,CRYONE,CD11,CD12,CD21,CD22,CDET,CDDELX,CDDELY,CMF1,CMF2
!
...
!
   ...
   IF(WC.EQ.'FOB') THEN
      IF(sys_fang_y().EQ.0.0D0) THEN
         SCLFACY=1.0D0
      ELSE
         SCLFACY=(1.0D0/sys_fang_y())
      END IF
      IF(sys_fang_x().EQ.0.0D0) THEN
         SCLFACX=1.0D0
      ELSE
         SCLFACX=(1.0D0/sys_fang_x())
      END IF
      AWW1=WW1/SCLFACY
      AWW2=WW2/SCLFACX
   END IF
...
!       OBJ ANGLES CANT BE INPUT
   ...
   END IF
   IF(sys_scy_fang_set().EQ.1.AND.NEWOBJ.EQ.0.OR.sys_scx_fang_set().EQ.1.AND.NEWOBJ.EQ.0) THEN
!     OBJECT ANGLES INPUT
      ANGIN=.TRUE.
...
!
      ...
      IF(ANGJK1.GT.90.0D0.AND.ANGJK1.LT.270.0D0.OR.ANGJK1.LT.-90.0D0.AND.ANGJK1.GT.-270.0D0) THEN
!
         IF(ANGJK2.GT.90.0D0.AND.ANGJK2.LT.270.0D0.OR.ANGJK2.LT.-90.0D0.AND.ANGJK2.GT.-270.0D0) THEN
            ZSTRT=0.0D0
         END IF
      END IF
      IF(ANGJK2.GT.90.0D0.AND.ANGJK2.LT.270.0D0.OR.ANGJK2.LT.-90.0D0.AND.ANGJK2.GT.-270.0D0) THEN
!
         ...
         END IF
      END IF
      IF(ANGJK1.GE.0.0D0.AND.ANGJK1.LT.90.0D0.OR.ANGJK1.LE.0.0D0.AND.ANGJK1.GT.-90.0D0.OR.ANGJK1.GT.270.0D0.AND.ANGJK1.LE.0.0D0.OR.ANGJK1.LT.-270.0D0.AND.ANGJK1.GE.0.0D0) THEN
!
         IF(ANGJK2.GE.0.0D0.AND.ANGJK2.LT.90.0D0.OR.ANGJK2.LE.0.0D0.AND.ANGJK2.GT.-90.0D0.OR.ANGJK2.GT.270.0D0.AND.ANGJK2.LE.0.0D0.OR.ANGJK2.LT.-270.0D0.AND.ANGJK2.GE.0.0D0) THEN
            ZSTRT=0.0D0
         END IF
      END IF
      IF(ANGJK2.GE.0.0D0.AND.ANGJK2.LT.90.0D0.OR.ANGJK2.LE.0.0D0.AND.ANGJK2.GT.-90.0D0.OR.ANGJK2.GT.270.0D0.AND.ANGJK2.LE.0.0D0.OR.ANGJK2.LT.-270.0D0.AND.ANGJK2.GE.0.0D0) THEN
!
         ...
         END IF
      END IF
      IF(sys_scy_fang_set().EQ.1.0D0.AND.sys_fang_y().EQ.0.0D0) YSTRT=0.0D0
      IF(sys_scx_fang_set().EQ.1.0D0.AND.sys_fang_x().EQ.0.0D0) XSTRT=0.0D0
      IF(XSTRT.EQ.0.0D0.AND.YSTRT.EQ.0.0D0) ZSTRT=0.0D0
   ELSE
!     OBJECT ANGLES NOT INPUT
      ANGIN=.FALSE.
!
      IF(surf_clap_type(NEWOBJ) == 0.OR.sys_rxim_fang_set().NE.0.0D0 .OR.sys_ryim_fang_set().NE.0.0D0.OR.surf_array_parity(NEWOBJ) /= 0) THEN
...
!     USE Y HEIGHT OF CLAP WITH OFFSETS
         ...
         IF(surf_clap_type(NEWOBJ) == 6.AND.surf_array_parity(NEWOBJ) == 0) THEN
            ...
            ZSTRT=(WW3*surf_thickness(NEWOBJ))+ZSAG
         END IF
         IF(surf_clap_type(NEWOBJ) > 1.AND.surf_clap_type(NEWOBJ) <= 4.AND.surf_array_parity(NEWOBJ) == 0) THEN
            XSTRT=WW2*(surf_clap_dim(NEWOBJ, 2)+surf_clap_dim(NEWOBJ, 4))
            YSTRT=WW1*(surf_clap_dim(NEWOBJ, 1)+surf_clap_dim(NEWOBJ, 3))
            IISURF=NEWOBJ
...
!       TRACK INTENSITY FOR ILLUMINATION RAY TRACING
   ...
   IF((REFRY(11,NEWOBJ)).LT.0.0D0)REFRY(11,NEWOBJ)=REFRY(11,NEWOBJ)+(TWOPII)
   REFRY(12,NEWOBJ)=YYANG
   IF((REFRY(12,NEWOBJ)).LT.0.0D0)REFRY(12,NEWOBJ)=REFRY(12,NEWOBJ)+(TWOPII)
...
!     FROM I-1 TO I
      IF(NUMHITS(I-1).EQ.1.AND.I.GE.1) THEN
         REFRY(8,I)=DSQRT(((REFRY(3,I)-ZOLD)**2)+((REFRY(2,I)-YOLD)**2)+((REFRY(1,I)-XOLD)**2))
      ELSE
...
!       I-1 WAS AN NSS
         ...
         DO N_HITS=1,NUMHITS(I-1)
            STEPL=DSQRT((((MULTIREF_DATA(3,I-1,N_HITS))-ZOLD)**2)+(((MULTIREF_DATA(2,I-1,N_HITS))-YOLD)**2)+(((MULTIREF_DATA(1,I-1,N_HITS))-XOLD)**2))
...
!
      ...
      END IF
      IF(INT(WW4).GE.1.AND.INT(WW4).LE.5)REFRY(7,I)=REFRY(8,I)*DABS(ALENS(45+INT(WW4),(I-1)))
      IF(INT(WW4).GE.6.AND.INT(WW4).LE.10)REFRY(7,I)=REFRY(8,I)*DABS(ALENS(65+INT(WW4),(I-1)))
...
!
      ...
      ELSE
         ...
         ELSE
            ...
            END IF
            IF((REFRY(11,I)).LT.0.0D0)REFRY(11,I)=REFRY(11,I)+(TWOPII)
         END IF
      END IF
      IF(M.EQ.0.0D0) THEN
      ...
      ELSE
         ...
         ELSE
            ...
            END IF
            IF((REFRY(12,I)).LT.0.0D0)REFRY(12,I)=REFRY(12,I)+(TWOPII)
         END IF
      END IF
      REFRY(13,I)=LN
      REFRY(14,I)=MN
...
!       CALCULATE TARX AND TARY
         IF(surf_clap_type(I) >= 1.AND.surf_clap_type(I) <= 6.AND.surf_array_parity(I) == 0) THEN
...
!     TO HIT THE IMAGE SURFACE AT THE CORRECT POINT
   !call logger%logTextWithNum("sys_rxim_fang_set() is ", sys_rxim_fang_set())
   ...
   IF(sys_rxim_fang_set().NE.0.0D0.AND.sys_ryim_fang_set().NE.0.0D0.AND.ERRORVEC.GT.CAIMTOL) THEN
...
!     CURRENT X AND Y VALUES ON IMAGE SURFACE
         IF(sys_rxim_fang_set().EQ.-1.0D0)CRXLAST=REFRY(1,NEWIMG)
         IF(sys_rxim_fang_set().EQ.1.0D0)CRXLAST=REFRY(11,NEWIMG)*180.0D0/PII
         IF(sys_ryim_fang_set().EQ.-1.0D0)CRYLAST=REFRY(2,NEWIMG)
         IF(sys_ryim_fang_set().EQ.1.0D0)CRYLAST=REFRY(12,NEWIMG)*180.0D0/PII
         OLDCX=CRXLAST
         OLDCY=CRYLAST
!     SET INDEPENDENT VARIABLE VALUES
         ...
         CD22=0.0D0
         CDET=0.0D0
         CALL CRAYDERIV(CX1LAST,CY1LAST,CX1ONE,CY1ONE ,CRXONE,CRYONE,CRXLAST,CRYLAST,CD11,CD12,CD21,CD22)
      END IF
...
!
      ...
      CALL CNEWDEL(CMF1,CMF2,CD11,CD12,CD21,CD22,CDELFAIL,CDDELX,CDDELY)
      IF(CDELFAIL) THEN
         GO TO 14
      ELSE
         GO TO 9898
