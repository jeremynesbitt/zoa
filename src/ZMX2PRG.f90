...
!
   ...
   IF(SN.EQ.1.OR.SQ.EQ.1) THEN
      ...
      & '"ZMX2PRG" TAKES NO NUMERIC OR QUALIFIER INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SST.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"ZMX2PRG" REQUIRES EXPLICIT STRING (FILE NAME) INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   ZMXFILENAME=WS
   DO I=80,1,-1
      ...
      END IF
   END DO
   CALL REPORT_ERROR_AND_FAIL('INVALID (ZERO LENGTH) ZEMAX FILE NAME'//'\n'//'NO ACTION TAKEN', 1)
   RETURN
10 CONTINUE
   ZMXFILENAMELENGTH=N
   EXIS37=.FALSE.
...
75 CONTINUE
   CALL REPORT_ERROR_AND_FAIL(&
   & 'ERROR READING ZEMAX INPUT FILE'//'\n'//&
