...
SUBROUTINE INITKDP
   ...
   use zoa_logger
   use codeV_commands, only: initializeCmds
   use glass_manager
   use optim_types
...
!
   ...
   LOGICAL ITERROR
!
!       THIS IS CALLED BY THE UI at program initialization
!
...
!
   character (len=255) :: cwd, homedir
   REAL*8 VIEROT
   INTEGER :: macID
!
   INTEGER OLDOUTOLD,FOTLIM,VIEXOF,VIEYOF,N3,OIN,OLD_JUL,NOW_JUL ,JULDAY1,BIG_GUY,IREND
!
   EXTERNAL JULDAY1
!
...
!
   CHARACTER MCNAM1*8,FILNAM*10,DOSKEY(1:100)*140,DDDAT*10,TTTIM*8,MYED*8 ,DMNAM*10,STRUC*140,AI4*4,COMLINE*140
!
...
!
   CHARACTER SPL*140,AI*3,KKDP*3 ,BLANK*80,BL140*140 ,DRWNAM*11
!
   COMMON/DRWTAG/DRWNAM
!
   INTEGER J,I,K,L,M, IDOS ,FLG(0:20),IH,MAXPAG ,LINBOT,LINCUR,KLI
!
   LOGICAL DEG,RAD,TANG
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
   LOGICAL OPEN11,OPEN12,OPEN13,EXISMC, EXISTFP
!
   COMMON/FFL/FLG
!
   COMMON/STAT13/LINCUR,LINBOT,MAXPAG
!
   CHARACTER HCOL(0:9)*12,HCOL2(0:9)*12,YN*1,HROW(0:100)*12,HROW2(0:100)*12,WCOLD*8
!
...
!
   ...
   PRINT *, "File Path is ", trim(basePath)

   !EXISTFP=.FALSE.
   !INQUIRE(FILE='tstFilePath.txt',EXIST=EXISTFP)


   !call logger%logText('Got to line 140')
   !write(1,*) 'File Path Test Result is ', EXISTFP
   !INQUIRE(FILE='Info.plist',EXIST=EXISTFP)
   !write(1,*) 'File Path Test Result is ', EXISTFP
   !Call GETARG(0,basePath)
   !write(1,*) 'Base Path is ', basePath


   !PRINT *, "Desired file path is ", getZoaPath()
   !  close(1)
!
!     NO NSS SURFACES
   call set_sys_nss(0.0D0)
!
...
!       REAL*8 = 2*4=8 BYTES
!               NRECL=4
!       INITIALIZE NRECL TO 1 FOR CVF AND VAX (NUMBER OF 4 BYTE WORDS)
!       REAL*8 = 2*1=2 WORDS
   NRECL=1
!
...
!
   NOCOBSPSF=.FALSE.
!     SET PROGRAM RANDOM NUMBER GENERATOR
   SN=0
   SQ=0
   SST=0
   STI=0
...
!
!     PSF LI DEFAULT
   PSFLI='THIS IS AN ASCII REPRESENTATION OF A POINT SPREAD FUNCTION'
!
   CAPDEF=16
...
!     SET DEFAULT RSPH TO CHIEF
   REFLOC=1
!
!     (FNAMED initialization removed - binary macro system removed)
   DRWNAM='NEUTRAL.DAT'
...
!       107 USED FOR PROFIT.DAT 7/20/2005
!       108 USED FOR PROFIT BINARY LIBRAY FILE 7/20/2005
!       109 USED FOR PROFIT BINARY OUTPUT FILES 7/20/2005
...
!
   ...
   NUMHITS(0:MAXSUR)=1
!     REMEMBER TO SET THE ABOUT BOX AND THE PROGRAM NAME MESSAGE CORRECTLY IN GUICODE.FOR AND HARDWAR3.FOR
!
...
!       (TF macro trace flags removed with binary macro system)
!
...
!
   ...
   F99=0
   F100=0
!     PIVAXIS MODE ALWAYS STARTS AS "VERTEX" BECAUSE sys_verbose_optim() ALWAYS
!     STARTS AS 0
!
...
!
   ...
   ! = "GOT TO LINE 1218 IN INITKDP!"
   !write(1,*) 'Got to line 1227'
   !CALL SHOWIT(19)
!
!***********************************************************************
!
...
!
!***********************************************************************
!       (Binary macro function loading removed - binary macro system removed)

!                        PRINT *, "Got to line 1301"
!
!***********************************************************************
!
!                       PLOT INITIALIZATION
!     IS PERFORMED BT PLTRST.FOR WHEN ENTERING THE GRAPHIC MODE
...
!
   ...
   !write(1,*) 'Got to line 1385'
   CALL NAMES
   !write(1,*) 'Got to line 1387'
!
   PRINT *, "POST NAMES"
   IN  = 5
   OUT = 6
...
!       INSTRUCTIONS THERE.
!
   OPEN(UNIT=16,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=trim(basePath)//'DEFAULTS.DAT',STATUS='UNKNOWN')
   DO I=1,99999
      READ(UNIT=16,FMT=100,END=9887,ERR=9877) INPUT(1:140)
      MULTICOM=.TRUE.
      !write(1,*) 'Got to line 1410'
      CALL PROCES
      MULTICOM=.FALSE.
   END DO
...
!       LOAD INITIAL LENS DATA
   ...
   OPTMES=.FALSE.
   EXIS89=.FALSE.
   INQUIRE(FILE=trim(CURLENS)//'LENSTEXT.DAT',EXIST=EXIS89)
   IF(EXIS89) THEN
...
3141  READ(UNIT=89,FMT=100,END=8888,ERR=8887) INPUT(1:140)
      !PRINT *, "READ CURLENS INPUT IS ", INPUT(1:140)
      ...
      IF(INPUT(1:8).EQ.'FLDSRAYS') THEN
         ...
         DO I=1,200
            READ(89,*) AI,FIELDY(I),FIELDX(I),FIELDZ(I),N3
            FIELDW(I)=DBLE(N3)
      ...
      ELSE
         ...
         ELSE
!     SKIP COMMAND EXECUTION - LENS IS FULLY RESTORED FROM currlens.zoa BELOW.
!     LENSTEXT.DAT IS ONLY READ FOR FLDSRAYS (RAY GRID) DATA.
         END IF
      END IF
      GO TO 3141
...
!     NO LENSTEXT EXISTS
   END IF
!     ALWAYS RESTORE LENS FROM NEW-FORMAT SAVE FILE (currlens.zoa).
!     THIS RUNS WHETHER OR NOT LENSTEXT.DAT WAS PRESENT, AND PROVIDES
...
!     OPEN AND PROCESS CONTENTS
         OPEN(UNIT=44,ACCESS='SEQUENTIAL',FILE=basePath//'BATCH.DAT',FORM='FORMATTED',STATUS='UNKNOWN')
         REWIND(UNIT=44)
         ...
         DO I=1,99999
            ...
            IF(INPUT(1:3).NE.'OUT') THEN
               ...
               IF(INPUT(1:8).EQ.'FLDSRAYS') THEN
                  ...
                  IF(INPUT(10:13).EQ.'5000') IREND=5000
                  DO J=1,200
                     READ(44,*) AI,FIELDY(I),FIELDX(I),FIELDZ(I),N3
                     FIELDW(I)=DBLE(N3)
                     IF(FIELDW(I).EQ.0.0D0) THEN
                        FIELDW(I)=sys_wl_ref()
                     END IF
...
!
   ...
   call logger%logText('End of INITKDP')


   !call PROCESKDP('SANDBOX')
   !call PROCESKDP('EXIT')
