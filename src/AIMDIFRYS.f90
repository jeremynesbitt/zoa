...
!       AND STOP SEARCHING.
   IF(KKK.GT.NRAITR) THEN
      OUTLYNE='WARNING:'
      CALL SHOWIT(1)
      OUTLYNE='CHIEF DIFFERENTIAL REFERENCE RAY (XZ-PLANE) FAILURE:'
      CALL SHOWIT(1)
...
!
         ...
         IF(DSQRT(((TARX-X)**2)+((TARY-Y)**2)).LE.(HIT).AND.KKK.GT.1 .OR.sys_ray_aiming().EQ.0.0D0) THEN
...
!       AND STOP SEARCHING.
   IF(KKK.GT.100) THEN
      OUTLYNE='WARNING:'
      CALL SHOWIT(1)
      OUTLYNE='CHIEF DIFFERENTIAL REFERENCE RAY (YZ-PLANE) FAILURE:'
      CALL SHOWIT(1)
      OUTLYNE='RAY FAILED TO CONVERGE TO RAY-AIM POINT'
      CALL SHOWIT(1)
      STOPP=1
