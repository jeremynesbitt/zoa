...
!
   real(real64) SPDELX,SPDELY,DELZ,MTHETA
!
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      ...
      &'"REDSUMSQ"'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!     NUMERIC WORD #1
   IF(DF1.EQ.1) THEN
...
!
   COMMON/REDEXT/EXTRED,EXTKED
!
   real(real64) ABC,EMAX,THETA,XVAL,YVAL,&
...
!     STRING INPUT
   IF(SST.EQ.1.AND..NOT.SUMMIT) THEN
      CALL REPORT_ERROR_AND_FAIL('"REDSQ" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SST.EQ.1.AND.SUMMIT) THEN
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('"SREDSQ" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!     MUST BE ACC OR CACC
         CALL REPORT_ERROR_AND_FAIL(&
         & 'EXPLICIT NUMERIC WORD #1 INPUT REQUIRED WHEN THE'//'\n'//&
         & 'QUALIFIER IS "ACC", "CACC", "ACCX" OR "CACCX"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!
   IF(WC(1:1).EQ.'S') THEN
      ...
      IF(.NOT.EXIS32) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SUMMED SPOT DIAGRAM FILE "SPOTS.DAT"'//'\n'//&
         & 'DOES NOT EXIT'//'\n'//&
         & 'NO ACTION TAKEN', 1)
         RETURN
      END IF
...
!
   REAL GDTARP(1:101),GDTAR(1:101)
!
   real(real64) JA,JB,SPDELX,SPDELY,DELZ,MTHETA
!
   ...
   IF(WC.NE.'ESED'.AND.WC.NE.'SESED') THEN
      IF(WQ.EQ.'CENT'.OR.WQ.EQ.'CACC'.OR.WQ.EQ.'CACCX') THEN
         IF(DF3.EQ.0.OR.DF4.EQ.0.OR.DF5.EQ.0) THEN
            ...
            IF(SUMMIT) CALL SHOWIT(1)
            CALL REPORT_ERROR_AND_FAIL(&
            & 'TAKE NO NUMERIC WORD 3, 4 OR 5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      ELSE
...
!     NUMERIC WORD 5
   ...
   ELSE
      IF(DF5.EQ.0) THEN
         ...
         &'AND "ESED CACC"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('TAKE NO NUMERIC WORD #5 INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
9898 CONTINUE
...
!
   REAL GDTARP(1:101),GDTAR(1:101)
!
   real(real64) PSF,SPDELX,SPDELY,DELZ,MTHETA
!
   INTEGER IE,ENNL,UNITTT,IIRED
!
   COMMON/REDPASS/ENNL,GDTARP,GDTAR,MTHETA,SPDELX,SPDELY,DELZ
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"DRED" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'N') THEN
      SQ=0
...
!     MUST BE ACC
   ...
   IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"DRED" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
...
!
   REAL GDTARP(1:101),GDTAR(1:101)
!
   real(real64) X,Y,PSF,SPDELX,SPDELY,DELZ,MTHETA
!
   INTEGER IE,ENNL,UNITTT
!
   COMMON/REDPASS/ENNL,GDTARP,GDTAR,MTHETA,SPDELX,SPDELY,DELZ
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"DREDSQ" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'N') THEN
      SQ=0
...
!
   IF(WC.EQ.'OPSPDRST') THEN
      ...
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPSPDRST" TAKES NO ADDITIONAL INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
!       INPUT OK
...
!     NOT WC=OPSPDRST
   ...
   IF(WC.EQ.'OPSPOT') THEN
      ...
      IF(SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPSPOT" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPSPOT" REQUIRES EXPLICIT QUALIFIER INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     NOT WC=OPSPOT
   ...
   IF(WC.EQ.'OPRINGS') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPRINGS" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     CHECK FOR OVERFLOW
   ...
   IF(WC.EQ.'OPRECT') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPRECT" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
...
!     INPUT OK
   ...
   IF(WC.EQ.'OPRANNUM') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPRANNUM" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
...
100         FORMAT(2X,I2,2X,G13.6,4X,I4,3X,G13.6)
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"OPRING" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S5.EQ.1) THEN
...
!
   IF(WC.EQ.'SPDRESET') THEN
      ...
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPDRESET" TAKES NO ADDITIONAL INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
!       INPUT OK
...
!     NOT WC=SPDRESET
   ...
   IF(WC.EQ.'SPOT') THEN
      ...
      IF(SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPOT" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPOT" REQUIRES EXPLICIT QUALIFIER INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WQ.NE.'RECT'.AND.WQ.NE.'RING'.AND.WQ.NE.'RAND') THEN
...
!     NOT WC=SPOT
   ...
   IF(WC.EQ.'RINGS') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"RINGS" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
...
!     INPUT OK
   ...
   IF(WC.EQ.'RECT') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"RECT" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
   ...
   IF(WC.EQ.'RANNUM') THEN
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"RANNUM" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
...
100         FORMAT(2X,I2,2X,G13.6,4X,I4,3X,G13.6)
      ...
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"RING" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S5.EQ.1) THEN
...
!     NOT WC=RINGS
   ...
   IF(WC.EQ.'APOD') THEN
      ...
      END IF
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"APOD" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.0) THEN
         OUTLYNE=&
