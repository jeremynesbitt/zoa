...
1300 FORMAT('**************************************************',&
   ...
   IF(.NOT.SECONDCALL) THEN
      ...
      &VTYPE.GE.160.AND.VTYPE.LE.164) THEN
...
1502  FORMAT('TOLERANCE VARIABLE CHANGE VALUE = ',G15.8,1X,A8)
   ...
   &VTYPE.GE.11.AND.VTYPE.LE.133.OR.VTYPE.EQ.138.OR.&
   &VTYPE.EQ.139.OR.VTYPE.EQ.140.OR.VTYPE.GE.141.0D0.AND.&
   &VTYPE.LE.164.0D0) THEN
...
9321 FORMAT(' ')
1001 FORMAT(&
   &'(INVERSE SESITIVITY TOLERANCE VARIABLE CHANGE PROJECTION)')
2001 FORMAT(&
   &'(INVERSE SESITIVITY TOLERANCE VARIABLE CHANGE PROJECTION)')
   WRITE(OUTLYNE,1001)
   CALL SHOWIT(0)
...
1502  FORMAT('TOLERANCE VARIABLE CHANGE VALUE = ',G15.8,1X,A8)
   ...
   IF(VTYPE.GE.3.AND.VTYPE.LE.8.OR.&
   &VTYPE.GE.11.AND.VTYPE.LE.133.OR.VTYPE.EQ.138.OR.&
   &VTYPE.EQ.139.OR.VTYPE.EQ.140.OR.VTYPE.GE.141.0D0.AND.VTYPE &
   &.LE.164.0D0) THEN
601   FORMAT('TOLERANCE VARIABLE CHANGE VALUE = ',G15.8,1X,A8)
1601  FORMAT('TOLERANCE VARIABLE CHANGE VALUE = ',G15.8,1X,A8)
      CALL VUNITS(PNAME,VUNI)
...
1803 FORMAT('TOLERANCE OPERAND(#',I3,') = ',&
   ...
   DO I=1,MAXTOP
      ...
      IF(ISTOP(I)) THEN
         ...
         &CALL SHOWIT(0)
         IF(OPERDESC(II)(1:8).NE.'        ')&
         &WRITE(OUTLYNE,4701) OPNAM(II),OPERDESC(II)(1:69)
      END IF
   END DO
!     ARE THERE COMPENSATORS
   CRITTERS=.FALSE.
...
1808 FORMAT('FOCRIT(#',I3,') = ',&
   ...
   DO I=1,MAXFOCRIT
      IF(ISCRIT(I)) THEN
         ...
         &CALL SHOWIT(0)
         IF(OPERDESC(I)(1:8).NE.'        ')&
         &WRITE(OUTLYNE,4701) OPNAM(I),OPERDESC(I)(1:69)
      END IF
   END DO
901 FORMAT('(COMPENSATOR VARIABLE DATA)')
1901 FORMAT('(COMPENSATOR VARIABLE DATA)')
...
!     NOW OPEN SENSIOUT.DAT
   ...
   &,FORM='FORMATTED',FILE='SENSIOUT.DAT'&
   &,STATUS='UNKNOWN')
!
...
!     NOW OPEN ISENSOUT.DAT
   OPEN(UNIT=19,ACCESS='APPEND',BLANK='NULL'&
   &,FORM='FORMATTED',FILE='ISENSOUT.DAT'&
   &,STATUS='UNKNOWN')
!
!     PRINT HEADER INFORMATION
!
...
!     HERE THE CHANGES ARE COMPUTED.
   DO II=1,MAXFOCRIT
      ...
      &(((NCRIT(II)-OCRIT(II)))/OCRIT(II))*100.0D0
...
!
   IF(STI.EQ.1) THEN
      ...
      &'"SENSI" INITIATES A SENSITIVITY ANALYSIS'
      IF(ITY.EQ.1) WRITE(OUTLYNE,*)&
      &'"INVSENSI" INITIATES AN INVERSE SENSITIVITY ANALYSIS'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
...
!     CHECK THAT STUFF EXISTS
   IF(TVBCNT.EQ.0) THEN
      IF(ITY.EQ.0) WRITE(OUTLYNE,*)&
      &'"SENSI" REQUIRES TOLERANCE VARIABLES TO BE DEFINED'
      IF(ITY.EQ.1) WRITE(OUTLYNE,*)&
      &'"INVSENSI" REQUIRES TOLERANCE VARIABLES TO BE DEFINED'
      CALL SHOWIT(1)
...
!     COMPYES TRUE MEANS WE CALCULATE FOCRITS IN OPCALC
   ...
   IF(.NOT.TOLYES.AND.TVBCNT.NE.0) THEN
      IF(ITY.EQ.0) WRITE(OUTLYNE,*)&
      &'"SENSI" REQUIRES TOLERANCE OPERANDS TO BE DEFINED'
      IF(ITY.EQ.1) WRITE(OUTLYNE,*)&
      &'"INVSENSI" REQUIRES TOLERANCE OPERANDS TO BE DEFINED'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(TVBCNT.EQ.0.AND..NOT.TOLYES) THEN
      IF(ITY.EQ.0) WRITE(OUTLYNE,*)&
      &'"SENSI" REQUIRES TOLERANCE VARIABLES AND OPERANDS TO BE DEFINED'
      IF(ITY.EQ.1) WRITE(OUTLYNE,*)&
      &'"SENSI" REQUIRES TOLERANCE VARIABLES AND OPERANDS TO BE DEFINED'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO ACTION TAKEN'
   ...
   DO I=1,MAXTOP
      ...
      IF(ISTOP(I))THEN
         IF(OPERND(II,20).EQ.0.0D0.AND.ITY.NE.0) THEN
            WRITE(OUTLYNE,*)&
            &'AN EXPLICIT "tval" MUST BE ENTERED FOR EACH TOLERANCE OPERAND'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)&
            &'BEFORE AN INVERSE SENSITIVITY ANALYSIS CAN BE PERFORMED'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
...
1309        FORMAT(&
            &'WARNING: TOLERANCE DATA FOR NEXT VARIABLE IS IN ERROR')
         END IF
         IF(ITY.EQ.0) CALL SENSIOUT
         IF(ITY.EQ.1) CALL INVSENSIOUT(I)
!     DONE FOR VARIABLE I PLUS
...
!                       MINUS DELTA DONE
      ...
      &25.OR.VTYPE.EQ.75.OR.VTYPE.GE.124.AND.VTYPE.LT.133.OR.VTYPE &
      &.EQ.138.OR.VTYPE.GE.141.0D0.AND.VTYPE.LE.153.0D0.OR.&
      &VTYPE.EQ.139.OR.VTYPE.EQ.140) THEN
!
!     PLUS DELTA
!     NEW VALUE IS:
!     UPDATE THE LENS
...
!       NOW DO WC=DEL
   IF(WC.EQ.'DELK') THEN
      IF(F52.NE.2) THEN
         WRITE(OUTLYNE,*)&
         &'"DEL" IS ONLY AVAILABLE FROM THE "UPDATE COMPVAR" LEVEL'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(.NOT.ISCOMP(INT(W1))) THEN
         WRITE(OUTLYNE,*)&
         &'COMPENSATION VARIABLE ',INT(W1),' NOT CURRENTLY DEFINED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     START DOING THE VARIABLE NAMES HERE
   IF(WC.EQ.'RD') THEN
      WRITE(OUTLYNE,*)&
      &'"RD" IS A POORLY BEHAVED VARIABLE AND WILL BE AUTOMATICALLY'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'CONVERTED TO CURVATURE "CV" WHICH IS BETTER BEHAVED.'
      CALL SHOWIT(1)
      WC(1:2)='CV'
   END IF
   IF(WC.EQ.'RDTOR') THEN
      WRITE(OUTLYNE,*)&
      &'"RDTOR" IS A POORLY BEHAVED VARIABLE AND WILL BE AUTOMATICALLY'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'CONVERTED TO CURVATURE "CVTOR" WHICH IS BETTER BEHAVED.'
      CALL SHOWIT(1)
      WC(1:5)='CVTOR'
   END IF
   IF(WC.EQ.'RD      ') VALT=1
...
!     DEFAULT INPUT
      WRITE(OUTLYNE,*)&
      &'NUMERIC WORD #1 (COMPENSATOR NUMBER) MISSING OR'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'BEYOND LEGAL BOUNDS'
      CALL SHOWIT(1)
...
!     DEFAULT INPUT
      ...
      &WC.EQ.'AG'.OR.WC.EQ.'AH'.OR.WC.EQ.'AI'.OR.&
      &WC.EQ.'AJ'.OR.WC.EQ.'AK'.OR.WC.EQ.'AL'.OR.WC.EQ.'AC') THEN
         GOFORIT=.FALSE.
...
!     AT THE CONTROL WAVELENGTH
            ...
            IF(WC.EQ.'AG') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**10)
            END IF
            IF(WC.EQ.'AH') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**12)
            END IF
            IF(WC.EQ.'AI') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**14)
            END IF
            IF(WC.EQ.'AJ') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**16)
            END IF
            IF(WC.EQ.'AK') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**18)
            END IF
            IF(WC.EQ.'AL') THEN
               DINCR=WAVERCW/&
               &((DABS(HM1)+DABS(HC1))**20)
            END IF
         ELSE
...
!     DON'T USE SWANTNER'S ALGORITHM FOR THE DEFAULT
   ...
   IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'COMPENSATOR INPUT USES NO NUMERIC WORD #4 OR #5 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
         ...
         &VARABL(INT(W1),4)=FTFL01(VALT-27,VBSURF)
!
         ...
         IF(VALT.GE.76.AND.VALT.LE.123)&
         &VARABL(INT(W1),5)=FTFL01(VALT-27,VBSURF)
         ...
         IF(VALT.GE.76.AND.VALT.LE.123)&
         &VARABL(INT(W1),13)=FTFL01(VALT-27,VBSURF)
         ISCOMP(INT(W1))=.TRUE.
!
      END IF
      RETURN
...
!
   IF(STI.EQ.1.AND.SQ.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'"DINCR" SETS DEFAULT DINCR VALUES FOR CLASSES OF VARIABLES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'AND TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      RETURN
   END IF
...
654   FORMAT('DINCR   ',A8,' = ',D23.15)
   ...
   IF(SST.EQ.1.OR.S2.EQ.1.OR.3.EQ.1.OR.S4.EQ.4.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"DINCR" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.0.OR.S1.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'"DINCR" REQUIRES EXPLICIT QUALIFIER AND NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
...
!
   ...
   IF(DINO) RETURN
   WRITE(OUTLYNE,*)&
   &'INVALID QUALIFIER ISSUED WITH "DINCR"'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
   CALL SHOWIT(1)
   CALL MACFAL
...
FUNCTION JACME(NUB,PCOM,XICOM)
   ...
   &,FX,X,XMIN,FDEL1,FDEL2,FDEL3,DX1,DX2,DX3,DX12,DX22,DX32 &
   &,AXX,BXX,CXX,AAEE,BBEE,CCEE,PCOM,XICOM
...
!     TWO VALUES EQUAL
   ...
   &BX.EQ.CX.AND.AX.NE.CX.OR.&
   &AX.NE.BX.AND.AX.EQ.CX.OR.&
   &BX.NE.CX.AND.AX.EQ.CX) THEN
!
      IF(AX.EQ.BX.AND.AX.NE.CX.OR.&
      &BX.EQ.CX.AND.AX.NE.CX) AXX=(AX+CX)/2.0D0
      IF(AX.NE.BX.AND.AX.EQ.CX.OR.&
      &BX.NE.CX.AND.AX.EQ.CX) AXX=(BX+CX)/2.0D0
      JKX=AXX
      FAX=F1DIM(NUB,PCOM,XICOM)
      XMIN=AXX
      JACME=FAX
...
!
   REAL*8 &
   &X(1:100000),V1,MAX,VTEMP,OLDCUR,NEWCUR &
   &,N1
!
   LOGICAL LVAL
!
...
!
   IF(OPCNT.EQ.0.OR.VBCNT.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'"ITER POWL" OR "IT P" OR "ITER POWELL"'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'REQUIRES VARIABLES AND OPERANDS TO EXIST'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
