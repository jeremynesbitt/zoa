...
!
   real(real64) FNXP,FNYP,FNZP,MAG,XP,YP,ZP,NSSDELX,NSSDELY
!
   real(real64) CENTERX,CENTERY,CENTERZ
!
   real(real64) XCORR,YCORR,NEWRRX,NEWRRY,NEWRRZ,XERROR,YERROR
!
!
...
!       IN ORDER TO KEEP RAY TRACING SPEED FAST. ADDED 4/22/2004
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) X,Y,A2,A4,A6,A8,A10,LN,MN,MM,NX,NY,SGN,Q
   INTEGER J
   LOGICAL PERR
...
SUBROUTINE NSS_RAY_LENGTH(OX,OY,OZ,NX,NY,NZ,INDEX)
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) OX,OY,OZ,NX,NY,NZ,INDEX
   NSS_RAYLENGTH=&
   &DSQRT(((NX-OX)**2)&
   &+((NY-OY)**2)&
   &+((NZ-OZ)**2))
...
SUBROUTINE NSSSTATS
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) SUMLX,SUMLY,SUMLR,OX,OY,OZ,NX,NY,NZ,INDEX
   real(real64) SUML2X,SUML2Y,SUML2R
   real(real64) NSSOPL,NSSLEN
