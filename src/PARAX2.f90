!       SECOND SET OF PARAXIAL ROUTINES GO HERE

! SUB AB357.FOR
SUBROUTINE AB357
!
!       NOTE: THIS SUBROUTINE WILL REPLACE SA3CAL.FOR
!       AND ADD 5TH ORDER ABERRATION CALCULATIONS AS WELL AS
!       SA7. THESE EQUATIONS HAVE BEEN MODIFIED TO PROPERLY
!       CALCULATE ABERRATIONS USING FLAT SURFACES WITH A
!       SECOND ORDER TERM (SCHMIDT CORRECTOR PLATES ETC.)
!
!       THIS SUBROUTINE CALCULATES THE 3RD AND 5TH ORDER
!       SIDEL ABERRATION COEFICIENTS (AND SA7) AND LOADS THEM
!       INTO MAB3(1:11,0:MAXSURF) AND MAB57(1:20,0:MAXSURF)
!       THE SURFACE SUMS ARE CALCULATED ON THE FLY
!       WHEN A PARTICULAR PRINT OUT IS REQUESTED USING
!       THE CMD LEVEL COMMANDS MAB3,MAB5 OR MABX5.
!       LENS MODE IS TAKEN INTO ACCOUNT
!       MAB3(1:5,1:MAXSURF) CONTAINS THE MARGINAL COEFFICIENTS
!       MAB3(6:10,0:MAXSURF) CONTAINS THE CHIEF RAY COEFFICIENTS
!       CHIEF RAY COEFFICIENTS ARE CALCULATED ONLY FOR
!       THIRD ORDER ABERRATIONS. THIRD ORDER ABERRATIONS CONSIST
!       OF A SPHERICAL PART AND A PART CONTRIBUTED BY THE CONIC
!       CONSTANT AND THE 4TH ORDER ASPHERIC TERM.
!
!       THE SURFACE CONTRIBUTIONS OF 5TH AND 7TH ORDER ARE KEPT
!       SEPARATELY IN ARRAY SAB57 AND PASSED VIA COMMON/SAB
!
!       THIS IS DONE FOR THE
!       (Y-Z PLANE) AND XZ PLANE
!
...
!       SURFACE IS X-TORIC
         ...
         C2BAR=C2*(N-J_NP)
!
!       NOW C1,C2,C3 C1BAR AND C2BAR ARE DONE
!
!       NOW: THIRD ORDER ABERRATIONS INCLUDING ASPHERIC TERM
!       THIRD ORDER SPHERICAL (SA3) SURFACE
!       CONTRIBUTION (SURFACE I)
         !PRINT *, "I is ", I
         !PRINT *, "SI is ", SI
...
!
!       THIRD ORDER COMMA (CMA3) SURFACE
!       CONTRIBUTION (SURFACE I)
!
...
!
!       THIRD ORDER ASTIGMATISM (AST3) SURFACE CONTRIBUTION
!       AT SURFACE I
!
         C=SI*((PXTRAY(7,I))**2)
...
!       X-TORIC
         ...
         END IF
!
!       NOW FOR THE 3RD ORDER EXIT PUPIL ABERRATIONS
!
!       THIRD ORDER SPHERICAL (SA3) SURFACE
!       CONTRIBUTION (SURFACE I) (EXIT PUPIL)
!
...
!
!       THE S(HAT) TERMS USED FOR THE SHERICAL PART OF THE SURFACE
!       CONTRIBUTIONS
!
!       S1P2
...
10    CONTINUE
!
!       BEFORE CALCULATING THE TRANSFERED PLUS SURFACE CONTIBUTIONS,
!       STORE THE SURFACE CONTRIBUTIONS IN THE ARRAY SAB57
!       THIRD ORDER IS ALWAYS JUST SURFACE CONTRIBUTIONS BY DEFAULT.
!
      I=INT(sys_last_surf())
      SAB57(1:20,0:I)=MAB57(1:20,0:I)
!       NOW THE SURFACE CONTRIBUTIONS ARE AVAILABLE LATER.
!
!
!       NOW ALL OF THE INTRINSIC SURFACE CONTRIBUTIONS HAVE BEEN
!       CALCULATED
!       NOW CALCULATE THE COMPLETE ABERRATION COEFICIENTS
!       FOR EACH SURFACE
!
!       THIRD ORDER ALREADY ARE COMPLETE AS THEY HAVE NO TRANSFERRED
!       PART
!       FIFTH ORDER, START AT SURF 1 AS ALL SURFACE 0 COEFFS ARE
!       ALWAYS ZERO
!       SET ALL PRIMED VARIABLES TO 0.0
      BPRIME=0.0D0
      FPRIME=0.0D0
      CPRIME=0.0D0
      EPRIME=0.0D0
      ...
      B5PRIM=0.0D0
      EB5PRM=0.0D0
      DO 100 I=1,INT(sys_last_surf())
!       AT SURFACE I
!       CALCULATE BPRIME,FPRIME,CPRIME,EPRIME,PPRIME, ETC.
!
!       NOTE: ALL SLASH TERMS NOW INCLUDE THE ASPHERIC
!       CONTRIBUTION.
!
...
!       B5=B5SLASH+(3/2INV)*(BPRIME*F-EBARPRIME*B)
!       B5SLASH IS IN MAB57(1,I)
!       WHEN DONE B5 WILL REPLACE B5SLASH
...
!
!       NOW THE SEVENTH ORDER ABERATION FOR SURFACE I
!       IS:
!       B7=B7SLASH+(1/2INV)*
!       [(1/2INV)*(B'**2)(PI+3C)+3(B5'-(1/INV)B'EBAR')F+
...
!
!       BEFORE CALCULATING THE TRANSFERED PLUS SURFACE CONTIBUTIONS,
!       STORE THE SURFACE CONTRIBUTIONS IN THE ARRAY XSAB57
!       THIRD ORDER IS ALWAYS JUST SURFACE CONTRIBUTIONS BY DEFAULT.
!
      I=INT(sys_last_surf())
...
!       THE SA357I AND SXA357I (INTRINSIC SURFACE
!       COEFFICIENTS AND THEIR SUMS) CMD LEVEL COMMAND
!
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'SA357I') THEN
         ...
         & '"SA357I" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   ...
   IF(sys_last_surf().EQ.0.0) THEN
      ...
      & 'NO PARAXIAL,3RD, 5TH OR 7TH  ORDER DATA EXISTS', 1)
      RETURN
   END IF
!
   SF=INT(sys_last_surf())
...
!       MODE IS AFOCAL
   ...
   IF(INV.EQ.0.0D0) THEN
      ...
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('THEN RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!
   CALL PRTRC
!
!       THE MAB5I AND XMAB5I COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MAB5I') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MAB5I" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMAB5I') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMAB5I" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   ...
   IF(sys_last_surf().EQ.0.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'LENS SYSTEM HAS NO SURFACES'//'\n'//&
      & 'NO PARAXIAL OR FIFTH ORDER DATA EXISTS', 1)
      RETURN
   END IF
!
   SF=INT(sys_last_surf())
...
!
   CALL PRTRC
!
!       THE MABX5I AND XMABX5I COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MABX5I') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MABX5I" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMABX5I') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMABX5I" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MAB3') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MAB3" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMAB3') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMAB3" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   ...
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'LENS SYSTEM HAS NO SURFACES'//'\n'//&
      & 'NO PARAXIAL OR THIRD ORDER DATA EXISTS', 1)
      RETURN
   END IF
!
   SF=INT(sys_last_surf())
...
!
   CALL PRTRC
!
!       THE MAB5 AND XMAB5 COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MAB5') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MAB5" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMAB5') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMAB5" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   CALL PRTRC
!
!       THE MABX5 AND XMABX5 COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MABX5') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MABX5" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMABX5') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMABX5" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   CALL PRTRC
!
!       THE MABP3 COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'MABP3') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"MABP3" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XMABP3') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XMABP3" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   CALL PRTRC
!
!       THE SA357 AND XSA357 COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       "I","IM", AND "IMAGE"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
...
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      IF(WC.EQ.'SA357') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SA357" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'XSA357') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"XSA357" TAKES EITHER QUALIFIER OR NUMERIC INPUT"'//'\n'//&
         & 'BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
