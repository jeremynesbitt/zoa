...
!      PRINCIPLE PLANE DATA CALCULATED WITH THE LAST FIRD COMMAND
    real(real64) PPY1,PPY2,PPX1,PPX2
    COMMON/PPLANES/PPY1,PPY2,PPX1,PPX2
!
    CHARACTER IDTAG(1:10)*75
    COMMON/TAGID/IDTAG
    real(real64) DDATA(1:4,1:2000),CFTYPE(1:96)
    COMMON/SPDATA/DDATA
    COMMON/CFTYP/CFTYPE
    INTEGER MAXSPS
    COMMON/MXSPS/MAXSPS
...
!
    real(real64) SYSTEM(1:150),CFLDS(1:2,1:10),PFLDS(1:2,1:10),           &
...
!       VARIABLES FOR THE PERMANENT LENS USED TO FORM CFGS
 ...
 &,LLTYPEP*80
    real(real64) SYSP(1:150),ALENP(1:160,0:499),PIPOLYX(1:200,0:499,1:4), &
...
!       3,5,7 CHROMATIC DIFFERENCES
    ...
    real(real64) MAB3(1:11,0:499),PXTRAY(1:8,0:499),                      &
 ...
 &  ,O_PXTRAY(1:8,0:499),O_PXTRAX(1:8,0:499)
    ...
    real(real64) COLY(1:8,0:499),COLX(1:8,0:499)

!       CHROMATIC 357 CONTRIBUTIONS

    real(real64) PDF3(1:10,0:499),PDF57(1:20,0:499),                      &
...
!     FOURTH DIMENSION IS WAVELENGTH NUMBER 1 TO 10
  ...
  real(real64) FFAANN(1:3,1:21,1:6,1:10),                                 &
...
!       LFOB(1:7) HOLDS THE LAST NUMERIC DATA USED WITH THE LAST FOB
    real(real64) LFOB(1:7),LFOBA(1:3)
...
!     SIGNS IN OPD CALCS
    ...
    real(real64) SAVE(1:50,0:499),DIFF(1:18,0:499),                       &
 ...
 &  ,OLRFDIFF(1:18,0:499),H_RAY(1:50,0:499)
    ...
    real(real64) GPXY(1:4,0:499)
    COMMON/PXYG/GPXY

    real(real64) QRAY(1:9,0:499)
...
!       NOVIRT = .TRUE., SET IN PROGRAM.FOR
    ...
    real(real64) OFFX,OFFY,OFFZ,OFFA,OFFB,OFFC
    INTEGER GLSURF
    LOGICAL GLOBE
    COMMON/GLOBL1/GLOBE
    COMMON/GLOBL2/OFFX,OFFY,OFFZ,OFFA,OFFB,OFFC,GLSURF

    real(real64) OOFFX,OOFFY,OOFFZ,OOFFA,OOFFB,OOFFC
    INTEGER OLSURF
    LOGICAL OGLOBE
...
!       IN GLOBAL REPRESENTATION
    real(real64) VERTEX(1:12,0:499),GLRAY(1:12,0:499)
...
!       IN GLOBAL REPRESENTATION FOR PLOTTING
  ...
  real(real64) GLPRAY(1:9,0:499),PGLPRAY(1:12,0:499)
...
!     RAYTRACE VARIABLES PASSED WITH TH1S INCLUDE
    ...
    real(real64) DINFAC,ONTOL,SINGTOL                                     &
...
!
    ...
    real(real64) SCFAY,SCFAX,PSIZY,PSIZX                                  &
 &  ,SCFAYP,SCFAXP,PSIZYP,PSIZXP

    real(real64) LOOKY,LOOKX,LOOKZ,VIEALF,VIEPHI
...
!
 ...
 &,FANWV6,FANWV7,FANWV8,FANWV9,FANWV10,FSSIFLG
  ...
  real(real64) FANOFF,YFOB1,YFOB2,YFOB3,                                  &
...
!     NEW COMMONS FOR MARCH 4993
  ...
  COMMON/RAYSUR/ISURI
  CHARACTER WWQ*8
  real(real64) WW1,WW2,WW3,WW4,WW5
  COMMON/RAYWWQ/WWQ
  ...
  real(real64) R_X,R_Y,R_Z,R_L,R_M,R_N,XSTRT,YSTRT,ZSTRT                  &
 ...
 &,YR_M,YR_N,YOLDL,YOLDM,YOLDN
  real(real64) RXL(0:499),RXM(0:499),RXN(0:499)
  real(real64) RYL(0:499),RYM(0:499),RYN(0:499)
 ...
 &,ZZZANG
  ...
  real(real64) R_TX,R_TY,R_TZ
  COMMON/BAKK1/R_TX,R_TY,R_TZ

  real(real64) RCOR,OCOR
  COMMON/PASOPD/RCOR,OCOR

  real(real64) X0,Y0,XT(1:2000),YT(1:2000)
  INTEGER NP
...
!     COMMON FOR PHASE SURFACES
  real(real64) PHASE,DELL,DELM,RLRL,RMRM,RNRN,MYX,MYY
...
!     FFL,BFL AND MAGS BASED ON PARAXIAL OR DIFFERENTIAL TRACE
  ...
  real(real64) RMAGX,RMAGY,RFFLX,RFFLY,RBFLX,RBFLY                        &
...
!     CMD LEVEL PIVOT POSITIONS FOR STILT AND BTILT COMMANDS
  real(real64) STILTXP,STILTYP,STILTZP
  real(real64) BTILTXP,BTILTYP,BTILTZP
...
!     REPRESENTATIONS
  ...
  real(real64) IPLANE_X ,IPLANE_Y ,IPLANE_Z
  real(real64) IPLANE_L ,IPLANE_M ,IPLANE_N
  real(real64) IPLANE_LN,IPLANE_MN,IPLANE_NN
 ...
 &IPLANE_L,IPLANE_M,IPLANE_N,IPLANE_LN,IPLANE_MN,IPLANE_NN
  ...
  real(real64) COS_B_ANG,COS_A_ANG
  COMMON/CARTANG/COS_A_ANG,COS_B_ANG
  real(real64) NEWX,NEWY,NEWZ
...
!     USED BY SAVERAY, RESTRAY AND CLEARRAY COMMANDS
  ...
  real(real64) OLD_REF,OLD_RAY,OLD_REF_DIF,OLD_RAY_DIF                    &
 &,SLFOB,SLFOBA
...
!     REAL HOE STUFF
  ...
  INTEGER HOE_WAV_NUM
  real(real64) XHOE,YHOE,ZHOE,L1HOE,M1HOE,N1HOE,L2HOE,M2HOE,N2HOE
 ...
 &,HOE_X1,HOE_Y1,HOE_Z1,HOE_X2,HOE_Y2,HOE_Z2,HOE_WAV_NUM
  ...
  real(real64) POLANGX(0:499),POLANGY(0:499)
  LOGICAL POLEXT
...
!       see code changes in raytra5.for and raytra6.for
  ...
  real(real64) ABSCOEF(1:10),OLDABSCOEF(1:10)
  COMMON/COEFABS/ABSCOEF,OLDABSCOEF

    real(real64) GUTRAY(1:50,0:499)
...
!       EXIT PORT
    real(real64) LAST_NSS_R_X,LAST_NSS_R_Y,LAST_NSS_R_Z
    real(real64) LAST_NSS_R_L,LAST_NSS_R_M,LAST_NSS_R_N
    COMMON/NSS_LASTRAY/LAST_NSS_R_X,LAST_NSS_R_Y,LAST_NSS_R_Z ,     &
 &  LAST_NSS_R_L,LAST_NSS_R_M,LAST_NSS_R_N

    real(real64) LAST_NSS_OR_X,LAST_NSS_OR_Y,LAST_NSS_OR_Z
    real(real64) LAST_NSS_OR_L,LAST_NSS_OR_M,LAST_NSS_OR_N
...
!       DEFORMABLE MIRROR DATA
  ...
  real(real64) DEFGR1,DEFGR2,DEFGR3,DEFGR4,DEFGR5,DEFGR6                  &
 &,DEFGR7,DEFGR8
  INTEGER ACTNUM
...
!       ARRAY LENS SUPPORT (6/23/2004)
    real(real64) A_R_X,A_R_Y,A_R_Z,A_R_L,A_R_M,A_R_N,OLDARRAYX,OLDARRAYY
