!       THIRD FILE FOR LENS DATABASE MANAGER FILES

! SUB STAD.FOR
SUBROUTINE STAD
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_last_surf
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE STAD WHICH IMPLEMENTS THE TAD
!       COMMAND AT THE CMD LEVEL
!       THE ONLY VALID QUALIFIERS ARE (BLANK) AND "ALL","OB" AND "OBJ"
!       THE NUMERIC ENTRY W1, IS THE SURFACE DESIGNATOR
!       FOR PRINTOUT OF ONE SURFACE'S DATA ONLY
!
   CHARACTER TYPE1*5,TYPE2*3

!
   INTEGER TCNT,DCNT,TAD,DEC,TADCNT
!
!
!               THIS IS A CMD LEVEL COMMAND ONLY
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TAD" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TAD" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      OUTLYNE='NO TILTS OR DECENTERS EXIST'
      CALL SHOWIT(1)
      OUTLYNE='LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE TAD
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE TILT/DEC DATA FOR
!       THE ENTIRE LENS IS PRINTED. IF THE QUALIFIER IS "OB" OR "OBJ"
!       THEN DATA FOR THE OBJECT SURFACE IS PRINTED.
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!
!       THIS IS SAME AS TAD,0
      S1=1
      DF1=0
      SQ=0
      WQ='        '
      W1=0.0
   ELSE
!       WQ NOT OB OR OBJ
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'ALL') THEN
!
!       WE HAVE INVALID QUALIFIER
      CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
!       HANDEL AN INDIVIDUAL SURFACE INCLUDING "OB" AND "OBJ"
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      SURF=INT(W1)
      IF(SURF.GT.(INT(sys_last_surf())).OR.SURF.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
! SET DEC AND TAD APPROPRIATELY
!
      IF(surf_tilt_flag(SURF).NE.0.0D0) THEN
         TAD=1
      ELSE
         TAD=0
      END IF
      IF(surf_decenter_flag(SURF).NE.0.0D0) THEN
         DEC=1
      ELSE
         DEC=0
      END IF
!
!       FIRST PRINT TAD DATA. ONLY ONE KIND OF TAD CAN EXITS
!       ON A SURFACE
!
!               HANDEL NO DATA FOR A SURFACE
      IF(TAD.EQ.0.AND.DEC.EQ.0) THEN
         WRITE(OUTLYNE,100) SURF
         CALL SHOWIT(0)
         RETURN
      END IF
!
!                       DO PRINTING NOW
!*****************************************************************
!
!
!
      IF(surf_tilt_flag(SURF).NE.0.0D0) THEN
         IF(surf_tilt_flag(SURF).EQ.1.0.AND.surf_tilt_return_flag(SURF).EQ.0.0D0) THEN
            TYPE1 =' TILT'
            TYPE2 ='   '
         END IF
         IF(surf_tilt_flag(SURF).EQ.-1.0) THEN
            TYPE1 ='RTILT'
            TYPE2 ='   '
         END IF
         IF(surf_tilt_flag(SURF).EQ.2.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='  A'
         END IF
         IF(surf_tilt_flag(SURF).EQ.3.0) THEN
            TYPE1 =' TILT'
            TYPE2 =' AM'
         END IF
         IF(surf_tilt_flag(SURF).EQ.4.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='BEN'
         END IF
         IF(surf_tilt_flag(SURF).EQ.5.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='DAR'
         END IF
         IF(surf_tilt_flag(SURF).EQ.7.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='REV'
         END IF
         IF(surf_tilt_flag(SURF).EQ.6.0.OR.surf_tilt_flag(SURF).EQ.1.0D0.AND.surf_tilt_return_flag(SURF).EQ.1.0D0) THEN
            TYPE1 =' TILT'
            TYPE2 ='RET'
         END IF
         IF(HEADIN) WRITE(OUTLYNE,2000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(surf_tilt_flag(SURF).EQ.6.0D0.OR.surf_tilt_flag(SURF).EQ.1.0D0.AND.surf_tilt_return_flag(SURF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,2001) SURF,TYPE1,TYPE2,surf_alpha(SURF),surf_beta(SURF),surf_gamma(SURF),INT(surf_ret_surf_num(SURF))
         ELSE
            WRITE(OUTLYNE,200) SURF,TYPE1,TYPE2,surf_alpha(SURF),surf_beta(SURF),surf_gamma(SURF)
         END IF
         CALL SHOWIT(0)
      END IF
      IF(surf_decenter_flag(SURF).NE.0.0D0) THEN
         TYPE1='  DEC'
         TYPE2='   '
!      WRITE(OUTLYNE,1500)
!      CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,3000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,200) SURF,TYPE1,TYPE2,surf_decenter_y(SURF),surf_decenter_x(SURF),surf_decenter_z(SURF)
         CALL SHOWIT(0)
      END IF
!     GLOBAL DECENTERS AND TILTS
      IF(surf_global_dx(SURF).NE.0.0D0.OR.surf_global_dy(SURF).NE.0.0D0.OR.surf_global_dz(SURF).NE.0.0D0.OR.surf_global_alpha(SURF).NE.0.0D0.OR.surf_global_beta(SURF).NE.0.0D0.OR.surf_global_gamma(SURF).NE.0.0D0) THEN
         WRITE(OUTLYNE,9000) SURF
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9001) surf_global_dx(SURF)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9002) surf_global_dy(SURF)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9003) surf_global_dz(SURF)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9004) surf_global_alpha(SURF)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9005) surf_global_beta(SURF)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,9006) surf_global_gamma(SURF)
         CALL SHOWIT(0)
      END IF
      RETURN
   ELSE
!*********************************************************************
!       SQ=1 AND ONLY ALLOWED WQ BY NOW IS "ALL"
!       HANDEL THE WHOLE LENS
!       PRINT HEADING DATA
!
!       SET THE TILT COUNTER. IT IS USED TO DETERMINE
!       THE CASE OF NO TILTS OR DECENTERS IN A LENS
      TADCNT=0
      DCNT=0
      TCNT=0
      DO SURF=0,INT(sys_last_surf())
         IF(surf_tilt_flag(SURF).NE.0.0.AND.surf_decenter_flag(SURF).NE.0.0 .OR.surf_tilt_flag(SURF).NE.0.0.OR.surf_decenter_flag(SURF).NE.0.0D0) THEN
            IF(surf_tilt_flag(SURF).NE.0.0D0) TCNT=TCNT+1
            IF(surf_decenter_flag(SURF).NE.0.0D0) DCNT=DCNT+1
            TADCNT=TADCNT+1
         ELSE
         END IF
      END DO
      IF(TADCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
      WRITE(OUTLYNE,1500)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)
      CALL SHOWIT(0)
      DO SURF=0,INT(sys_last_surf())
!
! SET DEC AND TAD APPROPRIATELY
!
         IF(surf_tilt_flag(SURF).NE.0.0D0) THEN
            TAD=1
         ELSE
            TAD=0
         END IF
         IF(TAD.EQ.0) GO TO 15
!
!       SET THE TYPE1 AND TYPE2 TYPES CORRECTLY
!
         IF(surf_tilt_flag(SURF).EQ.1.0.AND.surf_tilt_return_flag(SURF).EQ.0.0D0) THEN
            TYPE1 =' TILT'
            TYPE2 ='   '
         END IF
         IF(surf_tilt_flag(SURF).EQ.-1.0) THEN
            TYPE1 ='RTILT'
            TYPE2 ='   '
         END IF
         IF(surf_tilt_flag(SURF).EQ.2.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='  A'
         END IF
         IF(surf_tilt_flag(SURF).EQ.3.0) THEN
            TYPE1 =' TILT'
            TYPE2 =' AM'
         END IF
         IF(surf_tilt_flag(SURF).EQ.4.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='BEN'
         END IF
         IF(surf_tilt_flag(SURF).EQ.5.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='DAR'
         END IF
         IF(surf_tilt_flag(SURF).EQ.7.0) THEN
            TYPE1 =' TILT'
            TYPE2 ='REV'
         END IF
         IF(surf_tilt_flag(SURF).EQ.6.0.OR.surf_tilt_flag(SURF).EQ.1.0D0.AND.surf_tilt_return_flag(SURF).EQ.1.0D0) THEN
            TYPE1 =' TILT'
            TYPE2 ='RET'
         END IF
!
!       FIRST PRINT TAD DATA. ONLY ONE KIND OF TAD CAN EXITS
!       ON A SURFACE
!
!                       DO PRINTING NOW
!*****************************************************************
!
         IF(surf_tilt_flag(SURF).EQ.6.0.OR.surf_tilt_flag(SURF).EQ.1.0D0.AND.surf_tilt_return_flag(SURF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,2001) SURF,TYPE1,TYPE2 ,surf_alpha(SURF),surf_beta(SURF),surf_gamma(SURF),INT(surf_ret_surf_num(SURF))
         ELSE
            WRITE(OUTLYNE,200) SURF,TYPE1,TYPE2 ,surf_alpha(SURF),surf_beta(SURF),surf_gamma(SURF)
         END IF
         CALL SHOWIT(0)
15       CONTINUE
      END DO
!
!     DECENTERS
      IF(DCNT.NE.0) THEN
         WRITE(OUTLYNE,1500)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000)
         CALL SHOWIT(0)
!
         DO SURF=0,INT(sys_last_surf())
!
! SET DEC
!
            IF(surf_decenter_flag(SURF).NE.0.0D0) THEN
               DEC=1
            ELSE
               DEC=0
            END IF
!
!
            IF(DEC.EQ.1) THEN
!       YOU GOT HERE BECAUSE DEC=1 NOT BECAUSE TAD=1
               TYPE1='  DEC'
               TYPE2='   '
            END IF
!
!       FIRST PRINT TAD DATA. ONLY ONE KIND OF TAD CAN EXITS
!       ON A SURFACE
!
!               HANDEL NO DATA FOR A SURFACE
            IF(DEC.EQ.0) THEN
!       JUMP TO THE CONTINUE STATEMENT AND DO THE NEXT SURFACE
               GO TO 16
            ELSE
!       PROCEED
            END IF
!
!                       DO PRINTING NOW
!*****************************************************************
!
            WRITE(OUTLYNE,200) SURF,TYPE1,TYPE2 ,surf_decenter_y(SURF),surf_decenter_x(SURF),surf_decenter_z(SURF)
            CALL SHOWIT(0)
!
16          CONTINUE
         END DO
      END IF
      DO SURF=0,INT(sys_last_surf())
!     GLOBAL DECENTERS AND TILTS
         IF(surf_global_dx(SURF).NE.0.0D0.OR.surf_global_dy(SURF).NE.0.0D0.OR.surf_global_dz(SURF).NE.0.0D0.OR.surf_global_alpha(SURF).NE.0.0D0.OR.surf_global_beta(SURF).NE.0.0D0.OR.surf_global_gamma(SURF).NE.0.0D0) THEN
            WRITE(OUTLYNE,9000) SURF
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9001) surf_global_dx(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9002) surf_global_dy(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9003) surf_global_dz(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9004) surf_global_alpha(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9005) surf_global_beta(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,9006) surf_global_gamma(SURF)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)
            CALL SHOWIT(0)
         END IF
      END DO
   END IF
!
200 FORMAT(I3,1X,A5,1X,A3,1X,G15.8,1X,G15.8,1X,G15.8)
2001 FORMAT(I3,1X,A5,1X,A3,1X,G15.8,1X,G15.8,1X,G15.8,5X,I3)
!
100 FORMAT('SURF',1X,I3,1X,':NO TILT AND DECENTER DATA')
110 FORMAT('NO TILT AND DECENTER DATA')
2000 FORMAT('SURF',1X,'TYPE',9X,'ALPHA',10X,'BETA',12X,'GAMMA',9X,'RETURN SURF ')
3000 FORMAT('SURF',1X,'TYPE',8X,'  YD ',12X,' XD ',11X,'  ZD ')
1500 FORMAT(1X)
9000 FORMAT('SURFACE NUMBER = ',I3)
9001 FORMAT('GLOBAL X-DECENTRATION (GDX)    = ',G15.8)
9002 FORMAT('GLOBAL Y-DECENTRATION (GDY)    = ',G15.8)
9003 FORMAT('GLOBAL Z-DECENTRATION (GDZ)    = ',G15.8)
9004 FORMAT('GLOBAL ALPHA TILT     (GALPHA) = ',G15.8)
9005 FORMAT('GLOBAL BETA TILT      (GBETA)  = ',G15.8)
9006 FORMAT('GLOBAL GAMMA TILT     (GGAMMA) = ',G15.8)
   RETURN
END
! SUB SPIV.FOR
SUBROUTINE SPIV
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_last_surf
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SPIV WHICH IMPLEMENTS THE PIV
!       COMMAND AT THE CMD LEVEL
!       THE ONLY VALID QUALIFIERS ARE (BLANK) AND "ALL","OB" AND "OBJ"
!       THE NUMERIC ENTRY W1, IS THE SURFACE DESIGNATOR
!       FOR PRINTOUT OF ONE SURFACE'S DATA ONLY
!
   CHARACTER TYPE1*5
!
   INTEGER PCNT,PIV,PIVCNT
!
!
!               THIS IS A CMD LEVEL COMMAND ONLY
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PIVOT" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PIVOT" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      OUTLYNE='NO PIVOT DEFINITIONS EXIST'
      CALL SHOWIT(1)
      OUTLYNE='LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE PIVOT
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE PIVOT DATA FOR
!       THE ENTIRE LENS IS PRINTED. IF THE QUALIFIER IS "OB" OR "OBJ"
!       THEN DATA FOR THE OBJECT SURFACE IS PRINTED.
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!
!       THIS IS SAME AS PIV,0
      S1=1
      DF1=0
      SQ=0
      WQ='        '
      W1=0.0
   ELSE
!       WQ NOT OB OR OBJ
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'ALL') THEN
!
!       WE HAVE INVALID QUALIFIER
      CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
!       HANDEL AN INDIVIDUAL SURFACE INCLUDING "OB" AND "OBJ"
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      SURF=INT(W1)
      IF(SURF.GT.(INT(sys_last_surf())).OR.SURF.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
! SET PIV APPROPRIATELY
!
      IF(surf_pivot_flag(SURF).NE.0.0D0) THEN
         PIV=1
      ELSE
         PIV=0
      END IF
!
!       FIRST PRINT PIV DATA. ONLY ONE KIND OF TAD CAN EXITS
!       ON A SURFACE
!
!               HANDEL NO DATA FOR A SURFACE
      IF(PIV.EQ.0) THEN
         WRITE(OUTLYNE,100) SURF
         CALL SHOWIT(0)
         RETURN
      END IF
!
!                       DO PRINTING NOW
!*****************************************************************
!
!
!
      IF(surf_pivot_flag(SURF).NE.0.0D0) THEN
         IF(surf_pivot_flag(SURF).EQ.1.0D0) THEN
            TYPE1 ='PIVOT'
         END IF
         IF(HEADIN) WRITE(OUTLYNE,3000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(surf_pivot_flag(SURF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,200) SURF,TYPE1,surf_pivot_x(SURF),surf_pivot_y(SURF),surf_pivot_z(SURF)
            CALL SHOWIT(0)
         END IF
      END IF
      RETURN
   ELSE
!*********************************************************************
!       SQ=1 AND ONLY ALLOWED WQ BY NOW IS "ALL"
!       HANDEL THE WHOLE LENS
!       PRINT HEADING DATA
!
!       SET THE PIVOT COUNTER. IT IS USED TO DETERMINE
!       THE CASE OF NO PIVOT IN A LENS
      PIVCNT=0
      PCNT=0
      DO SURF=0,INT(sys_last_surf())
         IF(surf_pivot_flag(SURF).NE.0.0D0) THEN
            IF(surf_pivot_flag(SURF).NE.0.0D0) PCNT=PCNT+1
            PIVCNT=PIVCNT+1
         ELSE
         END IF
      END DO
      IF(PIVCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
      WRITE(OUTLYNE,1000)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,3000)
      CALL SHOWIT(0)
      DO SURF=0,INT(sys_last_surf())
!
! SET PIVAPPROPRIATELY
!
         IF(surf_pivot_flag(SURF).NE.0.0D0) THEN
            PIV=1
         ELSE
            PIV=0
         END IF
         IF(PIV.EQ.0) GO TO 15
!
!       SET TYPE1 CORRECTLY
!
         IF(surf_pivot_flag(SURF).EQ.1.0D0) THEN
            TYPE1 ='PIVOT'
         END IF
!
!       FIRST PRINT PIV DATA. ONLY ONE KIND OF PIV CAN EXITS
!       ON A SURFACE
!
!                       DO PRINTING NOW
!*****************************************************************
!
         IF(surf_pivot_flag(SURF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,200) SURF,TYPE1 ,surf_pivot_x(SURF),surf_pivot_y(SURF),surf_pivot_z(SURF)
            CALL SHOWIT(0)
         END IF
15       CONTINUE
      END DO
!
   END IF
!
200 FORMAT(I3,2X,A5,3X,1X,G15.8,1X,G15.8,1X,G15.8)
!
100 FORMAT('SURF',1X,I3,1X,':NO USER-DEFINED PIVOT POINT DATA')
110 FORMAT('NO USER-DEFINED PIVOT POINT DATA')
1000 FORMAT('USER DEFINED PIVOT POINT DATA')
3000 FORMAT('SURF',1X,'    ',8X,'  X  ',12X,' Y  ',11X,'  Z  ')
1500 FORMAT(1X)
   RETURN
END
! SUB SPIDEROUT.FOR
SUBROUTINE SPIDEROUT
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_last_surf
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SPIDEROUT WHICH IMPLEMENTS THE SPIDER
!       COMMAND AT THE CMD LEVEL
!       THE ONLY VALID QUALIFIERS ARE (BLANK) AND "ALL","OB" AND "OBJ"
!       THE NUMERIC ENTRY W1, IS THE SURFACE DESIGNATOR
!       FOR PRINTOUT OF ONE SURFACE'S DATA ONLY
!
   INTEGER SCNT,SPID,SPIDCNT
!
!
!               THIS IS A CMD LEVEL COMMAND ONLY
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPIDER" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPIDER" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      OUTLYNE='NO SPIDER DEFINITIONS EXIST'
      CALL SHOWIT(1)
      OUTLYNE='LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE SPIDER
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE SPIDER DATA FOR
!       THE ENTIRE LENS IS PRINTED. IF THE QUALIFIER IS "OB" OR "OBJ"
!       THEN DATA FOR THE OBJECT SURFACE IS PRINTED.
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!
!       THIS IS SAME AS SPIDER,0
      S1=1
      DF1=0
      SQ=0
      WQ='        '
      W1=0.0
   ELSE
!       WQ NOT OB OR OBJ
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'ALL') THEN
!
!       WE HAVE INVALID QUALIFIER
      CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
!       HANDEL AN INDIVIDUAL SURFACE INCLUDING "OB" AND "OBJ"
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      SURF=INT(W1)
      IF(SURF.GT.(INT(sys_last_surf())).OR.SURF.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
! SET SPID APPROPRIATELY
!
      IF(surf_spider_flag(SURF).NE.0.0D0) THEN
         SPID=1
      ELSE
         SPID=0
      END IF
!
!       FIRST PRINT SPIDER DATA.
!
!               HANDEL NO DATA FOR A SURFACE
      IF(SPID.EQ.0) THEN
         WRITE(OUTLYNE,100) SURF
         CALL SHOWIT(0)
         RETURN
      END IF
!
!                       DO PRINTING NOW
!*****************************************************************
!
!
!
      IF(surf_spider_flag(SURF).NE.0.0D0) THEN
         IF(HEADIN) WRITE(OUTLYNE,3000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,200) SURF,INT(surf_spider_arms(SURF)),surf_spider_angle(SURF),surf_spider_width(SURF)
         CALL SHOWIT(0)
      END IF
      RETURN
   ELSE
!*********************************************************************
!       SQ=1 AND ONLY ALLOWED WQ BY NOW IS "ALL"
!       HANDEL THE WHOLE LENS
!       PRINT HEADING DATA
!
!       SET THE PIVOT COUNTER. IT IS USED TO DETERMINE
!       THE CASE OF NO PIVOT IN A LENS
      SPIDCNT=0
      SCNT=0
      DO SURF=0,INT(sys_last_surf())
         IF(surf_spider_flag(SURF).NE.0.0D0) THEN
            IF(surf_spider_flag(SURF).NE.0.0D0) SCNT=SCNT+1
            SPIDCNT=SPIDCNT+1
         ELSE
         END IF
      END DO
      IF(SPIDCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
      WRITE(OUTLYNE,1000)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,3000)
      CALL SHOWIT(0)
      DO SURF=0,INT(sys_last_surf())
!
         IF(surf_spider_flag(SURF).NE.0.0D0) THEN
            SPID=1
         ELSE
            SPID=0
         END IF
         IF(SPID.EQ.0) GO TO 15
!
!
!       FIRST PRINT SPIDER DATA.
!
!                       DO PRINTING NOW
!*****************************************************************
!
         WRITE(OUTLYNE,200) SURF,INT(surf_spider_arms(SURF)),surf_spider_angle(SURF),surf_spider_width(SURF)
         CALL SHOWIT(0)
15       CONTINUE
      END DO
!
   END IF
!
200 FORMAT(I3,2X,I4,3X,G15.8,1X,G15.8)
!
100 FORMAT('SURF',1X,I3,1X,':NO SPIDER EXISTS')
110 FORMAT('NO SPIDER EXISTS')
1000 FORMAT('SPIDER DATA')
3000 FORMAT('SURF',4X,'N',8X,'W',15X,'L')
1500 FORMAT(1X)
   RETURN
END
! SUB SSPTWT.FOR
SUBROUTINE SSPTWT
!
   use DATLEN
   use mod_surface
   use DATMAI
   use command_utils, only: is_command_query
   use mod_system, only: sys_wavelength, sys_wl_weight, sys_set_wl_weight
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SSPTWT WHICH IMPLEMENTS THE SPTWT
!       COMMAND AT THE CMD LEVEL.
!
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
   IF(WC.EQ.'SPTWT') THEN
!
      IF(is_command_query()) THEN
         WRITE(OUTLYNE,1000)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(1),sys_wavelength(2),sys_wavelength(3),sys_wavelength(4),sys_wavelength(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(1),sys_wl_weight(2),sys_wl_weight(3),sys_wl_weight(4),sys_wl_weight(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,1001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(6),sys_wavelength(7),sys_wavelength(8),sys_wavelength(9),sys_wavelength(10)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(6),sys_wl_weight(7),sys_wl_weight(8),sys_wl_weight(9),sys_wl_weight(10)
         CALL SHOWIT(0)
         RETURN
      ELSE
!       NOT STI
      END IF
      IF(SST.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPTWT" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(DF1.EQ.1.AND.DF2.EQ.1.AND.DF3.EQ.1.AND.DF4.EQ.1 .AND.DF5.EQ.1) THEN
!       SPTWT TYPED IN WITH NO INPUT SO PRINT OUT CURRENT VALUES
         WRITE(OUTLYNE,1000)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(1),sys_wavelength(2),sys_wavelength(3),sys_wavelength(4),sys_wavelength(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(1),sys_wl_weight(2),sys_wl_weight(3),sys_wl_weight(4),sys_wl_weight(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,1001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(6),sys_wavelength(7),sys_wavelength(8),sys_wavelength(9),sys_wavelength(10)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(6),sys_wl_weight(7),sys_wl_weight(8),sys_wl_weight(9),sys_wl_weight(10)
         CALL SHOWIT(0)
         RETURN
      ELSE
!       ALL NUMERIC INPUT NOT DEFAULT, IF SOME ARE, THEN LEAVE
!       THE ASSOCIATED VALUES AS THEY WERE BEFORE.
!       SET VALUES
!
         IF(DF1.EQ.1) W1=sys_wl_weight(1)
         IF(DF2.EQ.1) W2=sys_wl_weight(2)
         IF(DF3.EQ.1) W3=sys_wl_weight(3)
         IF(DF4.EQ.1) W4=sys_wl_weight(4)
         IF(DF5.EQ.1) W5=sys_wl_weight(5)
         IF(DF1.EQ.1) DF1=0
         IF(DF2.EQ.1) DF2=0
         IF(DF3.EQ.1) DF3=0
         IF(DF4.EQ.1) DF4=0
         IF(DF5.EQ.1) DF5=0
      END IF
      IF(W1.LT.0.0D0.OR.W2.LT.0.0D0.OR.W3.LT.0.0D0.OR.W4.LT.0.0D0.OR.W5.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SPECTRAL WEIGHTS MUST BE GREATER THAN ZERO'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(sys_wavelength(1).NE.0.0D0) THEN
         call sys_set_wl_weight(1,W1)
      ELSE
         IF(sys_wl_weight(1).NE.0.0D0) THEN
            call sys_set_wl_weight(1,0.0D0)
            OUTLYNE='WAVELENGTH #1 WAS 0.0 SO SPECTRAL WEIGHT #1 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(2).NE.0.0D0) THEN
         call sys_set_wl_weight(2,W2)
      ELSE
         IF(sys_wl_weight(2).NE.0.0D0) THEN
            call sys_set_wl_weight(2,0.0D0)
            OUTLYNE='WAVELENGTH #2 WAS 0.0 SO SPECTRAL WEIGHT #2 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(3).NE.0.0D0) THEN
         call sys_set_wl_weight(3,W3)
      ELSE
         IF(sys_wl_weight(3).NE.0.0D0) THEN
            call sys_set_wl_weight(3,0.0D0)
            OUTLYNE='WAVELENGTH #3 WAS 0.0 SO SPECTRAL WEIGHT #3 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(4).NE.0.0D0) THEN
         call sys_set_wl_weight(4,W4)
      ELSE
         IF(sys_wl_weight(4).NE.0.0D0) THEN
            call sys_set_wl_weight(4,0.0D0)
            OUTLYNE='WAVELENGTH #4 WAS 0.0 SO SPECTRAL WEIGHT #4 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(5).NE.0.0D0) THEN
         call sys_set_wl_weight(5,W5)
      ELSE
         IF(sys_wl_weight(5).NE.0.0D0) THEN
            call sys_set_wl_weight(5,0.0D0)
            OUTLYNE='WAVELENGTH #5 WAS 0.0 SO SPECTRAL WEIGHT #5 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(F12.EQ.1) THEN
!       UPDATE PERMANENT FILES AS WELL
         SYSP(31)=sys_wl_weight(1)
         SYSP(32)=sys_wl_weight(2)
         SYSP(33)=sys_wl_weight(3)
         SYSP(34)=sys_wl_weight(4)
         SYSP(35)=sys_wl_weight(5)
      ELSE
!       DON'T
      END IF
      RETURN
   ELSE
!     NOT SPTWT
   END IF
   IF(WC.EQ.'SPTWT2') THEN
!
      IF(is_command_query()) THEN
         WRITE(OUTLYNE,1000)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(1),sys_wavelength(2),sys_wavelength(3),sys_wavelength(4),sys_wavelength(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(1),sys_wl_weight(2),sys_wl_weight(3),sys_wl_weight(4),sys_wl_weight(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,1001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(6),sys_wavelength(7),sys_wavelength(8),sys_wavelength(9),sys_wavelength(10)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(6),sys_wl_weight(7),sys_wl_weight(8),sys_wl_weight(9),sys_wl_weight(10)
         CALL SHOWIT(0)
         RETURN
      ELSE
!       NOT STI
      END IF
      IF(SST.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPTWT2" TAKES NO QUALIFIER OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(DF1.EQ.1.AND.DF2.EQ.1.AND.DF3.EQ.1.AND.DF4.EQ.1 .AND.DF5.EQ.1) THEN
!       SPTWT2 TYPED IN WITH NO INPUT SO PRINT OUT CURRENT VALUES
         WRITE(OUTLYNE,1000)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(1),sys_wavelength(2),sys_wavelength(3),sys_wavelength(4),sys_wavelength(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(1),sys_wl_weight(2),sys_wl_weight(3),sys_wl_weight(4),sys_wl_weight(5)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,1001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wavelength(6),sys_wavelength(7),sys_wavelength(8),sys_wavelength(9),sys_wavelength(10)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wl_weight(6),sys_wl_weight(7),sys_wl_weight(8),sys_wl_weight(9),sys_wl_weight(10)
         CALL SHOWIT(0)
         RETURN
      ELSE
!       ALL NUMERIC INPUT NOT DEFAULT, IF SOME ARE, THEN LEAVE
!       THE ASSOCIATED VALUES AS THEY WERE BEFORE.
!       SET VALUES
!
         IF(DF1.EQ.1) W1=sys_wl_weight(6)
         IF(DF2.EQ.1) W2=sys_wl_weight(7)
         IF(DF3.EQ.1) W3=sys_wl_weight(8)
         IF(DF4.EQ.1) W4=sys_wl_weight(9)
         IF(DF5.EQ.1) W5=sys_wl_weight(10)
         IF(DF1.EQ.1) DF1=0
         IF(DF2.EQ.1) DF2=0
         IF(DF3.EQ.1) DF3=0
         IF(DF4.EQ.1) DF4=0
         IF(DF5.EQ.1) DF5=0
      END IF
      IF(W1.LT.0.0D0.OR.W2.LT.0.0D0.OR.W3.LT.0.0D0.OR.W4.LT.0.0D0.OR.W5.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SPECTRAL WEIGHTS MUST BE GREATER THAN ZERO'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(sys_wavelength(6).NE.0.0D0) THEN
         call sys_set_wl_weight(6,W1)
      ELSE
         IF(sys_wl_weight(6).NE.0.0D0) THEN
            call sys_set_wl_weight(6,0.0D0)
            OUTLYNE='WAVELENGTH #6 WAS 0.0 SO SPECTRAL WEIGHT #6 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(7).NE.0.0D0) THEN
         call sys_set_wl_weight(7,W2)
      ELSE
         IF(sys_wl_weight(7).NE.0.0D0) THEN
            call sys_set_wl_weight(7,0.0D0)
            OUTLYNE='WAVELENGTH #7 WAS 0.0 SO SPECTRAL WEIGHT #7 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(8).NE.0.0D0) THEN
         call sys_set_wl_weight(8,W3)
      ELSE
         IF(sys_wl_weight(8).NE.0.0D0) THEN
            call sys_set_wl_weight(8,0.0D0)
            OUTLYNE='WAVELENGTH #8 WAS 0.0 SO SPECTRAL WEIGHT #8 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(9).NE.0.0D0) THEN
         call sys_set_wl_weight(9,W4)
      ELSE
         IF(sys_wl_weight(9).NE.0.0D0) THEN
            call sys_set_wl_weight(9,0.0D0)
            OUTLYNE='WAVELENGTH #9 WAS 0.0 SO SPECTRAL WEIGHT #9 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(sys_wavelength(10).NE.0.0D0) THEN
         call sys_set_wl_weight(10,W5)
      ELSE
         IF(sys_wl_weight(10).NE.0.0D0) THEN
            call sys_set_wl_weight(10,0.0D0)
            OUTLYNE='WAVELENGTH #10 WAS 0.0 SO SPECTRAL WEIGHT #10 SET TO 0.0 ALSO'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
         END IF
      END IF
      IF(F12.EQ.1) THEN
!       UPDATE PERMANENT FILES AS WELL
         SYSP(76)=sys_wl_weight(6)
         SYSP(77)=sys_wl_weight(7)
         SYSP(78)=sys_wl_weight(8)
         SYSP(79)=sys_wl_weight(9)
         SYSP(80)=sys_wl_weight(10)
      ELSE
!       DON'T
      END IF
      RETURN
   ELSE
!     NOT SPTWT
   END IF
1000 FORMAT('WAVL NBR',10X,'1',12X,'2',12X,'3',12X ,'4',12X,'5')
1001 FORMAT('WAVL NBR',10X,'6',12X,'7',12X,'8',12X ,'9',12X,'10')
2000 FORMAT('WAVELENGTH',3X,G12.5,1X,G12.5,1X,G12.5,1X,G12.5,1X,G12.5)
3000 FORMAT('SPECTRAL WT',2X,G11.4,2X,G11.4,2X,G11.4,2X,G11.4,2X,G11.4)
END

! SUB SSPC.FOR
SUBROUTINE SSPC
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_fno_flag_x, sys_fno_flag_y, sys_fno_hold_x, sys_fno_hold_y, sys_last_surf
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SSPC WHICH IMPLEMENTS THE SPC
!       COMMAND AT THE CMD LEVEL. THIS COMMAND IS NOT
!       THE SAME AS HEXAGONS. THERE IS NO NUMERIC,QUALIFIER
!       OR STRING INPUT USED. ALL ASI,FNBY,FNBX,ERY,ERX AND
!       NORMAL STATUS IS LISTED FOR THE CURRENT LENS
!
   INTEGER ASICNT,I,NRCNT
!
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"SPC" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'SPECIAL CONDITIONS NOT DEFINED'//'\n'//&
      & 'LENS SYSTEM HAS NO SURFACES'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       PRINT SPECIAL CONDITION HEADER
   WRITE(OUTLYNE,999)
   CALL SHOWIT(0)
!
!       NOW DO THE FNBY AND FNBX
   IF(sys_fno_flag_y().EQ.-1.0) THEN
!       THERE IS FNBY HOLD ASSIGNMENT
      WRITE(OUTLYNE,601) sys_fno_hold_y()
      CALL SHOWIT(0)
   END IF
   IF(sys_fno_flag_x().EQ.-1.0) THEN
!       THERE IS FNBX HOLD ASSIGNMENT
      WRITE(OUTLYNE,701) sys_fno_hold_x()
      CALL SHOWIT(0)
      WRITE(OUTLYNE,7000)
      CALL SHOWIT(0)
   END IF
!
!       NOW DO THE ERY AND ERX
   IF(sys_fno_flag_y().EQ.-2.0) THEN
!       THERE IS ERY HOLD ASSIGNMENT
      WRITE(OUTLYNE,801) sys_fno_hold_y()
      CALL SHOWIT(0)
   END IF
   IF(sys_fno_flag_x().EQ.-2.0) THEN
!       THERE IS ERX HOLD ASSIGNMENT
      WRITE(OUTLYNE,901) sys_fno_hold_x()
      CALL SHOWIT(0)
      WRITE(OUTLYNE,9000)
      CALL SHOWIT(0)
   END IF
!
100 FORMAT('NO ALTERNATE SURFACE INTERSECTION IN CURRENT LENS')
101 FORMAT('NO NON-REGULAR SURFACE NORMAL DEFINITIONS')
102 FORMAT('IN CURRENT LENS')
200 FORMAT('ALTERNATE SURFACE INTERSECTION ON SURFACE',I3)
201 FORMAT('"REVERSE"  SURFACE NORMAL DEFINITION ON SURFACE',I3)
202 FORMAT('"POSITIVE" SURFACE NORMAL DEFINITION ON SURFACE',I3)
203 FORMAT('"NEGATIVE" SURFACE NORMAL DEFINITION ON SURFACE',I3)
601 FORMAT('FNBY HLD =',G12.5)
701 FORMAT('FNBX HLD =',G12.5)
801 FORMAT('ERY HLD =',G12.5)
901 FORMAT('ERX HLD =',G12.5)
7000 FORMAT('"FNBX" ADJUSTMENTS ONLY ACTIVE WITH ANAMORPHIC',1X,'SYSTEMS')
9000 FORMAT('"ERX" ADJUSTMENTS ONLY ACTIVE WITH ANAMORPHIC',1X,'SYSTEMS')
!
999 FORMAT('SPECIAL CONDITIONS')
   RETURN
END
! SUB SSC.FOR
SUBROUTINE SSC
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_astop, sys_astop_adj, sys_reverse_trace, sys_scx, sys_scx_fang, &
      & sys_scx_fang_set, sys_scy, sys_scy_fang, sys_scy_fang_set, sys_x1_scx, sys_x1_scx_fang, &
      & sys_y1_scy, sys_y1_scy_fang, &
      & sys_set_reverse_trace, sys_set_scx, sys_set_scx_fang, sys_set_scx_fang_set, &
      & sys_set_scx_set, sys_set_scy, sys_set_scy_fang, sys_set_scy_fang_set, sys_set_scy_set, &
      & sys_set_x1_scx, sys_set_x1_scx_fang, sys_set_x1_scx_fang_set, sys_set_x1_scx_set, &
      & sys_set_xz_data_flag, sys_set_y1_scy, sys_set_y1_scy_fang, sys_set_y1_scy_fang_set, &
      & sys_set_y1_scy_set
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SSC WHICH IMPLEMENTS THE SCY/SCX
!       COMMAND AT THE LENS OF UPDATE LENS LEVEL OR THE SCY/SCX
!       COMMAND AT THE CMD LEVEL. A QUALIFIER "FANG" IS ALLOWED
!       WHICH CAUSES DATA INPUT TO BE READ AS ANGULAR IN FRACTIONAL
!       DEGREES AND CAUSES SYSTEM(18) OR SYSTEM(19)
!       TO BE SET TO 1.0 FROM ITS
!       DEFAULT 0.0 VALUE
!       FOR SCY OR SCX
!       IF Y1 IS EXPLICITLY SET SYSTEM(51) =1
!       IF X1 IS EXPLICITLY SET SYSTEM(52) =1
!       FOR SCY FANG OR SCX FANG
!       IF Y1 IS EXPLICITLY SET SYSTEM(53) =1
!       IF X1 IS EXPLICITLY SET SYSTEM(54) =1
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.AND.WQ.NE.'FANG') THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'INVALID QUALIFIER WORD USED WITH "'//WC(1:3)//'" COMMAND.'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SN.EQ.1.AND.F1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "SCY" AND "SCX"'//'\n'//&
      & 'OR "SCY FANG" AND "SCX FANG"'//'\n'//&
      & 'TAKE NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(F1.EQ.1) THEN
      IF(WQ.NE.'FANG') THEN
         IF(WC.EQ.'SCY') THEN
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2000) sys_scy(),sys_y1_scy()
            CALL SHOWIT(0)
            IF(sys_scy_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT ANGLE'
            IF(sys_scy_fang_set().EQ.0.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'SCX') THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,4000) sys_scx(),sys_x1_scx()
            CALL SHOWIT(0)
            IF(sys_scx_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT ANGLE'
            IF(sys_scx_fang_set().EQ.0.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      IF(WQ.EQ.'FANG') THEN
         IF(WC.EQ.'SCY') THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000) sys_scy_fang(),sys_y1_scy_fang()
            CALL SHOWIT(0)
            IF(sys_scy_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT ANGLE'
            IF(sys_scy_fang_set().EQ.0.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'SCX') THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,5000) sys_scx_fang(),sys_x1_scx_fang()
            CALL SHOWIT(0)
            IF(sys_scx_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT ANGLE'
            IF(sys_scx_fang_set().EQ.0.0D0)OUTLYNE='REF. OBJ. HT. SET BY OBJECT HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      RETURN
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
         IF(WC.EQ.'SCY'.AND.WQ.EQ.'FANG'.AND.DABS(surf_thickness(NEWOBJ)).LE.1.0D10) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCY FANG" REQUIRES AN OBJECT DISTANCE'//'\n'//&
            & 'TO BE GREATER THAN 1.0D10 LENS UNITS IN MAGNITUDE'//'\n'//&
            & 'NO ACTION TAKEN', 1)
            RETURN
         END IF
         IF(WC.EQ.'SCX'.AND.WQ.EQ.'FANG'.AND.DABS(surf_thickness(NEWOBJ)).LE.1.0D10) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCX FANG" REQUIRES AN OBJECT DISTANCE'//'\n'//&
            & 'TO BE GREATER THAN 1.0D10 LENS UNITS IN MAGNITUDE'//'\n'//&
            & 'NO ACTION TAKEN', 1)
            RETURN
         END IF
         IF(WC.EQ.'SCY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCY AND SCY FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'SCX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCX AND SCX FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(DF1.EQ.1) THEN
!
         IF(WC.EQ.'SCY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCY AND SCY FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'AND ACCEPT OPTIONAL NUMERIC WORD #2 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'SCX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SCX AND SCX FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'AND ACCEPT OPTIONAL NUMERIC WORD #2 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
      IF(WQ.EQ.'FANG') THEN
!       MAKE SURE YOU DON'T STEP ON AN ENTRANCE PUPIL ADJUSTMENT
         IF(sys_astop().NE.-99.0D0) THEN
!       THERE IS AN ASTOP
!       IS THERE AN EN ADJUSTMENT
            IF(sys_astop_adj().EQ.1.0D0.OR.sys_astop_adj().EQ.2.0D0) THEN
!       YES EN ADJUSTMENT, DON'T ALLOW FANG ENTRIES
               IF(WC.EQ.'SCY') THEN
                  CALL REPORT_ERROR_AND_FAIL(&
                  & 'AN ASTOP (EN) ADJUSTMENT IS IN EFFECT'//'\n'//&
                  & '"SCY FANG" INPUT IS NOT ALLOWED'//'\n'//&
                  & 'RE-ENTER OBJECT DATA WITH AN "SCY" VALUE', 1)
                  RETURN
               END IF
               IF(WC.EQ.'SCX') THEN
                  CALL REPORT_ERROR_AND_FAIL(&
                  & 'AN ASTOP (EN) ADJUSTMENT IS IN EFFECT'//'\n'//&
                  & '"SCX FANG" INPUT IS NOT ALLOWED'//'\n'//&
                  & 'RE-ENTER OBJECT DATA WITH AN "SCX" VALUE', 1)
                  RETURN
               END IF
               RETURN
            END IF
         END IF
!
!     ASSIGN VALUES
!
         call sys_set_scy_fang_set(1.0D0)
         call sys_set_scx_fang_set(1.0D0)
!       FANG IS USED HERE
         IF(WC.EQ.'SCY') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "SCY FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            call sys_set_scy_fang((W1))
            SYSTEM(92:99)=0.0D0
            IF(DF2.EQ.1) W2=0.0D0
            call sys_set_y1_scy_set(1.0D0)
            call sys_set_y1_scy_fang_set(1.0D0)
            call sys_set_y1_scy_fang(W2)
            call sys_set_y1_scy(W2)
            IF(WC.EQ.'SCY'.OR.WC.EQ.'SCX') THEN
               call sys_set_scy_set(1.0D0)
               call sys_set_scx_set(1.0D0)
            END IF
         ELSE
         END IF
         IF(WC.EQ.'SCX') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "SCX FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            call sys_set_scx_fang((W1))
            SYSTEM(92:99)=0.0D0
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
            IF(DF2.EQ.1) W2=0.0D0
            call sys_set_x1_scx_set(1.0D0)
            call sys_set_x1_scx_fang_set(1.0D0)
            call sys_set_x1_scx_fang(W2)
            call sys_set_x1_scx(W2)
            IF(WC.EQ.'SCY'.OR.WC.EQ.'SCX') THEN
               call sys_set_scy_set(1.0D0)
               call sys_set_scx_set(1.0D0)
            END IF
         ELSE
         END IF
      ELSE
!     NOT FANG
         IF(WC.EQ.'SCY') THEN
            call sys_set_scy_fang_set(0.0D0)
            IF(DF2.EQ.1) W2=0.0D0
            call sys_set_y1_scy_set(1.0D0)
            call sys_set_y1_scy_fang_set(1.0D0)
            call sys_set_scy(W1)
            SYSTEM(92:99)=0.0D0
            call sys_set_y1_scy(W2)
            call sys_set_y1_scy_fang(W2)
            IF(WC.EQ.'SCY'.OR.WC.EQ.'SCX') THEN
               call sys_set_scy_set(1.0D0)
               call sys_set_scx_set(1.0D0)
            END IF
         ELSE
         END IF
         IF(WC.EQ.'SCX') THEN
            call sys_set_scx_fang_set(0.0D0)
            IF(DF2.EQ.1) W2=0.0D0
            call sys_set_x1_scx_set(1.0D0)
            call sys_set_x1_scx_fang_set(1.0D0)
            call sys_set_scx(W1)
            SYSTEM(92:99)=0.0D0
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
            call sys_set_x1_scx(W2)
            call sys_set_x1_scx_fang(W2)
            IF(WC.EQ.'SCY'.OR.WC.EQ.'SCX') THEN
               call sys_set_scy_set(1.0D0)
               call sys_set_scx_set(1.0D0)
            END IF
         ELSE
         END IF
      END IF
!     GET RID OF REVRAY SETTING IF IN EFFECT
      IF(sys_reverse_trace().NE.0.0D0) THEN
         call sys_set_reverse_trace(0.0D0)
         OUTLYNE='"REVRAY" SETTING HAS BEEN AUTOMATICALLY CANCELLED'
         CALL SHOWIT(1)
      END IF
      RETURN
   ELSE
   END IF
100 FORMAT(1X)
2002 FORMAT('REFERENCE OBJECT HEIGHT')
3002 FORMAT('REFERENCE OBJECT ANGLE')
2000 FORMAT ('SCY(YZ-PLANE IN LENS UNITS)=',1X,D23.15,1X,',',1X,D23.15)
4000 FORMAT ('SCX(XZ-PLANE IN LENS UNITS)=',1X,D23.15,1X,',',1X,D23.15)
3000 FORMAT ('SCY FANG(YZ-PLANE IN DEGREES)=',1X,D23.15,1X,',',1X,D23.15)
5000 FORMAT ('SCX FANG(XZ-PLANE IN DEGREES)=',1X,D23.15,1X,',',1X,D23.15)
   RETURN
END
SUBROUTINE PXYIM
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_pxim, sys_pxim_fang_set, sys_pyim, sys_pyim_fang_set, &
      & sys_reverse_trace, &
      & sys_set_pxim, sys_set_pxim_fang_set, sys_set_pyim, sys_set_pyim_fang_set, &
      & sys_set_reverse_trace, sys_set_scx_set, sys_set_scy_set, sys_set_xz_data_flag
   IMPLICIT NONE
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.AND.WQ.NE.'FANG') THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'INVALID QUALIFIER WORD USED WITH "'//WC(1:4)//'" COMMAND.'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SN.EQ.1.AND.F1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "PXIM" AND "PYIM"'//'\n'//&
      & 'OR "PXIM FANG" AND "PYIM FANG"'//'\n'//&
      & 'TAKE NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(F1.EQ.1) THEN
      IF(WC.EQ.'PXIM'.AND.sys_pxim_fang_set().EQ.0.0D0) THEN
         OUTLYNE='REF. OBJ. HT. NOT SET BY PARAXIAL IMAGE SPACE HT OR ANGLE'
         CALL SHOWIT(0)
         RETURN
      END IF
      IF(WC.EQ.'PYIM'.AND.sys_pyim_fang_set().EQ.0.0D0) THEN
         OUTLYNE='REF. OBJ. HT. NOT SET BY PARAXIAL IMAGE SPACE HT OR ANGLE'
         CALL SHOWIT(0)
         RETURN
      END IF
      IF(WQ.NE.'FANG') THEN
         IF(WC.EQ.'PYIM'.AND.sys_pyim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2000) sys_pyim()
            CALL SHOWIT(0)
            IF(sys_pxim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE SPACE ANGLE'
            IF(sys_pxim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'PXIM'.AND.sys_pxim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,4000) sys_pxim()
            CALL SHOWIT(0)
            IF(sys_pyim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE SPACE ANGLE'
            IF(sys_pyim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      IF(WQ.EQ.'FANG') THEN
         IF(WC.EQ.'PYIM'.AND.sys_pyim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000) sys_pyim()
            CALL SHOWIT(0)
            IF(sys_pxim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE SPACE ANGLE'
            IF(sys_pxim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE SPACE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'PXIM'.AND.sys_pxim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,5000) sys_pxim()
            CALL SHOWIT(0)
            IF(sys_pyim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PAXAXIAL IMAGE SPACE ANGLE'
            IF(sys_pyim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY PARAXIAL IMAGE SPACE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      RETURN
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
         IF(WC.EQ.'PYIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"PYIM AND PYIM FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #2, #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'PXIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"PXIM AND PXIM FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #2, #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(DF1.EQ.1) THEN
!
         IF(WC.EQ.'PYIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"PYIM AND PYIM FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'PXIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"PXIM AND PXIM FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
      IF(WQ.EQ.'FANG') THEN
!       FANG IS USED HERE
         IF(WC.EQ.'PYIM') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "PYIM FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            call sys_set_pyim((W1))
            call sys_set_pyim_fang_set(1.0D0)
            SYSTEM(96:99)=0.0D0
            call sys_set_scy_set(1.0D0)
            call sys_set_scx_set(1.0D0)
         ELSE
         END IF
         IF(WC.EQ.'PXIM') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "PXIM FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            SYSTEM(96:99)=0.0D0
            call sys_set_pxim((W1))
            call sys_set_pxim_fang_set(1.0D0)
            call sys_set_scx_set(1.0D0)
            call sys_set_scy_set(1.0D0)
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
         ELSE
         END IF
      ELSE
!     NOT FANG
         IF(WC.EQ.'PXIM') THEN
            call sys_set_pxim_fang_set(-1.0D0)
            call sys_set_pxim(W1)
            SYSTEM(96:99)=0.0D0
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
            call sys_set_scx_set(1.0D0)
            call sys_set_scy_set(1.0D0)
         ELSE
         END IF
         IF(WC.EQ.'PYIM') THEN
            call sys_set_pyim_fang_set(-1.0D0)
            call sys_set_pyim(W1)
            SYSTEM(96:99)=0.0D0
            call sys_set_scy_set(1.0D0)
            call sys_set_scx_set(1.0D0)
         ELSE
         END IF
      END IF
!     GET RID OF REVRAY SETTING IF IN EFFECT
      IF(sys_reverse_trace().NE.0.0D0) THEN
         call sys_set_reverse_trace(0.0D0)
         OUTLYNE='"REVRAY" SETTING HAS BEEN AUTOMATICALLY CANCELLED'
         CALL SHOWIT(1)
      END IF
      IF(WC.EQ.'PYIM') THEN
         IF(sys_pxim_fang_set().EQ.0.0D0) THEN
            call sys_set_pxim_fang_set(sys_pyim_fang_set())
            call sys_set_pxim(sys_pyim())
         ELSE
            call sys_set_pxim_fang_set(sys_pyim_fang_set())
         END IF
      END IF
      IF(WC.EQ.'PXIM') THEN
         IF(sys_pyim_fang_set().EQ.0.0D0) THEN
            call sys_set_pyim_fang_set(sys_pxim_fang_set())
            call sys_set_pyim(sys_pxim())
         ELSE
            call sys_set_pyim_fang_set(sys_pxim_fang_set())
         END IF
      END IF
      RETURN
   ELSE
   END IF
100 FORMAT(1X)
2002 FORMAT('PARAXIAL REFERENCE IMAGE HEIGHT')
3002 FORMAT('PARAXIAL REFERENCE IMAGE ANGLE')
2000 FORMAT ('PYIM(YZ-PLANE IN LENS UNITS)=',1X,D23.15)
4000 FORMAT ('PXIM(XZ-PLANE IN LENS UNITS)=',1X,D23.15)
3000 FORMAT ('PYIM FANG(YZ-PLANE IN DEGREES)=',1X,D23.15)
5000 FORMAT ('PXIM FANG(XZ-PLANE IN DEGREES)=',1X,D23.15)
   RETURN
END
SUBROUTINE RXYIM
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_pyim_fang_set, sys_rxim, sys_rxim_fang_set, sys_ryim, &
      & sys_ryim_fang_set, &
      & sys_set_rxim, sys_set_rxim_fang_set, sys_set_ryim, sys_set_ryim_fang_set, &
      & sys_set_scx_set, sys_set_scy_set, sys_set_xz_data_flag
   IMPLICIT NONE
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.AND.WQ.NE.'FANG') THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'INVALID QUALIFIER WORD USED WITH "'//WC(1:4)//'" COMMAND.'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SN.EQ.1.AND.F1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "RXIM" AND "RYIM"'//'\n'//&
      & 'OR "RXIM FANG" AND "RYIM FANG"'//'\n'//&
      & 'TAKE NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(F1.EQ.1) THEN
      IF(WC.EQ.'RXIM'.AND.sys_rxim_fang_set().EQ.0.0D0) THEN
         OUTLYNE='REF. OBJ. HT. NOT SET BY REAL IMAGE SPACE HT OR ANGLE'
         CALL SHOWIT(0)
         RETURN
      END IF
      IF(WC.EQ.'RYIM'.AND.sys_ryim_fang_set().EQ.0.0D0) THEN
         OUTLYNE='REF. OBJ. HT. NOT SET BY REAL IMAGE SPACE HT OR ANGLE'
         CALL SHOWIT(0)
         RETURN
      END IF
      IF(WQ.NE.'FANG') THEN
         IF(WC.EQ.'RYIM'.AND.sys_ryim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2000) sys_ryim()
            CALL SHOWIT(0)
            IF(sys_rxim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE ANGLE'
            IF(sys_rxim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'RXIM'.AND.sys_rxim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,4000) sys_rxim()
            CALL SHOWIT(0)
            IF(sys_ryim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE ANGLE'
            IF(sys_ryim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      IF(WQ.EQ.'FANG') THEN
         IF(WC.EQ.'RYIM'.AND.sys_ryim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000) sys_ryim()
            CALL SHOWIT(0)
            IF(sys_rxim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE ANGLE'
            IF(sys_rxim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
         IF(WC.EQ.'RXIM'.AND.sys_rxim_fang_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,100)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3002)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,5000) sys_rxim()
            CALL SHOWIT(0)
            IF(sys_ryim_fang_set().EQ.1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE ANGLE'
            IF(sys_pyim_fang_set().EQ.-1.0D0)OUTLYNE='REF. OBJ. HT. SET BY REAL IMAGE SPACE HEIGHT'
            CALL SHOWIT(0)
         ELSE
         END IF
      ELSE
      END IF
      RETURN
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
         IF(WC.EQ.'RYIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"RYIM AND RYIM FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #2, #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'RXIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"RXIM AND RXIM FANG" TAKE NO STRING INPUT'//'\n'//&
            & 'OR NUMERIC WORD #2, #3, #4 OR #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(DF1.EQ.1) THEN
!
         IF(WC.EQ.'RYIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"RYIM AND RYIM FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'RXIM') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"RXIM AND RXIM FANG" REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
      IF(WQ.EQ.'FANG') THEN
!       FANG IS USED HERE
         IF(WC.EQ.'RYIM') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "RYIM FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            call sys_set_ryim((W1))
            SYSTEM(92:95)=0.0D0
            call sys_set_ryim_fang_set(1.0D0)
            call sys_set_scy_set(1.0D0)
            call sys_set_scx_set(1.0D0)
         ELSE
         END IF
         IF(WC.EQ.'RXIM') THEN
            IF(DABS(W1).GE.90.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE MAGNITUDE OF THE "RXIM FANG" VALUE'//'\n'//&
               & 'MUST BE LESS THAN 90 DEGREES', 1)
               RETURN
            END IF
            call sys_set_rxim((W1))
            SYSTEM(92:95)=0.0D0
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
            call sys_set_rxim_fang_set(1.0D0)
            call sys_set_scx_set(1.0D0)
            call sys_set_scy_set(1.0D0)
         ELSE
         END IF
      ELSE
!     NOT FANG
         IF(WC.EQ.'RXIM') THEN
            call sys_set_rxim_fang_set(-1.0D0)
            call sys_set_rxim(W1)
            SYSTEM(92:95)=0.0D0
            call sys_set_xz_data_flag(2.0D0)
            call sys_set_xz_data_flag(3.0D0)
            call sys_set_scx_set(1.0D0)
            call sys_set_scy_set(1.0D0)
         ELSE
         END IF
         IF(WC.EQ.'RYIM') THEN
            call sys_set_ryim_fang_set(-1.0D0)
            call sys_set_ryim(W1)
            SYSTEM(92:95)=0.0D0
            call sys_set_scy_set(1.0D0)
            call sys_set_scx_set(1.0D0)
         ELSE
         END IF
      END IF
      IF(WC.EQ.'RYIM') THEN
         IF(sys_rxim_fang_set().EQ.0.0D0) THEN
            call sys_set_rxim_fang_set(sys_ryim_fang_set())
            call sys_set_rxim(sys_ryim())
         ELSE
            call sys_set_rxim_fang_set(sys_ryim_fang_set())
         END IF
      END IF
      IF(WC.EQ.'RXIM') THEN
         IF(sys_ryim_fang_set().EQ.0.0D0) THEN
            call sys_set_ryim_fang_set(sys_rxim_fang_set())
            call sys_set_ryim(sys_rxim())
         ELSE
            call sys_set_ryim_fang_set(sys_rxim_fang_set())
         END IF
      END IF
      RETURN
   ELSE
   END IF
100 FORMAT(1X)
2002 FORMAT('REAL REFERENCE IMAGE HEIGHT')
3002 FORMAT('REAL REFERENCE IMAGE ANGLE')
2000 FORMAT ('RYIM(YZ-PLANE IN LENS UNITS)=',1X,D23.15)
4000 FORMAT ('RXIM(XZ-PLANE IN LENS UNITS)=',1X,D23.15)
3000 FORMAT ('RYIM FANG(YZ-PLANE IN DEGREES)=',1X,D23.15)
5000 FORMAT ('RXIM FANG(XZ-PLANE IN DEGREES)=',1X,D23.15)
   RETURN
END
! SUB SBD.FOR
SUBROUTINE SBD
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_bdx, sys_bdy, sys_units, sys_wavelength, sys_wl_ref, &
      & sys_wrx, sys_wry, sys_set_bdx, sys_set_bdy
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SBD WHICH IMPLEMENTS THE BDX,BDY COMMANDS
!       AT THE LENS OF UPDATE LENS LEVEL OR THE BDX,BDY COMMANDS AT
!       THE CMD LEVEL
!
   REAL*8 WAVER
!
!
   IF(F1.EQ.1) THEN
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "BDY" AND "BDX"'//'\n'//&
         & 'TAKE NO EXPLICIT INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(WC.EQ.'BDY') THEN
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_bdy()
         CALL SHOWIT(0)
         RETURN
      ELSE
!     NOT BDY
      END IF
      IF(WC.EQ.'BDX') THEN
         WRITE(OUTLYNE,2001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_bdx()
         CALL SHOWIT(0)
         RETURN
      ELSE
!     NOT BDX
      END IF
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SST.EQ.1) THEN
!
         IF(WC.EQ.'BDY') THEN
            CALL REPORT_ERROR_AND_FAIL('"BDY" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'BDX') THEN
            CALL REPORT_ERROR_AND_FAIL('"BDX" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1 .OR.S4.EQ.1.OR.S5.EQ.1) THEN
!
         IF(WC.EQ.'BDY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDY" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'BDX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDX" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(SQ.EQ.1.AND.WQ.NE.'TEM00') THEN
!
         IF(WC.EQ.'BDY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDY" ONLY TAKES "TEM00" AS AN OPTIONAL QUALIFIER INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'BDX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDX" ONLY TAKES "TEM00" AS AN OPTIONAL QUALIFIER INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!
!     DEFAULT NW1
!
      IF(DF1.EQ.1.AND.SQ.EQ.0) THEN
!
         IF(WC.EQ.'BDY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDY" WITH NO QUALIFIER WORD'//'\n'//&
            & '"REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'BDX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDX" WITH NO QUALIFIER WORD'//'\n'//&
            & '"REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(DF1.EQ.1.AND.SQ.EQ.1) W1=sys_wl_ref()
      IF(SQ.EQ.1) THEN
         IF(W1.NE.1.0D0.AND.W1.NE.2.0D0.AND.W1.NE.3.0D0.AND.W1.NE.4.0D0 .AND.W1.NE.5.0D0.AND.W1.NE.6.0D0.AND.W1.NE.7.0D0.AND.W1.NE.8.0D0.AND.W1.NE.9.0D0.AND.W1.NE.10.0D0) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDX TEM00" AND "BDY TEM00"'//'\n'//&
            & '"REQUIRE A WAVELENGTH NUMBER FROM 1 TO 10'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(W1.EQ.1.0D0)  WAVER=sys_wavelength(1)
         IF(W1.EQ.2.0D0)  WAVER=sys_wavelength(2)
         IF(W1.EQ.3.0D0)  WAVER=sys_wavelength(3)
         IF(W1.EQ.4.0D0)  WAVER=sys_wavelength(4)
         IF(W1.EQ.5.0D0)  WAVER=sys_wavelength(5)
         IF(W1.EQ.6.0D0)  WAVER=sys_wavelength(6)
         IF(W1.EQ.7.0D0)  WAVER=sys_wavelength(7)
         IF(W1.EQ.8.0D0)  WAVER=sys_wavelength(8)
         IF(W1.EQ.9.0D0)  WAVER=sys_wavelength(9)
         IF(W1.EQ.10.0D0) WAVER=sys_wavelength(10)
         IF(sys_units().EQ.1.0D0) WAVER=WAVER*(1.0D-3/25.4D0)
         IF(sys_units().EQ.2.0D0) WAVER=WAVER*1.0D-4
         IF(sys_units().EQ.3.0D0) WAVER=WAVER*1.0D-3
         IF(sys_units().EQ.4.0D0) WAVER=WAVER*1.0D-6
      END IF
!
      IF(SQ.EQ.0) THEN
         IF(F6.EQ.1.AND.W1.LE.0.0D0.AND.WC.EQ.'BDX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDX" WITH NO QUALIFIER WORD'//'\n'//&
            & '"REQUIRES POSITIVE NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(F6.EQ.1.AND.W1.LE.0.0D0.AND.WQ.EQ.'BDY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"BDY" WITH NO QUALIFIER WORD'//'\n'//&
            & '"REQUIRES POSITIVE NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         call sys_set_bdx(W1)
         call sys_set_bdy(W1)
      END IF
      IF(SQ.EQ.1) THEN
         call sys_set_bdx(1000.0D0*(WAVER)/(PII*sys_wrx()))
         call sys_set_bdy(1000.0D0*(WAVER)/(PII*sys_wry()))
      END IF
      RETURN
   END IF
2001 FORMAT(1X)
2002 FORMAT('HALF-ANGLE BEAM DIVERGENCE')
2000 FORMAT('BDY(YZ-PLANE IN MRAD) = ',1X,D23.15)
3000 FORMAT('BDX(XZ-PLANE IN MRAD) = ',1X,D23.15)
END
! SUB SWR.FOR
SUBROUTINE SWR
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wrx, sys_wry, sys_set_wrx, sys_set_wry
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SWR WHICH IMPLEMENTS THE WRX,WRY COMMANDS
!       AT THE LENS OF UPDATE LENS LEVEL OR THE WRX,WRY COMMANDS AT
!       THE CMD LEVEL
!
!
   IF(F1.EQ.1) THEN
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "WRY" AND "WRX"'//'\n'//&
         & 'TAKE NO EXPLICIT INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(WC.EQ.'WRY') THEN
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_wry()
         CALL SHOWIT(0)
         RETURN
      ELSE
!     NOT WRY
      END IF
      IF(WC.EQ.'WRX') THEN
         WRITE(OUTLYNE,2001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_wrx()
         CALL SHOWIT(0)
         RETURN
      ELSE
!     NOT WRX
      END IF
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SST.EQ.1) THEN
!
         IF(WC.EQ.'WRY') THEN
            CALL REPORT_ERROR_AND_FAIL('"WRY" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'WRX') THEN
            CALL REPORT_ERROR_AND_FAIL('"WRX" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1 .OR.S4.EQ.1.OR.S5.EQ.1.OR.SQ.EQ.1) THEN
!
         IF(WC.EQ.'WRY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRY" TAKES NO QUALIFIER OR NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'WRX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRX" TAKES NO QUALIFIER OR NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!
!     DEFAULT NW1
!
      IF(DF1.EQ.1) THEN
!
         IF(WC.EQ.'WRY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRY" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'WRX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRX" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!
      IF(W1.LE.0.0D0) THEN
         IF(WC.EQ.'WRY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRY" MAY NOT BE SET TO LESS THAN ZERO'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'WRX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"WRX" MAY NOT BE SET TO LESS THAN ZERO'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      call sys_set_wrx(W1)
      call sys_set_wry(W1)
      RETURN
   END IF
2001 FORMAT(1X)
2002 FORMAT('1/e2 WAIST RADIUS')
2000 FORMAT('WRY(YZ-PLANE IN LENS UNITS) = ',1X,D23.15)
3000 FORMAT('WRX(XZ-PLANE IN LENS UNITS) = ',1X,D23.15)
END
! SUB SSA.FOR
SUBROUTINE SSA
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_astop, sys_fno_flag_x, sys_fno_flag_y, sys_fno_val_set, &
      & sys_na_set, sys_sax, sys_sax_float, sys_say, sys_say_float, &
      & sys_set_fno_val_set, sys_set_na_set, sys_set_sax, sys_set_sax_float, &
      & sys_set_say, sys_set_say_float, sys_set_xz_data_flag
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SSAY WHICH IMPLEMENTS THE SAY COMMAND
!       AT THE LENS OF UPDATE LENS LEVEL OR THE SAY COMMAND AT
!       THE CMD LEVEL
!
!
   IF(F1.EQ.1) THEN
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "SAY" AND "SAX"'//'\n'//&
         & 'TAKE NO EXPLICIT INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
      IF(WC.EQ.'SAY') THEN
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2000) sys_say()
         CALL SHOWIT(0)
         IF(sys_na_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2010)
            CALL SHOWIT(0)
2010        FORMAT('"SAY" IS BEING HELD WITH AN "NAOY" ASSIGNMENT')
         END IF
         IF(sys_fno_val_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2020)
            CALL SHOWIT(0)
2020        FORMAT('"SAY" IS BEING HELD WITH AN "FNOY" ASSIGNMENT')
         END IF
         IF(sys_say_float().NE.0.0D0) THEN
            WRITE(OUTLYNE,2050)
            CALL SHOWIT(0)
2050        FORMAT('"SAY" IS CURRENTLY FLOATING')
         END IF
         RETURN
      ELSE
!     NOT SAY
      END IF
      IF(WC.EQ.'SAX') THEN
         WRITE(OUTLYNE,2001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2002)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,3000) sys_sax()
         CALL SHOWIT(0)
!
         IF(sys_na_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2030)
            CALL SHOWIT(0)
2030        FORMAT('"SAX" IS BEING HELD WITH AN "NAOX" ASSIGNMENT')
         END IF
         IF(sys_fno_val_set().NE.0.0D0) THEN
            WRITE(OUTLYNE,2040)
            CALL SHOWIT(0)
2040        FORMAT('"SAX" IS BEING HELD WITH AN "FNOX" ASSIGNMENT')
         END IF
         IF(sys_sax_float().NE.0.0D0) THEN
            WRITE(OUTLYNE,2060)
            CALL SHOWIT(0)
2060        FORMAT('"SAX" IS CURRENTLY FLOATING')
         END IF
         RETURN
      ELSE
!     NOT SAX
      END IF
   ELSE
!               NOT AT CMD LEVEL
   END IF
!
   IF(F5.EQ.1.OR.F6.EQ.1) THEN
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"'//WC(1:3)//'" TAKES NO STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1 .OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"'//WC(1:3)//'" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WQ.EQ.'FLOAT'.AND.sys_fno_flag_y().NE.0.0D0.OR.WQ.EQ.'FLOAT'.AND.sys_fno_flag_x().NE.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"FLOAT" MAY NOT BE USED SINCE THERE ARE EXIT PUPIL'//'\n'//&
         & 'OR F-NUMBER HOLDS PRESENT'//'\n'//&
         & 'NO ACTION TAKEN', 1)
         RETURN
      END IF
!
      IF(SQ.EQ.1.AND.F5.EQ.1.AND.WQ.NE.'FLOAT'.AND.WQ.NE.'NOFLOAT') THEN
!
!     CHECK FOR CLAP ON ASTOP IF FLOAT
         IF(WQ.EQ.'FLOAT') THEN
            IF(sys_astop().LE.0.0D0) THEN
!     NO ASTOP ASSIGNED
               CALL REPORT_ERROR_AND_FAIL(&
               & 'NO APERTURE STOP IS DEFINED, "FLOAT" MAY NOT BE USED'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
               RETURN
            END IF
            IF(surf_clap_type(INT(sys_astop())).EQ.0.0D0) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'NO CLEAR APERTURE EXISTS ON THE APERTURE STOP SURFACE'//'\n'//&
               & '"FLOAT" MAY NOT BE USED'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
!     NO CLAP ON ASTOP, RETURN
               RETURN
            END IF
         END IF
!
         IF(WC.EQ.'SAY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"FLOAT" AND "NOFLOAT" ARE THE ONLY VALID QUALIFIER'//'\n'//&
            & 'WORDS FOR "SAY" IN LENS INPUT MODE'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'SAX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"FLOAT" AND "NOFLOAT" ARE THE ONLY VALID QUALIFIER'//'\n'//&
            & 'WORDS FOR "SAX" IN LENS INPUT MODE'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
      IF(F5.EQ.1) THEN
         IF(WQ.NE.'        '.AND.WQ.NE.'FLOAT'.AND.WQ.NE.'NOFLOAT') THEN
!
            IF(WC.EQ.'SAY') THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'INVALID QUALIFIER WORD USED WITH "SAY"'//'\n'//&
               & 'AT THE LENS INPUT LEVEL'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
               RETURN
            END IF
            IF(WC.EQ.'SAX') THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'INVALID QUALIFIER WORD USED WITH "SAX"'//'\n'//&
               & 'AT THE LENS INPUT LEVEL'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
               RETURN
            END IF
         END IF
      END IF
      IF(F6.EQ.1) THEN
         IF(WQ.NE.'        '.AND.WQ.NE.'DELT'.AND.WQ.NE.'CENT'.AND.WQ.NE.'FLOAT'.AND.WQ.NE.'NOFLOAT') THEN
!
            IF(WC.EQ.'SAY') THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'INVALID QUALIFIER WORD USED WITH "SAY"'//'\n'//&
               & 'AT THE LENS UPDATE LEVEL'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
               RETURN
            END IF
            IF(WC.EQ.'SAX') THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'INVALID QUALIFIER WORD USED WITH "SAX"'//'\n'//&
               & 'AT THE LENS UPDATE LEVEL'//'\n'//&
               & 'RE-ENTER COMMAND', 1)
               RETURN
            END IF
         END IF
      END IF
      call sys_set_say_float(1.0D0)
      call sys_set_sax_float(1.0D0)
      call sys_set_say_float(0.0D0)
      call sys_set_sax_float(0.0D0)
!     IF NUMERIC INPUT, SHUT OFF FLOAT
      IF(DF1.EQ.0.AND.WC.EQ.'SAY'.OR.DF1.EQ.0.AND.WC.EQ.'SAX') THEN
         call sys_set_say_float(0.0D0)
         call sys_set_sax_float(0.0D0)
      END IF
!
      IF(DF1.EQ.1.AND.WQ.NE.'FLOAT'.AND.WQ.NE.'NOFLOAT') THEN
         IF(WC.EQ.'SAY') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SAY" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'SAX') THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"SAX" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
      END IF
!
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
      IF(W1.EQ.0.0.AND.WQ.NE.'FLOAT'.AND.WQ.NE.'NOFLOAT') THEN
         IF(WC.EQ.'SAY') THEN
            CALL REPORT_ERROR_AND_FAIL('"SAY" MAY NOT BE SET TO ZERO'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(WC.EQ.'SAX') THEN
            CALL REPORT_ERROR_AND_FAIL('"SAX" MAY NOT BE SET TO ZERO'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         RETURN
      END IF
      IF(SQ.EQ.0) THEN
         call sys_set_na_set(0.0D0)
         call sys_set_fno_val_set(0.0D0)
         call sys_set_say((W1))
         call sys_set_sax((W1))
!     IF NUMERIC INPUT, SHUT OFF FLOAT
         call sys_set_say_float(0.0D0)
         call sys_set_sax_float(0.0D0)
         IF(WC.EQ.'SAX') THEN
            call sys_set_xz_data_flag(1.0D0)
            call sys_set_xz_data_flag(3.0D0)
         END IF
      END IF
      IF(WQ.EQ.'DELT') THEN
         call sys_set_na_set(0.0D0)
         call sys_set_fno_val_set(0.0D0)
         call sys_set_say(sys_say()+(W1))
         call sys_set_sax(sys_sax()+(W1))
         call sys_set_say_float(0.0D0)
         call sys_set_sax_float(0.0D0)
         IF(WC.EQ.'SAX') THEN
            call sys_set_xz_data_flag(1.0D0)
            call sys_set_xz_data_flag(3.0D0)
         END IF
      END IF
      IF(WQ.EQ.'CENT') THEN
         call sys_set_na_set(0.0D0)
         call sys_set_fno_val_set(0.0D0)
         call sys_set_say(sys_say()+(W1*0.01D0*sys_say()))
         call sys_set_sax(sys_sax()+(W1*0.01D0*sys_sax()))
         call sys_set_say_float(0.0D0)
         call sys_set_sax_float(0.0D0)
         IF(WC.EQ.'SAX') THEN
            call sys_set_xz_data_flag(1.0D0)
            call sys_set_xz_data_flag(3.0D0)
         END IF
      END IF
      RETURN
   END IF
2001 FORMAT(1X)
2002 FORMAT('SEMI-APERTURE REFERENCE RAY HEIGHT AT SURFACE #1')
2000 FORMAT('SAY(YZ-PLANE IN LENS UNITS) = ',1X,D23.15)
3000 FORMAT('SAX(XZ-PLANE IN LENS UNITS) = ',1X,D23.15)
   RETURN
END
! SUB SRTG.FOR
SUBROUTINE SRTG(LBT,RTGERROR)
   use global_widgets
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_astop, sys_last_surf, sys_ref_surf
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SRTG. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE RTG AND CTG (AND LBL) CMD LEVEL LENS OUTPUT COMMANDS.
!
   CHARACTER SPEC*1,SPECC*75
!
   INTEGER SF,I,NF
!
   REAL*8 RD,CV ,INDEX,DISP,VNUM,PARTL
!
   LOGICAL AIRY,LBT,SPECF,RTGERROR
!
   COMMON/NSIN/NF,INDEX,DISP,VNUM,PARTL

   integer idraw1
   COMMON/DRAWI1/IDRAW1
!
!
!       LENS DATA IS STORED IN C-T-G MODE. IF A CURVATURE
!       IS ZERO, AND RTG VICE CTG IS ISSUED THEN THE RADIUS
!       PRINTS OUT AS ZERO EVEN THOUGH IT IS REALLY INF.
!
!       THE RTG  AND CTG COMMANDS ACCEPT QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. THEY DO NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT.
   RTGERROR=.FALSE.
!
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      OUTLYNE='"'//WC(1:3)//'"'//' TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='BUT NOT BOTH'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RTGERROR=.TRUE.
      CALL MACFAL
      RETURN
   ELSE
   END IF
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"'//WC(1:3)//'"'//' TAKES NO STRING OR NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RTGERROR=.TRUE.
      CALL MACFAL
      RETURN
   END IF
   IF(sys_last_surf().EQ.0.0D0) THEN
      OUTLYNE='LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RTGERROR=.TRUE.
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=INT(sys_last_surf())
      IF(WC.EQ.'RTG'.OR.WC.EQ.'RTGLBL') THEN
         WRITE(OUTLYNE,5001)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5000)
         CALL SHOWIT(0)
      ELSE
      END IF
      IF(WC.EQ.'CTG'.OR.WC.EQ.'CTGLBL') THEN
         WRITE(OUTLYNE,6001)
         CALL SHOWIT(0)

         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,6000)
         CALL SHOWIT(0)
      ELSE
      END IF



      DO 10 I=0,INT(sys_last_surf())
         NF=I
         CALL SINDEX
         IF(WC.EQ.'RTG'.OR.WC.EQ.'RTGLBL') THEN
            IF(surf_curvature(I).EQ.0.0D0) THEN
               RD=0.0D0
            ELSE
               RD=1.0D0/(surf_curvature(I))
            END IF
            SPEC=' '
            SPECC=AA//AA//AA//'               '
            SPECF=.TRUE.
!     Dump data to interface
            !curr_lens_data % radii(I+1) = RD
            !PRINT *, "I = ", I
            !PRINT *, " RD = ", RD
            !PRINT *, "Data says it is ", curr_lens_data%radii(I+1)
            WRITE(*,1000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
            !curr_lens_data % curvatures(I+1) = surf_curvature(I)
            !curr_lens_data % thicknesses(I+1) = surf_thickness(I)
            !curr_lens_data % surf_index(I+1) = INDEX
            !curr_lens_data % surf_vnum(I+1) = VNUM
            !curr_lens_data % glassnames(I+1) = GLANAM(I,1) // GLANAM(I,2)
!       CONIC
            IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
            IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
            IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
            IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
            IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
            IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
            IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
            IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
            IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
            IF(INT(sys_astop()).EQ.I) SPEC='*'
!       GRATING
            IF(surf_diffraction_flag(I).EQ.1.0D0) SPEC='*'
!       CONIC
            IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
            IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
            IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
            IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
            IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
            IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
            IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
            IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
            IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
            IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
            IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
            IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
            IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
            IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
!
            IF(GLANAM(I,2).EQ.'LAST SURFACE') THEN
               IF(GLANAM((I-1),2).EQ.'AIR'.OR.GLANAM((I-1),2).EQ.'REFLTIRO'.OR.GLANAM((I-1),2).EQ.'REFLTIR'.OR.GLANAM((I-1),2).EQ.'REFL') THEN
!
                  AIRY=.TRUE.
                  IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                  IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY) THEN
                  ELSE
!      WRITE(OUTLYNE,2501)
!      CALL SHOWIT(0)
                  END IF
                  WRITE(OUTLYNE,2000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)

                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
1001                 FORMAT(I3,1X,A74)
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               ELSE
                  WRITE(OUTLYNE,2500)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)

                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               END IF
            ELSE
            END IF
!
            IF(I.NE.0) THEN
               IF(GLANAM(I,2).NE.'LAST SURFACE') THEN
                  IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'REFLTIR'.OR.GLANAM(I,2).EQ.'REFLTIRO')THEN
                     IF(GLANAM((I-1),2).EQ.'AIR'.OR.GLANAM((I-1),2).EQ.'REFL'.OR.GLANAM((I-1),2).EQ.'REFLTIR'.OR.GLANAM((I-1),2).EQ.'REFLTIRO') THEN
!
                        AIRY=.TRUE.
                        IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                        IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY) THEN
                        ELSE
!      WRITE(OUTLYNE,2501)
!      CALL SHOWIT(0)
                        END IF
                        WRITE(OUTLYNE,2000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                        CALL SHOWIT(0)
                        IF(SPECF) THEN
                           CALL SHRINKSPEC(SPECC)
                           WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                           CALL SHOWIT(0)
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     ELSE
                        WRITE(OUTLYNE,2500)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                        CALL SHOWIT(0)
                        IF(SPECF) THEN
                           CALL SHRINKSPEC(SPECC)
                           WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                           CALL SHOWIT(0)
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     END IF
                  ELSE
                     IF(GLANAM((I-1),2).NE.'AIR'.AND.GLANAM((I-1),2).NE.'REFL'.AND.GLANAM((I-1),2).NE.'REFLTIR'.AND.GLANAM(I-1,2).NE.'REFLTIRO') THEN
                        IF(GLANAM(I,1).EQ.'MODEL') THEN
                           WRITE(OUTLYNE,1006)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                           CALL SHOWIT(0)
1009                       FORMAT('(MODEL DATA: Nd=',G13.6,1X,'Vd=',G13.6,1X,'DPART=',G13.6,')')
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        ELSE
                           IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     ELSE
!
                        AIRY=.TRUE.
                        IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                        IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY) THEN
                        ELSE
                        END IF
                        IF(GLANAM(I,1).EQ.'MODEL') THEN
                           WRITE(OUTLYNE,1006)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        ELSE
                           IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           !PRINT *, "Writing here?  3036"
                           IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           PRINT *, "Writing here?  3040"

                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     END IF
                  END IF
               ELSE
!       WAS "LAST SURFACE" AND WAS ALREADY DONE JUST ABOVE
               END IF
            ELSE
!       I MUST BE 0
            END IF
!
            IF(I.EQ.0) THEN
               IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'PERFECT'.OR.GLANAM(I,2).EQ.'IDEAL'.OR.GLANAM(I,2).EQ.'REFLTIRO'.OR.GLANAM(I,2).EQ.'REFLTIR')THEN
                  WRITE(OUTLYNE,2500)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               ELSE
                  IF(GLANAM(I,1).EQ.'MODEL') THEN
                     WRITE(OUTLYNE,1006)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     CALL SHOWIT(0)
                     WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                     CALL SHOWIT(0)
                     IF(SPECF) THEN
                        CALL SHRINKSPEC(SPECC)
                        WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                        CALL SHOWIT(0)
                     END IF
                  ELSE
                     IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     PRINT *, "Writing here?  3092"
                     IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     PRINT *, "Writing here?  3096"
                     CALL SHOWIT(0)
                     IF(SPECF) THEN
                        CALL SHRINKSPEC(SPECC)
                        WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                        CALL SHOWIT(0)
                     END IF
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               END IF
            ELSE
!       NOT 0
            END IF
!
         ELSE
!       WC MUST BE CTG
!
            CV=(surf_curvature(I))
            SPEC=' '
            SPECC=AA//AA//AA//'               '
            SPECF=.TRUE.
!       CONIC
            IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
            IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
            IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
            IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
            IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
            IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
            IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
            IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
            IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
            IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
            IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
            IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
            IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
            IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
            IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
            IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
            IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
            IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
            IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
            IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
            IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
            IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
            IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
            IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
!
            IF(GLANAM(I,2).EQ.'LAST SURFACE') THEN
               IF(GLANAM((I-1),2).EQ.'AIR'.OR.GLANAM((I-1),2).EQ.'REFL'.OR.GLANAM((I-1),2).EQ.'REFLTIRO'.OR.GLANAM((I-1),2).EQ.'REFLTIR')THEN
                  AIRY=.TRUE.
                  IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                  IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                  IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY) THEN
                  ELSE
                  END IF
                  WRITE(OUTLYNE,2000)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               ELSE
                  WRITE(OUTLYNE,2500)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               END IF
            ELSE
            END IF
!
            IF(I.NE.0) THEN
               IF(GLANAM(I,2).NE.'LAST SURFACE') THEN
                  IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'TIR'.OR.GLANAM(I,2).EQ.'REFLTIRO')THEN
                     IF(GLANAM((I-1),2).EQ.'AIR'.OR.GLANAM((I-1),2).EQ.'REFL'.OR.GLANAM((I-1),2).EQ.'REFLTIR'.OR.GLANAM((I-1),2).EQ.'REFLTIRO')THEN
                        AIRY=.TRUE.
                        IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                        IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY) THEN
                        ELSE
                        END IF
                        WRITE(OUTLYNE,2000)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                        CALL SHOWIT(0)
                        IF(SPECF) THEN
                           CALL SHRINKSPEC(SPECC)
                           WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                           CALL SHOWIT(0)
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     ELSE
                        WRITE(OUTLYNE,2500)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                        CALL SHOWIT(0)
                        IF(SPECF) THEN
                           CALL SHRINKSPEC(SPECC)
                           WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                           CALL SHOWIT(0)
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     END IF
                  ELSE
                     IF(GLANAM((I-1),2).NE.'AIR'.AND.GLANAM((I-1),2).NE.'REFL'.AND.GLANAM((I-1),2).NE.'REFLTIRO'.AND.GLANAM((I-1),2).NE.'REFLTIR') THEN
                        IF(GLANAM(I,1).EQ.'MODEL') THEN
                           WRITE(OUTLYNE,1006)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        ELSE
                           IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     ELSE
!
                        AIRY=.TRUE.
                        IF(DABS(surf_refractive_index(I-1, 1)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 2)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 3)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 4)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 5)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 6)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 7)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 8)).NE.1.0D0) AIRY=.FALSE.
                        IF(DABS(surf_refractive_index(I-1, 10)).NE.1.0D0) AIRY=.FALSE.
                        IF(GLANAM(I-1,2).EQ.'REFL'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIR'.AND..NOT.AIRY.OR.GLANAM(I-1,2).EQ.'REFLTIRO'.AND..NOT.AIRY) THEN
                        ELSE
                        END IF

                        IF(GLANAM(I,1).EQ.'MODEL') THEN
                           WRITE(OUTLYNE,1006)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        ELSE
                           IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                           CALL SHOWIT(0)
                           IF(SPECF) THEN
                              CALL SHRINKSPEC(SPECC)
                              WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                              CALL SHOWIT(0)
                           END IF
                        END IF
                        IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                           WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                           CALL SHOWIT(0)
                        END IF
                     END IF
                  END IF
               ELSE
!       WAS "LAST SURFACE" AND WAS ALREADY DONE JUST ABOVE
               END IF
            ELSE
!       I MUST BE 0
            END IF
!
            IF(I.EQ.0) THEN
               IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'REFLTIR'.OR.GLANAM(I,2).EQ.'REFLTIRO'.OR.GLANAM(I,2).EQ.'PERFECT'.OR.GLANAM(I,2).EQ.'IDEAL') THEN
                  WRITE(OUTLYNE,2500)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
                  CALL SHOWIT(0)
                  IF(SPECF) THEN
                     CALL SHRINKSPEC(SPECC)
                     WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                     CALL SHOWIT(0)
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               ELSE
                  IF(GLANAM(I,1).EQ.'MODEL') THEN
                     WRITE(OUTLYNE,1006)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     CALL SHOWIT(0)
                     WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
                     CALL SHOWIT(0)
                     IF(SPECF) THEN
                        CALL SHRINKSPEC(SPECC)
                        WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                        CALL SHOWIT(0)
                     END IF
                  ELSE
                     IF(DABS(VNUM).LT.100.0D0)WRITE(OUTLYNE,1005)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     IF(DABS(VNUM).GE.100.0D0)WRITE(OUTLYNE,1000)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
                     CALL SHOWIT(0)
                     IF(SPECF) THEN
                        CALL SHRINKSPEC(SPECC)
                        WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                        CALL SHOWIT(0)
                     END IF
                  END IF
                  IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
                     WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
                     CALL SHOWIT(0)
                  END IF
               END IF
            ELSE
!       NOT 0
            END IF
         END IF
!
10    CONTINUE

      RETURN
   ELSE
!       QUALIFIER NOT "ALL"
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      NF=I
      CALL SINDEX
      IF(WC.EQ.'RTG'.OR.WC.EQ.'RTGLBL') THEN
         IF(surf_curvature(0).EQ.0.0D0) THEN
            RD=0.0D0
         ELSE
            RD=1.0D0/(surf_curvature(0))
         END IF
         SURF=0
         I=0
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         I=SURF
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(GLANAM(0,2).EQ.'AIR'.OR.GLANAM(0,2).EQ.'REFL'.OR.GLANAM(0,2).EQ.'REFLTIRO'.OR.GLANAM(0,2).EQ.'REFLTIR'.OR.GLANAM(0,2).EQ.'PERFECT'.OR.GLANAM(0,2).EQ.'IDEAL') THEN
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2500) SURF,SPEC,RD,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(SURF,2)
            CALL SHOWIT(0)
            IF(SPECF) THEN
               CALL SHRINKSPEC(SPECC)
               WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
               CALL SHOWIT(0)
            END IF
            IF(LBT.AND.surf_label_flag(SURF).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) SURF,LBL(SURF)(1:75)
               CALL SHOWIT(0)
            END IF
         ELSE
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            IF(GLANAM(I,1).EQ.'MODEL') THEN
               WRITE(OUTLYNE,1501)SURF,SPEC,RD,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(SURF,2),INDEX,VNUM
               CALL SHOWIT(0)
               WRITE(OUTLYNE,1009)surf_fict_n(SURF),surf_fict_v(SURF),surf_fict_w(SURF)
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            ELSE
               WRITE(OUTLYNE,1500) SURF,SPEC,RD,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(SURF,2),INDEX,VNUM
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            END IF
            IF(LBT.AND.surf_label_flag(SURF).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) SURF,LBL(SURF)(1:75)
               CALL SHOWIT(0)
            END IF
         END IF
         RETURN
      ELSE
!       WC NOT RTG, MUST BE CTG
         CV=(surf_curvature(0))
         SURF=0
         I=0
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(GLANAM(0,2).EQ.'AIR'.OR.GLANAM(0,2).EQ.'REFL'.OR.GLANAM(0,2).EQ.'PERFECT'.OR.GLANAM(0,2).EQ.'IDEAL'.OR.GLANAM(0,2).EQ.'REFLTIRO'.OR.GLANAM(0,2).EQ.'REFLTIR') THEN
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2500) SURF,SPEC,CV,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(I,2)
            CALL SHOWIT(0)
            IF(SPECF) THEN
               CALL SHRINKSPEC(SPECC)
               WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
               CALL SHOWIT(0)
            END IF
            IF(LBT.AND.surf_label_flag(SURF).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) SURF,LBL(SURF)(1:75)
               CALL SHOWIT(0)
            END IF
         ELSE
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            IF(GLANAM(I,1).EQ.'MODEL') THEN
               WRITE(OUTLYNE,1501)SURF,SPEC,CV,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(SURF,2),INDEX,VNUM
               CALL SHOWIT(0)
               WRITE(OUTLYNE,1009)surf_fict_n(SURF),surf_fict_v(SURF),surf_fict_w(SURF)
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            ELSE
               WRITE(OUTLYNE,1500) SURF,SPEC,CV,surf_thickness(SURF),GLANAM(SURF,1),GLANAM(SURF,2),INDEX,VNUM
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) SURF,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            END IF
            IF(LBT.AND.surf_label_flag(SURF).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) SURF,LBL(SURF)(1:75)
               CALL SHOWIT(0)
            END IF
         END IF
         RETURN
      END IF
   ELSE
!       QUALIFIER NOT "OBJ" OR "OB"
   END IF
!
   IF(SQ.EQ.1.AND.WQ.NE.'OBJ'.OR.SQ.EQ.1.AND.WQ.NE.'ALL'.OR.SQ.EQ.1.AND.WQ.NE.'OB') THEN
      OUTLYNE='INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RTGERROR=.TRUE.
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=INT(sys_last_surf())
      NF=SF
      CALL SINDEX
      IF(WC.EQ.'RTG'.OR.WC.EQ.'RTGLBL') THEN
         IF(surf_curvature(SF).EQ.0.0D0) THEN
            RD=0.0D0
         ELSE
            RD=1.0D0/(surf_curvature(SF))
         END IF
         I=SF
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2500) SF,SPEC,RD,surf_thickness(SF),GLANAM(SF,1),GLANAM(SF,2)
         CALL SHOWIT(0)
         IF(SPECF) THEN
            CALL SHRINKSPEC(SPECC)
            WRITE(OUTLYNE,0202) SF,SPEC//SPECC(1:75)
            CALL SHOWIT(0)
         END IF
         IF(LBT.AND.surf_label_flag(SF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,1001) SF,LBL(SF)(1:75)
            CALL SHOWIT(0)
         END IF
         RETURN
      ELSE
!       WC NOT RTG,MUST BE CTG
         CV=(surf_curvature(SF))
         I=SF
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2500) SF,SPEC,CV,surf_thickness(SF),GLANAM(SF,1),GLANAM(SF,2)
         CALL SHOWIT(0)
         IF(SPECF) THEN
            CALL SHRINKSPEC(SPECC)
            WRITE(OUTLYNE,0202) SF,SPEC//SPECC(1:75)
            CALL SHOWIT(0)
         END IF
         IF(LBT.AND.surf_label_flag(SF).EQ.1.0D0) THEN
            WRITE(OUTLYNE,1001) SF,LBL(SF)(1:75)
            CALL SHOWIT(0)
         END IF
         RETURN
      END IF
   ELSE
!       DON'T OUTPUT IMAGE SURFACE
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=INT(sys_last_surf())
      IF(I.GT.SF.OR.I.LT.0) THEN
         OUTLYNE='SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RTGERROR=.TRUE.
         CALL MACFAL
         RETURN
      END IF
      NF=I
      CALL SINDEX
      IF(WC.EQ.'RTG'.OR.WC.EQ.'RTGLBL') THEN
         IF(surf_curvature(I).EQ.0.0D0) THEN
            RD=0.0D0
         ELSE
            RD=1.0D0/(surf_curvature(I))
         END IF
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'LAST SURFACE'.OR.GLANAM(I,2).EQ.'PERFECT'.OR.GLANAM(I,2).EQ.'IDEAL'.OR.GLANAM(I,2).EQ.'REFLTIR'.OR.GLANAM(I,2).EQ.'REFLTIR') THEN
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2500) I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)

            CALL SHOWIT(0)
            IF(SPECF) THEN
               CALL SHRINKSPEC(SPECC)
               WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
               CALL SHOWIT(0)
            END IF
            IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
               CALL SHOWIT(0)
            END IF
         ELSE
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            IF(GLANAM(I,1).EQ.'MODEL') THEN
               WRITE(OUTLYNE,1501)I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
               CALL SHOWIT(0)
               WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            ELSE
               WRITE(OUTLYNE,1500) I,SPEC,RD,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            END IF
            IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
               CALL SHOWIT(0)
            END IF
         END IF
         RETURN
      ELSE
!       WC NOT RTG, MUST BE CTG
         CV=(surf_curvature(I))
!
         SPEC=' '
         SPECC=AA//AA//AA//'               '
         SPECF=.TRUE.
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPEC='*'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPEC='*'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPEC='*'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPEC='*'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPEC='*'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPEC='*'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPEC='*'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPEC='*'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPEC='*'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPEC='*'
!       CONIC
         IF(surf_conic(I).NE.0.0D0) SPECC(1:5)='Conic'
!       ASPHERIC
         IF(surf_is_asphere(I)) SPECC(7:13)='Asphere'
!       TORIC
         IF(surf_toric_flag(I).NE.0.0D0) SPECC(15:19)='Toric'
!       TILT
         IF(surf_tilt_flag(I).NE.0.0D0) SPECC(21:24)='Tilt'
!       DECENTER
         IF(surf_decenter_flag(I).NE.0.0D0) SPECC(26:33)='Decenter'
!       SPECIAL SURFACE (SPSRF TYPE)
         IF(surf_asi_flag(I).NE.0.0D0) SPECC(35:41)='Special'
!       PIKUPS
         IF(surf_special_type(I).NE.0.0D0) SPECC(43:47)='Pikup'
!       SOLVES
         IF(surf_solve_flag(I).NE.0.0D0) SPECC(49:53)='Solve'
!       REFS
         IF(INT(sys_ref_surf()).EQ.I) SPECC(55:58)='REFS'
!       STOP
         IF(INT(sys_astop()).EQ.I) SPECC(60:63)='STOP'
!       GRT
         IF(surf_diffraction_flag(I).EQ.1.0D0) SPECC(65:67)='GRT'
!       DEFORMABLE SURFACE
         IF(surf_default_flag(I).EQ.1.0D0) SPECC(69:71)='DEF'
!       NSS SURFACE
         IF(surf_ccr_flag(I).NE.0.0D0) SPECC(73:75)='NSS'
         IF(SPECC(1:75).EQ.AA//AA//AA//'               ') SPECF=.FALSE.
         IF(GLANAM(I,2).EQ.'AIR'.OR.GLANAM(I,2).EQ.'REFL'.OR.GLANAM(I,2).EQ.'LAST SURFACE'.OR.GLANAM(I,2).EQ.'PERFECT'.OR.GLANAM(I,2).EQ.'IDEAL'.OR.GLANAM(I,2).EQ.'REFLTIRO'.OR.GLANAM(I,2).EQ.'REFLTIR') THEN
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2500) I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2)
            CALL SHOWIT(0)
            IF(SPECF) THEN
               CALL SHRINKSPEC(SPECC)
               WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
               CALL SHOWIT(0)
            END IF
            IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
               CALL SHOWIT(0)
            END IF
         ELSE
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            IF(GLANAM(I,1).EQ.'MODEL') THEN
               WRITE(OUTLYNE,1501)I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
               CALL SHOWIT(0)
               WRITE(OUTLYNE,1009)surf_fict_n(I),surf_fict_v(I),surf_fict_w(I)
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            ELSE
               WRITE(OUTLYNE,1500) I,SPEC,CV,surf_thickness(I),GLANAM(I,1),GLANAM(I,2),INDEX,VNUM
               CALL SHOWIT(0)
               IF(SPECF) THEN
                  CALL SHRINKSPEC(SPECC)
                  WRITE(OUTLYNE,0202) I,SPEC//SPECC(1:75)
                  CALL SHOWIT(0)
               END IF
            END IF
            IF(LBT.AND.surf_label_flag(I).EQ.1.0D0) THEN
               WRITE(OUTLYNE,1001) I,LBL(I)(1:75)
               CALL SHOWIT(0)
            END IF
         END IF
         RETURN
      END IF
   ELSE
   END IF
   RETURN
1500 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13,1X,G13.6,1X,G10.3)
1501 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13,1X,G13.6,1X,G10.3)
2500 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13)
2000 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13)
1000 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13,1X,G13.6,1X,G10.3)
1005 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13,1X,G13.6,1X,G10.3)
1006 FORMAT(I3,A1,1X,G13.6,1X,G13.6,1X,A7,1X,A13,1X,G13.6,1X,G10.3)
5000 FORMAT('SURF',2X,'RADIUS',8X,'THICKNESS',10X,'MATERIAL',10X,'INDEX',8X,'V-NUM')
6000 FORMAT('SURF',2X,'CURVATURE',5X,'THICKNESS',10X,'MATERIAL',10X,'INDEX',8X,'V-NUM')
5001 FORMAT('BASIC LENS DATA (RADIUS MODE)')
6001 FORMAT('BASIC LENS DATA (CURVATURE MODE)')
2501 FORMAT(1X)
202  FORMAT(I3,A75)
END
SUBROUTINE SHRINKSPEC(SPECC)
   use DATMAI
   IMPLICIT NONE
   CHARACTER SPECC*75,SP1*5,SP2*7,SP3*5,SP4*4,SP5*8,SP6*7,SP7*5 ,SP8*5,SP9*4,SP10*4,SP11*3,SP12*3,SP13*3
   INTEGER I,L1,L2,L3,L4,L5,L6,L7,L8,L9,L10,L11,L12,L13
   L1 =LEN_TRIM(SPECC(1:5))
   L2 =LEN_TRIM(SPECC(7:13))
   L3 =LEN_TRIM(SPECC(15:19))
   L4 =LEN_TRIM(SPECC(21:24))
   L5 =LEN_TRIM(SPECC(26:33))
   L6 =LEN_TRIM(SPECC(35:41))
   L7 =LEN_TRIM(SPECC(43:47))
   L8 =LEN_TRIM(SPECC(49:53))
   L9 =LEN_TRIM(SPECC(55:58))
   L10=LEN_TRIM(SPECC(60:63))
   L11=LEN_TRIM(SPECC(65:67))
   L12=LEN_TRIM(SPECC(69:71))
   L13=LEN_TRIM(SPECC(73:75))
   SP1=SPECC(1:5)
   SP2=SPECC(7:13)
   SP3=SPECC(15:19)
   SP4=SPECC(21:24)
   SP5=SPECC(26:33)
   SP6=SPECC(35:41)
   SP7=SPECC(43:47)
   SP8=SPECC(49:53)
   SP9=SPECC(55:58)
   SP10=SPECC(60:63)
   SP11=SPECC(65:67)
   SP12=SPECC(69:71)
   SP13=SPECC(73:75)
   SPECC(1:75)='                                                               '
   I=0
   IF(L1.EQ.0) THEN
   ELSE
      SPECC(1:L1)=SP1(1:L1)
      I=L1
   END IF
   IF(L2.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L2)=SPECC(1:I)//','//SP2(1:L2)
      I=I+1+L2
   END IF
   IF(L3.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L3)=SPECC(1:I)//','//SP3(1:L3)
      I=I+1+L3
   END IF
   IF(L4.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L4)=SPECC(1:I)//','//SP4(1:L4)
      I=I+1+L4
   END IF
   IF(L5.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L5)=SPECC(1:I)//','//SP5(1:L5)
      I=I+1+L5
   END IF
   IF(L6.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L6)=SPECC(1:I)//','//SP6(1:L6)
      I=I+1+L6
   END IF
   IF(L7.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L7)=SPECC(1:I)//','//SP7(1:L7)
      I=I+1+L7
   END IF
   IF(L8.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L8)=SPECC(1:I)//','//SP8(1:L8)
      I=I+1+L8
   END IF
   IF(L9.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L9)=SPECC(1:I)//','//SP9(1:L9)
      I=I+1+L9
   END IF
   IF(L10.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L10)=SPECC(1:I)//','//SP10(1:L10)
      I=I+1+L10
   END IF
   IF(L11.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L11)=SPECC(1:I)//','//SP11(1:L11)
      I=I+1+L11
   END IF
   IF(L12.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L12)=SPECC(1:I)//','//SP12(1:L12)
      I=I+1+L12
   END IF
   IF(L13.EQ.0) THEN
   ELSE
      SPECC(1:I+1+L13)=SPECC(1:I)//','//SP13(1:L13)
      I=I+1+L13
   END IF
   DO I=1,75
      IF(SPECC(1:1).EQ.' ') SPECC(1:75)=SPECC(2:75)//' '
      IF(SPECC(1:1).NE.' ') GO TO 20
   END DO

20 IF(SPECC(1:1).EQ.',') SPECC(1:75)=SPECC(2:75)//' '

   RETURN
END
! SUB SPL23.FOR

SUBROUTINE SPL23(AR,AX,AY,AZ,IFUNC)
!
   use DATLEN
   use mod_surface
   use DATMAI
   IMPLICIT NONE
!
!     MAIN CUBIC SPLINE SURFACE ROUTINE
!
   REAL*8 XV,XARAY(1:96),YARAY(1:96),AX,AY,AZ,SPARRAY,INCRM,ARHO
!
   INTEGER AR,IFUNC,ISURF,ALLOERR,I,NV
!
   DIMENSION SPARRAY(1:96)
!
!
   REAL*8 SPL23A
   DIMENSION SPL23A(:,:)
   ALLOCATABLE :: SPL23A
!
   NV=INT(FTFL01(1,AR))
   ISURF=AR
!
!     IF IFUNC = 1, JUST DEALLOCATE THE SPL23 ARRAYS. THIS HAPPENS
!     WHENEVER LENNS IS CALLED. THIS FREES MEMORY
!
!     IF IFUNC = 3, INTERPOLATE A SPLINE TO GET A SAG VALUE
!
   IF(IFUNC.EQ.1) THEN
      DEALLOCATE(SPL23A,STAT=ALLOERR)
      RETURN
   END IF
   IF(IFUNC.EQ.3) THEN
!     ALLOCATE THE SPL23 ARRAY FOR THIS SURFACE AND LOAD IT
      DEALLOCATE(SPL23A,STAT=ALLOERR)
      ALLOCATE(SPL23A(1:NV,1:3),STAT=ALLOERR)
      IF(FTFL01(2,AR).EQ.0.0D0) THEN
         INCRM=ALENS(76,ISURF/(DABS(FTFL01(1,ISURF))-1.0D0))
      ELSE
         INCRM=DABS(FTFL01(2,ISURF)/(DABS(FTFL01(1,ISURF))-1.0D0))
      END IF
      XV=0.0D0
      NV=INT(FTFL01(1,ISURF))
      DO I=1,NV
         XARAY(I)=XV
         YARAY(I)=FTFL01(I+2,ISURF)
         XV=XV+INCRM
      END DO
      CALL SPLINE(XARAY,YARAY,NV,1.0D35,1.0D35,SPARRAY)
      NV=INT(FTFL01(1,ISURF))
      ARHO=DSQRT((AX**2)+(AY**2))
      CALL SPLINT(XARAY,YARAY,SPARRAY,NV,ARHO,AZ)
   END IF
   DEALLOCATE(SPL23A,STAT=ALLOERR)
   RETURN
END
