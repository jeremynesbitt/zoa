...
!
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"FOOT" CAUSES A FOOTPRINT RAY GRID TO BE TRACED'
      CALL SHOWIT(1)
...
!
   OUTLYNE=&
   &'BEAM FOOTPRINT DATA BEING GENERATED'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)&
   &'CURRENT RAY GRID SIZE IS : ',(2*FOTLIM)+1,' x ',(2*FOTLIM)+1
   CALL SHOWIT(1)
   OUTLYNE='PLEASE WAIT...'
   CALL SHOWIT(1)
!
...
!       CALC AN AREA
            ...
            &(DIFF(8,KSF)-RAYRAY(2,KSF)))/(DELX*DELY)
            AREA2=((DIFF(1,NEWREF)-RAYRAY(1,NEWREF))*&
            &(DIFF(8,NEWREF)-RAYRAY(2,NEWREF)))/(DELX*DELY)
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE=&
      &'"FOOT GRID" CAUSES A FOOTPRINT GRID SIZE TO BE SET'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'CURRENT FOOT RAY GRID  IS : ',(2*FOTLIM+1),' x ',(2*FOTLIM)+1
      CALL SHOWIT(1)
      RETURN
   END IF
!
...
!
   ...
   &,WWW1,WWW2,D21,D22,GAMMA,JKX,JKY,XL,XM,XN,YL,YM,YN,&
   &D11,D12,LS,SNINDX,SNIND2,JK1,JK2,JK3,&
   &LSTART,MSTART,NSTART,YANG,XANG,X1LAST,LARGE,&
   &Y1LAST,X1ONE,Y1ONE,RXLAST,RYLAST,RXONE,RYONE,DDELX,DDELY,&
   &MAG,MF1,MF2,DET,TARX,TARY,XVALUE,YVALUE,&
   &VALUE,LOLD,MOLD,NOLD,TARRY,TARRX
!
...
!       NO RAY WAS TRACED, NO DATA EXISTS TO DUMP
      WRITE(OUTLYNE,5)&
      &'NO CURRENT RAY DATA EXISTS TO DUMP'
      CALL SHOWIT(0)
5     FORMAT(A34)
      RETURN
   END IF
...
!       RAY WAS TRACED SUCCESSFULLY
      WRITE(OUTLYNE,10)&
      &'CURRENT RAY WAS TRACED SUCCESSFULLY'
      CALL SHOWIT(0)
10    FORMAT(A35)
!
...
!     CIRCULAR AP
         IF(ALENS(A_CLAP_P1,NEWOBJ+1).LE.ALENS(A_CLAP_P2,NEWOBJ+1)) THEN
            IF(DABS(ALENS(A_CLAP_P1,NEWOBJ+1)).LT.DABS(PXTRAX(1,NEWOBJ+1)))&
            &JKX=DABS(ALENS(A_CLAP_P1,NEWOBJ+1))
            IF(DABS(ALENS(A_CLAP_P1,NEWOBJ+1)).LT.DABS(PXTRAY(1,NEWOBJ+1)))&
            &JKY=DABS(ALENS(A_CLAP_P1,NEWOBJ+1))
         ELSE
            IF(DABS(ALENS(A_CLAP_P2,NEWOBJ+1)).LT.DABS(PXTRAX(1,NEWOBJ+1)))&
            &JKX=DABS(ALENS(A_CLAP_P2,NEWOBJ+1))
            IF(DABS(ALENS(A_CLAP_P2,NEWOBJ+1)).LT.DABS(PXTRAY(1,NEWOBJ+1)))&
            &JKY=DABS(ALENS(A_CLAP_P2,NEWOBJ+1))
         END IF
      END IF
...
!     CIRCULAR AP
         IF(DABS(ALENS(A_CLAP_P3,NEWOBJ+1)).LT.DABS(PXTRAX(1,NEWOBJ+1)))&
         &JKX=DABS(ALENS(A_CLAP_P3,NEWOBJ+1))
         IF(DABS(ALENS(A_CLAP_P3,NEWOBJ+1)).LT.DABS(PXTRAY(1,NEWOBJ+1)))&
         &JKY=DABS(ALENS(A_CLAP_P3,NEWOBJ+1))
      END IF
      IF(ALENS(A_APTYPE,NEWOBJ+1).GT.1.0D0.AND.ALENS(A_APTYPE,NEWOBJ+1)&
      &.LE.4.0D0) THEN
!     OTHER AP
         IF(DABS(ALENS(A_CLAP_P2,NEWOBJ+1)).LT.DABS(PXTRAX(1,NEWOBJ+1)))&
         &JKX=DABS(ALENS(A_CLAP_P2,NEWOBJ+1))
...
!
            ELSE
!     TEL ON
               IF(SYSTEM(SYS_SCX).NE.0.0D0)&
               &X1AIM=(LFOB(2)*DABS(PXTRAX(5,NEWOBJ)))+(WW2*JKX)
               IF(SYSTEM(SYS_SCX).EQ.0.0D0)&
               &X1AIM=(WW2*JKX)
               IF(SYSTEM(SYS_SCY).NE.0.0D0)&
               &Y1AIM=(LFOB(1)*(PXTRAY(5,NEWOBJ)))+(WW1*JKY)
               IF(SYSTEM(SYS_SCY).EQ.0.0D0)&
               &Y1AIM=(WW1*JKY)
               Z1AIM=0.0D0
               XC=X1AIM
               YC=Y1AIM
...
!       RAYRAY(41,SURF) TO RAYRAY(50,SURF) RESERVED
   ...
   IF(INT(WW3).GE.1.AND.INT(WW3).LE.5) THEN
      SNIND2=DABS(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))/&
      &ALENS(A_N_OFFSET+INT(WW3),NEWOBJ)
      RN1=(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))
      RN2=(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))
   END IF
   IF(INT(WW3).GE.6.AND.INT(WW3).LE.10) THEN
      SNIND2=DABS(ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ))/&
      &ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ)
      RN1=(ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ))
      RN2=(ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ))
   END IF
...
!     WHAT IS THE SIGN OF THE INDEX IN THE I-1 SPACE
      IF(INT(WW3).GE.1.AND.INT(WW3).LE.5) THEN
         SNINDX=DABS(ALENS(A_N_OFFSET+INT(WW3),I-1))/&
         &ALENS(A_N_OFFSET+INT(WW3),I-1)
         SNIND2=DABS(ALENS(A_N_OFFSET+INT(WW3),I))/&
         &ALENS(A_N_OFFSET+INT(WW3),I)
         RN1=(ALENS(A_N_OFFSET+INT(WW3),I-1))
         RN2=(ALENS(A_N_OFFSET+INT(WW3),I))
      END IF
      IF(INT(WW3).GE.6.AND.INT(WW3).LE.10) THEN
         SNINDX=DABS(ALENS(A_N2_OFFSET+INT(WW3),I-1))/&
         &ALENS(A_N2_OFFSET+INT(WW3),I-1)
         SNIND2=DABS(ALENS(A_N2_OFFSET+INT(WW3),I))/&
         &ALENS(A_N2_OFFSET+INT(WW3),I)
         RN1=(ALENS(A_N2_OFFSET+INT(WW3),I-1))
         RN2=(ALENS(A_N2_OFFSET+INT(WW3),I))
      END IF
      RAYRAY(29,I)=YL
      RAYRAY(30,I)=YM
      RAYRAY(31,I)=YN
...
!
      ...
      IF(GLANAM(I-1,2).EQ.'IDEAL        ') THEN
         RAYRAY(8,I)=-(ALENS(A_IDEAL_FL,I-1)-ALENS(A_THI,I-1))*&
         &RAYRAY(6,I-1)
      END IF
      IF(INT(WW3).GE.1.AND.INT(WW3).LE.5)&
      &RAYRAY(7,I)=RAYRAY(8,I)*DABS(ALENS(A_N_OFFSET+INT(WW3),(I-1)))
      IF(INT(WW3).GE.6.AND.INT(WW3).LE.10)&
      &RAYRAY(7,I)=RAYRAY(8,I)*DABS(ALENS(A_N2_OFFSET+INT(WW3),(I-1)))
...
!     NO CLAP/COBS CHECK FOR TYPE 24 SPECIAL SURFACE
            IF(INT(ALENS(A_MULTICLAP,R_I)).EQ.0.AND.&
            &INT(ALENS(A_MULTICOBS,R_I)).EQ.0) THEN
               CALL CACHEK(0.0D0,0.0D0,0.0D0,0)
            ELSE
...
!
   ...
   &Y1LAST,X1ONE,Y1ONE,RXLAST,RYLAST,RXONE,RYONE,DDELX,DDELY,&
   &XC1,YC1,ZC1,MF1,MF2,DET,TARX,TARY,XVALUE,YVALUE,&
   &TEST,MAG,VALUE,LOLD,MOLD,NOLD,TARRY,TARRX,JKX,JKY,STEPL,STEPL1,&
   &JK_L1,JK_M1,JK_N1,JK_L2,JK_M2,JK_N2,JK_CPL,JK_CPM,JK_CPN&
   &,SA_CPL,SA_CPM,SA_CPN,DP,POLANG,FACT_PAR,FACT_PER,PHASE_PAR&
   &,PHASE_PER,PATHL
!
