...
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"FOOT" ONLY TAKES NUMERIC WORD #1 AND QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'GRID'.AND.WQ.NE.'APE') THEN
         OUTLYNE='"FOOT" ONLY TAKES "GRID" AND "APE" AS QUALIFIER WORDS'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   END IF
   IF(DF1.EQ.1) THEN
      OUTLYNE='"FOOT" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(INT(W1).LT.NEWOBJ.OR.INT(W1).GT.NEWIMG) THEN
      OUTLYNE='SURFACE NUMBER BEYOND RANGE SPECIFIED IN "FOB" COMMAND'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       NO CHIEF RAY EXISTS, STOP
      OUTLYNE='AN "FOB" COMMAND MUST BE ISSUED BEFORE RAYS CAN BE TRACED'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"FOOT GRID" ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W1.LT.2.0D0) THEN
      OUTLYNE='"FOOT GRID" MINIMUM VALUE FOR NUMERIC WORD #1 IS 2'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
            WWW1=WW1
            WWW2=WW2
            IF(sys_aplanatic().EQ.1.0D0.AND.surf_curvature(I).NE.0.0D0.AND.surf_clap_type(I) == 1.AND.surf_clap_dim(I, 3).EQ.0.0D0.AND.surf_clap_dim(I, 4).EQ.0.0D0.AND.surf_clap_tilt(I).EQ.0.0D0) THEN
...
!
            IF(ABS(surf_clap_type(I)) == 1) THEN
               IF(CLAPT) THEN
                  ...
                  END IF
                  IF(sys_flip_ref_x().NE.0.0D0) TARX=-TARX
                  IF(sys_flip_ref_y().NE.0.0D0) TARY=-TARY
...
!
   ...
   ! PRINT *, "WW2 is ", WW2
   ! PRINT *, "WW3 is ", WW3
   ! PRINT *, "SAY is ",  sys_say()
   ! PRINT *, "SAX is ",  sys_sax()
   ...
   !   !PRINT *, "TST YSTRT ", LFOB(1)*REG(9)*TAND(sys_fang_y())
   !   !sys_scy()
   !   YSTRT = WW1*sys_say()-LFOB(1)*REG(9)*TAND(sys_fang_y())
   !   XSTRT = WW2*sys_sax()
   ! END IF
...
!       RAYRAY(41,SURF) TO RAYRAY(50,SURF) RESERVED
   ...
   IF(INT(WW3).GE.1.AND.INT(WW3).LE.5) THEN
      SNIND2=DABS(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))/ALENS(A_N_OFFSET+INT(WW3),NEWOBJ)
      RN1=(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))
      RN2=(ALENS(A_N_OFFSET+INT(WW3),NEWOBJ))
   END IF
   IF(INT(WW3).GE.6.AND.INT(WW3).LE.10) THEN
      SNIND2=DABS(ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ))/ALENS(A_N2_OFFSET+INT(WW3),NEWOBJ)
...
!     WHAT IS THE SIGN OF THE INDEX IN THE I-1 SPACE
      IF(INT(WW3).GE.1.AND.INT(WW3).LE.5) THEN
         SNINDX=DABS(ALENS(A_N_OFFSET+INT(WW3),I-1))/ALENS(A_N_OFFSET+INT(WW3),I-1)
         SNIND2=DABS(ALENS(A_N_OFFSET+INT(WW3),I))/ALENS(A_N_OFFSET+INT(WW3),I)
         RN1=(ALENS(A_N_OFFSET+INT(WW3),I-1))
         RN2=(ALENS(A_N_OFFSET+INT(WW3),I))
      END IF
      IF(INT(WW3).GE.6.AND.INT(WW3).LE.10) THEN
         SNINDX=DABS(ALENS(A_N2_OFFSET+INT(WW3),I-1))/ALENS(A_N2_OFFSET+INT(WW3),I-1)
         SNIND2=DABS(ALENS(A_N2_OFFSET+INT(WW3),I))/ALENS(A_N2_OFFSET+INT(WW3),I)
...
subroutine compute_aim_target(ref_surf, ww1_in, ww2_in, tarx, tary)
   ...
   if (dabs(ALENS(A_APTYPE,ref_surf)) >= 1.0d0 .and. dabs(ALENS(A_APTYPE,ref_surf)) <= 6.0d0 .and. ALENS(A_MULTICLAP,ref_surf) == 0.0d0) then
      ...
      if (sys_aplanatic() == 1.0d0             .and. ALENS(A_CURV,ref_surf)      /= 0.0d0        .and. ALENS(A_APTYPE,ref_surf)    == 1.0d0        .and. ALENS(A_CLAP_YD,ref_surf)   == 0.0d0        .and. ALENS(A_CLAP_XD,ref_surf)   == 0.0d0        .and. ALENS(A_CLAP_TILT,ref_surf) == 0.0d0) then
      ...
      case (AP_CIRC)  ! circular
         if (clapt) then
            ...
            if (sys_flip_ref_x() /= 0.0d0) tarx = -tarx
            if (sys_flip_ref_y() /= 0.0d0) tary = -tary
