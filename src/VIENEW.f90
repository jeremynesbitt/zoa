...
!
  ...
  !PRINT *, "W3 is for j ", W3
    ...
    VIEW2=Si !  Start Surface
    VIEW3=Sf !  End Surface          
...
do ii = 0, numRays-1
  ...
  SAVE_KDP(1)=SAVEINPT(1)
  call VIE_PSM_TRACERAY(0.0D0, relAngle, sysConfig%refWavelengthIndex, Si,Sf)
...
! X Fields (if necessesary)
   ...
   do jj = 1, numFields
    ...
    do ii = 0, numRays-1
      ...
      SAVE_KDP(1)=SAVEINPT(1)
      call VIE_PSM_TRACERAY(relAngle, 0.0D0, sysConfig%refWavelengthIndex, Si,Sf)
...
!     real lens surface keeps its normal profile.  PRO is in device coords here.
      IF(I.EQ.ldm%getStopSurf().AND.DUMMMY(I)) THEN
        ...
        STOPXC=PRO(1,1,I)
        STOPEXT=0.25D0*(STOPYMAX-STOPYMIN)/2.0D0
...
!     LINE TYPE SETTING
      COLPAS=COLPRO
      IF(I.EQ.HILITE_SURF) COLPAS=HILITE_COLOR
      CALL MY_COLTYP(COLPAS)
      OLLNTP=LNTYPE
      LNTYPE=0
