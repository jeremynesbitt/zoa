...
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
      IF(sys_units().EQ.4.0D0)PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
      IF(sys_units().EQ.3.0D0)PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
      IF(sys_units().EQ.2.0D0)PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
      IF(sys_units().EQ.1.0D0)PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
      RETURN
   END IF
END
! SUB PHAS6.FOR
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX6,DERY6,TERM,MAG,FAZE6,DEL,ERR,MMYX,MMYY
!
...
!
   ...
   IF(sys_units().EQ.4.0D0)L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX9,DERY9,TERM,MAG,FAZE9,DEL,ERR,MMYX,MMYY
!
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX10,DERY10,TERM,MAG,FAZE10,DEL,ERR,MMYX,MMYY
!
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX7,DERY7,TERM,MAG,FAZE7,DEL,ERR,MMYY,MMYX
!
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX15,DERY15,TERM,MAG,FAZE15,DEL,ERR,MMYY,MMYX
!
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX13,DERY13,TERM,MAG,FAZE13,DEL,ERR,MMYX,MMYY,WAVEY
!
...
!       PROCEED WITH EVALUATION
   ...
   MMYY=MYY
!
   PHASE=PHASE+(FAZE13(MMYX,MMYY)*WAVEY/FTFL01(2,MYI))
   RETURN
END
FUNCTION FAZE13(ARG1,ARG2)
   ...
   IF(FTFL01(11,MYI).EQ.5.0D0) THEN
      ...
      DO I=1,78
         IF(FTFL01(I+11,MYI).NE.0.0D0) THEN
            FAZE13=FAZE13+((FTFL01(I+11,MYI))*FF4(DABS(ARG1),DABS(ARG2),I))
         END IF
      END DO
   END IF
   RETURN
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 PX,PY,XX,YY,DERX12,DERY12 ,DEL,ERR,WAVEY
!
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 PX,PY,XX,YY,DERX13,DERY13 ,DEL,ERR,WAVEY
!
   EXTERNAL DERX12,DERY12
!
   INTEGER WWVN
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 L1,M1,DERX12,DERY12,N1 ,TERM,MAG,PHAS,FAZE12,DEL,ERR,MMYX,MMYY,WAVEY
!
...
!       PROCEED WITH EVALUATION
   ...
   MMYY=MYY
!
   PHASE=PHASE+(FAZE12(MMYX,MMYY)*WAVEY/FTFL01(2,MYI))
   RETURN
END
...
!
   ...
   IF((FAZE12(X+HH,Y)-FAZE12(X-HH,Y)).EQ.0.0D0) THEN
      ...
      RETURN
   END IF
   A(1,1)=(FAZE12(X+HH,Y)-FAZE12(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE12(X+HH,Y)-FAZE12(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
...
!
   ...
   IF((FAZE12(X,Y+HH)-FAZE12(X,Y-HH)).EQ.0.0D0) THEN
      ...
      RETURN
   END IF
   A(1,1)=(FAZE12(X,Y+HH)-FAZE12(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE12(X,Y+HH)-FAZE12(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 VALUE,NUM5,XRAY(1:4),YRAY(1:4),XFOB(1:4),YFOB(1:4),DDF1(1:4),DDF2(1:4),LAM(1:2),LAMCW
!
   INTEGER II,I,J
!
...
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)'NO CURRENT REFERENCE RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
...
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRREF" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      DO 10 I=NEWOBJ,SF
         ...
         WRITE(OUTLYNE,2000)I,REFRY(1,I),REFRY(2,I),REFRY(3,I),TREFA,TREFB
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      WRITE(OUTLYNE,1500) SF,REFRY(1,SF),REFRY(2,SF),REFRY(3,SF),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P'.AND.SQ.EQ.1) THEN
...
!       OUTPUT IMAGE SURFACE
   ...
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      ...
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)I,REFRY(1,I),REFRY(2,I),REFRY(3,I),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
...
!       OUTPUT IMAGE SURFACE
      ...
      CALL SHOWIT(0)
1600  FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
      SF=NEWIMG
      TREFA=DTAN(REFRY(11,SF))
      TREFB=DTAN(REFRY(12,SF))
      IF(HEADIN) WRITE(OUTLYNE,5000)
   ...
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      ...
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,1X,G13.6,1X,G13.6,1X,G11.4,1X,G11.4)
2000 FORMAT(I3,2X,G13.6,1X,G13.6,1X,G13.6,1X,G11.4,1X,G11.4)
5000 FORMAT('SURF',6X,'X',13X,'Y',13X,'Z',9X,'XA(TAN)',5X,'YA(TAN)')
5001 FORMAT('CURRENT REFERENCE RAY: RAYTRACE DATA ',' - (CFG #',I2,')')
5002 FORMAT('SLOPE ANGLES ARE MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRDIFF.FOR
SUBROUTINE PRDIFF
...
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)'NO CURRENT DIFFERENTIAL RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
...
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"',WC(1:8),'" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'"',WQ(1:8),'" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         IF(WC.EQ.'PRDIFFXM')WRITE(OUTLYNE,2000)I,DIFF(1,I),DIFF(2,I),DIFF(3,I),DIFF(4,I),DIFF(5,I),DIFF(6,I)
         IF(WC.EQ.'PRDIFFYM')WRITE(OUTLYNE,2000)I,DIFF(7,I),DIFF(8,I),DIFF(9,I),DIFF(10,I),DIFF(11,I),DIFF(12,I)
         IF(WC.EQ.'PRDIFFXR')WRITE(OUTLYNE,2000)I,RFDIFF(1,I),RFDIFF(2,I),RFDIFF(3,I),RFDIFF(4,I),RFDIFF(5,I),RFDIFF(6,I)
         IF(WC.EQ.'PRDIFFYR')WRITE(OUTLYNE,2000)I,RFDIFF(7,I),RFDIFF(8,I),RFDIFF(9,I),RFDIFF(10,I),RFDIFF(11,I),RFDIFF(12,I)
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')WRITE(OUTLYNE,2000)SF,DIFF(1,SF),DIFF(2,SF),DIFF(3,SF),DIFF(4,SF),DIFF(5,SF),DIFF(6,SF)
      IF(WC.EQ.'PRDIFFYM')WRITE(OUTLYNE,2000)SF,DIFF(7,SF),DIFF(8,SF),DIFF(9,SF),DIFF(10,SF),DIFF(11,SF),DIFF(12,SF)
      IF(WC.EQ.'PRDIFFXR')WRITE(OUTLYNE,2000)SF,RFDIFF(1,SF),RFDIFF(2,SF),RFDIFF(3,SF),RFDIFF(4,SF),RFDIFF(5,SF),RFDIFF(6,SF)
      IF(WC.EQ.'PRDIFFYR')WRITE(OUTLYNE,2000)SF,RFDIFF(7,SF),RFDIFF(8,SF),RFDIFF(9,SF),RFDIFF(10,SF),RFDIFF(11,SF),RFDIFF(12,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'OBJ'.AND.WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
...
1600  FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
   ...
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      ...
      RETURN
   END IF
1500 FORMAT(I3,2X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4)
2000 FORMAT(I3,2X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4,1X,G11.4)
5000 FORMAT('SURF',7X,'X',11X,'Y',11X,'Z',11X,'L',11X,'M',11X,'N')
5001 FORMAT('XZ-PLANE MARGINAL RAY DIFFERENTIAL RAYTRACE DATA ',' - (CFG #',I2,')')
5002 FORMAT('YZ-PLANE MARGINAL RAY DIFFERENTIAL RAYTRACE DATA ',' - (CFG #',I2,')')
5003 FORMAT('XZ-PLANE CHIEF RAY DIFFERENTIAL RAYTRACE DATA ',' - (CFG #',I2,')')
5004 FORMAT('YZ-PLANE CHIEF RAY DIFFERENTIAL RAYTRACE DATA ',' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PRLMN.FOR
SUBROUTINE PRLMN
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRLMN" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         WRITE(OUTLYNE,1500)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(3,I),RAYRAY(4,I),RAYRAY(5,I),RAYRAY(6,I)
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(3,SF),RAYRAY(4,SF),RAYRAY(5,SF),RAYRAY(6,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
...
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
   ...
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      ...
      RETURN
   END IF
1500 FORMAT(I3,2X,D9.2,2X,D9.2,2X,D9.2,2X,G12.4,2X,G12.4,2X,G12.4)
1600 FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
5000 FORMAT('SURF',6X,'X ',9X,'Y  ',8X,'Z  ',5X,'L-DIRCOS',6X,'M-DIRCOS',6X,'N-DIRCOS')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',' - (CFG #',I2,')')
5002 FORMAT('DIRECTION COSINES MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRXY.FOR
SUBROUTINE PRXY
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRX" AND "PRY" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      DO 10 I=NEWOBJ,SF
         IF(WC.EQ.'PRY') THEN
            ...
            CALL RS(XA,YA)
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),(YA)
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         XA=RAYRAY(11,I)
         ...
         IF(WC.EQ.'PRX') THEN
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),((180.0D0/PII)*XA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000) I,RAYRAY(1,I),XA
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),(DTAN(XA))
            END IF
            CALL SHOWIT(0)
         END IF
10    CONTINUE
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      IF(WC.EQ.'PRY') THEN
         ...
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRX') THEN
         ...
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),((180.0D0/PII)*XA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),(XA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),(DTAN(XA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
...
5001 FORMAT('RAYTRACE DATA (YZ-PLANE)',' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT('SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
6000 FORMAT('SURF',6X,'X ',13X,'XANG')
6001 FORMAT('RAYTRACE DATA (XZ-PLANE)',' - (CFG #',I2,')')
END
! SUB PRR.FOR
SUBROUTINE PRR
!
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRR" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRXYD" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      DO 10 I=NEWOBJ,SF
         ...
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(7,I),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(7,I),(XA),(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(7,I),(DTAN(XA)),(DTAN(YA))
         END IF
         CALL SHOWIT(0)
10    CONTINUE
      IF(.NOT.NULL) THEN
...
!
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      CALL RS(XA,YA)
      IF(DEG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(7,SF),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(7,SF),(XA),(YA)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(7,SF),(DTAN(XA)),(DTAN(YA))
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
...
1600 FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
1650 FORMAT ('OPD = ',G13.6,' WAVES AT WAVELENGTH = ',G13.6,' MICRONS')
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'OPL',11X,'XANG',11X,'YANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT('SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
...
!
   ...
   IF(WC.EQ.'PRXYZ') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PRXYZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(WC.EQ.'PRXYI') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PRXYI" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(WC.EQ.'PRXYIP') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PRXYIP" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       DUMP DATA TO OUTPUT
         ...
         IF(WC.EQ.'PRXYZ') THEN
            ...
            CALL RS(XA,YA)
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(3,I),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(3,I),(XA),(YA)
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),RAYRAY(3,I),(DTAN(XA)),(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         IF(WC.EQ.'PRXYI') THEN
            ...
            IF(DEG) THEN
               ...
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),((180.0D0/PII)*DACOS(COSARG)),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               COSARG=RAYRAY(9,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),DACOS(COSARG),(XA),(YA)
            END IF
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(TANG) THEN
               ...
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),DTAN(DACOS(COSARG)),(DTAN(YA)),(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         IF(WC.EQ.'PRXYIP') THEN
            ...
            IF(TANG) THEN
               ...
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I),DTAN(DACOS(COSARG)),(DTAN(XA)),(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
10    CONTINUE
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      IF(WC.EQ.'PRXYZ') THEN
         ...
         IF(HEADIN) CALL SHOWIT(0)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(3,SF),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(3,SF),XA,(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),RAYRAY(3,SF),(DTAN(XA)),(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYI') THEN
         ...
         IF(DEG) THEN
            ...
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),((180.0D0/PII)*DACOS(COSARG)),((180.0D0/PII)*XA),((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),DACOS(COSARG),XA,(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF),DTAN(DACOS(COSARG)),(DTAN(XA)),(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYIP') THEN
...
1600 FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'Z  ',11X,'XANG',11X,'YANG')
6000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IANG',10X,'XANG',11X,'YANG')
7000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IPANG',9X,'XANG',11X,'YANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT('SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
...
!
   ...
   IF(WC.EQ.'PRZ') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PRZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
5001 FORMAT('RAYTRACE DATA (ALONG Z-AXIS)',' - (CFG #',I2,')')
2501 FORMAT(1X)
END
SUBROUTINE RS(XA,YA)
   use DATMAI
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRFLUX" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
5001 FORMAT('RAYTRACE RELATIVE RAY ENERGY DATA',' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PRPOL.FOR
SUBROUTINE PRPOL
...
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRPOL" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      ...
      DO 10 I=NEWOBJ,SF
         ...
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         WRITE(OUTLYNE,2000)I,RAYRAY(34,I),RAYRAY(35,I),RAYRAY(36,I),RAYRAY(37,I),RAYRAY(38,I)
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
10    CONTINUE
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1500)SF,RAYRAY(34,SF),RAYRAY(35,SF),RAYRAY(36,SF),RAYRAY(37,SF),RAYRAY(38,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
...
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
5000 FORMAT('SURF',2X,'FACTOR-PAR.',4X,'FACTOR-PER.',5X,'PHASE_PAR.',5X,'PHASE_PER',6X,'POLANG(DEG.)')
5001 FORMAT('RAYTRACE POLARIZATION DATA, PHASE IN RADIANS',' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PROPD.FOR
SUBROUTINE PROPD
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"OPD" TAKES NO EXPLICIT INPUT'
      CALL SHOWIT(1)
...
!       MODE FOCAL
         ...
         OCOR=0.0D0
         CALL FOPD
         LEN=LEN-(OCOR*ALENS(WWVN,NEWOBJ))+(RCOR*ALENS(WWVN,NEWOBJ))
         RCOR=0.0D0
         OCOR=0.0D0
         CENCEN=.FALSE.
         CALL LOPD
         LEN=LEN-(OCOR*ALENS(WWVN,NEWIMG-1))+(RCOR*ALENS(WWVN,NEWIMG-1))
      ELSE
!       MODE AFOCAL
         RCOR=0.0D0
         OCOR=0.0D0
      ...
      CALL SHOWIT(0)
100   FORMAT('OPD FOR LAST RAY TRACED = ',G18.10,' LENS UNITS')
      WRITE(OUTLYNE,101)
      CALL SHOWIT(0)
101   FORMAT(1X)
      ...
      IF(INT(CURLAM).GE.6.AND.INT(CURLAM).LE.10) THEN
         ...
         CALL SHOWIT(0)
      END IF
102   FORMAT('OPD FOR LAST RAY TRACED = ',G18.10,' IN WAVES AT ',G12.4,' MICRONS')
   ELSE
!       NO RAY EXITS OR NO REF RAY
      IF(.NOT.REFEXT) THEN
         OUTLYNE='NO REFERENCE RAY DATA EXISTS, "OPD" CAN NOT BE CALCULATED'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(.NOT.RAYEXT) THEN
         OUTLYNE='NO RAY DATA EXISTS, "OPD" CAN NOT BE CALCULATED'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
...
!       NO VERTEX DATA TO OUTPUT
      WRITE(OUTLYNE,*)'NO CURRENT GLOBAL RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   ...
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      WRITE(OUTLYNE,*)WC(1:8),' TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
...
!       PRINT WARNING FOR OBJECT THICKNESS INFINITE
      IF(DABS(surf_thickness(NEWOBJ)).GT.1.0D10)THEN
         ...
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'INFINITE OBJECT THICKNESS (MAGNITUDE GREATER THAN 1.0D+10'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'OBJECT THICKNESS WAS IGNORED IN ALL GLOBAL CALCULATIONS'
         CALL SHOWIT(1)
         JK=NEWOBJ+1
      ELSE
         JK=NEWOBJ
...
12    CONTINUE
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      ...
      IF(DABS(surf_thickness(SF)).GT.1.0D10) THEN
         WRITE(OUTLYNE,*)'NO GLOBAL RAY DATA EXISTS FOR THE OBJECT SURFACE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'WHICH HAS INFINTE THICKNESS (GREATER THAN 1.0D10)'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
...
5001 FORMAT('CURRENT GLOBAL RAY DATA ','  - (CFG #',I2,')')
5002 FORMAT('GLOBAL RAY DATA REFERENCED TO SURFACE NUMBER ',I3)
2501 FORMAT(1X)
...
!
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PRNSS" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
