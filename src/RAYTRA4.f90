!       THIS IS THE FOURTH FILE OF RAYTRACING ROUTINES

! SUB PHASOR.FOR
SUBROUTINE PHASOR
!
   use DATLEN
   use DATMAI
   use mod_surface, only: surf_special_type
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,MAG
!
   INTEGER IR,II,KLI
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
   IF(surf_special_type(MYI) == 6) THEN
!
!     RADIAL PHASE SURFACE
      CALL PHAS6
      RETURN
   END IF
   IF(surf_special_type(MYI) == 9) THEN
!
!     66 TERM ZERNIKE PHASE
      CALL PHAS9
      RETURN
   END IF
   IF(surf_special_type(MYI) == 10) THEN
      CALL PHAS10
!
!     37 TERM ZERNIKE PHASE
      RETURN
   END IF
   IF(surf_special_type(MYI) == 15) THEN
      CALL PHAS15
!
!     48 TERM ABERRATION PHASE
      RETURN
   END IF
   IF(surf_special_type(MYI) == 7) THEN
!
!     91 TERM RECTANGULAR PHASE
      CALL PHAS7
      RETURN
   END IF
   IF(surf_special_type(MYI) == 11) THEN
      REG(40)=REG(9)
      REG(9)=MYX
      REG(10)=MYY
      REG(11)=INR
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN09
!     FOR THE INITIAL USER FUCTION TO BE CALLED
!     PHASE IS RETURNED (IN RADIANS) IN THE IZ-REGISTER
!     CHANGE IN THE X DERIVATIVE OF THE PHASE IS RETURNED IN THE IX-REGISTER
!     CHANGE IN THE Y DERIVATIVE OF THE PHASE IS RETURNED IN THE IY-REGISTER
!
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN9 AND ANY MACRO FUNCTION CALLED BY FUN09.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 201 TO 248 AND ACCESSED BY THE USER VIA RCL COMMANDS
      IR=MYI
      GPREG(201)=FTFL01(1,IR)
      GPREG(202)=FTFL01(2,IR)
      GPREG(203)=FTFL01(3,IR)
      GPREG(204)=FTFL01(4,IR)
      GPREG(205)=FTFL01(5,IR)
      GPREG(206)=FTFL01(6,IR)
      GPREG(207)=FTFL01(7,IR)
      GPREG(208)=FTFL01(8,IR)
      GPREG(209)=FTFL01(9,IR)
      GPREG(210)=FTFL01(10,IR)
      GPREG(211)=FTFL01(11,IR)
      GPREG(212)=FTFL01(12,IR)
      GPREG(213)=FTFL01(13,IR)
      GPREG(214)=FTFL01(14,IR)
      GPREG(215)=FTFL01(15,IR)
      GPREG(216)=FTFL01(16,IR)
      GPREG(217)=FTFL01(17,IR)
      GPREG(218)=FTFL01(18,IR)
      GPREG(219)=FTFL01(19,IR)
      GPREG(220)=FTFL01(20,IR)
      GPREG(221)=FTFL01(21,IR)
      GPREG(222)=FTFL01(22,IR)
      GPREG(223)=FTFL01(23,IR)
      GPREG(224)=FTFL01(24,IR)
      GPREG(225)=FTFL01(25,IR)
      GPREG(226)=FTFL01(26,IR)
      GPREG(227)=FTFL01(27,IR)
      GPREG(228)=FTFL01(28,IR)
      GPREG(229)=FTFL01(29,IR)
      GPREG(230)=FTFL01(30,IR)
      GPREG(231)=FTFL01(31,IR)
      GPREG(232)=FTFL01(32,IR)
      GPREG(233)=FTFL01(33,IR)
      GPREG(234)=FTFL01(34,IR)
      GPREG(235)=FTFL01(35,IR)
      GPREG(236)=FTFL01(36,IR)
      GPREG(237)=FTFL01(37,IR)
      GPREG(238)=FTFL01(38,IR)
      GPREG(239)=FTFL01(39,IR)
      GPREG(240)=FTFL01(40,IR)
      GPREG(241)=FTFL01(41,IR)
      GPREG(242)=FTFL01(42,IR)
      GPREG(243)=FTFL01(43,IR)
      GPREG(244)=FTFL01(44,IR)
      GPREG(245)=FTFL01(45,IR)
      GPREG(246)=FTFL01(46,IR)
      GPREG(247)=FTFL01(47,IR)
      GPREG(248)=FTFL01(48,IR)
      GPREG(249)=FTFL01(49,IR)
      GPREG(250)=FTFL01(50,IR)
      GPREG(251)=FTFL01(51,IR)
      GPREG(252)=FTFL01(52,IR)
      GPREG(253)=FTFL01(53,IR)
      GPREG(254)=FTFL01(54,IR)
      GPREG(255)=FTFL01(55,IR)
      GPREG(256)=FTFL01(56,IR)
      GPREG(257)=FTFL01(57,IR)
      GPREG(258)=FTFL01(58,IR)
      GPREG(259)=FTFL01(59,IR)
      GPREG(260)=FTFL01(60,IR)
      GPREG(261)=FTFL01(61,IR)
      GPREG(262)=FTFL01(62,IR)
      GPREG(263)=FTFL01(63,IR)
      GPREG(264)=FTFL01(64,IR)
      GPREG(265)=FTFL01(65,IR)
      GPREG(266)=FTFL01(66,IR)
      GPREG(267)=FTFL01(67,IR)
      GPREG(268)=FTFL01(68,IR)
      GPREG(269)=FTFL01(69,IR)
      GPREG(270)=FTFL01(70,IR)
      GPREG(271)=FTFL01(71,IR)
      GPREG(272)=FTFL01(72,IR)
      GPREG(273)=FTFL01(73,IR)
      GPREG(274)=FTFL01(74,IR)
      GPREG(275)=FTFL01(75,IR)
      GPREG(276)=FTFL01(76,IR)
      GPREG(277)=FTFL01(77,IR)
      GPREG(278)=FTFL01(78,IR)
      GPREG(279)=FTFL01(79,IR)
      GPREG(280)=FTFL01(80,IR)
      GPREG(281)=FTFL01(81,IR)
      GPREG(282)=FTFL01(82,IR)
      GPREG(283)=FTFL01(83,IR)
      GPREG(284)=FTFL01(84,IR)
      GPREG(285)=FTFL01(85,IR)
      GPREG(286)=FTFL01(86,IR)
      GPREG(287)=FTFL01(87,IR)
      GPREG(288)=FTFL01(88,IR)
      GPREG(289)=FTFL01(89,IR)
      GPREG(290)=FTFL01(90,IR)
      GPREG(291)=FTFL01(91,IR)
      GPREG(292)=FTFL01(92,IR)
      GPREG(293)=FTFL01(93,IR)
      GPREG(294)=FTFL01(94,IR)
      GPREG(295)=FTFL01(95,IR)
      GPREG(296)=FTFL01(96,IR)
!     NO FUN09 EXISTS, RETURN 0.0D0 AS THE Z CONTRIBUTION
      PHASE=0.0D0
      L1=0.0D0
      M1=0.0D0
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
      IF(sys_units().EQ.4.0D0)&
      &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
      IF(sys_units().EQ.3.0D0)&
      &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
      IF(sys_units().EQ.2.0D0)&
      &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
      IF(sys_units().EQ.1.0D0)&
      &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
      RETURN
   END IF
END
! SUB PHAS6.FOR
SUBROUTINE PHAS6
!
!     RADIAL PHASE SURFACE
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX6,DERY6,&
   &TERM,MAG,FAZE6,DEL,ERR,MMYX,MMYY
!
   EXTERNAL FAZE6,DERX6,DERY6
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   TERM=0.0D0
   MMYX=MYX
   MMYY=MYY
   PHASE=FAZE6(MMYX,MMYY)
!
   DEL=DELSUR
   L1=DERX6(MMYX,MMYY,DEL,ERR)
   M1=DERY6(MMYX,MMYY,DEL,ERR)
   IF(sys_units().EQ.4.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   CALL ADDPHASE(RLRL,RMRM,RNRN,L1,M1,LN,MN,NN,PHASE)
!
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
   IF(sys_units().EQ.4.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   RETURN
END
FUNCTION FAZE6(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARG,ARG1,ARG2,FAZE6
   INTEGER I
   FAZE6=0.0D0
   ARG=DSQRT(DBLE(ARG1**2)+DBLE(ARG2**2))
   DO I=9,48
      IF(FTFL01(I,MYI).EQ.0.0D0.OR.ARG.EQ.0.0D0.AND.(I-9).EQ.0) THEN
         FAZE6=FAZE6+FTFL01(I,MYI)
      ELSE
         FAZE6=FAZE6+(FTFL01(I,MYI)*(ARG**(I-9)))
      END IF
   END DO
   RETURN
END
FUNCTION DERX6(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX6,ERR,H,X,Y,FAZE6,CON,CON2,BIG,SAFE
   EXTERNAL FAZE6
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE6(X+HH,Y)-FAZE6(X-HH,Y)).EQ.0.0D0) THEN
      DERX6=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE6(X+HH,Y)-FAZE6(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE6(X+HH,Y)-FAZE6(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX6=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY6(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY6,ERR,H,X,Y,FAZE6,CON,CON2,BIG,SAFE
   EXTERNAL FAZE6
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE6(X,Y+HH)-FAZE6(X,Y-HH)).EQ.0.0D0) THEN
      DERY6=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE6(X,Y+HH)-FAZE6(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE6(X,Y+HH)-FAZE6(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY6=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB PHAS9.FOR
SUBROUTINE PHAS9
!
!     30 TERM ZERNIKE PHASE
!
!     RADIAL PHASE SURFACE
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX9,DERY9,&
   &TERM,MAG,FAZE9,DEL,ERR,MMYX,MMYY
!
   EXTERNAL FAZE9,DERX9,DERY9
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   TERM=0.0D0
   MMYX=MYX/INR
   MMYY=MYY/INR
   PHASE=FAZE9(MMYX,MMYY)
!
   DEL=DELSUR
   L1=DERX9(MMYX,MMYY,DEL,ERR)
   M1=DERY9(MMYX,MMYY,DEL,ERR)
   IF(sys_units().EQ.4.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   CALL ADDPHASE(RLRL,RMRM,RNRN,L1,M1,LN,MN,NN,PHASE)
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
   IF(sys_units().EQ.4.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   RETURN
END
FUNCTION FAZE9(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARGA,ARGB,ARG1,ARG2,FAZE9,FF2
   EXTERNAL FF2
   INTEGER I
   ARGA=DSQRT(DBLE(ARG1**2)+DBLE(ARG2**2))
   IF(ARG1.EQ.0.0D0.AND.ARG2.EQ.0.0D0) THEN
      ARGB=0.0D0
   ELSE
      IF(DABS(ARG2).GE.DABS(((1.0D35)*ARG1))) THEN
         IF(ARG2.GE.0.0D0) THEN
            ARGB=PII/2.0D0
         END IF
         IF(ARG2.LT.0.0D0) THEN
            ARGB=(3.0D0*PII)/2.0D0
         END IF
      ELSE
         IF(DABS(ARG1).EQ.0.0D0.AND.DABS(ARG2).EQ.0.0D0) THEN
            ARGB=0.0D0
         ELSE
            ARGB=DATAN2(ARG1,ARG2)
         END IF
         IF(ARGB.LT.0.0D0) ARGB=ARGB+(TWOPII)
      END IF
   END IF
   FAZE9=0.0D0
   DO I=1,66
      IF(FTFL01(I,MYI).NE.0.0D0) THEN
         FAZE9=FAZE9+(FTFL01(I,MYI)*FF2(ARGA,ARGB,I))
      END IF
   END DO
   RETURN
END
FUNCTION DERX9(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX9,ERR,H,X,Y,FAZE9,CON,CON2,BIG,SAFE
   EXTERNAL FAZE9
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE9(X+HH,Y)-FAZE9(X-HH,Y)).EQ.0.0D0) THEN
      DERX9=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE9(X+HH,Y)-FAZE9(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE9(X+HH,Y)-FAZE9(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX9=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY9(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY9,ERR,H,X,Y,FAZE9,CON,CON2,BIG,SAFE
   EXTERNAL FAZE9
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE9(X,Y+HH)-FAZE9(X,Y-HH)).EQ.0.0D0) THEN
      DERY9=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE9(X,Y+HH)-FAZE9(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE9(X,Y+HH)-FAZE9(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY9=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB PHAS10.FOR
SUBROUTINE PHAS10
!
!     37 TERM ZERNIKE PHASE
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX10,DERY10,&
   &TERM,MAG,FAZE10,DEL,ERR,MMYX,MMYY
!
   EXTERNAL FAZE10,DERX10,DERY10
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   TERM=0.0D0
   MMYX=MYX/INR
   MMYY=MYY/INR
   PHASE=FAZE10(MMYX,MMYY)
!
   DEL=DELSUR
   L1=DERX10(MMYX,MMYY,DEL,ERR)
   IF(sys_units().EQ.4.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   M1=DERY10(MMYX,MMYY,DEL,ERR)
   CALL ADDPHASE(RLRL,RMRM,RNRN,L1,M1,LN,MN,NN,PHASE)
!
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
   IF(sys_units().EQ.4.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   RETURN
END
FUNCTION FAZE10(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARGA,ARGB,ARG1,ARG2,FAZE10,FF3
   EXTERNAL FF3
   INTEGER I
   FAZE10=0.0D0
   ARGA=DSQRT(DBLE(ARG1**2)+DBLE(ARG2**2))
   IF(ARG1.EQ.0.0D0.AND.ARG2.EQ.0.0D0) THEN
      ARGB=0.0D0
   ELSE
      IF(DABS(ARG2).GE.DABS(((1.0D35)*ARG1))) THEN
         IF(ARG2.GE.0.0D0) THEN
            ARGB=PII/2.0D0
         END IF
         IF(ARG2.LT.0.0D0) THEN
            ARGB=(3.0D0*PII)/2.0D0
         END IF
      ELSE
         IF(DABS(ARG1).EQ.0.0D0.AND.DABS(ARG2).EQ.0.0D0) THEN
            ARGB=0.0D0
         ELSE
            ARGB=DATAN2(ARG1,ARG2)
         END IF
         IF(ARGB.LT.0.0D0) ARGB=ARGB+(TWOPII)
      END IF
   END IF
   DO I=1,37
      IF(FTFL01(I,MYI).NE.0.0D0) THEN
         FAZE10=FAZE10+(FTFL01(I,MYI)*FF3(ARGA,ARGB,I))
      END IF
   END DO
   RETURN
END
FUNCTION DERX10(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX10,ERR,H,X,Y,FAZE10,CON,CON2,BIG,SAFE
   EXTERNAL FAZE10
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE10(X+HH,Y)-FAZE10(X-HH,Y)).EQ.0.0D0) THEN
      DERX10=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE10(X+HH,Y)-FAZE10(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE10(X+HH,Y)-FAZE10(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX10=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY10(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY10,ERR,H,X,Y,FAZE10,CON,CON2,BIG,SAFE
   EXTERNAL FAZE10
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE10(X,Y+HH)-FAZE10(X,Y-HH)).EQ.0.0D0) THEN
      DERY10=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE10(X,Y+HH)-FAZE10(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE10(X,Y+HH)-FAZE10(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY10=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB PHAS7.FOR
SUBROUTINE PHAS7
!
!     91 TERM RECTANGULAR PHASE
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX7,DERY7,&
   &TERM,MAG,FAZE7,DEL,ERR,MMYY,MMYX
!
   EXTERNAL FAZE7,DERX7,DERY7
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   TERM=0.0D0
   MMYX=MYX
   MMYY=MYY
   PHASE=FAZE7(MMYX,MMYY)
!
   DEL=DELSUR
   L1=DERX7(MMYX,MMYY,DEL,ERR)
   M1=DERY7(MMYX,MMYY,DEL,ERR)
   IF(sys_units().EQ.4.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   CALL ADDPHASE(RLRL,RMRM,RNRN,L1,M1,LN,MN,NN,PHASE)
!
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
   IF(sys_units().EQ.4.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   RETURN
END
FUNCTION FAZE7(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARG,ARG1,ARG2,FAZE7,FF4
   EXTERNAL FF4
   INTEGER I
   FAZE7=0.0D0
   DO I=1,91
      IF(FTFL01(I,MYI).NE.0.0D0) THEN
         FAZE7=FAZE7+(FTFL01(I,MYI)*FF4(ARG1,ARG2,I))
      END IF
   END DO
   RETURN
END
FUNCTION DERX7(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX7,ERR,H,X,Y,FAZE7,CON,CON2,BIG,SAFE
   EXTERNAL FAZE7
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE7(X+HH,Y)-FAZE7(X-HH,Y)).EQ.0.0D0) THEN
      DERX7=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE7(X+HH,Y)-FAZE7(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE7(X+HH,Y)-FAZE7(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX7=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY7(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY7,ERR,H,X,Y,FAZE7,CON,CON2,BIG,SAFE
   EXTERNAL FAZE7
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE7(X,Y+HH)-FAZE7(X,Y-HH)).EQ.0.0D0) THEN
      DERY7=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE7(X,Y+HH)-FAZE7(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE7(X,Y+HH)-FAZE7(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY7=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB PHAS15.FOR
SUBROUTINE PHAS15
!
!     48 TERM ABERRATION PHASE
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX15,DERY15,&
   &TERM,MAG,FAZE15,DEL,ERR,MMYY,MMYX
!
   EXTERNAL FAZE15,DERX15,DERY15
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   TERM=0.0D0
   MMYX=MYX/INR
   MMYY=MYY/INR
   PHASE=FAZE15(MMYX,MMYY)
!
   DEL=DELSUR
   L1=DERX15(MMYX,MMYY,DEL,ERR)
   M1=DERY15(MMYX,MMYY,DEL,ERR)
   IF(sys_units().EQ.4.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &L1=L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &L1=(L1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   IF(sys_units().EQ.4.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &M1=M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &M1=(M1*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   CALL ADDPHASE(RLRL,RMRM,RNRN,L1,M1,LN,MN,NN,PHASE)
!
!     NOW MULTIPLY BY THE CONTROL WAVELENGTH IN LENS UNITS
   IF(sys_units().EQ.4.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-6
   IF(sys_units().EQ.3.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-3
   IF(sys_units().EQ.2.0D0)&
   &PHASE=PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4
   IF(sys_units().EQ.1.0D0)&
   &PHASE=(PHASE*sys_wavelength(INT(sys_wl_ref()))*1.0D-4)/2.54D0
   RETURN
END
FUNCTION FAZE15(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARGA,ARGB,ARG1,ARG2,FAZE15,FF5
   EXTERNAL FF5
   INTEGER I
   FAZE15=0.0D0
   ARGA=DSQRT(DBLE(ARG1**2)+DBLE(ARG2**2))
   IF(ARG1.EQ.0.0D0.AND.ARG2.EQ.0.0D0) THEN
      ARGB=0.0D0
   ELSE
      IF(DABS(ARG2).GE.DABS(((1.0D35)*ARG1))) THEN
         IF(ARG2.GE.0.0D0) THEN
            ARGB=PII/2.0D0
         END IF
         IF(ARG2.LT.0.0D0) THEN
            ARGB=(3.0D0*PII)/2.0D0
         END IF
      ELSE
         IF(DABS(ARG1).EQ.0.0D0.AND.DABS(ARG2).EQ.0.0D0) THEN
            ARGB=0.0D0
         ELSE
            ARGB=DATAN2(ARG1,ARG2)
         END IF
         IF(ARGB.LT.0.0D0) ARGB=ARGB+(TWOPII)
      END IF
   END IF
   DO I=1,48
      IF(FTFL01(I,MYI).NE.0.0D0) THEN
         FAZE15=FAZE15+(FTFL01(I,MYI)*FF5(ARGA,ARGB,I))
      END IF
   END DO
   RETURN
END
FUNCTION DERX15(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX15,ERR,H,X,Y,FAZE15,CON,CON2,BIG,SAFE
   EXTERNAL FAZE15
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE15(X+HH,Y)-FAZE15(X-HH,Y)).EQ.0.0D0) THEN
      DERX15=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE15(X+HH,Y)-FAZE15(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE15(X+HH,Y)-FAZE15(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX15=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY15(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY15,ERR,H,X,Y,FAZE15,CON,CON2,BIG,SAFE
   EXTERNAL FAZE15
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE15(X,Y+HH)-FAZE15(X,Y-HH)).EQ.0.0D0) THEN
      DERY15=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE15(X,Y+HH)-FAZE15(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE15(X,Y+HH)-FAZE15(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY15=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
SUBROUTINE ADDPHASE(L,M,N,L1,M1,LN,MN,NN,PHASE)
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) N1,MAG,L,M,N,L1,M1,NV,LN,MN,NN,PHASE
   real(real64) IX,IY,LT,MT,NT,X1,Y1
   real(real64) X,Y,IXP,IYP,XN,YN
   NV=0.0D0
   IF(N.GE.0.0D0) NV=1.0D0
   IF(N.LT.0.0D0) NV=-1.0D0
   N1=NV
!     SURFACE NORMAL SLOPES ARE:
   IF(DABS(LN).EQ.0.0D0.AND.DABS(NN).EQ.0.0D0) THEN
      XN=0.0D0
   ELSE
      XN=DATAN2(LN,NN)
   END IF
   IF(DABS(MN).EQ.0.0D0.AND.DABS(NN).EQ.0.0D0) THEN
      YN=0.0D0
   ELSE
      YN=DATAN2(MN,NN)
   END IF
   IF(XN.GE.PII) XN=XN-(TWOPII)
   IF(YN.GE.PII) YN=YN-(TWOPII)
   IF(XN.LT.PII) XN=XN+(TWOPII)
   IF(YN.LT.PII) YN=YN+(TWOPII)
!     ANGLE OF INCIDENCE IS:
   IF(DABS(L).EQ.0.0D0.AND.DABS(N).EQ.0.0D0) THEN
      X=0.0D0
   ELSE
      X=DATAN2(L,N)
   END IF
   IF(DABS(M).EQ.0.0D0.AND.DABS(N).EQ.0.0D0) THEN
      Y=0.0D0
   ELSE
      Y=DATAN2(M,N)
   END IF
   IF(X.GE.PII) X=X-(TWOPII)
   IF(Y.GE.PII) Y=Y-(TWOPII)
   IF(X.LT.PII) X=X+(TWOPII)
   IF(Y.LT.PII) Y=Y+(TWOPII)
   IX=X-XN
   IY=Y-YN
!     ANGLE OF DIFFRACTION IS:
   IF(DABS(L1).EQ.0.0D0.AND.DABS(N1).EQ.0.0D0) THEN
      X1=0.0D0
   ELSE
      X1=DATAN2(L1,N1)
   END IF
   IF(DABS(M1).EQ.0.0D0.AND.DABS(N1).EQ.0.0D0) THEN
      Y1=0.0D0
   ELSE
      Y1=DATAN2(M1,N1)
   END IF
   IF(X1.GE.PII) X1=X1-(TWOPII)
   IF(Y1.GE.PII) Y1=Y1-(TWOPII)
   IF(X1.LT.PII) X1=X1+(TWOPII)
   IF(Y1.LT.PII) Y1=Y1+(TWOPII)
   IXP=DSIN(X1*DCOS(XN))+DSIN(IX)
   IYP=DSIN(Y1*DCOS(YN))+DSIN(IY)
   IF(IXP.GT.1.0D0) IXP=1.0D0
   IF(IYP.GT.1.0D0) IYP=1.0D0
   IF(IXP.LT.-1.0D0) IXP=-1.0D0
   IF(IYP.LT.-1.0D0) IYP=-1.0D0
   IXP=DASIN(IXP)
   IYP=DASIN(IYP)
   IF(IXP.GE.PII) IXP=IXP-(TWOPII)
   IF(IYP.GE.PII) IYP=IYP-(TWOPII)
   IF(IXP.LT.PII) IXP=IXP+(TWOPII)
   IF(IYP.LT.PII) IYP=IYP+(TWOPII)
   LT=DTAN(IXP+XN)
   MT=DTAN(IYP+YN)
   NT=NV*DSQRT((1.0D0)/(1.0D0+(LT**2)+(MT**2)))
   L=LT*NT
   M=MT*NT
   N=NV*DSQRT(1.0D0-(L**2)-(M**2))
   RETURN
END
! SUB PH11.FOR
SUBROUTINE PH11(L1,M1,XX,YY,PHA)
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) WAVEY,L1,M1,PHA,XX,YY
!
   INTEGER IR,II,KLI
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
   IF(WVN.EQ.1) WAVEY=sys_wavelength(1)
   IF(WVN.EQ.2) WAVEY=sys_wavelength(2)
   IF(WVN.EQ.3) WAVEY=sys_wavelength(3)
   IF(WVN.EQ.4) WAVEY=sys_wavelength(4)
   IF(WVN.EQ.5) WAVEY=sys_wavelength(5)
   IF(WVN.EQ.6) WAVEY=sys_wavelength(6)
   IF(WVN.EQ.7) WAVEY=sys_wavelength(7)
   IF(WVN.EQ.8) WAVEY=sys_wavelength(8)
   IF(WVN.EQ.9) WAVEY=sys_wavelength(9)
   IF(WVN.EQ.10) WAVEY=sys_wavelength(10)
   REG(40)=REG(9)
   REG(9)=XX
   REG(10)=YY
   REG(11)=INR
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN09
!     FOR THE INITIAL USER FUCTION TO BE CALLED
!     PHASE IS RETURNED (IN RADIANS) IN THE IZ-REGISTER
!     CHANGE IN THE X DERIVATIVE OF THE PHASE IS RETURNED IN THE IX-REGISTER
!     CHANGE IN THE Y DERIVATIVE OF THE PHASE IS RETURNED IN THE IY-REGISTER
!
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN9 AND ANY MACRO FUNCTION CALLED BY FUN09.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 201 TO 248 AND ACCESSED BY THE USER VIA RCL COMMANDS
   IR=MYI
   GPREG(201)=FTFL01(1,IR)
   GPREG(202)=FTFL01(2,IR)
   GPREG(203)=FTFL01(3,IR)
   GPREG(204)=FTFL01(4,IR)
   GPREG(205)=FTFL01(5,IR)
   GPREG(206)=FTFL01(6,IR)
   GPREG(207)=FTFL01(7,IR)
   GPREG(208)=FTFL01(8,IR)
   GPREG(209)=FTFL01(9,IR)
   GPREG(210)=FTFL01(10,IR)
   GPREG(211)=FTFL01(11,IR)
   GPREG(212)=FTFL01(12,IR)
   GPREG(213)=FTFL01(13,IR)
   GPREG(214)=FTFL01(14,IR)
   GPREG(215)=FTFL01(15,IR)
   GPREG(216)=FTFL01(16,IR)
   GPREG(217)=FTFL01(17,IR)
   GPREG(218)=FTFL01(18,IR)
   GPREG(219)=FTFL01(19,IR)
   GPREG(220)=FTFL01(20,IR)
   GPREG(221)=FTFL01(21,IR)
   GPREG(222)=FTFL01(22,IR)
   GPREG(223)=FTFL01(23,IR)
   GPREG(224)=FTFL01(24,IR)
   GPREG(225)=FTFL01(25,IR)
   GPREG(226)=FTFL01(26,IR)
   GPREG(227)=FTFL01(27,IR)
   GPREG(228)=FTFL01(28,IR)
   GPREG(229)=FTFL01(29,IR)
   GPREG(230)=FTFL01(30,IR)
   GPREG(231)=FTFL01(31,IR)
   GPREG(232)=FTFL01(32,IR)
   GPREG(233)=FTFL01(33,IR)
   GPREG(234)=FTFL01(34,IR)
   GPREG(235)=FTFL01(35,IR)
   GPREG(236)=FTFL01(36,IR)
   GPREG(237)=FTFL01(37,IR)
   GPREG(238)=FTFL01(38,IR)
   GPREG(239)=FTFL01(39,IR)
   GPREG(240)=FTFL01(40,IR)
   GPREG(241)=FTFL01(41,IR)
   GPREG(242)=FTFL01(42,IR)
   GPREG(243)=FTFL01(43,IR)
   GPREG(244)=FTFL01(44,IR)
   GPREG(245)=FTFL01(45,IR)
   GPREG(246)=FTFL01(46,IR)
   GPREG(247)=FTFL01(47,IR)
   GPREG(248)=FTFL01(48,IR)
   GPREG(249)=FTFL01(49,IR)
   GPREG(250)=FTFL01(50,IR)
   GPREG(251)=FTFL01(51,IR)
   GPREG(252)=FTFL01(52,IR)
   GPREG(253)=FTFL01(53,IR)
   GPREG(254)=FTFL01(54,IR)
   GPREG(255)=FTFL01(55,IR)
   GPREG(256)=FTFL01(56,IR)
   GPREG(257)=FTFL01(57,IR)
   GPREG(258)=FTFL01(58,IR)
   GPREG(259)=FTFL01(59,IR)
   GPREG(260)=FTFL01(60,IR)
   GPREG(261)=FTFL01(61,IR)
   GPREG(262)=FTFL01(62,IR)
   GPREG(263)=FTFL01(63,IR)
   GPREG(264)=FTFL01(64,IR)
   GPREG(265)=FTFL01(65,IR)
   GPREG(266)=FTFL01(66,IR)
   GPREG(267)=FTFL01(67,IR)
   GPREG(268)=FTFL01(68,IR)
   GPREG(269)=FTFL01(69,IR)
   GPREG(270)=FTFL01(70,IR)
   GPREG(271)=FTFL01(71,IR)
   GPREG(272)=FTFL01(72,IR)
   GPREG(273)=FTFL01(73,IR)
   GPREG(274)=FTFL01(74,IR)
   GPREG(275)=FTFL01(75,IR)
   GPREG(276)=FTFL01(76,IR)
   GPREG(277)=FTFL01(77,IR)
   GPREG(278)=FTFL01(78,IR)
   GPREG(279)=FTFL01(79,IR)
   GPREG(280)=FTFL01(80,IR)
   GPREG(281)=FTFL01(81,IR)
   GPREG(282)=FTFL01(82,IR)
   GPREG(283)=FTFL01(83,IR)
   GPREG(284)=FTFL01(84,IR)
   GPREG(285)=FTFL01(85,IR)
   GPREG(286)=FTFL01(86,IR)
   GPREG(287)=FTFL01(87,IR)
   GPREG(288)=FTFL01(88,IR)
   GPREG(289)=FTFL01(89,IR)
   GPREG(290)=FTFL01(90,IR)
   GPREG(291)=FTFL01(91,IR)
   GPREG(292)=FTFL01(92,IR)
   GPREG(293)=FTFL01(93,IR)
   GPREG(294)=FTFL01(94,IR)
   GPREG(295)=FTFL01(95,IR)
   GPREG(296)=FTFL01(96,IR)
!     NO FUN09 EXISTS, RETURN 0.0D0 AS THE Z CONTRIBUTION
   PHA=0.0D0
   L1=0.0D0
   M1=0.0D0
   RETURN
END
! SUB PHAS13.FOR
SUBROUTINE PHAS13
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX13,DERY13,&
   &TERM,MAG,FAZE13,DEL,ERR,MMYX,MMYY,WAVEY
!
   EXTERNAL FAZE13,DERX13,DERY13
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
   IF(WVN.EQ.1) WAVEY=sys_wavelength(1)
   IF(WVN.EQ.2) WAVEY=sys_wavelength(2)
   IF(WVN.EQ.3) WAVEY=sys_wavelength(3)
   IF(WVN.EQ.4) WAVEY=sys_wavelength(4)
   IF(WVN.EQ.5) WAVEY=sys_wavelength(5)
   IF(WVN.EQ.6) WAVEY=sys_wavelength(6)
   IF(WVN.EQ.7) WAVEY=sys_wavelength(7)
   IF(WVN.EQ.8) WAVEY=sys_wavelength(8)
   IF(WVN.EQ.9) WAVEY=sys_wavelength(9)
   IF(WVN.EQ.10) WAVEY=sys_wavelength(10)
!
!     RECTANGULAR PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   MMYX=MYX
   MMYY=MYY
!
   PHASE=PHASE+(FAZE13(MMYX,MMYY)*&
   &WAVEY/FTFL01(2,MYI))
   RETURN
END
FUNCTION FAZE13(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARG1,ARG2,FAZE13,FF4
   EXTERNAL FF4
   INTEGER I
   IF(FTFL01(11,MYI).EQ.2.0D0) THEN
      FAZE13=0.0D0
      DO I=1,78
         IF(FTFL01(I+11,MYI).NE.0.0D0) THEN
            FAZE13=FAZE13+((FTFL01(I+11,MYI))*FF4(ARG1,ARG2,I))
         END IF
      END DO
   END IF
   IF(FTFL01(11,MYI).EQ.3.0D0) THEN
      FAZE13=0.0D0
      DO I=1,78
         IF(FTFL01(I+11,MYI).NE.0.0D0) THEN
            FAZE13=FAZE13+((FTFL01(I+11,MYI))*FF4(DABS(ARG1),ARG2,I))
         END IF
      END DO
   END IF
   IF(FTFL01(11,MYI).EQ.4.0D0) THEN
      FAZE13=0.0D0
      DO I=1,78
         IF(FTFL01(I+11,MYI).NE.0.0D0) THEN
            FAZE13=FAZE13+((FTFL01(I+11,MYI))*FF4(ARG1,DABS(ARG2),I))
         END IF
      END DO
   END IF
   IF(FTFL01(11,MYI).EQ.5.0D0) THEN
      FAZE13=0.0D0
      DO I=1,78
         IF(FTFL01(I+11,MYI).NE.0.0D0) THEN
            FAZE13=FAZE13+((FTFL01(I+11,MYI))*&
            &FF4(DABS(ARG1),DABS(ARG2),I))
         END IF
      END DO
   END IF
   RETURN
END
FUNCTION DERX13(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX13,ERR,H,X,Y,FAZE13,CON,CON2,BIG,SAFE
   EXTERNAL FAZE13
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE13(X+HH,Y)-FAZE13(X-HH,Y)).EQ.0.0D0) THEN
      DERX13=0.0D0
   END IF
   A(1,1)=(FAZE13(X+HH,Y)-FAZE13(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE13(X+HH,Y)-FAZE13(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX13=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY13(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY13,ERR,H,X,Y,FAZE13,CON,CON2,BIG,SAFE
   EXTERNAL FAZE13
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   HH=H
   IF((FAZE13(X,Y+HH)-FAZE13(X,Y-HH)).EQ.0.0D0) THEN
      DERY13=0.0D0
   END IF
   A(1,1)=(FAZE13(X,Y+HH)-FAZE13(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE13(X,Y+HH)-FAZE13(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY13=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB PH12.FOR
SUBROUTINE PH12(PX,PY,XX,YY)
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) PX,PY,XX,YY,DERX12,DERY12 &
   &,DEL,ERR,WAVEY
!
   EXTERNAL DERX12,DERY12
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
   IF(WVN.EQ.1) WAVEY=sys_wavelength(1)
   IF(WVN.EQ.2) WAVEY=sys_wavelength(2)
   IF(WVN.EQ.3) WAVEY=sys_wavelength(3)
   IF(WVN.EQ.4) WAVEY=sys_wavelength(4)
   IF(WVN.EQ.5) WAVEY=sys_wavelength(5)
   IF(WVN.EQ.6) WAVEY=sys_wavelength(6)
   IF(WVN.EQ.7) WAVEY=sys_wavelength(7)
   IF(WVN.EQ.8) WAVEY=sys_wavelength(8)
   IF(WVN.EQ.9) WAVEY=sys_wavelength(9)
   IF(WVN.EQ.10) WAVEY=sys_wavelength(10)
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
!
   DEL=DELSUR
   PX=(DERX12(XX,YY,DEL,ERR))*(WAVEY/FTFL01(2,MYI))
   PY=(DERY12(XX,YY,DEL,ERR))*(WAVEY/FTFL01(2,MYI))
   RETURN
END
! SUB PH13.FOR
SUBROUTINE PH13(PX,PY,XX,YY)
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) PX,PY,XX,YY,DERX13,DERY13 &
   &,DEL,ERR,WAVEY
!
   EXTERNAL DERX12,DERY12
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
   IF(WVN.EQ.1) WAVEY=sys_wavelength(1)
   IF(WVN.EQ.2) WAVEY=sys_wavelength(2)
   IF(WVN.EQ.3) WAVEY=sys_wavelength(3)
   IF(WVN.EQ.4) WAVEY=sys_wavelength(4)
   IF(WVN.EQ.5) WAVEY=sys_wavelength(5)
   IF(WVN.EQ.6) WAVEY=sys_wavelength(6)
   IF(WVN.EQ.7) WAVEY=sys_wavelength(7)
   IF(WVN.EQ.8) WAVEY=sys_wavelength(8)
   IF(WVN.EQ.9) WAVEY=sys_wavelength(9)
   IF(WVN.EQ.10) WAVEY=sys_wavelength(10)
!
!     RECT PHASE SURFACE
!
!       PROCEED WITH EVALUATION
!
   DEL=DELSUR
   PX=(DERX13(XX,YY,DEL,ERR))*(WAVEY/FTFL01(2,MYI))
   PY=(DERY13(XX,YY,DEL,ERR))*(WAVEY/FTFL01(2,MYI))
   RETURN
END
! SUB PHAS12.FOR
SUBROUTINE PHAS12
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) L1,M1,DERX12,DERY12,N1 &
   &,TERM,MAG,PHAS,FAZE12,DEL,ERR,MMYX,MMYY,WAVEY
!
   EXTERNAL FAZE12,DERX12,DERY12
!
   INTEGER I,IR,KLI,II
!
   LOGICAL FUN
!
   INTEGER WWVN
   COMMON/WVPASS/WWVN
   IF(WVN.EQ.1) WAVEY=sys_wavelength(1)
   IF(WVN.EQ.2) WAVEY=sys_wavelength(2)
   IF(WVN.EQ.3) WAVEY=sys_wavelength(3)
   IF(WVN.EQ.4) WAVEY=sys_wavelength(4)
   IF(WVN.EQ.5) WAVEY=sys_wavelength(5)
   IF(WVN.EQ.6) WAVEY=sys_wavelength(6)
   IF(WVN.EQ.7) WAVEY=sys_wavelength(7)
   IF(WVN.EQ.8) WAVEY=sys_wavelength(8)
   IF(WVN.EQ.9) WAVEY=sys_wavelength(9)
   IF(WVN.EQ.10) WAVEY=sys_wavelength(10)
!
!     RADIAL PHASE SURFACE
!
!       PROCEED WITH EVALUATION
   MMYX=MYX
   MMYY=MYY
!
   PHASE=PHASE+(FAZE12(MMYX,MMYY)*&
   &WAVEY/FTFL01(2,MYI))
   RETURN
END
FUNCTION FAZE12(ARG1,ARG2)
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) ARG1,ARG2,FAZE12,ARG
   INTEGER I
   IF(FTFL01(11,MYI).EQ.1.0D0) THEN
      ARG=(ARG1**2)+(ARG2**2)
      FAZE12=0.0D0
      DO I=1,10
         IF(ARG.EQ.0.0D0) THEN
         ELSE
            FAZE12=FAZE12+((FTFL01(I+11,MYI))*(ARG**I))
         END IF
      END DO
   END IF
   IF(FTFL01(11,MYI).EQ.6.0D0) THEN
      ARG=DSQRT((ARG1**2)+(ARG2**2))
      FAZE12=0.0D0
      DO I=1,20
         IF(ARG.EQ.0.0D0) THEN
         ELSE
            FAZE12=FAZE12+((FTFL01(I+11,MYI))*(ARG**I))
         END IF
      END DO
   END IF
   RETURN
END
FUNCTION DERX12(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERX12,ERR,H,X,Y,FAZE12,CON,CON2,BIG,SAFE
   EXTERNAL FAZE12
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   IF(X.LT.0.0D0) THEN
      HH=-H
   ELSE
      HH=H
   END IF
   IF((FAZE12(X+HH,Y)-FAZE12(X-HH,Y)).EQ.0.0D0) THEN
      DERX12=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE12(X+HH,Y)&
   &-FAZE12(X-HH,Y))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE12(X+HH,Y)&
      &-FAZE12(X-HH,Y))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERX12=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
FUNCTION DERY12(X,Y,H,ERR)
!
!     BETTER DERIVATIVE ROUTINE FROM NUMERICAL RECIPES 2ND ED.
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DERY12,ERR,H,X,Y,FAZE12,CON,CON2,BIG,SAFE
   EXTERNAL FAZE12
   INTEGER I,J,NTAB
   real(real64) ERRT,FAC,HH,A(1:10,1:10)
!
   CON=1.4D0
   CON2=CON*CON
   BIG=1.0D30
   NTAB=10
   SAFE=2.0D0
!
   IF(Y.LT.0.0D0) THEN
      HH=-H
   ELSE
      HH=H
   END IF
   IF((FAZE12(X,Y+HH)-FAZE12(X,Y-HH)).EQ.0.0D0) THEN
      DERY12=0.0D0
      RETURN
   END IF
   A(1,1)=(FAZE12(X,Y+HH)&
   &-FAZE12(X,Y-HH))/(2.0D0*HH)
   ERR=BIG
   DO I=2,NTAB
      HH=HH/CON
      A(1,I)=(FAZE12(X,Y+HH)&
      &-FAZE12(X,Y-HH))/(2.0D0*HH)
      FAC=CON2
      DO J=2,I
         A(J,I)=(A(J-1,I)*FAC-A(J-1,I-1))/(FAC-1.0D0)
         FAC=CON2*FAC
         ERRT=DMAX1(DABS(A(J,I)-A(J-1,I)),DABS(A(J,I)-A(J-1,I-1)))
         IF(ERRT.LE.ERR) THEN
            ERR=ERRT
            DERY12=A(J,I)
         END IF
      END DO
      IF(DABS(A(I,I)-A(I-1,I-1)).GE.SAFE*ERR) THEN
         RETURN
      END IF
   END DO
   RETURN
END
! SUB OLDSUR.FOR
SUBROUTINE OLDSUR
!     RESTORES THE OBJECT,REFERENCE AND IMAGE SURFACES TO THERE
!     STORED VALUES
!     RESTORES THE OBJECT,REFERENCE AND IMAGE SURFACES TO THERE
!
   use DATLEN
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!
   NEWOBJ=OLDOBJ
   NEWREF=OLDREF
   NEWIMG=OLDIMG
!
   RETURN
END
! SUB RESSUR.FOR
SUBROUTINE RESSUR
!     SAVES THE CURRENT OBJECT,REFERENCE AND IMAGE SURFACES
!     SO THEY CAN BE RESTORED LATER
!
   use DATLEN
   use mod_system, only: sys_last_surf, sys_ref_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!
   OLDOBJ=NEWOBJ
   OLDREF=NEWREF
   OLDIMG=NEWIMG
   NEWOBJ=0
   NEWREF=INT(sys_ref_surf())
   NEWIMG=INT(sys_last_surf())
!
   RETURN
END
SUBROUTINE REALCOLR(I,ERRR)
!
!     DOES CALC FOR PACM,PACZ,SACM,SACZ,PLCM,PLCZ,SLCM,SLCZ
!     FOR GET.FOR AND CALCPRE.FOR
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_mode, sys_wl_pri1, sys_wl_pri2, sys_wl_ref, sys_wl_sec1, sys_wl_sec2
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) VALUE,NUM5,XRAY(1:4),YRAY(1:4)&
   &,XFOB(1:4),YFOB(1:4),DDF1(1:4),DDF2(1:4),LAM(1:2),LAMCW
!
   INTEGER II,I,J
!
   LOGICAL ERRR,OLDLDIF,OLDLDIF2
!
   COMMON/GV/VALUE,NUM5
!
!
   LAMCW=sys_wl_ref()
!     SET COLORS PRIMARY
   IF(I.EQ.1.OR.I.EQ.2.OR.I.EQ.5.OR.I.EQ.6) THEN
!     PRIMARY COLOR
      LAM(1)=sys_wl_pri1()
      LAM(2)=sys_wl_pri2()
   END IF
!
!     SET COLORS SECONDARY
   IF(I.EQ.3.OR.I.EQ.4.OR.I.EQ.7.OR.I.EQ.8) THEN
!     SECONDARY COLOR
      LAM(1)=sys_wl_sec1()
      LAM(2)=sys_wl_sec2()
   END IF
!
!     SET FIELDS AND RAYS FOR AXIAL COLOR
   IF(I.GE.1.AND.I.LE.4) THEN
!     SET FIELDS
      DO J=1,4
         XFOB(J)=0.0D0
         YFOB(J)=0.0D0
      END DO
!     SET RAYS
      IF(I.EQ.1.OR.I.EQ.3) THEN
!     MARGINAL AXIAL
         XRAY(1)=0.0D0
         XRAY(2)=1.0D0
         XRAY(3)=0.0D0
         XRAY(4)=-1.0D0
         YRAY(1)=1.0D0
         YRAY(2)=0.0D0
         YRAY(3)=-1.0D0
         YRAY(4)=0.0D0
      END IF
      IF(I.EQ.2.OR.I.EQ.4) THEN
!     ZONAL AXIAL
         XRAY(1)=0.0D0
         XRAY(2)=0.7D0
         XRAY(3)=0.0D0
         XRAY(4)=-0.7D0
         YRAY(1)=0.7D0
         YRAY(2)=0.0D0
         YRAY(3)=-0.7D0
         YRAY(4)=0.0D0
      END IF
   END IF
!
!     SET FIELDS AND RAYS FOR LATERAL COLOR
   IF(I.GE.5.AND.I.LE.8) THEN
!     SET FIELDS
      IF(I.EQ.5.OR.I.EQ.7) THEN
!     MARGINAL FIELD
         XFOB(1)=0.0D0
         XFOB(2)=1.0D0
         XFOB(3)=0.0D0
         XFOB(4)=-1.0D0
         YFOB(1)=1.0D0
         YFOB(2)=0.0D0
         YFOB(3)=-1.0D0
         YFOB(4)=0.0D0
      END IF
      IF(I.EQ.6.OR.I.EQ.8) THEN
!     ZONAL FIELD
         XFOB(1)=0.0D0
         XFOB(2)=0.7D0
         XFOB(3)=0.0D0
         XFOB(4)=-0.7D0
         YFOB(1)=0.7D0
         YFOB(2)=0.0D0
         YFOB(3)=-0.7D0
         YFOB(4)=0.0D0
      END IF
!     SET RAYS
      DO J=1,4
         XRAY(J)=0.0D0
         YRAY(J)=0.0D0
      END DO
   END IF
   IF(I.GE.1.AND.I.LE.4) THEN
!     TRACE ONE FIELD AND FOUR RAY POSITIONS
!     TRACE CHIEF RAY AT CONTROL WAVELENGTH
!     DO THE FOB
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=0.0D0
      W2=0.0D0
      W3=0.0D0
      W4=LAMCW
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=1
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      REST_KDP(1)=RESTINPT(1)
      IF(RAYCOD(1).NE.0) GO TO 200
      DO II=1,4
! NOW THE 4 RAYS AT THE WAVELENGTH PAIRS AND FORM
!     THE CHROMATIC DIFFERENCES
!     AT LAM1
         SAVE_KDP(1)=SAVEINPT(1)
         WQ='        '
         SQ=0
         SST=0
         W1=YRAY(II)
         W2=XRAY(II)
         W3=LAM(1)
         W4=0.0D0
         W5=0.0D0
         S1=1
         S2=1
         S3=1
         S4=0
         S5=0
         DF1=0
         DF2=0
         DF3=0
         DF4=1
         DF5=1
         SN=1
         WC='RAY     '
         NOCOAT=.TRUE.
         GRASET=.FALSE.
         DXFSET=.FALSE.
         CALL RRAY
         REST_KDP(1)=RESTINPT(1)
         IF(sys_mode().LE.2.0D0) THEN
!     FOCAL
            DDF1(II)=DABS(RAYRAY(1,NEWIMG))
            DDF2(II)=DABS(RAYRAY(2,NEWIMG))
         ELSE
!     AFOCAL
            DDF1(II)=DABS(RAYRAY(11,NEWIMG))
            DDF2(II)=DABS(RAYRAY(12,NEWIMG))
         END IF
         REST_KDP(1)=RESTINPT(1)
         IF(RAYCOD(1).NE.0) GO TO 200
!     AT LAM(2)
         SAVE_KDP(1)=SAVEINPT(1)
         WQ='        '
         SQ=0
         SST=0
         W1=YRAY(II)
         W2=XRAY(II)
         W3=LAM(2)
         W4=0.0D0
         W5=0.0D0
         S1=1
         S2=1
         S3=1
         S4=0
         S5=0
         DF1=0
         DF2=0
         DF3=0
         DF4=1
         DF5=1
         SN=1
         WC='RAY     '
         NOCOAT=.TRUE.
         GRASET=.FALSE.
         DXFSET=.FALSE.
         CALL RRAY
         REST_KDP(1)=RESTINPT(1)
         IF(sys_mode().LE.2.0D0) THEN
!     FOCAL
            DDF1(II)=DABS(DDF1(II)-DABS(RAYRAY(1,NEWIMG)))
            DDF2(II)=DABS(DDF2(II)-DABS(RAYRAY(2,NEWIMG)))
         ELSE
!     AFOCAL
            DDF1(II)=DABS(DDF1(II)-DABS(RAYRAY(11,NEWIMG)))
            DDF2(II)=DABS(DDF2(II)-DABS(RAYRAY(12,NEWIMG)))
            IF(REAL(DDF1(II)).GT.REAL(PII)) DDF1(II)=DDF1(II)-(TWOPII)
            IF(REAL(DDF1(II)).EQ.REAL(TWOPII)) DDF1(II)=0.0D0
            IF(REAL(DDF2(II)).GT.REAL(PII)) DDF2(II)=DDF2(II)-(TWOPII)
            IF(REAL(DDF2(II)).EQ.REAL(TWOPII)) DDF2(II)=0.0D0
         END IF
         REST_KDP(1)=RESTINPT(1)
         IF(RAYCOD(1).NE.0) GO TO 200
      END DO
   END IF
!
   IF(I.GE.5.AND.I.LE.8) THEN
!     TRACE ONE CENTRAL RAY AND FOUR FIELD POSITIONS
!     TRACE CHIEF RAYS AT CONTROL WAVELENGTH
      DO II=1,4
!     DO THE FOB
         SAVE_KDP(1)=SAVEINPT(1)
         OLDLDIF2=LDIF2
         OLDLDIF=LDIF
         LDIF2=.FALSE.
         LDIF=.FALSE.
         WQ='        '
         SQ=0
         SST=0
         STI=0
         W1=YFOB(II)
         W2=XFOB(II)
         W3=0.0D0
         W4=LAMCW
         W5=0.0D0
         DF1=0
         DF2=0
         DF3=1
         DF4=0
         DF5=1
         S1=1
         S2=1
         S3=0
         S4=1
         S5=0
         SN=1
!     SET MSG TO FALSE
         MSG=.FALSE.
         WC='FOB     '
         CALL FFOB
         REST_KDP(1)=RESTINPT(1)
         IF(RAYCOD(1).NE.0) GO TO 200
! NOW THE ONE CENTRAL RAY AT THE WAVELENGTH PAIRS AND FORM
!     THE CHROMATIC DIFFERENCES
!     AT LAM1
         SAVE_KDP(1)=SAVEINPT(1)
         WQ='        '
         SQ=0
         SST=0
         W1=0.0D0
         W2=0.0D0
         W3=LAM(1)
         W4=0.0D0
         W5=0.0D0
         S1=1
         S2=1
         S3=1
         S4=0
         S5=0
         DF1=0
         DF2=0
         DF3=0
         DF4=1
         DF5=1
         SN=1
         WC='RAY     '
         NOCOAT=.TRUE.
         GRASET=.FALSE.
         DXFSET=.FALSE.
         CALL RRAY
         REST_KDP(1)=RESTINPT(1)
         IF(sys_mode().LE.2.0D0) THEN
!     FOCAL
            DDF1(II)=DABS(RAYRAY(1,NEWIMG))
            DDF2(II)=DABS(RAYRAY(2,NEWIMG))
         ELSE
!     AFOCAL
            DDF1(II)=DABS(RAYRAY(11,NEWIMG))
            DDF2(II)=DABS(RAYRAY(12,NEWIMG))
         END IF
         REST_KDP(1)=RESTINPT(1)
         IF(RAYCOD(1).NE.0) GO TO 200
!     AT LAM(2)
         SAVE_KDP(1)=SAVEINPT(1)
         WQ='        '
         SQ=0
         SST=0
         W1=0.0D0
         W2=0.0D0
         W3=LAM(2)
         W4=0.0D0
         W5=0.0D0
         S1=1
         S2=1
         S3=1
         S4=0
         S5=0
         DF1=0
         DF2=0
         DF3=0
         DF4=1
         DF5=1
         SN=1
         WC='RAY     '
         NOCOAT=.TRUE.
         GRASET=.FALSE.
         DXFSET=.FALSE.
         CALL RRAY
         REST_KDP(1)=RESTINPT(1)
         IF(sys_mode().LE.2.0D0) THEN
!     FOCAL
            DDF1(II)=DABS(DDF1(II)-DABS(RAYRAY(1,NEWIMG)))
            DDF2(II)=DABS(DDF2(II)-DABS(RAYRAY(2,NEWIMG)))
         ELSE
!     AFOCAL
            DDF1(II)=DABS(DDF1(II)-DABS(RAYRAY(11,NEWIMG)))
            DDF2(II)=DABS(DDF2(II)-DABS(RAYRAY(12,NEWIMG)))
            IF(REAL(DDF1(II)).GT.REAL(PII)) DDF1(II)=DDF1(II)-(TWOPII)
            IF(REAL(DDF1(II)).EQ.REAL(TWOPII)) DDF1(II)=0.0D0
            IF(REAL(DDF2(II)).GT.REAL(PII)) DDF2(II)=DDF2(II)-(TWOPII)
            IF(REAL(DDF2(II)).EQ.REAL(TWOPII)) DDF2(II)=0.0D0
         END IF
         REST_KDP(1)=RESTINPT(1)
         IF(RAYCOD(1).NE.0) GO TO 200
      END DO
   END IF
!
!     NOW SUM THE ABSOLUTE VALUES OF THE ABERRATIONS
   VALUE=0.0D0
   DO II=1,4
      VALUE=VALUE+DDF1(II)+DDF2(II)
   END DO
   ERRR=.FALSE.
   RETURN
!
!     ERROR DROP THROUGH POINT
200 CONTINUE
   ERRR=.TRUE.
   VALUE=0.0D0
   RETURN
END
! SUB PRREF.FOR
SUBROUTINE PRREF
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRREF. THIS SUBROUTINE IMPLEMENTS
!       THE PRINTOUT OF THE CURRENT REFERENCE RAY RAYTRACE VALUES
!
   INTEGER SF,I,IWAVNM
!
   real(real64) TREFA,TREFB
!
!
   IWAVNM=INT(WVN)
!
   IF(.NOT.REFEXT) THEN
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)&
      &'NO CURRENT REFERENCE RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF

!
   TREFA=0.0D0
   TREFB=0.0D0
!
!       THE REFRY COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT.
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRREF" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRREF" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(sys_last_surf().EQ.0.0D0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO REFERENCE RAY DATA EXISTS'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         TREFA=DTAN(REFRY(11,I))
         TREFB=DTAN(REFRY(12,I))
         WRITE(OUTLYNE,2000)I,REFRY(1,I),REFRY(2,I)&
         &,REFRY(3,I),TREFA,TREFB
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      TREFA=DTAN(REFRY(11,SF))
      TREFB=DTAN(REFRY(12,SF))
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500) SF,REFRY(1,SF),REFRY(2,SF)&
      &,REFRY(3,SF),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'&
   &.AND.WQ.NE.'P'.AND.SQ.EQ.1) THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      TREFA=DTAN(REFRY(11,SF))
      TREFB=DTAN(REFRY(12,SF))
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500) SF,REFRY(1,SF),REFRY(2,SF),&
      &REFRY(3,SF),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      TREFA=DTAN(REFRY(11,I))
      TREFB=DTAN(REFRY(12,I))
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)I,REFRY(1,I),REFRY(2,I),&
      &REFRY(3,I),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
1600  FORMAT('RAY AT WAVL # ',I1,&
      &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
      SF=NEWIMG
      TREFA=DTAN(REFRY(11,SF))
      TREFB=DTAN(REFRY(12,SF))
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500) SF,REFRY(1,SF),REFRY(2,SF),&
      &REFRY(3,SF),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      TREFA=DTAN(REFRY(11,I))
      TREFB=DTAN(REFRY(12,I))
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)I,REFRY(1,I),REFRY(2,I),&
      &REFRY(3,I),TREFA,TREFB
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,1X,G13.6,1X,G13.6,1X,&
   &G11.4,1X,G11.4)
2000 FORMAT(I3,2X,G13.6,1X,G13.6,1X,G13.6,1X,&
   &G11.4,1X,G11.4)
5000 FORMAT('SURF',6X,'X',13X,'Y',13X,'Z',&
   &9X,'XA(TAN)',5X,'YA(TAN)')
5001 FORMAT('CURRENT REFERENCE RAY: RAYTRACE DATA ',&
   &' - (CFG #',I2,')')
5002 FORMAT(&
   &'SLOPE ANGLES ARE MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRDIFF.FOR
SUBROUTINE PRDIFF
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRDIFF. THIS SUBROUTINE IMPLEMENTS
!       THE PRINTOUT OF THE CURRENT DIFFERENTIAL RAY RAYTRACE VALUES
!
!     SUPPORTS COMMANDS PRDIFFXM, PRDIFFYM, PRDIFFXR AND PRDIFFYR
!
   INTEGER SF,I,IWAVNM
!
!
   IWAVNM=INT(WVN)
!
   IF(.NOT.REFEXT.OR..NOT.RAYEXT) THEN
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)&
      &'NO CURRENT DIFFERENTIAL RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.LDIF) THEN
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)&
      &'NO CURRENT DIFFERENTIAL RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF

!       THESE COMMANDS ACCEPT QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT.
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"',WC(1:8),'" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"',WQ(1:8),'" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(sys_last_surf().EQ.0.0D0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO REFERENCE RAY DATA EXISTS'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      IF(WC.EQ.'PRDIFFXM') WRITE(OUTLYNE,5001) INT(F12)
      IF(WC.EQ.'PRDIFFYM') WRITE(OUTLYNE,5002) INT(F12)
      IF(WC.EQ.'PRDIFFXR') WRITE(OUTLYNE,5003) INT(F12)
      IF(WC.EQ.'PRDIFFYR') WRITE(OUTLYNE,5004) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         IF(WC.EQ.'PRDIFFXM')&
         &WRITE(OUTLYNE,2000)I,DIFF(1,I),DIFF(2,I)&
         &,DIFF(3,I),DIFF(4,I),DIFF(5,I),DIFF(6,I)
         IF(WC.EQ.'PRDIFFYM')&
         &WRITE(OUTLYNE,2000)I,DIFF(7,I),DIFF(8,I)&
         &,DIFF(9,I),DIFF(10,I),DIFF(11,I),DIFF(12,I)
         IF(WC.EQ.'PRDIFFXR')&
         &WRITE(OUTLYNE,2000)I,RFDIFF(1,I),RFDIFF(2,I)&
         &,RFDIFF(3,I),RFDIFF(4,I),RFDIFF(5,I),RFDIFF(6,I)
         IF(WC.EQ.'PRDIFFYR')&
         &WRITE(OUTLYNE,2000)I,RFDIFF(7,I),RFDIFF(8,I)&
         &,RFDIFF(9,I),RFDIFF(10,I),RFDIFF(11,I),RFDIFF(12,I)
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(1,SF),DIFF(2,SF)&
      &,DIFF(3,SF),DIFF(4,SF),DIFF(5,SF),DIFF(6,SF)
      IF(WC.EQ.'PRDIFFYM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(7,SF),DIFF(8,SF)&
      &,DIFF(9,SF),DIFF(10,SF),DIFF(11,SF),DIFF(12,SF)
      IF(WC.EQ.'PRDIFFXR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(1,SF),RFDIFF(2,SF)&
      &,RFDIFF(3,SF),RFDIFF(4,SF),RFDIFF(5,SF),RFDIFF(6,SF)
      IF(WC.EQ.'PRDIFFYR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(7,SF),RFDIFF(8,SF)&
      &,RFDIFF(9,SF),RFDIFF(10,SF),RFDIFF(11,SF),RFDIFF(12,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'OBJ'.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(1,SF),DIFF(2,SF)&
      &,DIFF(3,SF),DIFF(4,SF),DIFF(5,SF),DIFF(6,SF)
      IF(WC.EQ.'PRDIFFYM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(7,SF),DIFF(8,SF)&
      &,DIFF(9,SF),DIFF(10,SF),DIFF(11,SF),DIFF(12,SF)
      IF(WC.EQ.'PRDIFFXR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(1,SF),RFDIFF(2,SF)&
      &,RFDIFF(3,SF),RFDIFF(4,SF),RFDIFF(5,SF),RFDIFF(6,SF)
      IF(WC.EQ.'PRDIFFYR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(7,SF),RFDIFF(8,SF)&
      &,RFDIFF(9,SF),RFDIFF(10,SF),RFDIFF(11,SF),RFDIFF(12,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')&
      &WRITE(OUTLYNE,2000)I,DIFF(1,I),DIFF(2,I)&
      &,DIFF(3,I),DIFF(4,I),DIFF(5,I),DIFF(6,I)
      IF(WC.EQ.'PRDIFFYM')&
      &WRITE(OUTLYNE,2000)I,DIFF(7,I),DIFF(8,I)&
      &,DIFF(9,I),DIFF(10,I),DIFF(11,I),DIFF(12,I)
      IF(WC.EQ.'PRDIFFXR')&
      &WRITE(OUTLYNE,2000)I,RFDIFF(1,I),RFDIFF(2,I)&
      &,RFDIFF(3,I),RFDIFF(4,I),RFDIFF(5,I),RFDIFF(6,I)
      IF(WC.EQ.'PRDIFFYR')&
      &WRITE(OUTLYNE,2000)I,RFDIFF(7,I),RFDIFF(8,I)&
      &,RFDIFF(9,I),RFDIFF(10,I),RFDIFF(11,I),RFDIFF(12,I)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
1600  FORMAT('RAY AT WAVL # ',I1,&
      &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(1,SF),DIFF(2,SF)&
      &,DIFF(3,SF),DIFF(4,SF),DIFF(5,SF),DIFF(6,SF)
      IF(WC.EQ.'PRDIFFYM')&
      &WRITE(OUTLYNE,2000)SF,DIFF(7,SF),DIFF(8,SF)&
      &,DIFF(9,SF),DIFF(10,SF),DIFF(11,SF),DIFF(12,SF)
      IF(WC.EQ.'PRDIFFXR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(1,SF),RFDIFF(2,SF)&
      &,RFDIFF(3,SF),RFDIFF(4,SF),RFDIFF(5,SF),RFDIFF(6,SF)
      IF(WC.EQ.'PRDIFFYR')&
      &WRITE(OUTLYNE,2000)SF,RFDIFF(7,SF),RFDIFF(8,SF)&
      &,RFDIFF(9,SF),RFDIFF(10,SF),RFDIFF(11,SF),RFDIFF(12,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(WC.EQ.'PRDIFFXM')&
      &WRITE(OUTLYNE,2000)I,DIFF(1,I),DIFF(2,I)&
      &,DIFF(3,I),DIFF(4,I),DIFF(5,I),DIFF(6,I)
      IF(WC.EQ.'PRDIFFYM')&
      &WRITE(OUTLYNE,2000)I,DIFF(7,I),DIFF(8,I)&
      &,DIFF(9,I),DIFF(10,I),DIFF(11,I),DIFF(12,I)
      IF(WC.EQ.'PRDIFFXR')&
      &WRITE(OUTLYNE,2000)I,RFDIFF(1,I),RFDIFF(2,I)&
      &,RFDIFF(3,I),RFDIFF(4,I),RFDIFF(5,I),RFDIFF(6,I)
      IF(WC.EQ.'PRDIFFYR')&
      &WRITE(OUTLYNE,2000)I,RFDIFF(7,I),RFDIFF(8,I)&
      &,RFDIFF(9,I),RFDIFF(10,I),RFDIFF(11,I),RFDIFF(12,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,G11.4,1X,G11.4,1X,G11.4,1X,&
   &G11.4,1X,G11.4,1X,G11.4)
2000 FORMAT(I3,2X,G11.4,1X,G11.4,1X,G11.4,1X,&
   &G11.4,1X,G11.4,1X,G11.4)
5000 FORMAT('SURF',7X,'X',11X,'Y',11X,'Z',&
   &11X,'L',11X,'M',11X,'N')
5001 FORMAT('XZ-PLANE MARGINAL RAY DIFFERENTIAL RAYTRACE DATA ',&
   &' - (CFG #',I2,')')
5002 FORMAT('YZ-PLANE MARGINAL RAY DIFFERENTIAL RAYTRACE DATA ',&
   &' - (CFG #',I2,')')
5003 FORMAT('XZ-PLANE CHIEF RAY DIFFERENTIAL RAYTRACE DATA ',&
   &' - (CFG #',I2,')')
5004 FORMAT('YZ-PLANE CHIEF RAY DIFFERENTIAL RAYTRACE DATA ',&
   &' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PRLMN.FOR
SUBROUTINE PRLMN
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRLMN. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRLMN' PRINTOUT
!       AT THE CMD LEVEL.
!
   INTEGER SF,IWAVNM,I
!
   LOGICAL DEG,RAD,TANG
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE PRLMN COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRLMN" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRLMN" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         WRITE(OUTLYNE,1500)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(3,I),RAYRAY(4,I),RAYRAY(5,I),RAYRAY(6,I)
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
      &,RAYRAY(3,SF),RAYRAY(4,SF),RAYRAY(5,SF),RAYRAY(6,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
      &,RAYRAY(3,SF),RAYRAY(4,SF),RAYRAY(5,SF),RAYRAY(6,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)I,RAYRAY(1,I),RAYRAY(2,I)&
      &,RAYRAY(3,I),RAYRAY(4,I),RAYRAY(5,I),RAYRAY(6,I)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
      &,RAYRAY(3,SF),RAYRAY(4,SF),RAYRAY(5,SF),RAYRAY(6,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      WRITE(OUTLYNE,1600) IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)I,RAYRAY(1,I),RAYRAY(2,I)&
      &,RAYRAY(3,I),RAYRAY(4,I),RAYRAY(5,I),RAYRAY(6,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,D9.2,2X,D9.2,2X,D9.2,2X,&
   &G12.4,2X,G12.4,2X,G12.4)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
5000 FORMAT('SURF',6X,'X ',9X,'Y  ',8X,'Z  ',&
   &5X,'L-DIRCOS',6X,'M-DIRCOS',6X,'N-DIRCOS')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',&
   &' - (CFG #',I2,')')
5002 FORMAT(&
   &'DIRECTION COSINES MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRXY.FOR
SUBROUTINE PRXY
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRXY. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRY' AND 'PRX' PRINTOUT
!       AT THE CMD LEVEL.
!
   CHARACTER AMODE*8
!
   INTEGER &
   &SF,IWAVNM,I
!
   COMMON/CMODD/AMODE
!
   LOGICAL DEG,RAD,TANG
!
   real(real64) XA,YA
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE PRY COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRX" AND "PRY" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRX" AND "PRY" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
!       SET AMODE
   IF(DEG) THEN
      AMODE='DEGREES '
   END IF
   IF(RAD)  THEN
      AMODE='RADIANS '
   END IF
   IF(TANG) THEN
      AMODE='TANGENT '
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5003) AMODE
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      IF(WC.EQ.'PRY') WRITE(OUTLYNE,5000)
      IF(WC.EQ.'PRX') WRITE(OUTLYNE,6000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         IF(WC.EQ.'PRY') THEN
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
               &((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
               &(YA)
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
               &(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(WC.EQ.'PRX') THEN
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
               &((180.0D0/PII)*XA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000) I,RAYRAY(1,I),XA
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
               &(DTAN(XA))
            END IF
            CALL SHOWIT(0)
         END IF
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(WC.EQ.'PRY') THEN
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRX') THEN
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &((180.0D0/PII)*XA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(XA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(DTAN(XA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(WC.EQ.'PRY') THEN
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRX') THEN
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &((180.0D0/PII)*XA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(XA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(DTAN(XA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(WC.EQ.'PRY') THEN
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRX') THEN
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &((180.0D0/PII)*XA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &(XA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &(DTAN(XA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      IF(WC.EQ.'PRY') THEN
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(2,SF),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRX') THEN
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,SF)
            YA=RAYRAY(12,SF)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &((180.0D0/PII)*XA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,SF)
            YA=RAYRAY(12,SF)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(XA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,SF)
            YA=RAYRAY(12,SF)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),&
            &(DTAN(XA))
            CALL SHOWIT(0)
         END IF
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(WC.EQ.'PRY') THEN
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(2,I),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRX') THEN
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &((180.0D0/PII)*XA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &(XA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),&
            &(DTAN(XA))
            CALL SHOWIT(0)
         END IF
      END IF
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,2X,G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'Y ',13X,'YANG')
5001 FORMAT('RAYTRACE DATA (YZ-PLANE)',&
   &' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT(&
   &'SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
6000 FORMAT('SURF',6X,'X ',13X,'XANG')
6001 FORMAT('RAYTRACE DATA (XZ-PLANE)',&
   &' - (CFG #',I2,')')
END
! SUB PRR.FOR
SUBROUTINE PRR
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRR. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRR' PRINTOUT
!       AT THE CMD LEVEL.
!
   CHARACTER AMODE*8
!
   INTEGER SF,IWAVNM,I
   real(real64) COSARG
!
   real(real64) RR,RRA
!
   COMMON/CMODD/AMODE
!
   LOGICAL DEG,RAD,TANG
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE PRR COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRR" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRR" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
!       SET AMODE
   IF(DEG) THEN
      AMODE='DEGREES '
   END IF
   IF(RAD)  THEN
      AMODE='RADIANS '
   END IF
   IF(TANG) THEN
      AMODE='TANGENT '
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5003) AMODE
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         RR=DSQRT((RAYRAY(1,I)**2)+(RAYRAY(2,I)**2))
         COSARG=RAYRAY(6,I)
         IF(COSARG.LT.0.0D0) COSARG=-COSARG
         IF(COSARG.GT.1.0D0) COSARG=1.0D0
         RRA=DACOS(COSARG)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RR,((180.0D0/PII)*RRA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RR,RRA
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RR,DTAN(RRA)
         END IF
         CALL SHOWIT(0)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      RR=DSQRT((RAYRAY(1,SF)**2)+(RAYRAY(2,SF)**2))
      COSARG=RAYRAY(6,SF)
      IF(COSARG.LT.0.0D0) COSARG=-COSARG
      IF(COSARG.GT.1.0D0) COSARG=1.0D0
      RRA=DACOS(COSARG)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(DEG) THEN
         WRITE(OUTLYNE,2000)SF,RR,((180.0D0/PII)*RRA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,2000)SF,RR,RRA
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,2000)SF,RR,DTAN(RRA)
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      RR=DSQRT((RAYRAY(1,SF)**2)+(RAYRAY(2,SF)**2))
      COSARG=RAYRAY(6,SF)
      IF(COSARG.LT.0.0D0) COSARG=-COSARG
      IF(COSARG.GT.1.0D0) COSARG=1.0D0
      RRA=DACOS(COSARG)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(DEG) THEN
         WRITE(OUTLYNE,2000)SF,RR,((180.0D0/PII)*RRA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,2000)SF,RR,RRA
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,2000)SF,RR,DTAN(RRA)
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      RR=DSQRT((RAYRAY(1,I)**2)+(RAYRAY(2,I)**2))
      COSARG=RAYRAY(6,I)
      IF(COSARG.LT.0.0D0) COSARG=-COSARG
      IF(COSARG.GT.1.0D0) COSARG=1.0D0
      RRA=DACOS(COSARG)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      IF(DEG) THEN
         WRITE(OUTLYNE,2000)I,RR,((180.0D0/PII)*RRA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,2000)I,RR,RRA
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,2000)I,RR,DTAN(RRA)
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      RR=DSQRT((RAYRAY(1,SF)**2)+(RAYRAY(2,SF)**2))
      COSARG=RAYRAY(6,SF)
      IF(COSARG.LT.0.0D0) COSARG=-COSARG
      IF(COSARG.GT.1.0D0) COSARG=1.0D0
      RRA=DACOS(COSARG)
      IF(DEG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,((180.0D0/PII)*RRA)
         CALL SHOWIT(0)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,RRA
         CALL SHOWIT(0)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,DTAN(RRA)
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      RR=DSQRT((RAYRAY(1,SF)**2)+(RAYRAY(2,SF)**2))
      COSARG=RAYRAY(6,SF)
      IF(COSARG.LT.0.0D0) COSARG=-COSARG
      IF(COSARG.GT.1.0D0) COSARG=1.0D0
      RRA=DACOS(COSARG)
      IF(DEG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,((180.0D0/PII)*RRA)
         CALL SHOWIT(0)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,RRA
         CALL SHOWIT(0)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)SF,RR,DTAN(RRA)
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,2X,G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'R ',13X,'R-ANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',&
   &' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT(&
   &'SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRXYD.FOR
SUBROUTINE PRXYD
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf, sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRXYD. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRXYD' PRINTOUT
!       AT THE CMD LEVEL.
!
   CHARACTER AMODE*8
!
   INTEGER SF,IWAVNM,I
!
   real(real64) OPLR,OPLC,OPD,DUMMY,XA,YA
!
   COMMON/CMODD/AMODE
!
   LOGICAL DEG,RAD,TANG,OPDERROR
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE PRXYD COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRXYD" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRXYD" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
!       SET AMODE
   IF(DEG) THEN
      AMODE='DEGREES '
   END IF
   IF(RAD)  THEN
      AMODE='RADIANS '
   END IF
   IF(TANG) THEN
      AMODE='TANGENT '
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5003) AMODE
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(7,I),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(7,I),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(7,I),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
10    CONTINUE
      IF(.NOT.NULL) THEN
!       CALCULATE AND PRINT RAY OPD IN WAVES AT WAVELENGTH
!       THAT RAY WAS TRACED.
         CALL GETOPD(DUMMY,OPD,OPDERROR)
!
!       OUTPUT OPD OF CURRENT RAY AND WAVELENGTH IN MICRONS
         IF(.NOT.OPDERROR) THEN
            WRITE(OUTLYNE,1650) OPD,sys_wavelength(IWAVNM)
            CALL SHOWIT(0)
         END IF
      END IF
!
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      IF(DEG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),((180.0D0/PII)*XA),&
         &((180.0D0/PII)*YA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(XA),&
         &(YA)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(DTAN(XA)),&
         &(DTAN(YA))
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      IF(DEG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),((180.0D0/PII)*XA),&
         &((180.0D0/PII)*YA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(XA),&
         &(YA)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(DTAN(XA)),&
         &(DTAN(YA))
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,I)
      YA=RAYRAY(12,I)
      CALL RS(XA,YA)
      IF(DEG) THEN
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),((180.0D0/PII)*XA),&
         &((180.0D0/PII)*YA)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),XA,&
         &(YA)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),(DTAN(XA)),&
         &(DTAN(YA))
      END IF
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      IF(DEG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),((180.0D0/PII)*XA),&
         &((180.0D0/PII)*YA)
         CALL SHOWIT(0)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(XA),&
         &(YA)
         CALL SHOWIT(0)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
         &,RAYRAY(7,SF),(DTAN(XA)),&
         &(DTAN(YA))
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(DEG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),((180.0D0/PII)*XA),&
         &((180.0D0/PII)*YA)
         CALL SHOWIT(0)
      END IF
      IF(RAD) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),(XA),&
         &(YA)
         CALL SHOWIT(0)
      END IF
      IF(TANG) THEN
         WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
         &,RAYRAY(7,I),(DTAN(XA)),&
         &(DTAN(YA))
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
1650 FORMAT &
   &('OPD = ',G13.6,' WAVES AT WAVELENGTH = ',G13.6,' MICRONS')
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'OPL',&
   &11X,'XANG',11X,'YANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',&
   &' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT(&
   &'SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRRAY.FOR
SUBROUTINE PRRAY
   use global_widgets
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRRAY. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRXYZ','PRXYI' AND 'PRXYIP' PRINTOUT
!       AT THE CMD LEVEL.
!
   CHARACTER AMODE*8
!
   real(real64) COSARG,XA,YA
!
   INTEGER &
   &SF,IWAVNM,I
!
   COMMON/CMODD/AMODE
!
   LOGICAL DEG,RAD,TANG
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(WC.EQ.'PRXYZ') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   END IF
!
   IF(WC.EQ.'PRXYI') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYI" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYI" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   END IF
!
   IF(WC.EQ.'PRXYIP') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYIP" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
         WRITE(OUTLYNE,*)&
         &'"PRXYIP" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   END IF


!
!       SET AMODE
   IF(DEG) THEN
      AMODE='DEGREES '
   END IF
   IF(RAD)  THEN
      AMODE='RADIANS '
   END IF
   IF(TANG) THEN
      AMODE='TANGENT '
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5003) AMODE
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      IF(WC.EQ.'PRXYZ')WRITE(OUTLYNE,5000)
      IF(WC.EQ.'PRXYI')WRITE(OUTLYNE,6000)
      IF(WC.EQ.'PRXYIP')WRITE(OUTLYNE,7000)
      CALL SHOWIT(0)

      DO 10 I=NEWOBJ,SF
!       DUMP DATA TO OUTPUT
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)


         IF(WC.EQ.'PRXYZ') THEN
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(DEG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,RAYRAY(3,I),((180.0D0/PII)*XA),&
               &((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,RAYRAY(3,I),(XA),&
               &(YA)
            END IF
            IF(TANG) THEN
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,RAYRAY(3,I),(DTAN(XA)),&
               &(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         IF(WC.EQ.'PRXYI') THEN
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(DEG) THEN
               COSARG=RAYRAY(9,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,((180.0D0/PII)*DACOS(COSARG)),&
               &((180.0D0/PII)*XA),&
               &((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               COSARG=RAYRAY(9,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,DACOS(COSARG),(XA),&
               &(YA)
            END IF
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(TANG) THEN
               COSARG=RAYRAY(9,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,DTAN(DACOS(COSARG)),(DTAN(YA)),&
               &(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
         IF(WC.EQ.'PRXYIP') THEN
            XA=RAYRAY(11,I)
            YA=RAYRAY(12,I)
            CALL RS(XA,YA)
            IF(DEG) THEN
               COSARG=RAYRAY(10,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,((180.0D0/PII)*DACOS(COSARG)),&
               &((180.0D0/PII)*XA),&
               &((180.0D0/PII)*YA)
            END IF
            IF(RAD) THEN
               COSARG=RAYRAY(10,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,DACOS(COSARG),XA,&
               &(YA)
            END IF
            IF(TANG) THEN
               COSARG=RAYRAY(10,I)
               IF(COSARG.LT.0.0D0) COSARG=-COSARG
               IF(COSARG.GT.1.0D0) COSARG=1.0D0
               WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
               &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
               &(DTAN(YA))
            END IF
            CALL SHOWIT(0)
         END IF
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(WC.EQ.'PRXYZ') THEN
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),XA,&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYI') THEN
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(DEG) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),XA,&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYIP') THEN
         IF(HEADIN) WRITE(OUTLYNE,7000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(WC.EQ.'PRXYZ') THEN
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYI') THEN
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(DEG) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),XA,&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYIP') THEN
         IF(HEADIN) WRITE(OUTLYNE,7000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(WC.EQ.'PRXYZ') THEN
         IF(HEADIN) WRITE(OUTLYNE,5000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYI') THEN
         IF(HEADIN) WRITE(OUTLYNE,6000)
         IF(HEADIN) CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DACOS(COSARG),(XA),&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      IF(WC.EQ.'PRXYIP') THEN
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(HEADIN) WRITE(OUTLYNE,7000)
         IF(HEADIN) CALL SHOWIT(0)
         IF(DEG) THEN
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
         END IF
         IF(RAD) THEN
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DACOS(COSARG),XA,&
            &(YA)
         END IF
         IF(TANG) THEN
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
         END IF
         CALL SHOWIT(0)
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      IF(WC.EQ.'PRXYZ') THEN
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,RAYRAY(3,SF),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRXYI') THEN
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            XA=RAYRAY(11,SF)
            YA=RAYRAY(12,SF)
            CALL RS(XA,YA)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRXYIP') THEN
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            XA=RAYRAY(11,SF)
            YA=RAYRAY(12,SF)
            CALL RS(XA,YA)
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         XA=RAYRAY(11,SF)
         YA=RAYRAY(12,SF)
         CALL RS(XA,YA)
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DACOS(COSARG),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(10,SF)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,1500)SF,RAYRAY(1,SF),RAYRAY(2,SF)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(WC.EQ.'PRXYZ') THEN
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,5000)
            IF(HEADIN) CALL SHOWIT(0)
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,RAYRAY(3,I),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRXYI') THEN
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DACOS(COSARG),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,6000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(9,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      IF(WC.EQ.'PRXYIP') THEN
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         IF(DEG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,((180.0D0/PII)*DACOS(COSARG)),&
            &((180.0D0/PII)*XA),&
            &((180.0D0/PII)*YA)
            CALL SHOWIT(0)
         END IF
         IF(RAD) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DACOS(COSARG),(XA),&
            &(YA)
            CALL SHOWIT(0)
         END IF
         IF(TANG) THEN
            WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
            CALL SHOWIT(0)
            IF(HEADIN) WRITE(OUTLYNE,7000)
            IF(HEADIN) CALL SHOWIT(0)
            COSARG=RAYRAY(10,I)
            IF(COSARG.LT.0.0D0) COSARG=-COSARG
            IF(COSARG.GT.1.0D0) COSARG=1.0D0
            WRITE(OUTLYNE,2000)I,RAYRAY(1,I),RAYRAY(2,I)&
            &,DTAN(DACOS(COSARG)),(DTAN(XA)),&
            &(DTAN(YA))
            CALL SHOWIT(0)
         END IF
      END IF
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'Z  ',&
   &11X,'XANG',11X,'YANG')
6000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IANG',&
   &10X,'XANG',11X,'YANG')
7000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IPANG',&
   &9X,'XANG',11X,'YANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',&
   &' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT(&
   &'SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
! SUB PRZ.FOR
SUBROUTINE PRZ
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf, sys_wavelength
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRY. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE 'PRZ' PRINTOUT
!       AT THE CMD LEVEL.
!
   CHARACTER AMODE*8
   INTEGER SF,IWAVNM,I
!
   COMMON/CMODD/AMODE
!
   real(real64) OPLR,OPLC,OPD,DUMMY,XA,YA
!
   LOGICAL DEG,RAD,TANG,OPDERROR
!
   COMMON/ANGMOD/DEG,RAD,TANG
!
!
!       THE PRZ COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(WC.EQ.'PRZ') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"PRZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
         WRITE(OUTLYNE,*)&
         &'"PRZ" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   END IF
!
!
!       SET AMODE
   IF(DEG) THEN
      AMODE='DEGREES '
   END IF
   IF(RAD)  THEN
      AMODE='RADIANS '
   END IF
   IF(TANG) THEN
      AMODE='TANGENT '
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         WRITE(OUTLYNE,2000)I,RAYRAY(3,I),RAYRAY(7,I)
         CALL SHOWIT(0)
10    CONTINUE
      IF(.NOT.NULL) THEN
!       CALCULATE AND PRINT RAY OPD IN WAVES AT WAVELENGTH
!       THAT RAY WAS TRACED.
         CALL GETOPD(DUMMY,OPD,OPDERROR)
!
!       OUTPUT OPD OF CURRENT RAY AND WAVELENGTH IN MICRONS
         IF(.NOT.OPDERROR) THEN
            WRITE(OUTLYNE,1650) OPD,sys_wavelength(IWAVNM)
            CALL SHOWIT(0)
         END IF
      END IF
!
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(3,SF),RAYRAY(7,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(3,SF),RAYRAY(7,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)I,RAYRAY(3,I),RAYRAY(7,I)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(3,SF),RAYRAY(7,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)I,RAYRAY(3,I),RAYRAY(7,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
1650 FORMAT &
   &('OPD = ',G13.6,' WAVES AT WAVELENGTH = ',G13.6,' MICRONS')
2000 FORMAT(I3,2X,G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'Z ',13X,'OPL')
5001 FORMAT('RAYTRACE DATA (ALONG Z-AXIS)',&
   &' - (CFG #',I2,')')
2501 FORMAT(1X)
END
SUBROUTINE RS(XA,YA)
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) XA,YA
   IF(XA.GT.PII) XA=XA-(TWOPII)
   IF(XA.LT.-PII) XA=XA+(TWOPII)
   IF(XA.EQ.TWOPII) XA=0.0D0
   IF(YA.GT.PII) YA=YA-(TWOPII)
   IF(YA.LT.-PII) YA=YA+(TWOPII)
   IF(YA.EQ.TWOPII) YA=0.0D0
   RETURN
END
! SUB PRFLUX.FOR
SUBROUTINE PRFLUX
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRFLUX. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE PRINTOUT WHICH LISTS THE RAY ENERGY
!       IN THE LOCAL X AND Y DIRECTIONS AT EACH SURFACE
!       AFTER SURFACE INTERACTION
!       THIS OPERATES AT THE CMD LEVEL.
!
   real(real64) XA,YA
!
   INTEGER &
   &SF,IWAVNM,I
!
!
!       THE PRFLUX COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT SRTING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRFLUX" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRFLUX" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         WRITE(OUTLYNE,2000)I,RAYRAY(25,I)
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1500)SF,RAYRAY(25,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1500)SF,RAYRAY(25,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,I)
      YA=RAYRAY(12,I)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,2000)I,RAYRAY(25,I)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(25,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      XA=RAYRAY(11,I)
      YA=RAYRAY(12,I)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)I,RAYRAY(25,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,12X,G13.6)
5000 FORMAT('SURF',6X,'RELATIVE RAY ENERGY')
5001 FORMAT('RAYTRACE RELATIVE RAY ENERGY DATA',&
   &' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PRPOL.FOR
SUBROUTINE PRPOL
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRPOL. THIS SUBROUTINE IMPLEMENTS
!       THE RAY TRACE PRINTOUT WHICH LISTS THE RAY POLARIZATION
!       INFORMATION WHEN POLARIZATION RAY TRACING IS ON.
!       THIS OPERATES AT THE CMD LEVEL.
!
   real(real64) XA,YA
!
   INTEGER &
   &SF,IWAVNM,I
!
!
!       THE PRPOL COMMAND ACCEPTS QUALIFIER OR NUMERIC
!       INPUT IN AN EITHER OR MODE. IT DOES NOT ACCEPT STRING
!       INPUT. THE VALID QUALIFIERS ARE "ALL" AND "OBJ" AND "OB"
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
   IWAVNM=INT(WVN)
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRPOL" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)&
      &'"PRPOL" ONLY TAKE QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.POLEXT) THEN
      WRITE(OUTLYNE,*)'NO POLARIZATION DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=NEWOBJ,SF
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
         WRITE(OUTLYNE,2000)I,RAYRAY(34,I),RAYRAY(35,I),&
         &RAYRAY(36,I),RAYRAY(37,I),RAYRAY(38,I)
         CALL SHOWIT(0)
         XA=RAYRAY(11,I)
         YA=RAYRAY(12,I)
         CALL RS(XA,YA)
10    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1500)SF,RAYRAY(34,SF),RAYRAY(35,SF),&
      &RAYRAY(36,SF),RAYRAY(37,SF),RAYRAY(38,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.NE.'OBJ'.AND.WQ.NE.' '.AND.&
   &WQ.NE.'ALL'.AND.WQ.NE.'OB'.AND.WQ.NE.'P') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1500)SF,RAYRAY(34,SF),RAYRAY(35,SF),&
      &RAYRAY(36,SF),RAYRAY(37,SF),RAYRAY(38,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      XA=RAYRAY(11,I)
      YA=RAYRAY(12,I)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,2000)I,RAYRAY(34,I),RAYRAY(35,I),&
      &RAYRAY(36,I),RAYRAY(37,I),RAYRAY(38,I)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE WITH FRACTIONAL OUTPUT HEADER
      SF=NEWIMG
      XA=RAYRAY(11,SF)
      YA=RAYRAY(12,SF)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)SF,RAYRAY(34,SF),RAYRAY(35,SF),&
      &RAYRAY(36,SF),RAYRAY(37,SF),RAYRAY(38,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'P'.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      XA=RAYRAY(11,I)
      YA=RAYRAY(12,I)
      CALL RS(XA,YA)
      WRITE(OUTLYNE,1600)IWAVNM,RELY,RELX
      CALL SHOWIT(0)
      IF(HEADIN) WRITE(OUTLYNE,5000)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)I,RAYRAY(34,I),RAYRAY(35,I),&
      &RAYRAY(36,I),RAYRAY(37,I),RAYRAY(38,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6)
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
5000 FORMAT('SURF',2X,'FACTOR-PAR.',4X,'FACTOR-PER.',5X,&
   &'PHASE_PAR.',5X,'PHASE_PER',6X,'POLANG(DEG.)')
5001 FORMAT('RAYTRACE POLARIZATION DATA, PHASE IN RADIANS',&
   &' - (CFG #',I2,')')
2501 FORMAT(1X)
END
! SUB PROPD.FOR
SUBROUTINE PROPD
!
   use DATLEN
   use DATMAI
   use mod_surface, only: surf_thickness
   use mod_system, only: sys_mode, sys_units, sys_wavelength
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PROPD WHICH IMPLEMENTS THE OPD
!       COMMAND AT THE CMD LEVEL.
!
   INTEGER J,JJ
!
   real(real64) LEN,LENW,WW,WAVE
!
!
!               CHECK FOR PRESENCE OF STRING,QUALIFIER,OR NUMERIC
!               INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE=&
      &'"OPD" TAKES NO EXPLICIT INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(RAYEXT.AND.REFEXT) THEN
      LEN=0.0D0
      RCOR=0.0D0
      OCOR=0.0D0
      IF(DABS(surf_thickness(NEWOBJ)).GE.1.0D10) JJ=NEWOBJ+2
      IF(DABS(surf_thickness(NEWOBJ)).LT.1.0D10) JJ=NEWOBJ+1
      DO J=JJ,NEWIMG
         LEN=LEN+RAYRAY(7,J)&
         &-(REFRY(7,J)*(ldm%getSurfIndex(J-1, INT(CURLAM))/ldm%getSurfIndex(J-1, INT(LFOB(4)))))
      END DO
      IF(sys_mode().EQ.1.0D0.OR.sys_mode().EQ.2.0D0) THEN
!       MODE FOCAL
         RCOR=0.0D0
         OCOR=0.0D0
         CALL FOPD
         LEN=LEN-(OCOR*ldm%getSurfIndex(NEWOBJ, INT(CURLAM)))&
         &+(RCOR*ldm%getSurfIndex(NEWOBJ, INT(CURLAM)))
         RCOR=0.0D0
         OCOR=0.0D0
         CENCEN=.FALSE.
         CALL LOPD
         LEN=LEN-(OCOR*ldm%getSurfIndex(NEWIMG-1, INT(CURLAM)))&
         &+(RCOR*ldm%getSurfIndex(NEWIMG-1, INT(CURLAM)))
      ELSE
!       MODE AFOCAL
         RCOR=0.0D0
         OCOR=0.0D0
         CALL FOPD
         LEN=LEN-(OCOR*ldm%getSurfIndex(NEWOBJ, INT(CURLAM)))&
         &+(RCOR*ldm%getSurfIndex(NEWOBJ, INT(CURLAM)))
         RCOR=0.0D0
         OCOR=0.0D0
         CENCEN=.FALSE.
         CALL LOPD
         LEN=LEN-(OCOR*ldm%getSurfIndex(NEWIMG-1, INT(CURLAM)))&
         &+(RCOR*ldm%getSurfIndex(NEWIMG-1, INT(CURLAM)))
      END IF
      WRITE(OUTLYNE,100) LEN
      CALL SHOWIT(0)
100   FORMAT('OPD FOR LAST RAY TRACED = ',G18.10,&
      &' LENS UNITS')
      WRITE(OUTLYNE,101)
      CALL SHOWIT(0)
101   FORMAT(1X)
      IF(INT(CURLAM).GE.1.AND.INT(CURLAM).LE.5) THEN
         WW=sys_wavelength(INT(CURLAM))
      END IF
      IF(INT(CURLAM).GE.6.AND.INT(CURLAM).LE.10) THEN
         WW=sys_wavelength(INT(CURLAM))
      END IF
      IF(sys_units().EQ.1.0) WAVE=(WW*1.0D-3)/(25.4D0)
      IF(sys_units().EQ.2.0) WAVE=WW*1.0D-4
      IF(sys_units().EQ.3.0) WAVE=WW*1.0D-3
      IF(sys_units().EQ.4.0) WAVE=WW*1.0D-6
      LEN=-LEN
      IF(REVSTR) LEN=-LEN
      LENW=LEN/WAVE
      IF(INT(CURLAM).GE.1.AND.INT(CURLAM).LE.5) THEN
         WRITE(OUTLYNE,102) LENW,sys_wavelength(INT(CURLAM))
         CALL SHOWIT(0)
      END IF
      IF(INT(CURLAM).GE.6.AND.INT(CURLAM).LE.10) THEN
         WRITE(OUTLYNE,102) LENW,sys_wavelength(INT(CURLAM))
         CALL SHOWIT(0)
      END IF
102   FORMAT('OPD FOR LAST RAY TRACED = ',G18.10,&
      &' IN WAVES AT ',G12.4,' MICRONS')
   ELSE
!       NO RAY EXITS OR NO REF RAY
      IF(.NOT.REFEXT) THEN
         OUTLYNE=&
         &'NO REFERENCE RAY DATA EXISTS, "OPD" CAN NOT BE CALCULATED'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(.NOT.RAYEXT) THEN
         OUTLYNE=&
         &'NO RAY DATA EXISTS, "OPD" CAN NOT BE CALCULATED'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   END IF
   RETURN
END
! SUB PRGLBL.FOR
SUBROUTINE PRGLBL
!
   use DATLEN
   use DATMAI
   use mod_surface, only: surf_thickness
   use mod_system, only: sys_last_surf, sys_units
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRGLBL. THIS SUBROUTINE IMPLEMENTS
!       THE CMD COMMAND "PRGLOBAL"
!
   CHARACTER UNIT*11
!
   INTEGER SF,I,JK
!
!
   IF(sys_units().EQ.1.0D0) UNIT='INCHES'
   IF(sys_units().EQ.2.0D0) UNIT='CENTIMETERS'
   IF(sys_units().EQ.3.0D0) UNIT='MILLIMETERS'
   IF(sys_units().EQ.4.0D0) UNIT='METERS'
   IF(.NOT.GLOBE) THEN
!       NO VERTEX DATA TO OUTPUT
      WRITE(OUTLYNE,*)&
      &'NO CURRENT GLOBAL RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
!       NO RAY DATA TO OUTPUT
      WRITE(OUTLYNE,*)&
      &'NO RAY DATA EXISTS TO OUTPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)WC(1:8),&
      &' ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      WRITE(OUTLYNE,*)WC(1:8),&
      &' TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(sys_last_surf().EQ.0.0D0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO GLOBAL RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
!       PRINT WARNING FOR OBJECT THICKNESS INFINITE
      IF(DABS(surf_thickness(NEWOBJ)).GT.1.0D10)THEN
         WRITE(OUTLYNE,*)'WARNING:'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'INFINITE OBJECT THICKNESS (MAGNITUDE GREATER THAN 1.0D+10'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'OBJECT THICKNESS WAS IGNORED IN ALL GLOBAL CALCULATIONS'
         CALL SHOWIT(1)
         JK=NEWOBJ+1
      ELSE
         JK=NEWOBJ
      END IF
      SF=NEWIMG
      WRITE(OUTLYNE,5001) INT(F12)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5002) GLSURF
      CALL SHOWIT(0)
      WRITE(OUTLYNE,102) OFFX,UNIT
      CALL SHOWIT(0)
      WRITE(OUTLYNE,103) OFFY,UNIT
      CALL SHOWIT(0)
      WRITE(OUTLYNE,104) OFFZ,UNIT
      CALL SHOWIT(0)
      WRITE(OUTLYNE,105) OFFA
      CALL SHOWIT(0)
      WRITE(OUTLYNE,106) OFFB
      CALL SHOWIT(0)
      WRITE(OUTLYNE,107) OFFC
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5000)
      CALL SHOWIT(0)
      DO 10 I=JK,SF
         WRITE(OUTLYNE,2000)I,GLRAY(1,I),GLRAY(2,I),GLRAY(3,I)
         CALL SHOWIT(0)
10    CONTINUE
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5003)
      CALL SHOWIT(0)
      DO 11 I=JK,SF
         WRITE(OUTLYNE,2000)I,GLRAY(4,I),GLRAY(5,I),GLRAY(6,I)
         CALL SHOWIT(0)
11    CONTINUE
      WRITE(OUTLYNE,2501)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,5004)
      CALL SHOWIT(0)
      DO 12 I=JK,SF
         WRITE(OUTLYNE,2000)I,GLRAY(7,I),GLRAY(8,I),GLRAY(9,I)
         CALL SHOWIT(0)
12    CONTINUE
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=NEWOBJ
      IF(DABS(surf_thickness(SF)).GT.1.0D10) THEN
         WRITE(OUTLYNE,*)&
         &'NO GLOBAL RAY DATA EXISTS FOR THE OBJECT SURFACE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'WHICH HAS INFINTE THICKNESS (GREATER THAN 1.0D10)'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) THEN
         WRITE(OUTLYNE,5001) INT(F12)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5002) GLSURF
         CALL SHOWIT(0)
         WRITE(OUTLYNE,102) OFFX,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,103) OFFY,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,104) OFFZ,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,105) OFFA
         CALL SHOWIT(0)
         WRITE(OUTLYNE,106) OFFB
         CALL SHOWIT(0)
         WRITE(OUTLYNE,107) OFFC
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5000)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(1,SF),GLRAY(2,SF),GLRAY(3,SF)
      CALL SHOWIT(0)
!
      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         WRITE(OUTLYNE,5003)
         CALL SHOWIT(0)

      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(4,SF),GLRAY(5,SF),GLRAY(6,SF)
      CALL SHOWIT(0)

      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5004)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(7,SF),GLRAY(8,SF),GLRAY(9,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'OBJ'.OR.SQ.EQ.1.AND.&
   &WQ.NE.'ALL'.OR.SQ.EQ.1.AND.WQ.NE.'OB') THEN
      WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
!       OUTPUT IMAGE SURFACE
      SF=NEWIMG
      IF(HEADIN) THEN
         WRITE(OUTLYNE,5001) INT(F12)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5002) GLSURF
         CALL SHOWIT(0)
         WRITE(OUTLYNE,102) OFFX,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,103) OFFY,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,104) OFFZ,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,105) OFFA
         CALL SHOWIT(0)
         WRITE(OUTLYNE,106) OFFB
         CALL SHOWIT(0)
         WRITE(OUTLYNE,107) OFFC
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5000)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(1,SF),GLRAY(2,SF),GLRAY(3,SF)
      CALL SHOWIT(0)
      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5003)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(4,SF),GLRAY(5,SF),GLRAY(6,SF)
      CALL SHOWIT(0)
      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5004)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)SF,GLRAY(7,SF),GLRAY(8,SF),GLRAY(9,SF)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SQ.EQ.0.AND.DF1.NE.1) THEN
      I=INT(W1)
      SF=NEWIMG
      IF(DABS(surf_thickness(I)).GT.1.0D10) THEN
         WRITE(OUTLYNE,*)&
         &'NO GLOBAL RAY DATA EXISTS FOR THE OBJECT SURFACE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'WHICH HAS INFINTE THICKNESS (GREATER THAN 1.0D10)'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(I.GT.SF.OR.I.LT.NEWOBJ) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(HEADIN) THEN
         WRITE(OUTLYNE,5001) INT(F12)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5002) GLSURF
         CALL SHOWIT(0)
         WRITE(OUTLYNE,102) OFFX,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,103) OFFY,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,104) OFFZ,UNIT
         CALL SHOWIT(0)
         WRITE(OUTLYNE,105) OFFA
         CALL SHOWIT(0)
         WRITE(OUTLYNE,106) OFFB
         CALL SHOWIT(0)
         WRITE(OUTLYNE,107) OFFC
         CALL SHOWIT(0)
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5000)
         CALL SHOWIT(0)

      END IF
      WRITE(OUTLYNE,1500)I,GLRAY(1,I),GLRAY(2,I),GLRAY(3,I)
      CALL SHOWIT(0)
      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5003)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)I,GLRAY(4,I),GLRAY(5,I),GLRAY(6,I)
      CALL SHOWIT(0)
      IF(HEADIN) THEN
         WRITE(OUTLYNE,2501)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,5004)
         CALL SHOWIT(0)
      END IF
      WRITE(OUTLYNE,1500)I,GLRAY(7,I),GLRAY(8,I),GLRAY(9,I)
      CALL SHOWIT(0)
      RETURN
   END IF
1500 FORMAT(I3,1X,G13.6,1X,G13.6,1X,G13.6)
2000 FORMAT(I3,1X,G13.6,1X,G13.6,1X,G13.6)
5000 FORMAT('SURF',7X,'X',13X,'Y',13X,'Z')
5003 FORMAT('SURF',7X,'LOLD',10X,'MOLD',10X,'NOLD')
5004 FORMAT('SURF',7X,'L',13X,'M',13X,'N')
5001 FORMAT('CURRENT GLOBAL RAY DATA ','&
   &  - (CFG #',I2,')')
5002 FORMAT(&
   &'GLOBAL RAY DATA REFERENCED TO SURFACE NUMBER ',I3)
2501 FORMAT(1X)
102 FORMAT('WITH X-OFFSET   = ',G18.10,1X,A11)
103 FORMAT('WITH Y-OFFSET   = ',G18.10,1X,A11)
104 FORMAT('WITH Z-OFFSET   = ',G18.10,1X,A11)
105 FORMAT('WITH ALPHA TILT = ',G18.10,' DEGREES')
106 FORMAT('WITH BETA  TILT = ',G18.10,' DEGREES')
107 FORMAT('WITH GAMMA TILT = ',G18.10,' DEGREES')
   RETURN
END
! SUB PRNSS.FOR
SUBROUTINE PRNSS
   USE GLOBALS
!
   use DATLEN
   use DATMAI
   use mod_system, only: sys_last_surf
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PRNSS. THIS SUBROUTINE IMPLEMENTS
!       THE PRNSS COMMAND WHICH OUTPUTS ANY EXISTING NSS DATA
!       IN THE MULTIRAY_DAT.
!
   INTEGER I
!
   LOGICAL DATAIS
!
!
!       THE VALID NUMERIC INPUT IS THE SURFACE NUMBER FOR
!       WHICH IT IS DESIRED TO PRODUCE OUTPUT. THE QUALIFIER "P"
!       CAUSES RELATIVE APERTURE COORDINATES TO BE PRINTED AS WELL.
!
!
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &'"PRNSS" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'INPUT IGNORED'
      CALL SHOWIT(1)
   END IF
!
   IF(sys_last_surf().EQ.0.0) THEN
      WRITE(OUTLYNE,*)'LENS SYSTEM HAS NO SURFACES'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO RAY DATA CAN EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(.NOT.RAYEXT) THEN
      WRITE(OUTLYNE,*)'NO RAY DATA EXISTS TO DISPLAY'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   DATAIS=.FALSE.
   DO I=0,INT(sys_last_surf())
      IF(NUMHITS(I).GT.1) THEN
         DATAIS=.TRUE.
         EXIT
      END IF
   END DO
1500 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
1600 FORMAT('RAY AT WAVL # ',I1,&
   &' (REL AP Y, REL AP X) = ( ',G13.6,', ',G13.6,' )')
2000 FORMAT(I3,2X,G13.6,2X,G13.6,2X,G13.6,2X,&
   &G13.6,2X,G13.6)
5000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'Z  ',&
   &11X,'XANG',11X,'YANG')
6000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IANG',&
   &10X,'XANG',11X,'YANG')
7000 FORMAT('SURF',6X,'X ',13X,'Y  ',12X,'IPANG',&
   &9X,'XANG',11X,'YANG')
5001 FORMAT('RAYTRACE DATA (XZ AND YZ-PLANE)',&
   &' - (CFG #',I2,')')
5003 FORMAT('ANGULAR OUTPUT MODE IS ',A8)
5002 FORMAT(&
   &'SLOPES/ANGLES  MEASURED WITH RESPECT TO THE Z-AXIS')
2501 FORMAT(1X)
END
