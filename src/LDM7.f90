!       SEVENTH FILE FOR LENS DATABASE MANAGER FILES

! SUB SCHG.FOR
SUBROUTINE SCHG
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SCHG WHICH IMPLEMENTS THE CHG
!       COMMANDS AT THE UPDATE LENS LEVEL.
!       THE FIRST NUMERIC WORD IS THE SURFACE TO WHICH WE WISH TO
!       CHANGE SURF TO. VALID VALUES ARE FROM 0 TO THE INTEGER VALUE
!       STORED IN sys_last_surf()
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
!
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CHG" ONLY TAKES NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CHG" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(F6.EQ.1) THEN
!
!               WE ARE AT LENS UPDATE LEVEL
!
      IF(W1.LT.0.0D0) W1=sys_last_surf()+W1
      IF(W1.LT.0.0D0.OR.W1.GT.sys_last_surf()) THEN
!
!       TRYING TO CHANGE A SURFACE NUMBER OUTSIDE THE VALID SURFACE
!       NUMBER RANGE FOR THIS LENS.
!       PRINT ERROR AND RETURN.
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER OUTSIDE VALID RANGE FOR THIS LENS'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
!       SURFACE NUMBER WITHIN VALID RANGE.
         SURF=INT(W1)
         RETURN
      END IF
   END IF
   RETURN
END
! SUB SCC.FOR
SUBROUTINE SCC
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SCC WHICH IMPLEMENTS THE CC
!        AND CCTOR COMMAND AT THE LENS OF UPDATE LENS LEVEL.
!
   INTEGER PIKCNT,I
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1) THEN
      IF(WC.EQ.'CCK')OUTLYNE='"CCK" TAKES NO STRING INPUT'
      IF(WC.EQ.'CCTOR')OUTLYNE='"CCTOR" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      IF(WC.EQ.'CCK')OUTLYNE='"CCK" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      IF(WC.EQ.'CCTOR')OUTLYNE='"CCTOR" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.F5.EQ.1) THEN
      IF(WC.EQ.'CCK')OUTLYNE='"CCK" TAKES NO QUALIFIER WORD IN LENS INPUT MODE'
      IF(WC.EQ.'CCTOR')OUTLYNE='"CCTOR" TAKES NO QUALIFIER WORD IN LENS INPUT MODE'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(F6.EQ.1) THEN
!
      IF(WQ.NE.'        '.AND.WQ.NE.'DELT'.AND.WQ.NE.'CENT') THEN
         IF(WC.EQ.'CCK')OUTLYNE='INVALID QUALIFIER WORD USED WITH "CCK"'
         IF(WC.EQ.'CCTOR')OUTLYNE='INVALID QUALIFIER WORD USED WITH "CCTOR"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(DF1.EQ.1)THEN
      IF(WC.EQ.'CCK')OUTLYNE='"CCK" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      IF(WC.EQ.'CCTOR')OUTLYNE='"CCTOR" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
   IF(WC.EQ.'CCK') THEN
      IF(SQ.EQ.0) call set_surf_conic(SURF, W1)
      IF(WQ.EQ.'DELT') call set_surf_conic(SURF, surf_conic(SURF)+W1)
      IF(WQ.EQ.'CENT')call set_surf_conic(SURF, surf_conic(SURF)+(W1*0.0D0*surf_conic(SURF)))

      ! For now, if K is set then make surr asphere
      if (surf_conic(SURF) .NE. 0) then
         call set_surf_asphere_flag(SURF, .true.)
      end if
!
!       CHECK FOR CC PIKUPS AND DELETE IF FOUND
!
!
!       DELETE THE PIKUP
      IF(PIKUP(1,SURF,4).GT.0.0D0) THEN
         PIKUP(1:6,SURF,4)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP (CCK) DELETED'
         CALL SHOWIT(1)
      END IF
      IF(PIKUP(1,SURF,11).GT.0.0D0) THEN
         PIKUP(1:6,SURF,11)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP (PRO) DELETED'
         CALL SHOWIT(1)
      END IF
      IF(PIKUP(1,SURF,12).GT.0.0D0) THEN
         PIKUP(1:6,SURF,12)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP (NPRO) DELETED'
         CALL SHOWIT(1)
      END IF
!
!       ARE THERE MORE PIKUPS? IF NOT SET surf_special_type(SURF) TO ZERO.
!
      PIKCNT=0
      DO 10 I=1,PSIZ
         IF(PIKUP(1,SURF,I).NE.0.0D0) THEN
            PIKCNT=PIKCNT+1
         ELSE
         END IF
10    CONTINUE
      IF(PIKCNT.EQ.0) call set_surf_special_type(SURF, 0)
      RETURN
   ELSE
   END IF
   IF(WC.EQ.'CCTOR') THEN
      IF(surf_toric_curvature(SURF).EQ.0.0D0)THEN
         OUTLYNE='WARNING:'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'FOR SURFACE ',SURF
         CALL SHOWIT(1)
         OUTLYNE='"CCTOR" WILL BE IGNORED FOR THIS CVTOR=0 SURFACE'
         CALL SHOWIT(1)
      END IF
      IF(SURF.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'OBJECT SURFACE MAY NOT BE A TORIC'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       CHECK FOR TORIC SURFACE
      IF(surf_toric_flag(SURF).EQ.0.0D0) THEN
         WRITE(OUTLYNE,*)'SURFACE',SURF,' NOT DEFINED AS ANAMORPHIC'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!               WE ARE AT LENS INPUT OR LENS UPDATE LEVEL
!
      IF(SQ.EQ.0) call set_surf_anamorphic_conic(SURF, W1)
      IF(WQ.EQ.'DELT') call set_surf_anamorphic_conic(SURF, surf_anamorphic_conic(SURF)+W1)
      IF(WQ.EQ.'CENT')call set_surf_anamorphic_conic(SURF, surf_anamorphic_conic(SURF)+(W1*0.0D0*surf_anamorphic_conic(SURF)))
!
!
!       DELETE THE PIKUP
      IF(PIKUP(1,SURF,4).GT.0.0D0) THEN
         PIKUP(1:6,SURF,21)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP(CC) DELETED'
         CALL SHOWIT(1)
      END IF
!       DUMP PIKUP PRO AND NPRO IF FOUND
      IF(PIKUP(1,SURF,11).GT.0.0D0) THEN
         PIKUP(1:6,SURF,11)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP (PRO) DELETED'
         CALL SHOWIT(1)
      END IF
      IF(PIKUP(1,SURF,12).GT.0.0D0) THEN
         PIKUP(1:6,SURF,12)=0.0D0
         call set_surf_special_type(SURF, surf_special_type(SURF)-1)
         WRITE(OUTLYNE,*)'SURFACE',SURF,' :PIKUP (NPRO) DELETED'
         CALL SHOWIT(1)
      END IF
!
!       ARE THERE MORE PIKUPS? IF NOT SET surf_special_type(SURF) TO ZERO.
!
      PIKCNT=0
      DO 101 I=1,PSIZ
         IF(PIKUP(1,SURF,I).NE.0.0D0) THEN
            PIKCNT=PIKCNT+1
         ELSE
         END IF
101   CONTINUE
!
      IF(PIKCNT.EQ.0) call set_surf_special_type(SURF, 0)
   ELSE
   END IF
   RETURN
END
! SUB SCASPC.FOR
SUBROUTINE SCASPC(SCW1)
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SCALE WHICH IMPLEMENTS ALL SC
!       AND WSC COMMAND FOR CFG1 SPECIAL SURFACE DATA.
!       SCASPC.FOR WHICH IS CALLED BY SCALEA.FOR
!
   real(real64) SCW1
!
   INTEGER I,J
!
!
   DO I=0,INT(sys_last_surf())
!     TYPE 1 AND 6
      IF(ABS(surf_asi_flag(I)).EQ.1.0D0.OR.ABS(surf_asi_flag(I)).EQ.6.0D0) THEN
         IF(FTFL01(9,I).NE.0.0D0) FTFL01(9,I)= FTFL01(9,I)*SCW1
         IF(FTFL01(10,I).NE.0.0D0) FTFL01(10,I)=FTFL01(10,I)
         DO J=11,48
            IF(FTFL01(J,I).NE.0.0D0) THEN
               FTFL01(J,I)=FTFL01(J,I)/(SCW1**(J-10))
            END IF
         END DO
      END IF
!     TYPE 23
      IF(ABS(surf_asi_flag(I)).EQ.23.0D0) THEN
         DO J=2,96
            FTFL01(J,I)=FTFL01(J,I)*(SCW1)
         END DO
      END IF
!     TYPE 4
      IF(ABS(surf_asi_flag(I)).EQ.4.0D0) THEN
         FTFL01(1,I)=FTFL01(1,I)*SCW1
         FTFL01(2,I)=FTFL01(2,I)*SCW1
         FTFL01(3,I)=FTFL01(3,I)*SCW1
         FTFL01(4,I)=FTFL01(4,I)*SCW1
         FTFL01(5,I)=FTFL01(5,I)*SCW1
         FTFL01(6,I)=FTFL01(6,I)*SCW1
         FTFL01(7,I)=FTFL01(7,I)*SCW1
         FTFL01(8,I)=FTFL01(8,I)*SCW1
         FTFL01(9,I)=FTFL01(9,I)*SCW1
         FTFL01(10,I)=FTFL01(10,I)*SCW1
         FTFL01(11,I)=FTFL01(11,I)*SCW1
         FTFL01(12,I)=FTFL01(12,I)*SCW1
         FTFL01(13,I)=FTFL01(13,I)*SCW1
         FTFL01(14,I)=FTFL01(14,I)*SCW1
         FTFL01(15,I)=FTFL01(15,I)*SCW1
      END IF
!     TYPE 7 AND 8
      IF(ABS(surf_asi_flag(I)).EQ.7.0D0.OR.ABS(surf_asi_flag(I)).EQ.8.0D0) THEN
         IF(FTFL01(1,I).NE.0.0D0) FTFL01(1,I)=FTFL01(1,1)*SCW1
         IF(FTFL01(2,I).NE.0.0D0) FTFL01(2,I)=FTFL01(2,I)
         IF(FTFL01(3,I).NE.0.0D0) FTFL01(3,I)=FTFL01(3,I)
         IF(FTFL01(4,I).NE.0.0D0) FTFL01(4,I)=FTFL01(4,I)/(SCW1)
         IF(FTFL01(5,I).NE.0.0D0) FTFL01(5,I)=FTFL01(5,I)/(SCW1)
         IF(FTFL01(6,I).NE.0.0D0) FTFL01(6,I)=FTFL01(6,I)/(SCW1)
         IF(FTFL01(7,I).NE.0.0D0) FTFL01(7,I)=FTFL01(7,I)/(SCW1**2)
         IF(FTFL01(8,I).NE.0.0D0) FTFL01(8,I)=FTFL01(8,I)/(SCW1**2)
         IF(FTFL01(9,I).NE.0.0D0) FTFL01(9,I)=FTFL01(9,I)/(SCW1**2)
         IF(FTFL01(10,I).NE.0.0D0) FTFL01(10,I)=FTFL01(10,I)/(SCW1**2)
         IF(FTFL01(11,I).NE.0.0D0) FTFL01(11,I)=FTFL01(11,I)/(SCW1**3)
         IF(FTFL01(12,I).NE.0.0D0) FTFL01(12,I)=FTFL01(12,I)/(SCW1**3)
         IF(FTFL01(13,I).NE.0.0D0) FTFL01(13,I)=FTFL01(13,I)/(SCW1**3)
         IF(FTFL01(14,I).NE.0.0D0) FTFL01(14,I)=FTFL01(14,I)/(SCW1**3)
         IF(FTFL01(15,I).NE.0.0D0) FTFL01(15,I)=FTFL01(15,I)/(SCW1**3)
         IF(FTFL01(16,I).NE.0.0D0) FTFL01(16,I)=FTFL01(16,I)/(SCW1**4)
         IF(FTFL01(17,I).NE.0.0D0) FTFL01(17,I)=FTFL01(17,I)/(SCW1**4)
         IF(FTFL01(18,I).NE.0.0D0) FTFL01(18,I)=FTFL01(18,I)/(SCW1**4)
         IF(FTFL01(19,I).NE.0.0D0) FTFL01(19,I)=FTFL01(19,I)/(SCW1**4)
         IF(FTFL01(20,I).NE.0.0D0) FTFL01(20,I)=FTFL01(20,I)/(SCW1**4)
         IF(FTFL01(21,I).NE.0.0D0) FTFL01(21,I)=FTFL01(21,I)/(SCW1**4)
         IF(FTFL01(22,I).NE.0.0D0) FTFL01(22,I)=FTFL01(22,I)/(SCW1**5)
         IF(FTFL01(23,I).NE.0.0D0) FTFL01(23,I)=FTFL01(23,I)/(SCW1**5)
         IF(FTFL01(24,I).NE.0.0D0) FTFL01(24,I)=FTFL01(24,I)/(SCW1**5)
         IF(FTFL01(25,I).NE.0.0D0) FTFL01(25,I)=FTFL01(25,I)/(SCW1**5)
         IF(FTFL01(26,I).NE.0.0D0) FTFL01(26,I)=FTFL01(26,I)/(SCW1**5)
         IF(FTFL01(27,I).NE.0.0D0) FTFL01(27,I)=FTFL01(27,I)/(SCW1**5)
         IF(FTFL01(28,I).NE.0.0D0) FTFL01(28,I)=FTFL01(28,I)/(SCW1**5)
         IF(FTFL01(29,I).NE.0.0D0) FTFL01(29,I)=FTFL01(29,I)/(SCW1**6)
         IF(FTFL01(30,I).NE.0.0D0) FTFL01(30,I)=FTFL01(30,I)/(SCW1**6)
         IF(FTFL01(31,I).NE.0.0D0) FTFL01(31,I)=FTFL01(31,I)/(SCW1**6)
         IF(FTFL01(32,I).NE.0.0D0) FTFL01(32,I)=FTFL01(32,I)/(SCW1**6)
         IF(FTFL01(33,I).NE.0.0D0) FTFL01(33,I)=FTFL01(33,I)/(SCW1**6)
         IF(FTFL01(34,I).NE.0.0D0) FTFL01(34,I)=FTFL01(34,I)/(SCW1**6)
         IF(FTFL01(35,I).NE.0.0D0) FTFL01(35,I)=FTFL01(35,I)/(SCW1**6)
         IF(FTFL01(36,I).NE.0.0D0) FTFL01(36,I)=FTFL01(36,I)/(SCW1**6)
         IF(FTFL01(37,I).NE.0.0D0) FTFL01(37,I)=FTFL01(37,I)/(SCW1**7)
         IF(FTFL01(38,I).NE.0.0D0) FTFL01(38,I)=FTFL01(38,I)/(SCW1**7)
         IF(FTFL01(39,I).NE.0.0D0) FTFL01(39,I)=FTFL01(39,I)/(SCW1**7)
         IF(FTFL01(40,I).NE.0.0D0) FTFL01(40,I)=FTFL01(40,I)/(SCW1**7)
         IF(FTFL01(41,I).NE.0.0D0) FTFL01(41,I)=FTFL01(41,I)/(SCW1**7)
         IF(FTFL01(42,I).NE.0.0D0) FTFL01(42,I)=FTFL01(42,I)/(SCW1**7)
         IF(FTFL01(43,I).NE.0.0D0) FTFL01(43,I)=FTFL01(43,I)/(SCW1**7)
         IF(FTFL01(44,I).NE.0.0D0) FTFL01(44,I)=FTFL01(44,I)/(SCW1**7)
         IF(FTFL01(45,I).NE.0.0D0) FTFL01(45,I)=FTFL01(45,I)/(SCW1**7)
         IF(FTFL01(46,I).NE.0.0D0) FTFL01(46,I)=FTFL01(46,I)/(SCW1**8)
         IF(FTFL01(47,I).NE.0.0D0) FTFL01(47,I)=FTFL01(47,I)/(SCW1**8)
         IF(FTFL01(48,I).NE.0.0D0) FTFL01(48,I)=FTFL01(48,I)/(SCW1**8)
         IF(FTFL01(49,I).NE.0.0D0) FTFL01(49,I)=FTFL01(49,I)/(SCW1**8)
         IF(FTFL01(50,I).NE.0.0D0) FTFL01(50,I)=FTFL01(50,I)/(SCW1**8)
         IF(FTFL01(51,I).NE.0.0D0) FTFL01(51,I)=FTFL01(51,I)/(SCW1**8)
         IF(FTFL01(52,I).NE.0.0D0) FTFL01(52,I)=FTFL01(52,I)/(SCW1**8)
         IF(FTFL01(53,I).NE.0.0D0) FTFL01(53,I)=FTFL01(53,I)/(SCW1**8)
         IF(FTFL01(54,I).NE.0.0D0) FTFL01(54,I)=FTFL01(54,I)/(SCW1**8)
         IF(FTFL01(55,I).NE.0.0D0) FTFL01(55,I)=FTFL01(55,I)/(SCW1**8)
         IF(FTFL01(56,I).NE.0.0D0) FTFL01(56,I)=FTFL01(56,I)/(SCW1**9)
         IF(FTFL01(57,I).NE.0.0D0) FTFL01(57,I)=FTFL01(57,I)/(SCW1**9)
         IF(FTFL01(58,I).NE.0.0D0) FTFL01(58,I)=FTFL01(58,I)/(SCW1**9)
         IF(FTFL01(59,I).NE.0.0D0) FTFL01(59,I)=FTFL01(59,I)/(SCW1**9)
         IF(FTFL01(60,I).NE.0.0D0) FTFL01(60,I)=FTFL01(60,I)/(SCW1**9)
         IF(FTFL01(61,I).NE.0.0D0) FTFL01(61,I)=FTFL01(61,I)/(SCW1**9)
         IF(FTFL01(62,I).NE.0.0D0) FTFL01(62,I)=FTFL01(62,I)/(SCW1**9)
         IF(FTFL01(63,I).NE.0.0D0) FTFL01(63,I)=FTFL01(63,I)/(SCW1**9)
         IF(FTFL01(64,I).NE.0.0D0) FTFL01(64,I)=FTFL01(64,I)/(SCW1**9)
         IF(FTFL01(65,I).NE.0.0D0) FTFL01(65,I)=FTFL01(65,I)/(SCW1**9)
         IF(FTFL01(66,I).NE.0.0D0) FTFL01(66,I)=FTFL01(66,I)/(SCW1**9)
         IF(FTFL01(67,I).NE.0.0D0) FTFL01(67,I)=FTFL01(67,I)/(SCW1**10)
         IF(FTFL01(68,I).NE.0.0D0) FTFL01(68,I)=FTFL01(68,I)/(SCW1**10)
         IF(FTFL01(69,I).NE.0.0D0) FTFL01(69,I)=FTFL01(69,I)/(SCW1**10)
         IF(FTFL01(70,I).NE.0.0D0) FTFL01(70,I)=FTFL01(70,I)/(SCW1**10)
         IF(FTFL01(71,I).NE.0.0D0) FTFL01(71,I)=FTFL01(71,I)/(SCW1**10)
         IF(FTFL01(72,I).NE.0.0D0) FTFL01(72,I)=FTFL01(72,I)/(SCW1**10)
         IF(FTFL01(73,I).NE.0.0D0) FTFL01(73,I)=FTFL01(73,I)/(SCW1**10)
         IF(FTFL01(74,I).NE.0.0D0) FTFL01(74,I)=FTFL01(74,I)/(SCW1**10)
         IF(FTFL01(75,I).NE.0.0D0) FTFL01(75,I)=FTFL01(75,I)/(SCW1**10)
         IF(FTFL01(76,I).NE.0.0D0) FTFL01(76,I)=FTFL01(76,I)/(SCW1**10)
         IF(FTFL01(77,I).NE.0.0D0) FTFL01(77,I)=FTFL01(77,I)/(SCW1**10)
         IF(FTFL01(78,I).NE.0.0D0) FTFL01(78,I)=FTFL01(78,I)/(SCW1**10)
         IF(FTFL01(79,I).NE.0.0D0) FTFL01(79,I)=FTFL01(79,I)/(SCW1**11)
         IF(FTFL01(80,I).NE.0.0D0) FTFL01(80,I)=FTFL01(80,I)/(SCW1**11)
         IF(FTFL01(81,I).NE.0.0D0) FTFL01(81,I)=FTFL01(81,I)/(SCW1**11)
         IF(FTFL01(82,I).NE.0.0D0) FTFL01(82,I)=FTFL01(82,I)/(SCW1**11)
         IF(FTFL01(83,I).NE.0.0D0) FTFL01(83,I)=FTFL01(83,I)/(SCW1**11)
         IF(FTFL01(84,I).NE.0.0D0) FTFL01(84,I)=FTFL01(84,I)/(SCW1**11)
         IF(FTFL01(85,I).NE.0.0D0) FTFL01(85,I)=FTFL01(85,I)/(SCW1**11)
         IF(FTFL01(86,I).NE.0.0D0) FTFL01(86,I)=FTFL01(86,I)/(SCW1**11)
         IF(FTFL01(87,I).NE.0.0D0) FTFL01(87,I)=FTFL01(87,I)/(SCW1**11)
         IF(FTFL01(88,I).NE.0.0D0) FTFL01(88,I)=FTFL01(88,I)/(SCW1**11)
         IF(FTFL01(89,I).NE.0.0D0) FTFL01(89,I)=FTFL01(89,I)/(SCW1**11)
         IF(FTFL01(90,I).NE.0.0D0) FTFL01(90,I)=FTFL01(90,I)/(SCW1**11)
         IF(FTFL01(91,I).NE.0.0D0) FTFL01(91,I)=FTFL01(91,I)/(SCW1**11)
      END IF
!     TYPE 12
      IF(ABS(surf_asi_flag(I)).EQ.12.0D0) THEN
         FTFL01(1,I)=FTFL01(1,I)*(SCW1/DABS(SCW1))
         IF(FTFL01(3,I).NE.0.0D0) FTFL01(3,I)=FTFL01(3,I)*SCW1
         IF(FTFL01(4,I).NE.0.0D0) FTFL01(4,I)=FTFL01(4,I)*SCW1
         IF(FTFL01(5,I).NE.0.0D0) FTFL01(5,I)=FTFL01(5,I)*SCW1
         IF(FTFL01(7,I).NE.0.0D0) FTFL01(7,I)=FTFL01(7,I)*SCW1
         IF(FTFL01(8,I).NE.0.0D0) FTFL01(8,I)=FTFL01(8,I)*SCW1
         IF(FTFL01(9,I).NE.0.0D0) FTFL01(9,I)=FTFL01(9,I)*SCW1
         IF(FTFL01(11,I).NE.0.0D0) FTFL01(11,I)=FTFL01(11,I)*SCW1
         IF(FTFL01(12,I).NE.0.0D0) FTFL01(12,I)=FTFL01(12,I)
         IF(FTFL01(13,I).NE.0.0D0) FTFL01(13,I)=FTFL01(13,I)/(SCW1)
         IF(FTFL01(14,I).NE.0.0D0) FTFL01(14,I)=FTFL01(14,I)/(SCW1**2)
         IF(FTFL01(15,I).NE.0.0D0) FTFL01(15,I)=FTFL01(15,I)/(SCW1**3)
         IF(FTFL01(16,I).NE.0.0D0) FTFL01(16,I)=FTFL01(16,I)/(SCW1**4)
         IF(FTFL01(17,I).NE.0.0D0) FTFL01(17,I)=FTFL01(17,I)/(SCW1**5)
         IF(FTFL01(18,I).NE.0.0D0) FTFL01(18,I)=FTFL01(18,I)/(SCW1**6)
         IF(FTFL01(19,I).NE.0.0D0) FTFL01(19,I)=FTFL01(19,I)/(SCW1**7)
         IF(FTFL01(20,I).NE.0.0D0) FTFL01(20,I)=FTFL01(20,I)/(SCW1**8)
         IF(FTFL01(21,I).NE.0.0D0) FTFL01(21,I)=FTFL01(21,I)/(SCW1**9)
         IF(FTFL01(22,I).NE.0.0D0) FTFL01(22,I)=FTFL01(22,I)/(SCW1**10)
         IF(FTFL01(23,I).NE.0.0D0) FTFL01(23,I)=FTFL01(23,I)/(SCW1**11)
      END IF
   END DO
!
   RETURN
END
! SUB AUTOFUNC.FOR
SUBROUTINE AUTOFUNC
!
   use DATLEN
   use DATMAI
   use mod_surface
   use command_utils, only: is_command_query
   use mod_system, only: sys_autofunc
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE AUTOFUNC WHICH IMPLEMENTS THE AUTOFUNC
!       COMMAND AT THE CMD LEVEL
!
   INTEGER I,CLCNT
!
!
!     CMD LEVEL
   IF(SST.EQ.0.AND.SN.EQ.0.AND.SQ.EQ.0.OR.is_command_query()) THEN
      IF(HEADIN) WRITE(OUTLYNE,401)
      IF(HEADIN) CALL SHOWIT(0)
      IF(sys_autofunc().NE.0.0D0) WRITE(OUTLYNE,100)INT(sys_autofunc())
      IF(sys_autofunc().EQ.0.0D0) WRITE(OUTLYNE,101)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "AUTOFUNC" TAKES NO INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
401 FORMAT(1X)
100 FORMAT('CURRENT AUTOFUNC FUNCTION NUMBER = ',I2)
101 FORMAT('NO AUTOFUNC FUNCTION IS CURRENTLY SPECIFIED')
   RETURN
END
! SUB TTHM.FOR
SUBROUTINE TTHM
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE TTHM WHICH IMPLEMENTS THE THM
!       COMMANDS
!
   INTEGER I,CLCNT
!
!
!     CMD LEVEL
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "THM" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "THM" TAKES EITHER QUALIFIER OR'//'\n'//&
      & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"THM" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO MIRROR THICKNESS DATA EXISTS'//'\n'//&
      & 'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE NORMAL
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE NORMAL DATA FOR
!       THE ENTIRE LENS IS PRINTED.
!
!       PRINT OUT FOR AN INDIVIDUAL SURFACE
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!       THIS IS THE SAME AS NORMAL,0
      SQ=0
      WQ='        '
      S1=1
      W1=0.0D0
      DF1=0
   ELSE
!       NOT "OB" OR "OBJ"
   END IF
!
   IF(SQ.EQ.0) THEN
      IF(W1.LT.0.0D0) W1=sys_last_surf()+W1
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      I=INT(W1)
      SURF=I
      IF(I.GT.INT(sys_last_surf()).OR.I.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(GLANAM(I,2).NE.'REFL         '.AND.GLANAM(I,2).NE.'REFLTIRO     '.AND.GLANAM(I,2).NE.'REFLTIR      ') THEN
         CALL REPORT_ERROR_AND_FAIL('NON-REFLECTIVE SURFACES HAVE NO THM VALUE ASSIGNED', 1)
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,500)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,100)I,DABS(surf_mirror_thickness(I))
      CALL SHOWIT(0)
      RETURN
   ELSE
!       THERE WAS A QUALIFIER.
      IF(WQ.NE.'ALL') THEN
         CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER WORD'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!       CHECK FOR NO DATA
      CLCNT=0
      DO SURF=0,INT(sys_last_surf())
         IF(surf_mirror_thickness(SURF).NE.0.0D0) THEN
            CLCNT=CLCNT+1
         ELSE
         END IF
      END DO
      IF(CLCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
110   FORMAT('NO MIRROR THICKNESS DATA')
!
!       PRINT HEADER MESSAGE
      WRITE(OUTLYNE,401)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,400)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,401)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,500)
      CALL SHOWIT(0)
      DO I=0,INT(sys_last_surf())

         IF(GLANAM(I,2).NE.'REFL         '.AND.GLANAM(I,2).NE.'REFLTIRO     '.AND.GLANAM(I,2).NE.'REFLTIR      ') THEN
            WRITE(OUTLYNE,101)I
            CALL SHOWIT(0)
            GO TO 200
         END IF
         IF(surf_mirror_thickness(I).NE.0.0D0) THEN
            WRITE(OUTLYNE,100)I,DABS(surf_mirror_thickness(I))
            CALL SHOWIT(0)
         END IF
200      CONTINUE
      END DO
!
400   FORMAT('MIRROR THICKNESS VALUES')
401   FORMAT(1X)
500   FORMAT('SURF',5X,'"MIRROR THICKNESS" VALUE (IN CURRENT LENS UNITS)')
100   FORMAT(I3,11X,G15.8)
101   FORMAT(I3,11X,'THM NOT VALID ON THIS NON-REFLECTIVE SURFACE')
   END IF
   RETURN
END
! SUB PPRICE.FOR
SUBROUTINE PPRICE
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PPRIC WHICH IMPLEMENTS THE PRICE
!       COMMANDS
!
   INTEGER I,CLCNT
!
!
!     CMD LEVEL
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "PRICE" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL, "PRICE" TAKES EITHER QUALIFIER OR'//'\n'//&
      & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'AT THE CMD LEVEL,'//'\n'//&
      & '"PRICE" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('NO "PRICE" DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE NORMAL
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE NORMAL DATA FOR
!       THE ENTIRE LENS IS PRINTED.
!
!       PRINT OUT FOR AN INDIVIDUAL SURFACE
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!       THIS IS THE SAME AS NORMAL,0
      SQ=0
      WQ='        '
      S1=1
      W1=0.0D0
      DF1=0
   ELSE
!       NOT "OB" OR "OBJ"
   END IF
!
   IF(SQ.EQ.0) THEN
      IF(W1.LT.0.0D0) W1=sys_last_surf()+W1
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      I=INT(W1)
      SURF=I
      IF(I.GT.INT(sys_last_surf()).OR.I.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(HEADIN) WRITE(OUTLYNE,500)
      IF(HEADIN) CALL SHOWIT(0)
      WRITE(OUTLYNE,100)I,DABS(surf_price(I))
      CALL SHOWIT(0)
      RETURN
   ELSE
!       THERE WAS A QUALIFIER.
      IF(WQ.NE.'ALL') THEN
         CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER WORD'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!       CHECK FOR NO DATA
      CLCNT=0
      DO SURF=0,INT(sys_last_surf())
         IF(surf_price(SURF).NE.0.0D0) THEN
            CLCNT=CLCNT+1
         ELSE
         END IF
      END DO
      IF(CLCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
110   FORMAT('NO PRICE DATA')
!
!       PRINT HEADER MESSAGE
      WRITE(OUTLYNE,401)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,400)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,401)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,500)
      CALL SHOWIT(0)
      DO I=0,INT(sys_last_surf())
         IF(surf_price(I).NE.0.0D0) THEN
            WRITE(OUTLYNE,100)I,DABS(surf_price(I))
            CALL SHOWIT(0)
         END IF
      END DO
!
400   FORMAT('PRICE UNIT PER Kg VALUES')
401   FORMAT(1X)
500   FORMAT('SURF',5X,'"PRICE" VALUE IN PRICE UNITS)')
100   FORMAT(I3,11X,F15.4)
   END IF
   RETURN
END
! SUB INRINR.FOR
SUBROUTINE INRINR
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE INRINR WHICH IMPLEMENTS THE INR
!       COMMANDS
!
   INTEGER I
!
!
   IF(F1.NE.1) THEN
!     NOT CMD LEVEL
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"INR" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"INR" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.F5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"INR" TAKES NO QUALIFIER INPUT AT LENS INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.F6.EQ.1.AND.WQ.NE.'CENT'.AND.SQ.EQ.1.AND.F6.EQ.1.AND.WQ.NE.'DELT') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER INPUT FOR "INR" AT LENS UPDATE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S1.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"INR" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(F6.EQ.1.AND.WQ.EQ.'DELT') THEN
         call set_surf_inr_value(SURF, surf_inr_value(SURF)+(W1))
         call set_surf_inr_flag(SURF, 1)
      END IF
      IF(F6.EQ.1.AND.WQ.EQ.'CENT') THEN
         call set_surf_inr_value(SURF, surf_inr_value(SURF)+(W1*0.01D0*surf_inr_value(SURF)))
         call set_surf_inr_flag(SURF, 1)
      END IF
      IF(F5.EQ.1.OR.F6.EQ.1.AND.SQ.EQ.0) THEN
         call set_surf_inr_value(SURF, W1)
         call set_surf_inr_flag(SURF, 1)
      END IF
!
   ELSE
!     CMD LEVEL
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "INR" TAKES NO STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "INR" TAKES EITHER QUALIFIER OR'//'\n'//&
         & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL,'//'\n'//&
         & '"INR" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       WHAT IF NO SURFACES EXIST
      IF(sys_last_surf().EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('NO "INR" DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
         RETURN
      END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE NORMAL
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE NORMAL DATA FOR
!       THE ENTIRE LENS IS PRINTED.
!
!       PRINT OUT FOR AN INDIVIDUAL SURFACE
!
      IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!       THIS IS THE SAME AS NORMAL,0
         SQ=0
         WQ='        '
         S1=1
         W1=0.0D0
         DF1=0
      ELSE
!       NOT "OB" OR "OBJ"
      END IF
!
      IF(SQ.EQ.0) THEN
         IF(W1.LT.0.0D0) W1=sys_last_surf()+W1
         IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
         I=INT(W1)
         SURF=I
         IF(I.GT.INT(sys_last_surf()).OR.I.LT.0) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(HEADIN) WRITE(OUTLYNE,500)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,100)I,surf_inr_value(I)
         CALL SHOWIT(0)
         RETURN
      ELSE
!       THERE WAS A QUALIFIER.
         IF(WQ.NE.'ALL') THEN
            CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER WORD'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
!
!       CHECK FOR NO DATA
!
!       PRINT HEADER MESSAGE
         WRITE(OUTLYNE,401)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,400)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,402)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,401)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,500)
         CALL SHOWIT(0)
         DO I=0,INT(sys_last_surf())
            WRITE(OUTLYNE,100)I,surf_inr_value(I)
            CALL SHOWIT(0)
         END DO
!
400      FORMAT('SURFACE "INR" VALUES')
402      FORMAT ('"INR" IS THE REFERENCE RADIUS FOR "FRINGE" CALCULATIONS')
401      FORMAT(1X)
500      FORMAT('SURF',5X,'"INR" VALUE (IN CURRENT LENS UNITS)')
100      FORMAT(I3,11X,G15.8)
      END IF
   END IF
   RETURN
END
! SUB INRINRD.FOR
SUBROUTINE INRINRD
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE INRINRD WHICH IMPLEMENTS THE INRD
!
   INTEGER SF
!
!
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               OR NUMERIC INPUT.
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"INRD" TAKES NO STRING OR QUALIFIER'//'\n'//&
      & 'OR NUMERIC WORD #3 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1.AND.DF2.EQ.1) THEN
      W1=DBLE(SURF)
      W2=DBLE(SURF)
      S1=1
      S2=1
      DF1=0
      DF2=0
   END IF
   IF(DF1.EQ.1.AND.DF2.EQ.0.OR.DF1.EQ.0.AND.DF2.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"INRD" USES EITHER TWO OR ZERO NUMERIC WORDS'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(INT(W1).LT.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'STARTING SURFACE NUMBER MUST BE GREATER THAN OR EQUAL TO 0'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(INT(W2).GT.INT(sys_last_surf())) THEN
      WRITE(OUTLYNE,*)'ENDING SURFACE NUMBER MUST BE LESS THAN OR EQUAL TO ',INT(sys_last_surf())
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.GT.W2) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'THE ENDING SURFACE # MUST BE GREATER THAN OR EQUAL TO#'//'\n'//&
      & 'THE STARTING SURFACE #'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   DO SF=INT(W1),INT(W2)
      IF(surf_inr_flag(SF).EQ.1.0D0) THEN
!     EXPLICIT ASSIGNMENT EXISTS
         call set_surf_inr_value(SF, 0.0D0)
         call set_surf_inr_flag(SF, 0)
         WRITE(OUTLYNE,*)'EXPLICIT "INR" ASSIGNMENT FOR SURFACE',SF,' DELETED'
         CALL SHOWIT(1)
      ELSE
         WRITE(OUTLYNE,*)'EXPLICIT "INR" ASSIGNMENT NOT DEFINED FOR SURFACE',SF
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
   END DO
   RETURN
END
! SUB SPGR.FOR
SUBROUTINE SPGR
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SPGR WHICH IMPLEMENTS THE SPGR
!       COMMANDS
!
   INTEGER I
!
!
   IF(F1.NE.1) THEN
!     NOT CMD LEVEL
!               CHECK FOR PRESENCE OF QUALIFIER OR STRING INPUT
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPGR" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"SPGR" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.F5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPGR" TAKES NO QUALIFIER INPUT AT LENS INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.F6.EQ.1.AND.WQ.NE.'CENT'.AND.SQ.EQ.1.AND.F6.EQ.1.AND.WQ.NE.'DELT') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER INPUT FOR "SPGR" AT LENS UPDATE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S1.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPCR" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(F6.EQ.1.AND.WQ.EQ.'DELT') THEN
         call set_surf_spgr(SURF, DABS(surf_spgr(SURF)+(W1)))
      END IF
      IF(F6.EQ.1.AND.WQ.EQ.'CENT') THEN
         call set_surf_spgr(SURF, DABS(surf_spgr(SURF)+(W1*0.01D0*surf_spgr(SURF))))
      END IF
      IF(F5.EQ.1.OR.F6.EQ.1.AND.SQ.EQ.0) THEN
         call set_surf_spgr(SURF, DABS(W1))
      END IF
!
   ELSE
!     CMD LEVEL
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "SPGR" TAKES NO STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL, "SPGR" TAKES EITHER QUALIFIER OR'//'\n'//&
         & 'NUMERIC WORD #1 INPUT BUT NOT BOTH'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'AT THE CMD LEVEL,'//'\n'//&
         & '"SPGR" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       WHAT IF NO SURFACES EXIST
      IF(sys_last_surf().EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('NO "SPGR" DATA EXISTS'//'\n'//'LENS SYSTEM HAS NO SURFACES', 1)
         RETURN
      END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE NORMAL
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE NORMAL DATA FOR
!       THE ENTIRE LENS IS PRINTED.
!
!       PRINT OUT FOR AN INDIVIDUAL SURFACE
!
      IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!       THIS IS THE SAME AS NORMAL,0
         SQ=0
         WQ='        '
         S1=1
         W1=0.0D0
         DF1=0
      ELSE
!       NOT "OB" OR "OBJ"
      END IF
!
      IF(SQ.EQ.0) THEN
         IF(W1.LT.0.0D0) W1=sys_last_surf()+W1
         IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
         I=INT(W1)
         SURF=I
         IF(I.GT.INT(sys_last_surf()).OR.I.LT.0) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(HEADIN) WRITE(OUTLYNE,500)
         IF(HEADIN) CALL SHOWIT(0)
         WRITE(OUTLYNE,100)I,DABS(surf_spgr(I))
         CALL SHOWIT(0)
         RETURN
      ELSE
!       THERE WAS A QUALIFIER.
         IF(WQ.NE.'ALL') THEN
            CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER WORD'//'\n'//'RE-ENTER COMMAND', 1)
            RETURN
         END IF
!
!       CHECK FOR NO DATA
!
!       PRINT HEADER MESSAGE
         WRITE(OUTLYNE,401)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,400)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,401)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,500)
         CALL SHOWIT(0)
         DO I=0,INT(sys_last_surf())
            WRITE(OUTLYNE,100)I,DABS(surf_spgr(I))
            CALL SHOWIT(0)
         END DO
!
400      FORMAT('SURFACE "SPECIFIC GRAVITY (gm/cc)" VALUES')
401      FORMAT(1X)
500      FORMAT('SURF',5X,'"SPGR" VALUE (IN CURRENT LENS UNITS)')
100      FORMAT(I3,11X,G15.8)
      END IF
   END IF
   RETURN
END
! SUB SCAOB.FOR
SUBROUTINE SCAOB
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SCAOB WHICH IMPLEMENTS THE CAOB
!       COMMAND AT THE CMD LEVEL
!       THE ONLY VALID QUALIFIERS ARE (BLANK) AND "ALL","OB" AND "OBJ"
!       THE NUMERIC ENTRY W1, IS THE SURFACE DESIGNATOR
!       FOR PRINTOUT OF ONE SURFACE'S DATA ONLY
!
   CHARACTER CLTYPE*8,COTYPE*8,CLETYPE*8,COETYPE*8 ,ASURF*3,SPCE*1,LINE*80,SP10*10,AAL*10
!
   INTEGER SF,I,J,K ,CLAP,COBS,CLCNT,CLAPE,COBSE
!
   LOGICAL NOHEAD
!
!
!               THIS IS A CMD LEVEL COMMAND ONLY
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   SP10='          '
   SPCE=' '
   CLAP=0
   CLAPE=0
   COBS=0
   COBSE=0
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"CAOB" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      S1=0
      DF1=0
      W1=0.0D0
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CAOB" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'//'\n'//&
      & 'BUT NOT BOTH'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       WHAT IF NO SURFACES EXIST
   IF(sys_last_surf().EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO CLEAR APERTURES OR OBSCURATIONS EXIST'//'\n'//&
      & 'LENS SYSTEM HAS NO SURFACES', 1)
      RETURN
   END IF
!
!       W1 DESIGNATES THE SURFACE FOR WHICH THE CAOB
!       DATA IS TO BE OUTPUT.
!       IF THE QUALIFIER "ALL" IS USED, THEN THE CLEAR APERTURE
!       DATA FOR
!       THE ENTIRE LENS IS PRINTED. IF THE QUALIFIER IS "OB" OR "OBJ"
!       THEN DATA FOR THE OBJECT SURFACE IS PRINTED.
!
   IF(WQ.EQ.'OB'.OR.WQ.EQ.'OBJ') THEN
!
!       THIS IS SAME AS CAOB,0
      S1=1
      DF1=0
      SQ=0
      WQ='        '
      W1=0.0
   ELSE
!       WQ NOT OB OR OBJ
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'ALL') THEN
!
!       WE HAVE INVALID QUALIFIER
      CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
!       HANDEL AN INDIVIDUAL SURFACE INCLUDING "OB" AND "OBJ"
      IF(DF1.EQ.1) W1=DBLE(INT(sys_last_surf()))
      SURF=INT(W1)
      IF(SURF.GT.(INT(sys_last_surf())).OR.SURF.LT.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'SURFACE NUMBER BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF

      COBS=0
      COBSE=0
      CLAP=0
      CLAPE=0
      IF(surf_clap_type(SURF).NE.0.0D0) CLAP=1
      IF(surf_cobs_ape_type(SURF).NE.0.0D0) CLAPE=1
      IF(surf_clap_type(SURF).EQ.1.0) CLTYPE ='    CIRC'
      IF(surf_clap_type(SURF).EQ.2.0) CLTYPE ='    RECT'
      IF(surf_clap_type(SURF).EQ.3.0) CLTYPE ='    ELIP'
      IF(surf_clap_type(SURF).EQ.4.0) CLTYPE ='    RCTK'
      IF(surf_clap_type(SURF).EQ.5.0) CLTYPE ='    POLY'
      IF(surf_clap_type(SURF).EQ.6.0) CLTYPE ='   IPOLY'
      IF(surf_cobs_ape_type(SURF).EQ.1.0) CLETYPE='   ERASE'
      IF(surf_cobs_ape_type(SURF).EQ.2.0) CLETYPE='   RECTE'
      IF(surf_cobs_ape_type(SURF).EQ.3.0) CLETYPE='   ELIPE'
      IF(surf_cobs_ape_type(SURF).EQ.4.0) CLETYPE='   RCTKE'
      IF(surf_cobs_ape_type(SURF).EQ.5.0) CLETYPE='   POLYE'
      IF(surf_cobs_ape_type(SURF).EQ.6.0) CLETYPE='  IPOLYE'
      IF(surf_coat_type(SURF).NE.0.0D0) COBS=1
      IF(surf_cobs_era_type(SURF).NE.0.0D0) COBSE=1
      IF(surf_coat_type(SURF).EQ.1.0) COTYPE= 'OB  CIRC'
      IF(surf_coat_type(SURF).EQ.2.0) COTYPE= 'OB  RECT'
      IF(surf_coat_type(SURF).EQ.3.0) COTYPE= 'OB  ELIP'
      IF(surf_coat_type(SURF).EQ.4.0) COTYPE= 'OB  RCTK'
      IF(surf_coat_type(SURF).EQ.5.0) COTYPE= 'OB  POLY'
      IF(surf_coat_type(SURF).EQ.6.0) COTYPE= 'OB IPOLY'
      IF(surf_cobs_era_type(SURF).EQ.1.0) COETYPE='OB ERASE'
      IF(surf_cobs_era_type(SURF).EQ.2.0) COETYPE='OB RECTE'
      IF(surf_cobs_era_type(SURF).EQ.3.0) COETYPE='OB ELIPE'
      IF(surf_cobs_era_type(SURF).EQ.4.0) COETYPE='OB RCTKE'
      IF(surf_cobs_era_type(SURF).EQ.5.0) COETYPE='OB POLYE'
      IF(surf_cobs_era_type(SURF).EQ.6.0) COETYPE='OBIPOLYE'
!
!       FIRST PRINT CLAP DATA THEN COBS DATA
!               HANDEL NO DATA FOR A SURFACE
      IF(CLAP.EQ.0.AND.COBS.EQ.0) THEN

         CALL NTOAN1(SURF,ASURF)
         WRITE(OUTLYNE,100) ASURF(1:3)
         CALL SHOWIT(0)
         RETURN
      END IF
!
!                       DO PRINTING NOW
!*****************************************************************
!
!                       HANDEL CLAP DATA
!
      IF(CLAP.EQ.1) THEN
         CALL NTOAN1(SURF,ASURF)
         LINE=ASURF(1:3)//SPCE(1:1)//CLTYPE(1:8)//SPCE(1:1)
         CALL NTOAN2(surf_clap_dim(SURF, 1),AAL(1:10))
         LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
         IF(surf_clap_dim(SURF, 2).EQ.0.0D0) THEN
            LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_clap_dim(SURF, 2),AAL(1:10))
            LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_clap_dim(SURF, 3).EQ.0.0D0) THEN
            LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_clap_dim(SURF, 3),AAL(1:10))
            LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_clap_dim(SURF, 4).EQ.0.0D0) THEN
            LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_clap_dim(SURF, 4),AAL(1:10))
            LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_clap_dim(SURF, 5).EQ.0.0D0) THEN
            LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_clap_dim(SURF, 5),AAL(1:10))
            LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_clap_tilt(SURF).EQ.0.0D0) THEN
            LINE=LINE(1:68)//SP10(1:10)
         ELSE
            CALL NTOAN2(surf_clap_tilt(SURF),AAL(1:10))
            LINE=LINE(1:68)//AAL(1:10)
         END IF

         IF(HEADIN) THEN
            WRITE(OUTLYNE,2000)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2001)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000)
            CALL SHOWIT(0)
         END IF

         WRITE(OUTLYNE,200) LINE(1:79)
         CALL SHOWIT(0)
      END IF
      IF(CLAPE.EQ.1) THEN
         CALL NTOAN1(SURF,ASURF)
         LINE=ASURF(1:3)//SPCE(1:1)//CLETYPE(1:8)//SPCE(1:1)
         CALL NTOAN2(surf_cobs_ape_data(SURF, 1),AAL(1:10))
         LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
         IF(surf_cobs_ape_data(SURF, 2).EQ.0.0D0) THEN
            LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_ape_data(SURF, 2),AAL(1:10))
            LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_ape_data(SURF, 3).EQ.0.0D0) THEN
            LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_ape_data(SURF, 3),AAL(1:10))
            LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_ape_data(SURF, 4).EQ.0.0D0) THEN
            LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_ape_data(SURF, 4),AAL(1:10))
            LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_ape_data(SURF, 5).EQ.0.0D0) THEN
            LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_ape_data(SURF, 5),AAL(1:10))
            LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_ape_data(SURF, 6).EQ.0.0D0) THEN
            LINE=LINE(1:68)//SP10(1:10)
         ELSE
            CALL NTOAN2(surf_cobs_ape_data(SURF, 6),AAL(1:10))
            LINE=LINE(1:68)//AAL(1:10)
         END IF
         IF(HEADIN) THEN
            WRITE(OUTLYNE,2000)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2001)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000)
            CALL SHOWIT(0)
         END IF
         WRITE(OUTLYNE,200) LINE(1:79)
         CALL SHOWIT(0)
      END IF
!       HANDEL COBS DATA
      IF(COBS.EQ.1) THEN
         CALL NTOAN1(SURF,ASURF)
         LINE=ASURF(1:3)//SPCE(1:1)//COTYPE(1:8)//SPCE(1:1)
         CALL NTOAN2(surf_cobs_poly(SURF, 1),AAL)
         LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
!
         IF(surf_cobs_poly(SURF, 2).EQ.0.0D0) THEN
            LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_poly(SURF, 2),AAL(1:10))
            LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
         END IF
!
         IF(surf_cobs_poly(SURF, 3).EQ.0.0D0) THEN
            LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_poly(SURF, 3),AAL(1:10))
            LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_poly(SURF, 4).EQ.0.0D0) THEN
            LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_poly(SURF, 4),AAL(1:10))
            LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_poly(SURF, 5).EQ.0.0D0) THEN
            LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_poly(SURF, 5),AAL(1:10))
            LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_poly(SURF, 6).EQ.0.0D0) THEN
            LINE=LINE(1:68)//SP10(1:10)
         ELSE
            CALL NTOAN2(surf_cobs_poly(SURF, 6),AAL(1:10))
            LINE=LINE(1:68)//AAL(1:10)
         END IF
         IF(HEADIN) THEN
            WRITE(OUTLYNE,2000)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2001)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000)
            CALL SHOWIT(0)
         END IF
         WRITE(OUTLYNE,300) LINE(1:79)
         CALL SHOWIT(0)
      END IF
      IF(COBSE.EQ.1) THEN
         CALL NTOAN1(SURF,ASURF)
         LINE=ASURF(1:3)//SPCE(1:1)//COETYPE(1:8)//SPCE(1:1)
         CALL NTOAN2(surf_cobs_era_data(SURF, 1),AAL)
         LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
!
         IF(surf_cobs_era_data(SURF, 2).EQ.0.0D0) THEN
            LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_era_data(SURF, 2),AAL(1:10))
            LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
         END IF
!
         IF(surf_cobs_era_data(SURF, 3).EQ.0.0D0) THEN
            LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_era_data(SURF, 3),AAL(1:10))
            LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_era_data(SURF, 4).EQ.0.0D0) THEN
            LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_era_data(SURF, 4),AAL(1:10))
            LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_era_data(SURF, 5).EQ.0.0D0) THEN
            LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
         ELSE
            CALL NTOAN2(surf_cobs_era_data(SURF, 5),AAL(1:10))
            LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
         END IF
         IF(surf_cobs_era_data(SURF, 6).EQ.0.0D0) THEN
            LINE=LINE(1:68)//SP10(1:10)
         ELSE
            CALL NTOAN2(surf_cobs_era_data(SURF, 6),AAL(1:10))
            LINE=LINE(1:68)//AAL(1:10)
         END IF
         IF(HEADIN) THEN
            WRITE(OUTLYNE,2000)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,2001)
            CALL SHOWIT(0)
            WRITE(OUTLYNE,3000)
            CALL SHOWIT(0)
         END IF
         WRITE(OUTLYNE,300) LINE(1:79)
         CALL SHOWIT(0)
      END IF
2008  FORMAT(I3,4X,I4,5X,G13.6,1X,G13.6,1X,G13.6)
      IF(surf_multi_clap_flag(SURF).NE.0.0D0) THEN
         DO I=1,INT(surf_multi_clap_flag(SURF))
            IF(HEADIN) THEN
               WRITE(OUTLYNE,2009)
               CALL SHOWIT(0)
               WRITE(OUTLYNE,2010)
               CALL SHOWIT(0)
            END IF
            WRITE(OUTLYNE,2008)SURF,I,MULTCLAP(I,1,SURF),MULTCLAP(I,2,SURF),MULTCLAP(I,3,SURF)
            CALL SHOWIT(1)
         END DO
      END IF
      IF(surf_multi_cobs_flag(SURF).NE.0.0D0) THEN
         DO I=1,INT(surf_multi_cobs_flag(SURF))
            IF(HEADIN) THEN
               WRITE(OUTLYNE,3009)
               CALL SHOWIT(0)
               WRITE(OUTLYNE,2010)
               CALL SHOWIT(0)
            END IF
            WRITE(OUTLYNE,2008)SURF,I,MULTCOBS(I,1,SURF),MULTCOBS(I,2,SURF),MULTCOBS(I,3,SURF)
            CALL SHOWIT(1)
         END DO
      END IF
      RETURN
   ELSE
!*********************************************************************
!       SQ=1 AND ONLY ALLOWED WQ BY NOW IS "ALL"
!       HANDEL THE WHOLE LENS
      CLCNT=0
!       PRINT HEADING DATA
!
      DO 15 SURF=0,INT(sys_last_surf())
         IF(surf_clap_type(SURF).NE.0.0.AND.surf_coat_type(SURF).NE.0.0 .OR.surf_clap_type(SURF).NE.0.0.OR.surf_coat_type(SURF).NE.0.0D0) THEN
            CLCNT=CLCNT+1
         ELSE
         END IF
15    CONTINUE
      IF(CLCNT.EQ.0) THEN
         WRITE(OUTLYNE,110)
         CALL SHOWIT(0)
         RETURN
      END IF
!       PROCEED WITH PROCESSING
      WRITE(OUTLYNE,1000)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,1500)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2000)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2001)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,3000)
      CALL SHOWIT(0)
      DO 5 SURF=0,INT(sys_last_surf())
         CLAP=0
         CLAPE=0
         COBS=0
         COBSE=0
         IF(surf_clap_type(SURF).NE.0.0D0) CLAP=1
         IF(surf_cobs_ape_type(SURF).NE.0.0D0) CLAPE=1
         IF(surf_clap_type(SURF).EQ.1.0) CLTYPE ='    CIRC'
         IF(surf_clap_type(SURF).EQ.2.0) CLTYPE ='    RECT'
         IF(surf_clap_type(SURF).EQ.3.0) CLTYPE ='    ELIP'
         IF(surf_clap_type(SURF).EQ.4.0) CLTYPE ='    RCTK'
         IF(surf_clap_type(SURF).EQ.5.0) CLTYPE ='    POLY'
         IF(surf_clap_type(SURF).EQ.6.0) CLTYPE ='   IPOLY'
         IF(surf_cobs_ape_type(SURF).EQ.1.0) CLETYPE='   ERASE'
         IF(surf_cobs_ape_type(SURF).EQ.2.0) CLETYPE='   RECTE'
         IF(surf_cobs_ape_type(SURF).EQ.3.0) CLETYPE='   ELIPE'
         IF(surf_cobs_ape_type(SURF).EQ.4.0) CLETYPE='   RCTKE'
         IF(surf_cobs_ape_type(SURF).EQ.5.0) CLETYPE='   POLYE'
         IF(surf_cobs_ape_type(SURF).EQ.6.0) CLETYPE='  IPOLYE'
         IF(surf_coat_type(SURF).NE.0.0D0) COBS=1
         IF(surf_cobs_era_type(SURF).NE.0.0D0) COBSE=1
         IF(surf_coat_type(SURF).EQ.1.0) COTYPE= 'OB  CIRC'
         IF(surf_coat_type(SURF).EQ.2.0) COTYPE= 'OB  RECT'
         IF(surf_coat_type(SURF).EQ.3.0) COTYPE= 'OB  ELIP'
         IF(surf_coat_type(SURF).EQ.4.0) COTYPE= 'OB  RCTK'
         IF(surf_coat_type(SURF).EQ.5.0) COTYPE= 'OB  POLY'
         IF(surf_coat_type(SURF).EQ.6.0) COTYPE= 'OB IPOLY'
         IF(surf_cobs_era_type(SURF).EQ.1.0) COETYPE='OB ERASE'
         IF(surf_cobs_era_type(SURF).EQ.2.0) COETYPE='OB RECTE'
         IF(surf_cobs_era_type(SURF).EQ.3.0) COETYPE='OB ELIPE'
         IF(surf_cobs_era_type(SURF).EQ.4.0) COETYPE='OB RCTKE'
         IF(surf_cobs_era_type(SURF).EQ.5.0) COETYPE='OB POLYE'
         IF(surf_cobs_era_type(SURF).EQ.6.0) COETYPE='OBIPOLYE'
!
!                       DO PRINTING NOW
!*****************************************************************
!
!                       HANDEL CLAP DATA
!
         IF(CLAP.EQ.1) THEN
            CALL NTOAN1(SURF,ASURF)
            LINE=ASURF(1:3)//SPCE(1:1)//CLTYPE(1:8)//SPCE(1:1)
            CALL NTOAN2(surf_clap_dim(SURF, 1),AAL(1:10))
            LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
            IF(surf_clap_dim(SURF, 2).EQ.0.0D0) THEN
               LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_clap_dim(SURF, 2),AAL(1:10))
               LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_clap_dim(SURF, 3).EQ.0.0D0) THEN
               LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_clap_dim(SURF, 3),AAL(1:10))
               LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_clap_dim(SURF, 4).EQ.0.0D0) THEN
               LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_clap_dim(SURF, 4),AAL(1:10))
               LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_clap_dim(SURF, 5).EQ.0.0D0) THEN
               LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_clap_dim(SURF, 5),AAL(1:10))
               LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_clap_tilt(SURF).EQ.0.0D0) THEN
               LINE=LINE(1:68)//SP10(1:10)
            ELSE
               CALL NTOAN2(surf_clap_tilt(SURF),AAL(1:10))
               LINE=LINE(1:68)//AAL(1:10)
            END IF
            WRITE(OUTLYNE,200) LINE(1:79)
            CALL SHOWIT(0)
         END IF
         IF(CLAPE.EQ.1) THEN
            CALL NTOAN1(SURF,ASURF)
            LINE=ASURF(1:3)//SPCE(1:1)//CLETYPE(1:8)//SPCE(1:1)
            CALL NTOAN2(surf_cobs_ape_data(SURF, 1),AAL(1:10))
            LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
            IF(surf_cobs_ape_data(SURF, 2).EQ.0.0D0) THEN
               LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_ape_data(SURF, 2),AAL(1:10))
               LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_ape_data(SURF, 3).EQ.0.0D0) THEN
               LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_ape_data(SURF, 3),AAL(1:10))
               LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_ape_data(SURF, 4).EQ.0.0D0) THEN
               LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_ape_data(SURF, 4),AAL(1:10))
               LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_ape_data(SURF, 5).EQ.0.0D0) THEN
               LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_ape_data(SURF, 5),AAL(1:10))
               LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_ape_data(SURF, 6).EQ.0.0D0) THEN
               LINE=LINE(1:68)//SP10(1:10)
            ELSE
               CALL NTOAN2(surf_cobs_ape_data(SURF, 6),AAL(1:10))
               LINE=LINE(1:68)//AAL(1:10)
            END IF
            WRITE(OUTLYNE,200) LINE(1:79)
            CALL SHOWIT(0)
         END IF
!       HANDEL COBS DATA
         IF(COBS.EQ.1) THEN
            CALL NTOAN1(SURF,ASURF)
            LINE=ASURF(1:3)//SPCE(1:1)//COTYPE(1:8)//SPCE(1:1)
            CALL NTOAN2(surf_cobs_poly(SURF, 1),AAL(1:10))
            LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
!
            IF(surf_cobs_poly(SURF, 2).EQ.0.0D0) THEN
               LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_poly(SURF, 2),AAL(1:10))
               LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
            END IF
!
            IF(surf_cobs_poly(SURF, 3).EQ.0.0D0) THEN
               LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_poly(SURF, 3),AAL(1:10))
               LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_poly(SURF, 4).EQ.0.0D0) THEN
               LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_poly(SURF, 4),AAL(1:10))
               LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_poly(SURF, 5).EQ.0.0D0) THEN
               LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_poly(SURF, 5),AAL(1:10))
               LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_poly(SURF, 6).EQ.0.0D0) THEN
               LINE=LINE(1:68)//SP10(1:10)
            ELSE
               CALL NTOAN2(surf_cobs_poly(SURF, 6),AAL(1:10))
               LINE=LINE(1:68)//AAL(1:10)
            END IF
            WRITE(OUTLYNE,300) LINE(1:79)
            CALL SHOWIT(0)
         END IF
         IF(COBSE.EQ.1) THEN
            CALL NTOAN1(SURF,ASURF)
            LINE=ASURF(1:3)//SPCE(1:1)//COTYPE(1:8)//SPCE(1:1)
            CALL NTOAN2(surf_cobs_era_data(SURF, 1),AAL(1:10))
            LINE=LINE(1:13)//AAL(1:10)//SPCE(1:1)
!
            IF(surf_cobs_era_data(SURF, 2).EQ.0.0D0) THEN
               LINE=LINE(1:24)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_era_data(SURF, 2),AAL(1:10))
               LINE=LINE(1:24)//AAL(1:10)//SPCE(1:1)
            END IF
!
            IF(surf_cobs_era_data(SURF, 3).EQ.0.0D0) THEN
               LINE=LINE(1:35)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_era_data(SURF, 3),AAL(1:10))
               LINE=LINE(1:35)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_era_data(SURF, 4).EQ.0.0D0) THEN
               LINE=LINE(1:46)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_era_data(SURF, 4),AAL(1:10))
               LINE=LINE(1:46)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_era_data(SURF, 5).EQ.0.0D0) THEN
               LINE=LINE(1:57)//SP10(1:10)//SPCE(1:1)
            ELSE
               CALL NTOAN2(surf_cobs_era_data(SURF, 5),AAL(1:10))
               LINE=LINE(1:57)//AAL(1:10)//SPCE(1:1)
            END IF
            IF(surf_cobs_era_data(SURF, 6).EQ.0.0D0) THEN
               LINE=LINE(1:68)//SP10(1:10)
            ELSE
               CALL NTOAN2(surf_cobs_era_data(SURF, 6),AAL(1:10))
               LINE=LINE(1:68)//AAL(1:10)
            END IF
            WRITE(OUTLYNE,300) LINE(1:79)
            CALL SHOWIT(0)
         END IF
5     CONTINUE
   END IF
   NOHEAD=.TRUE.
   DO SURF=1,INT(sys_last_surf())
      IF(surf_multi_clap_flag(SURF).NE.0.0D0) THEN
         NOHEAD=.FALSE.
      END IF
   END DO
   IF(.NOT.NOHEAD) THEN
      WRITE(OUTLYNE,2009)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2010)
      CALL SHOWIT(0)
   END IF
   DO SURF=1,INT(sys_last_surf())
      IF(surf_multi_clap_flag(SURF).NE.0.0D0) THEN
         DO I=1,INT(surf_multi_clap_flag(SURF))
            WRITE(OUTLYNE,2008) SURF,I,MULTCLAP(I,1,SURF),MULTCLAP(I,2,SURF),MULTCLAP(I,3,SURF)
            CALL SHOWIT(1)
         END DO
      END IF
   END DO
   NOHEAD=.TRUE.
   DO SURF=1,INT(sys_last_surf())
      IF(surf_multi_cobs_flag(SURF).NE.0.0D0) THEN
         NOHEAD=.FALSE.
      END IF
   END DO
   IF(.NOT.NOHEAD) THEN
      WRITE(OUTLYNE,3009)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,2010)
      CALL SHOWIT(0)
   END IF
   DO SURF=1,INT(sys_last_surf())
      IF(surf_multi_cobs_flag(SURF).NE.0.0D0) THEN
         DO I=1,INT(surf_multi_cobs_flag(SURF))
            WRITE(OUTLYNE,2008) SURF,I,MULTCOBS(I,1,SURF),MULTCOBS(I,2,SURF),MULTCOBS(I,3,SURF)
            CALL SHOWIT(1)
         END DO
      END IF
   END DO
!
200 FORMAT(A79)
300 FORMAT(A79)
!
100 FORMAT('SURF',1X,A3,1X,':NO CLEAR APERTURE OR OBSCURATION DATA')
110 FORMAT('NO CLEAR APERTURE OR OBSCURATION DATA')
1000 FORMAT('CLEAR APERTURES AND OBSCURATIONS')
2000 FORMAT('SURF',4X,'TYPE',3X,'Y-SEMI.',4X,'X-SEMI.',4X,'Y-DEC',6X,'X-DEC',2X,'CORNER-RADIUS',1X,'TILT(DEG)')
2009 FORMAT('MULTIPLE CLEAR APERTURE DEFINITIONS')
3009 FORMAT('MULTIPLE OBSCURATION DEFINITIONS')
2010 FORMAT('SURF',4X,'MULTI#',3X,'  X  ',8X,'   Y   ',9X,' GAMMA ')
2001 FORMAT(14X,'(RADIUS)',3X,'(N-POLY)')
3000 FORMAT(25X,'(RAD-FLT)',22X,'(DELTA-Z)')
1500 FORMAT(1X)
   RETURN
END
! SUB SCALLE.FOR
SUBROUTINE SCALLE
!
   use DATCFG
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SCALE WHICH IMPLEMENTS THE SC AND WSC
!       COMMANDS AT THE CMD LEVEL.
!
   CHARACTER DUMPY*140
!
   COMMON/LUMPY/DUMPY
!
   INTEGER OLDCFG,I
!
!
!       RESET NEWOBJ,NEWREF AND NEWIMG
   CALL RESSUR
!
!       WHAT WAS CURRENT CONFIG NUMBER
   OLDCFG=F12
   IF(OLDCFG.NE.1) THEN
!       CHANGE TO CONFIG 1
      F12=1
      CALL PTOC
!************************************************************
!               FINISHED
!       COPYING PERM LENS TO CURRENT LENS
   ELSE
!       ALREADY AT CFG 1
   END IF
!
!               CHECK FOR ADDITIONAL INPUT AND
!               PRINT ERROR AND RETURN IF DISCOVERED.
!
   IF(SST.EQ.1) THEN
!
      IF(WC.EQ.'SC') THEN
         CALL REPORT_ERROR_AND_FAIL('"SC" ACCEPTS NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(WC.EQ.'WSC') THEN
         CALL REPORT_ERROR_AND_FAIL('"WSC" ACCEPTS NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WC.EQ.'SC'.AND.SQ.EQ.0) THEN
!*************************************************
!       SIMPLE SCALE FACTOR
!
      IF(W1.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ZERO FOR THE SCALE FACTOR IS NOT ALLOWED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       DEFAULTS FOR I AND J IN THE INPUT LINE.
!       FINITE CONJUGATE
      IF(DF1.EQ.1) W1=1.0D0
      IF(DF2.EQ.1) W2=NEWOBJ
      IF(DF3.EQ.1) W3=NEWIMG
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SC" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.LT.NEWOBJ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W3.GT.NEWIMG) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ENDING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.GE.W3) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING AND ENDING SURFACE NUMBERS OUT OF ORDER'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       PERFORM SCALING
      CALL SCALEA1
!       STRAIGHT SCALING SCALES LENS SURFACE DATA AND
!       SYSTEM DATA.
   ELSE
!       DON'T SCALE
   END IF
!*********************************************************
   IF(WC.EQ.'WSC'.AND.SQ.EQ.0) THEN
!*********************************************************
!       SIMPLE WORKING SCALE FACTOR
      IF(W1.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ZERO FOR THE SCALE FACTOR IS NOT ALLOWED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       DEFAULTS FOR I AND J IN THE INPUT LINE.
!       FINITE CONJUGATE
      IF(DF1.EQ.1) W1=1.0D0
      IF(DF2.EQ.1) W2=NEWOBJ
      IF(DF3.EQ.1) W3=NEWIMG
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         OUTLYNE='"WSC" TAKES NO NUMERIC WORD #4 OR #5 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(W2.LT.NEWOBJ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W3.GT.NEWIMG) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ENDING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.GE.W3) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING AND ENDING SURFACE NUMBERS OUT OF ORDER'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       PERFORM SCALING
      CALL SCALEA2
   END IF
!*****************************************************
!
   IF(WC.EQ.'SC'.AND.SQ.EQ.1) THEN
      IF(WQ.NE.'FY') THEN
!*****************************************************
         CALL REPORT_ERROR_AND_FAIL('INVALID QUALIFIER USED WITH "SC"'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      ELSE
      END IF
!       WQ MUST BE "FY", SCALE EFL
!
      IF(W1.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ZERO FOR THE SCALE FACTOR IS NOT ALLOWED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       DEFAULTS FOR I AND J IN THE INPUT LINE.
!       FINITE CONJUGATE
      IF(DF1.EQ.1) W1=1.0D0
      IF(DF2.EQ.1) W2=NEWOBJ
      IF(DF3.EQ.1) W3=NEWIMG
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SC FY" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.LT.NEWOBJ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W3.GT.NEWIMG) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ENDING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.GE.W3) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING AND ENDING SURFACE NUMBERS OUT OF ORDER'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       PERFORM SCALING
      CALL SCALEA3
   END IF
!
!*********************************************************
   IF(WC.EQ.'WSC'.AND.SQ.EQ.1) THEN
!*******************************************************
      IF(WQ.NE.'FY') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER USED WITH "WSC"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       WQ MUST BE "FY", (WORKING) SCALE EFL
!
      IF(W1.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ZERO FOR THE SCALE FACTOR IS NOT ALLOWED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       DEFAULTS FOR I AND J IN THE INPUT LINE.
!       FINITE CONJUGATE
      IF(DF1.EQ.1) W1=1.0D0
      IF(DF2.EQ.1) W2=NEWOBJ
      IF(DF3.EQ.1) W3=NEWIMG
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"WSC FY" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.LT.NEWOBJ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W3.GT.NEWIMG) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ENDING SURFACE BEYOND LEGAL RANGE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.GE.W3) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'STARTING AND ENDING SURFACE NUMBERS OUT OF ORDER'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       PERFORM SCALING
      CALL SCALEA4
   END IF
!
!       COPY CURRENT LENS TO PERM LENS SINCE CHANGES WERE MADE
!       TO CFG 1
!
   CALL CTOP
!************************************************************
!               PERMANENT LENS UPDATED
!       NOW RETURN TO CONFIG OLDCFG
   IF(OLDCFG.NE.1) THEN
      F12=OLDCFG
!
!       NOW THE MAGIC. FOR THE PARTICULAR CFG
!       DESIGNATED BY THE CURRENT VALE OF FLAG F12.
!       DURING THIS PROCESS, FLAG F15 IS SET TO 1
!       TO HELP THE UPDATE LENS AND UPDATE SPSRF
!       LEVELS KNOW HOW TO PROERLY RESPOND
      F15=1
!       READ THE CONTENTS OF
!       CONFG(F12,I)
!        FROM I=1 TO I=CFGCNT(F12)
!       AND ONE BY ONE RUN THE STRING VALUES FOUND BACK
!       THROUGH THE PROCESS SUBROUTINE VIA CFCHG1
      DO 14 I=1,CFGCNT(F12)
         EE12=CONFG(F12,I)
         HOLDER=EE12
         DUMPY=HOLDER(1:140)
         CALL CFCHG1
14    CONTINUE
!
!       PROCESS DONE, SET F15 TO 0
      F15=0
   ELSE
!       OLD CONFIG WAS 1, NO NEED TO RETURN TO ANOTHER
   END IF
! NOTE: ALL SCALE CHANGES WERE MADE TO CONFIG DATA
!       FROM CALLS FROM SCALE1 AND SCALE3 TO SUBROUTINE CFSC1
!       AND FROM SCALE2 AND SCALE4 (WORKING SCALE) TO SUBROUTINE
!       CFSC2.
   RETURN
END
! SUB SCALEA1.FOR
SUBROUTINE SCALEA1
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang, &
      sys_y1_scy, sys_x1_scx, sys_y1_scy_fang, sys_x1_scx_fang, &
      sys_fno_flag_y, sys_fno_hold_x, sys_fno_val_set, sys_na_set, &
      sys_set_say, sys_set_sax, sys_set_scy, sys_set_scx, &
      sys_set_scy_fang, sys_set_scx_fang, &
      sys_set_y1_scy, sys_set_x1_scx, sys_set_y1_scy_fang, sys_set_x1_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     THIS DOES SC WITHOUT FY
!
   INTEGER I,J,J1,J2
!
   real(real64) EFLY,SCW1,M1
!
!
!       STRAIGHT SCALING (SC,FACT,I,J)
!       W1=FACTOR
!       W2=STARTING SURFACE
!       W3=ENDING SURFACE
!       SAY,SAX,SCY,SCX AND WRX,WRY,BDX,BDY STARTING VALUES ALSO SCALED
!       FIRST OPERATE ON THE CURRENT LENS
!
!       SCALE THE NON-SURFACE SPECIFIC DATA
!
!       SAY,SAX,SCY,SCX,SCY FANG,SCX FANG
!       ANY CONFLICT WITH SCY AND SCY FANG WILL BE RESOLVED
!       AT THE END OF SCALING WHEN LNSEOS IS CALLED
!
   IF(sys_na_set().EQ.0.0D0.AND.sys_fno_val_set().EQ.0.0D0) THEN
      call sys_set_say(sys_say()*W1)
      call sys_set_sax(sys_sax()*W1)
   END IF
   call sys_set_scy(sys_scy()*W1)
   call sys_set_y1_scy(sys_y1_scy()*W1)
   call sys_set_scx(sys_scx()*W1)
   call sys_set_x1_scx(sys_x1_scx()*W1)
!     FIELD ANGLES DO NOT GET SCALED !
   call sys_set_scy_fang(sys_scy_fang())
   call sys_set_scx_fang(sys_scx_fang())
!
   call sys_set_y1_scy_fang(sys_y1_scy_fang()*W1)
   call sys_set_x1_scx_fang(sys_x1_scx_fang()*W1)
!
!       REMOVE FNBY HLD OR ER HOLD IF PRESENT
!
   IF(sys_fno_flag_y().EQ.-1.0D0.OR.sys_fno_hold_x().EQ.-1.0D0)THEN
      OUTLYNE='REMOVING F/# HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF
   IF(sys_fno_flag_y().EQ.-2.0D0.OR.sys_fno_hold_x().EQ.-2.0D0)THEN
      OUTLYNE='REMOVING EXIT PUPIL RADIUS HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF

!
!       NOW FOR SURFACE DATA
   J1=INT(W2)
   J2=INT(W3)
   DO I=J1,J2
!       CURVATURE
!       HERE SCALE FACTOR CHANGES THE RADIUS OF CURVATURE
!       FACTOR = 2 INCREASES RADIUS BY 2X OR REDUCES CURVATURE
!       BY FACTOR OF 2X.
!
      call set_surf_curvature(I, surf_curvature(I)*(1/W1))
!
!       CONIC CONSTANT DOES NOT CHANGE
!
!       THICKNESS
!
      call set_surf_thickness(I, surf_thickness(I)*W1)
!
!       ROOF AND THICKNESS
!
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     ROOF
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_roof_b(I, surf_roof_b(I)*W1)
      END IF
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     CCR
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_ccr_b(I, surf_ccr_b(I)*W1)
      END IF
!
!       IDEAL LENS THICKNESS
!
      call set_surf_ideal_efl(I, surf_ideal_efl(I)*W1)
!
!       FOURTH, SIXTH, EIGHTH AND TENTH ORDER ASPHERICS
!       FACTOR OF NX CHANGES THE FOURTH ORDER COEFFICIENT
!       FROM AD TO AD/(NX**3)
!       AND
!       AE TO AE/(NX**5) AND AF TO AF/(NX**7) AND
!       AG TO AG/(NX**9)
!
      call set_surf_asphere_coeff(I, 2, surf_asphere_coeff(I, 2)/W1)
      call set_surf_asphere_coeff(I, 4, surf_asphere_coeff(I, 4)/(W1**3))
      call set_surf_asphere_coeff(I, 6, surf_asphere_coeff(I, 6)/(W1**5))
      call set_surf_asphere_coeff(I, 8, surf_asphere_coeff(I, 8)/(W1**7))
      call set_surf_asphere_coeff(I, 10, surf_asphere_coeff(I, 10)/(W1**9))
      call set_surf_asphere_coeff(I, 12, surf_asphere_coeff(I, 12)/(W1**11))
      call set_surf_asphere_coeff(I, 14, surf_asphere_coeff(I, 14)/(W1**13))
      call set_surf_asphere_coeff(I, 16, surf_asphere_coeff(I, 16)/(W1**15))
      call set_surf_asphere_coeff(I, 18, surf_asphere_coeff(I, 18)/(W1**17))
      call set_surf_asphere_coeff(I, 20, surf_asphere_coeff(I, 20)/(W1**19))
!
!       NOW THE CLEAR APERTURES AND OBSCURATIONS
!
      M1=DABS(W1)
!     CLAP MULT SCALING
      IF(surf_multi_clap_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_clap_flag(I))
            MULTCLAP(J,1,I)=MULTCLAP(J,1,I)*M1
            MULTCLAP(J,2,I)=MULTCLAP(J,2,I)*M1
         END DO
      END IF
!     COBS MULT SCALING
      IF(surf_multi_cobs_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_cobs_flag(I))
            MULTCOBS(J,1,I)=MULTCOBS(J,1,I)*M1
            MULTCOBS(J,2,I)=MULTCOBS(J,2,I)*M1
         END DO
      END IF
!     SPIDER SCALING
      IF(surf_spider_flag(I).NE.0.0D0) THEN
         call set_surf_spider_angle(I, surf_spider_angle(I)*M1)
         call set_surf_spider_width(I, surf_spider_width(I)*M1)
      END IF
!       CIRCULAR
      IF(ABS(surf_clap_type(I)).EQ.1.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.1.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       ELLIPTICAL
      IF(ABS(surf_clap_type(I)).EQ.3.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.3.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       RECTANGULAR
      IF(ABS(surf_clap_type(I)).EQ.2.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.2.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!
!       RACETRACK
      IF(ABS(surf_clap_type(I)).EQ.4.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*W1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.4.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       POLY
      IF(ABS(surf_clap_type(I)).EQ.5.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2))
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2))
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.5.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2))
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2))
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       IPOLY
      IF(ABS(surf_clap_type(I)).EQ.6.0D0) THEN
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.6.0D0) THEN
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       ALL CLAPS/COBS HAVE BEEN SCALED
!
!       TORIC CURVATURE
!
      call set_surf_toric_curvature(I, surf_toric_curvature(I)/(W1))
!
!       X,Y AND Z DECENTERS
!
      call set_surf_decenter_y(I, surf_decenter_y(I)*DABS(W1))
      call set_surf_decenter_x(I, surf_decenter_x(I)*DABS(W1))
      call set_surf_decenter_z(I, surf_decenter_z(I)*W1)
      call set_surf_focus_dx(I, surf_focus_dx(I)*DABS(W1))
      call set_surf_focus_dy(I, surf_focus_dy(I)*DABS(W1))
      call set_surf_focus_dz(I, surf_focus_dz(I)*W1)
      call set_surf_global_dx(I, surf_global_dx(I)*DABS(W1))
      call set_surf_global_dy(I, surf_global_dy(I)*DABS(W1))
      call set_surf_global_dz(I, surf_global_dz(I)*W1)
      IF(W1.LT.0.0D0) call set_surf_alpha(I, -surf_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_beta(I, -surf_beta(I))
      IF(W1.LT.0.0D0) call set_surf_gamma(I, -surf_gamma(I))
      IF(W1.LT.0.0D0) call set_surf_alpha_deg(I, -surf_alpha_deg(I))
      IF(W1.LT.0.0D0) call set_surf_beta_deg(I, -surf_beta_deg(I))
      IF(W1.LT.0.0D0) call set_surf_gamma_deg(I, -surf_gamma_deg(I))
      IF(W1.LT.0.0D0) call set_surf_global_alpha(I, -surf_global_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_global_beta(I, -surf_global_beta(I))
      IF(W1.LT.0.0D0) call set_surf_global_gamma(I, -surf_global_gamma(I))

!       X,Y AND Z PIVOTS
!
      call set_surf_pivot_x(I, surf_pivot_x(I)*DABS(W1))
      call set_surf_pivot_y(I, surf_pivot_y(I)*DABS(W1))
      call set_surf_pivot_z(I, surf_pivot_z(I)*W1)
!
!       ASPHERIC TORIC COEFFICIENTS
!
      call set_surf_anamorphic_coeff(I, 4, surf_anamorphic_coeff(I, 4)/(W1**3))
      call set_surf_anamorphic_coeff(I, 6, surf_anamorphic_coeff(I, 6)/(W1**5))
      call set_surf_anamorphic_coeff(I, 8, surf_anamorphic_coeff(I, 8)/(W1**7))
      call set_surf_anamorphic_coeff(I, 10, surf_anamorphic_coeff(I, 10)/(W1**9))
!
!       ALL THE SYSTEM AND ALENS VALUES THAT NEEDED
!       SCALING HAVE BEEN SCALED.
!
!       NOW SOLVE TARGET VALUES.
!
!       ANGULAR TARGETS ARE NOT SCALED
!       DIMENSIONAL TARGETS ARE SCALED
!
      IF(SOLVE(6,I).EQ.1.0D0.OR.SOLVE(6,I).EQ.2.0D0.OR.SOLVE(6,I).EQ.3.0D0) THEN
         SOLVE(7,I)=SOLVE(7,I)*W1
      ELSE
      END IF
      IF(SOLVE(4,I).EQ.4.0.OR.SOLVE(4,I).EQ.5.0D0.OR.SOLVE(4,I).EQ.6.0D0) THEN
         SOLVE(3,I)=SOLVE(3,I)*W1
      ELSE
      END IF
!
!       ALL SOLVE SCALING IS COMPLETE
!
!       PIKUP SCALING
!       IN PIKUP SCALING, THE ONLY VALUE SCALED IS THE
!       ADDITIVE CONSTANT IF NUMERIC WORD 4 IS NOT 1.0
!       RD
      PIKUP(4,I,1)=PIKUP(4,I,1)*W1
!       CV
      PIKUP(4,I,2)=PIKUP(4,I,2)*W1
!       TH
      PIKUP(4,I,3)=PIKUP(4,I,3)*W1
!       THOAL (THE FIFTH TERM IS SCALED CAUSE THERE WERE 2 SURFACES ENTERED)
      PIKUP(5,I,32)=PIKUP(5,I,32)*W1
!       CC
      PIKUP(4,I,4)=PIKUP(4,I,4)*W1
!       AD
      PIKUP(4,I,5)=PIKUP(4,I,5)/(W1**3)
!       AE
      PIKUP(4,I,6)=PIKUP(4,I,6)/(W1**5)
!       AF
      PIKUP(4,I,7)=PIKUP(4,I,7)/(W1**7)
!       AG
      PIKUP(4,I,8)=PIKUP(4,I,8)/(W1**9)
!       CVTOR
      PIKUP(4,I,9)=PIKUP(4,I,9)*W1
!       RDTOR
      PIKUP(4,I,10)=PIKUP(4,I,10)*W1
!       YD
      PIKUP(4,I,13)=PIKUP(4,I,13)*W1
!       XD
      PIKUP(4,I,14)=PIKUP(4,I,14)*W1
!       ALPHA
      PIKUP(4,I,15)=PIKUP(4,I,15)*W1
!       BETA
      PIKUP(4,I,16)=PIKUP(4,I,16)*W1
!       GAMMA
      PIKUP(4,I,17)=PIKUP(4,I,17)*W1
!       CLAP
      PIKUP(4,I,18)=PIKUP(4,I,18)*M1
!       COBS
      PIKUP(4,I,19)=PIKUP(4,I,19)*M1
!       CCTOR
      PIKUP(4,I,20)=PIKUP(4,I,20)*W1
!       ADTOR
      PIKUP(4,I,22)=PIKUP(4,I,22)/(W1**3)
!       AETOR
      PIKUP(4,I,23)=PIKUP(4,I,23)/(W1**5)
!       AFTOR
      PIKUP(4,I,24)=PIKUP(4,I,24)/(W1**7)
!       AGTOR
      PIKUP(4,I,25)=PIKUP(4,I,25)/(W1**9)
!       AC
      PIKUP(4,I,26)=PIKUP(4,I,26)/(W1)
!       AH
      PIKUP(4,I,27)=PIKUP(4,I,27)/(W1**11)
!       AI
      PIKUP(4,I,28)=PIKUP(4,I,28)/(W1**13)
!       AJ
      PIKUP(4,I,29)=PIKUP(4,I,29)/(W1**15)
!       AK
      PIKUP(4,I,30)=PIKUP(4,I,30)/(W1**17)
!       AL
      PIKUP(4,I,31)=PIKUP(4,I,31)/(W1**19)
!
!               NOW ALL THE PIKUPS ARE SCALED
!       PRO,NPRO AND GLASS DON'T SCALE!
   END DO
!
!       NOW MAKE CHANGES TO THE CONFIG DATA
!       WHICH IS APPROPRIATE FOR THE PARTICULAR
!       SCALING REQUESTED.
   CALL CFSC1
!       ALL RESCALING WAS PERFORMED.
!       RESCALING AFFECTS SPECIAL SURFACE DATA
!       AS OF 1/6/94
   SCW1=W1
   CALL SCASPC(SCW1)
   F1=0
   F6=1
   F22=0
   LNSTYP=1
   CALL LNSEOS
   RETURN
END
! SUB SCALEA2.FOR
SUBROUTINE SCALEA2
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang, &
      sys_fno_flag_y, sys_fno_hold_x
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     THIS DOES WSC WITHOUT FY
!
   INTEGER I,J,J1,J2
!
   real(real64) EFLY,SCW1,M1
!
!
!
!       STRAIGHT SCALING (SC,FACT,I,J)
!       WITHOUT SAY,SCY,SAX OR SCX AFFECTED
!       W1=FACTOR
!       W2=STARTING SURFACE
!       W3=ENDING SURFACE
!       SAY,SAX,SCY,SCX STARTING VALUES NOT SCALED
!       FIRST OPERATE ON THE CURRENT LENS
!
!
!       REMOVE FNBY HLD OR ER HOLD IF PRESENT
!
   IF(sys_fno_flag_y().EQ.-1.0D0.OR.sys_fno_hold_x().EQ.-1.0D0)THEN
      OUTLYNE='REMOVING F/# HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF
   IF(sys_fno_flag_y().EQ.-2.0D0.OR.sys_fno_hold_x().EQ.-2.0D0)THEN
      OUTLYNE='REMOVING EXIT PUPIL RADIUS HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF

!
!       NOW FOR SURFACE DATA
   J1=INT(W2)
   J2=INT(W3)
   DO I=J1,J2
!       CURVATURE
!       HERE SCALE FACTOR CHANGES THE RADIUS OF CURVATURE
!       FACTOR = 2 INCREASES RADIUS BY 2X OR REDUCES CURVATURE
!       BY FACTOR OF 2X.
!
      call set_surf_curvature(I, surf_curvature(I)*(1/W1))
!
!       CONIC CONSTANT DOES NOT CHANGE
!
!       THICKNESS
!
      call set_surf_thickness(I, surf_thickness(I)*W1)
!
!       ROOF AND THICKNESS
!
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     ROOF
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_roof_b(I, surf_roof_b(I)*W1)
      END IF
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     CCR
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_ccr_b(I, surf_ccr_b(I)*W1)
      END IF
!
!       IDEAL LENS THICKNESS
!
      call set_surf_ideal_efl(I, surf_ideal_efl(I)*W1)
!
!       FOURTH, SIXTH, EIGHTH AND TENTH ORDER ASPHERICS
!       FACTOR OF NX CHANGES THE FOURTH ORDER COEFFICIENT
!       FROM AD TO AD/(NX**3)
!       AND
!       AE TO AE/(NX**5) AND AF TO AF/(NX**7) AND
!       AG TO AG/(NX**9)
!
      call set_surf_asphere_coeff(I, 2, surf_asphere_coeff(I, 2)/W1)
      call set_surf_asphere_coeff(I, 4, surf_asphere_coeff(I, 4)/(W1**3))
      call set_surf_asphere_coeff(I, 6, surf_asphere_coeff(I, 6)/(W1**5))
      call set_surf_asphere_coeff(I, 8, surf_asphere_coeff(I, 8)/(W1**7))
      call set_surf_asphere_coeff(I, 10, surf_asphere_coeff(I, 10)/(W1**9))
      call set_surf_asphere_coeff(I, 12, surf_asphere_coeff(I, 12)/(W1**11))
      call set_surf_asphere_coeff(I, 14, surf_asphere_coeff(I, 14)/(W1**13))
      call set_surf_asphere_coeff(I, 16, surf_asphere_coeff(I, 16)/(W1**15))
      call set_surf_asphere_coeff(I, 18, surf_asphere_coeff(I, 18)/(W1**17))
      call set_surf_asphere_coeff(I, 20, surf_asphere_coeff(I, 20)/(W1**19))
!
!       NOW THE CLEAR APERTURES AND OBSCURATIONS
      M1=DABS(W1)
!
!     CLAP MULT SCALING
      IF(surf_multi_clap_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_clap_flag(I))
            MULTCLAP(J,1,I)=MULTCLAP(J,1,I)*M1
            MULTCLAP(J,2,I)=MULTCLAP(J,2,I)*M1
         END DO
      END IF
!     COBS MULT SCALING
      IF(surf_multi_cobs_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_cobs_flag(I))
            MULTCOBS(J,1,I)=MULTCOBS(J,1,I)*M1
            MULTCOBS(J,2,I)=MULTCOBS(J,2,I)*M1
         END DO
      END IF
!     SPIDER SCALING
      IF(surf_spider_flag(I).NE.0.0D0) THEN
         call set_surf_spider_angle(I, surf_spider_angle(I)*M1)
         call set_surf_spider_width(I, surf_spider_width(I)*M1)
      END IF
!       CIRCULAR
      IF(ABS(surf_clap_type(I)).EQ.1.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.1.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       RECTANGULAR
      IF(ABS(surf_clap_type(I)).EQ.2.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.2.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       ELLIPTICAL
      IF(ABS(surf_clap_type(I)).EQ.3.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.3.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!
!       RACETRACK
      IF(ABS(surf_clap_type(I)).EQ.4.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.4.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       POLY
      IF(ABS(surf_clap_type(I)).EQ.5.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2))
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2))
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.5.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2))
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2))
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       IPOLY
      IF(ABS(surf_clap_type(I)).EQ.6.0D0) THEN
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.6.0D0) THEN
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       ALL CLAPS/COBS HAVE BEEN SCALED
!
!       TORIC CURVATURE
!
      call set_surf_toric_curvature(I, surf_toric_curvature(I)/(W1))
!
!       X,Y AND Z DECENTERS
!
      call set_surf_decenter_y(I, surf_decenter_y(I)*DABS(W1))
      call set_surf_decenter_x(I, surf_decenter_x(I)*DABS(W1))
      call set_surf_decenter_z(I, surf_decenter_z(I)*W1)
      call set_surf_focus_dx(I, surf_focus_dx(I)*DABS(W1))
      call set_surf_focus_dy(I, surf_focus_dy(I)*DABS(W1))
      call set_surf_focus_dz(I, surf_focus_dz(I)*W1)
      call set_surf_global_dx(I, surf_global_dx(I)*DABS(W1))
      call set_surf_global_dy(I, surf_global_dy(I)*DABS(W1))
      call set_surf_global_dz(I, surf_global_dz(I)*W1)
      IF(W1.LT.0.0D0) call set_surf_alpha(I, -surf_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_beta(I, -surf_beta(I))
      IF(W1.LT.0.0D0) call set_surf_gamma(I, -surf_gamma(I))
      IF(W1.LT.0.0D0) call set_surf_alpha_deg(I, -surf_alpha_deg(I))
      IF(W1.LT.0.0D0) call set_surf_beta_deg(I, -surf_beta_deg(I))
      IF(W1.LT.0.0D0) call set_surf_gamma_deg(I, -surf_gamma_deg(I))
      IF(W1.LT.0.0D0) call set_surf_global_alpha(I, -surf_global_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_global_beta(I, -surf_global_beta(I))
      IF(W1.LT.0.0D0) call set_surf_global_gamma(I, -surf_global_gamma(I))
!
!       X,Y AND Z PIVOTS
!
      call set_surf_pivot_x(I, surf_pivot_x(I)*DABS(W1))
      call set_surf_pivot_y(I, surf_pivot_y(I)*DABS(W1))
      call set_surf_pivot_z(I, surf_pivot_z(I)*W1)
!
!       ASPHERIC TORIC COEFFICIENTS
!
      call set_surf_anamorphic_coeff(I, 4, surf_anamorphic_coeff(I, 4)/(W1**3))
      call set_surf_anamorphic_coeff(I, 6, surf_anamorphic_coeff(I, 6)/(W1**5))
      call set_surf_anamorphic_coeff(I, 8, surf_anamorphic_coeff(I, 8)/(W1**7))
      call set_surf_anamorphic_coeff(I, 10, surf_anamorphic_coeff(I, 10)/(W1**9))
!
!       ALL THE SYSTEM AND ALENS VALUES THAT NEEDED
!       SCALING HAVE BEEN SCALED.
!
!       NOW SOLVE TARGET VALUES.
!
!       ANGULAR TARGETS ARE NOT SCALED
!       DIMENSIONAL TARGETS ARE SCALED
!
      IF(SOLVE(6,I).EQ.1.0D0.OR.SOLVE(6,I).EQ.2.0D0.OR.SOLVE(6,I).EQ.3.0D0) THEN
         SOLVE(7,I)=SOLVE(7,I)*W1
      ELSE
      END IF
      IF(SOLVE(4,I).EQ.4.0D0.OR.SOLVE(4,I).EQ.5.0D0.OR.SOLVE(4,I).EQ.6.0D0) THEN
         SOLVE(3,I)=SOLVE(3,I)*W1
      ELSE
      END IF
!
!       ALL SOLVE SCALING IS COMPLETE
!
!       PIKUP SCALING
!       IN PIKUP SCALING, THE ONLY VALUE SCALED IS THE
!       ADDITIVE CONSTANT IF NUMERIC WORD 4 IS NOT 1.0
!       RD
      PIKUP(4,I,1)=PIKUP(4,I,1)*W1
!       CV
      PIKUP(4,I,2)=PIKUP(4,I,2)*W1
!       TH
      PIKUP(4,I,3)=PIKUP(4,I,3)*W1
!       THOAL (THE FIFTH TERM IS SCALED CAUSE THERE WERE 2 SURFACES ENTERED)
      PIKUP(5,I,32)=PIKUP(5,I,32)*W1
!       CC
      PIKUP(4,I,4)=PIKUP(4,I,4)*W1
!       AD
      PIKUP(4,I,5)=PIKUP(4,I,5)/(W1**3)
!       AE
      PIKUP(4,I,6)=PIKUP(4,I,6)/(W1**5)
!       AF
      PIKUP(4,I,7)=PIKUP(4,I,7)/(W1**7)
!       AG
      PIKUP(4,I,8)=PIKUP(4,I,8)/(W1**9)
!       CVTOR
      PIKUP(4,I,9)=PIKUP(4,I,9)*W1
!       RDTOR
      PIKUP(4,I,10)=PIKUP(4,I,10)*W1
!       YD
      PIKUP(4,I,13)=PIKUP(4,I,13)*W1
!       XD
      PIKUP(4,I,14)=PIKUP(4,I,14)*W1
!       ALPHA
      PIKUP(4,I,15)=PIKUP(4,I,15)*W1
!       BETA
      PIKUP(4,I,16)=PIKUP(4,I,16)*W1
!       GAMMA
      PIKUP(4,I,17)=PIKUP(4,I,17)*W1
!       CLAP
      PIKUP(4,I,18)=PIKUP(4,I,18)*M1
!       COBS
      PIKUP(4,I,19)=PIKUP(4,I,19)*M1
!       CCTOR
      PIKUP(4,I,20)=PIKUP(4,I,20)*W1
!       ADTOR
      PIKUP(4,I,22)=PIKUP(4,I,22)/(W1**3)
!       AETOR
      PIKUP(4,I,23)=PIKUP(4,I,23)/(W1**5)
!       AFTOR
      PIKUP(4,I,24)=PIKUP(4,I,24)/(W1**7)
!       AGTOR
      PIKUP(4,I,25)=PIKUP(4,I,25)/(W1**9)
!       AC
      PIKUP(4,I,26)=PIKUP(4,I,26)/(W1)
!       AH
      PIKUP(4,I,27)=PIKUP(4,I,27)/(W1**11)
!       AI
      PIKUP(4,I,28)=PIKUP(4,I,28)/(W1**13)
!       AJ
      PIKUP(4,I,29)=PIKUP(4,I,29)/(W1**15)
!       AK
      PIKUP(4,I,30)=PIKUP(4,I,30)/(W1**17)
!       AL
      PIKUP(4,I,31)=PIKUP(4,I,31)/(W1**19)
!
!               NOW ALL THE PIKUPS ARE SCALED
!       PRO,NPRO AND GLASS DON'T SCALE!
   END DO
!
!       NOW MAKE CHANGES TO THE CONFIG DATA
!       WHICH IS APPROPRIATE FOR THE PARTICULAR
!       SCALING REQUESTED.
   CALL CFSC2
!       ALL RESCALING WAS PERFORMED.
!       RESCALING AFFECTS SPECIAL SURFACE DATA
!       AS OF 1/6/94
   SCW1=W1
   CALL SCASPC(SCW1)
   F1=0
   F6=1
   F22=0
   LNSTYP=1
   CALL LNSEOS
   RETURN
END
! SUB SCALEA3.FOR
SUBROUTINE SCALEA3
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang, &
      sys_y1_scy, sys_x1_scx, sys_y1_scy_fang, sys_x1_scx_fang, &
      sys_fno_flag_y, sys_fno_hold_x, sys_fno_val_set, sys_na_set, &
      sys_set_say, sys_set_sax, sys_set_scy, sys_set_scx, &
      sys_set_scy_fang, sys_set_scx_fang, &
      sys_set_y1_scy, sys_set_x1_scx, sys_set_y1_scy_fang, sys_set_x1_scx_fang
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     THIS DOES SC WITH FY
!
   INTEGER I,J,J1,J2
!
   real(real64) EFLY,SCW1,M1
!
!
!
!       SCALING TO A SPECIFIC EFL (SC EFL,EFL,I,J)
!       W1=DESIRED
!       W2=STARTING SURFACE
!       W3=ENDING SURFACE
!       SAY,SAX,SCY,SCX STARTING VALUES ALSO SCALED
!       FIRST OPERATE ON THE CURRENT LENS
!
!       WE MUST FIRST DETERMINE THE ORIGINAL EFL
!       OF THE ELEMENTS FROM SURFACE I TO J
!       THEN THE CORRECT SCALE FACTOR( USED FOR ALL CONFIGS BUT
!       DETERMINED FROM DATA IN CONFIG 1 IS GIVEN BY:
!
!       SCALE FACTOR= (NEW DESIRED EFL)/(ORIGINAL EFL)
!
!       YZ PLANE EFL CALCULATION
!
   IF(W2.GT.0.0D0) THEN
      I=INT(W2)-1
   ELSE
      I=INT(W2)
   END IF
   J=INT(W3)
   EFLY=-(((PXTRAY(2,I)*PXTRAY(5,I+1))-(PXTRAY(1,I+1)*PXTRAY(6,I)))/((PXTRAY(2,I)*PXTRAY(6,J))-(PXTRAY(6,I)*PXTRAY(2,J))))
   IF(EFLY.EQ.0.0D0.OR.DABS(EFLY).GT.1.0D10) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO EFL CAN BE CALCULATED FOR SPECIFIED SURFACES'//'\n'//&
      & 'NO EFL SCALING CAN BE PERFORMED', 1)
      RETURN
   END IF
!
!       NEW SCALE FACTOR IS W1
!
   W1=W1/(EFLY)
!
!       SCALE THE NON-SURFACE SPECIFIC DATA
!
!       SAY,SAX,SCY,SCX,SCY FANG,SCX FANG
!       ANY CONFLICT WITH SCY AND SCY FANG WILL BE RESOLVED
!       AT THE END OF SCALING WHEN LNSEOS IS CALLED
!
   IF(sys_na_set().EQ.0.0D0.AND.sys_fno_val_set().EQ.0.0D0) THEN
      call sys_set_say(sys_say()*W1)
      call sys_set_sax(sys_sax()*W1)
   END IF
   call sys_set_scy(sys_scy()*W1)
   call sys_set_y1_scy(sys_y1_scy()*W1)
   call sys_set_scx(sys_scx()*W1)
   call sys_set_x1_scx(sys_x1_scx()*W1)
!     FIELD ANGLES DO NOT GET SCALED !
   call sys_set_scy_fang(sys_scy_fang())
   call sys_set_scx_fang(sys_scx_fang())
!
   call sys_set_y1_scy_fang(sys_y1_scy_fang()*W1)
   call sys_set_x1_scx_fang(sys_x1_scx_fang()*W1)
!
!       REMOVE FNBY HLD OR ER HOLD IF PRESENT
!
   IF(sys_fno_flag_y().EQ.-1.0D0.OR.sys_fno_hold_x().EQ.-1.0D0)THEN
      OUTLYNE='REMOVING F/# HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF
   IF(sys_fno_flag_y().EQ.-2.0D0.OR.sys_fno_hold_x().EQ.-2.0D0)THEN
      OUTLYNE='REMOVING EXIT PUPIL RADIUS HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF

!
!       NOW FOR SURFACE DATA
   J1=INT(W2)
   J2=INT(W3)
   DO I=J1,J2
!       CURVATURE
!       HERE SCALE FACTOR CHANGES THE RADIUS OF CURVATURE
!       FACTOR = 2 INCREASES RADIUS BY 2X OR REDUCES CURVATURE
!       BY FACTOR OF 2X.
!
      call set_surf_curvature(I, surf_curvature(I)*(1/W1))
!
!       CONIC CONSTANT DOES NOT CHANGE
!
!       THICKNESS
!
      call set_surf_thickness(I, surf_thickness(I)*W1)
!
!       ROOF AND THICKNESS
!
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     ROOF
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_roof_b(I, surf_roof_b(I)*W1)
      END IF
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     CCR
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_ccr_b(I, surf_ccr_b(I)*W1)
      END IF
!
!       IDEAL LENS THICKNESS
!
      call set_surf_ideal_efl(I, surf_ideal_efl(I)*W1)
!
!       FOURTH, SIXTH, EIGHTH AND TENTH ORDER ASPHERICS
!       FACTOR OF NX CHANGES THE FOURTH ORDER COEFFICIENT
!       FROM AD TO AD/(NX**3)
!       AND
!       AE TO AE/(NX**5) AND AF TO AF/(NX**7) AND
!       AG TO AG/(NX**9)
!
      call set_surf_asphere_coeff(I, 2, surf_asphere_coeff(I, 2)/W1)
      call set_surf_asphere_coeff(I, 4, surf_asphere_coeff(I, 4)/(W1**3))
      call set_surf_asphere_coeff(I, 6, surf_asphere_coeff(I, 6)/(W1**5))
      call set_surf_asphere_coeff(I, 8, surf_asphere_coeff(I, 8)/(W1**7))
      call set_surf_asphere_coeff(I, 10, surf_asphere_coeff(I, 10)/(W1**9))
      call set_surf_asphere_coeff(I, 12, surf_asphere_coeff(I, 12)/(W1**11))
      call set_surf_asphere_coeff(I, 14, surf_asphere_coeff(I, 14)/(W1**13))
      call set_surf_asphere_coeff(I, 16, surf_asphere_coeff(I, 16)/(W1**15))
      call set_surf_asphere_coeff(I, 18, surf_asphere_coeff(I, 18)/(W1**17))
      call set_surf_asphere_coeff(I, 20, surf_asphere_coeff(I, 20)/(W1**19))
!
!       NOW THE CLEAR APERTURES AND OBSCURATIONS
      M1=DABS(W1)
!
!     CLAP MULT SCALING
      IF(surf_multi_clap_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_clap_flag(I))
            MULTCLAP(J,1,I)=MULTCLAP(J,1,I)*M1
            MULTCLAP(J,2,I)=MULTCLAP(J,2,I)*M1
         END DO
      END IF
!     COBS MULT SCALING
      IF(surf_multi_cobs_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_cobs_flag(I))
            MULTCOBS(J,1,I)=MULTCOBS(J,1,I)*M1
            MULTCOBS(J,2,I)=MULTCOBS(J,2,I)*M1
         END DO
      END IF
!     SPIDER SCALING
      IF(surf_spider_flag(I).NE.0.0D0) THEN
         call set_surf_spider_angle(I, surf_spider_angle(I)*M1)
         call set_surf_spider_width(I, surf_spider_width(I)*M1)
      END IF
!       CIRCULAR
      IF(ABS(surf_clap_type(I)).EQ.1.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.1.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       RECTANGULAR
      IF(ABS(surf_clap_type(I)).EQ.2.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.2.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       ELLIPTICAL
      IF(ABS(surf_clap_type(I)).EQ.3.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.3.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!
!       RACETRACK
      IF(ABS(surf_clap_type(I)).EQ.4.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.4.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       POLY
      IF(ABS(surf_clap_type(I)).EQ.5.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2))
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2))
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.5.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2))
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2))
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       IPOLY
      IF(ABS(surf_clap_type(I)).EQ.6.0D0) THEN
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.6.0D0) THEN
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       ALL CLAPS/COBS HAVE BEEN SCALED
!
!       TORIC CURVATURE
!
      call set_surf_toric_curvature(I, surf_toric_curvature(I)/(W1))
!
!       X,Y AND Z DECENTERS
!
      call set_surf_decenter_y(I, surf_decenter_y(I)*DABS(W1))
      call set_surf_decenter_x(I, surf_decenter_x(I)*DABS(W1))
      call set_surf_decenter_z(I, surf_decenter_z(I)*W1)
      call set_surf_focus_dx(I, surf_focus_dx(I)*DABS(W1))
      call set_surf_focus_dy(I, surf_focus_dy(I)*DABS(W1))
      call set_surf_focus_dz(I, surf_focus_dz(I)*W1)
      call set_surf_global_dx(I, surf_global_dx(I)*DABS(W1))
      call set_surf_global_dy(I, surf_global_dy(I)*DABS(W1))
      call set_surf_global_dz(I, surf_global_dz(I)*W1)
      IF(W1.LT.0.0D0) call set_surf_alpha(I, -surf_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_beta(I, -surf_beta(I))
      IF(W1.LT.0.0D0) call set_surf_gamma(I, -surf_gamma(I))
      IF(W1.LT.0.0D0) call set_surf_alpha_deg(I, -surf_alpha_deg(I))
      IF(W1.LT.0.0D0) call set_surf_beta_deg(I, -surf_beta_deg(I))
      IF(W1.LT.0.0D0) call set_surf_gamma_deg(I, -surf_gamma_deg(I))
      IF(W1.LT.0.0D0) call set_surf_global_alpha(I, -surf_global_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_global_beta(I, -surf_global_beta(I))
      IF(W1.LT.0.0D0) call set_surf_global_gamma(I, -surf_global_gamma(I))
!
!       X,Y AND Z PIVOTS
!
      call set_surf_pivot_x(I, surf_pivot_x(I)*DABS(W1))
      call set_surf_pivot_y(I, surf_pivot_y(I)*DABS(W1))
      call set_surf_pivot_z(I, surf_pivot_z(I)*W1)
!
!       ASPHERIC TORIC COEFFICIENTS
!
      call set_surf_anamorphic_coeff(I, 4, surf_anamorphic_coeff(I, 4)/(W1**3))
      call set_surf_anamorphic_coeff(I, 6, surf_anamorphic_coeff(I, 6)/(W1**5))
      call set_surf_anamorphic_coeff(I, 8, surf_anamorphic_coeff(I, 8)/(W1**7))
      call set_surf_anamorphic_coeff(I, 10, surf_anamorphic_coeff(I, 10)/(W1**9))
!
!       ALL THE SYSTEM AND ALENS VALUES THAT NEEDED
!       SCALING HAVE BEEN SCALED.
!
!       NOW SOLVE TARGET VALUES.
!
!       ANGULAR TARGETS ARE NOT SCALED
!       DIMENSIONAL TARGETS ARE SCALED
!
      IF(SOLVE(6,I).EQ.1.0D0.OR.SOLVE(6,I).EQ.2.0D0.OR.SOLVE(6,I).EQ.3.0D0) THEN
         SOLVE(7,I)=SOLVE(7,I)*W1
      ELSE
      END IF
      IF(SOLVE(4,I).EQ.4.0D0.OR.SOLVE(4,I).EQ.5.0D0.OR.SOLVE(4,I).EQ.6.0D0) THEN
         SOLVE(3,I)=SOLVE(3,I)*W1
      ELSE
      END IF
!
!       ALL SOLVE SCALING IS COMPLETE
!
!       PIKUP SCALING
!       IN PIKUP SCALING, THE ONLY VALUE SCALED IS THE
!       ADDITIVE CONSTANT IF NUMERIC WORD 4 IS NOT 1.0
!       RD
      PIKUP(4,I,1)=PIKUP(4,I,1)*W1
!       CV
      PIKUP(4,I,2)=PIKUP(4,I,2)*W1
!       TH
      PIKUP(4,I,3)=PIKUP(4,I,3)*W1
!       THOAL (THE FIFTH TERM IS SCALED CAUSE THERE WERE 2 SURFACES ENTERED)
      PIKUP(5,I,32)=PIKUP(5,I,32)*W1
!       CC
      PIKUP(4,I,4)=PIKUP(4,I,4)*W1
!       AD
      PIKUP(4,I,5)=PIKUP(4,I,5)/(W1**3)
!       AE
      PIKUP(4,I,6)=PIKUP(4,I,6)/(W1**5)
!       AF
      PIKUP(4,I,7)=PIKUP(4,I,7)/(W1**7)
!       AG
      PIKUP(4,I,8)=PIKUP(4,I,8)/(W1**9)
!       CVTOR
      PIKUP(4,I,9)=PIKUP(4,I,9)*W1
!       RDTOR
      PIKUP(4,I,10)=PIKUP(4,I,10)*W1
!       YD
      PIKUP(4,I,13)=PIKUP(4,I,13)*W1
!       XD
      PIKUP(4,I,14)=PIKUP(4,I,14)*W1
!       ALPHA
      PIKUP(4,I,15)=PIKUP(4,I,15)*W1
!       BETA
      PIKUP(4,I,16)=PIKUP(4,I,16)*W1
!       GAMMA
      PIKUP(4,I,17)=PIKUP(4,I,17)*W1
!       CLAP
      PIKUP(4,I,18)=PIKUP(4,I,18)*M1
!       COBS
      PIKUP(4,I,19)=PIKUP(4,I,19)*M1
!       CCTOR
      PIKUP(4,I,20)=PIKUP(4,I,20)*W1
!       ADTOR
      PIKUP(4,I,22)=PIKUP(4,I,22)/(W1**3)
!       AETOR
      PIKUP(4,I,23)=PIKUP(4,I,23)/(W1**5)
!       AFTOR
      PIKUP(4,I,24)=PIKUP(4,I,24)/(W1**7)
!       AGTOR
      PIKUP(4,I,25)=PIKUP(4,I,25)/(W1**9)
!       AC
      PIKUP(4,I,26)=PIKUP(4,I,26)/(W1)
!       AH
      PIKUP(4,I,27)=PIKUP(4,I,27)/(W1**11)
!       AI
      PIKUP(4,I,28)=PIKUP(4,I,28)/(W1**13)
!       AJ
      PIKUP(4,I,29)=PIKUP(4,I,29)/(W1**15)
!       AK
      PIKUP(4,I,30)=PIKUP(4,I,30)/(W1**17)
!       AL
      PIKUP(4,I,31)=PIKUP(4,I,31)/(W1**19)
!
!               NOW ALL THE PIKUPS ARE SCALED
!       PRO,NPRO AND GLASS DON'T SCALE!
   END DO
!
!       NOW MAKE CHANGES TO THE CONFIG DATA
!       WHICH IS APPROPRIATE FOR THE PARTICULAR
!       SCALING REQUESTED.
!               CALL CFSC1
!       ALL RESCALING WAS PERFORMED.
!       RESCALING AFFECTS SPECIAL SURFACE DATA
!       AS OF 1/6/94
   SCW1=W1
   CALL SCASPC(SCW1)
   F1=0
   F6=1
   F22=0
   LNSTYP=1
   CALL LNSEOS
   RETURN
END
! SUB SCALEA4.FOR
SUBROUTINE SCALEA4
!
   use DATLEN
   use DATMAI
   use mod_surface
   use mod_system, only: sys_last_surf, sys_autofunc, sys_say, sys_sax, &
      sys_scy, sys_scx, sys_scy_fang, sys_scx_fang, &
      sys_fno_flag_y, sys_fno_hold_x
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     THIS DOES WSC WITH FY
!
   INTEGER I,J,J1,J2
!
   real(real64) EFLY,SCW1,M1
!
!
!
!       WORKING SCALING TO A SPECIFIC EFL (SC EFL,EFL,I,J)
!       W1=DESIRED
!       W2=STARTING SURFACE
!       W3=ENDING SURFACE
!       SAY,SAX,SCY,SCX STARTING VALUES NOT SCALED
!       FIRST OPERATE ON THE CURRENT LENS
!
!       WE MUST FIRST DETERMINE THE ORIGINAL EFL
!       OF THE ELEMENTS FROM SURFACE I TO J
!       THEN THE CORRECT SCALE FACTOR( USED FOR ALL CONFIGS BUT
!       DETERMINED FROM DATA IN CONFIG 1 IS GIVEN BY:
!
!       SCALE FACTOR= (NEW DESIRED EFL)/(ORIGINAL EFL)
!
!       YZ PLANE EFL CALCULATION
!
   IF(W2.GT.0.0D0) THEN
      I=INT(W2)-1
   ELSE
      I=INT(W2)
   END IF
   J=INT(W3)
   EFLY=-(((PXTRAY(2,I)*PXTRAY(5,I+1))-(PXTRAY(1,I+1)*PXTRAY(6,I)))/((PXTRAY(2,I)*PXTRAY(6,J))-(PXTRAY(6,I)*PXTRAY(2,J))))
   IF(EFLY.EQ.0.0D0.OR.DABS(EFLY).GT.1.0D10) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO EFL CAN BE CALCULATED FOR SPECIFIED SURFACES'//'\n'//&
      & 'NO EFL SCALING CAN BE PERFORMED', 1)
      RETURN
   END IF
!
!       NEW SCALE FACTOR IS W1
!
   W1=W1/(EFLY)
!
!       REMOVE FNBY HLD OR ER HOLD IF PRESENT
!
   IF(sys_fno_flag_y().EQ.-1.0D0.OR.sys_fno_hold_x().EQ.-1.0D0)THEN
      OUTLYNE='REMOVING F/# HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF
   IF(sys_fno_flag_y().EQ.-2.0D0.OR.sys_fno_hold_x().EQ.-2.0D0)THEN
      OUTLYNE='REMOVING EXIT PUPIL RADIUS HOLD'
      CALL SHOWIT(1)
      SYSTEM(44:47)=0.0D0
   END IF

!
!       NOW FOR SURFACE DATA
   J1=INT(W2)
   J2=INT(W3)
   DO I=J1,J2
!       CURVATURE
!       HERE SCALE FACTOR CHANGES THE RADIUS OF CURVATURE
!       FACTOR = 2 INCREASES RADIUS BY 2X OR REDUCES CURVATURE
!       BY FACTOR OF 2X.
!
      call set_surf_curvature(I, surf_curvature(I)*(1/W1))
!
!       CONIC CONSTANT DOES NOT CHANGE
!
!       THICKNESS
!
      call set_surf_thickness(I, surf_thickness(I)*W1)
!
!       ROOF AND THICKNESS
!
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     ROOF
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_roof_b(I, surf_roof_b(I)*W1)
      END IF
      IF(surf_ccr_flag(I).EQ.1.0D0) THEN
!     CCR
         call set_surf_roof_a(I, surf_roof_a(I)*W1)
         call set_surf_ccr_b(I, surf_ccr_b(I)*W1)
      END IF
!
!       IDEAL LENS THICKNESS
!
      call set_surf_ideal_efl(I, surf_ideal_efl(I)*W1)
!
!       FOURTH, SIXTH, EIGHTH AND TENTH ORDER ASPHERICS
!       FACTOR OF NX CHANGES THE FOURTH ORDER COEFFICIENT
!       FROM AD TO AD/(NX**3)
!       AND
!       AE TO AE/(NX**5) AND AF TO AF/(NX**7) AND
!       AG TO AG/(NX**9)
!
      call set_surf_asphere_coeff(I, 2, surf_asphere_coeff(I, 2)/W1)
      call set_surf_asphere_coeff(I, 4, surf_asphere_coeff(I, 4)/(W1**3))
      call set_surf_asphere_coeff(I, 6, surf_asphere_coeff(I, 6)/(W1**5))
      call set_surf_asphere_coeff(I, 8, surf_asphere_coeff(I, 8)/(W1**7))
      call set_surf_asphere_coeff(I, 10, surf_asphere_coeff(I, 10)/(W1**9))
      call set_surf_asphere_coeff(I, 12, surf_asphere_coeff(I, 12)/(W1**11))
      call set_surf_asphere_coeff(I, 14, surf_asphere_coeff(I, 14)/(W1**13))
      call set_surf_asphere_coeff(I, 16, surf_asphere_coeff(I, 16)/(W1**15))
      call set_surf_asphere_coeff(I, 18, surf_asphere_coeff(I, 18)/(W1**17))
      call set_surf_asphere_coeff(I, 20, surf_asphere_coeff(I, 20)/(W1**19))
!
!       NOW THE CLEAR APERTURES AND OBSCURATIONS
!
      M1=DABS(W1)
!     CLAP MULT SCALING
      IF(surf_multi_clap_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_clap_flag(I))
            MULTCLAP(J,1,I)=MULTCLAP(J,1,I)*M1
            MULTCLAP(J,2,I)=MULTCLAP(J,2,I)*M1
         END DO
      END IF
!     COBS MULT SCALING
      IF(surf_multi_cobs_flag(I).NE.0.0D0) THEN
         DO J=1,INT(surf_multi_cobs_flag(I))
            MULTCOBS(J,1,I)=MULTCOBS(J,1,I)*M1
            MULTCOBS(J,2,I)=MULTCOBS(J,2,I)*M1
         END DO
      END IF
!     SPIDER SCALING
      IF(surf_spider_flag(I).NE.0.0D0) THEN
         call set_surf_spider_angle(I, surf_spider_angle(I)*M1)
         call set_surf_spider_width(I, surf_spider_width(I)*M1)
      END IF
!       CIRCULAR
      IF(ABS(surf_clap_type(I)).EQ.1.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.1.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       RECTANGULAR
      IF(ABS(surf_clap_type(I)).EQ.2.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.2.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!       ELLIPTICAL
      IF(ABS(surf_clap_type(I)).EQ.3.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.3.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
      ELSE
      END IF
!
!       RACETRACK
      IF(ABS(surf_clap_type(I)).EQ.4.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.4.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       POLY
      IF(ABS(surf_clap_type(I)).EQ.5.0D0) THEN
         call set_surf_clap_dim(I, 1, surf_clap_dim(I, 1)*M1)
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 2))
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 5, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 1, surf_cobs_ape_data(I, 1)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 2))
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 5, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.5.0D0) THEN
         call set_surf_cobs_poly(I, 1, surf_cobs_poly(I, 1)*M1)
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 2))
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 5, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 1, surf_cobs_era_data(I, 1)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 2))
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 5, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       IPOLY
      IF(ABS(surf_clap_type(I)).EQ.6.0D0) THEN
         call set_surf_clap_dim(I, 2, surf_clap_dim(I, 3)*M1)
         call set_surf_clap_dim(I, 3, surf_clap_dim(I, 4)*M1)
         call set_surf_clap_dim(I, 4, surf_clap_dim(I, 5)*M1)
         call set_surf_cobs_ape_data(I, 2, surf_cobs_ape_data(I, 3)*M1)
         call set_surf_cobs_ape_data(I, 3, surf_cobs_ape_data(I, 4)*M1)
         call set_surf_cobs_ape_data(I, 4, surf_cobs_ape_data(I, 5)*M1)
      ELSE
      END IF
      IF(ABS(surf_coat_type(I)).EQ.6.0D0) THEN
         call set_surf_cobs_poly(I, 2, surf_cobs_poly(I, 3)*M1)
         call set_surf_cobs_poly(I, 3, surf_cobs_poly(I, 4)*M1)
         call set_surf_cobs_poly(I, 4, surf_cobs_poly(I, 5)*M1)
         call set_surf_cobs_era_data(I, 2, surf_cobs_era_data(I, 3)*M1)
         call set_surf_cobs_era_data(I, 3, surf_cobs_era_data(I, 4)*M1)
         call set_surf_cobs_era_data(I, 4, surf_cobs_era_data(I, 5)*M1)
      ELSE
      END IF
!
!       ALL CLAPS/COBS HAVE BEEN SCALED
!
!       TORIC CURVATURE
!
      call set_surf_toric_curvature(I, surf_toric_curvature(I)/(W1))
!
!       X,Y AND Z DECENTERS
!
      call set_surf_decenter_y(I, surf_decenter_y(I)*DABS(W1))
      call set_surf_decenter_x(I, surf_decenter_x(I)*DABS(W1))
      call set_surf_decenter_z(I, surf_decenter_z(I)*W1)
      call set_surf_focus_dx(I, surf_focus_dx(I)*DABS(W1))
      call set_surf_focus_dy(I, surf_focus_dy(I)*DABS(W1))
      call set_surf_focus_dz(I, surf_focus_dz(I)*W1)
      call set_surf_global_dx(I, surf_global_dx(I)*DABS(W1))
      call set_surf_global_dy(I, surf_global_dy(I)*DABS(W1))
      call set_surf_global_dz(I, surf_global_dz(I)*W1)
      IF(W1.LT.0.0D0) call set_surf_alpha(I, -surf_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_beta(I, -surf_beta(I))
      IF(W1.LT.0.0D0) call set_surf_gamma(I, -surf_gamma(I))
      IF(W1.LT.0.0D0) call set_surf_alpha_deg(I, -surf_alpha_deg(I))
      IF(W1.LT.0.0D0) call set_surf_beta_deg(I, -surf_beta_deg(I))
      IF(W1.LT.0.0D0) call set_surf_gamma_deg(I, -surf_gamma_deg(I))
      IF(W1.LT.0.0D0) call set_surf_global_alpha(I, -surf_global_alpha(I))
      IF(W1.LT.0.0D0) call set_surf_global_beta(I, -surf_global_beta(I))
      IF(W1.LT.0.0D0) call set_surf_global_gamma(I, -surf_global_gamma(I))
!
!       X,Y AND Z PIVOTS
!
      call set_surf_pivot_x(I, surf_pivot_x(I)*DABS(W1))
      call set_surf_pivot_y(I, surf_pivot_y(I)*DABS(W1))
      call set_surf_pivot_z(I, surf_pivot_z(I)*W1)
!
!       ASPHERIC TORIC COEFFICIENTS
!
      call set_surf_anamorphic_coeff(I, 4, surf_anamorphic_coeff(I, 4)/(W1**3))
      call set_surf_anamorphic_coeff(I, 6, surf_anamorphic_coeff(I, 6)/(W1**5))
      call set_surf_anamorphic_coeff(I, 8, surf_anamorphic_coeff(I, 8)/(W1**7))
      call set_surf_anamorphic_coeff(I, 10, surf_anamorphic_coeff(I, 10)/(W1**9))
!
!       ALL THE SYSTEM AND ALENS VALUES THAT NEEDED
!       SCALING HAVE BEEN SCALED.
!
!       NOW SOLVE TARGET VALUES.
!
!       ANGULAR TARGETS ARE NOT SCALED
!       DIMENSIONAL TARGETS ARE SCALED
!
      IF(SOLVE(6,I).EQ.1.0D0.OR.SOLVE(6,I).EQ.2.0D0.OR.SOLVE(6,I).EQ.3.0D0) THEN
         SOLVE(7,I)=SOLVE(7,I)*W1
      ELSE
      END IF
      IF(SOLVE(4,I).EQ.4.0D0.OR.SOLVE(4,I).EQ.5.0D0.OR.SOLVE(4,I).EQ.6.0D0) THEN
         SOLVE(3,I)=SOLVE(3,I)*W1
      ELSE
      END IF
!
!       ALL SOLVE SCALING IS COMPLETE
!
!       PIKUP SCALING
!       IN PIKUP SCALING, THE ONLY VALUE SCALED IS THE
!       ADDITIVE CONSTANT IF NUMERIC WORD 4 IS NOT 1.0
!       RD
      PIKUP(4,I,1)=PIKUP(4,I,1)*W1
!       CV
      PIKUP(4,I,2)=PIKUP(4,I,2)*W1
!       TH
      PIKUP(4,I,3)=PIKUP(4,I,3)*W1
!       THOAL (THE FIFTH TERM IS SCALED CAUSE THERE WERE 2 SURFACES ENTERED)
      PIKUP(5,I,32)=PIKUP(5,I,32)*W1
!       CC
      PIKUP(4,I,4)=PIKUP(4,I,4)*W1
!       AD
      PIKUP(4,I,5)=PIKUP(4,I,5)/(W1**3)
!       AE
      PIKUP(4,I,6)=PIKUP(4,I,6)/(W1**5)
!       AF
      PIKUP(4,I,7)=PIKUP(4,I,7)/(W1**7)
!       AG
      PIKUP(4,I,8)=PIKUP(4,I,8)/(W1**9)
!       CVTOR
      PIKUP(4,I,9)=PIKUP(4,I,9)*W1
!       RDTOR
      PIKUP(4,I,10)=PIKUP(4,I,10)*W1
!       YD
      PIKUP(4,I,13)=PIKUP(4,I,13)*W1
!       XD
      PIKUP(4,I,14)=PIKUP(4,I,14)*W1
!       ALPHA
      PIKUP(4,I,15)=PIKUP(4,I,15)*W1
!       BETA
      PIKUP(4,I,16)=PIKUP(4,I,16)*W1
!       GAMMA
      PIKUP(4,I,17)=PIKUP(4,I,17)*W1
!       CLAP
      PIKUP(4,I,18)=PIKUP(4,I,18)*M1
!       COBS
      PIKUP(4,I,19)=PIKUP(4,I,19)*M1
!       CCTOR
      PIKUP(4,I,20)=PIKUP(4,I,20)*W1
!       ADTOR
      PIKUP(4,I,22)=PIKUP(4,I,22)/(W1**3)
!       AETOR
      PIKUP(4,I,23)=PIKUP(4,I,23)/(W1**5)
!       AFTOR
      PIKUP(4,I,24)=PIKUP(4,I,24)/(W1**7)
!       AGTOR
      PIKUP(4,I,25)=PIKUP(4,I,25)/(W1**9)
!       AC
      PIKUP(4,I,26)=PIKUP(4,I,26)/(W1)
!       AH
      PIKUP(4,I,27)=PIKUP(4,I,27)/(W1**11)
!       AI
      PIKUP(4,I,28)=PIKUP(4,I,28)/(W1**13)
!       AJ
      PIKUP(4,I,29)=PIKUP(4,I,29)/(W1**15)
!       AK
      PIKUP(4,I,30)=PIKUP(4,I,30)/(W1**17)
!       AL
      PIKUP(4,I,31)=PIKUP(4,I,31)/(W1**19)
!
!               NOW ALL THE PIKUPS ARE SCALED
!       PRO,NPRO AND GLASS DON'T SCALE!
   END DO
!
!       NOW MAKE CHANGES TO THE CONFIG DATA
!       WHICH IS APPROPRIATE FOR THE PARTICULAR
!       SCALING REQUESTED.
   CALL CFSC2
!       ALL RESCALING WAS PERFORMED.
!       RESCALING AFFECTS SPECIAL SURFACE DATA
!       AS OF 1/6/94
   SCW1=W1
   CALL SCASPC(SCW1)
   F1=0
   F6=1
   F22=0
   LNSTYP=1
   CALL LNSEOS
   RETURN
END
