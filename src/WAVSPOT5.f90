...
!       CALLED BY EXFIT. IT CALCULATES OTHER RAY AND REFERENCE RAY
!       INTERSECTIONS WITH A TANGENT PLANE TO THE EXIT PUPIL
!       IT ALSO CALCULATES THE REFERENCE SURFACE RADIUS OF CURVATURE
!       THIS WAS DERIVED FROM LOPD.FOR ON 1/11/94
!
   DOUBLE PRECISION XREFI,XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,YREFI,RAD,AL,BE,GA,A,B,C,LEN,LEN1,LO,LOOX,LOOY,MO,MOOX,MOOY,LEN2,Q,ARG,ZREFI,SIGNB,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,V1,V2,V3,ZB,ZA,LLL,MMM,NNN,ZAR,ZBR,RADD,ZCR,V4,V5,V6,LOO,MOO,NOO
...
!       AT THE FINAL SURFACE (NEWOBJ). WITH THIS DEFINITION,
...
!       AND SOLVING FOR T.
!
      T=((RL0*(RX0-RRX0))+(RM0*(RY0-RRY0))+(RN0*(RZ0-RRZ0)))/((RL0*L0)+(RM0*M0)+(RN0*N0))
...
!       NOW X,Y AND Z ARE:
      ZA=RRX0+(T*L0)
      ZB=RRY0+(T*M0)
!     THE COORDINATES ON THE CHIEF RAY INTERSECTION WITH THE
!     EXIT PUPIL ARE:
      ZAR=RX0+(T*RL0)
      ZBR=RY0+(T*RM0)
!     DONE
...
!
!     CASE OF NOT THE REAL RAY EXIT PUPIL
!
...
!       THIS IS THE REFERENCE SPHERE CENTER LOCATION.
         ...
         IF(surf_thickness((NEWIMG-1)).EQ.0.0D0.OR.surf_thickness((NEWIMG-1)).NE.0.0D0.AND..NOT.LDIF2) THEN
!       DIST FROM I-1 TO I IS ZERO OR LDIF2 IS FALSE AND DIST NOT 0
!       DO AN INTERNAL "ASTOP EX" ADJUSTMENT
            IF(DABS(PXTRAY(5,NEWIMG)*1.0D-10).LT.DABS(PXTRAY(6,NEWIMG))) THEN
               RAD=(PXTRAY(5,NEWIMG)/PXTRAY(6,NEWIMG))
               RADD=DABS(RAD)
            ELSE
            END IF
            IF(DABS(PXTRAY(5,NEWIMG)*1.0D-10).GE.DABS(PXTRAY(6,NEWIMG))) THEN
...
!       USE EXISTING THICKNESS OF NEWIMG-1 AS RAD
            ...
            RADD=DABS(RAD)
         END IF
!     IF RAD IS POSITIVE, THE EXIT PUPIL LIES ON THE NEGATIVE
!     SIDE OF THE IMAGE PLANE. IF RAD IS NEGATIVE, THE EXIT PUPIL
!     LIES ON THE POSITIVE SIDE OF THE IMAGE PLANE.
!     NOW CALCULATE ZAR,ZBR AT THE PARAXIAL EXIT PUPIL
!     THE COORDINATES ON THE CHIEF RAY INTERSECTION WITH THE
!     EXIT PUPIL ARE:
         ZAR=RX0+(-RAD*RL0)
         ZBR=RY0+(-RAD*RM0)
!     THEN CALCULATE THE OTEHER RAY INTERSECTION WITH THE EXIT PUPIL
!     TANGENT PLANE
!
!       NOW X,Y AND Z ARE:
         ZA=RRX0+(-RAD*L0)
...
!
      IF(EXPAUT) THEN
!
!     CASE OF THE REAL RAY EXIT PUPIL
!
!       THE REFERENCE SPHERE IS CENTERED WHERE THE REAL CHIEF
!       RAY CROSSES THE FINAL SURFACE.
!       THE RADIUS IS EQUAL
...
!
         ...
         THXNUM=((XO-XOOX)*(LO-LOOX))+((YO-YOOX)*(MO-MOOX))+((ZO-ZOOX)*(NO-NOOX))
         THYNUM=((XO-XOOY)*(LO-LOOY))+((YO-YOOY)*(MO-MOOY))+((ZO-ZOOY)*(NO-NOOY))
         THXDEN=((LO-LOOX)**2)+((MO-MOOX)**2)+((NO-NOOX)**2)
         THYDEN=((LO-LOOY)**2)+((MO-MOOY)**2)+((NO-NOOY)**2)
         IF(DABS(THXNUM*1.0D-10).LT.DABS(THXDEN).AND.DABS(THYNUM*1.0D-10).LT.DABS(THYDEN)) THEN
            RAD=-((-THXNUM/THXDEN)+(-THYNUM/THYDEN))/2.0D0
!     THE EXIT PUPIL IS LOCATED AT:
!     IN THE COORDINATE SYSTEM OF THE IMAGE SURFACE
            RADD=DABS(RAD)
            ZAR=RX0+(-RAD*RL0)
            ZBR=RY0+(-RAD*RM0)
            ZA=RRX0+(-RAD*L0)
         ...
         ELSE
         END IF
         IF(DABS(THXNUM*1.0D-10).GE.DABS(THXDEN).OR.DABS(THYNUM*1.0D-10).GE.DABS(THYDEN)) THEN
!       REF RAY IS TELECENTRIC, SET RAD INFINITY
!       FLAT REFERENCE PLANE
!       THE SO-CALLED FLAT REFERENCE PLANE IS A PLANE
!       PERPENDICULAR TO THE REF RAY. THE INTERSECTION
!       OF THIS REFERENCE PLANE WITH THE REFERENCE RAY OCCURRS
!       AT THE FINAL SURFACE (NEWIMG).
            RADD=1.0D10
            RAD=1.0D10
            ZAR=RX0+(-RAD*RL0)
            ZBR=RX0+(-RAD*RM0)
...
!     NOT FOCAL
   ...
   RETURN
END
!       FIFTH FILE OF CAPFN/SPOT ROUTINES
...
SUBROUTINE APSTREHL
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) STREHLR,SUML2,SUML4,SUMSIG,WEI(1:10),WEIS ,SIG,LAM(1:10),STVALUE
   INTEGER I
   LOGICAL STERROR
   LOGICAL NOCOBSPSF
...
!
   IF(WC.EQ.'STREHL') THEN
      ...
      IF(SN.EQ.1.OR.SST.EQ.1) THEN
         ...
         & '"STREHL" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(SQ.EQ.1.AND.WQ.NE.'ACC'.AND.WC.EQ.'STREHL'.AND.WQ.NE.'NOOB'.AND.WQ.NE.'ACCNOOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ONLY "ACC", "NOOB" AND "ACCNOOB" CAN BE ISSUED'//'\n'//&
         & 'AS OPTIONAL QUALIFIERS'//'\n'//&
         & 'WITH "STREHL"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(.NOT.REFEXT) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO REFERENCE RAY DATA EXISTS'//'\n'//&
         & 'THE "STREHL" COMMAND CAN NOT BE EXECUTED', 1)
         RETURN
      END IF
      STERROR=.FALSE.
      STVALUE=0.0D0
      ...
      IF(STERROR) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'THE CURRENT "STREHL" CALCULATION FAILED'//'\n'//&
         & 'NO CAPFN OR PSF COULD BE COMPUTED', 1)
      END IF
      REG(40)=REG(9)
      REG(9)=STVALUE
      RSTREHL=STVALUE
...
9000  FORMAT('THE CURRENT EXACT STREHL RATIO = ',G15.8)
   ...
   IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1.AND.WC.EQ.'VAR') THEN
      CALL REPORT_ERROR_AND_FAIL('"VAR" TAKES NO ADDITIONAL INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      IF(WC.EQ.'APSTREHL')OUTLYNE='"APSTREHL" TAKES NO NUMERIC OR STRING INPUT'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1.AND.WQ.NE.'ACC'.AND.WC.EQ.'APSTREHL') THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'ONLY "ACC" CAN BE ISSUED AS AN OPTIONAL QUALIFIER'//'\n'//&
      & 'WITH "APSTREHL"'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.CPFNEXT) THEN
      IF(WC.EQ.'VAR')OUTLYNE='"VAR" REQUIRES AN EXISTING CAPFN'
      IF(WC.EQ.'APSTREHL')OUTLYNE='"APSTREHL" REQUIRES AN EXISTING CAPFN'
      CALL SHOWIT(1)
...
1000  FORMAT('THE CURRENT CAPFN WAVEFRONT VARIENCE = ',G15.8,' WAVES')
   ...
   IF(WC.EQ.'APSTREHL') THEN
      ...
      DO I=1,10
         ...
         END IF
      END DO
      IF(SUML2.NE.0.0D0.AND.SUML4.NE.0.0D0)LAMAVE=DSQRT(SUML2/SUML4)
      IF(LAMAVE.NE.0.0D0) THEN
!     CALC SUMSIG
         SUMSIG=0.0D0
         DO I=1,10
            IF(WEI(I).NE.0.0D0.AND.RMSOP(I).NE.0.0D0.AND.LAM(I).NE.0.0D0)SUMSIG=SUMSIG+((WEI(I)*(RMSOP(I)**2))/(LAM(I)**2))
         END DO
      END IF
      SIG=0.0D0
      IF(SUML2.NE.0.0D0.AND.SUMSIG.NE.0.0D0)SIG=DSQRT(SUMSIG/SUML2)
...
!       INITIATED BY THE TFDOTF COMMAND.
!
   INTEGER MINTF,MAXTF,I,ALLOERR,K
!
   real(real64) XDOLD,YDOLD,ZDOLD,FPOS,TFSPACE ,CUTFR,CUTFRX,CUTFRY,XMOD,YMOD,XPHA,YPHA,MMENT,AL,FREQ1,FREQ2
!
...
!     DETERMINE NEAR FOR FAR
   ...
   IF(NEAR.AND.SPACEBALL.EQ.1.AND.AL.GT.1.0D5) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO GOTF CAN BE PERFORMED WHEN SPACE IS SET TO "O"'//'\n'//&
      & 'WITH "NEAR" AND WITH AN OBJECT DISTANCE'//'\n'//&
      & 'GREATER THAN 1.0D5 MILLIMETERS.'//'\n'//&
      & 'NO ACTION TAKEN', 1)
      RETURN
   END IF
!
...
!
   IF(ERROR) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'ERROR IN OBJECT/IMAGE SPACE FREQUENCY RELATIONSHIP'//'\n'//&
      & 'CALCULATION OCCURED'//'\n'//&
      & 'NO ACTION TAKEN', 1)
      RETURN
   END IF
...
!
   ...
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TFDOTF" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S1.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TFDOTF" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   TFFREQ=DABS(W1)
   IF(DF2.EQ.1) THEN
      ...
      DF2=0
   END IF
   IF(W2.LT.16.0D0.OR.(W2-DBLE(INT(W2))).NE.0.0D0.OR.((W2/2.0D0)-DBLE(INT(W2/2.0D0))).NE.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NUMERIC WORD #2 IS THE RAY GRID SIZE FOR THE CAPFN'//'\n'//&
      & 'CALCULATION. IT MUST BE A POSITIVE'//'\n'//&
      & 'EVEN INTEGER VALUE GREATER THAN OR EQUAL TO 16'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   TFSPACE=W2
!
   IF(.NOT.REFEXT) THEN
      CALL REPORT_ERROR_AND_FAIL('NO CHIEF RAY EXISTS, NO "TFDOTF" MAY BE PERFORMED', 1)
      RETURN
   END IF
!     SAVE THE LAST FOB DATA FOR RE-USE
   TFFOB(1)=LFOB(1)
   TFFOB(2)=LFOB(2)
   TFFOB(3)=LFOB(3)
   TFFOB(4)=LFOB(4)
   ...
   TFFOB(6)=LFOB(6)
   TFFOB(7)=LFOB(7)
!     NOW SET UP THE LOOP AND ALLOCATE STORAGE ARRAYS FOR THE THRU-FOCUS
!     CALCULATION
   MAXTF=NINT(DABS((TFTMAX-TFTMIN)/TFDELT))
   MINTF=0
!     GET XD,YD OR ZD FOR THE FOCUS SURFACE AND REMEMBER IT
   XDOLD=surf_focus_dx(TFSURF)
   YDOLD=surf_focus_dy(TFSURF)
   ZDOLD=surf_focus_dz(TFSURF)
   WRITE(OUTLYNE,300)
   CALL SHOWIT(0)
...
!     UPDATE THE LENS
      ...
      F22=0
      LNSTYP=1
      IF(TFDIRECTION.EQ.0)call set_surf_decenter_x(TFSURF, XDOLD+FPOS)
      IF(TFDIRECTION.EQ.1)call set_surf_decenter_y(TFSURF, YDOLD+FPOS)
      IF(TFDIRECTION.EQ.2)call set_surf_decenter_z(TFSURF, ZDOLD+FPOS)
      IF(TFDIRECTION.EQ.0)call set_surf_focus_dx(TFSURF, XDOLD+FPOS)
      IF(TFDIRECTION.EQ.1)call set_surf_focus_dy(TFSURF, YDOLD+FPOS)
      IF(TFDIRECTION.EQ.2)call set_surf_focus_dz(TFSURF, ZDOLD+FPOS)
      IF(TFDIRECTION.EQ.0) MMENT=surf_focus_dx(TFSURF)
      IF(TFDIRECTION.EQ.1) MMENT=surf_focus_dy(TFSURF)
      IF(TFDIRECTION.EQ.2) MMENT=surf_focus_dz(TFSURF)
      CALL LNSEOS
!     NOW TRACE THE CHIEF RAY
      SAVE_KDP(1)=SAVEINPT(1)
      WC='FOB     '
      SQ=0
      SST=0
      ...
      IF(.NOT.REFEXT) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'CHIEF RAY COULD NOT BE TRACED'//'\n'//&
         & '"TFDOTF" ANALYSIS TERMINATED', 1)
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     NOW DO THE CAPFN
      ...
      IF(.NOT.CPFNEXT) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'COMPLEX APERTURE FUNCTION COULD NOT BE GENERATED'//'\n'//&
         & '"TFDOTF" ANALYSIS TERMINATED', 1)
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
...
!     NOW DO THE X AND Y DOTFS AT THE SPECIFIED FREQUENCY
      SAVE_KDP(1)=SAVEINPT(1)
      WC='DOTF    '
      WQ='XACC    '
      SQ=1
...
200   FORMAT('   X-MOTION  ',2X,' XZ-MTF(MOD) ',2X,'XZ-MTF(PHASE)',2X,' YZ-MTF(MOD) ',2X,'YZ-MTF(PHASE)')
201   FORMAT('   Y-MOTION  ',2X,' XZ-MTF(MOD) ',2X,'XZ-MTF(PHASE)',2X,' YZ-MTF(MOD) ',2X,'YZ-MTF(PHASE)')
202   FORMAT('   Z-MOTION  ',2X,' XZ-MTF(MOD) ',2X,'XZ-MTF(PHASE)',2X,' YZ-MTF(MOD) ',2X,'YZ-MTF(PHASE)')
!     DO THE NEXT FOCUS POSITION
   END DO
   call set_surf_focus_dx(TFSURF, XDOLD)
   call set_surf_focus_dy(TFSURF, YDOLD)
   call set_surf_focus_dz(TFSURF, ZDOLD)
   call set_surf_decenter_x(TFSURF, XDOLD)
   call set_surf_decenter_y(TFSURF, YDOLD)
   call set_surf_decenter_z(TFSURF, ZDOLD)
   F1=0
   F6=1
   F22=1
   LNSTYP=1
...
!     CALLED BY CMDER FOR COMMAND SPACE
...
!
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPACE" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPACE" REQUIRES EXPLICIT QUALIFIER INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'I'.AND.WQ.NE.'O') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"SPACE" QUALIFIER MUST BE "O" OR "I"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     CALLED BY CMDER FOR COMMAND CUTOFF
!     THIS CALCULATES CUTOFF FREQ
!
   real(real64) EXPUZMM ,REFIN,NREFIN,CUTFRX,CUTFRY,VALVAL,GRNX,GRNY
!
   LOGICAL ERRR
!
   INTEGER SHTNM
...
!
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CUTOFF" TAKES NO NUMERIC OR STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CUTOFF" REQUIRES EXPLICIT QUALIFIER INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'I'.AND.WQ.NE.'O'.AND.WQ.NE.'IACC'.AND.WQ.NE.'OACC') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"CUTOFF" QUALIFIER MUST BE "O", "I", "IACC" OR "OACC"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
!                       PROCEED
      END IF
...
314 CONTINUE
   IF(SHRTWAVE.EQ.0.0D0) THEN
      ...
      & 'NO CUTOFF FREQUENCY CAN BE DEFINED', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'I'.AND.WQ.NE.'O'.AND.WQ.NE.'IACC'.AND.WQ.NE.'OACC') THEN
...
!
   ...
   END IF
!
!     TRY TO TRACE A GUT RAY IF ONE DOES NOT EXITS
   IF(.NOT.REFEXT) THEN
      SAVE_KDP(1)=SAVEINPT(1)
      WC='FOB     '
      SQ=0
...
!
         ...
         REG(9)=CUTFRY
         IF(WQ.EQ.'I') THEN
10          FORMAT('XZ-PLANE IMAGE SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MRAD')
11          FORMAT('YZ-PLANE IMAGE SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MRAD')
...
!
      ELSE
!     FOCAL
!     GET THE X AND Y FNUM MULT BY THE SHORTEST WAVELENGTH
!     AND INVERT
!
         GRNX=RBFNX
         GRNY=RBFNY
...
!
         ...
         REG(10)=CUTFRX
         REG(9)=CUTFRY
20       FORMAT('XZ-PLANE IMAGE SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MM')
21       FORMAT('YZ-PLANE IMAGE SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MM')
         IF(WQ(2:4).NE.'ACC') THEN
...
!     DO THE CALCULATION FOR OBJECT SPACE
      IF(NEAR) THEN
!
         GRNX=RFFNX
         GRNY=RFFNY
...
!
         ...
         REG(10)=CUTFRX
         REG(9)=CUTFRY
30       FORMAT('XZ-PLANE OBJECT SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MM')
31       FORMAT('YZ-PLANE OBJECT SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MM')
         IF(WQ(2:4).NE.'ACC') THEN
...
!
         ...
         REG(10)=CUTFRX
         REG(9)=CUTFRY
40       FORMAT('XZ-PLANE OBJECT SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MRAD')
41       FORMAT('YZ-PLANE OBJECT SPACE CUTOFF FREQUENCY = ',G13.6,' LP/MRAD')
         IF(WQ(2:4).NE.'ACC') THEN
...
!
   ...
   IMPLICIT NONE
!
   real(real64) EXPUZMM,FREQ1,FREQ2 ,REFIN,NREFIN,CUTFRX,CUTFRY,VALVAL,GRNX,GRNY
!
   LOGICAL ERRR,ERROR
!
   INTEGER SHTNM,TYPE
...
!
   CHARACTER UN*11
!
   real(real64) SPT,SPT1,SPT2,SPT3,SPT4,IWLIJK(1:10),WAY,SPT5,SPT6,SPT7,SPT8,SPT9,SPT10,LAMFACTOR ,OPPEAK(0:10),OPPIT(0:10),OPPTV(0:10),SUMONE(0:10),SUMTWO(0:10),ARG,LAM(1:10),WEI(1:10),SUML2,SUML4 ,WEIS,SIG,SUMSIG,DELFOB,APODX2,APODR2
!
   real(real64) NCOS,A,DSPO
!
   DIMENSION DSPO(:,:)
!
...
!
   ...
   COMMON/PSFCOBS/NOCOBSPSF
!
   COMMON/SPTWTS/SPT1,SPT2,SPT3,SPT4,SPT5 ,SPT6,SPT7,SPT8,SPT9,SPT10
!
...
!
!       THIS IS COMPAP.FOR.
!       CREATES THE COMPLEX APERTURE FUNCTION FROM THE SPOT DIAGRAM
!       THERE MUST BE AN n**2 X n**2 GRID OF RAYS IN THE SPOT DIAGRAM
!       FO THIS TO WORK PROPERLY
!
...
!                           MIN VALUE = 8
!
!     ONLY VAULES WHICH ARE POWERS OF 2 ARE ALLOWED
!
   INTEGER KIK,KIKI
!
   real(real64) TOT,W,AMSS ,X,Y,MSSX,MSSY,AMSSX,AMSSY
!
   real(real64) MAX ,FRAC,FRACA,FRACB,MSS,APFAC
!
   INTEGER I,J,IWL,NUMT1,NUMT2,NUMT3,NUMT4,NUMT5,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10,II,JJ,K ,NSTART,NSTOP,IX,IY,NUMT0,TEMPHOLDER
!
...
!     SPHERE
!
   real(real64) XSPH,YSPH,ZSPH,RADSPH,ADJUSTW1,ADJUSTW2
!
...
!
!     TPT=1 FOR REGULAR CAPFN , 2 FOR TOL AND OPT
!
!     TEST VALUES FOR OPD LIMITS
   OPPEAK(0)=-1.0E20
   OPPEAK(1)=-1.0E20
   OPPEAK(2)=-1.0E20
   OPPEAK(3)=-1.0E20
...
!       DEFAULT NUMERIC INPUT, ALL DEFAULTS ARE ZERO
      ...
      END IF
   END IF
   IF(W1.LE.0.0D0.AND.TPT.EQ.1.AND.WC.NE.'PSFK'.AND.WC.NE.'PUPIL') THEN
      OUTLYNE='"CAPFN" REQUIERS SOME POSITIVE NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
      ...
      CALL MACFAL
      RETURN
!       NOT ALL ZERO INPUT , PROCEED
   END IF
   IF(WC.EQ.'CAPFN'.AND.TPT.EQ.1) THEN
      IF(W1.LT.16.0D0.OR.W1.GT.8192.0D0.OR.(W1-DBLE(INT(W1))).NE.0.0D0.OR.((W1/2.0D0)-DBLE(INT(W1/2.0D0))).NE.0.0D0) THEN
         IF(WC.EQ.'CAPFN') OUTLYNE='"CAPFN" REQUIERS NUMERIC WORD #1 TO BE A POSITIVE'
         CALL SHOWIT(1)
         OUTLYNE='EVEN INTEGER VALUE FROM 16 TO 8192'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!       NOT ALL ZERO INPUT , PROCEED
   ...
   IF(TPT.EQ.1.AND.WC.EQ.'CAPFN') THEN
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1)THEN
         call printMSG('"CAPFN" TAKES NO', 1)
         call printMSG('NUMERIC WORD #2 THROUGH #5 INPUT', 1)
         call printMSG('RE-ENTER COMMAND', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
         CALL MACFAL
         RETURN
!       NOT ALL ZERO INPUT , PROCEED
      END IF
   END IF
   IF(TPT.EQ.1.AND.WC.EQ.'PSFK') THEN
      IF(S4.EQ.1.OR.S5.EQ.1)THEN
         OUTLYNE='"PSF" TAKES NO NUMERIC WORD #4 TO #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!       NOT ALL ZERO INPUT , PROCEED
   ...
   IF(TPT.EQ.1.AND.WC.EQ.'PUPIL') THEN
      IF(S5.EQ.1)THEN
         OUTLYNE='"PUPIL" TAKES NO NUMERIC WORD #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!     FIND THE SHORTEST WAVELENGTH
      SHORT=0
!     FIND THE TOTAL NUMBER OF NON-ZERO WEIGHT WAVELENGTHS
      NUMCOL=0
      WVSHORT=1.0D10
      DO IWL=1,10
!
...
!
         ...
         END IF
      END DO
!     SHORT HAS THE WAVELENGTH NUMBER OF THE SHORTEST WAVELENGTH
!     WVSHORT IS THE SHORTEST WAVELENGTH IN MICRONS
!
   END IF
   LSF=.FALSE.
!
...
!     DOES THE NEWREF SURFACE HAVE A CLAP ON IT ?
   TCLPRF=.FALSE.
   IF(surf_clap_type(NEWREF).EQ.0.0D0.OR.surf_multi_clap_flag(NEWREF).NE.0.0D0) THEN
!     ASSIGN A TEMPORARY CIRCULAR CLAP
      ...
      IF(TPT.EQ.1) THEN
         ...
         IF(MSGSPD)WRITE(OUTLYNE,*)'HAS BEEN ASSIGNED TO THE REFERENCE SURFACE'
         IF(MSGSPD)CALL SHOWIT(0)
         IF(WC.EQ.'CAPFN') THEN
...
301   FORMAT('TEMPORARY CIRCULAR CLEAR APERTURE = ',G11.4)
   ELSE
      IF(surf_clap_type(NEWREF).EQ.2.0D0) THEN
         IF(surf_clap_dim(NEWREF, 2).GT.surf_clap_dim(NEWREF, 1)) THEN
            ADJUSTW1=surf_clap_dim(NEWREF, 2)/surf_clap_dim(NEWREF, 1)
            ADJUSTW2=1.0D0
         ELSE
            ADJUSTW1=1.0D0
            ADJUSTW2=surf_clap_dim(NEWREF, 1)/surf_clap_dim(NEWREF, 2)
         END IF
      END IF
      IF(surf_clap_type(NEWREF).EQ.3.0D0) THEN
         IF(surf_clap_dim(NEWREF, 2).GT.surf_clap_dim(NEWREF, 1)) THEN
            ADJUSTW1=surf_clap_dim(NEWREF, 2)/surf_clap_dim(NEWREF, 1)
            ADJUSTW2=1.0D0
         ELSE
         ...
         END IF
      END IF
      IF(surf_clap_type(NEWREF).EQ.4.0D0) THEN
         IF(surf_clap_dim(NEWREF, 2).GT.surf_clap_dim(NEWREF, 1)) THEN
            ADJUSTW1=surf_clap_dim(NEWREF, 2)/surf_clap_dim(NEWREF, 1)
            ADJUSTW2=1.0D0
         ELSE
         ...
         END IF
      END IF
      IF(surf_clap_type(NEWREF).EQ.5.0D0) THEN
         ADJUSTW1=1.0D0
         ADJUSTW2=1.0D0
      END IF
      IF(surf_clap_type(NEWREF).EQ.6.0D0) THEN
         ADJUSTW1=1.0D0
         ADJUSTW2=1.0D0
      END IF
   END IF
...
!     REMOVE TEMP CLAP ON NEWREF
      TCLPRF=.FALSE.
      call set_surf_clap_type(NEWREF, 0)
      call set_surf_clap_dim(NEWREF, 1, 0.0D0)
      call set_surf_clap_dim(NEWREF, 2, 0.0D0)
      call set_surf_clap_dim(NEWREF, 3, 0.0D0)
      call set_surf_clap_dim(NEWREF, 4, 0.0D0)
      call set_surf_clap_dim(NEWREF, 5, 0.0D0)
      call set_surf_clap_tilt(NEWREF, 0.0D0)
      IF(TPT.EQ.1) THEN
         ...
         IF(MSGSPD)WRITE(OUTLYNE,301) PXTRAY(1,NEWREF)
         IF(MSGSPD)CALL SHOWIT(0)
         IF(MSGSPD)WRITE(OUTLYNE,*)'HAS BEEN REMOVED FROM THE REFERENCE SURFACE'
         IF(MSGSPD)CALL SHOWIT(0)
         IF(WC.EQ.'CAPFN') THEN
            IF(MSGSPD)WRITE(OUTLYNE,*)'FOR THE CURRENT CAPFN CALCULATIONS'
         END IF
...
!
!     NOW DO PART 2 OF THE OPD CALCULATIONS
!     THIS TAKES CARE OF ENTRANCE AND EXIT REFERENCE SPHERES
   RMSOPD=0.0D0
   RMSOP(0:10)=0.0D0
   SUMONE(0:10)=0.0D0
   SUMTWO(0:10)=0.0D0
...
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
         ...
         IF(PERFECT) OOPD=0.0D0
         CURLAM=DSPOT(16)
!     NOW FINISH THE OPD CALCULATION
         ...
         IF(REFERR) THEN
            ...
            RETURN
         END IF
!     HERE WE MULTIPLY IN 2PI FOR THE LATER PUPIL FUNCTION
...
!
      IF(DSPOT(12).NE.0.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(0))OPPEAK(0)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(0))OPPIT(0)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.1.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(1))OPPEAK(1)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(1))OPPIT(1)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.2.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(2))OPPEAK(2)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(2))OPPIT(2)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.3.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(3))OPPEAK(3)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(3))OPPIT(3)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.4.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(4))OPPEAK(4)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(4))OPPIT(4)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.5.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(5))OPPEAK(5)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(5))OPPIT(5)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.6.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(6))OPPEAK(6)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(6))OPPIT(6)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.7.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(7))OPPEAK(7)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(7))OPPIT(7)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.8.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(8))OPPEAK(8)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(8))OPPIT(8)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.9.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(9))OPPEAK(9)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(9))OPPIT(9)=DSPOT(4)/(TWOPII)
      END IF
!
      IF(DSPOT(12).NE.0.0D0.AND.DSPOT(16).EQ.10.0D0) THEN
         IF((DSPOT(4)/(TWOPII)).GE.OPPEAK(10))OPPEAK(10)=DSPOT(4)/(TWOPII)
         IF((DSPOT(4)/(TWOPII)).LE.OPPIT(10))OPPIT(10)=DSPOT(4)/(TWOPII)
      END IF
   END DO
!
!       THE NUMBER OF RAYS WHICH REPRESENT 100% OF THE ENERGY
!       EQUAL THE NUMBER OF RAYS TRACED (IWIW) TIMES THE NUNBER
!       OF NON-ZERO SPECTRAL WEIGHTS.
!
   IF(NUMT0.NE.0.0D0) THEN
      ARG=((SUMTWO(0)-((SUMONE(0)**2)/DBLE(NUMT0)))/DBLE(NUMT0-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
         RETURN
      ELSE
   ...
   IF(NUMT1.NE.0.0D0) THEN
      ARG=((SUMTWO(1)-((SUMONE(1)**2)/DBLE(NUMT1)))/DBLE(NUMT1-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT2.NE.0.0D0) THEN
      ARG=((SUMTWO(2)-((SUMONE(2)**2)/DBLE(NUMT2)))/DBLE(NUMT2-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT3.NE.0.0D0) THEN
      ARG=((SUMTWO(3)-((SUMONE(3)**2)/DBLE(NUMT3)))/DBLE(NUMT3-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT4.NE.0.0D0) THEN
      ARG=((SUMTWO(4)-((SUMONE(4)**2)/DBLE(NUMT4)))/DBLE(NUMT4-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT5.NE.0.0D0) THEN
      ARG=((SUMTWO(5)-((SUMONE(5)**2)/DBLE(NUMT5)))/DBLE(NUMT5-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT6.NE.0.0D0) THEN
      ARG=((SUMTWO(6)-((SUMONE(6)**2)/DBLE(NUMT6)))/DBLE(NUMT6-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT7.NE.0.0D0) THEN
      ARG=((SUMTWO(7)-((SUMONE(7)**2)/DBLE(NUMT7)))/DBLE(NUMT7-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT8.NE.0.0D0) THEN
      ARG=((SUMTWO(8)-((SUMONE(8)**2)/DBLE(NUMT8)))/DBLE(NUMT8-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT9.NE.0.0D0) THEN
      ARG=((SUMTWO(9)-((SUMONE(9)**2)/DBLE(NUMT9)))/DBLE(NUMT9-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
   ...
   IF(NUMT10.NE.0.0D0) THEN
      ARG=((SUMTWO(10)-((SUMONE(10)**2)/DBLE(NUMT10)))/DBLE(NUMT10-1))
      IF(ARG.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL('RMSOPD ERROR OCCURED, CAPFN CALCULATION HAS FAILED', 1)
!     DEALLOCATE DSPOTT
         CALL SPOTIT(1)
      ...
      END IF
   END IF
!     NOW THE POLY CHROMATIC RMSOPD
   IF(LAMAVE.NE.0.0D0) THEN
!     CALC SUMSIG
      SUMSIG=0.0D0
      DO ISS=1,10
         IF(WEI(ISS).NE.0.0D0.AND.RMSOP(ISS).NE.0.0D0.AND.LAM(ISS).NE.0.0D0)SUMSIG=SUMSIG+((WEI(ISS)*(RMSOP(ISS)**2))/(LAM(ISS)**2))
      END DO
   END IF
   SIG=0.0D0
   IF(SUML2.NE.0.0D0.AND.SUMSIG.NE.0.0D0)RMSOPD=DSQRT(SUMSIG/SUML2)
   RMSOP(0)=RMSOPD
!
!
...
!     CALL SHOWIT(1)
!     IF(WC.EQ.'CAPFN')
!    1OUTLYNE='NO CAPFN WAS CREATED'
!     IF(WC.EQ.'PSFK')
!    1OUTLYNE='NO PSF WAS CREATED'
!     IF(WC.EQ.'PUPIL')
!    1OUTLYNE='NO PUPIL WAS CREATED'
!     CALL SHOWIT(1)
!              END IF
!                       CALL MACFAL
!                      CPFNEXT=.FALSE.
...
!                       RETURN
!                       END IF
!     PUT THE RMSOPD IN THE ACCUMULATOR
   REG(40)=REG(9)
   REG(9)=RMSOPD
...
!     WE HAVE PEAK TO VALLEY OPD (OPPTV) IN WAVES
!     IF WE HAVE NORE THAN LAMBDA/2 WAVES PER GRID IN THE NRD
!     THEN THE CAPFN IS FOR SHIT AND WE BETTER NOT CONTINUE
   ...
   IF(DABS(OPPTV(0)/(DBLE(NRD))).GE.0.25D0) THEN
      IF(TPT.EQ.1) THEN
         ...
         ELSE
            ...
            CALL SHOWIT(1)
            OUTLYNE='USE CAUTION INTERPRETING DIFFRACTION BASED RESULTS.'
            CALL SHOWIT(1)
         END IF
      END IF
   ELSE
...
!
!     IT IS NOW 4/20/93
!     DSPOTT CONTAINS THE X AND Y COORDINATES
!     OF EACH RAY AT THE REFERENCE SURFACE IN DSPOT(5) AND DSPOT(6)
!     THE RAY ENERGY TERM FOR THE RAY, ADJUSTED FOR PUPIL ABERRATIONS
!     AND SOLID ANGLE SUBTENT AS SEEN FROM THE CENTER OF THE REFERENCE
!     SPHERE IS STORED IN DSPOT(12)
!     THE 2pi*OPD OF THE RAY IS STORED IN DSPOT(4). IT IS REPRESENTED IN
!     WAVES AT WAVELENGTH (WAV) WHERE WAV IS EQUAL TO THE WAVELENGTH
!     AT WHICH THE LAST CHIEF RAY WAS TRACED. ITS UNIT ARE RADIANS
!
...
!     CALLED BY CMDER FOR COMMAND DOTF
!     THIS DOES DIFFRACTION OTF CALCULATIONS
!
   INTEGER DORI1,DORI2,N,I,J,IX,IQ,IY,SHTNM,K,L,M,II,IG,IIG ,SSSQS,IJ,M4,ALLOERR,IJK,SS1,SS2,SSN,DDFF1,DDFF2
   COMMON/GII/IIG
!
...
!
   CHARACTER*8 WWWQW
!
   real(real64) SUM1,SUM2,CUTFRX,CUTFRY,JK_TEMP,COFACT ,VALVAL,GRNX,GRNY,WVNUM,FREQFACT,X,Y,Y1,Y2,nor,SHRTWV ,IWLIJK(1:10),REFIN,NREFIN,VXHI,VXLO,VYHI,VYLO,FNXX,FNYY
!
   LOGICAL REFERR,EXTDMTF1,EXTDMTF2,PERF
   COMMON/DMTFEXT/EXTDMTF1,EXTDMTF2,PERF
!
   COMMON/PEPITO/IWLIJK,IJK
!
   real(real64) YP1,YPN,CUTFR,SPTT ,NORMI,DOTFR1,DOTFI1,DOTFM1,DOTFP1,XFR ,DOTFM0,DOTFP0,DOTFI0,DOTFR0,XFR0
!
   REAL DDTAF(1:101),DDTAM(1:101,0:21),DDTAP(1:101,0:21)
!
   COMMON/DOTFPAS/DDTAF,DDTAM,DDTAP,N,DORI1,CUTFRX,CUTFRY ,DORI2
!
!
   real(real64) PUPL,SUMMER1,SUMMER2,FREQ,XY12,XY22,FREQ1,FREQ2,AL,WWWW1,WWWW2,DOTFR,DOTFI,DOTFP,DOTFM,FR
   DIMENSION PUPL(:,:,:),SUMMER1(:),SUMMER2(:),FREQ(:),XY12(:),XY22(:),DOTFI(:),DOTFM(:),DOTFR(:),DOTFP(:),FR(:)
   ALLOCATABLE :: PUPL,SUMMER1,SUMMER2,FREQ,XY12,XY22,DOTFI,DOTFR ,DOTFP,DOTFM,FR
!
...
!     DETERMINE NEAR FOR FAR
   IF(NEAR_FAR.EQ.0) NEAR=.TRUE.
   IF(NEAR_FAR.EQ.1) NEAR=.FALSE.
   IF(NEAR.AND.SPACEBALL.EQ.1.AND.AL.GE.1.0D5) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO DOTF CAN BE PERFORMED WHEN SPACE IS SET TO "O"'//'\n'//&
      & 'WITH "NEAR" AND WITH AN OBJECT DISTANCE'//'\n'//&
      & 'GREATER THAN 1.0D5 MILLIMETERS.'//'\n'//&
      & 'NO ACTION TAKEN', 1)
      RETURN
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"DOTF" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'Y'.AND.WQ.NE.'X'.AND.WQ.NE.'YACC'.AND.WQ.NE.'XACC') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"DOTF" QUALIFIER MUST BE "Y", "X", "YACC" OR "XACC"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
!
   IF(WQ.EQ.'ACC'.AND.DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"DOTF ACC" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WQ.EQ.'XACC'.AND.DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"DOTF XACC" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WQ.EQ.'YACC'.AND.DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"DOTF YACC" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
...
!
   IF(.NOT.CPFNEXT) THEN
      ...
      IF(.NOT.CPFNEXT.OR.REFERR) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"NO COMPLEX APERTURE FUNCTION COULD NOT BE COMPUTED"'//'\n'//&
         & '"DOTF" NOT POSSIBLE WITH EXISTING SYSTEM', 1)
         RETURN
      END IF
      w1=wwww1
      w2=wwww2
...
!
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"DOTF" TAKES NO NUMERIC WORD #3, #4 OR #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   ...
   IF(W2.LT.3.0D0.OR.W2.GT.100.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'VALID NUMBER OF FREQUENCY POINTS RANGES FROM 3 TO 100'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   N=INT(W2)
   IF(DF1.EQ.0) THEN
      IF(W1.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'FREQUENCY MUST BE GREATER THAN 0.0D0'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
314 CONTINUE
   IF(SHRTWAVE.EQ.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('WAVELENGTHS ARE ALL ZERO'//'\n'//'NO "DOTF" CAN BE CALCULATED', 1)
      RETURN
   END IF
!
   IF(sys_wavelength(1).LE.SHRTWAVE .AND.sys_wavelength(1).NE.0.0D0.AND.sys_wl_weight(1).NE.0.0D0) THEN
...
!
   ...
   END IF
!
   DEALLOCATE (PUPL,SUMMER1,SUMMER2,FREQ ,XY12,XY22,STAT=ALLOERR)
   ALLOCATE (PUPL(IW,IW,2),SUMMER1(IW+1),SUMMER2(IW+1),FREQ(IW+1),XY12(IW+1),XY22(IW+1),STAT=ALLOERR)
   IWIW=IW**2
!     IF NO QUAL AND W1 INPUT, SET WQ TO 'Y'
...
!
   DO IG=1,IIG
!     BEFORE WE START AN IG LOOKM ZERO OUT THE VARIABLES WHICH HOLD THE
!     MODULUS AND PHASE TRANSFER FUNCTIONS
      DEALLOCATE(DOTFR,DOTFI,FR,DOTFM,DOTFP,FREQ,XY12,XY22,STAT=ALLOERR)
      ALLOCATE(DOTFR(N+1),DOTFI(N+1),FR(N+1),DOTFM(N+1),DOTFP(N+1),XY12(IW+1),XY22(IW+1),FREQ(IW+1),STAT=ALLOERR)
      IJ=IW+1
      FREQ(1:IJ)=0.0D0
      XY12(1:IJ)=0.0D0
      XY22(1:IJ)=0.0D0
...
!
      ...
      DO J=1,NUMCOL
!     DOING A COLOR NOW, ZERO OUT THE SUMMER ARRAYS
!     BEFORE WE DO A COLOR, ZERO THE SUMMER1 AND SUMMER2 ARRAYS
         IJ=IW+1
         SUMMER1(1:IJ)=0.0D0
         SUMMER2(1:IJ)=0.0D0
         FREQ(1:IJ)=0.0D0
!     ARRAYS FOR A COLOR ZEROED
         IX=0
         IY=1
         II=0

!     WE WANT TO DO IWIW READS AND LOADS OF THE PUPL ARRAY
10       II=II+1
         IX=IX+1
         IF(IX.GT.IW) THEN
            IY=IY+1
            IX=1
         END IF
!     INSTEAD OF READING FROM UNIT 61 FROM RECORD I
!     TRANSFER DATA FROM DSPOTT(*,ID) WITH ID SET TO I-1
         ID=I-1
         CALL SPOTIT(4)
         SPTT=DSPOT(17)
         I=I+1
...
!     FOR A COLOR
!     PUPL IS COL,ROW INDEXED
!     THIS IS THE REAL PART
         PUPL(IX,IY,1)=DSQRT(DABS(DSPOT(12)))
!
!     THIS IS THE IMAGINARY PART 2*PI*OPD IS FRACTIONS OF A WAVE
!     AT THE REFERENCE WAVELENGTH (UNITS ARE RADIANS)
         IF(PERFECT) DSPOT(4)=0.0D0
         PUPL(IX,IY,2)=DSPOT(4)
!     THE PARTIAL COHERENCE FACTOR IS COHERENCE_FACTOR*PII
!     WHERE THE COHERENCE FACTOR CAN RANGE FROM 0=INCOHERENT
!     TO 1=COHERENT
         WVNUM=DSPOT(16)
         IF(II.LT.IWIW) GO TO 10
!     FELL THROUGH, FINISHED READING IWIW POINTS
!     THE ARRAYS FOR A COLOR ARE WRITTEN, NOW SHEAR THEM
!     IN Y IF 'Y' WAS THE QUALIFIER OR IN X IF 'X' WAS THE QUALIFIER
!     'Y' MEANS (Y SHEAR) Y-RESPONSE (HORIZONTAL BARS)
!     'X' MEANS (X SHEAR) X-RESPONSE (VERTICAL BARS)
!     OUTPUT THE ROTF AND IOTF AS A FUNCTION OF FRACTIONAL
!     CUTOFF FREQUENCY
!     K COUNTS THE SHIFTS. THE SHIFTS START WITH 0 AND END WITH IW-1
!     THE IWTH SHIFT ALWAYS YIELDS A ZERO TERM
!
         DO K=0,IW-1
!     SUM1 AND SUM2 ARE RE-INITIALIZED TO ZERO FOR EACH NEW K
            SUM1=0.0D0
            SUM2=0.0D0
!
            IF(WQ.EQ.'X'.OR.WQ.EQ.'XACC') THEN
!     X-RESPONSE (VERTICAL BARS)
!
               DO L=(1+K),IW
                  DO M=1,IW
                     SUM1=SUM1 +((PUPL(L,M,1)*PUPL(L-K,M,1))*DCOS(-PUPL(L-K,M,2)+PUPL(L,M,2)))
                     SUM2=SUM2 +((PUPL(L,M,1)*PUPL(L-K,M,1))*DSIN(-PUPL(L-K,M,2)+PUPL(L,M,2)))
                  END DO
               END DO
            END IF
            IF(WQ.EQ.'Y'.OR.WQ.EQ.'YACC') THEN
!     Y-RESPONSE (HORIZONTAL BARS)


               DO L=(1+K),IW
                  DO M=1,IW
                     SUM1=SUM1 +((PUPL(M,L,1)*PUPL(M,L-K,1))*DCOS(-PUPL(M,L-K,2)+PUPL(M,L,2)))
                     SUM2=SUM2 +((PUPL(M,L,1)*PUPL(M,L-K,1))*DSIN(-PUPL(M,L-K,2)+PUPL(M,L,2)))
                  END DO
               END DO
            END IF
...
!     PARTS OF THE OTF
            SUMMER1(K+1)=SUM1
            SUMMER2(K+1)=SUM2
            IF(WQ(1:1).EQ.'X')FREQ(K+1)=DABS((DBLE(K)*CUTFRX*FREQFACT)/DBLE(IW))
            IF(WQ(1:1).EQ.'Y')FREQ(K+1)=DABS((DBLE(K)*CUTFRY*FREQFACT)/DBLE(IW))
         END DO
         IF(WQ(1:1).EQ.'X')FREQ(IW+1)=CUTFRX*FREQFACT
         IF(WQ(1:1).EQ.'Y')FREQ(IW+1)=CUTFRY*FREQFACT
         SUMMER1(IW+1)=0.0D0
         SUMMER2(IW+1)=0.0D0
!
!     ALL DATA CALCULATED FOR A WAVELENGTH AND STORED
!     NOW SPLINE FIT THIS DATA, USE THE FIT TO GENERATE "IW" POINTS,
!     MULTIPLY THESE "IW" POINTS BY THE SPECTRAL WEIGHT AND SAVE
!     THE VALUES IN THE DOTFR AND DOTFI ARRAYS
!     AT N POINTS AND SAVE IT
!
...
!
            DO IJ=0,N
!
!     CALCULATE X, THE INPUT FREQUENCY
!     N REPRESENTS THE CUTOFF FREQ
!
...
!     JUST DO A SINGLE MTF AT FREQUENCY W1
!
            XFR0=0.0D0
            XFR=W1
!
...
!
         ...
         END IF
!                     END OF THE COLOR LOOP
      END DO
!
!     DONE WITH ALL COLORS NOW
!
      IF(DF1.EQ.1) THEN
...
!     INTERPOLATE INTO THE DOTFM/P ARRAYS
         YP1=1.0D35
         YPN=1.0D35
!
         DO IJ=0,N
...
!     N REPRESENTS THE CUTOFF FREQ
!
!     OUTPUT THE DOTFM AND P AND FR ARRAYS AT N+1 POINTS
            ...
            IF(DOTFR(IJ+1).NE.0.0D0.AND.DABS(DOTFI(IJ+1)).GT.1.0D-3) THEN
               IF(DABS(DOTFI(IJ+1)).EQ.0.0D0.AND.DABS(DOTFR(IJ+1)).EQ.0.0D0) THEN
                  JK_TEMP=0.0D0
               ELSE
            ...
            WRITE(OUTLYNE,208) FR(IJ+1),(DOTFM(IJ+1)/NORMI),DOTFP(IJ+1)
            CALL SHOWIT(0)
!
...
!     DO A SINGLE VALUE
         ...
         IF(DOTFR1.NE.0.0D0.AND.DABS(DOTFI1).GT.1.0D-3) THEN
            IF(DABS(DOTFI1).EQ.0.0D0.AND.DABS(DOTFR1).EQ.0.0D0) THEN
               JK_TEMP=0.0D0
            ELSE
...
!
111         FORMAT('THE DIFFRACTION MODULATION TRANSFER FUNCTION VALUE = ',G13.6)
112         FORMAT('     THE DIFFRACTION PHASE TRANSFER FUNCTION VALUE = ',G13.6)
            IF(WQ(1:1).EQ.'X') WRITE(OUTLYNE,206)
            IF(WQ(1:1).EQ.'Y') WRITE(OUTLYNE,205)
            CALL SHOWIT(0)
      ...
      END IF
   END DO
!     NEXT END DO IS AT END OF IG LOOP
   DEALLOCATE(PUPL,SUMMER1,SUMMER2,FREQ,XY12,XY22 ,DOTFI,DOTFR,DOTFP,DOTFM,STAT=ALLOERR)
   RETURN
END
