...
!
   ...
   &RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,V1,V2,V3,&
   &ZB,ZA,LLL,MMM,NNN,ZAR,ZBR,RADD,ZCR,V4,V5,V6,LOO,MOO,NOO
...
!
   IF(WC.EQ.'STREHL') THEN
      IF(STI.EQ.1) THEN
         OUTLYNE=&
         &'"STREHL" CALCULATES EXACT STREHL RATIO FOR CURRENT LEN'
         CALL SHOWIT(1)
      ...
      &WQ.NE.'NOOB'.AND.WQ.NE.'ACCNOOB') THEN
...
9000  FORMAT('THE CURRENT EXACT STREHL RATIO = ',G15.8)
   ...
   IF(STI.EQ.1) THEN
      IF(WC.EQ.'VAR')&
      &OUTLYNE='"VAR" RETURNS WAVEFRONT VARIENCE FOR CURRENT CAPFN'
      IF(WC.EQ.'APSTREHL')&
      &OUTLYNE=&
      &'"APSTREHL" CALCULATES APPROX. STREHL RATIO FOR CURRENT CAPFN'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   ...
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      IF(WC.EQ.'APSTREHL')&
      &OUTLYNE='"APSTREHL" TAKES NO NUMERIC OR STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(.NOT.CPFNEXT) THEN
      IF(WC.EQ.'VAR')&
      &OUTLYNE='"VAR" REQUIRES AN EXISTING CAPFN'
      IF(WC.EQ.'APSTREHL')&
      &OUTLYNE='"APSTREHL" REQUIRES AN EXISTING CAPFN'
      CALL SHOWIT(1)
...
!
   ...
   &,CUTFR,CUTFRX,CUTFRY,XMOD,YMOD,XPHA,YPHA,MMENT,AL,FREQ1,FREQ2
!
...
!
   ...
   END IF
   IF(W2.LT.16.0D0.OR.&
   &(W2-DBLE(INT(W2))).NE.0.0D0.OR.&
   &((W2/2.0D0)-DBLE(INT(W2/2.0D0))).NE.0.0D0) THEN
...
!
   REAL*8 EXPUZMM &
   &,REFIN,NREFIN,CUTFRX,CUTFRY,VALVAL,GRNX,GRNY
!
   LOGICAL ERRR
!
   INTEGER SHTNM
...
!
      ...
      &.AND.SYSTEM(1).NE.0.0D0.AND.SYSTEM(31).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(1)
         SHTNM=1
      END IF
      IF(SYSTEM(2).LE.SHRTWAVE &
      &.AND.SYSTEM(2).NE.0.0D0.AND.SYSTEM(32).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(2)
         SHTNM=2
      END IF
      IF(SYSTEM(3).LE.SHRTWAVE &
      &.AND.SYSTEM(3).NE.0.0D0.AND.SYSTEM(33).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(3)
         SHTNM=3
      END IF
      IF(SYSTEM(4).LE.SHRTWAVE &
      &.AND.SYSTEM(4).NE.0.0D0.AND.SYSTEM(34).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(4)
         SHTNM=4
      END IF
      IF(SYSTEM(5).LE.SHRTWAVE &
      &.AND.SYSTEM(5).NE.0.0D0.AND.SYSTEM(35).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(5)
         SHTNM=5
      END IF
      IF(SYSTEM(71).LE.SHRTWAVE &
      &.AND.SYSTEM(71).NE.0.0D0.AND.SYSTEM(76).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(71)
         SHTNM=6
      END IF
      IF(SYSTEM(72).LE.SHRTWAVE &
      &.AND.SYSTEM(72).NE.0.0D0.AND.SYSTEM(77).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(72)
         SHTNM=7
      END IF
      IF(SYSTEM(73).LE.SHRTWAVE &
      &.AND.SYSTEM(73).NE.0.0D0.AND.SYSTEM(78).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(73)
         SHTNM=8
      END IF
      IF(SYSTEM(74).LE.SHRTWAVE &
      &.AND.SYSTEM(74).NE.0.0D0.AND.SYSTEM(79).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(74)
         SHTNM=9
      END IF
      IF(SYSTEM(75).LE.SHRTWAVE &
      &.AND.SYSTEM(75).NE.0.0D0.AND.SYSTEM(80).NE.0.0D0) THEN
         SHRTWAVE=SYSTEM(75)
         SHTNM=10
      END IF
!
...
!
   ...
   &,WAY,SPT5,SPT6,SPT7,SPT8,SPT9,SPT10,LAMFACTOR &
   &,OPPEAK(0:10),OPPIT(0:10),OPPTV(0:10),SUMONE(0:10),&
   &SUMTWO(0:10),ARG,LAM(1:10),WEI(1:10),SUML2,SUML4 &
   &,WEIS,SIG,SUMSIG,DELFOB,APODX2,APODR2
!
   REAL*8 NCOS,A,DSPO
!
   DIMENSION DSPO(:,:)
...
!
   ...
   &NUMT5,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10,II,JJ,K &
   &,NSTART,NSTOP,IX,IY,NUMT0,TEMPHOLDER
!
   LOGICAL FOBB0 &
   &,FOBB0X,FOBB0Y
!
...
!       DEFAULT NUMERIC INPUT, ALL DEFAULTS ARE ZERO
      ...
      END IF
   END IF
   IF(W1.LE.0.0D0.AND.TPT.EQ.1.AND.&
   &WC.NE.'PSFK'.AND.WC.NE.'PUPIL') THEN
      OUTLYNE=&
      &'"CAPFN" REQUIERS SOME POSITIVE NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!       NOT ALL ZERO INPUT , PROCEED
   ...
   IF(WC.EQ.'CAPFN'.AND.TPT.EQ.1) THEN
      ...
      &((W1/2.0D0)-DBLE(INT(W1/2.0D0))).NE.0.0D0) THEN
         ...
         &'"CAPFN" REQUIERS NUMERIC WORD #1 TO BE A POSITIVE'
         CALL SHOWIT(1)
         OUTLYNE=&
         &'EVEN INTEGER VALUE FROM 16 TO 8192'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!       NOT ALL ZERO INPUT , PROCEED
   ...
   IF(TPT.EQ.1.AND.WC.EQ.'PUPIL') THEN
      IF(S5.EQ.1)THEN
         OUTLYNE=&
         &'"PUPIL" TAKES NO NUMERIC WORD #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
!     DEALLOCATE DSPOTT
...
!
   ...
   &.AND.SYSTEM(80).LE.0.0D0.AND.SYSTEM(35).LE.0.0D0) THEN
      IF(TPT.EQ.1) THEN
...
!     ASSIGN A TEMPORARY CIRCULAR CLAP
      ...
      IF(TPT.EQ.1) THEN
         ...
         &'HAS BEEN ASSIGNED TO THE REFERENCE SURFACE'
         IF(MSGSPD)CALL SHOWIT(0)
         IF(WC.EQ.'CAPFN') THEN
...
!     THE SHORTEST WAVELENGTH SO THAT THE SPACING WILL BE THE SAME IN THE PSF
               IF(IWL.NE.SHORT.AND.WC.EQ.'PSFK'.OR.IWL.EQ.SHORT.AND.WC &
               &.EQ.'PUPIL') THEN
...
!
   ...
   &SYSTEM(30).LE.2.0D0.AND.REFLOC.EQ.4) THEN
...
!     CALC SUMSIG
      ...
      DO ISS=1,10
         ...
         &LAM(ISS).NE.0.0D0)&
         &SUMSIG=SUMSIG+((WEI(ISS)*(RMSOP(ISS)**2))/(LAM(ISS)**2))
      END DO
   END IF
   SIG=0.0D0
   IF(SUML2.NE.0.0D0.AND.SUMSIG.NE.0.0D0)&
   ...
   IF(DABS(OPPTV(0)/(DBLE(NRD))).GE.0.25D0) THEN
      IF(TPT.EQ.1) THEN
         ...
         ELSE
            ...
            CALL SHOWIT(1)
            OUTLYNE=&
            &'USE CAUTION INTERPRETING DIFFRACTION BASED RESULTS.'
            CALL SHOWIT(1)
         END IF
      END IF
   ELSE
...
!
   ...
   &,SSSQS,IJ,M4,ALLOERR,IJK,SS1,SS2,SSN,DDFF1,DDFF2
   COMMON/GII/IIG
!
...
!
   ...
   &,VALVAL,GRNX,GRNY,WVNUM,FREQFACT,X,Y,Y1,Y2,nor,SHRTWV &
   &,IWLIJK(1:10),REFIN,NREFIN,VXHI,VXLO,VYHI,VYLO,FNXX,FNYY
!
   LOGICAL REFERR,EXTDMTF1,EXTDMTF2,PERF
   COMMON/DMTFEXT/EXTDMTF1,EXTDMTF2,PERF
!
...
!
   ...
   &,NORMI,DOTFR1,DOTFI1,DOTFM1,DOTFP1,XFR &
   &,DOTFM0,DOTFP0,DOTFI0,DOTFR0,XFR0
!
...
!
   ...
   &WWWW1,WWWW2,DOTFR,DOTFI,DOTFP,DOTFM,FR
   DIMENSION PUPL(:,:,:),SUMMER1(:),SUMMER2(:),FREQ(:),XY12(:),&
   &XY22(:),DOTFI(:),DOTFM(:),DOTFR(:),DOTFP(:),FR(:)
...
!
   DEALLOCATE (&
   &PUPL,SUMMER1,SUMMER2,FREQ &
   &,XY12,XY22,STAT=ALLOERR)
   ALLOCATE (&
   &PUPL(IW,IW,2),SUMMER1(IW+1),SUMMER2(IW+1),FREQ(IW+1)&
   &,XY12(IW+1),XY22(IW+1),STAT=ALLOERR)
   IWIW=IW**2
...
!     MODULUS AND PHASE TRANSFER FUNCTIONS
      ...
      &,DOTFP(N+1),XY12(IW+1),XY22(IW+1),FREQ(IW+1),STAT=ALLOERR)
      IJ=IW+1
      FREQ(1:IJ)=0.0D0
      XY12(1:IJ)=0.0D0
      XY22(1:IJ)=0.0D0
...
!
               DO L=(1+K),IW
                  DO M=1,IW
                     SUM1=SUM1 &
                     &+((PUPL(L,M,1)*PUPL(L-K,M,1))*&
                     &DCOS(-PUPL(L-K,M,2)+PUPL(L,M,2)))
                     SUM2=SUM2 &
                     &+((PUPL(L,M,1)*PUPL(L-K,M,1))*&
                     &DSIN(-PUPL(L-K,M,2)+PUPL(L,M,2)))
                  END DO
               END DO
            END IF
...
!     Y-RESPONSE (HORIZONTAL BARS)
               ...
               DO L=(1+K),IW
                  DO M=1,IW
                     SUM1=SUM1 &
                     &+((PUPL(M,L,1)*PUPL(M,L-K,1))*&
                     &DCOS(-PUPL(M,L-K,2)+PUPL(M,L,2)))
                     SUM2=SUM2 &
                     &+((PUPL(M,L,1)*PUPL(M,L-K,1))*&
                     &DSIN(-PUPL(M,L-K,2)+PUPL(M,L,2)))
                  END DO
               END DO
            END IF
...
!     PARTS OF THE OTF
            ...
            SUMMER2(K+1)=SUM2
            IF(WQ(1:1).EQ.'X')&
            &FREQ(K+1)=DABS((DBLE(K)*CUTFRX*FREQFACT)/DBLE(IW))
            IF(WQ(1:1).EQ.'Y')&
            &FREQ(K+1)=DABS((DBLE(K)*CUTFRY*FREQFACT)/DBLE(IW))
         END DO
         IF(WQ(1:1).EQ.'X')&
         &FREQ(IW+1)=CUTFRX*FREQFACT
         IF(WQ(1:1).EQ.'Y')&
         &FREQ(IW+1)=CUTFRY*FREQFACT
         SUMMER1(IW+1)=0.0D0
         SUMMER2(IW+1)=0.0D0
!
...
209      FORMAT('DIFFRACTION LIMIT CASE')
         ...
         &SPACEBALL.EQ.2.AND.SYSTEM(30).LE.2.0D0) WRITE(OUTLYNE,507)
         IF(SPACEBALL.EQ.1.AND..NOT.NEAR.OR.&
         &SPACEBALL.EQ.2.AND.SYSTEM(30).GT.2.0D0) WRITE(OUTLYNE,508)
         CALL SHOWIT(0)
...
!
111         FORMAT(&
            &'THE DIFFRACTION MODULATION TRANSFER FUNCTION VALUE = ',G13.6)
112         FORMAT(&
            &'     THE DIFFRACTION PHASE TRANSFER FUNCTION VALUE = ',G13.6)
            IF(WQ(1:1).EQ.'X') WRITE(OUTLYNE,206)
            IF(WQ(1:1).EQ.'Y') WRITE(OUTLYNE,205)
            CALL SHOWIT(0)
            IF(SPACEBALL.EQ.1.AND.NEAR)&
            &WRITE(OUTLYNE,102) XFR
            IF(SPACEBALL.EQ.2.AND.SYSTEM(30).LE.2.0D0)&
            &WRITE(OUTLYNE,101) XFR
            IF(SPACEBALL.EQ.1.AND..NOT.NEAR)&
            &WRITE(OUTLYNE,104) XFR
            IF(SPACEBALL.EQ.2.AND.SYSTEM(30).GT.2.0D0)&
            &WRITE(OUTLYNE,103) XFR
            CALL SHOWIT(0)
...
!     NEXT END DO IS AT END OF IG LOOP
   ...
   &,DOTFI,DOTFR,DOTFP,DOTFM,STAT=ALLOERR)
   RETURN
END
