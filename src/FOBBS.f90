...
!       CHECK FOR NATURE OF QUALIFIER WORD
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'NULL'.AND.WQ.NE.'P'.AND.WQ.NE.'PIC'.AND.WQ.NE.'PICNH'.AND.WQ.NE.'PFS'.AND.WQ.NE.'PFSNH') THEN
...
!     SAVE INPUT
   ...
   IF(sys_scy_fang_set().EQ.1.0D0.AND.sys_scx_fang_set().EQ.1.0D0) THEN
!     ANGLE INPUT MODE
      IF(sys_fang_y().NE.0.0D0) THEN
         W1=W1/sys_fang_y()
      ELSE
         W1=0.0D0
      END IF
      IF(sys_fang_x().NE.0.0D0) THEN
         W2=W2/sys_fang_x()
      ELSE
         W2=0.0D0
      END IF
...
!
   REAL*8 FT,FS,X00,Y00,Z0 ,LAMBDA,ANGLE1,XK,YK,XLN,XMN,XNN,XFOBB0,YFOBB0,ZFOBB0 ,ANGJK1,ANGJK2,XXANG,YYANG,FACTER,XA,YA,ZA,XJIM,YJIM,ZJIM ,ZSAG,ANGFACX,ANGFACY,ANGLE2,SCLFACY,SCLFACX,AWW1,AWW2 ,NWW1,NWW2,NWW3
!
...
!       CHECK FOR STRING INPUT
   IF(STI.EQ.1) THEN
      OUTLYNE='"FOB" TYPE COMMANDS DEFINE AN OBJECT POSITION FOR RAY TRACING'
      CALL SHOWIT(1)
      IF(REFEXT) THEN
...
!     USE Y HEIGHT OF CLAP WITH OFFSETS
      ...
      IF(surf_clap_type(NEWOBJ) == 5.AND.surf_array_parity(NEWOBJ) == 0) THEN
         ...
         ZSTRT=(W3*surf_thickness(NEWOBJ))+ZSAG
      END IF
      IF(surf_clap_type(NEWOBJ).GT.1.0D0.AND.surf_clap_type(NEWOBJ).LT.5.0D0.AND.surf_array_parity(NEWOBJ) == 0) THEN
         XSTRT=W2*(surf_clap_dim(NEWOBJ, 2)+surf_clap_dim(NEWOBJ, 4))
         YSTRT=W1*(surf_clap_dim(NEWOBJ, 1)+surf_clap_dim(NEWOBJ, 3))
         IISURF=NEWOBJ
         CALL SAGRET(IISURF,XSTRT,YSTRT,ZSAG,SAGERR)
...
!       NO QUALIFIER,PROCEED
   ...
   END IF
!       CHECK FOR LEGAL WAVELENGTH BOUNDS
   IF(INT(W4).NE.1.AND.INT(W4).NE.2.AND.INT(W4).NE.3.AND.INT(W4).NE.4.AND.INT(W4).NE.5.AND.INT(W4).NE.6.AND.INT(W4).NE.7.AND.INT(W4).NE.8.AND.INT(W4).NE.9.AND.INT(W4).NE.10) THEN
      OUTLYNE='WAVELENGTH NUMBER BEYOND LEGAL BOUNDS'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
   ...
   IF(W4.GE.1.0D0.AND.W4.LE.5.0D0) THEN
      IF(SYSTEM(INT(W4)).EQ.0.0D0) THEN
         IF(MSG) THEN
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',NEWOBJ
            CALL SHOWIT(1)
            OUTLYNE='REFERENCE RAY CAN NOT BE TRACED AT ZERO WAVELENGTH'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=11
...
!
   ...
   WVN=WW4
   WW5=W5
   IF(WW4.GE.1.0D0.AND.WW4.LE.5.0D0)LAMBDA=SYSTEM(INT(WW4))
   IF(WW4.GE.6.0D0.AND.WW4.LE.10.0D0)LAMBDA=SYSTEM(65+INT(WW4))
   Y00=NWW1
   X00=NWW2
   Z0=NWW3
!
...
!       RAY
      ...
      IF(STOPP.EQ.1) THEN
         ...
         REFEXT=.FALSE.
         IF(.NOT.NULL) FOBYES=.FALSE.
         IF(.NOT.NULL.AND.F34.EQ.0.AND.F28.EQ.0.AND.F35.EQ.0.AND.F31.EQ.0) CALL MACFAL
         RETURN
      ELSE
         STOPP=0
         REFEXT=.TRUE.
...
400 FORMAT(2X,'Y-POS.    ',2X,'X-POS.    ',2X,'Z-POS.',2X,'Y-IMAGE HT. ',4X,'X-IMAGE HT. ')
101 FORMAT(2X,'Y-POS.    ',2X,'X-POS.    ',2X,'Z-POS.    ',2X,'T-FOCUS POS.',4X,'S-FOCUS POS.')
...
!
   ...
   COMMON/PFOB/XPFOB,YPFOB,LUNI
!
   REAL*8 FT,FS,X00,Y00,Z0 ,LAMBDA,ANGLE1,XK,YK,XLN,XMN,XNN,XFOBB0,YFOBB0,ZFOBB0 ,ANGJK1,ANGJK2,XXANG,YYANG,FACTER,XA,YA,ZA,XJIM,YJIM,ZJIM ,ZSAG,ANGLE2,SCLFACY,SCLFACX,AWW1,AWW2
!
...
!     SET UP ANGLE FACTERS
   IF(sys_fang_y().EQ.0.0D0) THEN
      SCLFACY=1.0D0
   ELSE
      SCLFACY=DABS(1.0D0/sys_fang_y())
   END IF
   IF(sys_fang_x().EQ.0.0D0) THEN
      SCLFACX=1.0D0
   ELSE
      SCLFACX=DABS(1.0D0/sys_fang_x())
   END IF
   AWW1=W1/SCLFACY
   AWW2=W2/SCLFACX
!
...
!
      ANGJK1=AWW1
      ANGJK2=AWW2
      IF(ANGJK1.GT.89.9999D0.AND.ANGJK1.LT.90.0001D0.OR.ANGJK1.LT.-89.9999D0.AND.ANGJK1.GT.-90.0001D0.OR.ANGJK1.GT.-270.0001D0.AND.ANGJK1.LT.-269.9999D0.OR.ANGJK1.GT.269.9999D0.AND.ANGJK1.LT.270.0001D0) THEN
         IF(DF2.EQ.0) THEN
         ...
         END IF
      END IF
      IF(ANGJK1.GT.89.9999D0.AND.ANGJK1.LT.90.0001D0)  ANGJK1=89.9999D0

      IF(ANGJK1.LT.-89.9999D0.AND.ANGJK1.GT.-90.0001D0) ANGJK1=-89.9999D0

      IF(ANGJK1.GT.-270.0001D0.AND.ANGJK1.LT.-269.9999D0) ANGJK1=89.9999D0

      IF(ANGJK1.GT.269.9999D0.AND.ANGJK1.LT.270.0001D0) ANGJK1=-89.9999D0

      IF(ANGJK2.GT.89.9999D0.AND.ANGJK2.LT.90.0001D0)  ANGJK2=89.9999D0

      IF(ANGJK2.LT.-89.9999D0.AND.ANGJK2.GT.-90.0001D0) ANGJK2=-89.9999D0

      IF(ANGJK2.GT.-270.0001D0.AND.ANGJK2.LT.-269.9999D0) ANGJK2=89.9999D0

      IF(ANGJK2.GT.269.9999D0.AND.ANGJK2.LT.270.0001D0) ANGJK2=-89.9999D0
...
!
         ...
         IF(NEWREF.LE.NEWOBJ.OR.NEWIMG.LE.NEWOBJ.OR.NEWIMG.LE.NEWREF) THEN
...
!
         ...
         IF(NEWOBJ.LT.0.OR.NEWOBJ.GT.INT(sys_last_surf()))THEN
            ...
            RETURN
         END IF
         IF(NEWOBJ.GE.NEWREF.OR.NEWOBJ.GE.NEWIMG.OR.NEWREF.GE.NEWIMG)THEN
            OUTLYNE='NEW OBJECT/REFERENCE/IMAGE SURFACES ILLEGALLY ORDERED'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            NEWOBJ=0
...
!
      ...
      IF((W2*sys_fang_x()).GT.269.9999D0.AND.ANGJK2.LT.270.0001D0) ANGJK2=-89.9999D0
...
400 FORMAT(2X,'Y-FOB     ',2X,'X-FOB     ',2X,'DELTA-Z',2X,'Y-IMAGE HT. ',4X,'X-IMAGE HT. ')
101 FORMAT(2X,'Y-FOB     ',2X,'X-FOB     ',2X,'DELTA-Z   ',2X,'T-FOCUS POS.',4X,'S-FOCUS POS.')
301 FORMAT(G10.2,2X,G10.2,2X,G10.2)
END
! SUB FASTFFOB.FOR
...
!
   ...
   COMMON/PFOB/XPFOB,YPFOB,LUNI
!
   REAL*8 FT,FS,X00,Y00,Z0,WPAS ,LAMBDA,ANGLE1,XK,YK,XLN,XMN,XNN,XFOBB0,YFOBB0,ZFOBB0 ,ANGJK1,ANGJK2,XXANG,YYANG,FACTER,XA,YA,ZA,XJIM,YJIM,ZJIM ,ZSAG,ANGLE2,SCLFACY,SCLFACX,AWW1,AWW2
!
   REAL*8 XRAYER,YRAYER,ZRAYER
   COMMON/RAYERPASS/XRAYER,YRAYER,ZRAYER
!
...
!
   ...
   use mod_system
   use DATMAI
   use mod_surface, only: surf_thickness, surf_curvature, surf_clap_type, surf_clap_dim, surf_special_type, surf_array_parity, surf_decenter_y, surf_decenter_x, surf_tilt_flag
   IMPLICIT NONE
!
...
1235        FORMAT(A12, G15.7, G15.7, G15.7, G15.7, G15.7, I3, I3, I3)
            PRINT 1235, "B4 REFRAY",WW1,WW2,WW3,WW4,WW5,NEWOBJ,NEWREF,NEWIMG
...
subroutine ISYFIELDANGLE90DEGREES(ANGJK, boolResult)
   ...
   IF(ANGJK.GT.89.9999D0.AND.ANGJK1.LT.90.0001D0.OR.ANGJK1.LT.-89.9999D0.AND.ANGJK1.GT.-90.0001D0.OR.ANGJK1.GT.-270.0001D0.AND.ANGJK1.LT.-269.9999D0.OR.ANGJK1.GT.269.9999D0.AND.ANGJK1.LT.270.0001D0) THEN
      boolResult = .TRUE.
   end if
...
subroutine ADJUSTANGLEIFCLOSETO90(ANG)
   ...
   IF(ANGJK.GT.89.9999D0.AND.ANGJK.LT.90.0001D0)  ANGJK1=89.9999D0

   IF(ANGJK.LT.-89.9999D0.AND.ANGJK.GT.-90.0001D0) ANGJK=-89.9999D0

   IF(ANGJK.GT.-270.0001D0.AND.ANGJK.LT.-269.9999D0) ANGJK=89.9999D0

   IF(ANGJK.GT.269.9999D0.AND.ANGJK.LT.270.0001D0) ANGJK=-89.9999D0
