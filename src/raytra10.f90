...
!
   REAL*8 PX,PY,PCX,PCY,PUX,PUY,PUCX,PUCY,DWORD2,DWORD3 ,DWORD4,DWORD5,V1,V2,V3,V4,V5,V6,V7,V8,WRX,WRY,BDX,BDY ,VALVA,VAL1,VAL2
!
!
!
   WRX=sys_wrx()
...
!
   IF(STI.EQ.1) THEN
      WRITE(OUTLYNE,*)'"BEA" OR "BEAM" OUTPUTS GAUSSIAN BEAM PROPERTIES'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1) THEN
      WRITE(OUTLYNE,*)'GAUSSIAN BEAM OUTPUT COMMANDS TAKE NO STRING INPUT'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      WRITE(OUTLYNE,*)'"BEA" AND "BEAM" TAKE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'QUALIFIER OR NUMERIC WORD #1 INPUT BUT NOT BOTH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   OLDSTOP=sys_astop()
   OLDREF2=sys_ref_surf()
   call set_sys_ref_surf(1.0D0)
   call set_sys_astop(1.0D0)
!
!
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      CALL GNPRTGEN(0,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,1)
      IF(ERROR.EQ.1) THEN
         ...
         CALL SHOWIT(1)
         CALL MACFAL
         call set_sys_ref_surf(OLDREF2)
         call set_sys_astop(OLDSTOP)
         RETURN
      END IF
...
!
      ...
      CALL SHOWIT(0)
      DO I=0,SF
         CALL GNPRTGEN(I,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,1)
         V1=DSQRT((PX**2)+(PCX**2))
...
!
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=0
      CALL GNPRTGEN(SF,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,1)
      IF(ERROR.EQ.1) THEN
         WRITE(OUTLYNE,*)'RAY COULD NOT BE TRACED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'DIFFERENTIAL RAY VALUES NOT CALCULABLE'
...
6000 FORMAT('SURF',6X,'BEAM SEMI-DIA.',2X,'WAVFNT RADIUS',4X,'DIST TO WAIST',5X,'WAIST SEMI-DIA.')
6001 FORMAT('GAUSSIAN BEAM DATA (XZ-PLANE)',' - (CFG #',I2,')')
6002 FORMAT('GAUSSIAN BEAM DATA (YZ-PLANE)',' - (CFG #',I2,')')
!
...
6010 FORMAT('      XZ-PLANE INPUT SEMI-DIA. =  ',G13.6,' LENS UNITS')
6011 FORMAT('      YZ-PLANE INPUT SEMI-DIA. =  ',G13.6,' LENS UNITS')
6012 FORMAT('XZ-PLANE DIVERGENCE HALF-ANGLE =  ',G13.6,' MILLIRADIANS')
6013 FORMAT('YZ-PLANE DIVERGENCE HALF-ANGLE =  ',G13.6,' MILLIRADIANS')
!
END
! SUB GLPRY.FOR
SUBROUTINE GLPRY
...
!
   INTEGER JK,I,N_HITS
!
   REAL*8 GRX,GRY,GRZ,GRL,GRM,GRN,GRLP,GRMP,GRNP,X1,Y1,Z1,L1,M1,N1,X00,Y00,Z0,LX0,LY0,LZ0,MX0,MY0,MZ0,NX0,NY0,NZ0,LO1,MO1,NO1
!
!
...
!
      ...
      NZ0=VERTEX(12,I)
!
      X1=X00+((LX0*(RAYRAY(1,I)))+(LY0*(RAYRAY(2,I)))+(LZ0*(RAYRAY(3,I))))
      Y1=Y00+((MX0*(RAYRAY(1,I)))+(MY0*(RAYRAY(2,I)))+(MZ0*(RAYRAY(3,I))))
      Z1=Z0+((NX0*(RAYRAY(1,I)))+(NY0*(RAYRAY(2,I)))+(NZ0*(RAYRAY(3,I))))
!
      L1=((LX0*(RAYRAY(4,I)))+(LY0*(RAYRAY(5,I)))+(LZ0*(RAYRAY(6,I))))
      M1=((MX0*(RAYRAY(4,I)))+(MY0*(RAYRAY(5,I)))+(MZ0*(RAYRAY(6,I))))
      N1=((NX0*(RAYRAY(4,I)))+(NY0*(RAYRAY(5,I)))+(NZ0*(RAYRAY(6,I))))
!
      LO1=((LX0*(RAYRAY(19,I)))+(LY0*(RAYRAY(20,I)))+(LZ0*(RAYRAY(21,I))))
      MO1=((MX0*(RAYRAY(19,I)))+(MY0*(RAYRAY(20,I)))+(MZ0*(RAYRAY(21,I))))
      NO1=((NX0*(RAYRAY(19,I)))+(NY0*(RAYRAY(20,I)))+(NZ0*(RAYRAY(21,I))))
!
      GLPRAY(1,I)=X1
      GLPRAY(2,I)=Y1
      GLPRAY(3,I)=Z1
...
!     IF THE CURRENT SURFACE IS A TRUE DUMMY
      IF(DUM(I).AND.surf_special_type(I).EQ.0 .AND.I.NE.NEWIMG) GLVIRT(I)=.TRUE.
      ...
      IF(I.GE.1) THEN
         IF(NUMHITS(I-1).GT.1) THEN
            DO N_HITS=1,NUMHITS(I-1)
               ...
               X1=X00+((LX0*(GRX))+(LY0*(GRY))+(LZ0*(GRZ)))
               Y1=Y00+((MX0*(GRX))+(MY0*(GRY))+(MZ0*(GRZ)))
               Z1=Z0+((NX0*(GRX))+(NY0*(GRY))+(NZ0*(GRZ)))
!
               L1=((LX0*(GRL))+(LY0*(GRM))+(LZ0*(GRN)))
               M1=((MX0*(GRL))+(MY0*(GRM))+(MZ0*(GRN)))
               N1=((NX0*(GRL))+(NY0*(GRM))+(NZ0*(GRN)))
!
               LO1=((LX0*(GRLP))+(LY0*(GRMP))+(LZ0*(GRNP)))
               MO1=((MX0*(GRLP))+(MY0*(GRMP))+(MZ0*(GRNP)))
               NO1=((NX0*(GRLP))+(NY0*(GRMP))+(NZ0*(GRNP)))
!
...
!
      IF(SQ.EQ.1.AND.S1.EQ.1) THEN
         WRITE(OUTLYNE,*)'"',WC(1:6),'" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(SQ.EQ.1.AND.WQ.NE.'OFF') THEN
         WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD USED WITH "',WC(1:6),'" COMMAND'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"',WC(1:6),'" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(S1.EQ.1.AND.INT(W1).LT.NEWOBJ.OR.S1.EQ.1.AND.INT(W1).GT.NEWIMG) THEN
         WRITE(OUTLYNE,*)'SURFACE NUMBER BEYOND LEGAL RANGE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         ...
         RETURN
      END IF
      IF(DF1.EQ.1.AND.DF2.EQ.1.AND.DF3.EQ.1.AND.SQ.EQ.0.AND.DF4.EQ.1.AND.DF5.EQ.1) THEN
...
!                       RETURN
         ...
         IF(GLOBE) THEN
            ...
            CALL SHOWIT(0)
101         FORMAT('"GLOBAL" IS CURRENTLY "ON", REFERENCED TO SURFACE ',I3)
...
!       SET REFERENCE AND OTHER RAY STATUS TO FALSE
      ...
      END IF
      IF(S1.EQ.1) THEN
         IF(DABS(surf_thickness(INT(W1))).GT.1.0D10.AND.INT(W1).NE.NEWOBJ) THEN
            WRITE(OUTLYNE,*)'THE GLOBAL SURFACE SPECIFIED ,SURFACE NUMBER ',INT(W1)
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'HAS AN INFINITE THICKNESS ( MAGNITUDE GREATER THAN 1.0D+10)'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'"GLOBAL" WILL YIELD UNCERTAIN RESULTS IN THIS CASE'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'ADJUST THICKNESSES AND RE-ENTER THE "GLOBAL" COMMAND'
            CALL SHOWIT(1)
            REFEXT=.FALSE.
            SPDEXT=.FALSE.
            GSPDEXT=.FALSE.
            ...
            RETURN
         END IF
         IF(DABS(surf_thickness(INT(W1))).GT.1.0D10.AND.INT(W1).EQ.NEWOBJ) THEN
            WRITE(OUTLYNE,*)'GLOBAL SURFACE SELECTED WAS THE CURRENT OBJECT SURFACE'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'IT HAS AN INFINITE THICKNESS ( MAGNITUDE GREATER THAN 1.0D+10)'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'"GLOBAL" WILL YIELD UNCERTAIN RESULTS IN THIS CASE'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER "GLOBAL" WITH A DIFFERENT SURFACE NUMBER'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'OR A REDUCED OBJECT THICKNESS'
            CALL SHOWIT(1)
            REFEXT=.FALSE.
            SPDEXT=.FALSE.
            GSPDEXT=.FALSE.
...
!       CHECK FOR AND THICKNESSES GT 1.0D10
         DO 1010 I=NEWOBJ+1,NEWIMG
            IF(DABS(surf_thickness(I)).GT.1.0D10) THEN
               WRITE(OUTLYNE,*)'SURFACE NUMBER ',I
               CALL SHOWIT(1)
               WRITE(OUTLYNE,*)'HAS AN INFINITE THICKNESS ( MAGNITUDE GREATER THAN 1.0D+10)'
               CALL SHOWIT(1)
               WRITE(OUTLYNE,*)'"GLOBAL" WILL YIELD UNCERTAIN RESULTS IN THIS CASE'
...
199      FORMAT('"GLOBAL" IS CURRENTLY SET TO "OFF"')
         ...
         CALL SHOWIT(0)
198      FORMAT(A6,' ',A4,' IS NOT A VALID COMMAND  WHEN "GLOBAL" IS SET TO "OFF"')
         RETURN
      END IF
      ...
      IF(S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1) THEN
         WRITE(OUTLYNE,*)'"',WC(1:6),' ',WQ(1:4),'" TAKES NO STRING OR NUMERIC WORD 4 OR 5 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       PRINT CURRENT OFFSET STATUS
         IF(.NOT.GLOBE) THEN
            WRITE(OUTLYNE,200)
            CALL SHOWIT(0)
200         FORMAT('"GLOBAL" IS CURRENTLY SET TO "OFF", NO OFFSETS CAN EXIST')
            RETURN
         END IF
         IF(GLOBE) THEN
            WRITE(OUTLYNE,201) GLSURF
            CALL SHOWIT(0)
201         FORMAT('"GLOBAL" IS CURRENTLY "ON", REFERENCED TO SURFACE ',I3)
            WRITE(OUTLYNE,220)
            CALL SHOWIT(0)
...
!      OFFSET REQUIRES A QUALIFIER WITH NUMERIC INPUT
         WRITE(OUTLYNE,*)'"OFFSET" REQUIRES A QUALIFIER WITH NUMERIC INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       NO VERTEX DATA TO OUTPUT
         WRITE(OUTLYNE,*)'NO CURRENT GLOBAL VERTEX DATA EXISTS TO OUTPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"VERTEX" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.1.AND.S1.EQ.1) THEN
         WRITE(OUTLYNE,*)'"VERTEX" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
...
5001  FORMAT('CURRENT GLOBAL VERTEX DATA ','  - (CFG #',I2,')')
5002  FORMAT('GLOBAL VERTEX DATA REFERENCED TO SURFACE NUMBER ',I3)
2501  FORMAT(1X)
      RETURN
   END IF
   RETURN
...
!
   INTEGER JK,I
!
   REAL*8 X1,Y1,Z1,L1,M1,N1,X00,Y00,Z0,LX0,LY0,LZ0,MX0,MY0,MZ0,NX0,NY0,NZ0,LO1,MO1,NO1,LN1,MN1,NN1
!
!
...
!
      ...
      NO1=((NX0*(RAYRAY(19,I)))+(NY0*(RAYRAY(20,I)))+(NZ0*(RAYRAY(21,I))))
!
      LN1=((LX0*(RAYRAY(13,I)))+(LY0*(RAYRAY(14,I)))+(LZ0*(RAYRAY(15,I))))
      MN1=((MX0*(RAYRAY(13,I)))+(MY0*(RAYRAY(14,I)))+(MZ0*(RAYRAY(15,I))))
      NN1=((NX0*(RAYRAY(13,I)))+(NY0*(RAYRAY(14,I)))+(NZ0*(RAYRAY(15,I))))
!
      GLRAY(1,I)=X1
      GLRAY(2,I)=Y1
      GLRAY(3,I)=Z1
...
!
   REAL*8 XREFI,XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,YREFI,RAD,AL,BE,GA,A,B,C,LEN,LEN1,LO,LOOX,LOOY,MO,MOOX,MOOY,LEN2,Q,ARG,ZREFI,SIGNB,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,XA,THXDEN,THYDEN,YA,ZA,LLL,MMM,NNN
!
!
   RCOR=0.0D0
   OCOR=0.0D0
...
!     OR IDEAL LENS
   IF(GLANAM(INT(sys_last_surf())-1,2).EQ.'PERFECT      '.OR.GLANAM(INT(sys_last_surf())-1,2).EQ.'IDEAL        ') THEN
...
!       NOW X,Y AND Z ARE:
      ...
      YA=RRY0+(T*M0)
      ZA=RRZ0+(T*N0)
      OCOR=DSQRT(((RAYRAY(1,NEWIMG-1)-XA)**2)+((RAYRAY(2,NEWIMG-1)-YA)**2)+((RAYRAY(3,NEWIMG-1)-ZA)**2))
...
!       NOW X,Y AND Z ARE:
      ...
      YA=RRY0+(T*M0)
      ZA=RRZ0+(T*N0)
      OCOR=DSQRT(((RAYRAY(1,NEWIMG)-XA)**2)+((RAYRAY(2,NEWIMG)-YA)**2)+((RAYRAY(3,NEWIMG)-ZA)**2))
!     IF T IS LESS THAN ZERO, THE RAY INTERSECTION WITH THE
!     REFERENCE PLANE LIES ON THE NEGATIVE SIDE OF THE IMAGE SURFACE
...
!       DO AN INTERNAL "ASTOP EX" ADJUSTMENT
            IF(DABS(PXTRAY(5,NEWIMG)*1.0D-15).LT.DABS(PXTRAY(6,NEWIMG))) THEN
               RAD=(PXTRAY(5,NEWIMG)/PXTRAY(6,NEWIMG))
            END IF
            IF(DABS(PXTRAY(5,NEWIMG)*1.0D-15).GE.DABS(PXTRAY(6,NEWIMG))) THEN
...
!
         ...
         C=(AL**2)+(BE**2)+(GA**2)-(RAD**2)
         B=2.0D0*((RAYRAY(19,NEWIMG)*AL)+(RAYRAY(20,NEWIMG)*BE)+(RAYRAY(21,NEWIMG)*GA))
         A=1.0D0
!       SOLVE FOR LEN
         IF(B.NE.0.0D0) SIGNB=((DABS(B))/(B))
         IF(B.EQ.0.0D0) SIGNB=1.0D0
         ...
         ELSE
            ...
            IF(ARG.LT.0.0D0) THEN
               OUTLYNE='RAY(S) COULD NOT INTERSECT REFERENCE SPHERE'
               CALL SHOWIT(1)
               OUTLYNE='TRY USING AN "ASTOP EX" OR ASTOP "EN/EX" ADJUSTMENT'
               CALL SHOWIT(1)
               OUTLYNE='OPD VALUES MAY BE SUSPECT'
               CALL SHOWIT(1)
               OCOR=0.0D0
               RCOR=0.0D0
               RETURN
...
!
         ...
         IF(DABS(THXNUM*1.0D-15).LT.DABS(THXDEN).AND.DABS(THYNUM*1.0D-15).LT.DABS(THYDEN)) THEN
            RAD=-((-THXNUM/THXDEN)+(-THYNUM/THYDEN))/2.0D0
!     THE EXIT PUPIL IS LOCATED AT:
            XA=XO+(-RAD*LO)
            YA=YO+(-RAD*MO)
            ZA=ZO+(-RAD*NO)
            RAD=DSQRT(((XA-XREFI)**2)+((YA-YREFI)**2)+((ZA-ZREFI)**2))
            IF(ZA.LE.ZO) THEN
!     PUPIL LIES ON THE NEGATIVE SIDE OF NEWIMG SO RAD IS POS
            ...
            END IF
         END IF
         IF(DABS(THXNUM*1.0D-15).GE.DABS(THXDEN).OR.DABS(THYNUM*1.0D-15).GE.DABS(THYDEN)) THEN
!       REF RAY IS TELECENTRIC, SET RAD =1.0D20
            RAD=1.0D20
         END IF
!
...
!       SOLVE FOR LEN
         ...
         ELSE
            ...
            IF(ARG.LT.0.0D0) THEN
               OUTLYNE='RAY(S) COULD NOT INTERSECT REFERENCE SPHERE'
               CALL SHOWIT(1)
               OUTLYNE='THE CURRENT SYSTEM MAY HAVE EXCESSIVE ABERRATIONS'
               CALL SHOWIT(1)
               OUTLYNE='OPD VALUES MAY BE SUSPECT'
               CALL SHOWIT(1)
               OCOR=0.0D0
...
!
   INTEGER SF,I,ERROR
!
   REAL*8 PX,PY,PCX,PCY,PUX,PUY,PUCX,PUCY,DWORD2,DWORD3 ,DWORD4,DWORD5
!
!
   GPRAYEXT=.FALSE.
   IF(DF2.EQ.1) W2=0.0D0
...
!
   IF(STI.EQ.1) THEN
      WRITE(OUTLYNE,*)'"GPXT(XorY)" OUTPUT GENERALIZED PARAXIAL DATA'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1) THEN
      WRITE(OUTLYNE,*)'GENERALIZED PARAXIAL RAYTRACE OUTPUT COMMANDS TAKE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NO STRING INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1.AND.WQ.EQ.'ALL') THEN
      CALL GNPRTGEN(0,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,0)
      IF(ERROR.EQ.1) THEN
         WRITE(OUTLYNE,*)'RAY COULD NOT BE TRACED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'DIFFERENTIAL RAY VALUES NOT CALCULABLE'
      ...
      CALL SHOWIT(0)
      DO 10 I=0,SF
         CALL GNPRTGEN(I,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,0)
         IF(ERROR.EQ.1) THEN
            WRITE(OUTLYNE,*)'RAY COULD NOT BE TRACED'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'DIFFERENTIAL RAY VALUES NOT CALCULABLE'
...
10    CONTINUE
   ...
   IF(SQ.EQ.1.AND.WQ.EQ.'OBJ'.OR.SQ.EQ.1.AND.WQ.EQ.'OB') THEN
      SF=0
      CALL GNPRTGEN(SF,PY,PX,PUY,PUX,PCY,PCX,PUCY,PUCX,ERROR ,DWORD2,DWORD3,DWORD4,DWORD5,0)
      IF(ERROR.EQ.1) THEN
         WRITE(OUTLYNE,*)'RAY COULD NOT BE TRACED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'DIFFERENTIAL RAY VALUES NOT CALCULABLE'
...
6001 FORMAT('GENERALIZED PARAXIAL RAYTRACE DATA (XZ-PLANE)',' - (CFG #',I2,')')
6002 FORMAT('(PUX AND PUCX) MEASURED WITH RESPECT TO THE Z-AXIS')
!
!       GPXTY
!
6003 FORMAT('SURF',6X,'PY',12X,'PUY',13X,'PCY',11X,'PUCY')
6004 FORMAT('GENERALIZED PARAXIAL RAYTRACE DATA (YZ-PLANE)',' - (CFG #',I2,')')
...
!
   INTEGER JK,I
!
   REAL*8 X,Y,Z,X1,Y1,Z1,L1,M1,N1,X00,Y00,Z0,LX0,LY0,LZ0,MX0,MY0,MZ0,NX0,NY0,NZ0,LO1,MO1,NO1
!
!
   IF(DABS(surf_thickness(NEWOBJ)).LE.1.0D10) THEN
!       FINITE OBJECT THICKNESS
