...
!
   INTEGER I,KKK
!
   REAL*8 DDELX,DDELY,TARX,TARY,YANG,XANG ,D11,D12,D21,D22,DET,MF1,MF2,SHIFTX,SHIFTY,HIT,MAG,LSTART,MSTART,NSTART,XPARTX,YPARTX,XPARTY,YPARTY ,LOLD,MOLD,NOLD,XC1,YC1,ZC1,XL,XM,XN,YL,YM,YN ,ZPARTX,ZPARTY
!
   REAL*8 SIG,X,XHT,YHT,ZHT,XSCYFAC,YSCYFAC ,Y,Z,L,M,N,X1ONE,Y1ONE,X1LAST,Y1LAST,RXONE,RYONE,RXLAST,RYLAST
!
   LOGICAL DELFAIL
!
!
...
!
!       FIRST XZ PLANE
   IF(XHT.NE.0.0D0)SIG=XHT/DABS(XHT)
   IF(XHT.EQ.0.0D0)SIG=1.0D0
   ...
   DO 10 I=NEWOBJ+1,NEWIMG
      ...
      IF(STOPP.EQ.1) THEN
         IF(MSG) THEN
            ...
            CALL SHOWIT(1)
            OUTLYNE='WHILE TRACING THE XZ-PLANE CHIEF DIFFERENTIAL RAY'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         SREFDIFEXT=.FALSE.
...
!
         ...
         IF(KKK.GT.2 .OR.sys_ray_aiming().EQ.0.0D0) THEN
...
!
      ...
      IF(STOPP.EQ.1) THEN
         IF(MSG) THEN
            ...
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',I
            CALL SHOWIT(1)
            OUTLYNE='WHILE TRACING THE YZ-PLANE CHIEF DIFFERENTIAL RAY'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         SREFDIFEXT=.FALSE.
...
20 CONTINUE
   DO I=NEWOBJ,NEWIMG
      ...
      IF(XPARTX.EQ.0.0D0.AND.YPARTX.EQ.0.0D0.AND.I.NE.0)POLANGX(I)=-POLANGX(I-1)
...
!
      ...
      IF(XPARTX.NE.0.0D0.AND.YPARTX.NE.0.0D0)POLANGX(I)=DATAN2(XPARTX,YPARTX)
      XPARTY=RFDIFF(7,I)-REFRY(1,I)
      YPARTY=RFDIFF(8,I)-REFRY(2,I)
      ZPARTY=RFDIFF(9,I)-REFRY(3,I)
      IF(XPARTY.EQ.0.0D0.AND.YPARTY.EQ.0.0D0.AND.I.NE.0)POLANGY(I)=-POLANGY(I-1)
...
!
      ...
      IF(XPARTY.NE.0.0D0.AND.YPARTY.NE.0.0D0)POLANGY(I)=DATAN2(XPARTY,YPARTY)
...
!
   INTEGER I,KKK
!
   REAL*8 DDELX,DDELY,TARX,TARY,YANG,XANG,HIT,D11,D12,D21,D22,DET,MF1,MF2,RSHIFTX,RSHIFTY,SYS12,SYS13 ,MAG,LSTART,MSTART,NSTART,LOLD,MOLD,NOLD ,XC1,YC1,ZC1
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
   REAL*8 SIG,X,Y,Z,L,M,N,X1ONE,Y1ONE,XL,XM,XN,YL,YM,YN,X1LAST,Y1LAST,RXONE,RYONE,RXLAST,RYLAST
!
   LOGICAL DELFAIL
!
!
...
!     SET REF AP HT
!
   IF(surf_clap_type(NEWREF) >= 1.AND.surf_clap_type(NEWREF) <= 6.AND.surf_array_parity(NEWREF) == 0) THEN
!
!       CLAP IS ON REFERENCE SURFACE
!
!       CIRCULAR CLAP
!
      IF(surf_clap_type(NEWREF) == 1.AND.surf_array_parity(NEWREF) == 0) THEN
...
!        ELIP CLAP
!
      IF(surf_clap_type(NEWREF) == 3.AND.surf_array_parity(NEWREF) == 0) THEN
         SYS12=surf_clap_dim(NEWREF, 1)
         SYS13=surf_clap_dim(NEWREF, 2)
!       NOT ELIP CLAP
      END IF
...
!        POLY CLAP
!
      IF(surf_clap_type(NEWREF) == 5.AND.surf_array_parity(NEWREF) == 0) THEN
         SYS12=surf_clap_dim(NEWREF, 1)
         SYS13=surf_clap_dim(NEWREF, 1)
!       NOT POLY CLAP
      END IF
!        IPOLY CLAP
!
      IF(surf_clap_type(NEWREF) == 6.AND.surf_array_parity(NEWREF) == 0) THEN
   ...
   DO 10 I=NEWOBJ+1,NEWIMG
      ...
      IF(STOPP.EQ.1) THEN
         IF(MSG) THEN
            ...
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',I
            CALL SHOWIT(1)
            OUTLYNE='WHILE TRACING THE XZ-PLANE MARGINAL DIFFERENTIAL RAY'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         SRAYDIFEXT=.FALSE.
...
!
      ...
      IF(STOPP.EQ.1) THEN
         IF(MSG) THEN
            ...
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',I
            CALL SHOWIT(1)
            OUTLYNE='WHILE TRACING THE YZ-PLANE MARGINAL DIFFERENTIAL RAY'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         SRAYDIFEXT=.FALSE.
...
!
   INTEGER I,FFS,FFT
!
   REAL*8 D11,D12,D21,D22,HIT,DET,DDELX,DDELY,SHIFTY,SHIFTX,LSTART,MSTART,NSTART,YANG,XANG,TARY,TARX,MF1,MF2 ,MAG,LOLD,MOLD,NOLD,XC1,YC1,ZC1,XL,XM,XN,YL,YM,YN
!
   INTEGER KKK
!
   REAL*8 FT,FS,X,X1ONE,Y1ONE,X1LAST,Y1LAST,Y,Z,L,M,N,RXONE,RYONE,RXLAST,RYLAST
!
   REAL*8 SYS12,SYS13,A11,A12,A21,A22,JK_AA,JK_BB,FS1,FS2,FT1,FT2,S,T
!
...
!
   ...
   FFS=0
   IF(.NOT.LDIF) THEN
      OUTLYNE='DIFFERENTIAL RAYTRACE IS "OFF"'
      CALL SHOWIT(1)
      OUTLYNE='ALL DIFFERENTIAL RAY DATA IS ZERO'
      CALL SHOWIT(1)
      STOPP=1
      SREFDIFEXT=.FALSE.
      RETURN
...
!       AND STOP SEARCHING.
   IF(KKK.GT.NRAITR) THEN
      OUTLYNE='WARNING:'
      CALL SHOWIT(1)
      OUTLYNE='MARGINAL DIFFERENTIAL RAY (XZ-PLANE) FAILURE:'
      CALL SHOWIT(1)
      OUTLYNE='RAY FAILED TO CONVERGE TO RAY-AIM POINT'
      CALL SHOWIT(1)
      STOPP=1
      SREFDIFEXT=.FALSE.
      RETURN
...
!
         IF((0.001D0*AIMTOL).LE.1.0D-10) HIT=1.0D-10
         IF((0.001D0*AIMTOL).GT.1.0D-10) HIT=0.001D0*AIMTOL
         IF(DSQRT(((TARX-X)**2)+((TARY-Y)**2)).LE.(HIT).AND.KKK.GT.1 ) THEN
!       AIM IS GOOD ENOUGH, PROCEED
            GO TO 100
!       AIM NOT GOOD ENOUGH, IMPROVE GUESS
         END IF
...
!       AND STOP SEARCHING.
   IF(KKK.GT.NRAITR) THEN
      OUTLYNE='WARNING:'
      CALL SHOWIT(1)
      OUTLYNE='MARGINAL DIFFERENTIAL RAY (YZ-PLANE) FAILURE:'
      CALL SHOWIT(1)
      OUTLYNE='RAY FAILED TO CONVERGE TO RAY-AIM POINT'
      CALL SHOWIT(1)
      STOPP=1
