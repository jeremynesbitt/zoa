!       SECOND COLLECTION OF NSS FILES
...
!
   real(real64) N,NP,COSI,SINI,SDOTN,GRO,GRS,GRX,GRY,GRZ,IA,IAP,MAG,URAD
!
   real(real64) BIGLAM,BIGGAM,QX,QY,QZ,PX,PY,PZ,DSPACE_EFF,FACTOR
!
   real(real64) CHIX,CHIY,CHIZ,CHINOR,PNOR,QNOR,WAVE,A,B,SINIP,SDOTP,MU
!
   real(real64) ROOTTERM,TESTDOT,GAM1,SGNA,XTEMP,YTEMP,ZTEMP,ENERGY
!
   LOGICAL GRATE,TIR
!
!
   TIR=.FALSE.
!
!       LOCAL COORDINATE RAY DATA IS IN:
!               RRX
!               RRY
!               RRZ
!               RRL
...
!       IF THE RAY'S LOCAL RRN IS POSITIVE, THEN IT IS TRAVELING IN THE TRAVEL=1
!       DIRECTION FROM REALINDEX1=N TO REALINDEX2=NSS_NP
!       SET THE VALUE OF TRAVEL
   ...
   ELSE
      TRAVEL=2
   END IF
!       SET VALUES OF NSS_N AND NSS_NP
   IF(TRAVEL.EQ.1) THEN
      NSS_N=REALINDEX1
      NSS_NP=REALINDEX2
   ELSE
      ...
      NSS_NP=REALINDEX1
   END IF
!       SET THE VALUE OF THE NEW CURRENT INDEX VALUE
   CURINDEXR=NSS_N
   MU=DABS(NSS_N/NSS_NP)
!       DO WE NEED TO ADJUST THE WORKING VALUES OF THE SURFACE NORMALS?
!       GET CURRENT WAVELENGTH IN MICRONS
...
!       GRATING
      GRATE=.TRUE.
   END IF
!       FORM DOT PRODUCT OF INCOMING RAY WITH NORMAL TO SURFACE
   SDOTN=(RRL*NLN)+(RRM*NMN)+(RRN*NNN)
!       SDOTN IS THE COSINE OF THE ANGLE OF INCIDENCE
   COSI=SDOTN
...
!
   IF(GRATE) THEN
!       WE HAVE A DIFFRACTION GRATING TO DEAL WITH
      GRO=NSSALENS(141,J)
      GRS=NSSALENS(142,J)
      GRX=NSSALENS(143,J)
      GRY=NSSALENS(144,J)
      ...
      &.AND.GRZ.EQ.0.0D0) GRY=1.0D0
      CHINOR=DSQRT((GRX**2)+(GRY**2)+&
      &(GRZ**2))
!       THESE ARE THE DIRECTION COSINES OF THE NORMALS TO THE GRATING
!       GENERATING PLANES
      CHIX=GRX/CHINOR
      CHIY=GRY/CHINOR
      CHIZ=GRZ/CHINOR
!     CALCULATE THE COMPONENTS OF THE VECTOR Q WHICH IS PARALLEL
!     TO THE EFFECTIVE RULINGS
...
!       NO GRATING EXISTS
      ...
      SDOTP=0.0D0
   END IF
!       NOW FOR A AND B GOOD FOR CASES OF GRATINGS OR NO GRATINGS
   A=2.0*MU*SDOTN
   B=(MU**2)-1.0D0-(BIGLAM**2)+(2.0D0*MU*BIGLAM*SDOTP)
   ROOTTERM=(A**2)-(4.0D0*B)
   IF(ROOTTERM.LT.0.0D0) THEN
      IF(.NOT.GRATE) THEN
!       SIMPLE TIR, SET ROOTTERM=0.0D0 AND CONTINUE
         ROOTTERM=0.0D0
      ELSE
!       RAY FAILED DUE TO INREALISTIC ANGLE OF DIFFRACTION
         NSSRAYFAIL=.TRUE.
         NSSRAYFAILCODE(1)=J
         NSSRAYFAILCODE(2)=1
         RETURN
...
!       NOW ROOTTERM >=0
   IF(TRAVEL.EQ.1) THEN
!       DEAL WITH ABSORPTION OR OTHER FAILURE
      IF(NSSALENS(8,J).EQ.3.0D0.OR.NSSALENS(8,J)&
      &.EQ.0.0D0.AND.TIR.OR.DETSNUM.EQ.J) THEN
         NSSRAYFAIL=.TRUE.
         NSSRAY_ENERGY=0.0D0
         NSSRAYFAILCODE(1)=J
         NSSRAYFAILCODE(2)=1
...
!       REFLECTION
   ...
   ELSE
!       TRAVEL=2
!       DEAL WITH REFRACTION FIRST, THEN REFLECTION
...
!       NEW RAY DIRECTION COSINES ARE:
   ...
   NSS_LRAYP(14)=NMN
   NSS_LRAYP(15)=NNN
!       RESET CURRENT INDEX TO THE INDEX AFTER SURFACE INTERACTION
   PREVINDEXR=NSS_N
   CURINDEXR=NSS_NP
!
!       SET THE COATING FILE NUMBERS
!       TRAVEL DIRECTION MEDIA1 TO MEDIA2
   IF(TRAVEL.EQ.1) FILE_NUMBER=INT(NSSALENS(31,J))
!       TRAVEL DIRECTION MEDIA2 TO MEDIA1
   IF(TRAVEL.EQ.2) FILE_NUMBER=INT(NSSALENS(32,J))
   IF(FILE_NUMBER.LT.0.OR.FILE_NUMBER.GT.1000) FILE_NUMBER=0
   ENERGY=NSSRAY_ENERGY
   N=NSS_N
   NP=NSS_NP
!       CALL ENERGY_CALC(ENERGY,N,NP,IA,IAP,INTERACTION_CODE,TIR
!    1,FILE_NUMBER)
...
&INTERACTION_CODE,TIR,FILE_NUMBER)
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!       J=SURFACE NUMBER
!       N=MEDIA INDEX
!       NP=SUBSTRATE INDEX
!       IA=ANGLE OF INCIDENCE
!       IA=ANGLE OF REFRACTION,REFLECTION OR DIFFRACTION
!       INTERACTION_CODE
   INTEGER FILE_NUMBER
   real(real64) N,NP,IA,IAP,VAL,ENERGY,EFFICIENCY
   LOGICAL TIR,EXISF,OPENF
   INTEGER J,INTERACTION_CODE,K
   CHARACTER AI4*4,CFILE_NAME*12
   IF(FILE_NUMBER.LT.0.OR.FILE_NUMBER.GT.1000) FILE_NUMBER=0
   IF(FILE_NUMBER.EQ.0) THEN
!       NO COATING, NO LOSSES,DONT'CHANGE ENERGY
      RETURN
   END IF
!       FILE NUMBER WAS NOT ZERO
!       DOES A CORRESPONDING FILE EXIST
   ...
   IF(.NOT.EXISF) THEN
!       FILE DOES NOT EXIST, NO ENERGY CHANGE
      RETURN
   END IF
!       CORRESPONDING FILE DOES EXIST
...
!
   ...
   IMPLICIT NONE
!
!  RESULT IS .TRUE. IF (X0,Y0) LIES INSIDE THE RECTANGLE FORMED BY THE
!  X,Y PAIRS 1 TO 4
!  OR ON ITS BOUNDARY OR ON ITS POINTS
!  THIS ROUTINE IS USED TO CHECK LIMITS BOX INTERSECTIONS IN NSS RAY TRACING
!  DEFINED BY THE TABLES OF YT VS. XT.
!
   real(real64) TUPI,ANGL(1:4),ARG,X0,X1,X2,X3,X4,Y0,Y1,Y2,Y3,Y4,PII
!
   real(real64) NXT(1:4),NYT(1:4)
!
   INTEGER I
!
...
SUBROUTINE BOUNDER(J)
   ...
   IMPLICIT NONE
   INTEGER J
!       IGNORE BOUNDS ON TUBE SURFACES AS THEY WERE ALREADY CHECKED
   IF(NSSALENS(1,J).EQ.6.0D0) THEN
      RETURN
   END IF
!       IF(J.EQ.25) THEN
!       WRITE(OUTLYNE,*) RRX,RRY,RRZ
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) NSSBOUNDS(1,J),NSSBOUNDS(3,J),NSSBOUNDS(5,J)
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) NSSBOUNDS(2,J),NSSBOUNDS(4,J),NSSBOUNDS(6,J)
!       CALL SHOWIT(1)
!       END IF
   IF(RRX.GE.NSSBOUNDS(1,J).AND.RRX.LE.NSSBOUNDS(2,J)&
   &.AND.RRY.GE.NSSBOUNDS(3,J).AND.RRY.LE.NSSBOUNDS(4,J)&
   &.AND.RRZ.GE.NSSBOUNDS(5,J).AND.RRZ.LE.NSSBOUNDS(6,J))THEN
      NSS_INTERSECT=.TRUE.
   ELSE
      NSS_INTERSECT=.FALSE.
   END IF
...
SUBROUTINE CLAPER(J)
   ...
   LOGICAL INSID1,INS
   EXTERNAL INSID1
   real(real64) LS,RS,X1,X2,X3,X4,Y1,Y2,Y3,Y4,XR,YR,XRD,YRD,A15
!       IGNORE CLAPS ON TUBE SURFACES AS THEY WERE ALREADY CHECKED
   IF(NSSALENS(1,J).EQ.6.0D0) THEN
      RETURN
   END IF
...
SUBROUTINE HOLEER(J)
   ...
   EXTERNAL INSID2
   real(real64) LS,RS,X1,X2,X3,X4,Y1,Y2,Y3,Y4,XR,YR,XRD,YRD,A15
!       IGNORE HOLES ON TUBE SURFACES AS THEY WERE ALREADY CHECKED
   IF(NSSALENS(1,J).EQ.6.0D0) THEN
      RETURN
   END IF
...
SUBROUTINE NSSFLA(J)
   USE NSSMOD
!
!       THIS ROUTINE INTERSECTS AN NSS RAY WITH A FLAT SURFACE
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   INTEGER J
!
   real(real64) NUSUBV,T0
!
!
!       SURFACE IS PLANO AND CONTAINS 2ND, 4TH, 6TH, 8TH AND 10TH ORDER
!       ASPHERIC TERMS.
!       FIRST CALCULATE THE INTERSECTION TO THE SIMPLE PLANE
   T0=RRZ
!       T0/RRN IS THE DISTANCE ALONG THE RAY FROM
...
SUBROUTINE NSSMEM(J)
   USE NSSMOD
!
!       THIS ROUTINE INTERSECTS AN NSS RAY WITH A MEM SURFACE
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
...
SUBROUTINE NSSTUB(J)
   USE NSSMOD
!
!       THIS ROUTINE INTERSECTS AN NSS RAY WITH A TUBE SURFACE
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
...
!
   LOGICAL ONE1,TWO2
!
   real(real64) A,B,C,HV0,HV1,HV2,RRX1,RRY1,RRZ1,RRX2,RRY2,RRZ2
!
   real(real64) Q,ARG,SIGNB
!
!
!       SURFACE IS TUBE
!
!       HIT THE TUBE OF RADIUS R AND LENGTH L
!
   A=(RRL**2)+(RRM**2)
...
!       ARG NOT ZERO
      ...
      INTERS=2
   END IF
!       THE DISTANCE THAT THE RAY TRAVELS UNTIL IT INTERSECTS THE TUBE IS NUSUBV
!       AND IS SUCH THAT AT THAT POINT DSQRT(RRX**2+RRY**2) WILL EQUAL THE TUBE RADIUS
!       NSSALENS(10,J)
   IF(INTERS.EQ.1) THEN
      RRX1=RRX+(HV0*RRL)
      RRY1=RRY+(HV0*RRM)
...
!       CHECK Z-BOUNDS
   ...
   IF(ONE1.AND..NOT.TWO2) THEN
      ...
      RRZ=RRZ+(HV1*RRM)
      NSS_INTERSECT=.TRUE.
!       COMPUTE THE INWARD SURFACE NORMAL
      NSSLN=-2.0*RRX
      NSSMN=-2.0*RRY
      NSSNN=0.0D0
   END IF
...
!       COMPUTE THE INWARD SURFACE NORMAL
   ...
   END IF
90 CONTINUE
!       WRITE(OUTLYNE,*) NSS_INTERSECT
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) RRX,RRY,RRZ
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) NSSLN,NSSMN,NSSNN
!       CALL SHOWIT(1)
   RETURN
END
SUBROUTINE NSSNR1(J)
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NSSNR1.INC. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC PLANO SURFACE
!
   INTEGER J,I
   real(real64) A1,A2,A3,A4,A5,A6,A7,A8,A9,A10
   real(real64) AC,AD,AE,AF,AG,AH,AI,AJ,AK,AL,R,RHO,FUNC,NX,NY
!
   real(real64) LENG,MAG,ZCAL,ZZTOP,X,Y,Z,THETA &
   &,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,DEL
!
!       SURFACE SAG FUNCTION
   FUNC(R,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10)=&
   &(A1*(R**2))+(A2*(R**4))+(A3*(R**6))+(A4*(R**8))&
   &+(A5*(R**10))+(A6*(R**12))+(A7*(R**14))+(A8*(R**16))&
   &+(A9*(R**18))+(A10*(R**20))
!       SURFACE X DERIVATIVE FUNCTION
   NX(X,Y,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10)=&
   &+((1.0D0*2.0D0*X)*A1)&
   &+((2.0D0*((X**2)+(Y**2))*2.0D0*X)*A2)&
   &+((3.0D0*(((X**2)+(Y**2))**2)*2.0D0*X)*A3)&
   &+((4.0D0*(((X**2)+(Y**2))**3)*2.0D0*X)*A4)&
   &+((5.0D0*(((X**2)+(Y**2))**4)*2.0D0*X)*A5)&
   &+((6.0D0*(((X**2)+(Y**2))**5)*2.0D0*X)*A6)&
   &+((7.0D0*(((X**2)+(Y**2))**6)*2.0D0*X)*A7)&
   &+((8.0D0*(((X**2)+(Y**2))**7)*2.0D0*X)*A8)&
   &+((9.0D0*(((X**2)+(Y**2))**8)*2.0D0*X)*A9)&
   &+((10.0D0*(((X**2)+(Y**2))**9)*2.0D0*X)*A10)
!       SURFACE Y DERIVATIVE FUNCTION
   NY(X,Y,A1,A2,A3,A4,A5,A6,A7,A8,A9,A10)=&
   &+((1.0D0*2.0D0*Y)*A1)&
   &+((2.0D0*((X**2)+(Y**2))*2.0D0*Y)*A2)&
   &+((3.0D0*(((X**2)+(Y**2))**2)*2.0D0*Y)*A3)&
   &+((4.0D0*(((X**2)+(Y**2))**3)*2.0D0*Y)*A4)&
   &+((5.0D0*(((X**2)+(Y**2))**4)*2.0D0*Y)*A5)&
   &+((6.0D0*(((X**2)+(Y**2))**5)*2.0D0*Y)*A6)&
   &+((7.0D0*(((X**2)+(Y**2))**6)*2.0D0*Y)*A7)&
   &+((8.0D0*(((X**2)+(Y**2))**7)*2.0D0*Y)*A8)&
   &+((9.0D0*(((X**2)+(Y**2))**8)*2.0D0*Y)*A9)&
   &+((10.0D0*(((X**2)+(Y**2))**9)*2.0D0*Y)*A10)
!
   AC=NSSALENS(3,J)
   AD=NSSALENS(4,J)
   AE=NSSALENS(5,J)
...
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
      ...
      END IF
   END DO
!       INTERATIONS EXCEEDED, NO INTERSECTION WAS POSSIBLE
   NSS_INTERSECT=.FALSE.
   RETURN
END
SUBROUTINE NSSEOS
...
!
   ...
   real(real64) R1,MINX,MAXX,MINY,MAXY,DELX,DELY,X,Y,SAG,MAXZ,MINZ,NUDGE
   EXTERNAL NUDGE
!
!       THIS ROUTINE RESOLVES THE UNRESOLVED PARTS OF AN NSS DATABASE
!       PROGRAM
   ...
   DO J=MAXS,1,-1
      ...
      END IF
   END DO
!       ADDED 3/31/2004 TO SUPPORT I IN NSSTRACE WITH SPOT DIAGRAMS
...
!       MAX HAY INTERSECTIONS PER SURFACE
   IF(NSSSYSTEM(27).LE.0.0D0) NSSSYSTEM(27)=20.0D0
!       SOURCE GRID DIMENSION
   IF(NSSSYSTEM(32).LE.0.0D0) NSSSYSTEM(32)=1.0D0
!       SOURCE GRID DIMENSION
   IF(NSSSYSTEM(36).LE.0.0D0) NSSSYSTEM(36)=1.0D0
!
!       NOW FOR THE SURFACE DATA
!       SET WAVELENGTHS FOR CATALOG LOOKUPS
!
      CALL KDP_EXEC('GLASSWV NSSLENS')
!
   DO J=1,MAXS
!       FIND OUT IF THE CURRENT SURFACE #
!       IS SUPPOSED TO BE IN THE NSS DATABASE
      IF(NSSALENS(100,J).EQ.1.0D0) THEN
!       OUTPUT SURFACE NAME
...
!       OUTPUT SURFACE STRA
         ...
         &.EQ.0.0D0.AND.NSSALENS(91,J).EQ.0.0D0)&
         &NSSALENS(89,J)=1.0D0
!
!       OPTICAL MATERIALS FIRST SPACE
         IF(NSSGLASS1(1,J).EQ.'             '.AND.&
         &NSSGLASS1(2,J).EQ.'             ') THEN
            NSSGLASS1(2,J)(1:13)='AIR          '
         END IF
!       OPTICAL MATERIALS SECOND SPACE
         IF(NSSGLASS2(1,J).EQ.'             '.AND.&
         &NSSGLASS2(2,J).EQ.'             ') THEN
            NSSGLASS2(2,J)(1:13)='AIR          '
         END IF
      ELSE
!       NO OUTPUT FOR THIS SURFACE
      END IF
   END DO
!       NOW DO AUTOMATIC BOUNDS BOXES FOR AS MANY SURFACES
!       AS IT IS POSSIBLE TO DO THEM
   DO J=1,MAXS
      IF(NSSALENS(100,J).EQ.1.0D0) THEN
         IF(NSSALENS(1,J).EQ.4.0D0) THEN
            IF(NSSBOUNDS(1,J).EQ.NSSBOUNDS(2,J).OR.&
            &NSSBOUNDS(3,J).EQ.NSSBOUNDS(4,J).OR.&
            &NSSBOUNDS(5,J).EQ.NSSBOUNDS(6,J)) THEN
               WRITE(OUTLYNE,30) J
               CALL SHOWIT(1)
               WRITE(OUTLYNE,31)
               CALL SHOWIT(1)
...
!
   ...
   END DO
!       RESOLVE OBJECT SPACE GLASS AND SET CURRENT GLASS AND INDEX
!                        CALL NSSGLASSRES(0,0) JRN comment out due to fcn argument
   CALL NSSGLASSRES(0)
!       NOW RESOLVE LINKAGES
   DO J=1,MAXS
      IF(NSSALENS(44,J).NE.DBLE(J).AND.NSSALENS(44,J).NE.0.0D0) THEN
!       RESOLVE SPROFILE LINK
         K=NSSALENS(44,J)
...
!
   INTEGER J
!
   real(real64) Z,X,Y,SAG,C,CT,D,DT,E,ET,F,FT,G,GT,K,KT
   real(real64) D1,E1,MYF1,G1,H1,R1,R2,R3,R4,R,RHO,RHO2,C2
!
   C=NSSALENS(3,J)
   CT=NSSALENS(206,J)
   NSSSAGERR=.FALSE.
...
!       PLANO
      ...
      &((((X**2)+(Y**2))**2)*NSSALENS(4,J))+&
      &((((X**2)+(Y**2))**3)*NSSALENS(5,J))+&
      &((((X**2)+(Y**2))**4)*NSSALENS(6,J))+&
      &((((X**2)+(Y**2))**5)*NSSALENS(7,J))+&
      &((((X**2)+(Y**2))**6)*NSSALENS(201,J))+&
      &((((X**2)+(Y**2))**7)*NSSALENS(202,J))+&
      &((((X**2)+(Y**2))**8)*NSSALENS(203,J))+&
      &((((X**2)+(Y**2))**9)*NSSALENS(204,J))+&
      &((((X**2)+(Y**2))**10)*NSSALENS(205,J))
      RETURN
   END IF
!
...
!
   IF(J.EQ.0) THEN
!       DO OBJECT SPACE INDECIES
      OBJINDEX(1:10)=1.0D0
      OBJINDEX(11:20)=0.0D0
      NAME1=NSSOGLASS(1)
      NAME2=NSSOGLASS(2)
            CALL KDP_EXEC(NAME1//' '//NAME2)
...
SUBROUTINE NSSCALL
   ...
   real(real64) V1,V2,V3,V4,V5,OF1,OF2,V0,TOT,MYX,NUDGE,SAG
   real(real64) X,Y,DELY,DELX,MINX,MINY,MINZ,MAXX,MAXY,MAXZ,R1
   CHARACTER UNI*2,NSTRING*80,PTYPE*40,NNWORD*2,QWORD*13 &
   &,SWORD*13,CTYPE*4,ABL*20,BL80*80,VQ*8
   EXTERNAL NUDGE
   ABL='                    '
   BL80=ABL//ABL//ABL//ABL
!       THIS CALLS ALL NSS DATABASE COMMAND SUBROUTINES AFTER DOING
...
!       NSSINTER
   IF(WC.EQ.'NSSINTER') THEN
      IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
         ...
         &OUTLYNE='"NSSINTER" (SPACE#1) IS SET TO "T"'
         IF(NSSALENS(8,SFID).EQ.2.0D0)&
         &OUTLYNE='"NSSINTER" (SPACE#1) IS SET TO "R"'
         IF(NSSALENS(8,SFID).EQ.3.0D0)&
         &OUTLYNE='"NSSINTER" (SPACE#1) IS SET TO "A"'
         CALL SHOWIT(1)
         IF(NSSALENS(9,SFID).EQ.1.0D0)&
         &OUTLYNE='"NSSINTER" (SPACE#2) IS SET TO "T"'
         IF(NSSALENS(9,SFID).EQ.2.0D0)&
         &OUTLYNE='"NSSINTER" (SPACE#2) IS SET TO "R"'
         IF(NSSALENS(9,SFID).EQ.3.0D0)&
         &OUTLYNE='"NSSINTER" (SPACE#2) IS SET TO "A"'
         CALL SHOWIT(1)
         RETURN
      END IF
...
!       NSSLINK
   IF(WC.EQ.'NSSLINK') THEN
      IF(is_command_query()) THEN
         ...
         &NSSALENS(44,SFID).NE.0.0D0) THEN
            WRITE(OUTLYNE,*)&
            &'"SPROFILE" LINKED TO NSS SURFACE # ',NSSALENS(44,SFID)
            CALL SHOWIT(1)
         END IF
         IF(NSSALENS(45,SFID).NE.DBLE(SFID).AND.&
         &NSSALENS(45,SFID).NE.0.0D0) THEN
            WRITE(OUTLYNE,*)&
            &'"SMEDIA" LINKED TO NSS SURFACE # ',NSSALENS(45,SFID)
            CALL SHOWIT(1)
         END IF
         IF(NSSALENS(46,SFID).NE.DBLE(SFID).AND.&
         &NSSALENS(46,SFID).NE.0.0D0) THEN
            WRITE(OUTLYNE,*)&
            &'"SCOATING" LINKED TO NSS SURFACE # ',NSSALENS(46,SFID)
            CALL SHOWIT(1)
         END IF
         IF(NSSALENS(47,SFID).NE.DBLE(SFID).AND.&
         &NSSALENS(47,SFID).NE.0.0D0) THEN
            WRITE(OUTLYNE,*)&
            &'"SINTERAC" LINKED TO NSS SURFACE # ',NSSALENS(47,SFID)
            CALL SHOWIT(1)
         END IF
         RETURN
      END IF
...
24          FORMAT('UNIVERSE RADIUS = ',G13.6,1X,A2)
      ...
      ELSE
         OUTLYNE=&
         &'NO NSS DATABASE EXISTS. NO TERMINAL RAY DISTANCE CAN BE SET'
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
      RETURN
   ...
   IF(WC.EQ.'SPROFILE') THEN
      IF(NEXISTN) THEN
         IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
            ...
            &PTYPE='PLANO (POSSIBLY WITH ASPHERIC TERMS)'
            IF(NSSALENS(1,SFID).EQ.2.0D0)&
            &PTYPE='SPHERIC (SPHERICAL,CONIC OR ASPHERIC)'
            IF(NSSALENS(1,SFID).EQ.3.0D0)&
            &PTYPE='ANAMORPH (ANAMORPHIC OR ANAMORPHIC ASPHERIC)'
            IF(NSSALENS(1,SFID).EQ.4.0D0)&
            &PTYPE='USER-DEFINED (IN NSSUSER.FOR IN NSSFILE1.FOR)'
            IF(NSSALENS(1,SFID).EQ.5.0D0)&
            &PTYPE='MEM (MULTIPLE ELEMENT MIRROR)'
            IF(NSSALENS(1,SFID).EQ.6.0D0)&
            &PTYPE='TUBE (CYLINDER) SURFACE'
!
...
491         FORMAT('TUBE LENGTH  = ',G13.6)
            IF(NSSALENS(1,SFID).GE.1.0D0.AND.NSSALENS(1,SFID).LE.5.0D0) THEN
               ...
               &WRITE(OUTLYNE,48) NSSALENS(3,SFID)
               IF(NSSALENS(3,SFID).NE.0.0D0)&
               &WRITE(OUTLYNE,48) 1.0D0/NSSALENS(3,SFID)
               CALL SHOWIT(1)
         ...
         IF(SQ.EQ.1) THEN
            ...
            &WQ.NE.'ANAMORPH'.AND.WQ.NE.'TUBE'.AND.&
            &WQ.NE.'USER'.AND.WQ.NE.'MEM') THEN
         ...
         &NSSALENS(1,SFID).EQ.5.0D0) THEN
!       NOW DO AUTOMATIC BOUNDS BOXES FOR SURFACE SFID IF POSSIBLE
...
!       NSSGRIDS
   IF(WC.EQ.'NSSGRIDS') THEN
      IF(NEXISTN) THEN
         IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
            OUTLYNE=&
            &'"NSSGRIDS" SETS THE SOURCE GRID SIZE, SHAPE AND SPACING'
            CALL SHOWIT(1)
...
!       NSSGRIDR
   IF(WC.EQ.'NSSGRIDR') THEN
      IF(NEXISTN) THEN
         IF(is_command_query()) THEN
            OUTLYNE=&
            &'"NSSGRIDR" SETS THE REFERENCE GRID SIZE, SHAPE AND SPACING'
            CALL SHOWIT(1)
...
!       NSSAPODR
   IF(WC.EQ.'NSSAPODR') THEN
      IF(NEXISTN) THEN
         IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
            OUTLYNE=&
            &'"NSSAPODR" SETS THE REFERENCE GRID GAUSSIAN APODIZATION'
            CALL SHOWIT(1)
...
END
SUBROUTINE DEL_NSS
!       DELETES THE CURRENT NSS DATABASE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   ...
   &NSSSNAM,LVECTOR,VECTORL,VECTORM,VECTORN,STAT=ALLOERR)
   NEXISTN=.FALSE.
   SFID=0
   RETURN
END
SUBROUTINE NSS_TRACE
!       PERFORMS A FULL NSS RAY TRACE ON THE CURRENT NSS DATABASE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS TERMINAL RAY DISTANCE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS SURFACE COATING 1 ID NUMBER
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS SURFACE COATING 2 ID NUMBER
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS SURFACE PROFILE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) V0,V1,V2,V3,V4,V5,V6
   NSSALENS(1,SFID)=V0
   IF(V0.EQ.2.0D0.OR.V0.EQ.3.0D0) THEN
!       STORE CURVATURE THOUGH RADIUS WAS INPUT
      IF(V1.EQ.0.0D0)&
      &NSSALENS(3,SFID)=V1
      IF(V1.NE.0.0D0)&
      &NSSALENS(3,SFID)=1.0D0/V1
   END IF
   IF(V0.EQ.4.0D0) THEN
!       USER-DEFINES SURFACE
      NSSALENS(3,SFID)=V1
      NSSALENS(4,SFID)=V2
      NSSALENS(5,SFID)=V3
      NSSALENS(6,SFID)=V4
...
!       SETS NSS SURFACE PROFILE PARAMETERS
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS MAX RAY/SURFACE INTERSECTIONS
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS REFERENCE GRID VALUES
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS REFERENCE GRID APODIZATION VALUE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
...
!       SETS NSS CLAP VALUES
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) V0,V1,V2,V3,V4,V5
   NSSALENS(19,SFID)=V0
   NSSALENS(20,SFID)=V1
   NSSALENS(21,SFID)=V2
   NSSALENS(22,SFID)=V3
...
!       DOUBLES THE SIZE OF THE CURENT NSS DATABASE
   USE NSSMOD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) TMPSYSTEM,TMPALENS,TMPVERTEX,TMPBOUNDS,TMPLVECTOR
   real(real64) TMPVECTORL,TMPVECTORM,TMPVECTORN
   INTEGER J,K
   CHARACTER TMPGLASS1*13,TMPGLASS2*13,TMPSNAM*80
   DIMENSION TMPSYSTEM(:),TMPALENS(:,:),TMPGLASS1(:,:),&
   &TMPVERTEX(:,:),TMPGLASS2(:,:),TMPBOUNDS(:,:)&
   &,TMPLVECTOR(:),TMPVECTORL(:),TMPVECTORM(:),TMPVECTORN(:)
   DIMENSION TMPSNAM(:)
   ALLOCATABLE TMPSYSTEM,TMPALENS,TMPGLASS1,TMPVERTEX,TMPGLASS2,&
   &TMPSNAM,TMPBOUNDS,TMPLVECTOR,TMPVECTORL,TMPVECTORM,TMPVECTORN
   INTEGER ALLOERR
   MAXS=MAXS*2
...
!
   IF(is_command_query()) THEN
      OUTLYNE=&
      &'"NSSSAVE" SAVES THE CURRENT LENS IN AN ASCII FILE'
      CALL SHOWIT(1)
      RETURN
   END IF
...
!     DELETE CURRENT FILE
   ...
   &,FORM='FORMATTED',FILE=NSSDIR//LFILENAME(1:WSCNT)&
   &,STATUS='UNKNOWN')
...
!     RESTORE I/0
   ...
   OUTLYNE=&
   &'CURRENT NSS DATABASE SAVED AS:'//NSSDIR//LFILENAME(1:WSCNT)
   CALL SHOWIT(1)
   RETURN
END
SUBROUTINE NSSREST
...
!
   WRITE(OUTLYNE,*)&
   &'RESTORING NSS DATABASE FILE '//NSSDIR//LFILENAME(1:WSCNT)//&
   &' PLEASE WAIT...'
   CALL SHOWIT(1)
!     RESTORE
   SAVE_KDP(1)=SAVEINPT(1)
...
!     RESTORE I/0
   REST_KDP(1)=RESTINPT(1)
   OUTLYNE=&
   &'NSS DATABASE SAVED AS: '//NSSDIR//LFILENAME(1:WSCNT)//&
   &' HAS BEEN RESTORED'
   CALL SHOWIT(1)
   CALL NSSEOS
   RETURN
...
!     AND PLOT VIEW FOR THE NSS PLOTTING COMMANDS USING VERTEX DATA
!
   real(real64) X,Y,Z &
...
!
   ...
   IMPLICIT NONE
!
!       THIS ROUTINE DOES THE AUTO SCALE FACTOR FOR NSS PLOTTING
!
!
   IF(NSSSCFA.EQ.0.0D0) THEN
      WRITE(OUTLYNE,*)&
      &'THE NSS PLOT SCALE FACTOR IS ZERO AND WILL BE RESET TO 1.0'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'USE "PLOT NSSSCALE" TO SET IT TO A USER SPECIFIED VALUE'
      CALL SHOWIT(1)
            CALL KDP_EXEC('PLOT NSSSCALE 1')
   END IF
!
   RETURN
END
! USED FOR NSS CLAP PLOTTING
...
!
   INTEGER CAFLG,I
!
   real(real64) X,Y,GAM,THETA1,THETA2,THETA3,THETA4

   real(real64) A,B,RAD,XR,YR,ANGLE,AN2,XCOR1,XCOR2,XCOR3,XCOR4,DA,DB

   real(real64) YCOR1,YCOR2,YCOR3,YCOR4,A1TEST,A2TEST,QTEST,Q,JK_AA,JK_BB

   real(real64) XA,YA,XB,YB,M,RADIUS,CBX,CBY,SGNB,CEE,ZEE,KAPPA,RHO2,RHO

   real(real64) XDIM,YDIM,XOFF,YOFF,CANG
!
!
   CAFLG=INT(NSSALENS(19,I))
   XDIM=NSSALENS(20,I)
...
!     APPLY THE CLAP TILT ANGLE AND THE CLAP DECENTRATIONS
   ...
   RETURN
END
! USED FOR NSS TUBE CLAP PLOTTING (AND ALSO PROFILE PLOTTING)
...
!
   real(real64) C1,C2,C3,C4,ACLENG,DEL,NSSFNZ3 &
   &,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,CV,ZTEST,SNGX,SNGY,SNGZ,&
   &LENG,MAG,ZCAL,QQ,C5,C6,C8,C10,C11,C12,C13,C14,&
   &CY,KY,EY,FY,DY,GY,CX,KX,DX,EX,FX,GX,ZZTOP &
   &,NSSDER3X,NSSDER3Y,NSSFUNC3X,NSSFUNC3Y,ERR,XPASS,YPASS,ZPASS
   real(real64) R1,R2,RD
   INTEGER JPASS
   LOGICAL ERRFLAG
...
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION NSSFNZ3 EVALUATES THE FUNCTION
!
!     JRN add merge to fix conversion error
...
!
   ...
   IMPLICIT NONE
!
   real(real64) NSSFNZ3,AX,AY,ACX,ACY,ADX,ADY,AACX,AACY,AADX &
   &,AADY,AAX,AAY,NSSFNZZ3,R,THETA,AQ,Q,AEX,AEY,&
   &AAEX,AAEY,AAFX,AAFY,AAGX,AAGY,AFX,AFY,AGX,AGY,XX,YY &
   &,AAAX,AAAY
   real(real64) JK_WAVE,AMP,OMEGAX,OMEGAY
!
!
   INTEGER I
!
   ...
   &AAGX,AAGY,AQ)=&
   &(((AACX*(AAX**2))+(AACY*(AAY**2)))/AQ)&
   &+(AADY*((((1.0D0-AADX)*(AAX**2))+((1.0D0+AADX)*(AAY**2)))**2))&
   &+(AAEY*((((1.0D0-AAEX)*(AAX**2))+((1.0D0+AAEX)*(AAY**2)))**3))&
   &+(AAFY*((((1.0D0-AAFX)*(AAX**2))+((1.0D0+AAFX)*(AAY**2)))**4))&
   &+(AAGY*((((1.0D0-AAGX)*(AAX**2))+((1.0D0+AAGX)*(AAY**2)))**5))
!
...
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) NSSDER3X,DELH,X,NSSFUNC3X
   LOGICAL DERERROR
...
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) NSSDER3Y,DELH,Y,NSSFUNC3Y
   LOGICAL DERERROR
...
!
   EXTERNAL NSSFNZ3
!
   real(real64) NSSFUNC3X,AX,NSSFNZ3,C1,C2,C3,C4,C5,C6,C8 &
   &,C10,C11,C12,C13,C14,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY &
   &,QQ
!
!
!     AD
...
!
   EXTERNAL NSSFNZ3
!
   real(real64) NSSFUNC3Y,AY,NSSFNZ3,C1,C2,C3,C4,C5,C6,C8 &
   &,C10,C11,C12,C13,C14,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY &
   &,QQ
!
!
...
SUBROUTINE NSS_FIND_SURFACE(JJ)
   ...
   real(real64) NSSLTEST
!       THIS ROUTINE FINDS THE SURFACE NUMBER FOR WHICH THE RAY TRAVELS THE LEAST,
!       POSITIVE DISTANCE WITH THE RAY INSIDE BOUNDS AND CLAP BUT NOT IN HOLE.
!       LASTSUR IS THE HIGHEST SURFACE NUMBER IN THE CURRENT NSS DATABASE
!       SET THE PERMANENT GLOBAL RAY DATA BEFORE THE SEARCH
...
!       WHERE 1 AND 2 REFER TO THE SPACES ON EACH SIDE OF THE SURFACE
      ...
      IMAGINDEX1=NSSALENS(110+NSSWAVNUM,J)
      IMAGINDEX2=NSSALENS(130+NSSWAVNUM,J)
!       WE ALREADY KNOW WHAT THE CURINDEXR AND CURINDXI ARE!
!       CHECK THAT THE NEXT SURFACE IS ACTIVE
      IF(NSSALENS(100,J).EQ.1.0D0) THEN
!       SURFACE IS TO BE CONSIDERED FOR POSSIBLE INTERSECTION
!       TRANSFORM THE CURRENT GLOBAL RAY DATA INTO THE COORDINATE SYSTEM OF SURFACE J
!       THIS DOES NOT CHANGE THE RAY DATA IN THE PERMANENT RAY STORAGE LOCATIONS
         JJJ=J
         CALL NSS_FNDTRANSF(JJJ)
!       THE ARRAY NSS_LRAY NOW HOLDS THE DATA FOR THE RAY IN THE LOCAL COORDINATE
!       SYSTEM OF SURFACE J. DOES THE RAY INTERSECT THIS SURFACE
!       INSIDE THE SURFACE BOUNDS OF THE SURFACE. IF IT DOES AND THE
!       OPL IS LESS THAN THE LAST SAVED OPL, THEN REPLACE THE PROVIOUS SURFACE
!       AS THE BEST SURFACE INTERSECTION CANDIDATE, ELSE PROCEED TO THE NEXT SURFACE.
         JJJ=J
         CALL NSS_FIND_PART2(JJJ)
      ELSE
!       SKIP THAT SURFACE NUMBER
      END IF
   END DO
!       WE NOW HAVE THE LIST OF ALL SURFACES WITH AN ASSOCIATED LVECTOR
!       WHICH IS THE DISTANCE FROM THE STARTING RAY LOCATION IN GLOBAL
!       COORDINATES TO THE GLOBAL COORDINATE POSITION OF THE RAY AT EACH
!       SURFACE.
   DO JJJJ=1,LASTSUR
      IF(LVECTOR(JJJJ).GT.0.0D0.AND.&
      &LVECTOR(JJJJ).LT.NSSLTEST) THEN
         NSSLTEST=LVECTOR(JJJJ)
         NSSJTEST=JJJJ
      END IF
   END DO
!                       DO JJJJ=1,LASTSUR
!       WRITE(OUTLYNE,*)JJJJ,LVECTOR(JJJJ),NSSALENS(100,JJJ)
!       CALL SHOWIT(1)
!                       END DO
!       DID WE MISS EVERYONE?
   DO JJJJ=1,LASTSUR
      IF(LVECTOR(JJJJ).GT.0.0D0) GO TO 10
   END DO
!       IF WE GOT HERE, THE RAY MISSED ALL SURFACES SO WE SET NSSJTEST = MAXS+1
   NSSJTEST=MAXS+1
!       NOW THE VALUE OF JJ IS THE NEXT SURFACE NUMBER WHICH SHOULD BE
!       INTERSECTED AND INTERACTED WITH.
10 JJ=NSSJTEST
   RETURN
END
!
SUBROUTINE NSS_FIND_PART2(J)
   ...
   use DATMAI
   INTEGER J,JK1
   real(real64) RTESTINDEX,ITESTINDEX
   real(real64) GX0,GY0,GZ0,GX,GY,GZ,MAG,GL0,GM0,GN0,GL,GM,GN
   NSS_INTERSECT=.TRUE.
   RRX=LRRX
   RRY=LRRY
   RRZ=LRRZ
...
!       THE RAY DID NOT HIT THE SURFACE
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!       write(outlyne,*) 'herea fail',J
!       call showit(1)
!               END IF
      RETURN
   END IF
!        THE RAY CAN HIT THE SURFACE, PROCEED
!
!        IF(INT(NSSALENS(1,J)).EQ.4) CALL NSSUSER(J)
!
!        IS THE ONE INTERSECTION POINT A VALID ONE WITH RESPECT TO
!        THE MATERIAL IN WHICH IT WAS TRAVELING?
!        THE CURRENT INDEX IN WHICH THE RAY THINKS IT IS TRAVELING IS:
!                       CURINDEXR
!                       CURINDEXI
!        IS THE RAY APPROACHING THE SURFACE IN A SPACE WHICH HAS
!        A MATCHING INDEX?
!        COMPUTE THE COSINE OF THE ANGLE BETWEEN THE LOCAL RAY DIRECTION AND
!        THE SURFACE NORMAL
   IF(RRN.EQ.0.0D0) RRN=1.0D-10
   IF(RRN.GT.0.0D0) TRAVEL=1
   IF(RRN.LT.0.0D0) TRAVEL=2
   IF(TRAVEL.EQ.1) THEN
!        THE RAY IS TRAVELING TOWARD THE INTERFACE IN THE #1 MATERIAL
      TRAVEL=1
      RTESTINDEX=REALINDEX1
      ITESTINDEX=IMAGINDEX1
   ELSE
!        THE RAY IS TRAVELING TOWARD THE INTERFACE IN THE #2 MATERIAL
      TRAVEL=2
...
!       MATERIALS MATCH
      NSS_INTERSECT=.TRUE.
   ELSE
!       MATERIALS DONT MATCH
!       NOT A VALID INTERSECTION, TRY ANOTHER SURFACE
      NSS_INTERSECT=.FALSE.
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!       write(outlyne,*) 'hereb mat mismatch',J
!       call showit(1)
!               END IF
      RETURN
   END IF
!        NOW WE HAVE ONE SURFACE INTERSECTION POINT FOR THE SURFACE
!        IN LOCAL COORDINATES.
   CALL BOUNDER(J)
   IF(.NOT.NSS_INTERSECT)THEN
!       RAY HITS SURFACE OUTSIDE SURFACE BOUND, NOT A VALID INTERSECTION
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!       write(outlyne,*) 'herec outside bound',J
!       call showit(1)
!               END IF
      RETURN
   ELSE
   ...
   CALL CLAPER(J)
   IF(.NOT.NSS_INTERSECT)THEN
!       RAY HITS SURFACE OUTSIDE CLEAR APERTURE, NOT A VALID INTERSECTION
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!        write(outlyne,*) 'hered outside clap',J
!        call showit(1)
!               END IF
      RETURN
   ELSE
   ...
   CALL HOLEER(J)
   IF(.NOT.NSS_INTERSECT)THEN
!       RAY HITS SURFACE INSIDE HOLE, NOT A VALID INTERSECTION
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!        write(outlyne,*) 'heree inside hole',J
!        call showit(1)
!               END IF
      RETURN
   ELSE
   END IF
!       RAY HAS A VALID INTERSECTION INSIDE BOUNDS AND CLAP AND OUTSIDE HOLE
!       AND INDEX VALUES MATCH.
!       COMPUTE LVECTOR, THIS IS THE DISTANCE IN GLOBAL COORDINATES
!       BETWEEN THE STARTING GLOBAL RAY POSITION AND THE GLOBAL POSITION
!       AT THE INTERSECTION. WE ALREADY KNOW THE DIRECTION IS OK OR
!       LVECTOR(J) WOULD HAVE ALREADY BEEN SET TO -1.0D0.
!       COMPUTE THE GLOBAL COORDINATES GIVEN THE RRX0, RRY0, RRZ0, RX, RY, RZ
!       THE GX0,GY0,GZ0 WILL BE THE RETURNED GLOBAL RAY COORDINATES
!       BEFORE THE INTERSECTION, GX, GY AND GZ WILL BE AFTER INTERSECTION.
   GX0=RRX0
   GY0=RRY0
   GZ0=RRZ0
   GL0=RRL0
...
!
!       LVECTOR IS GLOBAL RAY TRAVEL TO INTERSECTION POINT.
...
!       TAKEN OUT ON 5/14/2004
!       WRITE(OUTLYNE,*) J
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) GL,GM,GN
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*) VECTORL(J),VECTORM(J),VECTORN(J)
!       CALL SHOWIT(1)
      IF(REAL(VECTORL(J)).NE.REAL(GL).OR.&
      &REAL(VECTORM(J)).NE.REAL(GM).OR.&
      &REAL(VECTORN(J)).NE.REAL(GN)) THEN
         LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!        write(outlyne,*) 'heref',J
!        call showit(1)
!               END IF
      END IF
   ELSE
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!        write(outlyne,*) 'hereg lvector < 0',J
!        call showit(1)
!               END IF
   END IF
   IF(DABS(LVECTOR(J)).LT.1.0D-8) THEN
      LVECTOR(J)=-1.0D0
!       IF(J.EQ.25) THEN
!        write(outLlyne,*) 'hereh lvect too small',J
!        call showit(1)
!               END IF
   END IF
!       AS WE RETURN, WE HAVE A RAY WHICH STRICKS A SURFACE AFTER TRAVELING
!       A NON-ZERO POSITIVE DISTANCE AND WHICH HITS THE J SURFACE INSIDE
!       BOUNDS, CLAPS AND OUTSIDE HOLES OR IT HAS A NEGATIVE LVECTOR.
   RETURN
END
SUBROUTINE NSSNR2(J)
   USE NSSMOD
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NSSNR2.INC. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC SURFACE
!
   INTEGER I,J
   real(real64) A4,A6,A8,A10,A12,A14,A16,A18,A20,QQ
   real(real64) CV,AD,AE,AF,AG,AH,AI,AJ,AK,AL,R,RHO,FUNC,NX,NY
   real(real64) CC,AQ,Q,C,K,RTEST
!
   real(real64) LENG,MAG,ZCAL,ZZTOP,X,Y,Z,THETA &
   &,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,DEL
!
!       SURFACE SAG FUNCTION
   ...
   &(((R**2)*C)/(1.0D0+DSQRT(1.0D0-((K+1.0D0)*(C**2)*(R**2)))))&
...
!       SURFACE X DERIVATIVE FUNCTION
   ...
   &(((2.0D0*C*X*QQ*(QQ-1.0D0))+&
   &((C**3)*X*((X**2)+(Y**2))*(K+1.0D0)))/((QQ-1.0D0)*(QQ**2)))&
   &+((2.0D0*((X**2)+(Y**2))*2.0D0*X)*A4)&
   &+((3.0D0*(((X**2)+(Y**2))**2)*2.0D0*X)*A6)&
   &+((4.0D0*(((X**2)+(Y**2))**3)*2.0D0*X)*A8)&
   &+((5.0D0*(((X**2)+(Y**2))**4)*2.0D0*X)*A10)&
   &+((6.0D0*(((X**2)+(Y**2))**5)*2.0D0*X)*A12)&
   &+((7.0D0*(((X**2)+(Y**2))**6)*2.0D0*X)*A14)&
   &+((8.0D0*(((X**2)+(Y**2))**7)*2.0D0*X)*A16)&
   &+((9.0D0*(((X**2)+(Y**2))**8)*2.0D0*X)*A18)&
   &+((10.0D0*(((X**2)+(Y**2))**9)*2.0D0*X)*A20)&
   &)
!
!       SURFACE Y DERIVATIVE FUNCTION
   NY(X,Y,C,K,A4,A6,A8,A10,A12,A14,A16,A18,A20,QQ)=(&
   &(((2.0D0*C*Y*QQ*(QQ-1.0D0))+&
   &((C**3)*Y*((X**2)+(Y**2))*(K+1.0D0)))/((QQ-1.0D0)*(QQ**2)))&
   &+((2.0D0*((X**2)+(Y**2))*2.0D0*Y)*A4)&
   &+((3.0D0*(((X**2)+(Y**2))**2)*2.0D0*Y)*A6)&
   &+((4.0D0*(((X**2)+(Y**2))**3)*2.0D0*Y)*A8)&
   &+((5.0D0*(((X**2)+(Y**2))**4)*2.0D0*Y)*A10)&
   &+((6.0D0*(((X**2)+(Y**2))**5)*2.0D0*Y)*A12)&
   &+((7.0D0*(((X**2)+(Y**2))**6)*2.0D0*Y)*A14)&
   &+((8.0D0*(((X**2)+(Y**2))**7)*2.0D0*Y)*A16)&
   &+((9.0D0*(((X**2)+(Y**2))**8)*2.0D0*Y)*A18)&
   &+((10.0D0*(((X**2)+(Y**2))**9)*2.0D0*Y)*A20)&
   &)
!
!       Q FUNCTION
   Q(X,Y,C,K)=&
   &(1.0D0+DSQRT(1.0D0-((K+1.0D0)*(C**2)*((X**2)+(Y**2)))))
!
!
!
   CV=NSSALENS(3,J)
...
!
   ...
   IMPLICIT NONE
!
!       THIS ROUTINE DOES THE PLOT NSSSCALE
