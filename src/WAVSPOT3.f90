...
!       WAS A FOB ISSUED
   IF(WC.EQ.'ISPD'.OR.WC.EQ.'ISPDA') THEN
      ...
      IF(W1.LE.1.0D0.AND.DF1.EQ.0) THEN
         ...
         & 'W1=NUMBER OF RAYS TO TRACE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.LE.0.0D0.AND.WC.EQ.'ISPDA') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"ISPDA" REQUIRES POSITIVE NUMERIC WORD #2 INPUT'//'\n'//&
         & 'W2=MAXIMUM CONE HALF-ANGLE'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.GT.180.0D0.AND.WC.EQ.'ISPDA') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'MAX ELEVATION ANGLE INPUT FOR "ISPDA" IS 180 DEGREES'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!
   ...
   IF(WC.EQ.'SPD') THEN
      ...
      &.NOT.FOBYES.AND.WQ.EQ.'IPSTAT') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO "FOB" EXISTS TO USE IN THE SPOT ANALYSIS'//'\n'//&
         & 'ISSUE A "FOB" BEFORE ISSUING "SPD" COMMANDS', 1)
         RETURN
      END IF
   END IF
   IF(WC.EQ.'PSFK') THEN
      ...
      &.OR..NOT.FOBYES.AND.WQ.EQ.'PERFNOOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO "FOB" EXISTS TO BE USE'//'\n'//&
         & 'ISSUE A "FOB" BEFORE ISSUING THE "PSF" COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WC.EQ.'PUPIL') THEN
      ...
      &.OR..NOT.FOBYES.AND.WQ.EQ.'PERFNOOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO "FOB" EXISTS TO BE USE'//'\n'//&
         & 'ISSUE A "FOB" BEFORE ISSUING THE "PUPIL" COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WC.EQ.'CAPFN') THEN
      ...
      &.OR..NOT.FOBYES.AND.WQ.EQ.'NOSCALE') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO "FOB" EXISTS TO BE USE'//'\n'//&
         & 'ISSUE A "FOB" BEFORE ISSUING THE "CAPFN" COMMAND', 1)
         RETURN
      END IF
   END IF
!       WAS FOB NULL USED
...
!
   IF(sys_mode().EQ.3.0D0.OR.sys_mode().EQ.4.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPD (MOVE OR MOVEACC)"'//'\n'//&
      & 'ONLY WORKS FOR MODES FOCAL AND UFAOCAL'//'\n'//&
      & 'NO ACTION TAKEN', 1)
      RETURN
   END IF
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SPD (MOVE OR MOVEACC)" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!       DEFAULT NUMERIC INPUT, ALL DEFAULTS ARE ZERO
   IF(DF1.EQ.1) THEN
      ...
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('WORD #1 INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
