...
!       WAS A FOB ISSUED
   IF(WC.EQ.'ISPD'.OR.WC.EQ.'ISPDA') THEN
      IF(.NOT.FOBYES) THEN
         ...
         CALL SHOWIT(1)
         IF(WC.EQ.'ISPD')OUTLYNE='ISSUE A "FOB" BEFORE ISSUING "ISPD"'
         IF(WC.EQ.'ISPDA')OUTLYNE='ISSUE A "FOB" BEFORE ISSUING "ISPDA"'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      ...
      IF(SST.EQ.1.OR.SQ.EQ.1) THEN
         IF(WC.EQ.'ISPD')OUTLYNE='"ISPD" TAKES NO STRING OR QUALIFIER INPUT'
         IF(WC.EQ.'ISPDA')OUTLYNE='"ISPDA" TAKES NO STRING OR QUALIFIER INPUT'
         CALL SHOWIT(1)
      ...
      IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         IF(WC.EQ.'ISPD')OUTLYNE='"ISPD" TAKES NO NUMERIC WORD #3, #4 OR #5 INPUT'
         IF(WC.EQ.'ISPDA')OUTLYNE='"ISPDA" TAKES NO NUMERIC WORD #3, #4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(WC.EQ.'ISPD'.AND.S2.EQ.1) THEN
         IF(WC.EQ.'ISPD')OUTLYNE='"ISPD" TAKES NO NUMERIC WORD #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(W1.LE.1.0D0.AND.DF1.EQ.0) THEN
         IF(WC.EQ.'ISPD')OUTLYNE='"ISPD" REQUIRES POSITIVE NUMERIC WORD #1 INPUT'
         IF(WC.EQ.'ISPDA')OUTLYNE='"ISPDA" REQUIRES POSITIVE NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
...
!
   ...
   END IF
   IF(WC.EQ.'SPD') THEN
      IF(.NOT.FOBYES.AND.SQ.EQ.0.OR..NOT.FOBYES.AND.WQ.EQ.'ACC'.OR..NOT.FOBYES.AND.WQ.EQ.'ISTAT'.OR..NOT.FOBYES.AND.WQ.EQ.'IPSTAT') THEN
   ...
   END IF
   IF(WC.EQ.'PSFK') THEN
      IF(.NOT.FOBYES.AND.SQ.EQ.0 .OR..NOT.FOBYES.AND.WQ.EQ.'PERFECT'.OR..NOT.FOBYES.AND.WQ.EQ.'PERFNOOB') THEN
   ...
   END IF
   IF(WC.EQ.'CAPFN') THEN
      IF(.NOT.FOBYES.AND.SQ.EQ.0 .OR..NOT.FOBYES.AND.WQ.EQ.'PERFECT'.OR..NOT.FOBYES.AND.WQ.EQ.'SILENT'.OR..NOT.FOBYES.AND.WQ.EQ.'NOSCALE') THEN
         OUTLYNE='NO "FOB" EXISTS TO BE USE'
         CALL SHOWIT(1)
...
!       CHECK FOR CORRECT QUALIFIER WORDS
   ...
   IF(WC.EQ.'SPD'.AND..NOT.SYES) THEN
      OUTLYNE='INVALID QUALIFIER WORD USED WITH "SPD" COMMAND'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'CAPFN'.AND..NOT.SYES) THEN
      OUTLYNE='INVALID QUALIFIER WORD USED WITH "CAPFN" COMMAND'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'CAPFN'.AND.STI.EQ.1) THEN
      OUTLYNE='"CAPFN" CREATES A COMPLEX APERTURE FUNCTION'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WC.EQ.'PSFK'.AND..NOT.SYES) THEN
      OUTLYNE='INVALID QUALIFIER WORD USED WITH "PSF" COMMAND'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'PUPIL'.AND..NOT.SYES) THEN
      OUTLYNE='INVALID QUALIFIER WORD USED WITH "PUPIL" COMMAND'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'PSFK'.AND.STI.EQ.1) THEN
      OUTLYNE='"PSF" CREATES A DIFFRACTION POINT SPREAD FUNCTION'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WC.EQ.'PSFK'.AND.STI.EQ.1) THEN
      OUTLYNE='"PSF" CREATES A PUPIL FUNCTION IN FOE MEMORY'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WC.EQ.'SPD'.AND.STI.EQ.1) THEN
      OUTLYNE='"SPD" CREATES A SPOT DIAGRAM'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WC.EQ.'CAPFN'.AND.SST.EQ.1) THEN
      OUTLYNE='"CAPFN" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'PSFK'.AND.SST.EQ.1) THEN
      OUTLYNE='"PSF" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'PUPIL'.AND.SST.EQ.1) THEN
      OUTLYNE='"PUPIL" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'PUPIL') THEN
      IF(S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PUPIL" TAKES NO NUMERIC WORD #4 TO #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   IF(WC.EQ.'SPD'.AND.SST.EQ.1) THEN
      OUTLYNE='"SPD" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!                       NOT CAPFN PERFECT
   END IF
   IF(WC.EQ.'PSFK'.AND.WQ.NE.'PERFECT'.AND.WC.EQ.'PSFK'.AND.WQ.NE.'PERFNOOB') THEN
!     OPMAP FALSE MEANS NO OPD MAP FITTING COEFFICIENTS EXIST
      NOOB=.FALSE.
      FIXWL=.FALSE.
      IF(DF2.EQ.0) THEN
...
!                       NOT PSF
   END IF
   IF(WC.EQ.'PSFK'.AND.WQ.EQ.'PERFECT'.OR.WC.EQ.'PSFK'.AND.WQ.EQ.'PERFNOOB') THEN
!     OPMAP FALSE MEANS NO OPD MAP FITTING COEFFICIENTS EXIST
!                      OPMAP=.FALSE.
      FIXWL=.FALSE.
      IF(DF2.EQ.0) THEN
...
!                       NOT PSF PERFECT
   END IF
   IF(WC.EQ.'PUPIL'.AND.WQ.NE.'PERFECT'.AND.WC.EQ.'PUPIL'.AND.WQ.NE.'PERFNOOB') THEN
!     OPMAP FALSE MEANS NO OPD MAP FITTING COEFFICIENTS EXIST
      NOOB=.FALSE.
      FIXWL=.FALSE.
      IF(DF2.EQ.0) THEN
...
!                       NOT PUPIL
   END IF
   IF(WC.EQ.'PUPIL'.AND.WQ.EQ.'PERFECT'.OR.WC.EQ.'PUPIL'.AND.WQ.EQ.'PERFNOOB') THEN
!     OPMAP FALSE MEANS NO OPD MAP FITTING COEFFICIENTS EXIST
!                      OPMAP=.FALSE.
      FIXWL=.FALSE.
      IF(DF2.EQ.0) THEN
...
!       CALL THE CORRECT ROUTINE
   IF(WC.EQ.'SPD') THEN
      IF(WQ.EQ.' '.OR.WQ.EQ.'ACC'.OR.WQ.EQ.'ISTAT'.OR.WQ.EQ.'IPSTAT') THEN
         SPOTSSI=.FALSE.
         DETER=.FALSE.
         DETCENT=.FALSE.
         DETCHIEF=.TRUE.
...
!
   REAL*8 STAR,ENDD,ANGVAL,STARANG,ENDANG,DELANG ,YOBP,XOBP
!
...
200 FORMAT('CALCULATING ANGLE OF REFRACTION, REFLECTION AND DIFFRACTION',' STATISTICS')
...
400 FORMAT('ANGLE OF REFRACTION, REFLECTION AND DIFFRACTION STATISTICS')
402 FORMAT(7X,'FROM',11X, 'TO',8X,'SUCCESSFUL RAYS')
406 FORMAT(5X,'DEGREES(S)',5X, 'DEGREE(S)')
403 FORMAT('SUCCESSFUL RAYS TRACED = ',I10)
404 FORMAT('SUCCESSFUL RAYS IN SPECIFIED RANGE = ',I10)
407 FORMAT('SPECIFIED ANGULAR RANGE = ',G15.8,'DEGREE(S)')
408 FORMAT('STATISTICS CALCULATED FOR SURFACE # = ',I4)
409 FORMAT('Y-OBJECT POINT = ',G15.8,1X,A9)
410 FORMAT('X-OBJECT POINT = ',G15.8,1X,A9)
...
!
   ...
   IMPLICIT NONE
!
   REAL*8 SPT,V1,VALUE ,JA,JB,XV,YV,XYANG,RRAD,RRANG,RPNT,RANDX ,RESLT,VXLO,VXHI,VYLO,VYHI,WOW1,WOW2,WAY,SPOTTY
!
   DIMENSION SPOTTY(:)
!
   ALLOCATABLE :: SPOTTY
...
!
   INTEGER KIK,KIKI
!
   REAL*8 NSTART1,NSTOP1,NDEL1,NSTART2,NSTOP2,NDEL2,TOT,W,AMSS,IX,IY ,X,Y,MSSX,MSSY,AMSSX,AMSSY
!
   REAL*8 MAX ,FRAC,FRACA,FRACB,MSS,APFAC
!
   INTEGER I,J,IWL
!
   LOGICAL ODD,FOBB0 ,FOBB0X,FOBB0Y
!
...
!     THIS LOOPS THROUGH THE POINTS IN EACH RING
            DO K=1,RPNT
               ...
               IF(JJ.NE.0) THEN
                  RRANG=((TWOPII)/360.0D0)*OPRINGANG(JJ)+(DBLE(K-1)*((TWOPII)/DBLE(OPRINGPNT(JJ))))
               END IF
               WW1=RRAD*DSIN(RRANG)
               WW2=RRAD*DCOS(RRANG)
               I=I+1
...
!     AFOCAL
                  ...
                  SPOTTY(2)=VALUE
                  SPOTTY(3)=DSPOT(12)
                  IF(DABS(DIFF(4,NEWIMG)).GE.(1.0D35*DABS(DIFF(6,NEWIMG)))) THEN
                     IF(REAL(DIFF(4,NEWIMG)).GE.0.0) V1=PII/2.0D0
                     IF(REAL(DIFF(4,NEWIMG)).LT.0.0) V1=(3.0D0*PII)/2.0D0
                  ELSE
                     IF(DABS(DIFF(4,NEWIMG)).EQ.0.0D0.AND.DABS(DIFF(6,NEWIMG)).EQ.0.0D0) THEN
                        V1=0.0D0
                     ELSE
                  ...
                  IF(DABS(DIFF(10,NEWIMG)).GE.(1.0D35*DABS(DIFF(12,NEWIMG)))) THEN
                     IF(REAL(DIFF(10,NEWIMG)).GE.0.0) V1=PII/2.0D0
                     IF(REAL(DIFF(10,NEWIMG)).LT.0.0) V1=(3.0D0*PII)/2.0D0
                  ELSE
                     IF(DABS(DIFF(10,NEWIMG)).EQ.0.0D0.AND.DABS(DIFF(12,NEWIMG)).EQ.0.0D0) THEN
                        V1=0.0D0
                     ELSE
                  ...
                  IF(DABS(DIFF(5,NEWIMG)).GE.(1.0D35*DABS(DIFF(6,NEWIMG)))) THEN
                     IF(REAL(DIFF(5,NEWIMG)).GE.0.0) V1=PII/2.0D0
                     IF(REAL(DIFF(5,NEWIMG)).LT.0.0) V1=(3.0D0*PII)/2.0D0
                  ELSE
                     IF(DABS(DIFF(5,NEWIMG)).EQ.0.0D0.AND.DABS(DIFF(6,NEWIMG)).EQ.0.0D0) THEN
                        V1=0.0D0
                     ELSE
                  ...
                  IF(DABS(DIFF(11,NEWIMG)).GE.(1.0D35*DABS(DIFF(12,NEWIMG)))) THEN
                     IF(REAL(DIFF(11,NEWIMG)).GE.0.0) V1=PII/2.0D0
                     IF(REAL(DIFF(11,NEWIMG)).LT.0.0) V1=(3.0D0*PII)/2.0D0
                  ELSE
                     IF(DABS(DIFF(11,NEWIMG)).EQ.0.0D0.AND.DABS(DIFF(12,NEWIMG)).EQ.0.0D0) THEN
                        V1=0.0D0
                     ELSE
               ...
               SLOPES=DABS(SPOTTY(3))*(DABS(SPOTTY(6))+DABS(SPOTTY(7))+DABS(SPOTTY(8))+DABS(SPOTTY(9)))
...
!
   REAL*8 SPT,TOT,W,JK_S ,MSSX,MSSY,JA,JB
!
   REAL*8 SPDELL,MSS,DELTA,APFAC
!
   INTEGER I,J,NUMT1,NUMT2,NUMT3,NUMT4,NUMT5,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10,ALLOERR
!
...
!       DEFAULT NUMERIC INPUT, ALL DEFAULTS ARE ZERO
   IF(DF1.EQ.1) THEN
      OUTLYNE='"SPD (MOVE OR MOVEACC)" REQUIERS EXPLICIT NUMERIC'
      CALL SHOWIT(1)
...
!       X ONLY
            ...
            SSSPX=SSSPX+(JK_S*(((LA-SPA)**2))*APFAC)
            SSSQX=SSSQX+(JK_S*((LA*LB)-(LA*SPB)-(LB*SPA)+(SPA*SPB))*APFAC)
            SSSRX=SSSRX+(JK_S*(((LB-SPB)**2))*APFAC)
!       Y ONLY
            SSSPY=SSSPY+(JK_S*(((LC-SPC)**2))*APFAC)
            SSSQY=SSSQY+(JK_S*((LC*LD)-(LC*SPD)-(LD*SPC)+(SPC*SPD))*APFAC)
            SSSRY=SSSRY+(JK_S*(((LD-SPD)**2))*APFAC)
         ELSE
         END IF
      ELSE
      END IF
...
107 FORMAT('"FOB NULL" WAS USED, CHIEF RAY DATA MAY BE SUSPECT')
...
!       DO THE PRINT OUT
      ...
      WRITE(OUTLYNE,103) REFRY(1,NEWIMG)+(DTAN(REFRY(11,NEWIMG))*DELTA),UN
      CALL SHOWIT(0)

      WRITE(OUTLYNE,104) REFRY(2,NEWIMG)+(DTAN(REFRY(12,NEWIMG))*DELTA),UN
      CALL SHOWIT(0)
...
!
   CHARACTER UN*11
!
   REAL*8 SPT,SPT1,SPT2,SPT3,SPT4,SPT5,XV,YV,XYANG,RRAD,RRANG,RPNT,RANDX,SPT6,SPT7,SPT8,SPT9 ,RINGTOT1,WAY,SPT10,OSPA,OSPB,OSPC,OSPD,OAFSPB,OAFSPD ,RESLT,WOW1,WOW2
!
   REAL*8 ELMAX
!
   COMMON/ELEV/ELMAX
...
!
   INTEGER KIK,KIKI
!
   REAL*8 NSTART1,NSTOP1,NDEL1,NSTART2,NSTOP2,NDEL2,TOT,W,AMSS,IX,IY,XUP,YUP,XLO,YLO ,X,Y,MSSX,MSSY,AMSSX,AMSSY,MAXX,MAXY,APODX2,APODR2
!
   COMMON/BIGGY/XUP,YUP,XLO,YLO,MAXX,MAXY
!
   REAL*8 MAX,STARANG,ENDANG,DELANG ,FRAC,FRACA,FRACB,MSS,APFAC
!
   INTEGER I,J,IWIW2,IWL,NUMT1,NUMT2,NUMT3,NUMT4,NUMT5,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10,TEMPHOLDER
!
   LOGICAL ODD,FOBB0 ,FOBB0X,FOBB0Y,IMFLAG
!
   COMMON/FFOOBB/FOBB0,FOBB0X,FOBB0Y
!
!     THE FOLLOWING VARIABLES TRACK THE REFERENCE SPHERE RADIUS
...
!
   IF(STI.EQ.1.AND.TPT.EQ.1) THEN
      OUTLYNE='"SPD" IS THE SPOT DIAGRAM COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
!
   IF(WC.NE.'ISPD'.AND.WC.NE.'ISPDA') THEN
      IF(WQ.NE.'ISTAT'.AND.WQ.NE.'IPSTAT'.AND.TPT.EQ.1) THEN
         IF(SST.EQ.1) THEN
            OUTLYNE='"SPD (ACC) TAKES NO STRING INPUT'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       NO STRINGS PRESENT, PROCEED
         END IF
         IF(S4.EQ.1.OR.S5.EQ.1) THEN
            OUTLYNE='"SPD (ACC)" TAKES NO NUMERIC WORD #4 OR #5 INPUT'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       DF3=0
         ...
         IF(S1.EQ.1) THEN
            IF(W1.NE.1.0D0.AND.W1.NE.2.0D0.AND.W1.NE.3.0D0.AND.W1.NE.4.0D0 .AND.W1.NE.5.0D0.AND.W1.NE.6.0D0.AND.W1.NE.7.0D0.AND.W1.NE.8.0D0 .AND.W1.NE.9.0D0.AND.W1.NE.10.0D0) THEN
               OUTLYNE='INVALID WAVELENGTH NUMBER ISSUED WITH THE LAST "SPD" COMMAND'
               CALL SHOWIT(1)
               OUTLYNE='RE-ENTER COMMAND'
               CALL SHOWIT(1)
               CALL MACFAL
      ...
      IF(WQ.EQ.'ISTAT'.AND.TPT.EQ.1.OR.WQ.EQ.'IPSTAT'.AND.TPT.EQ.1) THEN
         IF(S5.EQ.1.OR.SST.EQ.1) THEN
            OUTLYNE='"SPD (ISTAT OR IPSTAT) TAKES NO STRING OR NUMERIC WORD #5 INPUT'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!     NW2 AND NW3
         ...
         IF(W2.GE.W3) THEN
            OUTLYNE='STARTING ANGLE MUST BE LESS THAN ENDING ANGLE'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!     NW4
         ...
         IF(DABS(W4).GE.(DABS(W3)-DABS(W2))) THEN
            OUTLYNE='ANGLE INCREMENT TOO SMALL FOR SPECIFIED ANGLE RANGE'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!     NUMBER OF STEPS IS
            ...
            IF(NSTEP.GT.100) THEN
               OUTLYNE='THE MAXIMUM NUMBER OF STEPS IS 100'
               CALL SHOWIT(1)
               OUTLYNE='ANGLE INCREMENT YIELDS MORE THAN 100 STEPS'
               CALL SHOWIT(1)
               OUTLYNE='RE-ENTER COMMAND'
               CALL SHOWIT(1)
               CALL MACFAL
...
!
               IF(WQ.NE.'ACC') THEN
                  ...
                  IF(MSGSPD)WRITE(OUTLYNE,*)'(X x Y), HAS BEEN ASSIGNED TO THE REFERENCE SURFACE'
                  IF(MSGSPD) CALL SHOWIT(0)
                  IF(MSGSPD)WRITE(OUTLYNE,*)'FOR THE CURRENT SPOT DIAGRAM CALCULATIONS'
                  IF(MSGSPD) CALL SHOWIT(0)
301               FORMAT('TEMPORARY CIRCULAR CLEAR APERTURE = ',G11.4,' X ',G11.4)
               END IF
...
!
               IF(WQ.NE.'ACC') THEN
                  ...
                  IF(MSGSPD)WRITE(OUTLYNE,*)'FOR THE CURRENT SPOT DIAGRAM CALCULATIONS'
                  IF(MSGSPD) CALL SHOWIT(0)
3010              FORMAT('TEMPORARY CIRCULAR CLEAR APERTURE = ',G11.4,' X ',G11.4)
               END IF
...
!     THIS LOOPS THROUGH THE POINTS IN EACH RING
               DO K=1,RPNT
                  IF(JJ.EQ.0) RRANG=0.0D0
                  IF(JJ.NE.0) THEN
                     IF(TPT.EQ.1) RRANG=((TWOPII)/360.0D0)*RINGANG(JJ)+(DBLE(K-1)*((TWOPII)/DBLE(RINGPNT(JJ))))
                     IF(TPT.EQ.2) RRANG=((TWOPII)/360.0D0)*OPRINGANG(JJ)+(DBLE(K-1)*((TWOPII)/DBLE(OPRINGPNT(JJ))))
                  END IF
                  WW1=RRAD*DSIN(RRANG)
                  WW2=RRAD*DCOS(RRANG)
                  I=I+1
...
!     REMOVE TEMP CLAP ON NEWREF
   ...
   END IF
   IF(WQ.NE.'ACC'.AND.TPT.EQ.1) THEN
      IF(MSGSPD)WRITE(OUTLYNE,*)'CALCULATING SPOT DIAGRAM STATISTICS...'
      IF(MSGSPD) CALL SHOWIT(0)
   END IF
!
...
!     FOCAL SYSTEM
!       X ONLY
               SSSPX=SSSPX+(SPT*(((LA-SPA)**2))*APFAC)
!     AFOCAL SYSTEM
!       X ONLY
               ASSSPX=ASSSPX+(SPT*(((AFLB-AFSPB)**2))*APFAC)
!     FOCAL SYSTEM
!       Y ONLY
               SSSPY=SSSPY+(SPT*(((LC-SPC)**2))*APFAC)
!     AFOCAL SYSTEM
!       Y ONLY
               ASSSPY=ASSSPY+(SPT*(+((AFLD-AFSPD)**2))*APFAC)
            END IF
         END IF
      END DO
      MSS=SSSP
...
!       X ONLY
               SSSPX=SSSPX+(SPT*(((LA-SPA)**2))*APFAC)
               SSSQX=SSSQX+(SPT*((LA*LB)-(LA*SPB)-(LB*SPA)+(SPA*SPB))*APFAC)
               SSSRX=SSSRX+(SPT*(((LB-SPB)**2))*APFAC)
!     AFOCAL SYSTEM
!       X ONLY
               ASSSPX=ASSSPX+(SPT*(((AFLB-AFSPB)**2))*APFAC)
               ASSSQX=0.0D0
...
!       Y ONLY
               SSSPY=SSSPY+(SPT*(((LC-SPC)**2))*APFAC)
               SSSQY=SSSQY+(SPT*((LC*LD)-(LC*SPD)-(LD*SPC)+(SPC*SPD))*APFAC)
               SSSRY=SSSRY+(SPT*(((LD-SPD)**2))*APFAC)
!     AFOCAL SYSTEM
!       Y ONLY
               ASSSPY=ASSSPY+(SPT*(+((AFLD-AFSPD)**2))*APFAC)
               ASSSQY=0.0D0
