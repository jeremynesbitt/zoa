...
!        NY=INFO(3)
      ...
      &IOBJECTV,STAT=ALLOERR)
      ALLOCATE (IOBJECTX(NX,NY),IOBJECTY(NX,NY),&
      &IOBJECTV(NX,NY,3),STAT=ALLOERR)
...
!
   ...
   IF(II.EQ.2) THEN
      ...
      &IIMAGEV,STAT=ALLOERR)
      ALLOCATE (IIMAGEX(NX,NY),IIMAGEY(NX,NY),&
      &IIMAGEV(NX,NY,3,4),STAT=ALLOERR)
      ...
      IF(NUMCOLORS.EQ.3) THEN
         ...
         DO K=1,OBJNY
            DO J=1,OBJNX
               ...
               &(256*INT(((IOBJECTV(J,K,1)))/PEAKER))+&
               &(256*256*INT(((IOBJECTV(J,K,2)))/PEAKER))
...
101      format(1x, *(g0, ", "))
         ...
         DO K=1+INT(TRIMMER),IMGNY-INT(TRIMMER)
            DO J=1+INT(TRIMMER),IMGNX-INT(TRIMMER)
               ...
               &(256*INT(((IIMAGEV(J,K,1,1)))))+&
               &(256*256*INT(((IIMAGEV(J,K,2,1)))))
...
!        write(1, '(1x, F, 3(",", F))') a
!       CALL csv_write(lun=10, value=BMPDATA24(L), advance=.false. )
               L=L+1
            END DO
         END DO
!       call csv_write(lun=10, value=BMPDATA24(L), advance=.true. )
!        write(1, '(1x, F, 3(",", F))') BMPDATA24
         CLOSE(10)
         OPEN(unit = 10, access = "sequential", action = "write",&
         &status = "replace", file = "data2d.csv", form = "formatted")
         DO J=1,OBJNX
