!       THIRD GROUP OF SPECIAL SURFACE AND FITTING FILES
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITTYP. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "TYPE" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   real(real64)  STYPE,SSURF
!
   COMMON/ST/STYPE
!
   COMMON/SSS/SSURF
...
!
   ...
   &.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      ...
      & '"TYPE" ONLY TAKES NUMERIC WORD NUMBER #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TYPE" COMMAND REQUIRES NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   ...
   IF(DABS(STYPE).GT.6.0) THEN
      CALL REPORT_ERROR_AND_FAIL('TYPES 6 THROUGH 30 NOT YET SUPPORTED', 1)
      F23=0
      STYPE=0.0
      RETURN
   END IF
   ...
   SURF=-99
   SSURF=-1.0D0
!       SET SSURF TO NON-VALID TYPE (NEGATIVE)
   RETURN
END
! SUB FITSUR.FOR
SUBROUTINE FITSUR
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITSUR. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "SURF" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   real(real64) SSURF,STYPE
!
   COMMON/SSS/SSURF
!
   COMMON/ST/STYPE
...
!
   ...
   &.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SURF" ONLY TAKES NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1) THEN
      OUTLYNE=&
      &'"SURF" COMMAND REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
...
!
   SURF=INT(W1)
   SSURF=DBLE(surf_special_type(SURF))
   IF(SSURF.LT.1.0.OR.SSURF.GT.30.0) THEN
      WRITE(OUTLYNE,*)'WARNING FOR SURFACE ',INT(W1)
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL(&
      & 'SURFACE "TYPE" NUMBER OUTSIDE DEFINED BOUNDS'//'\n'//&
      & 'VALID TYPES 1 TO 30', 1)
      SSURF=0.0D0
      SURF=-99
      RETURN
   END IF
   IF(SSURF.GT.18.0) THEN
      CALL REPORT_ERROR_AND_FAIL('SURF TYPES 18 THROUGH 30 NOT YET SUPPORTED', 1)
      SSURF=0.0D0
      SURF=-99
      F23=0
      RETURN
...
!       VALID SSURF, SET STYPE TO NEGATIVE
   WRITE(OUTLYNE,*)&
   &'TYPE #',INT(SSURF),' DATA FITTING HAS BEEN DEFINED'
   CALL SHOWIT(1)
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITFIT. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "FIT" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
...
!
   ...
   IF(WC.EQ.'FITGLASS') THEN
      ...
      &S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1)THEN
         OUTLYNE=&
         &'"FITGLASS" ONLY TAKES QUALIFIER (GLASS NAME) INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         DEALLOCATE(W,V,U,B,XXX,STAT=ALLOERR)
         RETURN
      ...
      IF(SQ.EQ.0) THEN
         OUTLYNE=&
         &'"FITGLASS" REQUIRES EXPLICIT QUALIFIER (GLASS NAME) INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         DEALLOCATE(W,V,U,B,XXX,STAT=ALLOERR)
         RETURN
   ...
   END IF
!
!       SET F24=1 SAYING THERE IS DATA FITTED AND READY FOR
!       EVALUATION
   F24=1
!
...
352   CONTINUE
   END IF
!
!       THIS IS WHERE THE ACTUAL SOLUTION TO THE LEAST
!       SQUARES PROBLEM IS DONE. ALL THE COEFFICIENTS
!       OF THE NORMAL EQUATION HAVE BEEN DETERMINED IN
!       THE SUBROUTINE "FITDAT"
   DEALLOCATE(W,V,U,B,XXX,STAT=ALLOERR)
   RETURN
!
END
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITEOS. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE EXIT FROM THE SPFIT PROGRAM LEVEL TO THE CMD
!       LEVEL.
!
!
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITCFF. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "COEF" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   INTEGER I
!
   real(real64) X(1:96)
...
!
   ...
   &THEN
      OUTLYNE=&
      &'"COEF" ONLY TAKES NUMERIC WORD #1, #2 AND #3 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      OUTLYNE=&
      &'"COEF" REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
!       CHECK THAT TYPE OR SURF WAS ENTERED
   IF(F23.EQ.0) THEN
      OUTLYNE=&
      &'THE "TYPE" OR "SURF" COMMANDS MUST BE ENTERED BEFORE'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('COEFFICIENT FLAGS CAN BE SET', 1)
      RETURN
   END IF
!
   IF(INT(W1).LT.1.OR.INT(W1).GT.96) THEN
      CALL REPORT_ERROR_AND_FAIL('INVALID COEFFICIENT NUMBER'//'\n'//'VALID TYPES 1 TO 96', 1)
      RETURN
   END IF
   IF(INT(W2).LT.0.OR.INT(W2).GT.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'INVALID COEFFICIENT FLAG VALUE'//'\n'//&
      & 'VALID VALUES ARE 0-(OFF) AND 1-(ON)', 1)
      RETURN
   END IF
!
!       SET COEFFICIENT FLAG VALUE
   CFF(INT(W1))=INT(W2)
   IF(DF3.NE.1) THEN
!       EXPLICIT COEFF VALUES ARE BEING ENTERED.
      F24=1
      F25=1
!
...
!       EVALUATION AS IF FITTING IS DONE IS ASSUMED
   ELSE
      F24=0
      F25=0
!       NO COEFF VALUES
   ...
   END IF
!
!       NOW NOVE THE COEFFICIENT VALUES TO THE SOULTION VECTOR
!
!       SET UP THE SOLUTION VECTOR FOR EVALUATION
   IF(F23.EQ.1) THEN
!       SIMPLE DATA FIT
      DO 9898 I=1,96
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FITDAT. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "DATA" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   INTEGER COUNT1 &
   &,DATCNT,TYPE,COUNT2,COUNT,IDATA,I,J,II,JJ
!
   COMMON/PRSIZE/COUNT
!
   COMMON/NUMDAT/DATCNT
!
   real(real64) ACCUM(1:96,1:96),XX,YY,&
   &TERM,CCOL(1:96),Z,FF2,FF3,FF4,FF5
!
   COMMON/ACDATA/ACCUM,CCOL
!
   real(real64)  STYPE,SSURF,RHO,THETA
!
   COMMON/SPIDAT/IDATA
!
   COMMON/ST/STYPE
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"DATA" ONLY TAKES NUMERIC WORD #1, #2, #3 AND #4 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   ELSE
!       INPUT FORM OK SO FAR
   END IF
   IF(WC.EQ.'GDATA') THEN
      IF(SST.EQ.1.OR.SQ.EQ.1.OR.S5.EQ.1.OR.S4.EQ.1.OR.S3.EQ.1) THEN
         OUTLYNE=&
         &'"GDATA" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      ELSE
!       INPUT FORM OK SO FAR
   ...
   END IF
   IF(WC.EQ.'GDATA') THEN
!     RESET THE DATA FOR A "DATA" LINE
      W1=W1
      W3=W2**2
      W2=0.0D0
      W4=1.0D0
...
!       W1 IS Y VALUE
!       W2 IS X VALUE
!       W3 IF FUNCTION VALUE
!       W4 IS WEIGHT VALUE (DEFAULT 1.0)
   ...
   IF(DF1.EQ.1.AND.DF2.EQ.1.AND.DF3.EQ.1.AND.DF4.EQ.1) THEN
      IF(WC.EQ.'GDATA')&
      &OUTLYNE='"GDATA" COMMAND REQUIRES SOME EXPLICIT NUMERIC INPUT'
      IF(WC.EQ.'DATA')&
      &OUTLYNE='"DATA" COMMAND REQUIRES SOME EXPLICIT NUMERIC INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   ELSE
!       EXPLICIT INPUT GIVEN, CONTINUE
   END IF
!       CHECK FOR ALL ZERO COEF FLAGS
!
!       COUNT1 IS NUMBER OF LOWEST COEF
!       COUNT2 IS NUMBER OF HIGHEST COEF
   COUNT1=0
   COUNT2=0
   DO 100 I=1,96
      IF(CFF(I).EQ.1) THEN
...
!
   IF(COUNT2.EQ.0) THEN
      ...
      RETURN
   ELSE
!       COUNT1 AND COUNT2 NOT ZERO, PROCEED
   END IF
!
   IF(F23.EQ.1.AND.STYPE.EQ.1.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.1.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.6.0D0) THEN
!
!       SPECIAL SURFACE TYPE 1 OR 6 OR TYPE 1 FUNCTIONAL FORM
!       IS A RADIAL POWER SERIES
!
!       FROM RHO**-8 TO RHO**39 INCLUDING RHO**0=1
!
!       PROCEED WITH FITTING
!       SET F24=0, NO DATA READY TO EVALUATE UNTIL A FIT IS DONE
      F24=0
!       DATA NOW EXISTS TO FIT
      F25=1
!       FILL THE ACCUM ARRAY
      Z=DSQRT((W1**2)+(W2**2))
!       INCREMENT THE DATA ITEM COUNTER
      DATCNT=DATCNT+1
      IDATA=IDATA+1
!       PROCEED
      II=0
...
!
   ...
   &F23.EQ.2.AND.SSURF.EQ.2.0D0 &
   &.OR.F23.EQ.2.AND.SSURF.EQ.9.0D0) THEN
!
!       PROCEED WITH FITTING A TYPE 2 TYPE/SURFACE
!       SET F24=0, NO DATA READY TO EVALUATE UNTIL A FIT IS DONE
      F24=0
!       DATA NOW EXISTS TO FIT
      F25=1
...
!       FILL THE COLLUMN ARRAY
         IF(CFF(I).EQ.1) THEN
            II=II+1
!       CALCULATE COLUMN VECTOR
!       FUNCTION FF2 RETURNS THE CORRECT VALUE
...
!       FOR THE STANDARD ZERNIKE. I RANGES FROM COUNT1
!       TO COUNT2
!       MAX VALUE FOR TYPE 2 SURFACE IS 66
...
!
   ...
   &F23.EQ.2.AND.SSURF.EQ.14.0D0.OR.F23.EQ.2.AND.SSURF.EQ.15.0D0)&
   &THEN
!
!       PROCEED WITH FITTING A TYPE 5 TYPE/SURFACE
!       SET F24=0, NO DATA READY TO EVALUATE UNTIL A FIT IS DONE
      F24=0
!       DATA NOW EXISTS TO FIT
      F25=1
...
!       FILL THE COLLUMN ARRAY
         IF(CFF(I).EQ.1) THEN
            II=II+1
!       CALCULATE COLUMN VECTOR
!       FUNCTION FF5 RETURNS THE CORRECT VALUE
!       FOR THE ITH ENTRY IN THE COLUMN VECTOR
!       WITHOUT THE GIVEN DEPENDENT VARIABLE VALUE
!       OF THE WEIGHT FUNCTION FACTORED IN. THIS IS
!       FOR THE STANDARD ABERRATION POLY. I RANGES FROM COUNT1
!       TO COUNT2
!       MAX VALUE FOR TYPE 5 SURFACE IS 48
...
!
   ...
   &F23.EQ.2.AND.SSURF.EQ.3.0D0.OR.F23.EQ.2.AND.SSURF.EQ.10) THEN
!
...
!       FOR THE STANDARD ZERNIKE. I RANGES FROM COUNT1
!       TO COUNT2
!       MAX VALUE FOR TYPE 3 SURFACE IS 37
...
!
   ...
   &F23.EQ.2.AND.SSURF.EQ.7.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.8.0D0) THEN
!
!       PROCEED WITH FITTING A TYPE 4 FUNCTIONAL FORM OR A
!     TYPE 7 OR 8 SPECIAL SURFACE
!       SET F24=0, NO DATA READY TO EVALUATE UNTIL A FIT IS DONE
      F24=0
!       DATA NOW EXISTS TO FIT
      F25=1
...
!       FILL THE COLLUMN ARRAY
         IF(CFF(I).EQ.1) THEN
            II=II+1
!       CALCULATE COLUMN VECTOR
!       FUNCTION FF4 RETURNS THE CORRECT VALUE
!       FOR THE ITH ENTRY IN THE COLUMN VECTOR
!       WITHOUT THE GIVEN DEPENDENT VARIABLE VALUE
!       OF THE WEIGHT FUNCTION FACTORED IN. THIS IS
!       FOR THE STANDARD ZERNIKE. I RANGES FROM COUNT1
...
!
   ...
   CALL SHOWIT(1)
   CALL REPORT_ERROR_AND_FAIL('RETURNED TO CMD LEVEL', 1)
   F9=0
   F1=1
   RETURN
3500 CONTINUE
!       ADD THE CURRENT DATA TO THE DATA SAVING ARRAY
...
!       NO MORE DATA ALLOWED
      OUTLYNE=&
      &'WARNING: NO MORE DATA IS BEING SAVED IN THE "DATA" ARRAY'
      CALL SHOWIT(1)
   END IF
   RETURN
END
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FTEVAL. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "EVAL" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   INTEGER &
   &DATCNT,TYPE,COUNT2,COUNT,I
!
   COMMON/PRSIZE/COUNT
!
   COMMON/NUMDAT/DATCNT
!
   real(real64) &
   &TERM,X(1:96),RHO,THETA,FF2,FF3,FF4,XX,YY,FF5
!
   real(real64)  STYPE,SSURF
!
   COMMON/SOLU/X
...
!
   ...
   &S3.EQ.1) THEN
      OUTLYNE=&
      &'"EVAL" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   ELSE
!       INPUT FORM OK SO FAR
   ...
   IF(DF1.EQ.1.AND.DF2.EQ.1) THEN
      OUTLYNE=&
      &'"EVAL" COMMAND REQUIRES SOME EXPLICIT NUMERIC INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   ELSE
...
!
   IF(COUNT2.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'ALL COEFFICIENT FLAGS ARE ZERO'//'\n'//&
...
!
   IF(F23.EQ.1.AND.STYPE.EQ.2.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.2.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.9.0D0) THEN
!
!       PROCEED WITH EVALUATION
      TERM=0.0D0
      RHO=DSQRT((W1**2)+(W2**2))
   ...
   END IF
   IF(F23.EQ.1.AND.STYPE.EQ.5.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.14.0D0.OR.F23.EQ.2.AND.&
   &SSURF.EQ.15.0D0) THEN
!
!       PROCEED WITH EVALUATION
      TERM=0.0D0
      RHO=DSQRT((W1**2)+(W2**2))
   ...
   END IF
   IF(F23.EQ.1.AND.STYPE.EQ.3.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.3.0D0.OR.&
   &F23.EQ.2.AND.SSURF.EQ.10.0D0) THEN
!
!       PROCEED WITH EVALUATION
      TERM=0.0D0
      RHO=DSQRT((W1**2)+(W2**2))
...
!       PROCEED WITH EVALUATION
   ...
   RETURN
!
!       HERE IS WHERE OUTPUT IS DONE FOR ALL EVALUATIONS
!
2500 REG(40)=REG(9)
   REG(9)=TERM
   IF(F23.EQ.1) TYPE=INT(STYPE)
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FTLIST. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "LIST" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   INTEGER &
   &DATCNT,TYPE,COUNT2,COUNT,&
...
!
   COMMON/NUMDAT/DATCNT
!
   real(real64) TERM,X(1:96),RHO,FF2,FF3,FF4,XX,YY,FF5
!
   COMMON/SPIDAT/IDATA
!
   real(real64)  STYPE,SSURF
...
!
   IF(COUNT2.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL('ALL COEFFICIENT FLAGS ARE ZERO'//'\n'//'NO LISTING POSSIBLE', 1)
      RETURN
   END IF
!
   IF(F23.EQ.1.AND.STYPE.EQ.1.0D0.OR.&
...
!       PROCEED WITH EVALUATION
         ...
         ELSE
            ...
            &DABS(DDATA(2,IIII)).LE.1.0D-15) THEN
               THETA=0.0D0
            ELSE
...
!       DO A LINE OF OUTPUT
         ...
         &DDATA(3,IIII),TERM,(DDATA(3,IIII)-TERM)
         CALL SHOWIT(0)
4500  CONTINUE
      RETURN
   ELSE
...
!       PROCEED WITH EVALUATION
         ...
         &DABS(((1.0D35)*DDATA(2,IIIII)))) THEN
         ...
         ELSE
            ...
            &DABS(DDATA(2,IIIII)).LE.1.0D-15) THEN
               THETA=0.0D0
            ELSE
...
!       DO A LINE OF OUTPUT
         ...
         &DDATA(3,IIIII),TERM,(DDATA(3,IIIII)-TERM)
         CALL SHOWIT(0)
5500  CONTINUE
      RETURN
   ELSE
...
!
   EXTERNAL FF2,FF3,FF4,FF5
!
!       THIS IS SUBROUTINE RDDATA. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "READ" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   CHARACTER DDDD*4
!
   LOGICAL EXISTD
!
   INTEGER COUNT1 &
   &,DATCNT,TYPE,COUNT2,COUNT,IDATA,I,II,III &
   &,J,JJ
!
   COMMON/PRSIZE/COUNT
!
   COMMON/NUMDAT/DATCNT
!
   real(real64) ACCUM(1:96,1:96),DWW1,DWW2,DWW3,DWW4,&
   &TERM,CCOL(1:96),Z,FF2,FF3,RHO,THETA,FF4,XX,YY,FF5
!
   COMMON/ACDATA/ACCUM,CCOL
!
   real(real64)  STYPE,SSURF
...
!       INPUT FORM OK SO FAR
   END IF
!
!       READ CAUSES THE FILE DATA.DAT TO BE READ AS A SERIES OF DATA
!       INPUT STATEMENTS. THE DATA.DAT RECORD FORMAT LIST DIRECTED
!
!       WHERE FIELDS ARE:
!       "DATA"
!       Y-COORDINATE
...
!       WEIGTHING FACTOR
!
!       ENTRIES MAY BE SEPARATED BY A SPACE OR A COMMA
!
!       READS ARE COMPLETED WHEN AN EOF IS DETECTED BY THE
!       INPUT STATEMENT. BAD DATA IN THE DATA.DAT FILE MAY
!       CAUSE THE PROGRAM TO BOMB.
!
   EXISTD=.FALSE.
   INQUIRE(FILE='DATA.DAT',EXIST=EXISTD)
   IF(.NOT.EXISTD) THEN
!       CAN'T DO THE READ OPERATION
...
!       ALL VALUES CALCULATED FOR THE CURRENT "DATA"
!       STATEMENT.
!       READ NEXT ITEM IN DATA.DAT BY GOING TO 3501
         GO TO 3501
      ELSE
!       NOT TYPE 1 FITTING
      END IF
...
!       FOR THE ITH ENTRY IN THE COLUMN VECTOR
!       WITHOUT THE GIVEN DEPENDENT VARIABLE VALUE
!       OF THE WEIGHT FUNCTION FACTORED IN.
!       I RANGES FROM COUNT1
!       TO COUNT2
               TERM= FF4(XX,YY,I)
               TERM=TERM*DWW3*DWW4
               CCOL(III)=CCOL(III)+TERM
...
113 CONTINUE
   CALL CLOSE_FILE(18,1)
!       FINISHED READING DATA.DAT CORRECTLY
   F24=0
   F25=1
   CALL CLOSE_FILE(18,1)
   RETURN
!       EOF FOUND, READY TO RETURN TO SPFIT2
114 CONTINUE
   OUTLYNE=&
   &'READ ERROR IN READING "DATA.DAT" FILE'
   CALL SHOWIT(1)
   OUTLYNE=&
   &'RETURNED TO CMD LEVEL'
   CALL SHOWIT(1)
...
!
   ...
   &,FN22,FN23,FN24,FN25,FN26,FN27,FN28,FN29,FN30,RHO,THETA,FF2 &
   ...
   &,FN61,FN62,FN63,FN64,FN65,FN66 &
   &,R,A
!
   INTEGER FI
!
!       FUNCTIONS FOR TYPE 2 SURFACE (ZERNIKE)
   FN1(R,A)=1.0D0
   FN2(R,A)=R*DCOS(A)
   FN3(R,A)=R*DSIN(A)
...
!
   ...
   IMPLICIT NONE
!
   real(real64) FFN1,FFN2,FFN3,FFN4,&
   &FFN5,FFN6,FFN7,FFN8,FFN9 &
   &,FFN10,FFN11,FFN12,FFN13,FFN14,FFN15,FFN16,FFN17,FFN18,&
   &FFN19,FFN20,FFN21 &
   &,FFN22,FFN23,FFN24,FFN25,FFN26,FFN27,FFN28,FFN29,FFN30 &
   &,FFN31,FFN32,FFN33,FFN34,FFN35,FFN36,FFN37 &
   &,RHO,THETA,FF3,R,A
!
   INTEGER FI
!
!       FUNCTIONS FOR TYPE 3 SURFACE, FRINGE ZERNIKEIS
   ...
   &(3150.0D0*(R**8))&
   &-(1680.0D0*(R**6))+(420.0D0*(R**4))-(42.0D0*(R**2))+1.0D0
!
...
!
   ...
   IMPLICIT NONE
!
   real(real64) FFN1,FFN2,FFN3,FFN4,FFN40,FFN41,FFN42,FFN43,&
   &FFN5,FFN6,FFN7,FFN8,FFN9,FFN44,FFN45,FFN46,FFN47,FFN48,FFN49,&
   &FFN10,FFN11,FFN12,FFN13,FFN14,FFN15,FFN16,FFN17,FFN18,FFN57,&
   &FFN19,FFN20,FFN21,FFN50,FFN51,FFN52,FFN53,FFN54,FFN55,FFN56,&
   &FFN22,FFN23,FFN24,FFN25,FFN26,FFN27,FFN28,FFN29,FFN30,FFN58,&
   &FFN31,FFN32,FFN33,FFN34,FFN35,FFN36,FFN37,FFN38,FFN39,FFN59,&
   &X,Y,FF4,XX,YY,FFN60,FFN61,FFN62,FFN63,FFN64,FFN65,FFN66,FFN67,&
   &FFN68,FFN69,FFN70,FFN71,FFN72,FFN73,FFN74,FFN75,FFN76,FFN77,&
   &FFN78,FFN79,FFN80,FFN81,FFN82,FFN83,FFN84,FFN85,FFN86,FFN87,&
   &FFN88,FFN89,FFN90,FFN91
!
   INTEGER FI
!
!       FUNCTIONS FOR TYPE 4 SURFACE, RECTANGULAR POLYNOMIAL
!
   FFN1(XX,YY)=1.0D0
   FFN2(XX,YY)=XX
   FFN3(XX,YY)=YY
...
!
   ...
   IMPLICIT NONE
!
   real(real64) FFN1,FFN2,FFN3,FFN4,FFN43,FFN44,FFN45,&
   &FFN5,FFN6,FFN7,FFN8,FFN9,FFN38,FFN39,FFN40,FFN41,FFN42 &
   &,FFN10,FFN11,FFN12,FFN13,FFN14,FFN15,FFN16,FFN17,FFN18,&
   &FFN19,FFN20,FFN21,FFN46,FFN47,FFN48 &
   &,FFN22,FFN23,FFN24,FFN25,FFN26,FFN27,FFN28,FFN29,FFN30 &
   &,FFN31,FFN32,FFN33,FFN34,FFN35,FFN36,FFN37 &
   &,RHO,THETA,FF5,R,A
!
   INTEGER FI
!
!       FUNCTIONS FOR TYPE 5 SURFACE, FRINGE ZERNIKES
   FFN1(R,A)=1.0D0
   FFN2(R,A)=R*DCOS(A)
   FFN3(R,A)=R*DSIN(A)
   FFN4(R,A)=R**2
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE LFCOEFS. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "LISTCOEF" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   INTEGER I
!
   real(real64) STYPE,SSURF
...
!
   ...
   &THEN
      CALL REPORT_ERROR_AND_FAIL('"LISTCOEF" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
!       PROCEED
   END IF
...
!       PROCEED
   END IF
   IF(F23.EQ.2) THEN
!       OUTPUT SURFACE COEFFICIENTS
      WRITE(OUTLYNE,1212)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,129) SURF,INT(SSURF)
      CALL SHOWIT(0)
129   FORMAT(&
      &'FOR SURFACE NUMBER = ',I3,' WITH SURFACE TYPE = ',I2)
      WRITE(OUTLYNE,130)
      CALL SHOWIT(0)
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE FCOEFS. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "COEFS" COMMAND FROM THE SPFIT PROGRAM LEVEL.
!
   INTEGER I
!
   real(real64) STYPE,SSURF
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1)&
   &THEN
      CALL REPORT_ERROR_AND_FAIL('"COEFS" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   ELSE
!       PROCEED
   END IF
...
120 FORMAT('THE COEFFICIENTS ARE:')
   DO 10 I=1,96,4
      WRITE(OUTLYNE,110)&
      &CFTYPE(I),CFTYPE(I+1),CFTYPE(I+2),CFTYPE(I+3)
      CALL SHOWIT(0)
10 CONTINUE
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE CLEARREG. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE "CLEARREG"
!
   INTEGER I,J,K
!
!
   ...
   &S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"CLEARREG" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   ...
   IF(J.GE.K) THEN
      OUTLYNE=&
      &'NUMERIC WORD #1 MUST BE LESS THAN NUMERIC WORD #2 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(J.LT.1) THEN
      OUTLYNE=&
      &'NUMERIC WORD #1 MUST BE GREATER THAN ZERO'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(K.GT.MAXREG) THEN
      WRITE(OUTLYNE,*)&
      &'NUMERIC WORD #2 MUST BE LESS THAN ',MAXREG
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       ZERO THE COEFS
   ...
   END DO
   WRITE(OUTLYNE,*)&
   &'GENERAL PURPOSE STRORAGE REGISTERS ',J,' THROUGH ',K
   CALL SHOWIT(1)
   OUTLYNE=&
