!       SECOND SET OF OPTIMIZATION ROUTINES
...
!       HANDLES VARIABLE INPUT AND VARIABLE UPDATE COMMANDS AND VARIABLE
!       OUTPUT COMMANDS AT THE CMD LEVEL
!
!       THE ARRAY VARABL STORES VARIABLE INFORMATION
!       IT IS PASSED IN COMMON IN THE INCLUDE FILE DATSUB.FOR
!
!       VARABL(I,J) WHERE I COUNTS THE NUMBER OF VARIABLE ENTRIES
!       AND J TAKES ON THE FOLLOWING VALUES AND MEANIINGS.
!
!       J=1  > 1 THROUGH 150 AND 250 AND ON FOR ACT., A VARIABLE TYPE DESIGNATOR
...
!
   ...
   END IF
!
!       'VB' AND 'VBA' OUTPUT VARIABLE DATA FROM INSIDE
!       AND OUTSIDE OF THE VARIABLE SUBFILE VIA SUBROUTINE VBA.FOR.
...
!
   IF(WC.EQ.'CFG') THEN
      IF(SST.EQ.1.OR.SQ.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1.OR.S3.EQ.1.OR.S2.EQ.1) THEN
      ...
      IF(INT(W1).GT.LASCFG.AND.INT(W1).NE.1) THEN
         WRITE(OUTLYNE,*)'REQUESTED CONFIGURATION IS CURRENTLY EMPTY'
         CALL SHOWIT(1)
...
!       NOW DO WC=DEL
   IF(WC.EQ.'DELK') THEN
      IF(F29.NE.2) THEN
         WRITE(OUTLYNE,*)'"DEL" IS ONLY AVAILABLE FROM THE "UPDATE VARIABLE" LEVEL'
         CALL SHOWIT(1)
      ...
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1 ) THEN
         WRITE(OUTLYNE,*)'"DEL" ONLY TAKES QUALIFIER AND NUMERIC WORDS #1 AND #2'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     CONVERT WQ(4:7) INTO A NUMBER AND ASSIGN AN APPROPRIATE
!     DELVAL TO IT
         A4=WQ(4:7)
         CALL ATOI4(A4,I4)
         DELVAL=249+I4
      ...
      ELSE
         IF(WQ.EQ.'MACVAR') THEN
            IF(INT(W1).LT.1.OR.INT(W1).GT.MAXREG) THEN
               WRITE(OUTLYNE,*)'NUMERIC WORD #1 (GP REGISTER NUMBER) BEYOND LEGAL BOUNDS'
               CALL SHOWIT(1)
               WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
               CALL SHOWIT(1)
               CALL MACFAL
...
!     DOES THE DESIGNATED VARIABLE EXIST ?
      J=0
      DO I=1,VBCNT
         IF(INT(VARABL(I,1)).EQ.DELVAL.AND.INT(VARABL(I,2)).EQ.INT(WORD2).AND.INT(VARABL(I,3)).EQ.INT(W1)) THEN
            J=J+1
            DELTAG=I
...
!     ALL DELETIONS COMPLETED
         ...
         RETURN
      ELSE
         WRITE(OUTLYNE,*)TRIM(WQ),' VARIABLE FOR SURFACE ',INT(W1)
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'WAS NOT IN THE VARIABLE SUBFILE, NO DELETION PERFORMED'
         CALL SHOWIT(1)
         CALL MACFAL
!     ITEM DESIGNATED WAS NOT FOUND
      END IF
      RETURN
!
   END IF
!     START DOING THE VARIABLE NAMES HERE
   ...
   IF(WC(1:3).EQ.'ACT') THEN
      IF(VBCFG.EQ.1) THEN
!     ASSIGN VALT # FOR THE 3969 ACTUATOR VARIABLES
!     CONVERT WC(4:7) INTO A NUMBER AND ASSIGN AN APPROPRIATE
!     VALT VALUE
         A4=WC(4:7)
         CALL ATOI4(A4,I4)
         VALT=249+I4
!
      ELSE
!     NOT AT CFG 1, NO ASSIGNMENT MADE
         WRITE(OUTLYNE,*)'DEFORMABLE SURFACE ACTUATOR VARIABLES ARE ONLY AVAILABLE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'AT THE CFG 1, ALTERNATE CONFIGURATION LEVEL'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     DEFAULT INPUT
      ...
      IF(WC.EQ.'AD'.OR.WC.EQ.'AE'.OR.WC.EQ.'AF'.OR.WC.EQ.'AG'.OR.WC.EQ.'AH'.OR.WC.EQ.'AI'.OR.WC.EQ.'AJ'.OR.WC.EQ.'AK'.OR.WC.EQ.'AL'.OR.WC.EQ.'ACK') THEN
         GOFORIT=.FALSE.
         IF(surf_toric_flag(INT(W1)).EQ.0.0D0.OR.surf_toric_flag(INT(W1)).EQ.1.0D0) THEN
!     NO TORIC OR Y-TORIC
...
!     DEFAULT INPUT
   ...
   IF(VHIGH.LE.VLOW) THEN
      WRITE(OUTLYNE,*)'NUMERIC WORD #4 (LOW LIMIT) MUST BE LESS THAN W5 (HIGH LIMIT)'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
!     HIGH AND LOW FOR TILTS
   IF(VALT.EQ.149) THEN
      IF(DF4.EQ.1) VLOW=0.0D0
   END IF
!     NOW BUILD THE VARIABLE ENTRY
!       J=1  > 1 THROUGH 4118, A VARIABLE TYPE DESIGNATOR
!       J=2  > 1 THROUGH MAXCFG, THE CONFIGURATION DESIGNATOR
!       J=3  > 0 THROUGH MAXSUR, THE SURFACE NUMBER DESIGNATOR
!       J=4  > VARIABLE CURRENT VALUE
!       J=5  > VARIABLE PREVIOUS VALUE
...
!     NO MORE VARIABLES
      WRITE(OUTLYNE,*)'THE MAXIMUM OF ',MAXVB,' VARIABLES HAS BEEN REACHED'
      CALL SHOWIT(1)
...
!
            IF(VALT.EQ.2.OR.VALT.EQ.1) VARABL(VBCNT+1,4)=surf_curvature(VBSURF)
            IF(VALT.EQ.2.OR.VALT.EQ.1) VARABL(VBCNT+1,5)=surf_curvature(VBSURF)
            IF(VALT.EQ.2.OR.VALT.EQ.1) VARABL(VBCNT+1,13)=surf_curvature(VBSURF)
...
!
            VARABL(VBCNT+1,4)=surf_thickness(VBSURF)
            VARABL(VBCNT+1,5)=surf_thickness(VBSURF)
            VARABL(VBCNT+1,13)=surf_thickness(VBSURF)
...
!
            IF(VALT.EQ.4) VARABL(VBCNT+1,4)=surf_conic(VBSURF)
            IF(VALT.EQ.4) VARABL(VBCNT+1,5)=surf_conic(VBSURF)
            IF(VALT.EQ.4) VARABL(VBCNT+1,13)=surf_conic(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.5) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 4)
            IF(VALT.EQ.5) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 4)
            IF(VALT.EQ.5) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 4)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.6) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 6)
            IF(VALT.EQ.6) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 6)
            IF(VALT.EQ.6) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 6)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.7) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 8)
            IF(VALT.EQ.7) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 8)
            IF(VALT.EQ.7) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 8)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.8) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 10)
            IF(VALT.EQ.8) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 10)
            IF(VALT.EQ.8) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 10)
...
!
            ...
            ELSE
               IF(VALT.EQ.10.OR.VALT.EQ.9) VARABL(VBCNT+1,4)=surf_toric_curvature(VBSURF)
               IF(VALT.EQ.10.OR.VALT.EQ.9) VARABL(VBCNT+1,5)=surf_toric_curvature(VBSURF)
               IF(VALT.EQ.10.OR.VALT.EQ.9) VARABL(VBCNT+1,13)=surf_toric_curvature(VBSURF)
               IF(VALT.EQ.10.OR.VALT.EQ.9) VARABL(VBCNT+1,11)=24.0D0
            END IF
            VBCNT=VBCNT+1
         END IF
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.11) VARABL(VBCNT+1,4)=surf_anamorphic_conic(VBSURF)
            IF(VALT.EQ.11) VARABL(VBCNT+1,5)=surf_anamorphic_conic(VBSURF)
            IF(VALT.EQ.11) VARABL(VBCNT+1,13)=surf_anamorphic_conic(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.12) VARABL(VBCNT+1,4)=surf_anamorphic_coeff(VBSURF, 4)
            IF(VALT.EQ.12) VARABL(VBCNT+1,5)=surf_anamorphic_coeff(VBSURF, 4)
            IF(VALT.EQ.12) VARABL(VBCNT+1,13)=surf_anamorphic_coeff(VBSURF, 4)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.13) VARABL(VBCNT+1,4)=surf_anamorphic_coeff(VBSURF, 6)
            IF(VALT.EQ.13) VARABL(VBCNT+1,5)=surf_anamorphic_coeff(VBSURF, 6)
            IF(VALT.EQ.13) VARABL(VBCNT+1,13)=surf_anamorphic_coeff(VBSURF, 6)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.14) VARABL(VBCNT+1,4)=surf_anamorphic_coeff(VBSURF, 8)
            IF(VALT.EQ.14) VARABL(VBCNT+1,5)=surf_anamorphic_coeff(VBSURF, 8)
            IF(VALT.EQ.14) VARABL(VBCNT+1,13)=surf_anamorphic_coeff(VBSURF, 8)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.15) VARABL(VBCNT+1,4)=surf_anamorphic_coeff(VBSURF, 10)
            IF(VALT.EQ.15) VARABL(VBCNT+1,5)=surf_anamorphic_coeff(VBSURF, 10)
            IF(VALT.EQ.15) VARABL(VBCNT+1,13)=surf_anamorphic_coeff(VBSURF, 10)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.16) VARABL(VBCNT+1,4)=surf_alpha_deg(VBSURF)
            IF(VALT.EQ.16) VARABL(VBCNT+1,5)=surf_alpha_deg(VBSURF)
            IF(VALT.EQ.16) VARABL(VBCNT+1,13)=surf_alpha_deg(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.17) VARABL(VBCNT+1,4)=surf_beta_deg(VBSURF)
            IF(VALT.EQ.17) VARABL(VBCNT+1,5)=surf_beta_deg(VBSURF)
            IF(VALT.EQ.17) VARABL(VBCNT+1,13)=surf_beta_deg(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.18) VARABL(VBCNT+1,4)=surf_gamma_deg(VBSURF)
            IF(VALT.EQ.18) VARABL(VBCNT+1,5)=surf_gamma_deg(VBSURF)
            IF(VALT.EQ.18) VARABL(VBCNT+1,13)=surf_gamma_deg(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.19) VARABL(VBCNT+1,4)=surf_focus_dx(VBSURF)
            IF(VALT.EQ.19) VARABL(VBCNT+1,5)=surf_focus_dx(VBSURF)
            IF(VALT.EQ.19) VARABL(VBCNT+1,13)=surf_focus_dx(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.20) VARABL(VBCNT+1,4)=surf_focus_dy(VBSURF)
            IF(VALT.EQ.20) VARABL(VBCNT+1,5)=surf_focus_dy(VBSURF)
            IF(VALT.EQ.20) VARABL(VBCNT+1,13)=surf_focus_dy(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.134) VARABL(VBCNT+1,4)=surf_focus_dz(VBSURF)
            IF(VALT.EQ.134) VARABL(VBCNT+1,5)=surf_focus_dz(VBSURF)
            IF(VALT.EQ.134) VARABL(VBCNT+1,13)=surf_focus_dz(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.143) VARABL(VBCNT+1,4)=surf_global_dx(VBSURF)
            IF(VALT.EQ.143) VARABL(VBCNT+1,5)=surf_global_dx(VBSURF)
            IF(VALT.EQ.143) VARABL(VBCNT+1,13)=surf_global_dx(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.144) VARABL(VBCNT+1,4)=surf_global_dy(VBSURF)
            IF(VALT.EQ.144) VARABL(VBCNT+1,5)=surf_global_dy(VBSURF)
            IF(VALT.EQ.144) VARABL(VBCNT+1,13)=surf_global_dy(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.145) VARABL(VBCNT+1,4)=surf_global_dz(VBSURF)
            IF(VALT.EQ.145) VARABL(VBCNT+1,5)=surf_global_dz(VBSURF)
            IF(VALT.EQ.145) VARABL(VBCNT+1,13)=surf_global_dz(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.146) VARABL(VBCNT+1,4)=surf_global_alpha(VBSURF)
            IF(VALT.EQ.146) VARABL(VBCNT+1,5)=surf_global_alpha(VBSURF)
            IF(VALT.EQ.146) VARABL(VBCNT+1,13)=surf_global_alpha(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.147) VARABL(VBCNT+1,4)=surf_global_beta(VBSURF)
            IF(VALT.EQ.147) VARABL(VBCNT+1,5)=surf_global_beta(VBSURF)
            IF(VALT.EQ.147) VARABL(VBCNT+1,13)=surf_global_beta(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.148) VARABL(VBCNT+1,4)=surf_global_gamma(VBSURF)
            IF(VALT.EQ.148) VARABL(VBCNT+1,5)=surf_global_gamma(VBSURF)
            IF(VALT.EQ.148) VARABL(VBCNT+1,13)=surf_global_gamma(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.149) VARABL(VBCNT+1,4)=surf_grating_spacing(VBSURF)
            IF(VALT.EQ.149) VARABL(VBCNT+1,5)=surf_grating_spacing(VBSURF)
            IF(VALT.EQ.149) VARABL(VBCNT+1,13)=surf_grating_spacing(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.137) VARABL(VBCNT+1,4)=surf_pivot_x(VBSURF)
            IF(VALT.EQ.137) VARABL(VBCNT+1,5)=surf_pivot_x(VBSURF)
            IF(VALT.EQ.137) VARABL(VBCNT+1,13)=surf_pivot_x(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.138) VARABL(VBCNT+1,4)=surf_pivot_y(VBSURF)
            IF(VALT.EQ.138) VARABL(VBCNT+1,5)=surf_pivot_y(VBSURF)
            IF(VALT.EQ.138) VARABL(VBCNT+1,13)=surf_pivot_y(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.139) VARABL(VBCNT+1,4)=surf_pivot_z(VBSURF)
            IF(VALT.EQ.139) VARABL(VBCNT+1,5)=surf_pivot_z(VBSURF)
            IF(VALT.EQ.139) VARABL(VBCNT+1,13)=surf_pivot_z(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.141) VARABL(VBCNT+1,4)=surf_clap_dim(VBSURF, 2)
            IF(VALT.EQ.141) VARABL(VBCNT+1,5)=surf_clap_dim(VBSURF, 2)
            IF(VALT.EQ.141) VARABL(VBCNT+1,13)=surf_clap_dim(VBSURF, 2)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.142) VARABL(VBCNT+1,4)=surf_clap_dim(VBSURF, 1)
            IF(VALT.EQ.142) VARABL(VBCNT+1,5)=surf_clap_dim(VBSURF, 1)
            IF(VALT.EQ.142) VARABL(VBCNT+1,13)=surf_clap_dim(VBSURF, 1)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.21) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 1)
            IF(VALT.EQ.21) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 1)
            IF(VALT.EQ.21) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 1)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.22) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 2)
            IF(VALT.EQ.22) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 2)
            IF(VALT.EQ.22) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 2)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.23) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 3)
            IF(VALT.EQ.23) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 3)
            IF(VALT.EQ.23) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 3)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.24) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 4)
            IF(VALT.EQ.24) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 4)
            IF(VALT.EQ.24) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 4)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.25) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 5)
            IF(VALT.EQ.25) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 5)
            IF(VALT.EQ.25) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 5)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.124) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 6)
            IF(VALT.EQ.124) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 6)
            IF(VALT.EQ.124) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 6)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.125) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 7)
            IF(VALT.EQ.125) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 7)
            IF(VALT.EQ.125) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 7)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.126) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 8)
            IF(VALT.EQ.126) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 8)
            IF(VALT.EQ.126) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 8)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.127) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 9)
            IF(VALT.EQ.127) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 9)
            IF(VALT.EQ.127) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 9)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.128) VARABL(VBCNT+1,4)=surf_refractive_index(VBSURF, 10)
            IF(VALT.EQ.128) VARABL(VBCNT+1,5)=surf_refractive_index(VBSURF, 10)
            IF(VALT.EQ.128) VARABL(VBCNT+1,13)=surf_refractive_index(VBSURF, 10)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.135) VARABL(VBCNT+1,4)=surf_fict_n(VBSURF)
            IF(VALT.EQ.135) VARABL(VBCNT+1,5)=surf_fict_n(VBSURF)
            IF(VALT.EQ.135) VARABL(VBCNT+1,13)=surf_fict_n(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.136) VARABL(VBCNT+1,4)=surf_fict_v(VBSURF)
            IF(VALT.EQ.136) VARABL(VBCNT+1,5)=surf_fict_v(VBSURF)
            IF(VALT.EQ.136) VARABL(VBCNT+1,13)=surf_fict_v(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.140) VARABL(VBCNT+1,4)=surf_fict_w(VBSURF)
            IF(VALT.EQ.140) VARABL(VBCNT+1,5)=surf_fict_w(VBSURF)
            IF(VALT.EQ.140) VARABL(VBCNT+1,13)=surf_fict_w(VBSURF)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.129) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 12)
            IF(VALT.EQ.129) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 12)
            IF(VALT.EQ.129) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 12)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.130) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 14)
            IF(VALT.EQ.130) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 14)
            IF(VALT.EQ.130) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 14)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.131) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 16)
            IF(VALT.EQ.131) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 16)
            IF(VALT.EQ.131) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 16)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.132) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 18)
            IF(VALT.EQ.132) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 18)
            IF(VALT.EQ.132) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 18)
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.133) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 20)
            IF(VALT.EQ.133) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 20)
            IF(VALT.EQ.133) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 20)
...
!
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)VARABL(VBCNT+1,4)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)VARABL(VBCNT+1,5)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)VARABL(VBCNT+1,13)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)VARABL(VBCNT+1,11)=DBLE(VALT-26)
!
            VBCNT=VBCNT+1
         END IF
         CALL VARCLN
...
!     NO CONFLICTING PIKUP EXISTS, ASSIGN THE VARIABLE
!
            IF(VALT.EQ.75) VARABL(VBCNT+1,4)=surf_asphere_coeff(VBSURF, 2)
            IF(VALT.EQ.75) VARABL(VBCNT+1,5)=surf_asphere_coeff(VBSURF, 2)
            IF(VALT.EQ.75) VARABL(VBCNT+1,13)=surf_asphere_coeff(VBSURF, 2)
...
!     DO ACT(0001 TO 4118)
      IF(VALT.GE.250.AND.VALT.LE.4218) THEN
         ...
         IF(surf_default_flag(VBSURF).EQ.1.0D0) THEN
         ELSE
            WRITE(OUTLYNE,*)'THE SPECIFIED SURFACE REQUIRES A "DEFORM" DEFINITION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'VARIABLE ASSIGNMENT WAS NOT MADE'
            CALL SHOWIT(1)
            CALL MACFAL
         ...
         END IF
!
         NCNT=INT(surf_mtracei_ny(VBSURF))**2
         DO IC=1,NCNT
...
!     CONFIG NOT DEFINED AS ACTIVE, DISSALLOW DEFINITION
         WRITE(OUTLYNE,*)'REQUESTED CONFIGURATION IS CURRENTLY EMPTY'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,666) WC,INT(W1),VBCFG
         CALL SHOWIT(1)
...
!     CONFIG SUBFILE DEFINITION FOR THAT CONFIGURATION
      VAROK=.FALSE.
      DO I=1,AUXMAX
         IF(CFADD(I,2).EQ.VBSURF.AND.CFADD(I,3).EQ.VBCFG.AND.CFADD(I,1).EQ.VALT) VAROK=.TRUE.
      END DO
      IF(.NOT.VAROK) THEN
         WRITE(OUTLYNE,*)'REQUESTED VARIABLE NOT EXPLICITLY DEFINED IN SPECIFIED',' CONFIGURATION'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,666) WC,INT(W1),VBCFG
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'VARIABLE ASSIGNMENT WAS NOT MADE'
      ...
      END IF
      DO I=1,AUXMAX
         IF(CFADD(I,2).EQ.VBSURF.AND.CFADD(I,3).EQ.VBCFG) THEN
!
!     WE HAVE A CONFIG AND A SURFACE NUMBER MATCH
!
            ISRDCV=.FALSE.
            ISRDCVT=.FALSE.
!     REMEMBER WHERE THE VARIABLE IS IN THE AUXILLIARY ARRAYS
            IF(VALT.EQ.1.AND.CFADD(I,1).EQ.1.OR.VALT.EQ.2.AND.CFADD(I,1).EQ.2) ISRDCV=.TRUE.
            IF(VALT.EQ.9.AND.CFADD(I,1).EQ.9.OR.VALT.EQ.10.AND.CFADD(I,1).EQ.10) ISRDCV=.TRUE.
...
!     NOT RD/CV OR RDTOR/CVTOR
               VARABL(VBCNT+1,14)=DBLE(I)
!     WE FOUND THE VARIABLE DEFINED EXPLICITLY IN THE CONFIG
!     FILE SO ALL IS OK
!     ASSIGN WHAT NEEDS TO BE ASSIGNED  THEN GO TO 2000 FOR FURTHER
!     PROCESSING
               IF(CFADD(I,1).GE.27.AND.CFADD(I,1).LE.74.OR.CFADD(I,1).GE.76.AND.CFADD(I,1).LE.123) THEN
               ...
               ELSE
                  ...
                  END IF
               END IF
!     MATCH VARIABLE TYPE AND SURFACE NUMBER IN THIS CONFIGUATION
            END IF
         END IF
2000     CONTINUE
      END DO
!     FALLING THROUGH MEANS NO MATCH
!
   END IF
!
   CALL VARCLN
...
!
!     THIS READS THE CURRENT VARIABLES LIST AND RE-SUBMITS THE COMMANDS
...
!       AND J TAKES ON THE FOLLOWING VALUES AND MEANIINGS.
!
!       J=1  > 1 THROUGH 150, PLUS ACT STARTING AT 250 A VARIABLE TYPE DESIGNATOR
!       J=2  > 1 THROUGH MAXCFG, THE CONFIGURATION DESIGNATOR
!       J=3  > 0 THROUGH MAXSUR, THE SURFACE NUMBER DESIGNATOR
!       J=4  > VARIABLE CURRENT VALUE
!       J=5  > VARIABLE PREVIOUS VALUE
...
!
      IF(INT(JKVAR(I,2)).GT.LASCFG.OR.INT(JKVAR(I,2)).GT.MAXCFG) THEN
