...
!
   IF(WC.EQ.'CFG') THEN
      ...
      IF(INT(W1).GT.LASCFG.AND.INT(W1).NE.1) THEN
         WRITE(OUTLYNE,*)&
         &'REQUESTED CONFIGURATION IS CURRENTLY EMPTY'
         CALL SHOWIT(1)
...
!       NOW DO WC=DEL
   IF(WC.EQ.'DELK') THEN
      IF(F29.NE.2) THEN
         WRITE(OUTLYNE,*)&
         &'"DEL" IS ONLY AVAILABLE FROM THE "UPDATE VARIABLE" LEVEL'
         CALL SHOWIT(1)
      ...
      &) THEN
         WRITE(OUTLYNE,*)&
         &'"DEL" ONLY TAKES QUALIFIER AND NUMERIC WORDS #1 AND #2'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      ELSE
         IF(WQ.EQ.'MACVAR') THEN
            IF(INT(W1).LT.1.OR.INT(W1).GT.MAXREG) THEN
               WRITE(OUTLYNE,*)&
               &'NUMERIC WORD #1 (GP REGISTER NUMBER) BEYOND LEGAL BOUNDS'
               CALL SHOWIT(1)
               WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
               CALL SHOWIT(1)
               CALL MACFAL
...
!     DOES THE DESIGNATED VARIABLE EXIST ?
      ...
      DO I=1,VBCNT
         ...
         &.EQ.INT(WORD2).AND.INT(VARABL(I,3)).EQ.INT(W1)) THEN
            J=J+1
            DELTAG=I
...
!     ALL DELETIONS COMPLETED
      ...
      ELSE
         WRITE(OUTLYNE,*)&
         &TRIM(WQ),' VARIABLE FOR SURFACE ',INT(W1)
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'WAS NOT IN THE VARIABLE SUBFILE, NO DELETION PERFORMED'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     NOT AT CFG 1, NO ASSIGNMENT MADE
         WRITE(OUTLYNE,*)&
         &'DEFORMABLE SURFACE ACTUATOR VARIABLES ARE ONLY AVAILABLE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'AT THE CFG 1, ALTERNATE CONFIGURATION LEVEL'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     DEFAULT INPUT
   ...
   IF(VHIGH.LE.VLOW) THEN
      WRITE(OUTLYNE,*)&
      &'NUMERIC WORD #4 (LOW LIMIT) MUST BE LESS THAN W5 (HIGH LIMIT)'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     NO MORE VARIABLES
      WRITE(OUTLYNE,*)&
      &'THE MAXIMUM OF ',MAXVB,' VARIABLES HAS BEEN REACHED'
      CALL SHOWIT(1)
...
!     DO C1 TO 96
      ...
      &.OR.VALT.GE.76.AND.VALT.LE.123) THEN
...
!
            ...
            &VARABL(VBCNT+1,4)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)&
            &VARABL(VBCNT+1,5)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)&
            &VARABL(VBCNT+1,13)=FTFL01(VALT-27,VBSURF)
            ...
            IF(VALT.GE.76.AND.VALT.LE.123)&
            &VARABL(VBCNT+1,11)=DBLE(VALT-26)
!
            VBCNT=VBCNT+1
         END IF
         CALL VARCLN
...
!     DO ACT(0001 TO 4118)
      IF(VALT.GE.250.AND.VALT.LE.4218) THEN
         ...
         ELSE
            WRITE(OUTLYNE,*)&
            &'THE SPECIFIED SURFACE REQUIRES A "DEFORM" DEFINITION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'VARIABLE ASSIGNMENT WAS NOT MADE'
            CALL SHOWIT(1)
            CALL MACFAL
...
!     CONFIG SUBFILE DEFINITION FOR THAT CONFIGURATION
      ...
      DO I=1,AUXMAX
         ...
         &CFADD(I,3).EQ.VBCFG.AND.CFADD(I,1).EQ.VALT) VAROK=.TRUE.
      END DO
      IF(.NOT.VAROK) THEN
         WRITE(OUTLYNE,*)&
         &'REQUESTED VARIABLE NOT EXPLICITLY DEFINED IN SPECIFIED'&
         &,' CONFIGURATION'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,666) WC,INT(W1),VBCFG
         CALL SHOWIT(1)
...
!     PROCESSING
               ...
               &CFADD(I,1).GE.76.AND.CFADD(I,1).LE.123) THEN
