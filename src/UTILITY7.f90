!       SEVENTH SET OF UTILTIY ROUTINES GO HERE
...
!     GENERAL PURPOSE NUMERIC STORAGE REGISTERS
!     USING THE FORMAT SPECIFIED BY THE LFORMAT COMMAND
!
   CHARACTER CSTRING*132,LFORM1*82,LLAB*80,WSSA*150
!
...
!     QUALIFIER CHECK
   IF(SQ.EQ.1) THEN
!     CHECK FOR VALID QUALIFIER WORDS FOR "LWRITE"
      ...
      WRITE(OUTLYNE,*)&
      &'INVALID QUALIFIER WORD USED WITH "LWRITE"'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
...
20    CONTINUE
      ...
      IF(WQ.EQ.'A399')  I=399
      IF(WQ.EQ.'A400')  I=400
!     CHECK CONTENTS OF ALPHA REGISTER
      DO J=80,1,-1
...
21    CONTINUE
   ...
   DO I=1,5
      ...
      &.OR.I.EQ.1.AND.INT(W1).GT.400..AND.DF1.EQ.0.OR.&
      &I.EQ.2.AND.INT(W2).LT.1.AND.DF2.EQ.0 &
      &.OR.I.EQ.1.AND.INT(W1).GT.400.AND.DF2.EQ.0.OR.&
      &I.EQ.3.AND.INT(W3).LT.1.AND.DF3.EQ.0 &
      &.OR.I.EQ.3.AND.INT(W1).GT.400.AND.DF2.EQ.0.OR.&
      &I.EQ.4.AND.INT(W4).LT.1.AND.DF4.EQ.0 &
      &.OR.I.EQ.4.AND.INT(W1).GT.400.AND.DF2.EQ.0.OR.&
      &I.EQ.5.AND.INT(W5).LT.1.AND.DF5.EQ.0 &
      &.OR.I.EQ.5.AND.INT(W1).GT.400.AND.DF2.EQ.0) THEN
...
69 CONTINUE
   WRITE(OUTLYNE,*)&
   &'LINE FORMAT DOES NOT AGREE WITH OUTPUT LIST'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)&
   &'RE-ISSUE THE "LFORMAT" OR THE "LWRITE" COMMAND'
   CALL SHOWIT(1)
   CALL MACFAL
   RETURN
END
...
200   FORMAT(A79)
   ...
   IF(SQ.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)&
      &WC,' ONLY TAKES STRING OR NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   DO 10 I=1,5
      ...
      &DABS(N).LE.0.001D0.AND.DABS(N).GT.0.0001D0) THEN
         WRITE(B,180) N
         READ(B,300) BN
         IF(I.EQ.1) BN1=BN
         IF(I.EQ.2) BN2=BN
      ...
      &DABS(N).LE.0.01D0.AND.DABS(N).GT.0.001D0) THEN
         WRITE(B,170) N
         READ(B,300) BN
         IF(I.EQ.1) BN1=BN
         IF(I.EQ.2) BN2=BN
      ...
      &DABS(N).LE.0.1D0.AND.DABS(N).GT.0.01D0) THEN
         WRITE(B,160) N
         READ(B,300) BN
         IF(I.EQ.1) BN1=BN
         IF(I.EQ.2) BN2=BN
      ...
      &DABS(N).LE.1.0D0.AND.DABS(N).GT.0.1D0) THEN
         WRITE(B,150) N
         READ(B,300) BN
         IF(I.EQ.1) BN1=BN
         IF(I.EQ.2) BN2=BN
...
!
      IF(SQ.EQ.1.OR.SN.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"M" TAKES ONLY STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!       'M'- CAUSES THE MESSAGE IN THE STRING TO BE PRINTED TO THE
!       DEFAULT OUTPUT DEVICE. REMEMBER THE STRING BEGINS WITH A COLON.
!
!       EXAMPLE:
!
!       M,:THE NEXT INSTRUCTION CALCULATES POINT SPREAD FUNCTION.
!
!       M ,1 PRINTS MESSAGE STORED IN THE AGPREG(1) REGISTER
!
!       PRODUCES AT THE CURRENT OUTPUT DEVICE:
!
!               THE NEXT INSTRUCTION CALCULATES POINT SPREAD FUNCTION.
...
!       NOT MESAGE
   ...
   IF(WC.EQ.'CK') THEN
      IF(SQ.EQ.1.OR.SN.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"C" TAKES ONLY STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!       BLANK INPUT EQUATES TO :(BLANK)
!
!       'C'- THE COMMENT DOESN'T DO ANYTHING. IT JUST ALLOWS
!       ANNOTATION OF INPUT  OR ANNOTATION
!       OF MACROS. IT WORKS LIKE THE COMMENT LINE IN FORTRAN.
!
      RETURN
   ELSE
!       NOT COMENT
...
!
   ...
   END IF
   IF(SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"FIGURE" TAKES ONLY STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   FIGTITLE=WS
   RETURN
...
!
   ...
   IF(WC.EQ.'AVEC') THEN
      IF(S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"AVEC" ONLY TAKES NUMERIC WORD #1, #2 AND #3 INPUT'
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
      AXVEC=W1
   ...
   IF(WC.EQ.'BVEC') THEN
      IF(S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &'"BVEC" ONLY TAKES NUMERIC WORD #1, #2 AND #3 INPUT'
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
      BXVEC=W1
...
&(X,Y,Z,X0,Y0,Z0,L,M,N,XP,YP,ZP,LP,MP,NP,ERROR)
!
!       RETURNS THE X,Y,Z LOCATION WHEN VECTOR X,Y,Z,L,M,N (STARTING
!       AT X0,Y0,Z0 INTERSECTS THE PLANE WHOSE NORMAL VECTOR IS IN THE
