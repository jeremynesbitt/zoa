!       FOURTH FILE OF CAPFN/SPOT ROUTINES
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"FAIL" OR "FAILACC" TAKE NO QUALIFIER OR STRING INPUT'
      CALL SHOWIT(1)
...
!       DEFAULT NUMERIC INPUT
   ...
   END IF
!
!       CHECK FOR VALID SURFACE NUMBERS
   IF(INT(W1).LT.NEWOBJ) THEN
      OUTLYNE='NUMERIC WORD #1 SURFACE NUMBER BEYOND LEGAL BOUNDS'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   ...
   IF(DF2.EQ.0) THEN
      IF(INT(W2).GT.NEWIMG) THEN
         OUTLYNE='NUMERIC WORD #2 SURFACE NUMBER BEYOND LEGAL BOUNDS'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
      ...
      IF(INT(W2).LT.INT(W1)) THEN
         OUTLYNE='SURFACE NUMBERS OUT OF ORDER FOR THE CURRENT COMMAND'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
      END IF
   ELSE
!       W2 IS DEFAULT AND NOT USED
   END IF
!
   IF(.NOT.SPDEXT) THEN
      OUTLYNE='NO CURRENT SPOT DIAGRAM DATA EXISTS'
      CALL SHOWIT(1)
...
!     FOUND FAILED RAY
         IF(INT(DSPOT(8)).GE.NS1.AND.INT(DSPOT(8)).LE.NS2) NFAIL=NFAIL+1
      ELSE
...
!       NOW SET THE GENERAL PURPOSE REGISTER #1 TO THE
!       REAL*8 REPRESENTATION OF THE TOTAL NUMBER OF FAILED
!       RAYS
      GPREG(1)=DBLE(NFAIL)
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE SPDQAL DOES SPD WITH QUALIFIER WORDS.
!     VALID QUALIFIERS ARE MOVE
!
!
...
!
   IF(STI.EQ.1) THEN
      IF(REFLOC.EQ.1) THEN
!     CHIEF IS SET
100      FORMAT('SPOT DIAGRAM REFERNCE SPHERE CENTER LIES ON THE CHIEF RAY')
         WRITE(OUTLYNE,100)
         CALL SHOWIT(0)
      END IF
      IF(REFLOC.EQ.3) THEN
!     CENT IS SET
300      FORMAT('SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT')
         WRITE(OUTLYNE,300)
         CALL SHOWIT(0)
      ...
      IF(REFLOC.EQ.4) THEN
!     CENT IS SET
400      FORMAT('SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT AND FOCUS')
         WRITE(OUTLYNE,400)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,305)DLLX
         CALL SHOWIT(0)
   ...
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'CHIEF'.AND.WQ.NE.'NOTILT'.AND.WQ.NE.'BEST') THEN
   ...
   IF(WQ.EQ.'CHIEF') THEN
      ...
      DLLY=0.0D0
      DLLZ=0.0D0
301   FORMAT('SPOT DIAGRAM REFERENCE SPHERE CENTER WILL LIE ON THE CHIEF RAY')
      WRITE(OUTLYNE,301)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'NOTILT') THEN
      ...
      DLLY=0.0D0
      DLLZ=0.0D0
601   FORMAT('SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT')
      WRITE(OUTLYNE,601)
      CALL SHOWIT(0)
      RETURN
   END IF
   IF(WQ.EQ.'BEST') THEN
      ...
      DLLY=0.0D0
      DLLZ=0.0D0
701   FORMAT('SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT AND FOCUS')
      WRITE(OUTLYNE,701)
      CALL SHOWIT(0)
      RETURN
   END IF
...
!
   ...
   END DO
!     THE LARGER ARRAY IS IN STORAGE
!     WHAT IS THE LARGEST X AND Y VALUE
   MAXX=-1.0D+300
   MAXY=-1.0D+300
!
   IRED=IRED-1
...
!     MAXI IS THE SEMI-DIMENSION OF THE SQUARE WHICH ENSQUARES
!     ALL THE ENERGY. DIVIDE IT INTO IRED PARTS
...
!
   ...
   DO I=1,IRED
      ...
      IF(LEN.GT.0.0D0) THEN
         DO J=1,IRED
!     CALCULATE THE AMOUNT OF ENERGY INSIDE IT
            IF(DABS(LARGER(J,2)).LE.LEN.AND.DABS(LARGER(J,3)).LE.LEN) THEN
!     POINT IS INSIDE THE CURRENT SQUARE
...
!
!       THIS IS SUBROUTINE SAGFLROT.FOR. THIS SUBROUTINE CONTROLS
!     SAG FILE PLOT ROTATION OUTPUT
!
!
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE PSFSPOT
!       IT CREATES A SPOT DIAGRAM FILE FOR THE PSF.DAT FILE
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   REAL*8 EXTENT,SPACING,PSFXCENT,PSFYCENT,CRAYX,CRAYY ,X,Y
!
...
!      CALL IOSDELETEFILE('SPDPSF.DAT')
   OPEN(UNIT=53,ACCESS='SEQUENTIAL',FILE='SPDPSF.DAT',FORM='FORMATTED',STATUS='UNKNOWN')
!
!     DOES THE PSF FILE EXIST WITH AN EXISTING PSF
   EXIS51=.FALSE.
...
!     MMM IS ODD SO
      ...
      ALLOCATE(SPDPSF1(MMM**2),SPDPSF2(MMM**2),SPDPSF3(MMM**2),STAT=ALLOERR)
1016  FORMAT(I8)
      II=1
   ...
   ELSE
      IRRER=.TRUE.
!     NO ANALYSIS CAN BE DONE, NO PSF.DAT EXISTS
      RETURN
   END IF
   RETURN
END
...
!     CALLED BY CMDER FOR COMMANDS PIXEL AND CENTROID
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   REAL*8 EXTENT,SPACING,PSFXCENT,PSFYCENT,XCEN,YCEN ,XLSF,YLSF,PSF,DDUMMY,SHIFTER,SUMMERIT,XCENVAL,XCNT,YCNT ,SUMMX,SUMMY,TRUTH,YCENVAL,XRMS,YRMS,XMEEN,YMEEN,XSUMM,YSUMM ,XSSUMM,YSSUMM
!
   REAL*8 JXSUM,JYSUM,XSUM,YSUM,XDUM,YDUM ,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT ,XTOT1,YTOT1,TOT1
!
   INTEGER PPGR,IPSF,JJJ,I,J,K,SHIFT,PIX2,ISHIFT,JSHIFT ,SSHIFT,SHIFTY,PMMM,IP,JP,IQ,JQ,IR,JR,PGG
!
   DIMENSION XLSF(:),YLSF(:),XCEN(:),YCEN(:),XDUM(:),YDUM(:),PSF(:,:),IPSF(:,:),SHIFTER(:,:),SUMMERIT(:,:),SUMMX(:),SUMMY(:)
!
   INTEGER MMM,PIXAR1,PIXAR2,DUMMY,DUMIND,JJ,PRG,II
!
   COMMON/TOYSTORY/PIXAR1,PIXAR2
!
   ALLOCATABLE :: SHIFTER,IPSF,PSF,XLSF,YLSF,XCEN,YCEN,XDUM,YDUM ,SUMMERIT,SUMMX,SUMMY
!
...
!     WC='PIXEL'
   IF(WC.EQ.'PIXEL') THEN
      IF(STI.EQ.1) THEN
         OUTLYNE='"PIXEL" SETS UP PARAMETERS FOR PIXEL LINEARITY ANALYSIS'
         CALL SHOWIT(1)
         OUTLYNE='NUMERIC WORD #1 IS THE NUMBER OF SUB-PIXELS PER PIXEL'
         CALL SHOWIT(1)
         OUTLYNE='NUMERIC WORD #2 IS THE NUMBER OF PIXELS IN THE WINDOW'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'CURRENT NUMBER OF SUB-PIXELS PER PIXEL IS : ',PIXAR1
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'   CURRENT NUMBER PIXELS IN THE WINDOW IS : ',PIXAR2
         CALL SHOWIT(1)
         RETURN
      END IF
      IF(sys_mode().GE.3.0D0) THEN
         OUTLYNE='"PIXEL" IS NOT OPERATIONAL WITH AFOCAL SYSTEMS'
         CALL SHOWIT(1)
         OUTLYNE='NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
      IF(SST.EQ.1.OR.SQ.EQ.1) THEN
         OUTLYNE='"PIXEL" TAKES NO STRING OR QUALIFIER WORD INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE='"PIXEL" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
         OUTLYNE='"PIXEL" REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(W1.LE.0.0D0) THEN
         OUTLYNE='NUMERIC WORD #1 MUST BE A POSITIVE INTEGER'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(((DBLE(INT(W1/2.0D0))*2.0D0)-W1).EQ.0.0D0) THEN
         OUTLYNE='NUMERIC WORD #1 MUST BE AN ODD INTEGER'
         CALL SHOWIT(1)
         OUTLYNE='IT MAY NOT BE AN EVEN INTEGER'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(W2.LT.1.0D0) THEN
         OUTLYNE='NUMERIC WORD #2 MUST BE A POSITIVE INTEGER'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(W1.LE.W2) THEN
         OUTLYNE='NUMERIC WORD #1 MUST BE GREATER THAN NUMERIC WORD #2'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     WC='CENTROID'
   IF(WC.EQ.'CENTROID') THEN
      ...
      END IF
      IF(STI.EQ.1) THEN
         OUTLYNE='"CENTROID" PERFORMS A COMPLETE LINEARITY ANALYSIS'
         CALL SHOWIT(1)
         RETURN
      END IF
      IF(SST.EQ.1.OR.SN.EQ.1) THEN
         OUTLYNE='"CENTROID" TAKES NO NUMERIC OR STRING INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(sys_mode().GE.3.0D0) THEN
         OUTLYNE='"CENTROID" IS NOT OPERATIONAL WITH AFOCAL SYSTEMS'
         CALL SHOWIT(1)
         OUTLYNE='NO ACTION TAKEN'
         CALL SHOWIT(1)
         CALL MACFAL
      END IF
      IF(SQ.EQ.1.AND.WQ.NE.'LSF'.AND.WQ.NE.'SLSF'.AND.WQ.NE.'PSFK'.AND.WQ.NE.'SPSF') THEN
         OUTLYNE='"CENTROID" ONLY TAKES "LSF", "PSF", "SLSF" AND "SPSF"'
         CALL SHOWIT(1)
...
!     DOES THE PSF FILE EXIST WITH AN EXISTING PSF
      EXIS51=.FALSE.
      IF(WQ(1:1).NE.'S'.OR.SQ.EQ.0)INQUIRE(FILE='PSF.DAT',EXIST=EXIS51)
      IF(WQ(1:1).EQ.'S') INQUIRE(FILE='SPSF.DAT',EXIST=EXIS51)
      IF(EXIS51) THEN
!     DO ANALYSIS
         OPEN51=.FALSE.
         IF(SQ.EQ.0.OR.WQ.EQ.'PSFK'.OR.WQ.EQ.'LSF')INQUIRE(FILE='PSF.DAT',OPENED=OPEN51)
         IF(WQ(1:1).EQ.'S')INQUIRE(FILE='SPSF.DAT',OPENED=OPEN51)
         IF(.NOT.OPEN51) THEN
!     OPEN FILE
            IF(SQ.EQ.0.OR.WQ.EQ.'PSFK'.OR.WQ.EQ.'LSF')OPEN(UNIT=51,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='PSF.DAT',STATUS='UNKNOWN')
            IF(WQ(1:1).EQ.'S') OPEN(UNIT=51,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='SPSF.DAT',STATUS='UNKNOWN')
            REWIND(UNIT=51)
         ELSE
            REWIND(UNIT=51)
         END IF
!     NOW ALLOCATE AND LOAD ARRAYS
!     READ SOME STUFF
         READ(51,1001) PSFTAG
1001     FORMAT(A12)
         READ(51,1002) PSFLI
...
1006     FORMAT(I4,G23.15)
         IF(WQ.EQ.'LSF'.OR.WQ.EQ.'SLSF'.OR.SQ.EQ.0) THEN
            ...
            ALLOCATE(XLSF(-PPGR:PPGR),YLSF(-PPGR:PPGR),STAT=ALLOERR)
         END IF
         IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'SPSF') THEN
            DEALLOCATE(PSF,IPSF,STAT=ALLOERR)
            ALLOCATE(IPSF(-PMMM:PMMM,-PMMM:PMMM),PSF(-PMMM:PMMM,-PMMM:PMMM),STAT=ALLOERR)
         END IF
!     LOAD PSF/LSF ARRAYS
         DEALLOCATE(SPDPSF1,SPDPSF2,SPDPSF3,STAT=ALLOERR)
         ALLOCATE(SPDPSF1(MMM**2),SPDPSF2(MMM**2),SPDPSF3(MMM**2),STAT=ALLOERR)
         ...
         DO J=-PMMM,PMMM
            DO I=-PMMM,PMMM
               READ(51,1005) IDUM,JDUM,IPSF3(II)
               IF(WQ.EQ.'SPSF'.OR.WQ.EQ.'PSFK') THEN
...
!     READ IN THE YLSF
         DO I=-PPGR,PPGR
            ...
            END IF
         END DO
!     NEXT LINE BETTER BE EOF
         READ(51,*) EEOOFF
!     LSF ARRAYS LOADED
         REWIND(UNIT=51)
         CALL CLOSE_FILE(51,1)
!     FINISHED CENTROID ANALYSIS
         IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'SPSF') THEN
!******************************************************************************
!     PSF CENTROIDING
            IF(PGR.LT.((PIXAR1*PIXAR2)+PIXAR1)) THEN
               ...
               RETURN
            END IF
            IF(((DBLE(PIXAR2)/2.0D0)-DBLE(INT(DBLE(PIXAR2)/2.0D0))).NE.0.0D0) THEN
!     PIXAR2 ODD
               PIX2=1
...
!     A NEW SSHIFT VALUE, ZERO OUT THE SUMMERIT ARRAY
                     DO JQ=1,PIXAR2
                        DO IQ=1,PIXAR2
...
!     ODD NUMBER OF PIXELS
                     DO JP=-PGG,PGG
                        ...
                        END DO
                     END DO
!     NOW DO ANALYSIS ON THE SHIFTER ARRAY
!     AT PIXEL JQ,IQ
                     DO JQ=1,PIXAR2
                        DO IQ=1,PIXAR2
!     NEW PIXEL, ZERO THE SUMMINT VARIABLE
                           SUMMINT=0.0D0
!     SUMMINT IS AN INTERMEDIATE SUMMATION VALUE
                           DO JR=1,PIXAR1
                              DO IR=1,PIXAR1
                                 SUMMINT=SUMMINT+SHIFTER(-PGG-1+((IQ-1)*PIXAR1)+IR,-PGG-1+((JQ-1)*PIXAR1)+JR)
                              END DO
                           END DO

!     SUMMER IS THE ENERGY SUM OF THE IQ,JQ TH PIXEL
                           SUMMERIT(IQ,JQ)=SUMMINT
                        END DO
                     END DO
!     EVERYTIME WE GET HERE WE HAVE A NEW CENTROID VALUES TO CALCULATE
!     FOR A NEW SHIFT POSITION
!
!     CALCULATE CENTROID POSITIONS
!
!
!     X CENTROID, SUMM ALL THE Y VALUES
                     ...
                     XCENVAL=(JXSUM/XTOT1)-(TOT1/DBLE(PIXAR2))-TRUTH
                     XSUMM=XSUMM+XCENVAL
                     XSSUMM=XSSUMM+(XCENVAL**2)
                     XCNT=XCNT+1.0D0
!
!
!     Y CENTROID, SUMM ALL THE X VALUES
                     DEALLOCATE(SUMMX,SUMMY,STAT=ALLOERR)
                     ALLOCATE(SUMMX(PIXAR2),SUMMY(PIXAR2),STAT=ALLOERR)
                     DO IQ=1,PIXAR2
                        SUMMX(IQ)=0.0D0
...
!     NUMBER OF PIXELS IS ODD
                     ...
                     YCENVAL=(JYSUM/YTOT1)-(TOT1/DBLE(PIXAR2))-TRUTH
                     YSUMM=YSUMM+(YCENVAL)
...
!     X CENTROID, SUMM ALL THE Y VALUES
                     ...
                     DO IQ=1,PIXAR2
                        ...
                        JXSUM=JXSUM+(DBLE(IQ)*SUMMX(IQ))
                     END DO
                     IF(ISHIFT.LT.0)TRUTH=PSFXCENT+((DBLE(ISHIFT)+0.5D0)/DBLE(PIXAR1))
                     IF(ISHIFT.GE.0)TRUTH=PSFXCENT+((DBLE(ISHIFT+1)-0.5D0)/DBLE(PIXAR1))
                     XCENVAL=(JXSUM/XTOT1)-(TOT1/DBLE(PIXAR2))-TRUTH
                     XSUMM=XSUMM+XCENVAL
                     XSSUMM=XSSUMM+(XCENVAL**2)
                     XCNT=XCNT+1.0D0
...
!     Y CENTROID, SUMM ALL THE X VALUES
                     ...
                     DO JQ=1,PIXAR2
                        ...
                        JYSUM=JYSUM+(DBLE(JQ)*SUMMY(JQ))
                     END DO
                     IF(JSHIFT.LT.0)TRUTH=PSFYCENT+((DBLE(JSHIFT)+0.5D0)/DBLE(PIXAR1))
                     IF(JSHIFT.GE.0)TRUTH=PSFYCENT+((DBLE(JSHIFT+1)-0.5D0)/DBLE(PIXAR1))
                     YCENVAL=(JYSUM/YTOT1)-(TOT1/DBLE(PIXAR2))-TRUTH
...
!
               ...
               END DO
            END IF
!     NOW PROCESS TO GET MEAN AND RMS CENTROIDS
!     IN X AND Y
            XMEEN=XSUMM/XCNT
            YMEEN=YSUMM/YCNT
            XRMS=DSQRT((XSSUMM-((XSUMM**2)/XCNT))/(XCNT-1.0D0))
            YRMS=DSQRT((YSSUMM-((YSUMM**2)/YCNT))/(YCNT-1.0D0))
...
40          FORMAT('CENTROID RMS Y-CENTROID ERROR (IN PIXELS) = ',G13.6)
            IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'SPSF') WRITE(OUTLYNE,3)
            IF(WQ.EQ.'LSF'.OR.WQ.EQ.'SLSF'.OR.SQ.EQ.0) WRITE(OUTLYNE,6)
            CALL SHOWIT(0)
            IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'LSF'.OR.SQ.EQ.0) WRITE(OUTLYNE,4)
...
!     SET UP THE SHIFT VALUE
            ...
            END IF
!
!     ALLOCATE THE DUMMY ARRAYS FOR CENTROIDING
            DEALLOCATE(XDUM,YDUM,STAT=ALLOERR)
            ALLOCATE(XDUM(1:(PIXAR1*PIXAR2)),YDUM(1:(PIXAR1*PIXAR2)),STAT=ALLOERR)
!     CALCULATE CENTROIDS BASED ON THE ARITHMETIC MEAN ALGORITHM
!     AND THE CURRENT DEFINED WINDOW
!
!**********************************************************************
            IF(PIX2.EQ.1) THEN
!     NUMBER OF PIXELS IS ODD (ODD NUMBER OF PIXELS)
!**********************************************************************
...
!
               ...
               JKSUMY2=0.0D0
               DO I=1,SHIFT
!     BEFORE WE DO A SHIFT POSITION, ZERO THE SUMMING VARIABLES
!     ZERO THE CENTROID ARRAYS AND LOAD THE XDUM AND YDUM ARRAYS
                  XCEN(I)=0.0D0
                  YCEN(I)=0.0D0
                  JXSUM=0.0D0
                  JYSUM=0.0D0
...
!
                  ...
                  DO J=1,PIXAR2
!     WE ARE NOW DOING A CENTROID SUMMATION OVER PIXEL J
                     XSUM=0.0D0
                     YSUM=0.0D0

                     DO K=1,PIXAR1

!     HERE WE SUM THE VALUES OF PIXAR1 SUB-PIXELS OF PIXEL J
                        XSUM=XSUM+XDUM(K+((J-1)*PIXAR1))
                        YSUM=YSUM+YDUM(K+((J-1)*PIXAR1))
                     END DO

!     HERE WE FORM THE WEIGHTED SUMMATIONS FOR CENTROID CALCULATIONS
                  ...
                  XCEN(I)=(JXSUM/XTOT1)-(TOT1/DBLE(PIXAR2))-(((DBLE(SHIFT-1)/2.0D0)+1.0D0-DBLE(I))/(DBLE(SHIFT)))-PSFXCENT
!
                  YCEN(I)=(JYSUM/YTOT1)-(TOT1/DBLE(PIXAR2))-(((DBLE(SHIFT-1)/2.0D0)+1.0D0-DBLE(I))/(DBLE(SHIFT)))-PSFYCENT
...
2009              FORMAT(I2,1X,G13.6,1X,G13.6)
               ...
               END DO
               DEVX=DSQRT((JKSUMX2-((JKSUMX1**2)/DBLE(PIXAR1)))/(DBLE(PIXAR1)-1.0D0))
               DEVY=DSQRT((JKSUMY2-((JKSUMY1**2)/DBLE(PIXAR1)))/(DBLE(PIXAR1)-1.0D0))
               IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'SPSF') WRITE(OUTLYNE,3)
               IF(WQ.EQ.'LSF'.OR.WQ.EQ.'SLSF'.OR.SQ.EQ.0) WRITE(OUTLYNE,6)
               CALL SHOWIT(0)
               IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'LSF'.OR.SQ.EQ.0) WRITE(OUTLYNE,4)
...
!     HERE WE FORM THE WEIGHTED SUMMATIONS FOR CENTROID CALCULATIONS
                  ...
                  XCEN(I)=(JXSUM/XTOT1)-(TOT1/DBLE(PIXAR2))+(((DBLE(SHIFT)/2.0D0)-DBLE(I-1))/DBLE(SHIFT))-PSFXCENT
!
                  YCEN(I)=(JYSUM/YTOT1)-(TOT1/DBLE(PIXAR2))+(((DBLE(SHIFT)/2.0D0)-DBLE(I-1))/DBLE(SHIFT))-PSFYCENT
            ...
            END IF
!
            DEALLOCATE(SHIFTER,IPSF,PSF,XLSF,YLSF,XCEN,YCEN,XDUM,YDUM,SUMMERIT,STAT=ALLOERR)
            RETURN
         END IF
      ELSE
!     NO ANALYSIS CAN BE DONE
         OUTLYNE='NO CENTROID ANALYSIS CAN BE PERFORMED.'
         CALL SHOWIT(1)
         IF(SQ.EQ.0) OUTLYNE='"CENTROID" REQUIRES A PSF.DAT FILE TO EXIST'
         IF(WQ.EQ.'S') OUTLYNE='"CENTROID" REQUIRES AN SPSF.DAT FILE TO EXIST'
         CALL SHOWIT(1)
         CALL MACFAL
         DEALLOCATE(SHIFTER,IPSF,PSF,XLSF,YLSF,XCEN,YCEN,XDUM,YDUM,SUMMERIT,STAT=ALLOERR)
         RETURN
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE GETPSF
!     CALLED BY CMDER FOR GET PSF
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   REAL*8 EXTENT,SPACING,PSFXCENT,PSFYCENT ,PSF,DDUMMY,SHIFTER,V9
!
   REAL*8 JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT,TPSF,PEAKER ,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT
!
   INTEGER PPGR,IPSF,JJJ,I,J,K,SHIFT,PIX2,ISHIFT,JSHIFT,IGET,JGET ,SSHIFT,SHIFTY,SHIFTCNT,IP,JP,IQ,JQ,IR,JR,L,M,IID,JJD
!
...
1002  FORMAT(A78)
      ...
      PPGR=(PGR-1)/2
      IRRER2=.FALSE.
      IF(IGET.LT.-PPGR.OR.IGET.GT.PPGR.OR.JGET.LT.-PPGR.OR.JGET.GT.PPGR) THEN
         IRRER2=.TRUE.
         RETURN
      END IF
1003  FORMAT(I10,1X,I10)
...
1006  FORMAT(I4,G23.15)
      ...
      ALLOCATE(IPSF(1:MMM,1:MMM),TPSF(-PPGR:PPGR,-PPGR:PPGR),PSF(1:MMM,1:MMM),STAT=ALLOERR)
!     LOAD PSF/LSF ARRAYS
      II=1
      DO JJD=1,MMM
         DO IID=1,MMM
...
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   REAL*8 EXTENT,SPACING,PSFXCENT,PSFYCENT ,PSF,DDUMMYX,DDUMMYY,SHIFTER,V9,V8
!
   REAL*8 JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT,TPSF,PEAKER ,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT
!
   INTEGER PPGR,IPSF,JJJ,I,J,K,SHIFT,PIX2,ISHIFT,JSHIFT,IGET,JGET ,SSHIFT,SHIFTY,SHIFTCNT,IP,JP,IQ,JQ,IR,JR,L,M,IID,JJD
!
   DIMENSION PSF(:,:),IPSF(:,:),TPSF(:,:),DUMMYX(:),DDUMMYX(:),DUMMYY(:),DDUMMYY(:)
!
...
1006  FORMAT(I4,G23.15)
      DEALLOCATE(PSF,IPSF,TPSF,STAT=ALLOERR)
      ALLOCATE(IPSF(1:MMM,1:MMM),TPSF(-PPGR:PPGR,-PPGR:PPGR),PSF(1:MMM,1:MMM),DUMMYX(1:PGR),DDUMMYX(1:PGR),DUMMYY(1:PGR),DDUMMYY(1:PGR),STAT=ALLOERR)
!     LOAD PSF/LSF ARRAYS
      II=1
      DO JJD=1,MMM
         DO IID=1,MMM
...
!     CALLED BY CMDER FOR COMMAND STREAK
!
   REAL XPLT,YPLT,FPLT,EXTENT ,SPACING,CNTX,CNTY
...
!
   REAL*8 JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT ,CRAYX,CRAYY,SCENX,SCENY,PSFYCENT,PSFXCENT,DUMMY2,IV,FTOT,FSUM,IIII,PSF,PEAKER,SPSF
!
   INTEGER IPSF,I,J,K,SHIFT,LLSTART,LLSTOP,KKSTART,KKSTOP,L,III,JJJ,MMM,PCOUNT,PRG,II
!
...
!
   PRG=TGR-1
!
   IF(WQ.NE.'X'.AND.WQ.NE.'Y'.AND.WQ.NE.'XY'.AND.WQ.NE.'WRITE'.AND.WQ.NE.'PLOT'.AND.STI.NE.1) THEN
...
!
      IF(WQ.EQ.'WRITE') THEN
         ...
         END IF
         IF(STI.EQ.1) THEN
            OUTLYNE='"STREAK WRITE" TURNS STREAKED PSF PLOTTING "ON" OR "OFF"'
            CALL SHOWIT(1)
...
!
   ELSE
!     NOT STREAK PLOT OR STREAK WRITE
   END IF
!
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"STREAK" STREAKS THE STORED PSF IN THE X OR Y DIRECTION'
      CALL SHOWIT(1)
      OUTLYNE='QUALIFIER WORD IS "X" OR "Y" FOR THE STREAK DIRECTION'
      CALL SHOWIT(1)
      OUTLYNE='NUMERIC WORD #1 IS THE NUMBER OF "GRI" UNITS TO STREAK'
      CALL SHOWIT(1)
      RETURN
   END IF
!
   ...
   IF(SQ.EQ.1.AND.WQ.NE.'X'.AND.WQ.NE.'Y'.AND.WQ.NE.'XY') THEN
      OUTLYNE='"STREAK" REQUIRES "X", "Y" OR "XY" AS QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W1.LT.0.0D0) THEN
      OUTLYNE='NUMERIC WORD #1 MAY NOT BE NEGATIVE'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
9006  FORMAT(I4,G23.15)
      ...
      ALLOCATE(PSF(PRG,PRG),SPSF(SPGR,SPGR),SCENX(SPGR),SCENY(SPGR),IPSF(PRG,PRG),STAT=ALLOERR)
      III=SPGR
      SCENX(1:III)=0.0D0
      SCENY(1:III)=0.0D0
!     ZERO THE ADD ARRAY
...
!     LSF ARRAYS LOADED
      ...
      CALL CLOSE_FILE(51,1)
!     PSF ARRAY LOADED.
!     NOW ADD PSF TO SPSF TO BUILD THE STREAKED PSF
...
!
      ...
      DO JJJ=1,SPGR
         DO III=1,SPGR
            IF(PEAKER.NE.0.0D0) SPSF(III,JJJ)=DNINT((SPSF(III,JJJ)/PEAKER)*32767.0D0)
            IF(PEAKER.EQ.0.0D0) SPSF(III,JJJ)=DNINT(SPSF(III,JJJ))
         END DO
      END DO
!     CALCULATE THE TRUE SPSF X-CENTROID POSITION
      PSFXCENT=0.0D0
      IIII=0.0D0
      IV=0.0D0
      FTOT=0.0D0
...
!
!     NOW DO THE WRITING OF THE STREAKED PSF
      IF(.NOT.STKWRT) THEN
         EXIS52=.FALSE.
         OPEN52=.FALSE.
         ...
         INQUIRE(FILE='SPSF.DAT',EXIST=EXIS52)
         IF(EXIS52) THEN
            OPEN(UNIT=52,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='SPSF.DAT',STATUS='UNKNOWN')
...
3000     FORMAT(I4,G23.15)
         PEAKER=-1.0D300
         DO III=1,PGR
            IF(SCENX(((SPGR-PGR)/2)+III).GT.PEAKER) PEAKER=SCENX(((SPGR-PGR)/2)+III)
         END DO
         DO III=1,PGR
            WRITE(52,3000) III,(SCENX(((SPGR-PGR)/2)+III)/PEAKER)*1.0D0
         END DO
         PEAKER=-1.0D300
         DO III=1,PGR
            IF(SCENY(((SPGR-PGR)/2)+III).GT.PEAKER) PEAKER=SCENY(((SPGR-PGR)/2)+III)
         END DO
         DO III=1,PGR
            WRITE(52,3000) III,(SCENY(((SPGR-PGR)/2)+III)/PEAKER)*1.0D0
         END DO
         WRITE(52,1009)
1009     FORMAT('EOF')
      ELSE
!     DON'T WRITE THE FILE
      END IF
!     NOW DO THE PLOTTING OF THE STREAKED PSF
      IF(STKPLT) THEN
!     PLOT THE STREAKED FILE
!
         ...
         DO III=1,PGR
            ...
            END DO
         END DO
!     SET UP THE XPLT AND YPLT ARRAYS TO SPGRXSPGR
         YPLT(1)=-1.0
         XPLT(1)=-1.0
...
!
         DO III=1,PGR
            DO JJJ=1,PGR
               FPLT(III,JJJ)=SNGL(SPSF(((SPGR-PGR)/2)+III,((SPGR-PGR)/2)+JJJ))
            END DO
         END DO
!                       END IF
!
...
!     DON'T PLOT THE FILE
      END IF
!     FINISHED STREAK ANALYSIS
   ELSE
      OUTLYNE='NO PSF STREAKING CAN BE PERFORMED.'
      CALL SHOWIT(1)
      OUTLYNE='"STREAK" REQUIRES A PSF AND A PSF.DAT FILE TO EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   DEALLOCATE(IPSF,PSF,SPSF,SCENX,SCENY,XPLT,YPLT,FPLT,STAT=ALLOERR)
   CALL CLOSE_FILE(51,1)
   CALL CLOSE_FILE(52,1)
   RETURN
END
...
!     THESE DISPLAY EXISTING GEOMETRICAL LINE SPREAD FUNCTIONS
!
   REAL*8 ABC,IMAX,XVAL,YVAL,IL1,IL2,DELSP,XV,YV,OVAL,INTEN,IX1,IX2,IY1,IY2,USDEL,NEWW1 ,SPDELX,SPDELY,LSF1,LSF2,LSFP1,LSFP2,MTHETA,DELZ
!
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      OUTLYNE='"OLSF", "OLSF (ACC)", "OLSF (CACC) AND "OLSF (CENT)"'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'CENT'.AND.WQ.NE.'ACC'.AND.WQ.NE.'CACC') THEN
         OUTLYNE='"INVALID QUALIFIER USED WITH "OLSF"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     NUMMERIC WORD 1 IS THE DESIRED NUMBER OF POINTS ACROSS THE LSF
      ...
      IF(W1.LE.2.0D0) THEN
         OUTLYNE='"OLSF AND "OLSF CENT"'
         CALL SHOWIT(1)
         OUTLYNE='REQUIRE NUMERIC WORD 1 INPUT GREATER THAN 2.0'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     MUST BE ACC OR CACC
      IF(DF1.EQ.1) THEN
         OUTLYNE='"OLSF (ACC)" AND "OLSF (CACC)",'
         CALL SHOWIT(1)
...
!
   IF(.NOT.LSF) THEN
      OUTLYNE='NO CURRENT LINE SPREAD FUNCTION EXISTS'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     INTERPOLATE AND OUTPUT THE DATA REQUESTED
   OPEN66=isSPOFileOpen('LSF.DAT')
   IF(.NOT.OPEN66)OPEN(UNIT=66,ACCESS='DIRECT',FILE=trim(LIBSPO)//'LSF.DAT',FORM='UNFORMATTED',RECL=(10*NRECL),STATUS='UNKNOWN')
!
...
987   CONTINUE
      ...
      IF(NP.LT.NOPNTS.AND.NP.GT.1) THEN
         ...
         DO I=2,ILSF-1
            READ(UNIT=66,REC=I) LSF1,LSF2
            READ(UNIT=66,REC=I+1) LSFP1,LSFP2
            IF((NEWW1).GE.(LSF2).AND.(NEWW1).LE.(LSFP2)) THEN
!     DATA IS BETWEEN THESE POINTS, LINEARLY INTERPOLATE IT
!     INTERPOLATE
               IX1=LSF2
               IX2=LSFP2
...
!
   IMPLICIT NONE
!
   REAL*8 FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 ,FN10,X,Y,FFA,FN11,FN12,FN13,FN14,FN15
!
   INTEGER I
!
   COMMON/FFFA/X,Y,I
!
!       FUNCTIONS FOR RECTANGULAR FUNCTION FIT
   FN1(X,Y)=1.0D0
!
   FN2(X,Y)=X
   FN3(X,Y)=Y
...
!
   IMPLICIT NONE
!
   REAL*8 FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 ,FN10,X,Y,FFAX,FN11,FN12,FN13,FN14,FN15
!
   COMMON/FFAAER/X,Y
   COMMON/FFAIER/I
!
...
!
   IMPLICIT NONE
!
   REAL*8 FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 ,FN10,X,Y,FFAY,FN11,FN12,FN13,FN14,FN15
!
   INTEGER I
!
   COMMON/FFAAER/X,Y
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE ENEXFT. THIS IS THE SUBROUTINE WHICH
!       HANDLES THE MAPPING FUNCTION FITTING FROM THE ENTRANCE
!       TO THE EXIT SURFACE
!
!     THE DAREA1 FACTOR IS THE AREA IN THE ENTRANCE SURFACE WHICH IS
!     REPRESENTED BY A UNIT AREA IN THE REFERENCE SURFACE. SINCE
!     WE TRACE RAYS AT EQUAL INTERVALS IN THE REFERENCE SURFACE,
!     DAREA1 IS THE FACTOR WHICH WE MUST MULTIPLY THE ENERGY TERM
!     BY IN THE REFERENCE SURFACE SO THAT IT REPRESENTS THE CORRECT
!     AMOUNT OF COLLECTED ENERGY.
!
!     IF DAREA1 WAS 1.5, THEN A UNIT AREA IN THE REFERENCE SURFACE
!     WAS REPRESENTED BY AN AREA OF 1.5 IN THE ENTRANCE SURFACE
!     AN COLLECTED 1.5 NORMALIZED UNITS OF ENERGY.
!
!     THE DAREA2 FACTOR IS THE AREA IN THE EXIT PUPIL SURFACE WHICH IS
!     REPRESENTED BY A UNIT AREA IN THE REFERENCE SURFACE. SINCE
!     WE WISH TO REPRESENT RAYS TRACED OVER EQUAL AREAS IN THE
!     EXIT PUPIL SURFACE, THEN IF 1 AREA UNIT IN THE REFERENCE
!     SURFACE REPRESENTS DAREA2 UNITS IN THE EXIT PUPIL SURFACE,
!     WE MUST DIVIDE THE ENERGY TERM BY DAREA2.
!
!     IF DAREA2 WAS 1.5, THEN A UNIT AREA IN THE EXIT PUPIL SURFACE
!     WAS REPRESENTED BY AN AREA OF 1/1.5 UNITS IN THE REFERNCE
!     SURFACE. THIS IS THE INVERSE OF THE DAREA1 DEFINITION
!
   INTEGER N,M,TOT,NNTOT
!
   DOUBLE PRECISION RHO,SOLCOR1,WTOL,RHO2,FX,FY,TERM11,X(1:15),XX,XY,AAVAL,IAVAL,ABVAL,IBVAL ,ZAR,ZBR,RADD,SOLCOR2
!
   INTEGER FI
!
...
!
   DOUBLE PRECISION ZA1,ZB1,RFX1,RFY1,TERM,FFA,TERMA,TERMB,TERM1 ,FFAX,FFAY,DA1,DB1,DAREA1,DA2,DB2,DAREA2,ZA2,ZB2,TERM2 ,RFX2,RFY2,FA_X,FA_Y
!
   DOUBLE PRECISION HITER1A,HITER1B,HITER1C,HITER1D,HITER1E,HITER1F,HITER1G ,HITER1H,HITER1I,HITER1J,HITER1K
   COMMON/HITME1/HITER1A,HITER1B,HITER1C,HITER1D,HITER1E,HITER1F,HITER1G ,HITER1H,HITER1I,HITER1J,HITER1K
!
...
!
   ...
   DOUBLE PRECISION W,V,U,B,CFA1,CFB1 ,CFA2,CFB2,RACUMA1,RCCOLA1,RACUMB1,RCCOLB1,RACUMA2,RCCOLA2,RACUMB2,RCCOLB2
   DIMENSION W(15),V(15,15),U(15,15),B(15),CFA1(15),CFB1(15),CFA2(15),CFB2(15),RACUMA1(15,15),RCCOLA1(15),RACUMB1(15,15),RCCOLB1(15),RACUMA2(15,15),RCCOLA2(15),RACUMB2(15,15),RCCOLB2(15)
!
   AAVAL=0.0D0
   IAVAL=0.0D0
   ABVAL=0.0D0
   ...
   DAREA2=0.0D0
!
!       SETUP THE CALCULATION FOR THE 15 Ai AND Bi COEFFICIENTS
!       PROCEED WITH FITTING
!       RFX1=(ASSIGN A RAY X-HT FROM THE SPOT DIAG AT REF SURF)
!     DSPOTT(5)
!       RFY1=(ASSIGN A RAY Y-HT FROM THE SPOT DIAG AT REF SURF)
!     DSPOTT(6)
!       ZA1=(ASSIGN A RAY X-HT FROM THE SPOT DIAG AT SURF NEWOBJ+1)
!     DSPOTT(14)
!       ZB1=(ASSIGN A RAY Y-HT FROM THE SPOT DIAG AT SURF NEWOBJ+1)
!     DSPOTT(15)
!
!     ALSO
!
!       SETUP THE CALCULATION FOR THE 15 Ai AND Bi COEFFICIENTS
!       PROCEED WITH FITTING
!       RFX2=(ASSIGN A RAY X-HT FROM THE SPOT DIAG AT REF SURF)
!     DSPOTT(5)
!       RFY2=(ASSIGN A RAY Y-HT FROM THE SPOT DIAG AT REF SURF)
!     DSPOTT(6)
!       ZA2=(ASSIGN A RAY X-HT AT THE TANGENT PLANE TO THE EXIT PUPIL)
!       ZB2=(ASSIGN A RAY Y-HT AT THE TANGENT PLANE TO THE EXIT PUPIL)
!
   DO II=1,NNTOT-1
      IF(DSPOTT(7,II).EQ.0.0D0.AND.DSPOTT(12,II).NE.0.0D0) THEN
!     RAY DID NOT FAIL
!     TREAT X AND Y COORDINATES AS THEY PROJECT ALONG THE LOCAL Z-AXIS
!     ON TO THE XY-PLANE
...
!     HITEX1 RETURNS THE X AND Y RAY HEIGHTS IN THE TANGENT PLANE
!     TO THE REFERENCE SPHERE, THE X AND Y COORDINATES OF THE
!     REFERENCE RAY IN THAT PLANE AND THE RADIUS OF THE REFERENCE SPHERE
!     ZAR,ZBR AND RADD ARE USED LATER FOR THE CONVERTION TO SOLID
!     ANGLE AT THE EXIT PUPIL.
!     THESE ARE AT A PLANE ALREADY
         HITER1A=ZA2
         HITER1B=ZB2
         HITER1C=ZAR
         HITER1D=ZBR
...
!       FUNCTION FFA RETURNS THE CORRECT VALUE
...
!       DO THE APPROPRIATE ROW FOR COLUMN I
         ...
         END DO
!
!     RAY FAILED, GO TO NEXT RAY
      END IF
   END DO
!
!     NOW CALCULATE ALL THE Ai AND Bi COEFFICIENTS
!
!     FIRST DETERMINE THE Ai COEFFICIENTS
   DO I=1,15
      B(I)=RCCOLA1(I)
      J=1
      U(I,J)=RACUMA1(I,J)
...
!
   ...
   CFB2(I)=X(I)
!
!     NOW ALL THE COEFFICIENTS HAVE BEEN CALCULATED
!
!     dAREA1=(dZA1)*(dZB1)
!
!     DO THIS FOR EVERY RAY IN THE SPOT DIAGRAM
!
   DO II=1,NNTOT-1
      IF(DSPOTT(7,II).EQ.0.0D0.AND.DSPOTT(12,II).NE.0.0D0) THEN
!     RAY DID NOT FAIL
...
!
         ...
         DAREA2=DA2*DB2
!
!     HERE WE ADJUST THE RAY ENERGY TERM BY MULTIPLYING
!     THE EXISTING RAY ENERGY TERM BY DAREA1 AND DIVIDING BY DAREA2
         ...
         ELSE
            ...
            IAVAL=IAVAL+1.0D0
         END IF
         IF(DAREA2.NE.0.0D0)DSPOTT(12,II)=DSPOTT(12,II)*DABS(DAREA1/DAREA2)
!
!     NOW APPLY THE SOLID ANGLE CORRECTION TERM IF THE DISTANCE
!     ALONG THE CHIEF RAY FROM SURFACE NEWOBJ TO NEWOBJ+1 HAS AN
!     ABSOLUTE MAGNITUDE LESS THAN 1.0D10
!     IF THIS TEST IS NOT MET, THEN ALL AREAS IN THE TANGENT PLANE
!     TO SURFACE NEWOBJ+1 REPRESENT EQUAL SOLID ANGLES AND THEY
!     NEED NOT BE COUNTED.
!     CORRECTED SO SOLCOR1=COS(THETA) 1/10/94
!     SOLCOR1 IS THE AMOUNT OF SOLID ANGLE REPRESENTED BY ONE UNIT OF
!     AREA IN THE TANGENT PLANE OF THE NEWOBJ+1 SURFACE
!     THE EXACT OFF AXIS POSITION OF THE OBJECT POINT IS IGNORED
         SOLCOR1=DABS((REFRY(19,NEWOBJ+1)*DSPOTT(19,II))+(REFRY(20,NEWOBJ+1)*DSPOTT(20,II))+(REFRY(21,NEWOBJ+1)*DSPOTT(21,II)))
         DSPOTT(12,II)=DSPOTT(12,II)*SOLCOR1
!
!     NOW APPLY THE SOLID ANGLE CORRECTION TERM IF THE DISTANCE
!     ALONG THE CHIEF RAY FROM EXIT PUPIL TO NEWIMG HAS AN
!     ABSOLUTE MAGNITUDE LESS THAN 1.0D10
!     IF IT IS GREATER AND THIS TEST IS NOT MET, ALL CORRECTIONS
!     ARE ASSUMED THE SAME AND THUS NOT NEEDED.
!
!     SOLCOR2 IN THIS CASE IS THE SOLID ANGLE REPRESENTED BY A
!     UNIT AREA IN THE TANGENT PLANE OF THE EXIT PUPIL. SINCE
!     THE ENERGY TERM IS NOW ADJUSTED FOR UNIFORM AREAS IN THE
!     EXIT PUPIL WHICH ARE NOT UNIFORM SOLID ANGLES, WE NEED TO
!     DIVIDE THE ENERGY TERM BY THIS SOLCOR2
!
!     CORRECTED SO SOLCOR2=COS(THETA) ON 1/10/94
!
!     SOLCOR2 MULTIPLIES DAREA2 BUT NOW DAREA2 IS IN THE
!     DENOMINATOR SO WE DIVIDE
         SOLCOR2=DABS((REFRY(19,NEWIMG-1)*DSPOTT(22,II))+(REFRY(20,NEWIMG-1)*DSPOTT(23,II))+(REFRY(21,NEWIMG-1)*DSPOTT(24,II)))
         IF(SOLCOR2.NE.0.0D0)DSPOTT(12,II)=DSPOTT(12,II)/SOLCOR2
         ...
         END IF
!
!     RAY FAILED, PROCESS NEXT RAY
      END IF
   END DO
!     MAKE SURE THE AVERAGE VALUE OF THE FIRST CORRECTION TERM IS 1.0
   AAVAL=AAVAL/IAVAL
   ABVAL=ABVAL/IBVAL
   DO II=1,NNTOT-1
...
!     CALLED BY CMDER FOR COMMANDS PSFINT AND PSFINTS
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   REAL*8 F2JK,XPL1,YPL1,SPACER,EXTENT1,SPACING1
!
   REAL*8 JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT ,CRAYX,CRAYY,PSFYCENT,PSFXCENT,DUMMY2,IPS,IV,FTOT,FSUM,IIII,PSF,PEAKER,SPSF,SSPSF
!
   INTEGER IPSF,I,J,K,SHIFT,LLSTART,LLSTOP,KKSTART,KKSTOP,Q,L,III,JJJ,MMM,PCOUNT,PRG,UNITTT,II
!
...
!
   IF(STI.EQ.1) THEN
      IF(WC.EQ.'PSFINT')OUTLYNE='"PSFINT" INTERPOLATES THE CURRENT PSF TO AN'
      IF(WC.EQ.'PSFINTS')OUTLYNE='"PSFINTS" INTERPOLATES THE CURRENT STREAKED PSF TO AN'
      CALL SHOWIT(1)
   ...
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
      IF(WC.EQ.'PSFINT')OUTLYNE='"PSFINT" TAKES NO ADDITIONAL INPUT'
      IF(WC.EQ.'PSFINTS')OUTLYNE='"PSFINTS" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     DOES THE SPSF FILE EXIST WITH AN EXISTING PSF
      EXIS52=.FALSE.
      INQUIRE(FILE='SPSF.DAT',EXIST=EXIS52)
      IF(EXIS52) THEN
!     DO ANALYSIS
...
9006 FORMAT(I4,G23.15)
   ...
   ALLOCATE(PSF(MMM,MMM),SPSF(MMM-1,MMM-1),SSPSF(PGR+1,PGR+1),IPSF(MMM,MMM),STAT=ALLOERR)
!     ZERO THE ADD ARRAY
   I=MMM-1
   J=MMM-1
   SPSF(1:J,1:I)=0.0D0
...
!     NOW INTERPOLATE PSF TO SPSF TO BUILD THE INTERPOLATED PSF
   DO I=1,MMM-1
      DO J=1,MMM-1
...
!     TARGET ARRAY LARGER THAN SOURCE ARRAY
      Q=((PGR+1)-(MMM-1))/2
      DO III=1,MMM-1
         DO JJJ=1,MMM-1
...
!     TARGET ARRAY SMALLER OR EQUAL TO SOURCE ARRAY
      Q=((PGR+1)-(MMM-1))/2
      DO III=1,PGR+1
         DO JJJ=1,PGR+1
      ...
      END DO
   END IF
!     MAKE SURE INPUT FILES ARE CLOSED
   CALL CLOSE_FILE(UNITTT,1)
!
!     OPEN WRITE FILE
   EXIS52=.FALSE.
   ...
   INQUIRE(FILE='SPSF.DAT',EXIST=EXIS52)
   IF(EXIS52) THEN
      OPEN(UNIT=52,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='PSFINT.DAT',STATUS='UNKNOWN')
      CALL CLOSE_FILE(52,0)
   END IF
!     OPEN NEW FILE
   OPEN(UNIT=52,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='PSFINT.DAT',STATUS='UNKNOWN')
...
1007 FORMAT('M')
   ...
   DO JJJ=1,PGR+1
      DO III=1,PGR+1
         WRITE(52,2000) III,JJJ,INT(DNINT(SSPSF(III,JJJ)))
      END DO
   END DO
2000 FORMAT(2I4,I8)
   WRITE(52,1009)
