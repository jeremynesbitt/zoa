...
!
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE=&
      &'"FAIL" OR "FAILACC" TAKE NO QUALIFIER OR STRING INPUT'
      CALL SHOWIT(1)
...
!       DEFAULT NUMERIC INPUT
   ...
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1)THEN
      ...
      & 'TAKES NO NUMERIC WORD #3 THROUGH #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
!       CHECK FOR VALID SURFACE NUMBERS
   ...
   IF(DF2.EQ.0) THEN
      ...
      IF(INT(W2).LT.INT(W1)) THEN
         OUTLYNE=&
         &'SURFACE NUMBERS OUT OF ORDER FOR THE CURRENT COMMAND'
         CALL SHOWIT(1)
         OUTLYNE=&
         &'RE-ENTER COMMAND'
         CALL SHOWIT(1)
...
!       real(real64) REPRESENTATION OF THE TOTAL NUMBER OF FAILED
!       RAYS
      GPREG(1)=DBLE(NFAIL)
...
!
   ...
   IF(WC.EQ.'SPD') THEN
      IF(.NOT.SPDEXT) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'NO SPOT DIAGRAM EXISTS'//'\n'//&
         & 'USE "FOB" AND "SPD (GRID SIZE)" TO GENERATE'//'\n'//&
         & 'A SPOT DIAGRAM, THEN RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     CHIEF IS SET
100      FORMAT(&
         &'SPOT DIAGRAM REFERNCE SPHERE CENTER LIES ON THE CHIEF RAY')
         WRITE(OUTLYNE,100)
         CALL SHOWIT(0)
      END IF
      IF(REFLOC.EQ.3) THEN
!     CENT IS SET
300      FORMAT(&
         &'SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT')
         WRITE(OUTLYNE,300)
         CALL SHOWIT(0)
...
!     CENT IS SET
400      FORMAT(&
         &'SPOT DIAGRAM REFERENCE SPHERE CENTER REMOVES TILT AND FOCUS')
         WRITE(OUTLYNE,400)
         CALL SHOWIT(0)
         WRITE(OUTLYNE,305)DLLX
         CALL SHOWIT(0)
   ...
   IF(WQ.EQ.'CHIEF') THEN
      ...
      DLLZ=0.0D0
301   FORMAT(&
      &'SPOT DIAGRAM REFERENCE SPHERE CENTER WILL LIE ON THE CHIEF RAY')
      WRITE(OUTLYNE,301)
      CALL SHOWIT(0)
      RETURN
   END IF
...
SUBROUTINE REDSUMS(LARGER,M1,N1)
   ...
   real(real64) MAX50,LEN,MAXX,MAXY,MAXI
   LOGICAL OPEN62
!
   real(real64) LARGER,RED2,COUNTIT
!
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"CAPFNROT" TAKES NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SAGFLROT" TAKES NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.not. is_command_query()) THEN
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(WC(1:6)//' TAKES NO STRING OR QUALIFIER INPUT', 1)
      RETURN
   END IF
   IF(SN.EQ.1.AND.WC.EQ.'PSFLIN') THEN
      CALL REPORT_ERROR_AND_FAIL(WC(1:6)//' TAKES NO NUMERIC INPUT', 1)
      RETURN
   END IF
   IF(WC.EQ.'PSFLOG') THEN
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(WC(1:6)//' TAKES ONLY TAKES NUMERIC WORD #1 INPUT', 1)
         RETURN
      END IF
      IF(DF1.EQ.1) THEN
         DF1=0
   ...
   IF(W1.LE.0.0D0.AND.WC.EQ.'PSFLOG') THEN
      CALL REPORT_ERROR_AND_FAIL(WC(1:6)//' TAKES REQUIRES POSITIVE NUMERIC WORD #1 INPUT', 1)
      RETURN
   END IF
...
!
   IF(WC.EQ.'PSFTAG') THEN
      ...
      IF(SN.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"PSFTAG" ONLY TAKES STRING INPUT', 1)
         RETURN
      END IF
      IF(SST.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PSFTAG" REQUIRES EXPLICIT STRING INPUT'//'\n'//&
         & 'TWELVE (12) CHARACTERS OF INPUT ARE REGONIZED', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
         PSFTAG=WS(1:12)
...
!
   IF(WC.EQ.'PSFLI') THEN
      ...
      END IF
      IF(SN.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"PSFLI" ONLY TAKES STRING INPUT', 1)
         RETURN
      END IF
      IF(SST.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PSFLI" REQUIRES EXPLICIT STRING INPUT'//'\n'//&
         & 'EIGHTY (80) CHARACTERS OF INPUT ARE REGONIZED', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
         PSFLI=WS(1:80)
...
!
   IF(WC.EQ.'CAPFNTAG') THEN
      ...
      END IF
      IF(SN.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"CAPFNTAG" ONLY TAKES STRING INPUT', 1)
         RETURN
      END IF
      IF(SST.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"CAPFNTAG" REQUIRES EXPLICIT STRING INPUT'//'\n'//&
         & 'TWELVE (12) CHARACTERS OF INPUT ARE REGONIZED', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
...
!
   IF(WC.EQ.'CAPFNLI') THEN
      ...
      END IF
      IF(SN.EQ.1.OR.SQ.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"CAPFNLI" ONLY TAKES STRING INPUT', 1)
         RETURN
      END IF
      IF(SST.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"CAPFNLI" REQUIRES EXPLICIT STRING INPUT'//'\n'//&
         & 'EIGHTY (80) CHARACTERS OF INPUT ARE REGONIZED', 1)
         RETURN
      END IF
      IF(SST.EQ.1) THEN
...
!
   real(real64) EXTENT,SPACING,PSFXCENT,PSFYCENT,CRAYX,CRAYY &
   &,X,Y
!
...
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   real(real64) EXTENT,SPACING,PSFXCENT,PSFYCENT,XCEN,YCEN &
   &,XLSF,YLSF,PSF,DDUMMY,SHIFTER,SUMMERIT,XCENVAL,XCNT,YCNT &
   &,SUMMX,SUMMY,TRUTH,YCENVAL,XRMS,YRMS,XMEEN,YMEEN,XSUMM,YSUMM &
   &,XSSUMM,YSSUMM
!
   real(real64) JXSUM,JYSUM,XSUM,YSUM,XDUM,YDUM &
   &,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT &
   &,XTOT1,YTOT1,TOT1
!
   INTEGER PPGR,IPSF,JJJ,I,J,K,SHIFT,PIX2,ISHIFT,JSHIFT &
   &,SSHIFT,SHIFTY,PMMM,IP,JP,IQ,JQ,IR,JR,PGG
!
   DIMENSION XLSF(:),YLSF(:),XCEN(:),YCEN(:),XDUM(:),YDUM(:)&
   &,PSF(:,:),IPSF(:,:),SHIFTER(:,:),SUMMERIT(:,:),&
   &SUMMX(:),SUMMY(:)
!
...
!     WC='PIXEL'
   IF(WC.EQ.'PIXEL') THEN
      IF(is_command_query()) THEN
         ...
         CALL SHOWIT(1)
         OUTLYNE=&
         &'NUMERIC WORD #1 IS THE NUMBER OF SUB-PIXELS PER PIXEL'
         CALL SHOWIT(1)
         OUTLYNE=&
         &'NUMERIC WORD #2 IS THE NUMBER OF PIXELS IN THE WINDOW'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'CURRENT NUMBER OF SUB-PIXELS PER PIXEL IS : ',PIXAR1
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'   CURRENT NUMBER PIXELS IN THE WINDOW IS : ',PIXAR2
         CALL SHOWIT(1)
         RETURN
      END IF
      ...
      IF(SST.EQ.1.OR.SQ.EQ.1) THEN
         OUTLYNE=&
         &'"PIXEL" TAKES NO STRING OR QUALIFIER WORD INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE=&
         &'"PIXEL" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
         OUTLYNE=&
         &'"PIXEL" REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W1.LE.0.0D0) THEN
         OUTLYNE=&
         &'NUMERIC WORD #1 MUST BE A POSITIVE INTEGER'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(((DBLE(INT(W1/2.0D0))*2.0D0)-W1).EQ.0.0D0) THEN
         OUTLYNE=&
         &'NUMERIC WORD #1 MUST BE AN ODD INTEGER'
         CALL SHOWIT(1)
         OUTLYNE=&
         &'IT MAY NOT BE AN EVEN INTEGER'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W2.LT.1.0D0) THEN
         OUTLYNE=&
         &'NUMERIC WORD #2 MUST BE A POSITIVE INTEGER'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W1.LE.W2) THEN
         OUTLYNE=&
         &'NUMERIC WORD #1 MUST BE GREATER THAN NUMERIC WORD #2'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     WC='CENTROID'
   IF(WC.EQ.'CENTROID') THEN
      IF(PIXAR1.EQ.0.0D0.OR.PIXAR2.EQ.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'THE NUMBER OF SUB-PIXELS AND PIXELS MUST BE SET USING'//'\n'//&
         & 'THE "PIXEL" COMMAND BEFORE A PIXEL LINEARITY ANALYSIS'//'\n'//&
         & 'CAN BE PERFORMED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      ...
      IF(SST.EQ.1.OR.SN.EQ.1) THEN
         OUTLYNE=&
         &'"CENTROID" TAKES NO NUMERIC OR STRING INPUT'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(sys_mode().GE.3.0D0) THEN
         ...
         CALL REPORT_ERROR_AND_FAIL('NO ACTION TAKEN', 1)
      END IF
      IF(SQ.EQ.1.AND.WQ.NE.'LSF'.AND.WQ.NE.'SLSF'.AND.WQ.NE.'PSFK'&
      &.AND.WQ.NE.'SPSF') THEN
         OUTLYNE=&
         &'"CENTROID" ONLY TAKES "LSF", "PSF", "SLSF" AND "SPSF"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('AS QUALIFIER WORDS'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!
!     DO CENTROIDING
...
!     DO ANALYSIS
         OPEN51=.FALSE.
         IF(SQ.EQ.0.OR.WQ.EQ.'PSFK'.OR.WQ.EQ.'LSF')&
         &INQUIRE(FILE='PSF.DAT',OPENED=OPEN51)
...
!     PSF CENTROIDING
            IF(PGR.LT.((PIXAR1*PIXAR2)+PIXAR1)) THEN
               CALL REPORT_ERROR_AND_FAIL(&
               & 'THE CURRENT PSF WINDOW DEFINED BY THE PGR VALUE'//'\n'//&
               & 'IS TOO SMALL FOR THE CURRENTLY DEFINED PIXEL VALUES'//'\n'//&
               & 'NO CENTROID LINEARITY ANALYSIS CAN BE PERFORMED', 1)
...
!     SUMMINT IS AN INTERMEDIATE SUMMATION VALUE
                           DO JR=1,PIXAR1
                              DO IR=1,PIXAR1
                                 SUMMINT=SUMMINT+&
                                 &SHIFTER(-PGG-1+((IQ-1)*PIXAR1)+IR,-PGG-1+((JQ-1)*PIXAR1)+JR)
                              END DO
                           END DO
...
!     X CENTROID, SUMM ALL THE Y VALUES
                     ...
                     IF(ISHIFT.LT.0)&
                     &TRUTH=PSFXCENT+&
                     &((DBLE(ISHIFT)+0.5D0)/DBLE(PIXAR1))
                     IF(ISHIFT.GE.0)&
                     &TRUTH=PSFXCENT+&
                     &((DBLE(ISHIFT+1)-0.5D0)/DBLE(PIXAR1))
                     XCENVAL=(JXSUM/XTOT1)-(TOT1/DBLE(PIXAR2))&
                     &-TRUTH
                     XSUMM=XSUMM+XCENVAL
                     XSSUMM=XSSUMM+(XCENVAL**2)
...
!     Y CENTROID, SUMM ALL THE X VALUES
                     ...
                     IF(JSHIFT.LT.0)&
                     &TRUTH=PSFYCENT+&
                     &((DBLE(JSHIFT)+0.5D0)/DBLE(PIXAR1))
                     IF(JSHIFT.GE.0)&
                     &TRUTH=PSFYCENT+&
                     &((DBLE(JSHIFT+1)-0.5D0)/DBLE(PIXAR1))
                     YCENVAL=(JYSUM/YTOT1)-(TOT1/DBLE(PIXAR2))&
                     &-TRUTH
...
!     ALLOCATE THE DUMMY ARRAYS FOR CENTROIDING
            ...
            &YDUM(1:(PIXAR1*PIXAR2)),STAT=ALLOERR)
...
!     HERE WE FORM THE WEIGHTED SUMMATIONS FOR CENTROID CALCULATIONS
                  ...
                  &-(((DBLE(SHIFT-1)/2.0D0)+1.0D0-DBLE(I))/(DBLE(SHIFT)))&
                  &-PSFXCENT
!
...
2009              FORMAT(I2,1X,G13.6,1X,G13.6)
               ...
               END DO
               DEVX=DSQRT(&
               &(JKSUMX2-((JKSUMX1**2)/DBLE(PIXAR1)))&
               &/(DBLE(PIXAR1)-1.0D0)&
               &)
               DEVY=DSQRT(&
               &(JKSUMY2-((JKSUMY1**2)/DBLE(PIXAR1)))&
               &/(DBLE(PIXAR1)-1.0D0)&
               &)
               IF(WQ.EQ.'PSFK'.OR.WQ.EQ.'SPSF') WRITE(OUTLYNE,3)
               IF(WQ.EQ.'LSF'.OR.WQ.EQ.'SLSF'.OR.SQ.EQ.0) WRITE(OUTLYNE,6)
...
!
            ...
            &XDUM,YDUM,SUMMERIT,STAT=ALLOERR)
            RETURN
         END IF
      ELSE
!     NO ANALYSIS CAN BE DONE
         OUTLYNE=&
         &'NO CENTROID ANALYSIS CAN BE PERFORMED.'
         CALL SHOWIT(1)
         IF(SQ.EQ.0) OUTLYNE=&
         &'"CENTROID" REQUIRES A PSF.DAT FILE TO EXIST'
         IF(WQ.EQ.'S') OUTLYNE=&
         &'"CENTROID" REQUIRES AN SPSF.DAT FILE TO EXIST'
         CALL SHOWIT(1)
         CALL MACFAL
         DEALLOCATE(SHIFTER,IPSF,PSF,XLSF,YLSF,XCEN,YCEN,&
         &XDUM,YDUM,SUMMERIT,STAT=ALLOERR)
...
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   real(real64) EXTENT,SPACING,PSFXCENT,PSFYCENT &
   &,PSF,DDUMMY,SHIFTER,V9
!
   real(real64) JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT,TPSF,PEAKER &
   &,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT
!
   INTEGER PPGR,IPSF,JJJ,I,J,K,SHIFT,PIX2,ISHIFT,JSHIFT,IGET,JGET &
   &,SSHIFT,SHIFTY,SHIFTCNT,IP,JP,IQ,JQ,IR,JR,L,M,IID,JJD
!
...
1002  FORMAT(A78)
      ...
      &JGET.LT.-PPGR.OR.JGET.GT.PPGR) THEN
         IRRER2=.TRUE.
         RETURN
      END IF
1003  FORMAT(I10,1X,I10)
...
!
   real(real64) EXTENT,SPACING,PSFXCENT,PSFYCENT &
   &,PSF,DDUMMYX,DDUMMYY,SHIFTER,V9,V8
!
   real(real64) JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT,TPSF,PEAKER &
   &,CRAYX,CRAYY,JKSUMX1,JKSUMY1,JKSUMX2,JKSUMY2,DEVX,DEVY,SUMMINT
!
...
1006  FORMAT(I4,G23.15)
      DEALLOCATE(PSF,IPSF,TPSF,STAT=ALLOERR)
      ALLOCATE(IPSF(1:MMM,1:MMM),TPSF(-PPGR:PPGR,-PPGR:PPGR),&
      &PSF(1:MMM,1:MMM),DUMMYX(1:PGR),DDUMMYX(1:PGR),&
      &DUMMYY(1:PGR),DDUMMYY(1:PGR),&
      &STAT=ALLOERR)
!     LOAD PSF/LSF ARRAYS
      II=1
      DO JJD=1,MMM
...
!
   CHARACTER EEOOFF*4,UNITS*2,BLANK*80
!
   real(real64) F2JK,XPL1,YPL1,SPACER,EXTENT1,SPACING1
!
   real(real64) JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT &
   &,CRAYX,CRAYY,SCENX,SCENY,PSFYCENT,PSFXCENT,DUMMY2,&
   &IV,FTOT,FSUM,IIII,PSF,PEAKER,SPSF
!
   INTEGER IPSF,I,J,K,SHIFT,LLSTART,LLSTOP,KKSTART,&
   &KKSTOP,L,III,JJJ,MMM,PCOUNT,PRG,II
!
...
!     IF WQ EQ 'PLOT' OR 'WRITE'
   IF(WQ.EQ.'PLOT'.OR.WQ.EQ.'WRITE') THEN
      IF(WQ.EQ.'PLOT') THEN
         IF(SN.EQ.1) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"STREAK PLOT" TAKES NO NUMERIC INPUT"'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(is_command_query()) THEN
...
!
      IF(WQ.EQ.'WRITE') THEN
         IF(SN.EQ.1) THEN
            CALL REPORT_ERROR_AND_FAIL(&
            & '"STREAK WRITE" TAKES NO NUMERIC INPUT"'//'\n'//&
            & 'RE-ENTER COMMAND', 1)
            RETURN
         END IF
         IF(is_command_query()) THEN
...
!
   IF(is_command_query()) THEN
      ...
      CALL SHOWIT(1)
      OUTLYNE=&
      &'QUALIFIER WORD IS "X" OR "Y" FOR THE STREAK DIRECTION'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'NUMERIC WORD #1 IS THE NUMBER OF "GRI" UNITS TO STREAK'
      CALL SHOWIT(1)
      RETURN
   END IF
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"STREAK" OTHER THAN WITH "PLOT" OR "WRITE"'//'\n'//&
   ...
   IF(SQ.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"STREAK" REQUIRES EXPLICIT QUALIFIER WORD INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S1.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"STREAK" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"STREAK" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1.AND.WQ.NE.'X'.AND.WQ.NE.'Y'.AND.WQ.NE.'XY') THEN
      OUTLYNE=&
      &'"STREAK" REQUIRES "X", "Y" OR "XY" AS QUALIFIER INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.LT.0.0D0) THEN
      OUTLYNE=&
      &'NUMERIC WORD #1 MAY NOT BE NEGATIVE'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
9006  FORMAT(I4,G23.15)
      ...
      &,SCENX(SPGR)&
      &,SCENY(SPGR),IPSF(PRG,PRG),STAT=ALLOERR)
      III=SPGR
      SCENX(1:III)=0.0D0
      SCENY(1:III)=0.0D0
!     ZERO THE ADD ARRAY
...
3000     FORMAT(I4,G23.15)
         ...
         DO III=1,PGR
            ...
            &SCENX(((SPGR-PGR)/2)+III)
         END DO
         DO III=1,PGR
            WRITE(52,3000) III,&
            &(SCENX(((SPGR-PGR)/2)+III)/PEAKER)*1.0D0
         END DO
         PEAKER=-1.0D300
         DO III=1,PGR
            IF(SCENY(((SPGR-PGR)/2)+III).GT.PEAKER) PEAKER=&
            &SCENY(((SPGR-PGR)/2)+III)
         END DO
         DO III=1,PGR
            WRITE(52,3000) III,&
            &(SCENY(((SPGR-PGR)/2)+III)/PEAKER)*1.0D0
         END DO
         WRITE(52,1009)
1009     FORMAT('EOF')
      ELSE
...
!
         ...
         ALLOCATE(&
         &XPLT(PGR),YPLT(PGR),FPLT(PGR,PGR),STAT=ALLOERR)
         DO III=1,PGR
            XPLT(III)=0.0
            YPLT(III)=0.0
            DO JJJ=1,PGR
...
!
         DO III=1,PGR
            DO JJJ=1,PGR
               FPLT(III,JJJ)=&
               &SNGL(SPSF(((SPGR-PGR)/2)+III,((SPGR-PGR)/2)+JJJ))
            END DO
         END DO
!                       END IF
!
...
!     FINISHED STREAK ANALYSIS
   ELSE
      OUTLYNE=&
      &'NO PSF STREAKING CAN BE PERFORMED.'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'"STREAK" REQUIRES A PSF AND A PSF.DAT FILE TO EXIST'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
!
   ...
   &SCENY,XPLT,YPLT,FPLT,STAT=ALLOERR)
   CALL CLOSE_FILE(51,1)
   CALL CLOSE_FILE(52,1)
   RETURN
END
...
!
   real(real64) ABC,IMAX,XVAL,YVAL,IL1,IL2,&
   &DELSP,XV,YV,OVAL,INTEN,IX1,IX2,IY1,IY2,USDEL,NEWW1 &
   &,SPDELX,SPDELY,LSF1,LSF2,LSFP1,LSFP2,MTHETA,DELZ
!
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      OUTLYNE=&
      &'"OLSF", "OLSF (ACC)", "OLSF (CACC) AND "OLSF (CENT)"'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'CENT'.AND.WQ.NE.'ACC'.AND.WQ.NE.'CACC') THEN
         OUTLYNE=&
         &'"INVALID QUALIFIER USED WITH "OLSF"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     NUMERIC WORDS 2 TO 5
   IF(DF2.EQ.0.OR.DF3.EQ.0.OR.DF4.EQ.0.OR.DF5.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"OLSF" TAKES NO NUMERIC WORD 2 THROUGH 5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(.NOT.LSF) THEN
      OUTLYNE=&
      &'NO CURRENT LINE SPREAD FUNCTION EXISTS'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('NO ACTION TAKEN', 1)
      RETURN
   END IF
...
!
   ...
   IMPLICIT NONE
!
   real(real64) FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 &
   &,FN10,X,Y,FFA,FN11,FN12,FN13,FN14,FN15
!
   INTEGER I
!
   COMMON/FFFA/X,Y,I
...
!
   real(real64) FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 &
   &,FN10,X,Y,FFAX,FN11,FN12,FN13,FN14,FN15
!
   COMMON/FFAAER/X,Y
   COMMON/FFAIER/I
!
...
!
   real(real64) FN1,FN2,FN3,FN4,FN5,FN6,FN7,FN8,FN9 &
   &,FN10,X,Y,FFAY,FN11,FN12,FN13,FN14,FN15
!
   INTEGER I
!
   COMMON/FFAAER/X,Y
...
!
   ...
   &X(1:15),XX,XY,AAVAL,IAVAL,ABVAL,IBVAL &
   &,ZAR,ZBR,RADD,SOLCOR2
!
   INTEGER FI
!
...
!
   ...
   &TERMA,TERMB,TERM1 &
   &,FFAX,FFAY,DA1,DB1,DAREA1,DA2,DB2,DAREA2,&
   &ZA2,ZB2,TERM2 &
   &,RFX2,RFY2,FA_X,FA_Y
!
   DOUBLE PRECISION &
   &HITER1A,HITER1B,HITER1C,HITER1D,HITER1E,HITER1F,HITER1G &
   &,HITER1H,HITER1I,HITER1J,HITER1K
   COMMON/HITME1/&
   &HITER1A,HITER1B,HITER1C,HITER1D,HITER1E,HITER1F,HITER1G &
   &,HITER1H,HITER1I,HITER1J,HITER1K
!
...
!
   ...
   &,CFA2,CFB2,&
   &RACUMA1,&
   &RCCOLA1,RACUMB1,RCCOLB1,&
   &RACUMA2,&
   &RCCOLA2,RACUMB2,RCCOLB2
   DIMENSION W(15),V(15,15),U(15,15),&
   &B(15),CFA1(15),CFB1(15)&
   &,CFA2(15),CFB2(15),&
   &RACUMA1(15,15),&
   &RCCOLA1(15),RACUMB1(15,15),RCCOLB1(15),&
   &RACUMA2(15,15),&
   &RCCOLA2(15),RACUMB2(15,15),RCCOLB2(15)
!
   AAVAL=0.0D0
   IAVAL=0.0D0
   ABVAL=0.0D0
...
!     THE EXISTING RAY ENERGY TERM BY DAREA1 AND DIVIDING BY DAREA2
         ...
         END IF
         IF(DAREA2.NE.0.0D0)&
         &DSPOTT(12,II)=DSPOTT(12,II)*DABS(DAREA1/DAREA2)
!
...
!     THE EXACT OFF AXIS POSITION OF THE OBJECT POINT IS IGNORED
         ...
         &(REFRY(20,NEWOBJ+1)*DSPOTT(20,II))+&
         &(REFRY(21,NEWOBJ+1)*DSPOTT(21,II)))
...
!     DENOMINATOR SO WE DIVIDE
         ...
         &(REFRY(20,NEWIMG-1)*DSPOTT(23,II))+&
         &(REFRY(21,NEWIMG-1)*DSPOTT(24,II)))
         IF(SOLCOR2.NE.0.0D0)&
         &DSPOTT(12,II)=DSPOTT(12,II)/SOLCOR2
...
!
   real(real64) JXSUM,JYSUM,XSUM,YSUM,XTOT,YTOT,TOT &
   &,CRAYX,CRAYY,PSFYCENT,PSFXCENT,DUMMY2,IPS,&
   &IV,FTOT,FSUM,IIII,PSF,PEAKER,SPSF,SSPSF
!
   INTEGER IPSF,I,J,K,SHIFT,LLSTART,LLSTOP,KKSTART,&
   &KKSTOP,Q,L,III,JJJ,MMM,PCOUNT,PRG,UNITTT,II
!
...
!
   IF(is_command_query()) THEN
      ...
      IF(WC.EQ.'PSFINTS')&
      &OUTLYNE='"PSFINTS" INTERPOLATES THE CURRENT STREAKED PSF TO AN'
      CALL SHOWIT(1)
   ...
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
      IF(WC.EQ.'PSFINT')&
      &OUTLYNE='"PSFINT" TAKES NO ADDITIONAL INPUT'
      IF(WC.EQ.'PSFINTS')&
      &OUTLYNE='"PSFINTS" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!     NO FILE, RETURN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'FILE "PSF.DAT" DOES NOT EXITS TO BE ITERPOLATED'//'\n'//&
         & 'NO ACTION TAKEN', 1)
...
!     NO FILE, RETURN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'FILE "SPSF.DAT" DOES NOT EXITS TO BE ITERPOLATED'//'\n'//&
         & 'NO ACTION TAKEN', 1)
...
9006 FORMAT(I4,G23.15)
   ...
   &SSPSF(PGR+1,PGR+1),IPSF(MMM,MMM),STAT=ALLOERR)
!     ZERO THE ADD ARRAY
   I=MMM-1
   J=MMM-1
   SPSF(1:J,1:I)=0.0D0
...
!     OPEN WRITE FILE
   ...
   IF(EXIS52) THEN
      OPEN(UNIT=52,ACCESS='SEQUENTIAL',BLANK='NULL'&
      &,FORM='FORMATTED',FILE='PSFINT.DAT'&
      &,STATUS='UNKNOWN')
      CALL CLOSE_FILE(52,0)
   END IF
!     OPEN NEW FILE
