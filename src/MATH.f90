...
!     THIS IS USED TO SET UP A RANDOMLY SORTED LIST OF N
!     DOUBLE PRECISION REPRESENTATIONS OF N INTEGER VALUES
...
!     ARR(I,1)
   DO I=1,N
      ...
      ARR(I,1)=RANDMM()
   END DO
!     NOW SORT ARRAY ARR BY THE VALUES IN ARR(I,2)
   CALL DARRAY_SORT(N,ARR)
   DO I=1,N
!     WRITE(OUTLYNE,*) I,ARR(I,1),ARR(I,2)
!     CALL SHOWIT(1)
   END DO
END
! SUB RANDSET.FOR
...
10    FORMAT('THE CURRENT USER-SPECIFIED SEED = ',E15.7)
   ...
   &.OR.S5.EQ.1) THEN
      ...
      & '"SEED" ONLY TAKES NUMERIC WORD  #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SEED" REQUIRES EXPLICIT NUMERIC WORD  #1 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.LT.0.001D0.OR.W1.GT.1.0D0) THEN
      OUTLYNE=&
      &'"SEED" INPUT MUST NOT BE LESS THAN 0.001'
      CALL SHOWIT(1)
      OUTLYNE=&
...
FUNCTION RANDMM()
   ...
   RANDMM=RANDX
   REG(40)=REG(9)
!     PUT NORMAL RANDOM NUMBER IN REG(10)
   REG(10)=GASDEV()
   RETURN
END
