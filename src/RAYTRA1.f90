...
!       CHECK FOR STRING INPUT
   ...
   END IF
!
!       DEFAULT QUALIFIER IS NOTHING (NO QUALIFIER)
   CACOCH=0
...
!       SET DEFAULT NUMERICS
   ...
   END IF
   WVN=W3
!       CURLAM TRACK CURRENT WAVELENGTH TRACED VIA COMMON
!       KURLAM IN DATLEN.FOR
   CURLAM=W3
!       THE CONTROL WAVELENGTH
!       NUMERIC WORD 5 IS NOT USED IN THE CMD LEVEL
!       "IRAYA" COMMAND
...
!       CHECK FOR CORRECT WAVELENGTH BOUNDS
   IF(INT(W3).NE.1.AND.INT(W3).NE.2.AND.INT(W3).NE.3.AND.INT(W3).NE.4.AND.INT(W3).NE.5.AND.INT(W3).NE.6.AND.INT(W3).NE.7.AND.INT(W3).NE.8.AND.INT(W3).NE.9.AND.INT(W3).NE.10) THEN
...
!       NOW CALL THE SUBROUTINE WHICH STARTS THE
!       RAY TRACE. THIS IS RAYTRA.
!
   WWQ=WQ
   WW1=W1
   WW2=W2
...
!       CHECK FOR NUMERIC INPUT
   IF(S1.EQ.1.AND.WQ.NE.'VREAL') THEN
      OUTLYNE='ONLY "LIMRAYS VREAL" CAN TAKE NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"LIMRAYS" TAKE NO NUMERIC WORD #2 TO #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(S1.EQ.1.AND.WQ.EQ.'VREAL') THEN
      IF(W1.LT.10.0D0.OR.W1.GT.100.0D0) THEN
         OUTLYNE='"LIMRAYS VREAL" CAN TAKE NUMERIC WORD #1 FROM 10.0 TO 100.0'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   IF(WQ.NE.'REAL'.AND.WQ.NE.'PARAX'.AND.WQ.NE.'VREAL') THEN
      OUTLYNE='"LIMRAYS" ONLY TAKES "REAL", "VREAL" OR "PARAX" AS QUALIFIERS'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
         ...
         DO L=0,7
            KK=KK+1
            M=(3*KK)-2
            IF(surf_clap_type(NEWREF) /= 2.AND.surf_clap_type(NEWREF) /= 4 .AND.surf_array_parity(NEWOBJ) == 0) THEN
               IF(L.EQ.0) THEN
                  YR=0.0D0
                  XR=XHI
               END IF
...
1001  FORMAT('SURF#',1X,' X-MIN  RAY HT ',2X,' X-MAX  RAY HT ',2X,' Y-MIN  RAY HT ',2X,' Y-MAX  RAY HT ')
1002  FORMAT('SURF#',2X,'MAX RAD. DIST.',2X,' X-CENTER POS.',2X,' Y-CENTER POS.')
...
!
   ...
   use mod_system
   use DATMAI
   use mod_surface, only: surf_clap_type, surf_clap_dim, surf_clap_tilt, surf_array_parity, set_surf_clap_type, set_surf_clap_dim, set_surf_clap_tilt
   IMPLICIT NONE
!
...
!       CHECK FOR STRING INPUT
   ...
   IF(W1.LT.0.0D0.OR.W2.GT.sys_last_surf().OR.W2.LT.W1) THEN
...
!       HANDLE NO SURFACES
   ...
   END IF
!       CHECK FOR NUMERIC INPUT
   IF(S3.EQ.1.AND.WQ.NE.'VREAL') THEN
      OUTLYNE='ONLY "SETCLAP VREAL" CAN TAKE NUMERIC WORD #3 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(S3.EQ.1.AND.WQ.EQ.'VREAL') THEN
      IF(W3.LT.10.0D0.OR.W3.GT.100.0D0) THEN
         OUTLYNE='"SETCLAP VREAL" CAN TAKE NUMERIC WORD #1 FROM 10.0 TO 100.0'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   END IF
   IF(WQ.NE.'REAL'.AND.WQ.NE.'PARAX'.AND.WQ.NE.'VREAL') THEN
      OUTLYNE='"SETCLAP" ONLY TAKES "REAL", "VREAL" OR "PARAX" AS QUALIFIERS'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      ...
      DO I=0,INT(sys_last_surf())
         IF(I.GE.INT(W1).AND.I.LE.INT(W2)) THEN
            IF(.NOT.DUMMMY(I).OR.DF1.EQ.0.AND.DF2.EQ.0) THEN
               IF(surf_clap_type(I) == 0.AND.surf_clap_type(I) /= 5.AND.surf_array_parity(I) == 0) THEN
...
!
      DO I=0,INT(sys_last_surf())
         call LogTermFOR("Surf "//trim(int2str(I))//" ALENS(9,1 ) "//trim(real2str(surf_clap_type(I))) //" surf_array_parity(I) "//trim(real2str(surf_array_parity(I))) //" W1 is "//trim(real2str(W1)) //" W2 is "//trim(real2str(W2)))
         ...
         IF(I.GE.INT(W1).AND.I.LE.INT(W2)) THEN
            IF(.NOT.DUMMMY(I).OR.DF1.EQ.0.AND.DF2.EQ.0) THEN
               ...
               IF(surf_clap_type(I) == 0.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
                  ...
                  CALL SHOWIT(0)
               END IF
               IF(surf_clap_type(I) == 1.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
                  call set_surf_clap_dim(I, 1, RRAD)
                  call set_surf_clap_dim(I, 2, RRAD)
                  call set_surf_clap_dim(I, 3, 0.0D0)
                  call set_surf_clap_dim(I, 4, 0.0D0)
                  ...
                  CALL SHOWIT(0)
               END IF
               IF(surf_clap_type(I) == 2.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
                  ...
                  CALL SHOWIT(0)
               END IF
               IF(surf_clap_type(I) == 3.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
                  call set_surf_clap_dim(I, 1, YRAD)
                  call set_surf_clap_dim(I, 2, XRAD)
                  call set_surf_clap_dim(I, 3, 0.0D0)
                  call set_surf_clap_dim(I, 4, 0.0D0)
                  ...
                  CALL SHOWIT(0)
               END IF
               IF(surf_clap_type(I) == 4.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
                  call set_surf_clap_dim(I, 1, YRAD)
                  call set_surf_clap_dim(I, 2, XRAD)
                  call set_surf_clap_dim(I, 3, 0.0D0)
                  call set_surf_clap_dim(I, 4, 0.0D0)
                  ...
                  CALL SHOWIT(0)
               END IF
               IF(surf_clap_type(I) == 5.AND.FWARN.EQ.0.AND.RWARN.EQ.0 .AND.surf_array_parity(I) == 0) THEN
...
!
   IF(RAYCOD(1).EQ.5) THEN
      WRITE(OUTLYNE,*)'ANGLE OF DIFFRACTION AT THE GRATING AT SURFACE'
      CALL SHOWIT(1)
...
!
   IF(RAYCOD(1).EQ.9) THEN
      WRITE(OUTLYNE,*)'ANGLE OF DIFFRACTION AT THE HOE SURFACE IS PHYSICALLY'
      CALL SHOWIT(1)
...
!
   IF(RAYCOD(1).EQ.18) THEN
      WRITE(OUTLYNE,*)'RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
      CALL SHOWIT(1)
      RETURN
   END IF
!
   IF(RAYCOD(1).EQ.19) THEN
      WRITE(OUTLYNE,*)'AIMED CHIEF RAY COULD NOT HIT DEFINED TARGET POINT'
      CALL SHOWIT(1)
      RETURN
   END IF
!
...
!       THE TRACING OF A CONSTRUCTION OR PLAYBACK RAY IN TYPE 13
!       SPECIAL SURFACE HOE-R RAYTRACING. THE DIRECTION COSINES OF THE
...
!
   REAL*8 X,Y,Z,L,M,N,WW1W,WW2W,LER,MER,NER,TANN1,TANN2,IA,IAP ,D21,D22,GAMMA,XXX,YYY,TWW1,TWW2,XL,XM,XN,YL,YM,YN,RN1,RN2,D11,D12,LS,SNINDX,SNIND2,WW1WW,WW2WW,JK1,JK2,JK3,LSTART,MSTART,NSTART,YANG,XANG,X1LAST,LARGE,Y1LAST,X1ONE,Y1ONE,RXLAST,RYLAST,RXONE,RYONE,DDELX,DDELY,XC1,YC1,ZC1,MF1,MF2,DET,TARX,TARY,XVALUE,YVALUE,TEST,MAG,VALUE,LOLD,MOLD,NOLD,TARRY,TARRX
!
...
!
!     SET RAY AIMING TARGET VALUES
   TARY=YHOE
   TARX=XHOE
   KKK=0
!                       DDELX=0.001D0*sys_say()
!                       DDELY=0.001D0*sys_sax()
   DDELX=0.001D0
   DDELY=0.001D0
   R_X=0.0D0
   R_Y=0.0D0
...
!       THE STARTING RAY COORDINATES AT SURFACE 0
...
!
   ...
   ZSTRT=ZO
!
!       THE WAVELENGTH NUMBER FOR THE RAY TRACE IS HOE_WAV_NUM
!
!
989 CONTINUE
   ...
   YAIMOL=YC1
   ZAIMOL=ZC1
!     CALL BAKONE TO CONVERT TO 0 COORDINATES
   R_TX=X1AIM
   R_TY=Y1AIM
   R_TZ=Z1AIM
   CALL BAKONE
...
!       AND STOP SEARCHING.
   IF(KKK.GT.NRAITR) THEN
      IF(MSG) THEN
         WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',INT(sys_last_surf())
         CALL SHOWIT(1)
...
!
   ...
   H_RAY(11,0)=XANG
   IF((H_RAY(11,0)).LT.0.0D0) H_RAY(11,0)=H_RAY(11,0)+(TWOPII)
   H_RAY(12,0)=YANG
   IF((H_RAY(12,0)).LT.0.0D0) H_RAY(12,0)=H_RAY(12,0)+(TWOPII)
   H_RAY(13,0)=0.0D0
   H_RAY(14,0)=0.0D0
   H_RAY(15,0)=1.0D0
   H_RAY(16,0)=XSTRT
...
!       FOR SURFACE 0
!       H_RAY(1,SURF)=X(LOCAL)- COORDINATE AT SURFACE I
!       H_RAY(2,SURF)=Y(LOCAL)- COORDINATE AT SURFACE I
!       H_RAY(3,SURF)=Z(LOCAL)- COORDINATE AT SURFACE I
!       H_RAY(4,SURF)=L(LOCAL)- DIRECTION COSINE AT SURFACE I
!       H_RAY(5,SURF)=M(LOCAL)- DIRECTION COSINE AT SURFACE I
!       H_RAY(6,SURF)=N(LOCAL)- DIRECTION COSINE AT SURFACE I
!       H_RAY(8,SURF)=LEN - PHYSICAL LENGTH ALONG RAY FROM SURF I-1 TO I
!       H_RAY(7,SURF)=OPL - OPL FROM SURFACE I-1 TO I
!     ZERO IF I-1 IS PERFECT
!       H_RAY(9,SURF)=COSINE(I)
!       H_RAY(10,SURF)=COSINE(IP)
!       H_RAY(11,SURF)=UX - (RADIAN) MARGINAL XZ SLOPE ANGLE
!       H_RAY(12,SURF)=UY - (RADIAN) MARGINAL YZ SLOPE ANGLE
!       H_RAY(13,SURF)=L DIRECTION COSINE OF SURFACE NORMAL
!       H_RAY(14,SURF)=M DIRECTION COSINE OF SURFACE NORMAL
!       H_RAY(15,SURF)=N DIRECTION COSINE OF SURFACE NORMAL
!       H_RAY(16,SURF)=XOLD
!       H_RAY(17,SURF)=YOLD
!       H_RAY(18,SURF)=ZOLD
!       H_RAY(19,SURF)=LOLD
!       H_RAY(20,SURF)=MOLD
!       H_RAY(21,SURF)=NOLD
!       H_RAY(22,SURF)=OPL - OPL FROM SURFACE 0 TO I
!     OR TO I-1 IF I-1 IS "PERFECT'
!       H_RAY(23,SURF)=1 FOR NOT RV, -1 FOR RV
!       H_RAY(24,SURF)=1.0D0 FOR POS RAY, -1 FOR NEG RAY
!       H_RAY(25,SURF)=RAY ENERGY TERM
!       H_RAY(26,SURF)=RAY XL DIR COS
!       H_RAY(27,SURF)=RAY XM DIR COS
!       H_RAY(28,SURF)=RAY XN DIR COS
!       H_RAY(29,SURF)=RAY YL DIR COS
!       H_RAY(30,SURF)=RAY YM DIR COS
!       H_RAY(31,SURF)=RAY YN DIR COS

!       H_RAY(33,SURF) TO H_RAY(35,SURF) RESERVED
      H_RAY(32,I)=WW3
      H_RAY(1,I)=X
      H_RAY(2,I)=Y
      H_RAY(3,I)=Z
...
!     IS ALWAYS JUST:
!
      H_RAY(8,I)=DSQRT(((H_RAY(3,I)-ZOLD)**2)+((H_RAY(2,I)-YOLD)**2)+((H_RAY(1,I)-XOLD)**2))
!
...
!
      ...
      IF(INT(WW3).GE.1.AND.INT(WW3).LE.5)H_RAY(7,I)=H_RAY(8,I)*DABS(ALENS(45+INT(WW3),(I-1)))
      IF(INT(WW3).GE.6.AND.INT(WW3).LE.10)H_RAY(7,I)=H_RAY(8,I)*DABS(ALENS(65+INT(WW3),(I-1)))
...
!
      ...
      ELSE
         ...
         ELSE
            ...
            END IF
            IF((H_RAY(11,I)).LT.0.0D0) H_RAY(11,I)=H_RAY(11,I)+(TWOPII)
         END IF
      END IF
      IF(M.EQ.0.0D0) THEN
      ...
      ELSE
         ...
         ELSE
            ...
            END IF
            IF((H_RAY(12,I)).LT.0.0D0) H_RAY(12,I)=H_RAY(12,I)+(TWOPII)
         END IF
      END IF
      H_RAY(13,I)=LN
      H_RAY(14,I)=MN
...
!       1 SO THAT THE RAY HEIGHT AT NEWR IS WITHIN
...
!       THE CMD LEVEL COMMAND RAY FROM CALCPRE
!
   INTEGER I,J
!
...
!       SOMETIMES CLEAR APERTURE/OBSCURATION CHECKING
!       IS RESET.
!       DEFAULT QUALIFIER IS NOTHING (NO QUALIFIER)
   CACOCH=0
!
...
!       KURLAM IN DATLEN.FOR
   CURLAM=W3
!       THE CONTROL WAVELENGTH
!       NUMERIC WORD 5 ARE NOT USED IN THE CMD LEVEL
!       "RAY" COMMAND
!       CHECK FOR NATURE OF QUALIFIER WORD
!       CHECK FOR CORRECT WAVELENGTH BOUNDS
!
...
!
      ...
      END IF
!
!       ALL DIFFERENTIAL RAY DATA WILL BE STORED IN ARRAYS
!       AND SET TO COMMON FROM WITHIN THE DIFRAY SUBROUTINE
!       RESTORE RAYRAY DATA
      RAYRAY=SAVE
!       NO DIF TRACE
   END IF
...
!       CHECK FOR NUMERIC INPUT
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"BLKRAYS" TAKE NO NUMERIC WORD #2 TO #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       MAKE STORAGE ARRAY TWICE AS BIG
      ALLOCATE(RHIST_TEMP(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf())),STAT=ALLOERR)
      RHIST_TEMP(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))=0.0D0
      RHIST_TEMP(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))=RHIST(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))
      DEALLOCATE (RHIST, STAT=ALLOERR)
      RHIST_MAXRAYS=RHIST_MAXRAYS*2
      ALLOCATE (RHIST(1:93,1:RHIST_MAXRAYS,0:INT(sys_last_surf())),STAT=ALLOERR)
      RHIST(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))=0.0D0
      RHIST(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))=RHIST_TEMP(1:93,1:RAY_HIST_NUM,0:INT(sys_last_surf()))
...
!       CALCULATE X AND Y STEPS
      IF(RN.EQ.1) THEN
         ...
         SQ=0
         CACOCH=0
         IF(RAY_HIST_FLAG)CALL HIST_RAY_SAVE
      ELSE
...
!       CALL SHOWIT(1)
      ...
      IF(sys_scy_fang_set().EQ.0.0D0) THEN
         WRITE(104,10)sys_scy()*TT1,sys_scx()*TT2,INT(RTOT),INTTOT,DSPOT(38),INT(GPREG(2)),INT(GPREG(3)),INT(GPREG(4)),INT(GPREG(7))
         WRITE(OUTLYNE,10)sys_scy()*TT1,sys_scx()*TT2,INT(RTOT)
      ELSE
         WRITE(104,10)sys_fang_y()*TT1,-sys_fang_x()*TT2,INT(RTOT),INTTOT,DSPOT(38),INT(GPREG(2)),INT(GPREG(3)),INT(GPREG(4)),INT(GPREG(7))
         WRITE(OUTLYNE,10)sys_fang_y()*TT1,-sys_fang_x()*TT2,INT(RTOT)
      END IF
      CALL SHOWIT(0)
...
!       CALL SHOWIT(1)
!       WRITE(OUTLYNE,*)
!    1  'NO SPOT DIAGRAM WAS TRACED FOR FOB# ',FOBNUMBER
!       CALL SHOWIT(1)
            ELSE
               FOBNUMBER=FOBNUMBER+1
            END IF
...
!       NRECL IS ONE 4 BYTE WORD
   OPEN(UNIT=105,ACCESS='DIRECT',FILE='IRAD.DAT',CONVERT='LITTLE_ENDIAN',FORM='UNFORMATTED',RECL=(6*NRECL),STATUS='UNKNOWN')
!       READ MAIN HEADER
   K=1
...
!
   DO L=1,FN*FN
!       THERE ARE FN*FN FIELD OF VIEW POSITIONS TO READ
!       READ THE FOV HEADER
      K=K+1
      READ(UNIT=105,REC=K)YFOV,XFOV,RTOT
      WRITE(OUTLYNE,*) YFOV,XFOV,RTOT
      CALL SHOWIT(1)
      K=K+1
      READ(UNIT=105,REC=K)INTEN,AOI,AREA_FILL
      WRITE(OUTLYNE,*) INTEN,AOI,AREA_FILL
      CALL SHOWIT(1)
!       READ THE INTENSITY GRID DATA
      DO J=1,IARRAY_DIM
         DO I=1,IARRAY_DIM
            K=K+1
...
!       CHECK FOR STRING INPUT
   ...
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      OUTLYNE='"MTRACERI GRID" REQUIRES EXPLICIT NUMERIC WORDS #1 AND #2'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W1.LE.1.0D0) THEN
      OUTLYNE='"MTRACERI GRID" REQUIRES A POSITIVE GRID DIMENSIONALITY, (NW1)'
      CALL SHOWIT(1)
      OUTLYNE='EQUAL TO 2 OR GREATER'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W2.LE.0.0D0) THEN
      OUTLYNE='"MTRACERI GRID" REQUIRES A POSITIVE, NON-ZERO GRID SIZE (NW2)'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!                SPSURF=W3
   ...
   IF(.NOT.MFOBSSET) THEN
      ...
      RETURN
   END IF
!       SET UP THE GRID ARRAY AND DETERMINE WHICH GRID POINTS ARE
!       INSIDE ANY CLEAR APERTURE ON THE SPOT DIAGRAM SURFACE
   DEALLOCATE(IXARRAY,IYARRAY,IGRID ,STAT=ALLOERR)
   ALLOCATE(IXARRAY(1:IARRAY_DIM,1:IARRAY_DIM),IYARRAY(1:IARRAY_DIM,1:IARRAY_DIM),IGRID(1:IARRAY_DIM,1:IARRAY_DIM,1:10),STAT=ALLOERR)
...
!       ESTABLISH THE PIXEL CENTER LOCATION VALUES
   ...
   XCORRI=IXARRAY(1,1)
   YCORRI=IYARRAY(1,1)
!       IS THERE A CLAP ASSIGNED TO THE SPOT SURFACE
...
!       CALLCULATE TOTAL AREA OF THE CLAP ON THE SPOT DIAGRAM SURFACE
   IF(surf_clap_type(NEWIMG) == 1.OR.surf_clap_type(NEWIMG) == 2.OR.surf_clap_type(NEWIMG) == 3.OR.surf_clap_type(NEWIMG) == 4) THEN
      ...
      IF(surf_clap_type(NEWIMG) == 4) THEN
         TAREA=(4.0D0*surf_clap_dim(NEWIMG, 1)*surf_clap_dim(NEWIMG, 2))-(((2.0D0*surf_clap_dim(NEWIMG, 5))**2)-(PII*(surf_clap_dim(NEWIMG, 5)**2)))
      END IF
!       DIS-ALLOW POINTS
      DO I=1,NX
         DO J=1,NY
            ...
            IF(.NOT.ISITIN) THEN
               IGRID(I,J,1)=0.0D0
!       CAN'T WRITE INTO THAT SQUARE, IT IS OUTSIDE THE CLAP
            ELSE
!       CAN WRITE INTO THAT SQUARE, IT IS INSIDE THE CLAP
               IGRID(I,J,1)=1.0D0
            END IF
         END DO
      END DO
   ELSE
!       ALL POINTS ON GRID ARE TO BE USED.
   ...
   OPEN(UNIT=104,FILE='MTRACEI.DAT')
!
!       OPEN AND CLOSE IRRADIANCE FILE
   OPEN(UNIT=105,ACCESS='DIRECT',FILE='IRAD.DAT',CONVERT='LITTLE_ENDIAN',FORM='UNFORMATTED',RECL=(6*NRECL),STATUS='UNKNOWN')
   CALL CLOSE_FILE(105,0)
!       OPEN IRRADIANCE FILE
   OPEN(UNIT=105,ACCESS='DIRECT',FILE='IRAD.DAT',CONVERT='LITTLE_ENDIAN',FORM='UNFORMATTED',RECL=(6*NRECL),STATUS='UNKNOWN')
!
   FN=INT(FW4)
   RN=INT(RW5)
...
!       RAY WAS OUTSIDE THE INTENSITY ARRAY GRID AND WILL NOT BE FURTHER PROCESSED
         ELSE
!       RAY WAS INSIDE THE ARRAY GRID
            IF(IGRID(IX,IY,1).NE.0.0D0) THEN
!
!       ADD IRRADIANCE OF RAY
               IGRID(IX,IY,2)=IGRID(IX,IY,2)+DSPOT(12)
!
!       ADD IRRADIANCE OF RAY TIMES THE ANGLE OF INCIDENCE AT NEWIMG SURFACE
               IGRID(IX,IY,3)=IGRID(IX,IY,2)+(DSPOT(12)*DSPOT(39))
!
!       INCREMENT THE NUMBER OF RAYS IN THE IX,IY BOX
               IGRID(IX,IY,4)=IGRID(IX,IY,3)+1.0D0
!
!       TOTAL SUM OF COS(I)S IN BOX ON SPOT SURFACE
               IGRID(IX,IY,5)=IGRID(IX,IY,4)+DSPOT(51)
!
!       AVERAGE OF COS(I)S IN BOX ON SPOT SURFACE
...
!       TO THE IRRADIANCE FILE
      ...
      WRITE(104,10)sys_fang_y()*TT1,-sys_fang_x()*TT2,INT(RTOT),INTTOT,DSP38 ,AREA_FILL
      WRITE(OUTLYNE,11)sys_fang_y()*TT1,-sys_fang_x()*TT2,AREA_FILL,int(rtot),DACOS(DSP38)*180.0D0/PII
      CALL SHOWIT(0)
11    FORMAT(1X,F8.3,1X,F8.3,1X,F8.3,1X,I6,1X,F8.3)
10    FORMAT(1X,F8.3,1X,F8.3,1X,I6,1X,G15.7,1X,G15.7,1X,G15.7)
!       WRITE OUT THE IRAD.DAT FILE HEADER, THEN THE DATA
!       FOR THE CURRENT FOB
      KK=KK+1
      WRITE(UNIT=105,REC=KK)sys_fang_y()*TT1,sys_fang_x()*TT2,RTOT
      KK=KK+1
      WRITE(UNIT=105,REC=KK)INTTOT,DSP38,AREA_FILL
      DO J=1,NX
         DO I=1,NY
            KK=KK+1
            WRITE(UNIT=105,REC=KK)IGRID(I,J,1),IGRID(I,J,2),IGRID(I,J,6)
         END DO
      END DO
      FN=1.0D0
...
!       CHECK FOR STRING INPUT
   ...
   IF(DF3.EQ.1) THEN
      DF3=0
! JRN Modified to deal with compiler failure of comparing logical to double
      W3=DBLE(NEWIMG)
!                W3 = MERGE(1., 0., DBLE(NEWIMG) == 0)
...
!       FOR THE CURRENT FOB
      KK=KK+1
      WRITE(UNIT=105,REC=KK)sys_scy()*TT1,sys_scx()*TT2,RTOT
      KK=KK+1
      WRITE(UNIT=105,REC=KK)INTTOT,DSP38,AREA_FILL
      DO J=1,NX
         DO I=1,NY
