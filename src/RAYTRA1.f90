...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"IRAYA" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
...
!       SET DEFAULT NUMERICS
   ...
   IF(W1.LT.0.0D0.OR.W1.GT.180.0D0) THEN
      ...
      & 'NUMERIC WORD 1, THE THETA ANGLE,,'//'\n'//&
   ...
   IF(W4.GT.1.0D0.OR.W4.LE.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NUMERIC WORD 4, THE STARTING RAY INTENSITY,'//'\n'//&
      & 'MUST BE GREATER THE ZERO AND NO GREATER THAN 1'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   WVN=W3
...
!       CHECK FOR NATURE OF QUALIFIER WORD
   IF(SQ.EQ.1) THEN
      IF(WC.EQ.'IRAYA'.AND.WQ.NE.'CAOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER USED WITH "IRAYA"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!       CHECK FOR CORRECT WAVELENGTH BOUNDS
   ...
   &INT(W3).NE.10) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'WAVELENGTH NUMBER BEYOND LEGAL BOUNDS'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"LIMRAYS" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
   END IF
!       CHECK FOR NUMERIC INPUT
   ...
   &WQ.NE.'VREAL') THEN
      OUTLYNE=&
      &'ONLY "LIMRAYS VREAL" CAN TAKE NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"LIMRAYS" TAKE NO NUMERIC WORD #2 TO #5 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
   ...
   IF(S1.EQ.1.AND.WQ.EQ.'VREAL') THEN
      IF(W1.LT.10.0D0.OR.W1.GT.100.0D0) THEN
         OUTLYNE=&
         &'"LIMRAYS VREAL" CAN TAKE NUMERIC WORD #1 FROM 10.0 TO 100.0'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         DEALLOCATE(VERARRAY,STAT=ALLOERR)
         RETURN
   ...
   IF(WQ.NE.'REAL'.AND.WQ.NE.'PARAX'.AND.WQ.NE.'VREAL') THEN
      OUTLYNE=&
      &'"LIMRAYS" ONLY TAKES "REAL", "VREAL" OR "PARAX" AS QUALIFIERS'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
...
1001  FORMAT('SURF#',1X,' X-MIN  RAY HT ',2X,' X-MAX  RAY HT ',&
      &2X,' Y-MIN  RAY HT ',2X,' Y-MAX  RAY HT ')
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"SETCLAP" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
   END IF
   IF(S4.EQ.1.or.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"SETCLAP" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
   END IF
...
!       CHECK FOR NUMERIC INPUT
   ...
   &WQ.NE.'VREAL') THEN
      OUTLYNE=&
      &'ONLY "SETCLAP VREAL" CAN TAKE NUMERIC WORD #3 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
   ...
   IF(S3.EQ.1.AND.WQ.EQ.'VREAL') THEN
      IF(W3.LT.10.0D0.OR.W3.GT.100.0D0) THEN
         OUTLYNE=&
         &'"SETCLAP VREAL" CAN TAKE NUMERIC WORD #1 FROM 10.0 TO 100.0'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         DEALLOCATE(VERARRAY,STAT=ALLOERR)
         RETURN
   ...
   IF(WQ.NE.'REAL'.AND.WQ.NE.'PARAX'.AND.WQ.NE.'VREAL') THEN
      OUTLYNE=&
      &'"SETCLAP" ONLY TAKES "REAL", "VREAL" OR "PARAX" AS QUALIFIERS'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      DEALLOCATE(VERARRAY,STAT=ALLOERR)
      RETURN
...
!
   IF(RAYCOD(1).EQ.5) THEN
      WRITE(OUTLYNE,*)&
      &'ANGLE OF DIFFRACTION AT THE GRATING AT SURFACE'
      CALL SHOWIT(1)
...
!
   IF(RAYCOD(1).EQ.9) THEN
      WRITE(OUTLYNE,*)&
      &'ANGLE OF DIFFRACTION AT THE HOE SURFACE IS PHYSICALLY'
      CALL SHOWIT(1)
...
!
   IF(RAYCOD(1).EQ.18) THEN
      WRITE(OUTLYNE,*)&
      &'RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
      CALL SHOWIT(1)
      RETURN
   END IF
!
   IF(RAYCOD(1).EQ.19) THEN
      WRITE(OUTLYNE,*)&
      &'AIMED CHIEF RAY COULD NOT HIT DEFINED TARGET POINT'
      CALL SHOWIT(1)
      RETURN
   END IF
!
...
!       AND STOP SEARCHING.
   IF(KKK.GT.NRAITR) THEN
      IF(MSG) THEN
         CALL RAY_FAILURE(INT(sys_last_surf()))
         OUTLYNE=&
         &'RAY FAILED TO CONVERGE TO REFERENCE SURFACE RAY-AIM POINT'
         CALL SHOWIT(1)
      END IF
...
!
      H_RAY(8,I)=DSQRT(&
      &((H_RAY(3,I)-ZOLD)**2)+((H_RAY(2,I)-YOLD)**2)&
      &+((H_RAY(1,I)-XOLD)**2))
!
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"BLKRAYS" TAKES NO STRING OR QUALIFIER INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.REFEXT) THEN
      CALL REPORT_ERROR_AND_FAIL('A CHIEF RAY DOES NOT EXIST'//'\n'//'"BLKRAYS" DID NOT EXECUTE', 1)
      RETURN
   END IF
!       CHECK FOR NUMERIC INPUT
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE=&
      &'"BLKRAYS" TAKE NO NUMERIC WORD #2 TO #5 INPUT'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"MRAYS" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      IF(DF5.EQ.0) MSG=.TRUE.
      RETURN
   END IF
!
...
!       CHECK FOR NATURE OF QUALIFIER WORD
   IF(SQ.EQ.1) THEN
      IF(WC.EQ.'MRAYS'.AND.WQ.NE.'CAOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER USED WITH "MRAYS"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
!       NO QUALIFIER,PROCEED
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"MTRACER" TAKES NO ADDITIONAL INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.MRAYSSET) THEN
      CALL REPORT_ERROR_AND_FAIL('NO MRAYS DEFINED WITH "MRAYS"'//'\n'//'NO ACTION TAKEN', 1)
      RETURN
   END IF
   IF(.NOT.MFOBSSET) THEN
      CALL REPORT_ERROR_AND_FAIL('NO MFOBS DEFINED WITH "MFOBS"'//'\n'//'NO ACTION TAKEN', 1)
      RETURN
   END IF
!
   FN=INT(FW4)
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"RAY" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      IF(DF5.EQ.0) MSG=.TRUE.
      RETURN
   END IF
   IF(DF4.EQ.1) W4=1.0D0
...
!       CHECK FOR NATURE OF QUALIFIER WORD
   IF(SQ.EQ.1) THEN
      IF(WC.EQ.'RAY'.AND.WQ.NE.'CAOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER USED WITH "RAY"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         IF(DF5.EQ.0) MSG=.TRUE.
         RETURN
      END IF
      IF(WC.EQ.'IRAY'.AND.WQ.NE.'CAOB') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER USED WITH "IRAY"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         IF(DF5.EQ.0) MSG=.TRUE.
         RETURN
      END IF
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MTRACERI" TAKES NO ADDITIONAL INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(.NOT.MFOBSSET) THEN
...
!       NRECL IS ONE 4 BYTE WORD
   ...
   &FORM='UNFORMATTED',RECL=(6*NRECL),STATUS='UNKNOWN')
!       READ MAIN HEADER
   K=1
...
!       CHECK FOR STRING INPUT
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MTRACERI GRID" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"MTRACERI GRID" TAKES NO NUMERIC WORD #4 OR #5 INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      OUTLYNE=&
      &'"MTRACERI GRID" REQUIRES EXPLICIT NUMERIC WORDS #1 AND #2'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(W1.LE.1.0D0) THEN
      OUTLYNE=&
      &'"MTRACERI GRID" REQUIRES A POSITIVE GRID DIMENSIONALITY, (NW1)'
      CALL SHOWIT(1)
      OUTLYNE=&
   ...
   IF(W2.LE.0.0D0) THEN
      OUTLYNE=&
      &'"MTRACERI GRID" REQUIRES A POSITIVE, NON-ZERO GRID SIZE (NW2)'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!       INSIDE ANY CLEAR APERTURE ON THE SPOT DIAGRAM SURFACE
   ...
   &IYARRAY(1:IARRAY_DIM,1:IARRAY_DIM),IGRID(&
   &1:IARRAY_DIM,1:IARRAY_DIM,1:10),STAT=ALLOERR)
