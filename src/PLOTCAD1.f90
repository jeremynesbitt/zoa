...
!
   REAL UPPER,LOWER
!
   real(real64) SFI,MDX,MDY,GAMGAM,V1,V2
!
   CHARACTER NAME*40
!
...
!
   IF(WC.EQ.'PLOT') THEN
      ...
      IF(.NOT.PLQ) THEN
         ...
         & 'INVALID QUALIFIER USED WITH "PLOT"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!       CHECK FOR ZERO VALUES AND DIS-ALLOW
      IF(W1.LE.0.0D0.OR.W2.LE.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT SCALE" REQUIRES NON-ZERO, POSTIVE INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      SCFAXP=1.0D0/W1
...
!       CHECK SYNTAX
      IF(SST.EQ.1.OR.SN.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT END" TAKES NO STRING OR NUMERIC INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DEVTYP.NE.1) THEN
         CALL REPORT_ERROR_AND_FAIL('WARNING: NO PLOTFILE EXISTS TO CLOSE', 1)
         RETURN
      END IF
      CALL PSTOP
   END IF
...
!       CHECK FOR ZERO VALUES AND DIS-ALLOW
      IF(W1.LE.0.0D0.OR.W2.LE.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"PLOT SIZE" REQUIRES NON-ZERO, POSTIVE INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      SCFAXP=W1
...
!       PLOT CLAP COMMAND
!
   real(real64) X,Y,Z,XN,YN,ZN,ROT1X,ROT1Z,ROT2Y,TMAXI,TMINI,&
   &ROT2Z,AX,AY,AZ,AALF,APHI,YMAXI,YMINI,XMAXI,XMINI,MAXI,MINI &
...
!
   real(real64) X,Y,Z,XN,YN,ZN,ROT1X,ROT1Z,ROT2Y,XBASE,YBASE,&
   &ROT2Z,AX,AY,AZ,AALF,APHI,YMAXI,YMINI,XMAXI,XMINI,MAXI,MINI &
...
!
   real(real64) SAGARRAY1,SAGARRAY2,SAGARRAY3
!
   real(real64) XMAX,XMIN,ZMAX,ZMIN,X,Y,SAG
!
...
!
   ...
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT SAGFILE" TAKES NO STRING INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"PLOT SAGFILE" ONLY TAKES NUMERIC WORD #1'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!
   ...
   INTEGER ALLOERR
!
   real(real64) SAGARRAY1,SAGARRAY2,SAGARRAY3,XMIN,XMAX &
...
!
   real(real64) XMIN,XMAX,ZMIN,ZMAX
!
