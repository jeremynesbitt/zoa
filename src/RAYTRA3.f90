!       THIS IS THE THIRD FILE OF RAYTRACING ROUTINES
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR4.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO A TYPE 17 USER-DEFINED SURFACE
!
...
!     THE Z VALUE CORRESPONDING TO THE Y AND X VALUES.
...
!     NUMBERS 301 TO 348 AND ACCESSED BY THE USER VIA RCL COMMANDS
...
!     (FUN10 call removed - binary macro system removed)
   RETURN
END
! SUB NR1.FOR
SUBROUTINE NR1(ERRR)
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR1.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC PLANO SURFACE
!
...
!
   LOGICAL DIFCAL,ERRR
!
   REAL*8 C1,C2,C3,C4,MAG,ZCAL,ZZTOP,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,C5,ACLENG,DEL,A34,FNZ1 ,FUNC1X,FUNC1Y,ERR,SNGX,SNGY,SNGZ,DER1X,DER1Y ,C41,C42,C43,C44,C45,XPASS,YPASS,ZPASS
   REAL*8 LENG
...
!
   ...
   IF(surf_special_type(R_I) > 0.AND.ABS(surf_special_type(R_I)) /= 6.AND.ABS(surf_special_type(R_I)) /= 7.AND.ABS(surf_special_type(R_I)) /= 9.AND.ABS(surf_special_type(R_I)) /= 10.AND.ABS(surf_special_type(R_I)) /= 11.AND.ABS(surf_special_type(R_I)) /= 12.AND.ABS(surf_special_type(R_I)) /= 13.AND.ABS(surf_special_type(R_I)) /= 15.AND.ABS(surf_special_type(R_I)) /= 16.AND.ABS(surf_special_type(R_I)) /= 17.AND.ABS(surf_special_type(R_I)) /= 19.AND.ABS(surf_special_type(R_I)) /= 20.AND.ABS(surf_special_type(R_I)) /= 22.AND.ABS(surf_special_type(R_I)) /= 24 .OR.surf_default_flag(R_I) == 1) THEN
...
!       CALCULATE INTERSECTION
   DO 10 I=1,NRAITR
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ1 EVALUATES THE FUNCTION
!
      IF(DIFCAL) THEN
         ERRR=.FALSE.
         ...
         IF(ERRR) RETURN
      ELSE
         FNXP=-(((1.0D0*2.0D0*R_X)*C5))
         FNYP=-(((1.0D0*2.0D0*R_Y)*C5))
      END IF
      FNZP=SNGZ
!
...
!
      ...
      MN=FNYP/MAG
      NN=FNZP/MAG
!       THE INTERSECTION OF THE RAY WITH THIS TANGENT
!       PLANE
!       USING:
!
...
!
      ...
      ELSE
!       KEEP TRYING
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
         R_X=XP
         R_Y=YP
         R_Z=ZP
      END IF
10 CONTINUE
   IF(MSG) THEN
      ...
      CALL SHOWIT(1)
      OUTLYNE='CONVERGENCE FAILURE OCCURRED DURING RAY INTERSECTION'
      CALL SHOWIT(1)
   ...
   RETURN
END
FUNCTION FNZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AR,A34,AC41,AC42,AC43,AC44,AC45)
   USE GLOBALS
!
   use DATLEN
   use mod_system
...
!
   INTEGER IPASS1
!
   REAL*8 FNZ1,AX,AY,AC1,AC2,AC3,AC4,AC5,AAC1,AAC2,AAC3 ,AAC4,AAC5,AAX,AAY,FNZZ1,A34,R,THETA,FF2,FF3,FF4,XX,YY,FF5,AZ ,AAAX,AAAY,AAC41,AAC42,AAC43,AAC44,AAC45,AC41,AC42,AC43,AC44,AC45
!
...
!
!
!     AR IS THE SURFACE NUMBER
!
   INTEGER I
!
   FNZZ1(AAX,AAY,AAC1,AAC2,AAC3,AAC4,AAC5 ,AAC41,AAC42,AAC43,AAC44,AAC45)=(((AAX**2)+(AAY**2))*AAC5)+((((AAX**2)+(AAY**2))**2)*AAC1)+((((AAX**2)+(AAY**2))**3)*AAC2)+((((AAX**2)+(AAY**2))**4)*AAC3)+((((AAX**2)+(AAY**2))**5)*AAC4)+((((AAX**2)+(AAY**2))**6)*AAC41)+((((AAX**2)+(AAY**2))**7)*AAC42)+((((AAX**2)+(AAY**2))**8)*AAC43)+((((AAX**2)+(AAY**2))**9)*AAC44)+((((AAX**2)+(AAY**2))**10)*AAC45)
!
!
   DA34=DABS(A34)
   IF(A34.EQ.0.0D0.AND.surf_default_flag(AR) == 0.OR.DA34.EQ.6.0D0.OR.DA34.EQ.7.0D0.OR.DA34.EQ.9.0D0.OR.DA34.EQ.10.0D0.OR.DA34.EQ.11.0D0.OR.DA34.EQ.12.0D0.OR.DA34.EQ.15.0D0.OR.DA34.EQ.16.0D0.OR.DA34.EQ.17.0D0.OR.DA34.EQ.19.0D0.OR.DA34.EQ.20.0D0) THEN
...
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AC41,AC42,AC43,AC44 ,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      ...
      END IF
      FNZ1=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AC41,AC42,AC43,AC44 ,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      ...
      FNZ1=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      ...
      FNZ1=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      ...
      FNZ1=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     IF SPECIAL SURFACE TYPE 23
...
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      ...
      FNZ1=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     lSPECIAL SURFACE TYPE 5
...
!     TYPE 22 SAG GRID
      ...
      END IF
!     NOW ADD THE REGULAR TERM
!      IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!      IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ1=ZPASS+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     DEFORM SURFACE
...
!     CALL DEFGRID
      ...
      IF(GERROR1) THEN
         IF(MSG) THEN
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',ISURF
            CALL SHOWIT(1)
            OUTLYNE='DEFORM FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE='OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         ...
         IF(GERROR2) THEN
            IF(MSG) THEN
               WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',ISURF
               CALL SHOWIT(1)
               OUTLYNE='RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
               CALL SHOWIT(1)
            END IF
            STOPP=1
            RAYCOD(1)=18
...
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NR2.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC/CONIC SURFACE
!
   EXTERNAL FNZ2
   EXTERNAL FUNC2X,FUNC2Y,DER2X,DER2Y
!
   REAL*8 C1,C2,C3,C4,ACLENG,FNZ2,DEL,SNGX,SNGY,SNGZ,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,C41,C42,C43,C44,C45,MAG,ZCAL,Q,C5,C6,ZZTOP,A34,ZTEST,CV,CC,DER2X,DER2Y,FUNC2X,FUNC2Y,ERR,XPASS,YPASS,ZPASS
   REAL*8 LENG
   COMMON/PASSLENG/LENG
   REAL*8 ZZEMAX,ZZEMIN
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
...
!       C5=CONIC CONSTANT
!       C6=CURVATURE
!       C1 TO C4 ARE THE 4TH, 6TH, 8TH AND 10TH ORDER ASPHERIC
!       DEFORMATION TERMS
!       C41 TO C45 ARE THE 12TH THROUGH 20TH ORDER ASPHERIC
!       DEFORMATION TERMS
!
   !call logger%logTextWithNum("NR2 Start R_X ", R_X)
   !call logger%logTextWithNum("NR2 Start R_Y ", R_Y)

!       CALCULATE INTERSECTION

   DO 10 I=1,NRAITR
!
      Q=(1.0D0-((C5+1.0D0)*(C6**2)*(((R_X**2)+(R_Y**2)))))
      IF(Q.LT.0.0D0) THEN
         IF(MSG) THEN
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
            CALL SHOWIT(1)
      ...
      Q=1.0D0+Q
      DEL=DELSUR
      !call logger%logTextWithNum("NR2 Iter ", I)
      !call logger%logTextWithNum("Q = ", Q)
      !call logger%logTextWithNum("C5 = ", C5)
      !call logger%logTextWithNum("C6 = ", C6)



!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ2 EVALUATES THE FUNCTION
!
      IF(DIFCAL) THEN
         ERRR=.FALSE.
         FNXP=-DER2X(FUNC2X,R_X,DELSUR,ERRR)
         FNYP=-DER2Y(FUNC2Y,R_Y,DELSUR,ERRR)
         !call logger%logTextWithNum("FNXP is ", FNXP)
         !call logger%logTextWithNum("FNYP is ", FNYP)

         IF(ERRR) RETURN
      ELSE
         FNXP=-((((2.0D0*C6*R_X*Q*(Q-1.0D0))+((C6**3)*R_X*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2))))
         FNYP=-((((2.0D0*C6*R_Y*Q*(Q-1.0D0))+((C6**3)*R_Y*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2))))
      END IF
      FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
      ...
      NN=FNZP/MAG
!
!       THESE ARE THE DIRECTION COSINES FOR THE SURFACE NORMAL
!       FOR THE CURRENT X,Y AND Z
!
!       IF X,Y AND Z ARE THE CORRECT X,Y AND Z THEN
!       ZCAL WILL EQUAL Z SO THAT F(X,Y,Z)=0
      ZCAL=FNZ2(R_X,R_Y,C1,C2,C3,C4,C6,R_I,A34,Q ,C41,C42,C43,C44,C45)
      HV1=ZCAL-R_Z
      HV2=((R_L*LN)+(R_M*MN)+(R_N*NN))
      HV=HV1/HV2
      !call logger%logTextWithNum("Aspheric correction term is", HV)
!       NEW X,Y AND Z ARE
      XP=R_X+(HV*R_L)
      YP=R_Y+(HV*R_M)
      ZP=R_Z+(HV*R_N)
!       SHOULD WE CONTINUE?
      ZZTOP=FNZ2(XP,YP,C1,C2,C3,C4,C6,R_I,A34,Q ,C41,C42,C43,C44,C45)
      LENG=DABS(ZZTOP-ZP)
      ACLENG=LENG
      IF(LENG.LE.SURTOL) THEN
         R_X=XP
         ...
         Q=DSQRT(Q)
         Q=1.0D0+Q
         !call logger%logTextWithNum("Aspheric R_Y  is", R_Y)
         !call logger%logTextWithNum("Aspheric Q term is", Q)
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ2 EVALUATES THE FUNCTION
!
...
!
         ...
         NN=FNZP/MAG
         STOPP=0
         !call logger%logTextWithNum("Mag is ", MAG)
         RETURN
      ELSE
!       KEEP TRYING
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
...
10 CONTINUE
   ...
   RETURN
END
FUNCTION FNZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,AR,A34,Q ,AC41,AC42,AC43,AC44,AC45)
   USE GLOBALS
!
   use DATLEN
   use mod_system
...
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   REAL*8 FNZ2,AX,AY,AC1,AC2,AC3,AC4,AAC1,AAC2,AAC3 ,AAC4,AAX,AAY,FNZZ2,A34,R,THETA,FF2,FF3,AQ,Q,AC6,FF4,XX,YY,AAC6,FF5,AAAX,AAAY,AC41,AC42,AC43,AC44,AC45,AAC41,AAC42,AAC43 ,AAC44,AAC45,AZ
!
   INTEGER IPASS1
!
...
!
   INTEGER I
!
   FNZZ2(AAX,AAY,AAC1,AAC2,AAC3,AAC4,AAC6,AQ,AAC41,AAC42,AAC43,AAC44,AAC45)=((((AAX**2)+(AAY**2))**2)*AAC1)+((((AAX**2)+(AAY**2))**3)*AAC2)+((((AAX**2)+(AAY**2))**4)*AAC3)+((((AAX**2)+(AAY**2))**5)*AAC4)+((((AAX**2)+(AAY**2))**6)*AAC41)+((((AAX**2)+(AAY**2))**7)*AAC42)+((((AAX**2)+(AAY**2))**8)*AAC43)+((((AAX**2)+(AAY**2))**9)*AAC44)+((((AAX**2)+(AAY**2))**10)*AAC45)+((AAC6*((AAX**2)+(AAY**2)))/AQ)

   !FUNCTION FNZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,AR,A34,Q
   !AC41,AC42,AC43,AC44,AC45)

   ! call logger%logTextWithNum("FNZ2 AAC1=",AC1)
   ! call logger%logTextWithNum("FNZ2 AAC2=",AC2)
   ! call logger%logTextWithNum("FNZ2 AAC3=",AC3)
   ! call logger%logTextWithNum("FNZ2 AAC4=",AC4)
   ! call logger%logTextWithNum("FNZ2 AAC41=",AC41)
   ! call logger%logTextWithNum("FNZ2 AAC42=",AC42)
   ! call logger%logTextWithNum("FNZ2 AAC43=",AC43)
   ! call logger%logTextWithNum("FNZ2 AAC44=",AC44)
   ! call logger%logTextWithNum("FNZ2 AAC45=",AC45)
   ! call logger%logTextWithNum("FNZ2 AAX=",AX)
   ! call logger%logTextWithNum("FNZ2 AAY=",AY)
   ! call logger%logTextWithNum("FNZ2 AAC6=",AC6)
   ! call logger%logTextWithNum("FNZ2 AQ=",Q)
   !
   ! PRINT *, "TroubleTermis? ", ((((AX**2)+(AY**2))**6)*AC41)
   ! PRINT *, "Term w/o Asphere ",(((AX**2)+(AY**2))**6)
   ! T1 = ((((AX**2)+(AY**2))**2)*AC1)
   ! T2 = ((((AX**2)+(AY**2))**3)*AC2)
   ! T3 = ((((AX**2)+(AY**2))**4)*AC3)
   ! T4 = ((((AX**2)+(AY**2))**5)*AC4)
   ...
   ! T7 = ((((AX**2)+(AY**2))**8)*AC43)
   ! T8 = ((((AX**2)+(AY**2))**9)*AC44)
   ! T9 = ((((AX**2)+(AY**2))**10)*AC45)
   ! T10 = ((AC6*((AX**2)+(AY**2)))/Q)
   !
   ! PRINT *, "Total is ", T1+T2+T3+T4+T5+T6+T7+T8+T9+T10
   !
   ! call logger%logTextWithNum("T1=",T1)
   ! call logger%logTextWithNum("T2=",T2)
   ! call logger%logTextWithNum("T3=",T3)
   ! call logger%logTextWithNum("T4=",T4)
   ! call logger%logTextWithNum("T5=",T5)
   ! call logger%logTextWithNum("T6=",T6)
   ! call logger%logTextWithNum("T7=",T7)
   ! call logger%logTextWithNum("T8=",T8)
   ! call logger%logTextWithNum("T9=",T9)
   ! call logger%logTextWithNum("T10=",T10)
   !
   ! call logger%logTextWithNum("FNZ2 AAC3=",AC3)
   ! call logger%logTextWithNum("FNZ2 AAC4=",AC4)
   ! call logger%logTextWithNum("FNZ2 AAC41=",AC41)
   ...
   !call logger%logTextWithNum("FNZ2 Result=",FNZZ2)

!
!     IF SPECIAL SURFACE TYPE 1, 2 OR 3 ARE TO BE APPLIED
...
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      ...
      ELSE
         OMEGA5Y=TWOPII/DABS(FTFL01(15,AR))
      END IF
      FNZ2=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      ...
      FNZ2=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      ...
      FNZ2=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      ...
      FNZ2=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      ...
      FNZ2=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
...
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ2=ZPASS+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q ,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     DEFORM SURFACE
   IF(surf_default_flag(AR) == 1) THEN
...
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NRPARAX.FOR. THIS DOES SURF NORM. FOR PARAXIAL
!       SURFACES
!
   INTEGER I
!
   LOGICAL ERRR
!
   REAL*8 C1,C2,C3,C4,ACLENG,DEL,A34,FNZ3 ,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,CV,ZTEST,SNGX,SNGY,SNGZ,MAG,ZCAL,QQ,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,CY,KY,EY,FY,DY,GY,CX,KX,DX,EX,FX,GX,ZZTOP ,FUNC1X,FUNC1Y,ERR,XPASS,YPASS,ZPASS ,FUNC2X,FUNC2Y,DER1X,DER1Y,DER2X,DER2Y ,FUNC3X,FUNC3Y,DER3X,DER3Y
   REAL*8 LENG
   COMMON/PASSLENG/LENG
!
   REAL*8 ZZEMAX,ZZEMIN
...
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NR3.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC/CONIC TORIC SURFACE
!
   INTEGER I
!
   EXTERNAL FNZ3
...
!
   REAL*8 C1,C2,C3,C4,ACLENG,DEL,A34,FNZ3 ,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,CV,ZTEST,SNGX,SNGY,SNGZ,MAG,ZCAL,QQ,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,CY,KY,EY,FY,DY,GY,CX,KX,DX,EX,FX,GX,ZZTOP,DER3X,DER3Y ,FUNC3X,FUNC3Y,ERR,XPASS,YPASS,ZPASS
...
!       Y-TORIC, THE BASE CURVATURE LIES IN THE XZ PLANE.
!
   IF(C7.EQ.1.0D0) THEN
...
!       CALCULATE INTERSECTION
   DO 10 I=1,NRAITR
      QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(R_X**2))-((KY+1.0D0)*(CY**2)*(R_Y**2)))
      IF(QQ.LT.0.0D0) THEN
         IF(MSG) THEN
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
            CALL SHOWIT(1)
      ...
      QQ=DSQRT(QQ)
      QQ=1.0D0+QQ
!       CALCULATE CURRENT TANGENT PLANE NORMAL
      DEL=DELSUR
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ3 EVALUATES THE FUNCTION
!
      ERRR=.FALSE.
      FNXP=-DER3X(FUNC3X,R_X,DELSUR,ERRR)
      FNYP=-DER3Y(FUNC3Y,R_Y,DELSUR,ERRR)
...
!       USING:
!
      ZCAL=FNZ3(R_X,R_Y,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,R_I,A34,QQ)
      HV1=ZCAL-R_Z
      HV2=((R_L*LN)+(R_M*MN)+(R_N*NN))
      HV=HV1/HV2
!       NEW X,Y AND Z ARE
      ...
      ZP=R_Z+(HV*R_N)
!       SHOULD WE CONTINUE?
      QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(XP**2))-((KY+1.0D0)*(CY**2)*(YP**2)))
      IF(QQ.LT.0.0D0) THEN
         IF(MSG) THEN
            WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
            CALL SHOWIT(1)
      ...
      QQ=DSQRT(QQ)
      QQ=1.0D0+QQ
      ZZTOP=FNZ3(XP,YP,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,R_I,A34,QQ)
      LENG=DABS(ZZTOP-ZP)
      ACLENG=LENG
      IF(LENG.LE.SURTOL) THEN
         R_X=XP
...
10 CONTINUE
   ...
   RETURN
END
FUNCTION FNZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,AR,A34,Q)
   USE GLOBALS
!
   use DATLEN
   use mod_system
...
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   REAL*8 FNZ3,AX,AY,ACX,ACY,ADX,ADY,AACX,AACY,AADX ,AADY,AAX,AAY,FNZZ3,A34,R,THETA,FF2,FF3,AQ,Q,AEX,AEY,AAEX,AAEY,AAFX,AAFY,AAGX,AAGY,AFX,AFY,AGX,AGY,XX,YY,FF4 ,FF5,AAAX,AAAY,DA34
   REAL*8 ZZEMAX,ZZEMIN
   REAL*8 JK_WAVE
   REAL*8 AMP1,OMEGA1X,OMEGA1Y
   REAL*8 AMP2,OMEGA2X,OMEGA2Y
...
!
   INTEGER I
!
   FNZZ3(AAX,AAY,AACX,AACY,AADX,AADY,AAEX,AAEY,AAFX,AAFY,AAGX,AAGY,AQ)=(((AACX*(AAX**2))+(AACY*(AAY**2)))/AQ)+(AADY*(((1.0D0-AADX)*(AAX**2))+((1.0D0+AADX)*(AAY**2)))**2)+(AAEY*(((1.0D0-AAEX)*(AAX**2))+((1.0D0+AAEX)*(AAY**2)))**3)+(AAFY*(((1.0D0-AAFX)*(AAX**2))+((1.0D0+AAFX)*(AAY**2)))**4)+(AAGY*(((1.0D0-AAGX)*(AAX**2))+((1.0D0+AAGX)*(AAY**2)))**5)
!
!     IF SPECIAL SURFACE TYPE 1, 2 OR 3 ARE TO BE APPLIED
!     THEY ARE APPLIED HERE
!
...
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      ...
      ELSE
         OMEGA5Y=TWOPII/DABS(FTFL01(15,AR))
      END IF
      FNZ3=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      ...
      FNZ3=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
...
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      ...
      FNZ3=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
...
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      ...
      FNZ3=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
...
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      ...
      FNZ3=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
...
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ3=ZPASS+FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     DEFORM SURFACE
   IF(surf_default_flag(AR) == 1) THEN
...
!
   EXTERNAL FNZ1
!
   REAL*8 FUNC1X,AX,FNZ1,C1,C2,C3,C4,C5,A34 ,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
...
!
   EXTERNAL FNZ1
!
   REAL*8 FUNC1Y,AY,FNZ1,C1,C2,C3,C4,C5,A34,C41,C42,C43 ,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
...
!
   EXTERNAL FNZ2
!
   REAL*8 FUNC2X,AX,FNZ2,C1,C2,C3,C4,C5,C6,A34,Q ,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   ...
   C45=surf_asphere_coeff(R_I, 20)
   A34=surf_special_type(R_I)
   Q=(1.0D0-((C5+1.0D0)*(C6**2)*((AX**2)+(R_Y**2))))
   Q=DSQRT(DABS(Q))
   Q=1.0D0+Q
!
   FUNC2X=FNZ2(AX,R_Y,C1,C2,C3,C4,C6,R_I,A34,Q,C41,C42,C43,C44,C45)
!
   RETURN
END
FUNCTION FUNC2Y(AY)
...
!
   EXTERNAL FNZ2
!
   REAL*8 FUNC2Y,AY,FNZ2,C1,C2,C3,C4,C5,C6,A34,Q ,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   ...
   C5=surf_conic(R_I)
   C6=(surf_curvature(R_I))
   Q=(1.0D0-((C5+1.0D0)*(C6**2)*((R_X**2)+(AY**2))))
   Q=DSQRT(DABS(Q))
   Q=1.0D0+Q
!
   FUNC2Y=FNZ2(R_X,AY,C1,C2,C3,C4,C6,R_I,A34,Q,C41,C42,C43,C44,C45)
!
   RETURN
END
!
...
!
   EXTERNAL FNZ3
!
   REAL*8 FUNC3X,AX,FNZ3,C1,C2,C3,C4,C5,C6,C7,C8,C9 ,C10,C11,C12,C13,C14,A34,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY ,QQ
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
...
!       XTORIC, DO ASSIGNMENTS
      ...
      GX=C4
   END IF
   QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(AX**2))-((KY+1.0D0)*(CY**2)*(R_Y**2)))
   QQ=DSQRT(DABS(QQ))
   QQ=1.0D0+QQ
!
...
!
   EXTERNAL FNZ3
!
   REAL*8 FUNC3Y,AY,FNZ3,C1,C2,C3,C4,C5,C6,C7,C8,C9 ,C10,C11,C12,C13,C14,A34,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY ,QQ
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
...
!       XTORIC, DO ASSIGNMENTS
      ...
      GX=C4
   END IF
   QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(R_X**2))-((KY+1.0D0)*(CY**2)*(AY**2)))
   QQ=DSQRT(DABS(QQ))
   QQ=1.0D0+QQ
!
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR5.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO A CONIC SURFACE WITH FOURIER-
!       LEGENDRE COEFFICIENTS
!
   REAL*8 MAG,H,RHO,Z3,ZM ,X,Y,Z,L,M,N,XO,YO,ZO,LO,MO,NO,ZETA,Z1,Z2,CEE
!
...
!
   ...
   IF(R_Z.LT.0.0D0.AND.CEE.GT.0.0D0.OR.R_Z.GT.0.0D0.AND.CEE.LT.0.0D0) RETURN
   Z1=FTFL01(1,R_I)
   Z2=FTFL01(2,R_I)
   Z3=FTFL01(3,R_I)
   ZM=R_Z
...
!     INITIAL GUESS AT H IS 0
   ...
   NO=R_N
!
!     FIND THE NEW H VALUE WHICH MAKES F(H)=0
!
   CALL FINDH
   IF(STOPP.EQ.1) RETURN
!
!     H IS THE DISTANCE TRAVELED FROM THE CONIC INTERSECTION
!     TO THE DEFORMED CONIS INTERSECTION. IT IS A SMALL NUMBER!
!
!     THE RAY COORDINATES OF THE SURFACE INTERSECTION ARE NOW JUST:
   R_X=R_X+(H*R_L)
   R_Y=R_Y+(H*R_M)
   R_Z=R_Z+(H*R_N)
!
!     NOW CALCULATE THE DIRCTION COSINES OF THE SURFACE NORMAL
!
   X=R_X
   Y=R_Y
   Z=R_Z
...
!     THIS IS THE SURFACE FUNCTION
   ...
   use DATMAI
   IMPLICIT NONE
   REAL*8 FUNC,H,X,Y,Z,KAPPA,CEE,RHO,LO,MO,NO,Z3,ZM ,XO,YO,ZO,THETA,ZETA,Z1,Z2,FL,FL1,FL2,FL3,FL4,FL5,FL6,FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,FLT,FLZEE ,FLX,FLY
   EXTERNAL FL
   INTEGER FLII
   COMMON/FUNCER/H
   COMMON/FUNN5/XO,YO,ZO,LO,MO,NO
...
!
   ...
   FLII=15
   FL15=FL()
   FUNC=-RHO+DSQRT(((2.0D0*Z)/CEE)-((Z**2)*(KAPPA+1.0D0)))+(FTFL01(4,R_I)*FL1)+(FTFL01(5,R_I)*FL2)+(FTFL01(6,R_I)*FL3)+(FTFL01(7,R_I)*FL4)+(FTFL01(8,R_I)*FL5)+(FTFL01(9,R_I)*FL6)+(FTFL01(10,R_I)*FL7)+(FTFL01(11,R_I)*FL8)+(FTFL01(12,R_I)*FL9)+(FTFL01(13,R_I)*FL10)+(FTFL01(14,R_I)*FL11)+(FTFL01(15,R_I)*FL12)+(FTFL01(16,R_I)*FL13)+(FTFL01(17,R_I)*FL14)+(FTFL01(18,R_I)*FL15)
   RETURN
END
!
SUBROUTINE FINDNORM
   ...
   use DATMAI
   IMPLICIT NONE
   REAL*8 L,M,N,KAPPA,X,Y,Z,CEE ,RHO,Z1,Z2,THETA,ZETA,FLX,FLY,FLZ,FLZEE,FLXX,FLYY
   EXTERNAL FLX,FLY,FLZ
   REAL*8 FL1,FL2,FL3,FL4,FL5,FL6,FL7,FLT ,N1,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,ZETAD,Z3,ZM
   INTEGER FLII
   COMMON/NORMER/X,Y,Z,L,M,N
   ...
   FLII=15
   FL15=FLX()
   L=LN -(FTFL01(4,R_I)*FL1)-(FTFL01(5,R_I)*FL2)-(FTFL01(6,R_I)*FL3)-(FTFL01(7,R_I)*FL4)-(FTFL01(8,R_I)*FL5)-(FTFL01(9,R_I)*FL6)-(FTFL01(10,R_I)*FL7)-(FTFL01(11,R_I)*FL8)-(FTFL01(12,R_I)*FL9)-(FTFL01(13,R_I)*FL10)-(FTFL01(14,R_I)*FL11)-(FTFL01(15,R_I)*FL12)-(FTFL01(16,R_I)*FL13)-(FTFL01(17,R_I)*FL14)-(FTFL01(18,R_I)*FL15)
   FLII=1
   FL1=FLY()
   FLII=2
   FL2=FLY()
   ...
   FLII=15
   FL15=FLY()
   M=MN -(FTFL01(4,R_I)*FL1)-(FTFL01(5,R_I)*FL2)-(FTFL01(6,R_I)*FL3)-(FTFL01(7,R_I)*FL4)-(FTFL01(8,R_I)*FL5)-(FTFL01(9,R_I)*FL6)-(FTFL01(10,R_I)*FL7)-(FTFL01(11,R_I)*FL8)-(FTFL01(12,R_I)*FL9)-(FTFL01(13,R_I)*FL10)-(FTFL01(14,R_I)*FL11)-(FTFL01(15,R_I)*FL12)-(FTFL01(16,R_I)*FL13)-(FTFL01(17,R_I)*FL14)-(FTFL01(18,R_I)*FL15)
   FLII=1
   FL1=FLZ()
   FLII=2
   FL2=FLZ()
   ...
   FLII=15
   FL15=FLZ()
   N=NN -(FTFL01(4,R_I)*FL1)-(FTFL01(5,R_I)*FL2)-(FTFL01(6,R_I)*FL3)-(FTFL01(7,R_I)*FL4)-(FTFL01(8,R_I)*FL5)-(FTFL01(9,R_I)*FL6)-(FTFL01(10,R_I)*FL7)-(FTFL01(11,R_I)*FL8)-(FTFL01(12,R_I)*FL9)-(FTFL01(13,R_I)*FL10)-(FTFL01(14,R_I)*FL11)-(FTFL01(15,R_I)*FL12)-(FTFL01(16,R_I)*FL13)-(FTFL01(17,R_I)*FL14)-(FTFL01(18,R_I)*FL15)
!     NORMALS HAVE SAME SIGN AS THEY DID FOR JUST THE CONIC
!     INTERSECTION
!
   RETURN
END
...
!
   IMPLICIT NONE
!
   REAL*8 FL1,FL2,FL3,FL4,FL5,FL6,FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15 ,ZETA,THETA,FL,Z,A,FLXX,FLYY
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,FLXX,FLYY
!
!       FUNCTIONS FOR TYPE 18 SURFACE, FOURIER-LEGENDRE
   FL1(Z,A)=1.0D0
   ...
   FL14(Z,A)=DSQRT(10.0D0)*DCOS(2.0D0*A)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*DSIN(2.0D0*A)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
!
...
!
   IMPLICIT NONE
!
   REAL*8 FL1,FL2,FL3,FL4,FL5,FL6,FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,CX,SX ,AP,A,THETA,C0,C1,S0,S1,ZETA,X,Y,FLX,Z,AX,AY,P
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,X,Y
!
!       FUNCTIONS FOR TYPE 18 SURFACE, X-DERIVATIVE, FOURIER-LEGENDRE
   ...
   FL14(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
!
   P=DSQRT((X**2)+(Y**2))
!
   ...
   IF(FLII.EQ.11) FLX=FL11(ZETA,THETA)*((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.12) FLX=FL12(ZETA,THETA)*4.0D0*DCOS(THETA)*CX(X,Y,P)
   IF(FLII.EQ.13) FLX=FL13(ZETA,THETA)*((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.14) FLX=FL14(ZETA,THETA)*4.0D0*DCOS(THETA)*CX(X,Y,P)
   IF(FLII.EQ.15) FLX=FL15(ZETA,THETA)*((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   RETURN
END
! SUB FLY.FOR
FUNCTION FLY()
!
   IMPLICIT NONE
!
   REAL*8 FL1,FL2,FL3,FL4,FL5,FL6,FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,AP,P,CY,SY ,A,C0,C1,S0,S1,ZETA,X,Y,FLY,Z,AX,AY,THETA
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,X,Y
!
!       FUNCTIONS FOR TYPE 18 SURFACE, Y-DERIVATIVE, FOURIER-LEGENDRE
...
!
   ...
   IF(FLII.EQ.11) FLY=FL11(ZETA,THETA)*((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.12) FLY=FL12(ZETA,THETA)*4.0D0*DCOS(THETA)*CY(X,Y,P)
   IF(FLII.EQ.13) FLY=FL13(ZETA,THETA)*((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.14) FLY=FL14(ZETA,THETA)*4.0D0*DCOS(THETA)*CY(X,Y,P)
   IF(FLII.EQ.15) FLY=FL15(ZETA,THETA)*((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   RETURN
END
! SUB FLZ.FOR
FUNCTION FLZ()
!
   IMPLICIT NONE
!
   REAL*8 FL1,FL2,FL3,FL4,FL5,FL6,FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15 ,ZETA,THETA,FLZ,Z,A,X,Y,P,ZETAD
!
   COMMON/ZEETAA/ZETAD
...
!
   INTEGER FLII
!
!       FUNCTIONS FOR TYPE 18 SURFACE, Z-DERIVATIVE, FOURIER-LEGENDRE
   FL1(Z,A)=0.0D0
   FL2(Z,A)=DSQRT(3.0D0)
   ...
   FL14(Z,A)=DSQRT(10.0D0)*DCOS(2.0D0*A)*(((6.0D0*(Z)))/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*DSIN(2.0D0*A)*(((6.0D0*(Z)))/2.0D0)
!
   P=DSQRT((X**2)+(Y**2))
   IF(P.EQ.0.0D0) RETURN
...
!
   REAL*8 A,B,C,QQ,SIGNNU,OR_N,OR_Z,PHA11,MAGO,MAGR,TEST1,TEST2,NUSUBS,TIRTESTER,SINI,RR_N,TESTLEN,RR_Z ,Q,SIGNB,MAG,J,ARG,C1,C2,C3,C4,SNIND2 ,XX,YY,FPX,FPY,FPZ,C5,C6,ZMIN,ZMAX,AX,AY,AZ,X1,X2,Y1,Y2,Z1,Z2,LN1,LN2,MN1,MN2,NN1,NN2 ,SGNB,SNINDX,JIM,UX,UY,UXP,UYP,INEW,L1,M1,N1
!
   INTEGER ZPMIN,ZPMAX,ISURF
!
!     DIFFRACTION GRATING STUFF
   REAL*8 CHIX,CHIY,CHIZ,CHINOR,BLAM,CTERM,BTERM,FACTOR,SMU,WLU,BGAM,DD,DSPACE,QQUU,GAM1,GAM2,QX,QY,QZ,PX,PY,PZ,PNOR,QNOR
!
!     HOE STUFF
...
!
!
   IF(surf_diffraction_flag(R_I) == 1.AND..NOT.DUM(R_I).AND.surf_grating_spacing(R_I).NE.0.0D0) THEN
!
!     GRATING PRE-CALCULATIONS (A REAL LINEAR GRATING)
!
!     WE HAVE A GRATING, THINGS NEED TO BE CALCULATED
!     CALCULATE THE DIRCTION COSINES OF THE UNIT VECTOR NORMAL
!     TO THE GENERATING PLANES
!     MAKE SURE ALL DIRCOS NOT ZERO
      IF(GRX.EQ.0.0D0.AND.GRY.EQ.0.0D0 .AND.GRZ.EQ.0.0D0) GRY=1.0D0
!
      CHINOR=DSQRT((GRX**2)+(GRY**2)+(GRZ**2))
      CHIX=GRX/CHINOR
      CHIY=GRY/CHINOR
      CHIZ=GRZ/CHINOR
!     CALCULATE THE COMPONENTS OF THE VECTOR Q WHICH IS PARALLEL
!     TO THE RULINGS
      QX=-(CHIY*NN)+(CHIZ*MN)
      QY=-(CHIZ*LN)+(CHIX*NN)
      QZ=-(CHIX*MN)+(CHIY*LN)
      ...
      QY=QY/QNOR
      QZ=QZ/QNOR
!     CALCULATE THE COMPONENTS OF THE VECTOR P WHICH IS NORMAL
!     TO THE RULINGS
      PX=(QY*NN)-(QZ*MN)
      PY=(QZ*LN)-(QX*NN)
      PZ=(QX*MN)-(QY*LN)
...
!      WLU=WAVELENGTH IN MICRONS*(LENS UNITS/MICRON)
      IF(sys_units().EQ.1.0D0)WLU=SYSTEM(INT(WVN))*3.93700787402D-5
      IF(sys_units().EQ.2.0D0)WLU=SYSTEM(INT(WVN))*1.0D-4
      IF(sys_units().EQ.3.0D0)WLU=SYSTEM(INT(WVN))*1.0D-3
      IF(sys_units().EQ.4.0D0)WLU=SYSTEM(INT(WVN))*1.0D-6
!
      BLAM=-(GRO*WLU)/((DSPACE)*DABS(ALENS((WWVN),R_I)))
!
      BTERM=2.0D0*SMU*((R_L*LN)+(R_M*MN)+(R_N*NN))
      CTERM=(SMU**2)-1.0D0+(BLAM**2)-((2.0*SMU*BLAM)*((R_L*PX)+(R_M*PY)+(R_N*PZ)))
      DD=(BTERM**2)-(4.0D0*CTERM)
      IF(SIGNNU.LT.0.0D0) THEN
!
!                   WE HAVE A REFLECTION GRATING
!
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
               CALL SHOWIT(1)
               OUTLYNE='ANGLE OF DIFFRACTION AT THE GRATING IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=5
            RAYCOD(2)=R_I
...
!
      ELSE
!
!     WE HAVE A TRANSMISSION GRATING
!
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
...
!
      IF(surf_special_type(R_I) == 12) THEN
         ...
         MO=FTFL01(4,R_I)-R_Y
         NO=FTFL01(5,R_I)-R_Z
         IF(FTFL01(6,R_I).GT.0.0D0.AND.NO.GT.0.0D0.OR.FTFL01(6,R_I).LT.0.0D0.AND.NO.LT.0.0D0) THEN
            LO=-LO
            MO=-MO
            NO=-NO
         END IF
      ...
      IF(surf_special_type(R_I) == 13) THEN
         ...
         IF(FTFL01(6,R_I).LT.0.0D0) THEN
            NO=-NO
         END IF
         IF(FTFL01(7,R_I).GT.0.0D0.AND.NO.GT.0.0D0.OR.FTFL01(7,R_I).LT.0.0D0.AND.NO.LT.0.0D0) THEN
            LO=-LO
            MO=-MO
            NO=-NO
         END IF
...
!
      IF(surf_special_type(R_I) == 12) THEN
         ...
         MR=FTFL01(8,R_I)-R_Y
         NR=FTFL01(9,R_I)-R_Z
         IF(FTFL01(10,R_I).GT.0.0D0.AND.NR.GT.0.0D0.OR.FTFL01(10,R_I).LT.0.0D0.AND.NR.LT.0.0D0) THEN
            LR=-LR
            MR=-MR
            NR=-NR
         END IF
      ...
      IF(surf_special_type(R_I) == 13) THEN
         ...
         IF(FTFL01(11,R_I).LT.0.0D0) THEN
            NR=-NR
         END IF
         IF(FTFL01(12,R_I).GT.0.0D0.AND.NR.GT.0.0D0.OR.FTFL01(12,R_I).LT.0.0D0.AND.NR.LT.0.0D0) THEN
            LR=-LR
            MR=-MR
            NR=-NR
         END IF
...
!     BTA
      ...
      AY=BTA*(MO-MR)
      AZ=BTA*(NO-NR)
!     IS THERE PHASE FROM A TYPE 12 HOE TYPE SURFACE ?
      XX=R_X
      YY=R_Y
      MYI=R_I
      L1=0.0D0
      M1=0.0D0
      IF(FTFL01(11,R_I).EQ.1.0D0.OR.FTFL01(11,R_I).EQ.6.0D0)CALL PH12(L1,M1,XX,YY)
      IF(FTFL01(11,R_I).EQ.7.0D0) CALL PH11(L1,M1,XX,YY,PHA11)
      IF(FTFL01(11,R_I).EQ.2.0D0.OR.FTFL01(11,R_I).EQ.3.0D0.OR.FTFL01(11,R_I).EQ.4.0D0.OR.FTFL01(11,R_I).EQ.5.0D0)CALL PH13(L1,M1,XX,YY)
      GRX=-L1
      GRY=-M1
      GRZ=0.0D0
...
!     NOW CALC BIG GAMMA AND SMALL MU
!      WLU=WAVELENGTH IN MICRONS*(LENS UNITS/MICRON)
         IF(sys_units().EQ.1.0D0)WLU=FTFL01(2,R_I)*3.93700787402D-5
         IF(sys_units().EQ.2.0D0)WLU=FTFL01(2,R_I)*1.0D-4
         IF(sys_units().EQ.3.0D0)WLU=FTFL01(2,R_I)*1.0D-3
         IF(sys_units().EQ.4.0D0)WLU=FTFL01(2,R_I)*1.0D-6
!
         GRS=WLU/CHINOR
         IF(DABS(FACTOR).GT.1E-6) DSPACE=GRS/FACTOR
         IF(DABS(FACTOR).LE.1E-6) DSPACE=1.0D20
...
!
      BTERM=2.0D0*(SMU*((R_L*LN)+(R_M*MN)+(R_N*NN)))
      CTERM=((SMU**2)+((AX**2)+(AY**2)+(AZ**2)))+((BLAM**2)*((PX**2)+(PY**2)+(PZ**2)))-(2.0D0*SMU*((R_L*AX)+(R_M*AY)+(R_N*AZ)))-(2.0D0*SMU*BLAM*((R_L*PX)+(R_M*PY)+(R_N*PZ)))+(2.0D0*BLAM*((AX*PX)+(AY*PY)+(AZ*PZ)))-1.0D0
      DD=(BTERM**2)-(4.0D0*CTERM)
      IF(SIGNNU.LT.0.0D0) THEN
!     WE HAVE A REFLECTION HOE
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
               CALL SHOWIT(1)
               OUTLYNE='ANGLE OF DIFFRACTION AT THE HOE IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=9
            RAYCOD(2)=R_I
...
!       NO RAY ERROR
         ...
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
!     CASE OF A REFLECTION HOE, TAKE THE LARGER ROOT
         BGAM=GAM1
      ELSE
!     WE HAVE A TRANSMISSION HOE
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               WRITE(OUTLYNE,*)'RAY FAILURE OCCURRED AT SURFACE ',R_I
...
!       NO RAY ERROR
         END IF
         BGAM=CTERM/GAM1
!     NOT A HOE, NO PRECALCULATIONS
      END IF
   END IF
!     END OF HOE PRECALCULATOIONS
!     ***********************************************************************
!
!     NOW DEFLECT THE RAY
!
...
!       RE-NORMALIZE
            ...
            IF(surf_diffraction_flag(R_I) == 1.AND.surf_grating_spacing(R_I).NE.0.0D0 .OR.surf_special_type(R_I) == 12.OR.surf_special_type(R_I) == 13)CALL DPHASE
         END IF
...
!       NO TIR
            ...
            J=((DABS(ALENS((WWVN),R_I))*COSIP)-(DABS(ALENS((WWVN),(R_I-1)))*COSI))/(DABS(ALENS((WWVN),R_I)))
...
!     IF A PHASE SURFACE, FIX THE PHASE AND DIRCOS
   IF(surf_special_type(R_I) == 6.OR.surf_special_type(R_I) == 7 .OR.surf_special_type(R_I) == 9.OR.surf_special_type(R_I) == 10 .OR.surf_special_type(R_I) == 11.OR.surf_special_type(R_I) == 15) THEN
      R_L0=R_L
      R_M0=R_M
      R_N0=R_N
      RLRL=R_L
...
!     IF A TYPE 12 SURFACE, FIX THE PHASE AND DIRCOS
   IF(surf_special_type(R_I) == 12.AND.FTFL01(11,R_I).EQ.1.0D0.OR.surf_special_type(R_I) == 13.AND.FTFL01(11,R_I).EQ.1.0D0) THEN
      MYX=R_X
      MYY=R_Y
      MYI=R_I
      PHASE=0.0D0
      IF(FTFL01(11,R_I).EQ.1.0D0.OR.FTFL01(11,R_I).EQ.6.0D0)CALL PHAS12
      IF(FTFL01(11,R_I).EQ.2.0D0.OR.FTFL01(11,R_I).EQ.3.0D0.OR.FTFL01(11,R_I).EQ.4.0D0.OR.FTFL01(11,R_I).EQ.5.0D0)CALL PHAS13
...
!     LOAD LINEAR AND GAUSSIAN RANDOMS INTO REG 9 AND 10
         DOUB=RANDMM()
         RRR9=REG(9)
         RRR10=REG(10)
      END IF
