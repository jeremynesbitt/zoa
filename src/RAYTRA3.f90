!       THIS IS THE THIRD FILE OF RAYTRACING ROUTINES

! SUB NR4.FOR
SUBROUTINE NR4
!
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR4.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO A TYPE 17 USER-DEFINED SURFACE
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
   INTEGER KLI,II,AR
!
   LOGICAL FUN
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
!
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN10
!     FOR THE INITIAL USER FUCTION TO BE CALLED TO EVALUATE
!     THE Z VALUE CORRESPONDING TO THE Y AND X VALUES.
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN10 AND ANY MACRO FUNCTION CALLED BY FUN10.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 301 TO 348 AND ACCESSED BY THE USER VIA RCL COMMANDS
   GPREG(301)=FTFL01(1,R_I)
   GPREG(302)=FTFL01(2,R_I)
   GPREG(303)=FTFL01(3,R_I)
   GPREG(304)=FTFL01(4,R_I)
   GPREG(305)=FTFL01(5,R_I)
   GPREG(306)=FTFL01(6,R_I)
   GPREG(307)=FTFL01(7,R_I)
   GPREG(308)=FTFL01(8,R_I)
   GPREG(309)=FTFL01(9,R_I)
   GPREG(310)=FTFL01(10,R_I)
   GPREG(311)=FTFL01(11,R_I)
   GPREG(312)=FTFL01(12,R_I)
   GPREG(313)=FTFL01(13,R_I)
   GPREG(314)=FTFL01(14,R_I)
   GPREG(315)=FTFL01(15,R_I)
   GPREG(316)=FTFL01(16,R_I)
   GPREG(317)=FTFL01(17,R_I)
   GPREG(318)=FTFL01(18,R_I)
   GPREG(319)=FTFL01(19,R_I)
   GPREG(320)=FTFL01(20,R_I)
   GPREG(321)=FTFL01(21,R_I)
   GPREG(322)=FTFL01(22,R_I)
   GPREG(323)=FTFL01(23,R_I)
   GPREG(324)=FTFL01(24,R_I)
   GPREG(325)=FTFL01(25,R_I)
   GPREG(326)=FTFL01(26,R_I)
   GPREG(327)=FTFL01(27,R_I)
   GPREG(328)=FTFL01(28,R_I)
   GPREG(329)=FTFL01(29,R_I)
   GPREG(330)=FTFL01(30,R_I)
   GPREG(331)=FTFL01(31,R_I)
   GPREG(332)=FTFL01(32,R_I)
   GPREG(333)=FTFL01(33,R_I)
   GPREG(334)=FTFL01(34,R_I)
   GPREG(335)=FTFL01(35,R_I)
   GPREG(336)=FTFL01(36,R_I)
   GPREG(337)=FTFL01(37,R_I)
   GPREG(338)=FTFL01(38,R_I)
   GPREG(339)=FTFL01(39,R_I)
   GPREG(340)=FTFL01(40,R_I)
   GPREG(341)=FTFL01(41,R_I)
   GPREG(342)=FTFL01(42,R_I)
   GPREG(343)=FTFL01(43,R_I)
   GPREG(344)=FTFL01(44,R_I)
   GPREG(345)=FTFL01(45,R_I)
   GPREG(346)=FTFL01(46,R_I)
   GPREG(347)=FTFL01(47,R_I)
   GPREG(348)=FTFL01(48,R_I)
   GPREG(349)=FTFL01(49,R_I)
   GPREG(350)=FTFL01(50,R_I)
   GPREG(351)=FTFL01(51,R_I)
   GPREG(352)=FTFL01(52,R_I)
   GPREG(353)=FTFL01(53,R_I)
   GPREG(354)=FTFL01(54,R_I)
   GPREG(355)=FTFL01(55,R_I)
   GPREG(356)=FTFL01(56,R_I)
   GPREG(357)=FTFL01(57,R_I)
   GPREG(358)=FTFL01(58,R_I)
   GPREG(359)=FTFL01(59,R_I)
   GPREG(360)=FTFL01(60,R_I)
   GPREG(361)=FTFL01(61,R_I)
   GPREG(362)=FTFL01(62,R_I)
   GPREG(363)=FTFL01(63,R_I)
   GPREG(364)=FTFL01(64,R_I)
   GPREG(365)=FTFL01(65,R_I)
   GPREG(366)=FTFL01(66,R_I)
   GPREG(367)=FTFL01(67,R_I)
   GPREG(368)=FTFL01(68,R_I)
   GPREG(369)=FTFL01(69,R_I)
   GPREG(370)=FTFL01(70,R_I)
   GPREG(371)=FTFL01(71,R_I)
   GPREG(372)=FTFL01(72,R_I)
   GPREG(373)=FTFL01(73,R_I)
   GPREG(374)=FTFL01(74,R_I)
   GPREG(375)=FTFL01(75,R_I)
   GPREG(376)=FTFL01(76,R_I)
   GPREG(377)=FTFL01(77,R_I)
   GPREG(378)=FTFL01(78,R_I)
   GPREG(379)=FTFL01(79,R_I)
   GPREG(380)=FTFL01(80,R_I)
   GPREG(381)=FTFL01(81,R_I)
   GPREG(382)=FTFL01(82,R_I)
   GPREG(383)=FTFL01(83,R_I)
   GPREG(384)=FTFL01(84,R_I)
   GPREG(385)=FTFL01(85,R_I)
   GPREG(386)=FTFL01(86,R_I)
   GPREG(387)=FTFL01(87,R_I)
   GPREG(388)=FTFL01(88,R_I)
   GPREG(389)=FTFL01(89,R_I)
   GPREG(390)=FTFL01(90,R_I)
   GPREG(391)=FTFL01(91,R_I)
   GPREG(392)=FTFL01(92,R_I)
   GPREG(393)=FTFL01(93,R_I)
   GPREG(394)=FTFL01(94,R_I)
   GPREG(395)=FTFL01(95,R_I)
   GPREG(396)=FTFL01(96,R_I)
!     (FUN10 call removed - binary macro system removed)
   RETURN
END
! SUB NR1.FOR
SUBROUTINE NR1(ERRR)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR1.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC PLANO SURFACE
!
   EXTERNAL FNZ1,FUNC1X,FUNC1Y,DER1X,DER1Y
!
   INTEGER I
!
   LOGICAL DIFCAL,ERRR
!
   real(real64) C1,C2,C3,C4,MAG,ZCAL,ZZTOP,&
   &FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,C5,ACLENG,DEL,A34,FNZ1 &
   &,FUNC1X,FUNC1Y,ERR,SNGX,SNGY,SNGZ,DER1X,DER1Y &
   &,C41,C42,C43,C44,C45,XPASS,YPASS,ZPASS
   real(real64) LENG
   COMMON/PASSLENG/LENG
   real(real64) ZZEMAX,ZZEMIN
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   COMMON/ACLEN/ACLENG
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!


   PRINT *, "NR1 being called"

   IF(LN.NE.0.0D0) SNGX=LN/DABS(LN)
   IF(MN.NE.0.0D0) SNGY=MN/DABS(MN)
   IF(NN.NE.0.0D0) SNGZ=NN/DABS(NN)
   IF(LN.EQ.0.0D0) SNGX=1.0D0
   IF(MN.EQ.0.0D0) SNGY=1.0D0
   IF(NN.EQ.0.0D0) SNGZ=1.0D0
   DIFCAL=.FALSE.
   IF(surf_is_asphere(R_I)) DIFCAL=.TRUE.
   IF(surf_special_type(R_I) > 0.AND.&
   &ABS(surf_special_type(R_I)) /= 6.AND.&
   &ABS(surf_special_type(R_I)) /= 7.AND.&
   &ABS(surf_special_type(R_I)) /= 9.AND.&
   &ABS(surf_special_type(R_I)) /= 10.AND.&
   &ABS(surf_special_type(R_I)) /= 11.AND.&
   &ABS(surf_special_type(R_I)) /= 12.AND.&
   &ABS(surf_special_type(R_I)) /= 13.AND.&
   &ABS(surf_special_type(R_I)) /= 15.AND.&
   &ABS(surf_special_type(R_I)) /= 16.AND.&
   &ABS(surf_special_type(R_I)) /= 17.AND.&
   &ABS(surf_special_type(R_I)) /= 19.AND.&
   &ABS(surf_special_type(R_I)) /= 20.AND.&
   &ABS(surf_special_type(R_I)) /= 22.AND.&
   &ABS(surf_special_type(R_I)) /= 24 &
   &.OR.surf_default_flag(R_I) == 1) THEN
!     WE GOT A REAL SPECIAL SURFACE WHICH NEEDS FINITE DIFFERENCE
!     SURFACE SLOPE CALCS IF SURFACE IS TYPE 5 OR THE COEFS ARE NOT
!     ALL ZERO
      IF(surf_special_type(R_I) > 0) THEN
         IF(FTFL01(1,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(2,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(3,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(4,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(5,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(6,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(7,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(8,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(9,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(10,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(11,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(12,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(13,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(14,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(15,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(16,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(17,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(18,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(19,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(20,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(21,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(22,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(23,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(24,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(25,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(26,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(27,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(28,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(29,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(30,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(31,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(32,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(33,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(34,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(35,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(36,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(37,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(38,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(39,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(40,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(41,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(42,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(43,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(44,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(45,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(46,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(47,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(48,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(49,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(50,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(51,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(52,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(53,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(54,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(55,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(56,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(57,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(58,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(59,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(60,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(61,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(62,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(63,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(64,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(65,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(66,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(67,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(68,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(69,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(70,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(71,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(72,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(73,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(74,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(75,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(76,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(77,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(78,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(79,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(80,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(81,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(82,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(83,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(84,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(85,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(86,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(87,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(88,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(89,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(90,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(91,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(92,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(93,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(94,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(95,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(96,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(96,R_I).NE.0.0D0) DIFCAL=.TRUE.
      END IF
   END IF

!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   C5=surf_asphere_coeff(R_I, 2)
   A34=surf_special_type(R_I)
!
   DEL=DELSUR
!
!       THE RETURNED VALUES OF X,Y,Z ARE THE CORRECT INTERSECTIONS
!       TO WITHIN THE SURTOL VALUE
!
!       CALCULATE INTERSECTION
   DO 10 I=1,NRAITR
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ1 EVALUATES THE FUNCTION
!
      IF(DIFCAL) THEN
         ERRR=.FALSE.
         FNXP=-DER1X(FUNC1X,R_X,DELSUR,ERRR)
         FNYP=-DER1Y(FUNC1Y,R_Y,DELSUR,ERRR)
         IF(ERRR) RETURN
      ELSE
         FNXP=-(&
         &((1.0D0*2.0D0*R_X)*C5))
         FNYP=-(&
         &((1.0D0*2.0D0*R_Y)*C5))
      END IF
      FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
      MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
      LN=FNXP/MAG
      MN=FNYP/MAG
      NN=FNZP/MAG
!       THE INTERSECTION OF THE RAY WITH THIS TANGENT
!       PLANE
!       USING:
!
      ZCAL=FNZ1(R_X,R_Y,C1,C2,C3,C4,C5,R_I,A34,C41,C42,C43,C44,C45)
      HV1=(ZCAL-R_Z)
      HV2=((R_L*LN)+(R_M*MN)+(R_N*NN))
      HV=HV1/HV2
!       NEW X,Y AND Z ARE
      XP=R_X+(HV*R_L)
      YP=R_Y+(HV*R_M)
      ZP=R_Z+(HV*R_N)
!       SHOULD WE CONTINUE?
      ZZTOP=FNZ1(XP,YP,C1,C2,C3,C4,C5,R_I,A34,C41,C42,C43,C44,C45)
      LENG=DABS(ZZTOP-ZP)
      ACLENG=LENG
      IF(LENG.LE.SURTOL) THEN
         R_X=XP
         R_Y=YP
         R_Z=ZP
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ1 EVALUATES THE FUNCTION
!
         IF(DIFCAL) THEN
            ERRR=.FALSE.
            FNXP=-DER1X(FUNC1X,R_X,DELSUR,ERRR)
            FNYP=-DER1Y(FUNC1Y,R_Y,DELSUR,ERRR)
            IF(ERRR) RETURN
         ELSE
            FNXP=-(&
            &((1.0D0*2.0D0*R_X)*C5))
            FNYP=-(&
            &((1.0D0*2.0D0*R_Y)*C5))
         END IF
         FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
         MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
         LN=FNXP/MAG
         MN=FNYP/MAG
         NN=FNZP/MAG
         STOPP=0
         RETURN
      ELSE
!       KEEP TRYING
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
         R_X=XP
         R_Y=YP
         R_Z=ZP
      END IF
10 CONTINUE
   IF(MSG) THEN
      CALL RAY_FAILURE(R_I)
      OUTLYNE=&
      &'CONVERGENCE FAILURE OCCURRED DURING RAY INTERSECTION'
      CALL SHOWIT(1)
      OUTLYNE='WITH ASPHERIC, TORIC OR SPECIAL SURFACE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'ACCURACY OF INTERSECTION WAS = ',LENG
      CALL SHOWIT(1)
   END IF
   RAYCOD(1)=2
   RAYCOD(2)=R_I
   SPDCD1=RAYCOD(1)
   SPDCD2=R_I
   STOPP=1
   RETURN
END
FUNCTION FNZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AR,A34,AC41,AC42,AC43,&
&AC44,AC45)
   USE GLOBALS
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wavelength, sys_units, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   INTEGER IPASS1
!
   real(real64) FNZ1,AX,AY,AC1,AC2,AC3,AC4,AC5,AAC1,AAC2,AAC3 &
   &,AAC4,AAC5,AAX,AAY,FNZZ1,A34,R,THETA,FF2,FF3,FF4,XX,YY,FF5,AZ &
   &,AAAX,AAAY,AAC41,AAC42,AAC43,AAC44,AAC45,AC41,AC42,AC43,AC44,AC45
!
   real(real64) XPASS,YPASS,ZPASS,DA34
   real(real64) ZZEMAX,ZZEMIN
   real(real64) JK_WAVE
   real(real64) AMP1,OMEGA1X,OMEGA1Y
   real(real64) AMP2,OMEGA2X,OMEGA2Y
   real(real64) AMP3,OMEGA3X,OMEGA3Y
   real(real64) AMP4,OMEGA4X,OMEGA4Y
   real(real64) AMP5,OMEGA5X,OMEGA5Y
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   INTEGER KLI,II,AR,ISURF
!
   LOGICAL FUN,GERROR,GERROR1,GERROR2,UERROR
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!
!     AR IS THE SURFACE NUMBER
!
   INTEGER I
!
   FNZZ1(AAX,AAY,AAC1,AAC2,AAC3,AAC4,AAC5 &
   &,AAC41,AAC42,AAC43,AAC44,AAC45)=&
   &(((AAX**2)+(AAY**2))*AAC5)+&
   &((((AAX**2)+(AAY**2))**2)*AAC1)+&
   &((((AAX**2)+(AAY**2))**3)*AAC2)+&
   &((((AAX**2)+(AAY**2))**4)*AAC3)+&
   &((((AAX**2)+(AAY**2))**5)*AAC4)+&
   &((((AAX**2)+(AAY**2))**6)*AAC41)+&
   &((((AAX**2)+(AAY**2))**7)*AAC42)+&
   &((((AAX**2)+(AAY**2))**8)*AAC43)+&
   &((((AAX**2)+(AAY**2))**9)*AAC44)+&
   &((((AAX**2)+(AAY**2))**10)*AAC45)
!
!
   DA34=DABS(A34)
   IF(A34.EQ.0.0D0.AND.surf_default_flag(AR) == 0.OR.DA34.EQ.6.0D0.OR.&
   &DA34.EQ.7.0D0.OR.DA34.EQ.9.0D0.OR.DA34.EQ.10.0D0.OR.&
   &DA34.EQ.11.0D0.OR.DA34.EQ.12.0D0.OR.&
   &DA34.EQ.15.0D0.OR.DA34.EQ.16.0D0.OR.DA34.EQ.17.0D0.OR.&
   &DA34.EQ.19.0D0.OR.DA34.EQ.20.0D0) THEN
      FNZ1=FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   FNZ1=0.0D0
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+&
         &(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AC41,AC42,AC43,AC44 &
      &,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      JK_WAVE=sys_wavelength(INT(sys_wl_ref()))
      AMP1=DABS(FTFL01(1,AR)*JK_WAVE*0.5D0)
      AMP2=DABS(FTFL01(4,AR)*JK_WAVE*0.5D0)
      AMP3=DABS(FTFL01(7,AR)*JK_WAVE*0.5D0)
      AMP4=DABS(FTFL01(10,AR)*JK_WAVE*0.5D0)
      AMP5=DABS(FTFL01(13,AR)*JK_WAVE*0.5D0)
      IF(sys_units().EQ.1) AMP1=(AMP1*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP1=AMP1*1.0D-4
      IF(sys_units().EQ.3) AMP1=AMP1*1.0D-3
      IF(sys_units().EQ.4) AMP1=AMP1*1.0D-6
      IF(sys_units().EQ.1) AMP2=(AMP2*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP2=AMP2*1.0D-4
      IF(sys_units().EQ.3) AMP2=AMP2*1.0D-3
      IF(sys_units().EQ.4) AMP2=AMP2*1.0D-6
      IF(sys_units().EQ.1) AMP3=(AMP3*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP3=AMP3*1.0D-4
      IF(sys_units().EQ.3) AMP3=AMP3*1.0D-3
      IF(sys_units().EQ.4) AMP3=AMP3*1.0D-6
      IF(sys_units().EQ.1) AMP4=(AMP4*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP4=AMP4*1.0D-4
      IF(sys_units().EQ.3) AMP4=AMP4*1.0D-3
      IF(sys_units().EQ.4) AMP4=AMP4*1.0D-6
      IF(sys_units().EQ.1) AMP5=(AMP5*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP5=AMP5*1.0D-4
      IF(sys_units().EQ.3) AMP5=AMP5*1.0D-3
      IF(sys_units().EQ.4) AMP5=AMP5*1.0D-6
      AMP1=FTFL01(1,AR)/2.0D0
      AMP2=FTFL01(4,AR)/2.0D0
      AMP3=FTFL01(7,AR)/2.0D0
      AMP4=FTFL01(10,AR)/2.0D0
      AMP5=FTFL01(13,AR)/2.0D0
      IF(FTFL01(2,AR).EQ.0.0D0) THEN
         OMEGA1X=0.0D0
      ELSE
         OMEGA1X=TWOPII/DABS(FTFL01(2,AR))
      END IF
      IF(FTFL01(3,AR).EQ.0.0D0) THEN
         OMEGA1Y=0.0D0
      ELSE
         OMEGA1Y=TWOPII/DABS(FTFL01(3,AR))
      END IF
      IF(FTFL01(5,AR).EQ.0.0D0) THEN
         OMEGA2X=0.0D0
      ELSE
         OMEGA2X=TWOPII/DABS(FTFL01(5,AR))
      END IF
      IF(FTFL01(6,AR).EQ.0.0D0) THEN
         OMEGA2Y=0.0D0
      ELSE
         OMEGA2Y=TWOPII/DABS(FTFL01(6,AR))
      END IF
      IF(FTFL01(8,AR).EQ.0.0D0) THEN
         OMEGA3X=0.0D0
      ELSE
         OMEGA3X=TWOPII/DABS(FTFL01(8,AR))
      END IF
      IF(FTFL01(9,AR).EQ.0.0D0) THEN
         OMEGA3Y=0.0D0
      ELSE
         OMEGA3Y=TWOPII/DABS(FTFL01(9,AR))
      END IF
      IF(FTFL01(11,AR).EQ.0.0D0) THEN
         OMEGA4X=0.0D0
      ELSE
         OMEGA4X=TWOPII/DABS(FTFL01(11,AR))
      END IF
      IF(FTFL01(12,AR).EQ.0.0D0) THEN
         OMEGA4Y=0.0D0
      ELSE
         OMEGA4Y=TWOPII/DABS(FTFL01(12,AR))
      END IF
      IF(FTFL01(14,AR).EQ.0.0D0) THEN
         OMEGA5X=0.0D0
      ELSE
         OMEGA5X=TWOPII/DABS(FTFL01(14,AR))
      END IF
      IF(FTFL01(15,AR).EQ.0.0D0) THEN
         OMEGA5Y=0.0D0
      ELSE
         OMEGA5Y=TWOPII/DABS(FTFL01(15,AR))
      END IF
      FNZ1=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))&
      &+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))&
      &+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))&
      &+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))&
      &+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5,AC41,AC42,AC43,AC44 &
      &,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ1=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+&
         &(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ1=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+&
         &(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      INR=surf_inr_value(AR)
      AAAY=AY/INR
      AAAX=AX/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ1=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+&
         &(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     IF SPECIAL SURFACE TYPE 23
   IF(A34.EQ.23.0D0) THEN
!
      FNZ1=0.0D0
      IF(INT(FTFL01(1,AR)).LT.3.0D0) RETURN
      AZ=0.0D0
      IPASS1=3
      CALL SPL23(AR,AX,AY,AZ,IPASS1)
      FNZ1=AZ
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
   END IF
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      XX=AX
      YY=AY
      FNZ1=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ1=FNZ1+&
         &(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     lSPECIAL SURFACE TYPE 5
!     USER DEFINED SURFACE
   IF(A34.EQ.5.0D0) THEN
      REG(40)=REG(9)
      REG(9)=AX
      REG(10)=AY
      REG(12)=DBLE(I)
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN10
!     FOR THE INITIAL USER FUCTION TO BE CALLED TO EVALUATE
!     THE Z VALUE CORRESPONDING TO THE AY AND AX VALUES.
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN10 AND ANY MACRO FUNCTION CALLED BY FUN10.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 301 TO 348 AND ACCESSED BY THE USER VIA RCL COMMANDS
      GPREG(301)=FTFL01(1,AR)
      GPREG(302)=FTFL01(2,AR)
      GPREG(303)=FTFL01(3,AR)
      GPREG(304)=FTFL01(4,AR)
      GPREG(305)=FTFL01(5,AR)
      GPREG(306)=FTFL01(6,AR)
      GPREG(307)=FTFL01(7,AR)
      GPREG(308)=FTFL01(8,AR)
      GPREG(309)=FTFL01(9,AR)
      GPREG(310)=FTFL01(10,AR)
      GPREG(311)=FTFL01(11,AR)
      GPREG(312)=FTFL01(12,AR)
      GPREG(313)=FTFL01(13,AR)
      GPREG(314)=FTFL01(14,AR)
      GPREG(315)=FTFL01(15,AR)
      GPREG(316)=FTFL01(16,AR)
      GPREG(317)=FTFL01(17,AR)
      GPREG(318)=FTFL01(18,AR)
      GPREG(319)=FTFL01(19,AR)
      GPREG(320)=FTFL01(20,AR)
      GPREG(321)=FTFL01(21,AR)
      GPREG(322)=FTFL01(22,AR)
      GPREG(323)=FTFL01(23,AR)
      GPREG(324)=FTFL01(24,AR)
      GPREG(325)=FTFL01(25,AR)
      GPREG(326)=FTFL01(26,AR)
      GPREG(327)=FTFL01(27,AR)
      GPREG(328)=FTFL01(28,AR)
      GPREG(329)=FTFL01(29,AR)
      GPREG(330)=FTFL01(30,AR)
      GPREG(331)=FTFL01(31,AR)
      GPREG(332)=FTFL01(32,AR)
      GPREG(333)=FTFL01(33,AR)
      GPREG(334)=FTFL01(34,AR)
      GPREG(335)=FTFL01(35,AR)
      GPREG(336)=FTFL01(36,AR)
      GPREG(337)=FTFL01(37,AR)
      GPREG(338)=FTFL01(38,AR)
      GPREG(339)=FTFL01(39,AR)
      GPREG(340)=FTFL01(40,AR)
      GPREG(341)=FTFL01(41,AR)
      GPREG(342)=FTFL01(42,AR)
      GPREG(343)=FTFL01(43,AR)
      GPREG(344)=FTFL01(44,AR)
      GPREG(345)=FTFL01(45,AR)
      GPREG(346)=FTFL01(46,AR)
      GPREG(347)=FTFL01(47,AR)
      GPREG(348)=FTFL01(48,AR)
      GPREG(349)=FTFL01(49,AR)
      GPREG(350)=FTFL01(50,AR)
      GPREG(351)=FTFL01(51,AR)
      GPREG(352)=FTFL01(52,AR)
      GPREG(353)=FTFL01(53,AR)
      GPREG(354)=FTFL01(54,AR)
      GPREG(355)=FTFL01(55,AR)
      GPREG(356)=FTFL01(56,AR)
      GPREG(357)=FTFL01(57,AR)
      GPREG(358)=FTFL01(58,AR)
      GPREG(359)=FTFL01(59,AR)
      GPREG(360)=FTFL01(60,AR)
      GPREG(361)=FTFL01(61,AR)
      GPREG(362)=FTFL01(62,AR)
      GPREG(363)=FTFL01(63,AR)
      GPREG(364)=FTFL01(64,AR)
      GPREG(365)=FTFL01(65,AR)
      GPREG(366)=FTFL01(66,AR)
      GPREG(367)=FTFL01(67,AR)
      GPREG(368)=FTFL01(68,AR)
      GPREG(369)=FTFL01(69,AR)
      GPREG(370)=FTFL01(70,AR)
      GPREG(371)=FTFL01(71,AR)
      GPREG(372)=FTFL01(72,AR)
      GPREG(373)=FTFL01(73,AR)
      GPREG(374)=FTFL01(74,AR)
      GPREG(375)=FTFL01(75,AR)
      GPREG(376)=FTFL01(76,AR)
      GPREG(377)=FTFL01(77,AR)
      GPREG(378)=FTFL01(78,AR)
      GPREG(379)=FTFL01(79,AR)
      GPREG(380)=FTFL01(80,AR)
      GPREG(381)=FTFL01(81,AR)
      GPREG(382)=FTFL01(82,AR)
      GPREG(383)=FTFL01(83,AR)
      GPREG(384)=FTFL01(84,AR)
      GPREG(385)=FTFL01(85,AR)
      GPREG(386)=FTFL01(86,AR)
      GPREG(387)=FTFL01(87,AR)
      GPREG(388)=FTFL01(88,AR)
      GPREG(389)=FTFL01(89,AR)
      GPREG(390)=FTFL01(90,AR)
      GPREG(391)=FTFL01(91,AR)
      GPREG(392)=FTFL01(92,AR)
      GPREG(393)=FTFL01(93,AR)
      GPREG(394)=FTFL01(94,AR)
      GPREG(395)=FTFL01(95,AR)
      GPREG(396)=FTFL01(96,AR)
!     (FUN10 call removed - binary macro system removed)
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   IF(A34.EQ.21.0D0) THEN
      CALL USERSURF(AR,AX,AY,FNZ1,UERROR)
      IF(UERROR) THEN
         UERROR=.FALSE.
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
      FNZ1=FNZ1+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 22
!     GRID SAG
   IF(A34.EQ.22.0D0) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL GRID
!     TYPE 22 SAG GRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      CALL GRIDS(4,ISURF,GERROR)
      IF(.NOT.GERROR) GRIDSUNLOADED22(ISURF)=.FALSE.
      IF(GERROR) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'GRID FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=15
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!      IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!      IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ1=ZPASS+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     DEFORM SURFACE
   IF(surf_default_flag(AR) == 1) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL DEFGRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      IPASS1=4
      CALL DEFGRIDS(4,ISURF,GERROR1,GERROR2)
      IF(GERROR1) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'DEFORM FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=17
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         IF(GERROR2) THEN
            IF(MSG) THEN
               CALL RAY_FAILURE(ISURF)
               OUTLYNE=&
               &'RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
               CALL SHOWIT(1)
            END IF
            STOPP=1
            RAYCOD(1)=18
            RAYCOD(2)=ISURF
            RAYEXT=.FALSE.
            POLEXT=.FALSE.
            SPDCD1=RAYCOD(1)
            SPDCD2=ISURF
            RETURN
         END IF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!      IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!      IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ1=ZPASS+FNZZ1(AX,AY,AC1,AC2,AC3,AC4,AC5 &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   RETURN
END
! SUB NR2.FOR
SUBROUTINE NR2(ERRR)

   USE Globals
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   LOGICAL DIFCAL,ERRR
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NR2.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC/CONIC SURFACE
!
   EXTERNAL FNZ2
   EXTERNAL FUNC2X,FUNC2Y,DER2X,DER2Y
!
   real(real64) C1,C2,C3,C4,ACLENG,FNZ2,DEL,SNGX,SNGY,SNGZ,&
   &FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,C41,C42,C43,C44,C45,&
   &MAG,ZCAL,Q,C5,C6,ZZTOP,A34,ZTEST,CV,CC,DER2X,DER2Y,&
   &FUNC2X,FUNC2Y,ERR,XPASS,YPASS,ZPASS
   real(real64) LENG
   COMMON/PASSLENG/LENG
   real(real64) ZZEMAX,ZZEMIN
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   COMMON/ACLEN/ACLENG
!
   INTEGER I
!
!
   !PRINT *, "NR2 Being called"
   !call logger%logTextWithNum("surf_is_asphere(R_I) = ", surf_is_asphere(R_I))
   IF(LN.NE.0.0D0) SNGX=LN/DABS(LN)
   IF(MN.NE.0.0D0) SNGY=MN/DABS(MN)
   IF(NN.NE.0.0D0) SNGZ=NN/DABS(NN)
   IF(LN.EQ.0.0D0) SNGX=1.0D0
   IF(MN.EQ.0.0D0) SNGY=1.0D0
   IF(NN.EQ.0.0D0) SNGZ=1.0D0
   DIFCAL=.FALSE.
   IF(surf_is_asphere(R_I)) DIFCAL=.TRUE.
   IF(surf_special_type(R_I) > 0.AND.&
   &ABS(surf_special_type(R_I)) /= 6.AND.&
   &ABS(surf_special_type(R_I)) /= 7.AND.&
   &ABS(surf_special_type(R_I)) /= 9.AND.&
   &ABS(surf_special_type(R_I)) /= 10.AND.&
   &ABS(surf_special_type(R_I)) /= 11.AND.&
   &ABS(surf_special_type(R_I)) /= 12.AND.&
   &ABS(surf_special_type(R_I)) /= 13.AND.&
   &ABS(surf_special_type(R_I)) /= 15.AND.&
   &ABS(surf_special_type(R_I)) /= 16.AND.&
   &ABS(surf_special_type(R_I)) /= 17.AND.&
   &ABS(surf_special_type(R_I)) /= 19.AND.&
   &ABS(surf_special_type(R_I)) /= 20.AND.&
   &ABS(surf_special_type(R_I)) /= 22.AND.&
   &ABS(surf_special_type(R_I)) /= 24 &
   &.OR.surf_default_flag(R_I) == 1) THEN
!     WE GOT A REAL SPECIAL SURFACE WHICH NEEDS FINIT DIFFERENCE
!     SURFACE SLOPE CALCS IF SURFACE IS TYPE 5 OR THE COEFS ARE NOT
!     ALL ZERO
      IF(surf_special_type(R_I) > 0) THEN

         IF(FTFL01(1,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(2,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(3,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(4,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(5,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(6,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(7,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(8,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(9,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(10,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(11,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(12,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(13,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(14,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(15,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(16,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(17,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(18,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(19,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(20,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(21,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(22,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(23,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(24,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(25,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(26,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(27,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(28,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(29,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(30,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(31,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(32,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(33,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(34,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(35,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(36,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(37,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(38,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(39,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(40,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(41,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(42,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(43,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(44,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(45,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(46,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(47,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(48,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(49,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(50,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(51,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(52,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(53,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(54,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(55,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(56,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(57,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(58,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(59,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(60,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(61,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(62,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(63,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(64,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(65,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(66,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(67,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(68,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(69,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(70,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(71,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(72,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(73,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(74,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(75,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(76,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(77,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(78,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(79,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(80,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(81,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(82,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(83,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(84,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(85,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(86,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(87,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(88,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(89,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(90,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(91,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(92,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(93,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(94,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(95,R_I).NE.0.0D0) DIFCAL=.TRUE.
         IF(FTFL01(96,R_I).NE.0.0D0) DIFCAL=.TRUE.
      END IF
   END IF
   A34=surf_special_type(R_I)
!
   CV=surf_curvature(R_I)
   CC=surf_conic(R_I)
   IF(CC.EQ.-1.0D0.AND.CV.GT.0.0D0) ZTEST=1.0D20
   IF(CC.EQ.-1.0D0.AND.CV.LT.0.0D0) ZTEST=-1.0D20
   IF(CC.NE.-1.0D0) ZTEST=1.0D0/((CC+1.0D0)*CV)
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   C5=surf_conic(R_I)
   C6=(surf_curvature(R_I))


   !IF(DIFCAL) call logger%logText("DIFCAL ON")
!
!       NOW REFINE TO INTERSECT WITH THE ASPHERIC
!
!       THE SAG EQUATION IS:
!
!       THE RETURNED VALUES OF X,Y,Z ARE THE CORRECT INTERSECTIONS
!       TO WITHIN THE SURTOL VALUE
!
!       C5=CONIC CONSTANT
!       C6=CURVATURE
!       C1 TO C4 ARE THE 4TH, 6TH, 8TH AND 10TH ORDER ASPHERIC
!       DEFORMATION TERMS
!       C41 TO C45 ARE THE 12TH THROUGH 20TH ORDER ASPHERIC
!       DEFORMATION TERMS
!
   !call logger%logTextWithNum("NR2 Start R_X ", R_X)
   !call logger%logTextWithNum("NR2 Start R_Y ", R_Y)

!       CALCULATE INTERSECTION

   DO 10 I=1,NRAITR
!
      Q=(1.0D0-((C5+1.0D0)*(C6**2)*&
      &(((R_X**2)+(R_Y**2)))))
      IF(Q.LT.0.0D0) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(R_I)
            OUTLYNE='RAY DID NOT INTERSECT THE SURFACE'
            CALL SHOWIT(1)
         END IF
         RAYCOD(1)=1
         RAYCOD(2)=R_I
         SPDCD1=RAYCOD(1)
         SPDCD2=R_I
         STOPP=1
         RETURN
      END IF
      Q=DSQRT(Q)
      Q=1.0D0+Q
      DEL=DELSUR
      !call logger%logTextWithNum("NR2 Iter ", I)
      !call logger%logTextWithNum("Q = ", Q)
      !call logger%logTextWithNum("C5 = ", C5)
      !call logger%logTextWithNum("C6 = ", C6)



!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ2 EVALUATES THE FUNCTION
!
      IF(DIFCAL) THEN
         ERRR=.FALSE.
         FNXP=-DER2X(FUNC2X,R_X,DELSUR,ERRR)
         FNYP=-DER2Y(FUNC2Y,R_Y,DELSUR,ERRR)
         !call logger%logTextWithNum("FNXP is ", FNXP)
         !call logger%logTextWithNum("FNYP is ", FNYP)

         IF(ERRR) RETURN
      ELSE
         FNXP=-(&
         &(((2.0D0*C6*R_X*Q*(Q-1.0D0))+&
         &((C6**3)*R_X*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2)))&
         &)
         FNYP=-(&
         &(((2.0D0*C6*R_Y*Q*(Q-1.0D0))+&
         &((C6**3)*R_Y*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2)))&
         &)
      END IF
      FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
      MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
      LN=FNXP/MAG
      MN=FNYP/MAG
      NN=FNZP/MAG
!
!       THESE ARE THE DIRECTION COSINES FOR THE SURFACE NORMAL
!       FOR THE CURRENT X,Y AND Z
!
!       IF X,Y AND Z ARE THE CORRECT X,Y AND Z THEN
!       ZCAL WILL EQUAL Z SO THAT F(X,Y,Z)=0
      ZCAL=FNZ2(R_X,R_Y,C1,C2,C3,C4,C6,R_I,A34,Q &
      &,C41,C42,C43,C44,C45)
      HV1=ZCAL-R_Z
      HV2=((R_L*LN)+(R_M*MN)+(R_N*NN))
      HV=HV1/HV2
      !call logger%logTextWithNum("Aspheric correction term is", HV)
!       NEW X,Y AND Z ARE
      XP=R_X+(HV*R_L)
      YP=R_Y+(HV*R_M)
      ZP=R_Z+(HV*R_N)
!       SHOULD WE CONTINUE?
      ZZTOP=FNZ2(XP,YP,C1,C2,C3,C4,C6,R_I,A34,Q &
      &,C41,C42,C43,C44,C45)
      LENG=DABS(ZZTOP-ZP)
      ACLENG=LENG
      IF(LENG.LE.SURTOL) THEN
         R_X=XP
         R_Y=YP
         R_Z=ZP
         Q=(1.0D0-((C5+1.0D0)*(C6**2)*&
         &((R_X**2)+(R_Y**2))))
         IF(Q.LT.0.0D0) THEN
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE='RAY DID NOT INTERSECT THE SURFACE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=1
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
         END IF
         Q=DSQRT(Q)
         Q=1.0D0+Q
         !call logger%logTextWithNum("Aspheric R_Y  is", R_Y)
         !call logger%logTextWithNum("Aspheric Q term is", Q)
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ2 EVALUATES THE FUNCTION
!
         IF(DIFCAL) THEN
            ERRR=.FALSE.
            FNXP=-DER2X(FUNC2X,R_X,DELSUR,ERRR)
            FNYP=-DER2Y(FUNC2Y,R_Y,DELSUR,ERRR)
            !call logger%logTextWithNum("FNXP is ", FNXP)
            !call logger%logTextWithNum("FNYP is ", FNYP)
            IF(ERRR) RETURN
         ELSE
            FNXP=-(&
            &(((2.0D0*C6*R_X*Q*(Q-1.0D0))+&
            &((C6**3)*R_X*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2)))&
            &)
            FNYP=-(&
            &(((2.0D0*C6*R_Y*Q*(Q-1.0D0))+&
            &((C6**3)*R_Y*((R_X**2)+(R_Y**2))*(C5+1.0D0)))/((Q-1.0D0)*(Q**2)))&
            &)
         END IF
         FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
         MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
         LN=FNXP/MAG
         MN=FNYP/MAG
         NN=FNZP/MAG
         STOPP=0
         !call logger%logTextWithNum("Mag is ", MAG)
         RETURN
      ELSE
!       KEEP TRYING
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
         R_X=XP
         R_Y=YP
         R_Z=ZP
      END IF
10 CONTINUE
   IF(MSG) THEN
      CALL RAY_FAILURE(R_I)
      OUTLYNE=&
      &'CONVERGENCE FAILURE OCCURRED DURING RAY INTERSECTION'
      CALL SHOWIT(1)
      OUTLYNE='WITH ASPHERIC, TORIC OR SPECIAL SURFACE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'ACCURACY OF INTERSECTION WAS = ',LENG
      CALL SHOWIT(1)
   END IF
   RAYCOD(1)=2
   RAYCOD(2)=R_I
   SPDCD1=RAYCOD(1)
   SPDCD2=R_I
   R_X=XP
   R_Y=YP
   R_Z=ZP
   STOPP=0
   RETURN
END
FUNCTION FNZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,AR,A34,Q &
&,AC41,AC42,AC43,AC44,AC45)
   USE GLOBALS
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wavelength, sys_units, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   real(real64) FNZ2,AX,AY,AC1,AC2,AC3,AC4,AAC1,AAC2,AAC3 &
   &,AAC4,AAX,AAY,FNZZ2,A34,R,THETA,FF2,FF3,AQ,Q,AC6,FF4,XX,YY,&
   &AAC6,FF5,AAAX,AAAY,AC41,AC42,AC43,AC44,AC45,AAC41,AAC42,AAC43 &
   &,AAC44,AAC45,AZ
!
   INTEGER IPASS1
!
   real(real64) T1,T2,T3,T4,T5,T6,T7,T8,T9,T10
   real(real64) XPASS,YPASS,ZPASS
   real(real64) ZZEMAX,ZZEMIN,DA34
   real(real64) JK_WAVE
   real(real64) AMP1,OMEGA1X,OMEGA1Y
   real(real64) AMP2,OMEGA2X,OMEGA2Y
   real(real64) AMP3,OMEGA3X,OMEGA3Y
   real(real64) AMP4,OMEGA4X,OMEGA4Y
   real(real64) AMP5,OMEGA5X,OMEGA5Y
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
   INTEGER KLI,II,AR,ISURF
!
   LOGICAL FUN,GERROR,GERROR1,GERROR2,UERROR
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
!
   INTEGER I
!
   FNZZ2(AAX,AAY,AAC1,AAC2,AAC3,AAC4,AAC6,AQ,&
   &AAC41,AAC42,AAC43,AAC44,AAC45)=&
   &((((AAX**2)+(AAY**2))**2)*AAC1)+&
   &((((AAX**2)+(AAY**2))**3)*AAC2)+&
   &((((AAX**2)+(AAY**2))**4)*AAC3)+&
   &((((AAX**2)+(AAY**2))**5)*AAC4)+&
   &((((AAX**2)+(AAY**2))**6)*AAC41)+&
   &((((AAX**2)+(AAY**2))**7)*AAC42)+&
   &((((AAX**2)+(AAY**2))**8)*AAC43)+&
   &((((AAX**2)+(AAY**2))**9)*AAC44)+&
   &((((AAX**2)+(AAY**2))**10)*AAC45)+&
   &((AAC6*((AAX**2)+(AAY**2)))/AQ)

   !FUNCTION FNZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,AR,A34,Q
   !AC41,AC42,AC43,AC44,AC45)

   ! call logger%logTextWithNum("FNZ2 AAC1=",AC1)
   ! call logger%logTextWithNum("FNZ2 AAC2=",AC2)
   ! call logger%logTextWithNum("FNZ2 AAC3=",AC3)
   ! call logger%logTextWithNum("FNZ2 AAC4=",AC4)
   ! call logger%logTextWithNum("FNZ2 AAC41=",AC41)
   ! call logger%logTextWithNum("FNZ2 AAC42=",AC42)
   ! call logger%logTextWithNum("FNZ2 AAC43=",AC43)
   ! call logger%logTextWithNum("FNZ2 AAC44=",AC44)
   ! call logger%logTextWithNum("FNZ2 AAC45=",AC45)
   ! call logger%logTextWithNum("FNZ2 AAX=",AX)
   ! call logger%logTextWithNum("FNZ2 AAY=",AY)
   ! call logger%logTextWithNum("FNZ2 AAC6=",AC6)
   ! call logger%logTextWithNum("FNZ2 AQ=",Q)
   !
   ! PRINT *, "TroubleTermis? ", ((((AX**2)+(AY**2))**6)*AC41)
   ! PRINT *, "Term w/o Asphere ",(((AX**2)+(AY**2))**6)
   ! T1 = ((((AX**2)+(AY**2))**2)*AC1)
   ! T2 = ((((AX**2)+(AY**2))**3)*AC2)
   ! T3 = ((((AX**2)+(AY**2))**4)*AC3)
   ! T4 = ((((AX**2)+(AY**2))**5)*AC4)
   ! T5 = ((((AX**2)+(AY**2))**6)*AC41)
   ! T6 = ((((AX**2)+(AY**2))**7)*AC42)
   ! T7 = ((((AX**2)+(AY**2))**8)*AC43)
   ! T8 = ((((AX**2)+(AY**2))**9)*AC44)
   ! T9 = ((((AX**2)+(AY**2))**10)*AC45)
   ! T10 = ((AC6*((AX**2)+(AY**2)))/Q)
   !
   ! PRINT *, "Total is ", T1+T2+T3+T4+T5+T6+T7+T8+T9+T10
   !
   ! call logger%logTextWithNum("T1=",T1)
   ! call logger%logTextWithNum("T2=",T2)
   ! call logger%logTextWithNum("T3=",T3)
   ! call logger%logTextWithNum("T4=",T4)
   ! call logger%logTextWithNum("T5=",T5)
   ! call logger%logTextWithNum("T6=",T6)
   ! call logger%logTextWithNum("T7=",T7)
   ! call logger%logTextWithNum("T8=",T8)
   ! call logger%logTextWithNum("T9=",T9)
   ! call logger%logTextWithNum("T10=",T10)
   !
   ! call logger%logTextWithNum("FNZ2 AAC3=",AC3)
   ! call logger%logTextWithNum("FNZ2 AAC4=",AC4)
   ! call logger%logTextWithNum("FNZ2 AAC41=",AC41)
   ! call logger%logTextWithNum("FNZ2 AAC42=",AC42)
   ! call logger%logTextWithNum("FNZ2 AAC43=",AC43)
   ! call logger%logTextWithNum("FNZ2 AAC44=",AC44)
   ! call logger%logTextWithNum("FNZ2 AAC45=",AC45)
   ! call logger%logTextWithNum("FNZ2 AAX=",AX)
   ! call logger%logTextWithNum("FNZ2 AAY=",AY)
   ! call logger%logTextWithNum("FNZ2 AAC6=",AC6)
   ! call logger%logTextWithNum("FNZ2 AQ=",Q)

   !call logger%logTextWithNum("FNZ2 Result=",FNZZ2)

!
!     IF SPECIAL SURFACE TYPE 1, 2 OR 3 ARE TO BE APPLIED
!     THEY ARE APPLIED HERE
!
   DA34=DABS(A34)
   IF(A34.EQ.0.0D0.AND.surf_default_flag(AR) == 0.OR.DA34.EQ.6.0D0.OR.&
   &DA34.EQ.7.0D0.OR.DA34.EQ.9.0D0.OR.DA34.EQ.10.0D0.OR.&
   &DA34.EQ.11.0D0.OR.DA34.EQ.12.0D0.OR.&
   &DA34.EQ.15.0D0.OR.DA34.EQ.16.0D0.OR.DA34.EQ.17.0D0.OR.&
   &DA34.EQ.19.0D0.OR.DA34.EQ.20.0D0) THEN
      FNZ2=FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   FNZ2=0.0D0
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+&
         &(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      JK_WAVE=sys_wavelength(INT(sys_wl_ref()))
      AMP1=DABS(FTFL01(1,AR)*JK_WAVE*0.5D0)
      AMP2=DABS(FTFL01(4,AR)*JK_WAVE*0.5D0)
      AMP3=DABS(FTFL01(7,AR)*JK_WAVE*0.5D0)
      AMP4=DABS(FTFL01(10,AR)*JK_WAVE*0.5D0)
      AMP5=DABS(FTFL01(13,AR)*JK_WAVE*0.5D0)
      IF(sys_units().EQ.1) AMP1=(AMP1*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP1=AMP1*1.0D-4
      IF(sys_units().EQ.3) AMP1=AMP1*1.0D-3
      IF(sys_units().EQ.4) AMP1=AMP1*1.0D-6
      IF(sys_units().EQ.1) AMP2=(AMP2*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP2=AMP2*1.0D-4
      IF(sys_units().EQ.3) AMP2=AMP2*1.0D-3
      IF(sys_units().EQ.4) AMP2=AMP2*1.0D-6
      IF(sys_units().EQ.1) AMP3=(AMP3*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP3=AMP3*1.0D-4
      IF(sys_units().EQ.3) AMP3=AMP3*1.0D-3
      IF(sys_units().EQ.4) AMP3=AMP3*1.0D-6
      IF(sys_units().EQ.1) AMP4=(AMP4*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP4=AMP4*1.0D-4
      IF(sys_units().EQ.3) AMP4=AMP4*1.0D-3
      IF(sys_units().EQ.4) AMP4=AMP4*1.0D-6
      IF(sys_units().EQ.1) AMP5=(AMP5*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP5=AMP5*1.0D-4
      IF(sys_units().EQ.3) AMP5=AMP5*1.0D-3
      IF(sys_units().EQ.4) AMP5=AMP5*1.0D-6
      AMP1=FTFL01(1,AR)
      AMP2=FTFL01(4,AR)
      AMP3=FTFL01(7,AR)
      AMP4=FTFL01(10,AR)
      AMP5=FTFL01(13,AR)
      IF(FTFL01(2,AR).EQ.0.0D0) THEN
         OMEGA1X=0.0D0
      ELSE
         OMEGA1X=TWOPII/DABS(FTFL01(2,AR))
      END IF
      IF(FTFL01(3,AR).EQ.0.0D0) THEN
         OMEGA1Y=0.0D0
      ELSE
         OMEGA1Y=TWOPII/DABS(FTFL01(3,AR))
      END IF
      IF(FTFL01(5,AR).EQ.0.0D0) THEN
         OMEGA2X=0.0D0
      ELSE
         OMEGA2X=TWOPII/DABS(FTFL01(5,AR))
      END IF
      IF(FTFL01(6,AR).EQ.0.0D0) THEN
         OMEGA2Y=0.0D0
      ELSE
         OMEGA2Y=TWOPII/DABS(FTFL01(6,AR))
      END IF
      IF(FTFL01(8,AR).EQ.0.0D0) THEN
         OMEGA3X=0.0D0
      ELSE
         OMEGA3X=TWOPII/DABS(FTFL01(8,AR))
      END IF
      IF(FTFL01(9,AR).EQ.0.0D0) THEN
         OMEGA3Y=0.0D0
      ELSE
         OMEGA3Y=TWOPII/DABS(FTFL01(9,AR))
      END IF
      IF(FTFL01(11,AR).EQ.0.0D0) THEN
         OMEGA4X=0.0D0
      ELSE
         OMEGA4X=TWOPII/DABS(FTFL01(11,AR))
      END IF
      IF(FTFL01(12,AR).EQ.0.0D0) THEN
         OMEGA4Y=0.0D0
      ELSE
         OMEGA4Y=TWOPII/DABS(FTFL01(12,AR))
      END IF
      IF(FTFL01(14,AR).EQ.0.0D0) THEN
         OMEGA5X=0.0D0
      ELSE
         OMEGA5X=TWOPII/DABS(FTFL01(14,AR))
      END IF
      IF(FTFL01(15,AR).EQ.0.0D0) THEN
         OMEGA5Y=0.0D0
      ELSE
         OMEGA5Y=TWOPII/DABS(FTFL01(15,AR))
      END IF
      FNZ2=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))&
      &+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))&
      &+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))&
      &+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))&
      &+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ2=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+&
         &(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ2=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+&
         &(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ2=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+&
         &(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     IF SPECIAL SURFACE TYPE 23
   IF(A34.EQ.23.0D0) THEN
!
      FNZ2=0.0D0
      IF(INT(FTFL01(1,AR)).LT.3.0D0) RETURN
      AZ=0.0D0
      IPASS1=3
      CALL SPL23(AR,AX,AY,AZ,IPASS1)
      FNZ2=AZ
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
   END IF
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      XX=AX
      YY=AY
      FNZ2=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ2=FNZ2+&
         &(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 5
!     USER DEFINED SURFACE
   IF(A34.EQ.5.0D0) THEN
      REG(40)=REG(9)
      REG(9)=AX
      REG(10)=AY
      REG(12)=DBLE(I)
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN10
!     FOR THE INITIAL USER FUCTION TO BE CALLED TO EVALUATE
!     THE Z VALUE CORRESPONDING TO THE AY AND AX VALUES.
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN10 AND ANY MACRO FUNCTION CALLED BY FUN10.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 301 TO 396 AND ACCESSED BY THE USER VIA RCL COMMANDS
      GPREG(301)=FTFL01(1,AR)
      GPREG(302)=FTFL01(2,AR)
      GPREG(303)=FTFL01(3,AR)
      GPREG(304)=FTFL01(4,AR)
      GPREG(305)=FTFL01(5,AR)
      GPREG(306)=FTFL01(6,AR)
      GPREG(307)=FTFL01(7,AR)
      GPREG(308)=FTFL01(8,AR)
      GPREG(309)=FTFL01(9,AR)
      GPREG(310)=FTFL01(10,AR)
      GPREG(311)=FTFL01(11,AR)
      GPREG(312)=FTFL01(12,AR)
      GPREG(313)=FTFL01(13,AR)
      GPREG(314)=FTFL01(14,AR)
      GPREG(315)=FTFL01(15,AR)
      GPREG(316)=FTFL01(16,AR)
      GPREG(317)=FTFL01(17,AR)
      GPREG(318)=FTFL01(18,AR)
      GPREG(319)=FTFL01(19,AR)
      GPREG(320)=FTFL01(20,AR)
      GPREG(321)=FTFL01(21,AR)
      GPREG(322)=FTFL01(22,AR)
      GPREG(323)=FTFL01(23,AR)
      GPREG(324)=FTFL01(24,AR)
      GPREG(325)=FTFL01(25,AR)
      GPREG(326)=FTFL01(26,AR)
      GPREG(327)=FTFL01(27,AR)
      GPREG(328)=FTFL01(28,AR)
      GPREG(329)=FTFL01(29,AR)
      GPREG(330)=FTFL01(30,AR)
      GPREG(331)=FTFL01(31,AR)
      GPREG(332)=FTFL01(32,AR)
      GPREG(333)=FTFL01(33,AR)
      GPREG(334)=FTFL01(34,AR)
      GPREG(335)=FTFL01(35,AR)
      GPREG(336)=FTFL01(36,AR)
      GPREG(337)=FTFL01(37,AR)
      GPREG(338)=FTFL01(38,AR)
      GPREG(339)=FTFL01(39,AR)
      GPREG(340)=FTFL01(40,AR)
      GPREG(341)=FTFL01(41,AR)
      GPREG(342)=FTFL01(42,AR)
      GPREG(343)=FTFL01(43,AR)
      GPREG(344)=FTFL01(44,AR)
      GPREG(345)=FTFL01(45,AR)
      GPREG(346)=FTFL01(46,AR)
      GPREG(347)=FTFL01(47,AR)
      GPREG(348)=FTFL01(48,AR)
      GPREG(349)=FTFL01(49,AR)
      GPREG(350)=FTFL01(50,AR)
      GPREG(351)=FTFL01(51,AR)
      GPREG(352)=FTFL01(52,AR)
      GPREG(353)=FTFL01(53,AR)
      GPREG(354)=FTFL01(54,AR)
      GPREG(355)=FTFL01(55,AR)
      GPREG(356)=FTFL01(56,AR)
      GPREG(357)=FTFL01(57,AR)
      GPREG(358)=FTFL01(58,AR)
      GPREG(359)=FTFL01(59,AR)
      GPREG(360)=FTFL01(60,AR)
      GPREG(361)=FTFL01(61,AR)
      GPREG(362)=FTFL01(62,AR)
      GPREG(363)=FTFL01(63,AR)
      GPREG(364)=FTFL01(64,AR)
      GPREG(365)=FTFL01(65,AR)
      GPREG(366)=FTFL01(66,AR)
      GPREG(367)=FTFL01(67,AR)
      GPREG(368)=FTFL01(68,AR)
      GPREG(369)=FTFL01(69,AR)
      GPREG(370)=FTFL01(70,AR)
      GPREG(371)=FTFL01(71,AR)
      GPREG(372)=FTFL01(72,AR)
      GPREG(373)=FTFL01(73,AR)
      GPREG(374)=FTFL01(74,AR)
      GPREG(375)=FTFL01(75,AR)
      GPREG(376)=FTFL01(76,AR)
      GPREG(377)=FTFL01(77,AR)
      GPREG(378)=FTFL01(78,AR)
      GPREG(379)=FTFL01(79,AR)
      GPREG(380)=FTFL01(80,AR)
      GPREG(381)=FTFL01(81,AR)
      GPREG(382)=FTFL01(82,AR)
      GPREG(383)=FTFL01(83,AR)
      GPREG(384)=FTFL01(84,AR)
      GPREG(385)=FTFL01(85,AR)
      GPREG(386)=FTFL01(86,AR)
      GPREG(387)=FTFL01(87,AR)
      GPREG(388)=FTFL01(88,AR)
      GPREG(389)=FTFL01(89,AR)
      GPREG(390)=FTFL01(90,AR)
      GPREG(391)=FTFL01(91,AR)
      GPREG(392)=FTFL01(92,AR)
      GPREG(393)=FTFL01(93,AR)
      GPREG(394)=FTFL01(94,AR)
      GPREG(395)=FTFL01(95,AR)
      GPREG(396)=FTFL01(96,AR)
!     (FUN10 call removed - binary macro system removed)
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   IF(A34.EQ.21.0D0) THEN
      FNZ2=0.0D0
      CALL USERSURF(AR,AX,AY,FNZ2,UERROR)
      IF(UERROR) THEN
         UERROR=.FALSE.
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
      FNZ2=FNZ2+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 22
!     GRID SAG
   IF(A34.EQ.22.0D0) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL GRID
!     TYPE 22 SAG GRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      CALL GRIDS(4,ISURF,GERROR)
      IF(.NOT.GERROR) GRIDSUNLOADED22(ISURF)=.FALSE.
      IF(GERROR) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'GRID FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=15
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ2=ZPASS+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
!     DEFORM SURFACE
   IF(surf_default_flag(AR) == 1) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL DEFGRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      IPASS1=4
      CALL DEFGRIDS(4,ISURF,GERROR1,GERROR2)
      IF(GERROR1) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'DEFORM FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=17
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
      IF(GERROR2) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=18
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ2=ZPASS+FNZZ2(AX,AY,AC1,AC2,AC3,AC4,AC6,Q &
      &,AC41,AC42,AC43,AC44,AC45)
      RETURN
   END IF
   RETURN
END
! SUB NRPARAX.FOR
SUBROUTINE NRPARAX
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NRPARAX.FOR. THIS DOES SURF NORM. FOR PARAXIAL
!       SURFACES
!
   INTEGER I
!
   LOGICAL ERRR
!
   real(real64) C1,C2,C3,C4,ACLENG,DEL,A34,FNZ3 &
   &,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,CV,ZTEST,SNGX,SNGY,SNGZ,&
   &MAG,ZCAL,QQ,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,&
   &CY,KY,EY,FY,DY,GY,CX,KX,DX,EX,FX,GX,ZZTOP &
   &,FUNC1X,FUNC1Y,ERR,XPASS,YPASS,ZPASS &
   &,FUNC2X,FUNC2Y,DER1X,DER1Y,DER2X,DER2Y &
   &,FUNC3X,FUNC3Y,DER3X,DER3Y
   real(real64) LENG
   COMMON/PASSLENG/LENG
!
   real(real64) ZZEMAX,ZZEMIN
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   EXTERNAL FNZ1
   EXTERNAL FUNC1X,FUNC1Y
   EXTERNAL FNZ2
   EXTERNAL FUNC2X,FUNC2Y
   EXTERNAL FNZ3
   EXTERNAL FUNC3X,FUNC3Y
   EXTERNAL DER1X,DER1Y,DER2X,DER2Y,DER3X,DER3Y
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   COMMON/ACLEN/ACLENG
!
!
   IF(LN.NE.0.0D0) SNGX=LN/DABS(LN)
   IF(MN.NE.0.0D0) SNGY=MN/DABS(MN)
   IF(NN.NE.0.0D0) SNGZ=NN/DABS(NN)
   IF(LN.EQ.0.0D0) SNGX=1.0D0
   IF(MN.EQ.0.0D0) SNGY=1.0D0
   IF(NN.EQ.0.0D0) SNGZ=1.0D0
!
   IF(DABS(surf_curvature(R_I)).GT.DABS(surf_toric_curvature(R_I))) THEN
      CV=surf_curvature(R_I)
   ELSE
      CV=surf_toric_curvature(R_I)
   END IF
!
   IF(CV.NE.0.0D0) ZTEST=1.0D0/CV
!
!
   DEL=DELSUR
!
   ERRR=.FALSE.
   IF(surf_curvature(R_I).EQ.0.0D0.AND.surf_toric_curvature(R_I).EQ.0.0D0) THEN
      FNXP=-DER1X(FUNC1X,R_X,DELSUR,ERRR)
      FNYP=-DER1Y(FUNC1Y,R_Y,DELSUR,ERRR)
      IF(ERRR) RETURN
   END IF
   IF(surf_curvature(R_I).NE.0.0D0.AND.surf_toric_curvature(R_I).EQ.0.0D0) THEN
      FNXP=-DER2X(FUNC2X,R_X,DELSUR,ERRR)
      FNYP=-DER2Y(FUNC2Y,R_Y,DELSUR,ERRR)
      IF(ERRR) RETURN
   END IF
   IF(surf_toric_curvature(R_I).NE.0.0D0) THEN
      FNXP=-DER3X(FUNC3X,R_X,DELSUR,ERRR)
      FNYP=-DER3Y(FUNC3Y,R_Y,DELSUR,ERRR)
      IF(ERRR) RETURN
   END IF
   FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
   MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
   LN=FNXP/MAG
   MN=FNYP/MAG
   NN=FNZP/MAG
   RETURN
END
! SUB NR3.FOR
SUBROUTINE NR3(ERRR)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   INTEGER SPDCD1,SPDCD2
!
   LOGICAL ERRR
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
!       THIS IS SUBROUTINE NR3.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO AN ASPHERIC/CONIC TORIC SURFACE
!
   INTEGER I
!
   EXTERNAL FNZ3
!
   EXTERNAL FUNC3X,FUNC3Y,DER1X,DER1Y
!
   real(real64) C1,C2,C3,C4,ACLENG,DEL,A34,FNZ3 &
   &,FNXP,FNYP,FNZP,HV1,HV2,HV,XP,YP,ZP,CV,ZTEST,SNGX,SNGY,SNGZ,&
   &MAG,ZCAL,QQ,C5,C6,C7,C8,C9,C10,C11,C12,C13,C14,&
   &CY,KY,EY,FY,DY,GY,CX,KX,DX,EX,FX,GX,ZZTOP,DER3X,DER3Y &
   &,FUNC3X,FUNC3Y,ERR,XPASS,YPASS,ZPASS
   real(real64) R1,R2,RD,ZZEMAX,ZZEMIN
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
   real(real64) LENG
   COMMON/PASSLENG/LENG
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   COMMON/ACLEN/ACLENG
!
!
   IF(LN.NE.0.0D0) SNGX=LN/DABS(LN)
   IF(MN.NE.0.0D0) SNGY=MN/DABS(MN)
   IF(NN.NE.0.0D0) SNGZ=NN/DABS(NN)
   IF(LN.EQ.0.0D0) SNGX=1.0D0
   IF(MN.EQ.0.0D0) SNGY=1.0D0
   IF(NN.EQ.0.0D0) SNGZ=1.0D0
!
!       NOW REFINE TO INTERSECT WITH THE ASPHERIC
!
!       THE RETURNED VALUES OF X,Y,Z ARE THE CORRECT INTERSECTIONS
!       TO WITHIN THE SURTOL VALUE
!
!

!     AD
   C1=surf_asphere_coeff(R_I, 4)
!     AE
   C2=surf_asphere_coeff(R_I, 6)
!     AF
   C3=surf_asphere_coeff(R_I, 8)
!     AG
   C4=surf_asphere_coeff(R_I, 10)
!     CC
   C5=surf_conic(R_I)
!     CV
   C6=surf_curvature(R_I)
!     TORIC FLAG
   C7=surf_toric_flag(R_I)
!     CVTOR
   C8=surf_toric_curvature(R_I)
!     TASPH FLAG
   C9=surf_anamorphic_flag(R_I)
!     ADTOR
   C10=surf_anamorphic_coeff(R_I, 4)
!     AETOR
   C11=surf_anamorphic_coeff(R_I, 6)
!     AFTOR
   C12=surf_anamorphic_coeff(R_I, 8)
!     AGTOR
   C13=surf_anamorphic_coeff(R_I, 10)
!     CCTOR
   C14=surf_anamorphic_conic(R_I)
!     SPSRF FLAG
   A34=surf_special_type(R_I)
!
!       IF THE SURFACE IS AN Y-TORIC THE BASE CURVATURE (CV)
!       LIES IN THE YZ PLANE. IF,HOWEVER, THE SURFACE IS A
!       Y-TORIC, THE BASE CURVATURE LIES IN THE XZ PLANE.
!
   IF(C7.EQ.1.0D0) THEN
!       YTORIC, DO ASSIGNMENTS
      CY=C6
      KY=C5
      DY=C1
      EY=C2
      FY=C3
      GY=C4
      CX=C8
      KX=C14
      DX=C10
      EX=C11
      FX=C12
      GX=C13
   END IF
   IF(C7.EQ.2.0D0) THEN
!       XTORIC, DO ASSIGNMENTS
      CY=C8
      KY=C14
      DY=C10
      EY=C11
      FY=C12
      GY=C13
      CX=C6
      KX=C5
      DX=C1
      EX=C2
      FX=C3
      GX=C4
!
   END IF
!
!       CALCULATE INTERSECTION
   DO 10 I=1,NRAITR
      QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(R_X**2))&
      &-((KY+1.0D0)*(CY**2)*(R_Y**2)))
      IF(QQ.LT.0.0D0) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(R_I)
            OUTLYNE='RAY DID NOT INTERSECT THE SURFACE'
            CALL SHOWIT(1)
         END IF
         RAYCOD(1)=1
         RAYCOD(2)=R_I
         SPDCD1=RAYCOD(1)
         SPDCD2=R_I
         STOPP=1
         RETURN
      END IF
      QQ=DSQRT(QQ)
      QQ=1.0D0+QQ
!       CALCULATE CURRENT TANGENT PLANE NORMAL
      DEL=DELSUR
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ3 EVALUATES THE FUNCTION
!
      ERRR=.FALSE.
      FNXP=-DER3X(FUNC3X,R_X,DELSUR,ERRR)
      FNYP=-DER3Y(FUNC3Y,R_Y,DELSUR,ERRR)
      IF(ERRR) RETURN
      FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
      MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
      LN=FNXP/MAG
      MN=FNYP/MAG
      NN=FNZP/MAG
!       THE INTERSECTION OF THE RAY WITH THIS TANGENT
!       PLANE
!       USING:
!
      ZCAL=FNZ3(R_X,R_Y,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,&
      &R_I,A34,QQ)
      HV1=ZCAL-R_Z
      HV2=((R_L*LN)+(R_M*MN)+(R_N*NN))
      HV=HV1/HV2
!       NEW X,Y AND Z ARE
      XP=R_X+(HV*R_L)
      YP=R_Y+(HV*R_M)
      ZP=R_Z+(HV*R_N)
!       SHOULD WE CONTINUE?
      QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(XP**2))&
      &-((KY+1.0D0)*(CY**2)*(YP**2)))
      IF(QQ.LT.0.0D0) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(R_I)
            OUTLYNE='RAY DID NOT INTERSECT THE SURFACE'
            CALL SHOWIT(1)
         END IF
         RAYCOD(1)=1
         RAYCOD(2)=R_I
         SPDCD1=RAYCOD(1)
         SPDCD2=R_I
         STOPP=1
         RETURN
      END IF
      QQ=DSQRT(QQ)
      QQ=1.0D0+QQ
      ZZTOP=FNZ3(XP,YP,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,&
      &R_I,A34,QQ)
      LENG=DABS(ZZTOP-ZP)
      ACLENG=LENG
      IF(LENG.LE.SURTOL) THEN
         R_X=XP
         R_Y=YP
         R_Z=ZP
         QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(R_X**2))&
         &-((KY+1.0D0)*(CY**2)*(R_Y**2)))
         IF(QQ.LT.0.0D0) THEN
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE='RAY DID NOT INTERSECT THE SURFACE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=1
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
         END IF
         QQ=DSQRT(QQ)
         QQ=1.0D0+QQ
!       CALCULATE CURRENT TANGENT PLANE NORMAL
!
!     X AND Y DERIVATIVES OF THE SURFACE ARE CALCULATED HERE:
!     FUNCTION FNZ3 EVALUATES THE FUNCTION
!
         ERRR=.FALSE.
         FNXP=-DER3X(FUNC3X,R_X,DELSUR,ERRR)
         FNYP=-DER3Y(FUNC3Y,R_Y,DELSUR,ERRR)
         IF(ERRR) RETURN
         FNZP=SNGZ
!
!     X AND Y DERIVATIVES OF THE SURFACE HAVE BEEN CALCULATED
!
         MAG=DSQRT((FNXP**2)+(FNYP**2)+(FNZP**2))
         LN=FNXP/MAG
         MN=FNYP/MAG
         NN=FNZP/MAG
         STOPP=0
         RETURN
      ELSE
!       KEEP TRYING
!       MAKE X Y Z FROM XP YP ZP AND TRY AGAIN
         R_X=XP
         R_Y=YP
         R_Z=ZP
      END IF
10 CONTINUE
   IF(MSG) THEN
      CALL RAY_FAILURE(R_I)
      OUTLYNE=&
      &'CONVERGENCE FAILURE OCCURRED DURING RAY INTERSECTION'
      CALL SHOWIT(1)
      OUTLYNE='WITH ASPHERIC, TORIC OR SPECIAL SURFACE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'ACCURACY OF INTERSECTION WAS = ',LENG
      CALL SHOWIT(1)
   END IF
   RAYCOD(1)=2
   RAYCOD(2)=R_I
   SPDCD1=RAYCOD(1)
   SPDCD2=R_I
   STOPP=1
   RETURN
END
FUNCTION FNZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,&
&AR,A34,Q)
   USE GLOBALS
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wavelength, sys_units, sys_wl_ref
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FF2,FF3,FF4,FF5
!
   real(real64) FNZ3,AX,AY,ACX,ACY,ADX,ADY,AACX,AACY,AADX &
   &,AADY,AAX,AAY,FNZZ3,A34,R,THETA,FF2,FF3,AQ,Q,AEX,AEY,&
   &AAEX,AAEY,AAFX,AAFY,AAGX,AAGY,AFX,AFY,AGX,AGY,XX,YY,FF4 &
   &,FF5,AAAX,AAAY,DA34
   real(real64) ZZEMAX,ZZEMIN
   real(real64) JK_WAVE
   real(real64) AMP1,OMEGA1X,OMEGA1Y
   real(real64) AMP2,OMEGA2X,OMEGA2Y
   real(real64) AMP3,OMEGA3X,OMEGA3Y
   real(real64) AMP4,OMEGA4X,OMEGA4Y
   real(real64) AMP5,OMEGA5X,OMEGA5Y
   COMMON/KENMOOR/ZZEMIN,ZZEMAX
!
   real(real64) XPASS,YPASS,ZPASS
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   INTEGER SPDCD1,SPDCD2
!
   INTEGER IPASS1
!
   COMMON/SPRA2/SPDCD1,SPDCD2
!
   INTEGER KLI,II,AR,ISURF
!
   LOGICAL FUN,GERROR,GERROR1,GERROR2,UERROR
!
   COMMON/NEFER/KLI
!
   COMMON/COMFUN/FUN
!
!
   INTEGER I
!
   FNZZ3(AAX,AAY,AACX,AACY,AADX,AADY,AAEX,AAEY,AAFX,AAFY,&
   &AAGX,AAGY,AQ)=&
   &(((AACX*(AAX**2))+(AACY*(AAY**2)))/AQ)&
   &+(AADY*(((1.0D0-AADX)*(AAX**2))+((1.0D0+AADX)*(AAY**2)))**2)&
   &+(AAEY*(((1.0D0-AAEX)*(AAX**2))+((1.0D0+AAEX)*(AAY**2)))**3)&
   &+(AAFY*(((1.0D0-AAFX)*(AAX**2))+((1.0D0+AAFX)*(AAY**2)))**4)&
   &+(AAGY*(((1.0D0-AAGX)*(AAX**2))+((1.0D0+AAGX)*(AAY**2)))**5)
!
!     IF SPECIAL SURFACE TYPE 1, 2 OR 3 ARE TO BE APPLIED
!     THEY ARE APPLIED HERE
!
   DA34=DABS(A34)
   IF(A34.EQ.0.0D0.AND.surf_default_flag(AR) == 0.OR.DA34.EQ.6.0D0.OR.&
   &DA34.EQ.7.0D0.OR.DA34.EQ.9.0D0.OR.DA34.EQ.10.0D0.OR.&
   &DA34.EQ.11.0D0.OR.DA34.EQ.12.0D0.OR.&
   &DA34.EQ.15.0D0.OR.DA34.EQ.16.0D0.OR.DA34.EQ.17.0D0.OR.&
   &DA34.EQ.19.0D0.OR.DA34.EQ.20.0D0) THEN
      FNZ3=FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
   FNZ3=0.0D0
!     SPECIAL SURFACE TYPE 1
   IF(A34.EQ.1.0D0) THEN
      DO I=9,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+&
         &(FTFL01(I,AR)*(((DSQRT((AX**2)+(AY**2)))**(I-9))))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 4
   IF(A34.EQ.4.0D0) THEN
      JK_WAVE=sys_wavelength(INT(sys_wl_ref()))
      AMP1=DABS(FTFL01(1,AR)*JK_WAVE*0.5D0)
      AMP2=DABS(FTFL01(4,AR)*JK_WAVE*0.5D0)
      AMP3=DABS(FTFL01(7,AR)*JK_WAVE*0.5D0)
      AMP4=DABS(FTFL01(10,AR)*JK_WAVE*0.5D0)
      AMP5=DABS(FTFL01(13,AR)*JK_WAVE*0.5D0)
      IF(sys_units().EQ.1) AMP1=(AMP1*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP1=AMP1*1.0D-4
      IF(sys_units().EQ.3) AMP1=AMP1*1.0D-3
      IF(sys_units().EQ.4) AMP1=AMP1*1.0D-6
      IF(sys_units().EQ.1) AMP2=(AMP2*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP2=AMP2*1.0D-4
      IF(sys_units().EQ.3) AMP2=AMP2*1.0D-3
      IF(sys_units().EQ.4) AMP2=AMP2*1.0D-6
      IF(sys_units().EQ.1) AMP3=(AMP3*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP3=AMP3*1.0D-4
      IF(sys_units().EQ.3) AMP3=AMP3*1.0D-3
      IF(sys_units().EQ.4) AMP3=AMP3*1.0D-6
      IF(sys_units().EQ.1) AMP4=(AMP4*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP4=AMP4*1.0D-4
      IF(sys_units().EQ.3) AMP4=AMP4*1.0D-3
      IF(sys_units().EQ.4) AMP4=AMP4*1.0D-6
      IF(sys_units().EQ.1) AMP5=(AMP5*1.0D-4/2.54D0)
      IF(sys_units().EQ.2) AMP5=AMP5*1.0D-4
      IF(sys_units().EQ.3) AMP5=AMP5*1.0D-3
      IF(sys_units().EQ.4) AMP5=AMP5*1.0D-6
      AMP1=FTFL01(1,AR)
      AMP2=FTFL01(4,AR)
      AMP3=FTFL01(7,AR)
      AMP4=FTFL01(10,AR)
      AMP5=FTFL01(13,AR)
      IF(FTFL01(2,AR).EQ.0.0D0) THEN
         OMEGA1X=0.0D0
      ELSE
         OMEGA1X=TWOPII/DABS(FTFL01(2,AR))
      END IF
      IF(FTFL01(3,AR).EQ.0.0D0) THEN
         OMEGA1Y=0.0D0
      ELSE
         OMEGA1Y=TWOPII/DABS(FTFL01(3,AR))
      END IF
      IF(FTFL01(5,AR).EQ.0.0D0) THEN
         OMEGA2X=0.0D0
      ELSE
         OMEGA2X=TWOPII/DABS(FTFL01(5,AR))
      END IF
      IF(FTFL01(6,AR).EQ.0.0D0) THEN
         OMEGA2Y=0.0D0
      ELSE
         OMEGA2Y=TWOPII/DABS(FTFL01(6,AR))
      END IF
      IF(FTFL01(8,AR).EQ.0.0D0) THEN
         OMEGA3X=0.0D0
      ELSE
         OMEGA3X=TWOPII/DABS(FTFL01(8,AR))
      END IF
      IF(FTFL01(9,AR).EQ.0.0D0) THEN
         OMEGA3Y=0.0D0
      ELSE
         OMEGA3Y=TWOPII/DABS(FTFL01(9,AR))
      END IF
      IF(FTFL01(11,AR).EQ.0.0D0) THEN
         OMEGA4X=0.0D0
      ELSE
         OMEGA4X=TWOPII/DABS(FTFL01(11,AR))
      END IF
      IF(FTFL01(12,AR).EQ.0.0D0) THEN
         OMEGA4Y=0.0D0
      ELSE
         OMEGA4Y=TWOPII/DABS(FTFL01(12,AR))
      END IF
      IF(FTFL01(14,AR).EQ.0.0D0) THEN
         OMEGA5X=0.0D0
      ELSE
         OMEGA5X=TWOPII/DABS(FTFL01(14,AR))
      END IF
      IF(FTFL01(15,AR).EQ.0.0D0) THEN
         OMEGA5Y=0.0D0
      ELSE
         OMEGA5Y=TWOPII/DABS(FTFL01(15,AR))
      END IF
      FNZ3=(AMP1*(DCOS(OMEGA1X*AX)*(DCOS(OMEGA1Y*AY))))&
      &+(AMP2*(DCOS(OMEGA2X*AX)*(DCOS(OMEGA2Y*AY))))&
      &+(AMP3*(DCOS(OMEGA3X*AX)*(DCOS(OMEGA3Y*AY))))&
      &+(AMP4*(DCOS(OMEGA4X*AX)*(DCOS(OMEGA4Y*AY))))&
      &+(AMP5*(DCOS(OMEGA5X*AX)*(DCOS(OMEGA5Y*AY))))
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 2
   IF(A34.EQ.2.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ3=0.0D0
      DO I=1,66
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+&
         &(FTFL01(I,AR)*(FF2(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 14
   IF(A34.EQ.14.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ3=0.0D0
      DO I=1,48
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+&
         &(FTFL01(I,AR)*(FF5(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 3
   IF(A34.EQ.3.0D0) THEN
      INR=surf_inr_value(AR)
      AAAX=AX/INR
      AAAY=AY/INR
      R=DSQRT((AAAX**2)+(AAAY**2))
      IF(DABS(AAAY).GE.DABS(((1.0D35)*AAAX))) THEN
         IF(AAAY.GE.0.0D0) THETA=PII/2.0D0
         IF(AAAY.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(AAAY).EQ.0.0D0.AND.DABS(AAAX).EQ.0.0D0) THEN
            THETA=0.0D0
         ELSE
            THETA=DATAN2(AAAY,AAAX)
         END IF
         IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
      END IF
      FNZ3=0.0D0
      DO I=1,37
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+&
         &(FTFL01(I,AR)*(FF3(R,THETA,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 8
   IF(A34.EQ.8.0D0) THEN
      XX=AX
      YY=AY
      FNZ3=0.0D0
      DO I=1,91
         IF(FTFL01(I,AR).NE.0.0D0) FNZ3=FNZ3+&
         &(FTFL01(I,AR)*(FF4(XX,YY,I)))
      END DO
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 5
!     USER DEFINED SURFACE
   IF(A34.EQ.5.0D0) THEN
      REG(40)=REG(9)
      REG(9)=AX
      REG(10)=AY
      REG(12)=DBLE(I)
!     THIS SURFACE AUTOMATICALLY USES MACRO FUNCTION FUN10
!     FOR THE INITIAL USER FUCTION TO BE CALLED TO EVALUATE
!     THE Z VALUE CORRESPONDING TO THE AY AND AX VALUES.
!     COEFFICIENTS C1 TO C96 ARE USED BY THE USER IN THE
!     MACRO FUNCTION FUN10 AND ANY MACRO FUNCTION CALLED BY FUN10.
!     THEY ARE STORED INITIALLY IN GENERAL PURPOSE STORAGE REGISTERS
!     NUMBERS 301 TO 396 AND ACCESSED BY THE USER VIA RCL COMMANDS
      GPREG(301)=FTFL01(1,AR)
      GPREG(302)=FTFL01(2,AR)
      GPREG(303)=FTFL01(3,AR)
      GPREG(304)=FTFL01(4,AR)
      GPREG(305)=FTFL01(5,AR)
      GPREG(306)=FTFL01(6,AR)
      GPREG(307)=FTFL01(7,AR)
      GPREG(308)=FTFL01(8,AR)
      GPREG(309)=FTFL01(9,AR)
      GPREG(310)=FTFL01(10,AR)
      GPREG(311)=FTFL01(11,AR)
      GPREG(312)=FTFL01(12,AR)
      GPREG(313)=FTFL01(13,AR)
      GPREG(314)=FTFL01(14,AR)
      GPREG(315)=FTFL01(15,AR)
      GPREG(316)=FTFL01(16,AR)
      GPREG(317)=FTFL01(17,AR)
      GPREG(318)=FTFL01(18,AR)
      GPREG(319)=FTFL01(19,AR)
      GPREG(320)=FTFL01(20,AR)
      GPREG(321)=FTFL01(21,AR)
      GPREG(322)=FTFL01(22,AR)
      GPREG(323)=FTFL01(23,AR)
      GPREG(324)=FTFL01(24,AR)
      GPREG(325)=FTFL01(25,AR)
      GPREG(326)=FTFL01(26,AR)
      GPREG(327)=FTFL01(27,AR)
      GPREG(328)=FTFL01(28,AR)
      GPREG(329)=FTFL01(29,AR)
      GPREG(330)=FTFL01(30,AR)
      GPREG(331)=FTFL01(31,AR)
      GPREG(332)=FTFL01(32,AR)
      GPREG(333)=FTFL01(33,AR)
      GPREG(334)=FTFL01(34,AR)
      GPREG(335)=FTFL01(35,AR)
      GPREG(336)=FTFL01(36,AR)
      GPREG(337)=FTFL01(37,AR)
      GPREG(338)=FTFL01(38,AR)
      GPREG(339)=FTFL01(39,AR)
      GPREG(340)=FTFL01(40,AR)
      GPREG(341)=FTFL01(41,AR)
      GPREG(342)=FTFL01(42,AR)
      GPREG(343)=FTFL01(43,AR)
      GPREG(344)=FTFL01(44,AR)
      GPREG(345)=FTFL01(45,AR)
      GPREG(346)=FTFL01(46,AR)
      GPREG(347)=FTFL01(47,AR)
      GPREG(348)=FTFL01(48,AR)
      GPREG(349)=FTFL01(49,AR)
      GPREG(350)=FTFL01(50,AR)
      GPREG(351)=FTFL01(51,AR)
      GPREG(352)=FTFL01(52,AR)
      GPREG(353)=FTFL01(53,AR)
      GPREG(354)=FTFL01(54,AR)
      GPREG(355)=FTFL01(55,AR)
      GPREG(356)=FTFL01(56,AR)
      GPREG(357)=FTFL01(57,AR)
      GPREG(358)=FTFL01(58,AR)
      GPREG(359)=FTFL01(59,AR)
      GPREG(360)=FTFL01(60,AR)
      GPREG(361)=FTFL01(61,AR)
      GPREG(362)=FTFL01(62,AR)
      GPREG(363)=FTFL01(63,AR)
      GPREG(364)=FTFL01(64,AR)
      GPREG(365)=FTFL01(65,AR)
      GPREG(366)=FTFL01(66,AR)
      GPREG(367)=FTFL01(67,AR)
      GPREG(368)=FTFL01(68,AR)
      GPREG(369)=FTFL01(69,AR)
      GPREG(370)=FTFL01(70,AR)
      GPREG(371)=FTFL01(71,AR)
      GPREG(372)=FTFL01(72,AR)
      GPREG(373)=FTFL01(73,AR)
      GPREG(374)=FTFL01(74,AR)
      GPREG(375)=FTFL01(75,AR)
      GPREG(376)=FTFL01(76,AR)
      GPREG(377)=FTFL01(77,AR)
      GPREG(378)=FTFL01(78,AR)
      GPREG(379)=FTFL01(79,AR)
      GPREG(380)=FTFL01(80,AR)
      GPREG(381)=FTFL01(81,AR)
      GPREG(382)=FTFL01(82,AR)
      GPREG(383)=FTFL01(83,AR)
      GPREG(384)=FTFL01(84,AR)
      GPREG(385)=FTFL01(85,AR)
      GPREG(386)=FTFL01(86,AR)
      GPREG(387)=FTFL01(87,AR)
      GPREG(388)=FTFL01(88,AR)
      GPREG(389)=FTFL01(89,AR)
      GPREG(390)=FTFL01(90,AR)
      GPREG(391)=FTFL01(91,AR)
      GPREG(392)=FTFL01(92,AR)
      GPREG(393)=FTFL01(93,AR)
      GPREG(394)=FTFL01(94,AR)
      GPREG(395)=FTFL01(95,AR)
      GPREG(396)=FTFL01(96,AR)
!     (FUN10 call removed - binary macro system removed)
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 21
!     USER DEFINED SURFACE
   IF(A34.EQ.21.0D0) THEN
      FNZ3=0.0D0
      CALL USERSURF(AR,AX,AY,FNZ3,UERROR)
      IF(UERROR) THEN
         UERROR=.FALSE.
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
      FNZ3=FNZ3+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     SPECIAL SURFACE TYPE 22
!     GRID SAG
   IF(A34.EQ.22.0D0) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL GRID
!     TYPE 22 SAG GRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      CALL GRIDS(4,ISURF,GERROR)
      IF(.NOT.GERROR) GRIDSUNLOADED22(ISURF)=.FALSE.
      IF(GERROR) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'GRID FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=15
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ3=ZPASS+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
!     DEFORM SURFACE
   IF(surf_default_flag(AR) == 1) THEN
!     USE WHAT REMAINS IN THE Z-REGISTER
!     CALL GRID
      ISURF=AR
      XPASS=AX
      YPASS=AY
      ZPASS=0.0D0
      IPASS1=4
      CALL DEFGRIDS(4,ISURF,GERROR1,GERROR2)
      IF(GERROR1) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'DEFORM FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=17
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
      IF(GERROR2) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(ISURF)
            OUTLYNE=&
            &'RAY COORDINATE BEYOND DEFORMABLE SURFACE BOUNDS'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=18
         RAYCOD(2)=ISURF
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=ISURF
         RETURN
      END IF
!     NOW ADD THE REGULAR TERM
!     IF(ZPASS.GE.ZZEMAX) ZZEMAX=ZPASS
!     IF(ZPASS.LE.ZZEMIN) ZZEMIN=ZPASS
      FNZ3=ZPASS+&
      &FNZZ3(AX,AY,ACX,ACY,ADX,ADY,AEX,AEY,AFX,AFY,AGX,AGY,Q)
      RETURN
   END IF
   RETURN
END
FUNCTION DER1X(FUNC1X,X,DELH,DERERROR)
!
!     SIMPLE X-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER1X,DELH,X,FUNC1X
   LOGICAL DERERROR

   EXTERNAL FUNC1X
!
   DER1X=(FUNC1X(X+DELH)-FUNC1X(X-DELH))/(2.0D0*DELH)
   IF(DABS(DER1X).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION DER1Y(FUNC1Y,Y,DELH,DERERROR)
!
!     SIMPLE Y-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER1Y,DELH,Y,FUNC1Y
   LOGICAL DERERROR

   EXTERNAL FUNC1Y
!
   DER1Y=(FUNC1Y(Y+DELH)-FUNC1Y(Y-DELH))/(2.0D0*DELH)
   IF(DABS(DER1Y).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION FUNC1X(AX)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ1
!
   real(real64) FUNC1X,AX,FNZ1,C1,C2,C3,C4,C5,A34 &
   &,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C5=surf_asphere_coeff(R_I, 2)
   A34=surf_special_type(R_I)
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   A34=surf_special_type(R_I)
!
   FUNC1X=FNZ1(AX,R_Y,C1,C2,C3,C4,C5,R_I,A34,C41,C42,C43,C44,C45)
!
   RETURN
END
FUNCTION FUNC1Y(AY)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ1
!
   real(real64) FUNC1Y,AY,FNZ1,C1,C2,C3,C4,C5,A34,C41,C42,C43 &
   &,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C5=surf_asphere_coeff(R_I, 2)
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   A34=surf_special_type(R_I)
!
   FUNC1Y=FNZ1(R_X,AY,C1,C2,C3,C4,C5,R_I,A34,C41,C42,C43,C44,C45)
!
   RETURN
END
!
FUNCTION DER2X(FUNC2X,X,DELH,DERERROR)
!
!     SIMPLE X-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER2X,DELH,X,FUNC2X
   LOGICAL DERERROR

   EXTERNAL FUNC2X
!
   DER2X=(FUNC2X(X+DELH)-FUNC2X(X-DELH))/(2.0D0*DELH)
   IF(DABS(DER2X).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION DER2Y(FUNC2Y,Y,DELH,DERERROR)
!
!     SIMPLE Y-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER2Y,DELH,Y,FUNC2Y
   LOGICAL DERERROR

   EXTERNAL FUNC2Y
!
   DER2Y=(FUNC2Y(Y+DELH)-FUNC2Y(Y-DELH))/(2.0D0*DELH)
   IF(DABS(DER2Y).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION FUNC2X(AX)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ2
!
   real(real64) FUNC2X,AX,FNZ2,C1,C2,C3,C4,C5,C6,A34,Q &
   &,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C5=surf_conic(R_I)
   C6=(surf_curvature(R_I))
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   A34=surf_special_type(R_I)
   Q=(1.0D0-((C5+1.0D0)*(C6**2)*&
   &((AX**2)+(R_Y**2))))
   Q=DSQRT(DABS(Q))
   Q=1.0D0+Q
!
   FUNC2X=FNZ2(AX,R_Y,C1,C2,C3,C4,C6,R_I,A34,Q,C41,C42,C43,&
   &C44,C45)
!
   RETURN
END
FUNCTION FUNC2Y(AY)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ2
!
   real(real64) FUNC2Y,AY,FNZ2,C1,C2,C3,C4,C5,C6,A34,Q &
   &,C41,C42,C43,C44,C45
!
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C41=surf_asphere_coeff(R_I, 12)
   C42=surf_asphere_coeff(R_I, 14)
   C43=surf_asphere_coeff(R_I, 16)
   C44=surf_asphere_coeff(R_I, 18)
   C45=surf_asphere_coeff(R_I, 20)
   A34=surf_special_type(R_I)
   C5=surf_conic(R_I)
   C6=(surf_curvature(R_I))
   Q=(1.0D0-((C5+1.0D0)*(C6**2)*&
   &((R_X**2)+(AY**2))))
   Q=DSQRT(DABS(Q))
   Q=1.0D0+Q
!
   FUNC2Y=FNZ2(R_X,AY,C1,C2,C3,C4,C6,R_I,A34,Q,C41,C42,C43,C44,&
   &C45)
!
   RETURN
END
!
FUNCTION DER3X(FUNC3X,X,DELH,DERERROR)
!
!     SIMPLE X-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER3X,DELH,X,FUNC3X
   LOGICAL DERERROR

   EXTERNAL FUNC3X
!
   DER3X=(FUNC3X(X+DELH)-FUNC3X(X-DELH))/(2.0D0*DELH)
   IF(DABS(DER3X).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION DER3Y(FUNC3Y,Y,DELH,DERERROR)
!
!     SIMPLE Y-DERIVATIVE FUNCTION
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) DER3Y,DELH,Y,FUNC3Y
   LOGICAL DERERROR

   EXTERNAL FUNC3Y
!
   DER3Y=(FUNC3Y(Y+DELH)-FUNC3Y(Y-DELH))/(2.0D0*DELH)
   IF(DABS(DER3Y).GT.10.0D0) DERERROR=.TRUE.
   RETURN
END
FUNCTION FUNC3X(AX)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ3
!
   real(real64) FUNC3X,AX,FNZ3,C1,C2,C3,C4,C5,C6,C7,C8,C9 &
   &,C10,C11,C12,C13,C14,A34,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY &
   &,QQ
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C5=surf_conic(R_I)
   C6=surf_curvature(R_I)
   C7=surf_toric_flag(R_I)
   C8=surf_toric_curvature(R_I)
   C9=surf_anamorphic_flag(R_I)
   C10=surf_anamorphic_coeff(R_I, 4)
   C11=surf_anamorphic_coeff(R_I, 6)
   C12=surf_anamorphic_coeff(R_I, 8)
   C13=surf_anamorphic_coeff(R_I, 10)
   C14=surf_anamorphic_conic(R_I)
   A34=surf_special_type(R_I)
   IF(C7.EQ.1.0D0) THEN
!       YTORIC, DO ASSIGNMENTS
      CY=C6
      KY=C5
      DY=C1
      EY=C2
      FY=C3
      GY=C4
      CX=C8
      KX=C14
      DX=C10
      EX=C11
      FX=C12
      GX=C13
   END IF
   IF(C7.EQ.2.0D0) THEN
!       XTORIC, DO ASSIGNMENTS
      CY=C8
      KY=C14
      DY=C10
      EY=C11
      FY=C12
      GY=C13
      CX=C6
      KX=C5
      DX=C1
      EX=C2
      FX=C3
      GX=C4
   END IF
   QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(AX**2))&
   &-((KY+1.0D0)*(CY**2)*(R_Y**2)))
   QQ=DSQRT(DABS(QQ))
   QQ=1.0D0+QQ
!
   FUNC3X=FNZ3(AX,R_Y,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,R_I,A34,QQ)
!
   RETURN
END
FUNCTION FUNC3Y(AY)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   EXTERNAL FNZ3
!
   real(real64) FUNC3Y,AY,FNZ3,C1,C2,C3,C4,C5,C6,C7,C8,C9 &
   &,C10,C11,C12,C13,C14,A34,CX,CY,KX,KY,DX,DY,EX,EY,FX,FY,GX,GY &
   &,QQ
!
   C1=surf_asphere_coeff(R_I, 4)
   C2=surf_asphere_coeff(R_I, 6)
   C3=surf_asphere_coeff(R_I, 8)
   C4=surf_asphere_coeff(R_I, 10)
   C5=surf_conic(R_I)
   C6=surf_curvature(R_I)
   C7=surf_toric_flag(R_I)
   C8=surf_toric_curvature(R_I)
   C9=surf_anamorphic_flag(R_I)
   C10=surf_anamorphic_coeff(R_I, 4)
   C11=surf_anamorphic_coeff(R_I, 6)
   C12=surf_anamorphic_coeff(R_I, 8)
   C13=surf_anamorphic_coeff(R_I, 10)
   C14=surf_anamorphic_conic(R_I)
   A34=surf_special_type(R_I)
   IF(C7.EQ.1.0D0) THEN
!       YTORIC, DO ASSIGNMENTS
      CY=C6
      KY=C5
      DY=C1
      EY=C2
      FY=C3
      GY=C4
      CX=C8
      KX=C14
      DX=C10
      EX=C11
      FX=C12
      GX=C13
   END IF
   IF(C7.EQ.2.0D0) THEN
!       XTORIC, DO ASSIGNMENTS
      CY=C8
      KY=C14
      DY=C10
      EY=C11
      FY=C12
      GY=C13
      CX=C6
      KX=C5
      DX=C1
      EX=C2
      FX=C3
      GX=C4
   END IF
   QQ=(1.0D0-((KX+1.0D0)*(CX**2)*(R_X**2))&
   &-((KY+1.0D0)*(CY**2)*(AY**2)))
   QQ=DSQRT(DABS(QQ))
   QQ=1.0D0+QQ
!
   FUNC3Y=FNZ3(R_X,AY,CX,CY,DX,DY,EX,EY,FX,FY,GX,GY,R_I,A34,QQ)
!
   RETURN
END
! SUB NR5.FOR
SUBROUTINE NR5
!
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NR5.FOR. THIS SUBROUTINE IMPLEMENTS
!       INTERSECTION CALCULATION TO A CONIC SURFACE WITH FOURIER-
!       LEGENDRE COEFFICIENTS
!
   real(real64) MAG,H,RHO,Z3,ZM &
   &,X,Y,Z,L,M,N,XO,YO,ZO,LO,MO,NO,ZETA,Z1,Z2,CEE
!
   COMMON/FUNN5/XO,YO,ZO,LO,MO,NO
!
   COMMON/HFIND/H
!
   COMMON/NORMER/X,Y,Z,L,M,N
!
!
   CEE=surf_curvature(R_I)
   IF(R_Z.LT.0.0D0.AND.CEE.GT.0.0D0.OR.&
   &R_Z.GT.0.0D0.AND.CEE.LT.0.0D0) RETURN
   Z1=FTFL01(1,R_I)
   Z2=FTFL01(2,R_I)
   Z3=FTFL01(3,R_I)
   ZM=R_Z
   ZETA=(2.0D0*(ZM-Z3))/(Z2-Z1)
   RHO=DSQRT((R_X**2)+(R_Y**2))
   IF(DABS(ZETA).GT.1.5D0.OR.RHO.EQ.0.0D0) RETURN
!     INITIAL GUESS AT H IS 0
   H=0
   XO=R_X
   YO=R_Y
   ZO=R_Z
   LO=R_L
   MO=R_M
   NO=R_N
!
!     FIND THE NEW H VALUE WHICH MAKES F(H)=0
!
   CALL FINDH
   IF(STOPP.EQ.1) RETURN
!
!     H IS THE DISTANCE TRAVELED FROM THE CONIC INTERSECTION
!     TO THE DEFORMED CONIS INTERSECTION. IT IS A SMALL NUMBER!
!
!     THE RAY COORDINATES OF THE SURFACE INTERSECTION ARE NOW JUST:
   R_X=R_X+(H*R_L)
   R_Y=R_Y+(H*R_M)
   R_Z=R_Z+(H*R_N)
!
!     NOW CALCULATE THE DIRCTION COSINES OF THE SURFACE NORMAL
!
   X=R_X
   Y=R_Y
   Z=R_Z
   CALL FINDNORM
   LN=L
   MN=M
   NN=N
   MAG=DSQRT((LN**2)+(MN**2)+(NN**2))
   LN=LN/MAG
   MN=MN/MAG
   NN=NN/MAG
   RETURN
END
!
FUNCTION FUNC()
!     THIS IS THE SURFACE FUNCTION
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) FUNC,H,X,Y,Z,KAPPA,CEE,RHO,LO,MO,NO,Z3,ZM &
   &,XO,YO,ZO,THETA,ZETA,Z1,Z2,FL,FL1,FL2,FL3,FL4,FL5,FL6,&
   &FL7,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,FLT,FLZEE &
   &,FLX,FLY
   EXTERNAL FL
   INTEGER FLII
   COMMON/FUNCER/H
   COMMON/FUNN5/XO,YO,ZO,LO,MO,NO
   COMMON/JIM/FLZEE,FLT,FLII,FLX,FLY
!
   X=XO+(H*LO)
   Y=YO+(H*MO)
   Z=ZO+(H*NO)
!
   KAPPA=surf_conic(R_I)
   CEE=surf_curvature(R_I)
   IF(Z.LT.0.0D0.AND.CEE.GT.0.0D0) THEN
      Z=0.0D0
      H=-ZO/NO
   END IF
   IF(Z.GT.0.0D0.AND.CEE.LT.0.0D0) THEN
      Z=0.0D0
      H=-ZO/NO
   END IF
   Z1=FTFL01(1,R_I)
   Z2=FTFL01(2,R_I)
   Z3=FTFL01(3,R_I)
   ZM=Z
   ZETA=(2.0D0*(ZM-Z3))/(Z2-Z1)
   RHO=DSQRT((X**2)+(Y**2))
   IF(DABS(Y).GE.DABS(((1.0D35)*(X)))) THEN
      IF(Y.GE.0.0D0) THETA=PII/2.0D0
      IF(Y.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
   ELSE
      IF(DABS(Y).EQ.0.0D0.AND.DABS(X).EQ.0.0D0) THEN
         THETA=0.0D0
      ELSE
         THETA=DATAN2(Y,X)
      END IF
      IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
   END IF
   FLZEE=ZETA
   FLT=THETA
   FLII=1
   FL1=FL()
   FLII=2
   FL2=FL()
   FLII=3
   FL3=FL()
   FLII=4
   FL4=FL()
   FLII=5
   FL5=FL()
   FLII=6
   FL6=FL()
   FLII=7
   FL7=FL()
   FLII=8
   FL8=FL()
   FLII=9
   FL9=FL()
   FLII=10
   FL10=FL()
   FLII=11
   FL11=FL()
   FLII=12
   FL12=FL()
   FLII=13
   FL13=FL()
   FLII=14
   FL14=FL()
   FLII=15
   FL15=FL()
   FUNC=-RHO+DSQRT(((2.0D0*Z)/CEE)-((Z**2)*(KAPPA+1.0D0)))&
   &+(FTFL01(4,R_I)*FL1)&
   &+(FTFL01(5,R_I)*FL2)&
   &+(FTFL01(6,R_I)*FL3)&
   &+(FTFL01(7,R_I)*FL4)&
   &+(FTFL01(8,R_I)*FL5)&
   &+(FTFL01(9,R_I)*FL6)&
   &+(FTFL01(10,R_I)*FL7)&
   &+(FTFL01(11,R_I)*FL8)&
   &+(FTFL01(12,R_I)*FL9)&
   &+(FTFL01(13,R_I)*FL10)&
   &+(FTFL01(14,R_I)*FL11)&
   &+(FTFL01(15,R_I)*FL12)&
   &+(FTFL01(16,R_I)*FL13)&
   &+(FTFL01(17,R_I)*FL14)&
   &+(FTFL01(18,R_I)*FL15)
   RETURN
END
!
SUBROUTINE FINDNORM
   use DATLEN
   use mod_surface
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) L,M,N,KAPPA,X,Y,Z,CEE &
   &,RHO,Z1,Z2,THETA,ZETA,FLX,FLY,FLZ,FLZEE,FLXX,FLYY
   EXTERNAL FLX,FLY,FLZ
   real(real64) FL1,FL2,FL3,FL4,FL5,FL6,FL7,FLT &
   &,N1,FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,ZETAD,Z3,ZM
   INTEGER FLII
   COMMON/NORMER/X,Y,Z,L,M,N
   COMMON/JIM/FLZEE,FLT,FLII,FLXX,FLYY
   COMMON/ZEETAA/ZETAD
   CEE=surf_curvature(R_I)
   KAPPA=surf_conic(R_I)
   RHO=DSQRT((X**2)+(Y**2))
   Z1=FTFL01(1,R_I)
   Z2=FTFL01(2,R_I)
   Z3=FTFL01(3,R_I)
   ZM=Z
   ZETA=(2.0D0*(ZM-Z3))/(Z2-Z1)
   ZETAD=(Z2-Z1)
   IF(DABS(Y).GE.DABS(((1.0D35)*(X)))) THEN
      IF(Y.GE.0.0D0) THETA=PII/2.0D0
      IF(Y.LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
   ELSE
      IF(DABS(Y).EQ.0.0D0.AND.DABS(X).EQ.0.0D0) THEN
         THETA=0.0D0
      ELSE
         THETA=DATAN2(Y,X)
      END IF
      IF(THETA.LT.0.0D0) THETA=THETA+(TWOPII)
   END IF
   FLZEE=ZETA
   FLT=THETA
   FLXX=X
   FLYY=Y
   FLII=1
   FL1=FLX()
   FLII=2
   FL2=FLX()
   FLII=3
   FL3=FLX()
   FLII=4
   FL4=FLX()
   FLII=5
   FL5=FLX()
   FLII=6
   FL6=FLX()
   FLII=7
   FL7=FLX()
   FLII=8
   FL8=FLX()
   FLII=9
   FL9=FLX()
   FLII=10
   FL10=FLX()
   FLII=11
   FL11=FLX()
   FLII=12
   FL12=FLX()
   FLII=13
   FL13=FLX()
   FLII=14
   FL14=FLX()
   FLII=15
   FL15=FLX()
   L=LN &
   &-(FTFL01(4,R_I)*FL1)&
   &-(FTFL01(5,R_I)*FL2)&
   &-(FTFL01(6,R_I)*FL3)&
   &-(FTFL01(7,R_I)*FL4)&
   &-(FTFL01(8,R_I)*FL5)&
   &-(FTFL01(9,R_I)*FL6)&
   &-(FTFL01(10,R_I)*FL7)&
   &-(FTFL01(11,R_I)*FL8)&
   &-(FTFL01(12,R_I)*FL9)&
   &-(FTFL01(13,R_I)*FL10)&
   &-(FTFL01(14,R_I)*FL11)&
   &-(FTFL01(15,R_I)*FL12)&
   &-(FTFL01(16,R_I)*FL13)&
   &-(FTFL01(17,R_I)*FL14)&
   &-(FTFL01(18,R_I)*FL15)
   FLII=1
   FL1=FLY()
   FLII=2
   FL2=FLY()
   FLII=3
   FL3=FLY()
   FLII=4
   FL4=FLY()
   FLII=5
   FL5=FLY()
   FLII=6
   FL6=FLY()
   FLII=7
   FL7=FLY()
   FLII=8
   FL8=FLY()
   FLII=9
   FL9=FLY()
   FLII=10
   FL10=FLY()
   FLII=11
   FL11=FLY()
   FLII=12
   FL12=FLY()
   FLII=13
   FL13=FLY()
   FLII=14
   FL14=FLY()
   FLII=15
   FL15=FLY()
   M=MN &
   &-(FTFL01(4,R_I)*FL1)&
   &-(FTFL01(5,R_I)*FL2)&
   &-(FTFL01(6,R_I)*FL3)&
   &-(FTFL01(7,R_I)*FL4)&
   &-(FTFL01(8,R_I)*FL5)&
   &-(FTFL01(9,R_I)*FL6)&
   &-(FTFL01(10,R_I)*FL7)&
   &-(FTFL01(11,R_I)*FL8)&
   &-(FTFL01(12,R_I)*FL9)&
   &-(FTFL01(13,R_I)*FL10)&
   &-(FTFL01(14,R_I)*FL11)&
   &-(FTFL01(15,R_I)*FL12)&
   &-(FTFL01(16,R_I)*FL13)&
   &-(FTFL01(17,R_I)*FL14)&
   &-(FTFL01(18,R_I)*FL15)
   FLII=1
   FL1=FLZ()
   FLII=2
   FL2=FLZ()
   FLII=3
   FL3=FLZ()
   FLII=4
   FL4=FLZ()
   FLII=5
   FL5=FLZ()
   FLII=6
   FL6=FLZ()
   FLII=7
   FL7=FLZ()
   FLII=8
   FL8=FLZ()
   FLII=9
   FL9=FLZ()
   FLII=10
   FL10=FLZ()
   FLII=11
   FL11=FLZ()
   FLII=12
   FL12=FLZ()
   FLII=13
   FL13=FLZ()
   FLII=14
   FL14=FLZ()
   FLII=15
   FL15=FLZ()
   N=NN &
   &-(FTFL01(4,R_I)*FL1)&
   &-(FTFL01(5,R_I)*FL2)&
   &-(FTFL01(6,R_I)*FL3)&
   &-(FTFL01(7,R_I)*FL4)&
   &-(FTFL01(8,R_I)*FL5)&
   &-(FTFL01(9,R_I)*FL6)&
   &-(FTFL01(10,R_I)*FL7)&
   &-(FTFL01(11,R_I)*FL8)&
   &-(FTFL01(12,R_I)*FL9)&
   &-(FTFL01(13,R_I)*FL10)&
   &-(FTFL01(14,R_I)*FL11)&
   &-(FTFL01(15,R_I)*FL12)&
   &-(FTFL01(16,R_I)*FL13)&
   &-(FTFL01(17,R_I)*FL14)&
   &-(FTFL01(18,R_I)*FL15)
!     NORMALS HAVE SAME SIGN AS THEY DID FOR JUST THE CONIC
!     INTERSECTION
!
   RETURN
END
SUBROUTINE FINDH
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) XO,YO,ZO,LO,MO,NO,H,ACC,RTSEC,H1,H2
   COMMON/FUNN5/XO,YO,ZO,LO,MO,NO
   COMMON/HFIND/H
   EXTERNAL RTSEC
   COMMON/SECRTER/H1,H2,ACC
   H1=(FTFL01(1,R_I)-ZO)/NO
   H2=(FTFL01(2,R_I)-ZO)/NO
   ACC=1.0D-12
   IF(DABS(SURTOL).LE.1.0D-10) ACC=SURTOL
   H=RTSEC()
   RETURN
END
FUNCTION rtsec()
   use DATLEN
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   INTEGER MAXIT
   real(real64) rtsec,x1,x2,xacc,func
   EXTERNAL func
   INTEGER j
   COMMON/SECRTER/X1,X2,XACC
!
   INTEGER SPDCD1,SPDCD2
!
   COMMON/SPRA2/SPDCD1,SPDCD2
   real(real64) dx,f,fl,swap,xl,H
   COMMON/FUNCER/H
   MAXIT=NRAITR
   H=X1
   fl=func()
   X1=H
   H=X2
   f=func()
   X2=H
   if(dabs(fl).lt.dabs(f))then
      rtsec=x1
      xl=x2
      swap=fl
      fl=f
      f=swap
   else
      xl=x1
      rtsec=x2
   endif
   do j=1,MAXIT
      dx=(xl-rtsec)*f/(f-fl)
      xl=rtsec
      fl=f
      rtsec=rtsec+dx
      H=RTSEC
      f=func()
      RTSEC=H
      if(dabs(dx).lt.xacc.or.f.eq.0.0d0)return
   end do
   IF(MSG) THEN
      CALL RAY_FAILURE(R_I)
      OUTLYNE=&
      &'CONVERGENCE FAILURE OCCURRED DURING RAY INTERSECTION'
      CALL SHOWIT(1)
      OUTLYNE='WITH DEFORMED GRAZING INCIDENCE SURFACE'
      CALL SHOWIT(1)
   END IF
   RAYCOD(1)=2
   RAYCOD(2)=R_I
   SPDCD1=RAYCOD(1)
   SPDCD2=R_I
   STOPP=1
   RETURN
END
! SUB FL.FOR
FUNCTION FL()
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) FL1,FL2,FL3,FL4,FL5,FL6,FL7,&
   &FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15 &
   &,ZETA,THETA,FL,Z,A,FLXX,FLYY
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,FLXX,FLYY
!
!       FUNCTIONS FOR TYPE 18 SURFACE, FOURIER-LEGENDRE
   FL1(Z,A)=1.0D0
   FL2(Z,A)=DSQRT(3.0D0)*Z
   FL3(Z,A)=DSQRT(5.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL4(Z,A)=DSQRT(2.0D0)*DCOS(A)
   FL5(Z,A)=DSQRT(2.0D0)*DSIN(A)
   FL6(Z,A)=DSQRT(6.0D0)*Z*DCOS(A)
   FL7(Z,A)=DSQRT(6.0D0)*Z*DSIN(A)
   FL8(Z,A)=DSQRT(10.0D0)*DCOS(A)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL9(Z,A)=DSQRT(10.0D0)*DSIN(A)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL10(Z,A)=DSQRT(2.0D0)*DCOS(2.0D0*A)
   FL11(Z,A)=DSQRT(2.0D0)*DSIN(2.0D0*A)
   FL12(Z,A)=DSQRT(6.0D0)*DCOS(2.0D0*A)*Z
   FL13(Z,A)=DSQRT(6.0D0)*DSIN(2.0D0*A)*Z
   FL14(Z,A)=DSQRT(10.0D0)*DCOS(2.0D0*A)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*DSIN(2.0D0*A)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
!

   IF(FLII.EQ.1) FL=FL1(ZETA,THETA)
   IF(FLII.EQ.2) FL=FL2(ZETA,THETA)
   IF(FLII.EQ.3) FL=FL3(ZETA,THETA)
   IF(FLII.EQ.4) FL=FL4(ZETA,THETA)
   IF(FLII.EQ.5) FL=FL5(ZETA,THETA)
   IF(FLII.EQ.6) FL=FL6(ZETA,THETA)
   IF(FLII.EQ.7) FL=FL7(ZETA,THETA)
   IF(FLII.EQ.8) FL=FL8(ZETA,THETA)
   IF(FLII.EQ.9) FL=FL9(ZETA,THETA)
   IF(FLII.EQ.10) FL=FL10(ZETA,THETA)
   IF(FLII.EQ.11) FL=FL11(ZETA,THETA)
   IF(FLII.EQ.12) FL=FL12(ZETA,THETA)
   IF(FLII.EQ.13) FL=FL13(ZETA,THETA)
   IF(FLII.EQ.14) FL=FL14(ZETA,THETA)
   IF(FLII.EQ.15) FL=FL15(ZETA,THETA)
   RETURN
END
! SUB FLX.FOR
FUNCTION FLX()
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) FL1,FL2,FL3,FL4,FL5,FL6,FL7,&
   &FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,CX,SX &
   &,AP,A,THETA,C0,C1,S0,S1,ZETA,X,Y,FLX,Z,AX,AY,P
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,X,Y
!
!       FUNCTIONS FOR TYPE 18 SURFACE, X-DERIVATIVE, FOURIER-LEGENDRE
   CX(AX,AY,AP)=(1.0D0/AP)-((AX**2)/(AP**3))
   SX(AX,AY,AP)=-(AX*AY)/(AP**3)
   FL1(Z,A)=0.0D0
   FL2(Z,A)=0.0D0
   FL3(Z,A)=0.0D0
   FL4(Z,A)=DSQRT(2.0D0)
   FL5(Z,A)=DSQRT(2.0D0)
   FL6(Z,A)=DSQRT(6.0D0)*Z
   FL7(Z,A)=DSQRT(6.0D0)*Z
   FL8(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL9(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL10(Z,A)=DSQRT(2.0D0)
   FL11(Z,A)=DSQRT(2.0D0)
   FL12(Z,A)=DSQRT(6.0D0)*Z
   FL13(Z,A)=DSQRT(6.0D0)*Z
   FL14(Z,A)=DSQRT(10.0D0)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
!
   P=DSQRT((X**2)+(Y**2))
!
   IF(P.EQ.0.0D0) RETURN
   IF(FLII.EQ.1)  FLX=0.0D0
   IF(FLII.EQ.2)  FLX=0.0D0
   IF(FLII.EQ.3)  FLX=0.0D0
   IF(FLII.EQ.4)  FLX=FL4(ZETA,THETA)*CX(X,Y,P)
   IF(FLII.EQ.5)  FLX=FL5(ZETA,THETA)*SX(X,Y,P)
   IF(FLII.EQ.6)  FLX=FL6(ZETA,THETA)*CX(X,Y,P)
   IF(FLII.EQ.7)  FLX=FL7(ZETA,THETA)*SX(X,Y,P)
   IF(FLII.EQ.8)  FLX=FL8(ZETA,THETA)*CX(X,Y,P)
   IF(FLII.EQ.9)  FLX=FL9(ZETA,THETA)*SX(X,Y,P)
   IF(FLII.EQ.10) FLX=FL10(ZETA,THETA)*4.0D0*DCOS(THETA)*CX(X,Y,P)
   IF(FLII.EQ.11) FLX=FL11(ZETA,THETA)*&
   &((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.12) FLX=FL12(ZETA,THETA)*4.0D0*DCOS(THETA)*CX(X,Y,P)
   IF(FLII.EQ.13) FLX=FL13(ZETA,THETA)*&
   &((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.14) FLX=FL14(ZETA,THETA)*4.0D0*DCOS(THETA)*CX(X,Y,P)
   IF(FLII.EQ.15) FLX=FL15(ZETA,THETA)*&
   &((2.0D0*CX(X,Y,P)*DSIN(THETA))+(2.0D0*SX(X,Y,P)*DCOS(THETA)))
   RETURN
END
! SUB FLY.FOR
FUNCTION FLY()
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) FL1,FL2,FL3,FL4,FL5,FL6,FL7,&
   &FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15,AP,P,CY,SY &
   &,A,C0,C1,S0,S1,ZETA,X,Y,FLY,Z,AX,AY,THETA
!
   INTEGER FLII
!
   COMMON/JIM/ZETA,THETA,FLII,X,Y
!
!       FUNCTIONS FOR TYPE 18 SURFACE, Y-DERIVATIVE, FOURIER-LEGENDRE
   CY(AX,AY,AP)=-(AY*AX)/(AP**3)
   SY(AX,AY,AP)=(1.0D0/AP)-((AY**2)/(AP**3))
   FL1(Z,A)=0.0D0
   FL2(Z,A)=0.0D0
   FL3(Z,A)=0.0D0
   FL4(Z,A)=DSQRT(2.0D0)
   FL5(Z,A)=DSQRT(2.0D0)
   FL6(Z,A)=DSQRT(6.0D0)*Z
   FL7(Z,A)=DSQRT(6.0D0)*Z
   FL8(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL9(Z,A)=DSQRT(10.0D0)*(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL10(Z,A)=DSQRT(2.0D0)
   FL11(Z,A)=DSQRT(2.0D0)
   FL12(Z,A)=DSQRT(6.0D0)*Z
   FL13(Z,A)=DSQRT(6.0D0)*Z
   FL14(Z,A)=DSQRT(10.0D0)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*&
   &(((3.0D0*(Z**2))-1.0D0)/2.0D0)
!
   P=DSQRT((X**2)+(Y**2))
!
   IF(P.EQ.0.0D0) THEN
      FLY=0
      RETURN
   END IF
   IF(FLII.EQ.1)  FLY=0.0D0
   IF(FLII.EQ.2)  FLY=0.0D0
   IF(FLII.EQ.3)  FLY=0.0D0
   IF(FLII.EQ.4)  FLY=FL4(ZETA,THETA)*CY(X,Y,P)
   IF(FLII.EQ.5)  FLY=FL5(ZETA,THETA)*SY(X,Y,P)
   IF(FLII.EQ.6)  FLY=FL6(ZETA,THETA)*CY(X,Y,P)
   IF(FLII.EQ.7)  FLY=FL7(ZETA,THETA)*SY(X,Y,P)
   IF(FLII.EQ.8)  FLY=FL8(ZETA,THETA)*CY(X,Y,P)
   IF(FLII.EQ.9)  FLY=FL9(ZETA,THETA)*SY(X,Y,P)
   IF(FLII.EQ.10) FLY=FL10(ZETA,THETA)*4.0D0*DCOS(THETA)*CY(X,Y,P)
   IF(FLII.EQ.11) FLY=FL11(ZETA,THETA)*&
   &((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.12) FLY=FL12(ZETA,THETA)*4.0D0*DCOS(THETA)*CY(X,Y,P)
   IF(FLII.EQ.13) FLY=FL13(ZETA,THETA)*&
   &((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   IF(FLII.EQ.14) FLY=FL14(ZETA,THETA)*4.0D0*DCOS(THETA)*CY(X,Y,P)
   IF(FLII.EQ.15) FLY=FL15(ZETA,THETA)*&
   &((2.0D0*CY(X,Y,P)*DSIN(THETA))+(2.0D0*SY(X,Y,P)*DCOS(THETA)))
   RETURN
END
! SUB FLZ.FOR
FUNCTION FLZ()
!
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) FL1,FL2,FL3,FL4,FL5,FL6,FL7,&
   &FL8,FL9,FL10,FL11,FL12,FL13,FL14,FL15 &
   &,ZETA,THETA,FLZ,Z,A,X,Y,P,ZETAD
!
   COMMON/ZEETAA/ZETAD

   COMMON/JIM/ZETA,THETA,FLII,X,Y
!
   INTEGER FLII
!
!       FUNCTIONS FOR TYPE 18 SURFACE, Z-DERIVATIVE, FOURIER-LEGENDRE
   FL1(Z,A)=0.0D0
   FL2(Z,A)=DSQRT(3.0D0)
   FL3(Z,A)=DSQRT(5.0D0)*(((6.0D0*(Z)))/2.0D0)
   FL4(Z,A)=0.0D0
   FL5(Z,A)=0.0D0
   FL6(Z,A)=DSQRT(6.0D0)*DCOS(A)
   FL7(Z,A)=DSQRT(6.0D0)*DSIN(A)
   FL8(Z,A)=DSQRT(10.0D0)*DCOS(A)*(((6.0D0*(Z)))/2.0D0)
   FL9(Z,A)=DSQRT(10.0D0)*DSIN(A)*(((6.0D0*(Z)))/2.0D0)
   FL10(Z,A)=0.0D0
   FL11(Z,A)=0.0D0
   FL12(Z,A)=DSQRT(6.0D0)*DCOS(2.0D0*A)
   FL13(Z,A)=DSQRT(6.0D0)*DSIN(2.0D0*A)
   FL14(Z,A)=DSQRT(10.0D0)*DCOS(2.0D0*A)*&
   &(((6.0D0*(Z)))/2.0D0)
   FL15(Z,A)=DSQRT(10.0D0)*DSIN(2.0D0*A)*&
   &(((6.0D0*(Z)))/2.0D0)
!
   P=DSQRT((X**2)+(Y**2))
   IF(P.EQ.0.0D0) RETURN
   IF(FLII.EQ.1) FLZ=0.0D0
   IF(FLII.EQ.2) FLZ=(FL2(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.3) FLZ=(FL3(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.4) FLZ=0.0D0
   IF(FLII.EQ.5) FLZ=0.0D0
   IF(FLII.EQ.6) FLZ=(FL6(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.7) FLZ=(FL7(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.8) FLZ=(FL8(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.9) FLZ=(FL9(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.10) FLZ=0.0D0
   IF(FLII.EQ.11) FLZ=0.0D0
   IF(FLII.EQ.12) FLZ=(FL12(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.13) FLZ=(FL13(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.14) FLZ=(FL14(ZETA,THETA)*2.0D0)/ZETAD
   IF(FLII.EQ.15) FLZ=(FL15(ZETA,THETA)*2.0D0)/ZETAD
   RETURN
END
! SUB INTERACK.FOR
SUBROUTINE INTERACK(OR_N,OR_Z)
   USE GLOBALS
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_units, sys_wavelength
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     ERROR RAY VARIABLES 3/5/2006
   real(real64) ORIG_X,ORIG_Y,DELTAR
   real(real64) DOUB,RANDMM,DELTAX,DELTAY
   EXTERNAL RANDMM
   real(real64) RRR9,RRR10
   COMMON/RRRS/RRR9,RRR10
!
   real(real64) A,B,C,QQ,SIGNNU,OR_N,OR_Z,PHA11,MAGO,MAGR,&
   &TEST1,TEST2,NUSUBS,TIRTESTER,SINI,RR_N,TESTLEN,RR_Z &
   &,Q,SIGNB,MAG,J,ARG,C1,C2,C3,C4,SNIND2 &
   &,XX,YY,FPX,FPY,FPZ,C5,C6,ZMIN,ZMAX,AX,AY,AZ,&
   &X1,X2,Y1,Y2,Z1,Z2,LN1,LN2,MN1,MN2,NN1,NN2 &
   &,SGNB,SNINDX,JIM,UX,UY,UXP,UYP,INEW,L1,M1,N1
!
   INTEGER ZPMIN,ZPMAX,ISURF
!
!     DIFFRACTION GRATING STUFF
   real(real64) CHIX,CHIY,CHIZ,CHINOR,BLAM,CTERM,BTERM,FACTOR,&
   &SMU,WLU,BGAM,DD,DSPACE,QQUU,GAM1,GAM2,&
   &QX,QY,QZ,PX,PY,PZ,PNOR,QNOR
!
!     HOE STUFF
   real(real64) LO,MO,NO,LR,MR,NR,LAMC,LAMP,EMM,BTA
   real(real64) GRO,GRS,GRX,GRY,GRZ
!
   LOGICAL REFLEC,GERROR
!
   INTEGER SPDCD1,SPDCD2
   COMMON/SPRA2/SPDCD1,SPDCD2
!
   real(real64) XPASS,YPASS,ZPASS
!
   COMMON/SAGPAS/XPASS,YPASS,ZPASS
!
   LOGICAL TIR
   COMMON/RIT/TIR
!
   TIR=.FALSE.
!
!     TYPE 13 SETUP
   IF(surf_special_type(R_I) == 13) THEN
!     SET UP THE STARTING RAY COORDINATED ON THE OBJECT
!     SURFACES IN THE TWO CFGS WHERE RAYS ARE TRACED
      HOE_X1=FTFL01(3,R_I)
      HOE_Y1=FTFL01(4,R_I)
      HOE_Z1=FTFL01(5,R_I)
      HOE_X2=FTFL01(8,R_I)
      HOE_Y2=FTFL01(9,R_I)
      HOE_Z2=FTFL01(10,R_I)
   END IF


   DD=0.0D0
   BTERM=0.0D0
   CTERM=0.0D0
   R_L0=R_L
   R_M0=R_M
   R_N0=R_N
!******THE COSINE SQUARED OF THE ANGLE OF INCIDENCE IS JUST
   COSI=(LN*R_L)+(MN*R_M)+(NN*R_N)
   IF(COSI.LT.-1.0D0) COSI=-1.0D0
   IF(COSI.GT.+1.0D0) COSI=+1.0D0
   PHASE=0.0D0
   RR_Z=OR_Z
   RR_N=OR_N
   SNINDX=DABS(ldm%getSurfIndex(R_I-1, INT(WVN)))/ldm%getSurfIndex(R_I-1, INT(WVN))
   IF((1.0D0-(COSI**2)).LT.0.0D0) THEN
      SINI=0.0D0
   ELSE
      SINI=DSQRT(1.0D0-(COSI**2))
   END IF
   TIRTESTER=SINI*SNINDX
   IF(TIRTESTER.GT.1.0D0) TIR=.TRUE.
!     TIRTESTER IS USED DURING A REFLECTION SPECIFIED WITH A REFLTIRO
!     THIS MEANS THAT THE REFLECTION ONLY WORKS IF TIRTESTER>1.0
   SNIND2=DABS(ldm%getSurfIndex(R_I, INT(WVN)))/ldm%getSurfIndex(R_I, INT(WVN))
   NUSUBS=((ldm%getSurfIndex(R_I-1, INT(WVN)))/&
   &(ldm%getSurfIndex(R_I, INT(WVN))))
   SIGNNU=DABS(NUSUBS)/NUSUBS
   NUSUBS=DABS(NUSUBS)
   NUSUBS=DABS(NUSUBS)
   SMU=NUSUBS
   IF(surf_diffraction_flag(R_I) == 1) THEN
      GRO=surf_grating_order(R_I)
      GRS=surf_grating_spacing(R_I)
      GRX=surf_grating_vx(R_I)
      GRY=surf_grating_vy(R_I)
      GRZ=surf_grating_vz(R_I)
   ELSE
      GRO=0.0D0
      GRS=0.001D0
      GRX=0.0D0
      GRY=1.0D0
      GRZ=0.0D0
   END IF
!
!****************PERFECT LENS******************************************
!
   IF(GLANAM(R_I,2).EQ.'PERFECT      ') THEN
!     COMPUTE THE DIRECTION COSINES OF THE RAY
!     FOR A PERFECT LENS WITH EFL = surf_thickness(R_I)
      IF(R_L.EQ.0.0D0) THEN
         IF(R_N.GE.0.0D0) UX=0.0D0
         IF(R_N.LT.0.0D0) UX=PII
      ELSE
         IF(DABS(R_L).GE.DABS(1.0D35*R_N)) THEN
            IF(R_L.GE.0.0D0) UX=PII/2.0D0
            IF(R_L.LT.0.0D0) UX=(3.0D0*PII)/2.0D0
         ELSE
            IF(DABS(R_L).EQ.0.0D0.AND.DABS(R_N).EQ.0.0D0) THEN
               UX=0.0D0
            ELSE
               UX=DATAN2(R_L,R_N)
            END IF
         END IF
      END IF
      IF(R_M.EQ.0.0D0) THEN
         IF(R_N.GE.0.0D0) UY=0.0D0
         IF(R_N.LT.0.0D0) UY=PII
      ELSE
         IF(DABS(R_M).GE.DABS(1.0D35*R_N)) THEN
            IF(R_M.GE.0.0D0) UY=PII/2.0D0
            IF(R_M.LT.0.0D0) UY=(3.0D0*PII)/2.0D0
         ELSE
            IF(DABS(R_M).EQ.0.0D0.AND.DABS(R_N).EQ.0.0D0) THEN
               UY=0.0D0
            ELSE
               UY=DATAN2(R_M,R_N)
            END IF
         END IF
      END IF
      IF(UX.GT.(PII/2.0D0).AND.UX.LE.PII) THEN
         UX=-(PII-UX)
      END IF
      IF(UX.LT.(-PII/2.0D0).AND.UX.GE.-PII) THEN
         UX=(PII+UX)
      END IF
      IF(UY.GT.(PII/2.0D0).AND.UY.LE.PII) THEN
         UY=-(PII-UY)
      END IF
      IF(UY.LT.(-PII/2.0D0).AND.UY.GE.-PII) THEN
         UY=(PII+UY)
      END IF
      UXP=-((R_X/(surf_thickness(R_I)))-UX)
      UYP=-((R_Y/(surf_thickness(R_I)))-UY)
      INEW=surf_thickness(R_I)
      R_L=UXP*INEW
      R_M=UYP*INEW
      R_N=INEW
      MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
      R_L=R_L/MAG
      R_M=R_M/MAG
      R_N=R_N/MAG
      GO TO 90
   END IF
!****************PERFECT LENS DONE*************************************
!****************IDEAL LENS******************************************
!
   IF(GLANAM(R_I,2).EQ.'IDEAL        ') THEN
!     COMPUTE THE DIRECTION COSINES OF THE RAY
!     FOR A PERFECT LENS WITH EFL = surf_ideal_efl(R_I)
      IF(R_L.EQ.0.0D0) THEN
         IF(R_N.GE.0.0D0) UX=0.0D0
         IF(R_N.LT.0.0D0) UX=PII
      ELSE
         IF(DABS(R_L).GE.DABS(1.0D35*R_N)) THEN
            IF(R_L.GE.0.0D0) UX=PII/2.0D0
            IF(R_L.LT.0.0D0) UX=(3.0D0*PII)/2.0D0

         ELSE
            IF(DABS(R_L).EQ.0.0D0.AND.DABS(R_N).EQ.0.0D0) THEN
               UX=0.0D0
            ELSE
               UX=DATAN2(R_L,R_N)
            END IF
         END IF
      END IF
      IF(R_M.EQ.0.0D0) THEN
         IF(R_N.GE.0.0D0) UY=0.0D0
         IF(R_N.LT.0.0D0) UY=PII
      ELSE
         IF(DABS(R_M).GE.DABS(1.0D35*R_N)) THEN
            IF(R_M.GE.0.0D0) UY=PII/2.0D0
            IF(R_M.LT.0.0D0) UY=(3.0D0*PII)/2.0D0
         ELSE
            IF(DABS(R_M).EQ.0.0D0.AND.DABS(R_N).EQ.0.0D0) THEN
               UY=0.0D0
            ELSE
               UY=DATAN2(R_M,R_N)
            END IF
         END IF
      END IF
      IF(UX.GT.(PII/2.0D0).AND.UX.LE.PII) THEN
         UX=-(PII-UX)
      END IF
      IF(UX.LT.(-PII/2.0D0).AND.UX.GE.-PII) THEN
         UX=(PII+UX)
      END IF
      IF(UY.GT.(PII/2.0D0).AND.UY.LE.PII) THEN
         UY=-(PII-UY)
      END IF
      IF(UY.LT.(-PII/2.0D0).AND.UY.GE.-PII) THEN
         UY=(PII+UY)
      END IF
      UXP=-((R_X/(surf_ideal_efl(R_I)))-UX)
      UYP=-((R_Y/(surf_ideal_efl(R_I)))-UY)
      INEW=surf_thickness(R_I)
      R_L=UXP*INEW
      R_M=UYP*INEW
      R_N=INEW
      MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
      R_L=R_L/MAG
      R_M=R_M/MAG
      R_N=R_N/MAG
      GO TO 90
   END IF
!****************PERFECT LENS DONE*************************************
!
   IF(STOPP.EQ.1) RETURN
!
!       THE RETURNED VALUES OF X,Y,Z ARE THE CORRECT INTERSECTIONS
!       TO WITHIN THE SURTOL VALUE
!
!
   IF(surf_diffraction_flag(R_I) == 1.AND..NOT.DUM(R_I).AND.surf_grating_spacing(R_I)&
   &.NE.0.0D0) THEN
!
!     GRATING PRE-CALCULATIONS (A REAL LINEAR GRATING)
!
!     WE HAVE A GRATING, THINGS NEED TO BE CALCULATED
!     CALCULATE THE DIRCTION COSINES OF THE UNIT VECTOR NORMAL
!     TO THE GENERATING PLANES
!     MAKE SURE ALL DIRCOS NOT ZERO
      IF(GRX.EQ.0.0D0.AND.GRY.EQ.0.0D0 &
      &.AND.GRZ.EQ.0.0D0) GRY=1.0D0
!
      CHINOR=DSQRT((GRX**2)+(GRY**2)+&
      &(GRZ**2))
      CHIX=GRX/CHINOR
      CHIY=GRY/CHINOR
      CHIZ=GRZ/CHINOR
!     CALCULATE THE COMPONENTS OF THE VECTOR Q WHICH IS PARALLEL
!     TO THE RULINGS
      QX=-(CHIY*NN)+(CHIZ*MN)
      QY=-(CHIZ*LN)+(CHIX*NN)
      QZ=-(CHIX*MN)+(CHIY*LN)
      QNOR=DSQRT((QX**2)+(QY**2)+(QZ**2))
      QX=QX/QNOR
      QY=QY/QNOR
      QZ=QZ/QNOR
!     CALCULATE THE COMPONENTS OF THE VECTOR P WHICH IS NORMAL
!     TO THE RULINGS
      PX=(QY*NN)-(QZ*MN)
      PY=(QZ*LN)-(QX*NN)
      PZ=(QX*MN)-(QY*LN)
      PNOR=DSQRT((PX**2)+(PY**2)+(PZ**2))
      PX=PX/PNOR
      PY=PY/PNOR
      PZ=PZ/PNOR
      FACTOR=(CHIX*PX)+(CHIY*PY)+(CHIZ*PZ)
!
      IF(DABS(FACTOR).GT.1E-6) DSPACE=GRS/FACTOR
      IF(DABS(FACTOR).LE.1E-6) DSPACE=1.0D20
!     NOW CALC BIG GAMMA AND SMALL MU
!      WLU=WAVELENGTH IN MICRONS*(LENS UNITS/MICRON)
      IF(sys_units().EQ.1.0D0)&
      &WLU=sys_wavelength(INT(WVN))*3.93700787402D-5
      IF(sys_units().EQ.2.0D0)&
      &WLU=sys_wavelength(INT(WVN))*1.0D-4
      IF(sys_units().EQ.3.0D0)&
      &WLU=sys_wavelength(INT(WVN))*1.0D-3
      IF(sys_units().EQ.4.0D0)&
      &WLU=sys_wavelength(INT(WVN))*1.0D-6
!
      BLAM=-(GRO*WLU)/((DSPACE)*DABS(ldm%getSurfIndex(R_I, INT(WVN))))
!
      BTERM=2.0D0*SMU*((R_L*LN)+(R_M*MN)+(R_N*NN))
      CTERM=(SMU**2)-1.0D0+(BLAM**2)&
      &-((2.0*SMU*BLAM)*((R_L*PX)+(R_M*PY)+(R_N*PZ)))
      DD=(BTERM**2)-(4.0D0*CTERM)
      IF(SIGNNU.LT.0.0D0) THEN
!
!                   WE HAVE A REFLECTION GRATING
!
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE GRATING IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=5
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         IF(surf_reflection_mode(R_I) == 1.AND.TIRTESTER.GT.1.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'TIR CONDITION NOT MET'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=20
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
         BGAM=GAM1
!
      ELSE
!
!     WE HAVE A TRANSMISSION GRATING
!
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE GRATING IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=5
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0D0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
         IF(GAM1.EQ.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE GRATING IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=5
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         BGAM=CTERM/GAM1

      END IF
   ELSE
!     NOT A GRATING
      BLAM=0.0D0
!***************NON-GRATING PRE-CALCULATIONS***************************
!
      GRX=0.0D0
      GRY=0.0D0
      GRZ=0.0D0
      PX=0.0D0
      PY=0.0D0
      PZ=0.0D0
      BLAM=0.0D0
!     NOW CALC BIG GAMMA AND SMALL MU
      SMU=NUSUBS
!
      BTERM=2.0D0*SMU*((R_L*LN)+(R_M*MN)+(R_N*NN))
      CTERM=(SMU**2)-1.0D0+(BLAM**2)&
      &-((2.0*SMU*BLAM)*((R_L*PX)+(R_M*PY)+(R_N*PZ)))
      DD=(BTERM**2)-(4.0D0*CTERM)
      IF(SIGNNU.LT.0.0D0) THEN
!
!                   A REFLECTION
!
         IF(surf_reflection_mode(R_I) == 1.AND.TIRTESTER.GT.1.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'TIR CONDITION NOT MET'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=20
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0D0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
         BGAM=GAM1
!
      ELSE
!
!     WE HAVE A REFRACTION
!
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'TOTAL INTERNAL REFLECTION'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=4
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0D0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
         IF(GAM1.EQ.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'TOTAL INTERNAL REFLECTION'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=4
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         BGAM=CTERM/GAM1

      END IF
   END IF
!     ************************************************************************
!
   IF(surf_special_type(R_I) == 12.OR.surf_special_type(R_I) == 13) THEN
!     HOE PRECALCULATIONS
      EMM=FTFL01(1,R_I)
      LAMC=FTFL01(2,R_I)
      LAMP=sys_wavelength(INT(WVN))
!
      IF(surf_special_type(R_I) == 12) THEN
         LO=FTFL01(3,R_I)-R_X
         MO=FTFL01(4,R_I)-R_Y
         NO=FTFL01(5,R_I)-R_Z
         IF(FTFL01(6,R_I).GT.0.0D0.AND.NO.GT.0.0D0.OR.&
         &FTFL01(6,R_I).LT.0.0D0.AND.NO.LT.0.0D0) THEN
            LO=-LO
            MO=-MO
            NO=-NO
         END IF
      END IF
      IF(surf_special_type(R_I) == 13) THEN
         LO=L1HOE
         MO=M1HOE
         NO=N1HOE
         IF(FTFL01(6,R_I).LT.0.0D0) THEN
            NO=-NO
         END IF
         IF(FTFL01(7,R_I).GT.0.0D0.AND.NO.GT.0.0D0.OR.&
         &FTFL01(7,R_I).LT.0.0D0.AND.NO.LT.0.0D0) THEN
            LO=-LO
            MO=-MO
            NO=-NO
         END IF
      END IF
      MAGO=DSQRT((LO**2)+(MO**2)+(NO**2))
      IF(MAGO.NE.0.0D0) THEN
         LO=LO/MAGO
         MO=MO/MAGO
         NO=NO/MAGO
      END IF
!
      IF(surf_special_type(R_I) == 12) THEN
         LR=FTFL01(7,R_I)-R_X
         MR=FTFL01(8,R_I)-R_Y
         NR=FTFL01(9,R_I)-R_Z
         IF(FTFL01(10,R_I).GT.0.0D0.AND.NR.GT.0.0D0.OR.&
         &FTFL01(10,R_I).LT.0.0D0.AND.NR.LT.0.0D0) THEN
            LR=-LR
            MR=-MR
            NR=-NR
         END IF
      END IF
      IF(surf_special_type(R_I) == 13) THEN
         LR=L2HOE
         MR=M2HOE
         NR=N2HOE
         IF(FTFL01(11,R_I).LT.0.0D0) THEN
            NR=-NR
         END IF
         IF(FTFL01(12,R_I).GT.0.0D0.AND.NR.GT.0.0D0.OR.&
         &FTFL01(12,R_I).LT.0.0D0.AND.NR.LT.0.0D0) THEN
            LR=-LR
            MR=-MR
            NR=-NR
         END IF
      END IF
      MAGR=DSQRT((LR**2)+(MR**2)+(NR**2))
      IF(MAGR.NE.0.0D0) THEN
         LR=LR/MAGR
         MR=MR/MAGR
         NR=NR/MAGR
      END IF
!     BTA
      BTA=(EMM*LAMP)/(LAMC*DABS(ldm%getSurfIndex(R_I, INT(WVN))))
      AX=BTA*(LO-LR)
      AY=BTA*(MO-MR)
      AZ=BTA*(NO-NR)
!     IS THERE PHASE FROM A TYPE 12 HOE TYPE SURFACE ?
      XX=R_X
      YY=R_Y
      MYI=R_I
      L1=0.0D0
      M1=0.0D0
      IF(FTFL01(11,R_I).EQ.1.0D0.OR.FTFL01(11,R_I).EQ.6.0D0)&
      &CALL PH12(L1,M1,XX,YY)
      IF(FTFL01(11,R_I).EQ.7.0D0) CALL PH11(L1,M1,XX,YY,PHA11)
      IF(FTFL01(11,R_I).EQ.2.0D0.OR.FTFL01(11,R_I).EQ.3.0D0.OR.&
      &FTFL01(11,R_I).EQ.4.0D0.OR.FTFL01(11,R_I).EQ.5.0D0)&
      &CALL PH13(L1,M1,XX,YY)
      GRX=-L1
      GRY=-M1
      GRZ=0.0D0
      IF(GRX.EQ.0.0D0.AND.GRY.EQ.0.0D0) THEN
         BLAM=0.0D0
      ELSE
         CHINOR=DSQRT((GRX**2)+(GRY**2)+&
         &(GRZ**2))
         CHIX=GRX/CHINOR
         CHIY=GRY/CHINOR
         CHIZ=GRZ/CHINOR
!     CALCULATE THE COMPONENTS OF THE VECTOR Q WHICH IS PARALLEL
!     TO THE RULINGS
         QX=-(CHIY*NN)+(CHIZ*MN)
         QY=-(CHIZ*LN)+(CHIX*NN)
         QZ=-(CHIX*MN)+(CHIY*LN)
         QNOR=DSQRT((QX**2)+(QY**2)+(QZ**2))
         QX=QX/QNOR
         QY=QY/QNOR
         QZ=QZ/QNOR
!     CALCULATE THE COMPONENTS OF THE VECTOR P WHICH IS NORMAL
!     TO THE RULINGS
         PX=(QY*NN)-(QZ*MN)
         PY=(QZ*LN)-(QX*NN)
         PZ=(QX*MN)-(QY*LN)
         PNOR=DSQRT((PX**2)+(PY**2)+(PZ**2))
         PX=PX/PNOR
         PY=PY/PNOR
         PZ=PZ/PNOR
         FACTOR=(CHIX*PX)+(CHIY*PY)+(CHIZ*PZ)
!
!     NOW CALC BIG GAMMA AND SMALL MU
!      WLU=WAVELENGTH IN MICRONS*(LENS UNITS/MICRON)
         IF(sys_units().EQ.1.0D0)&
         &WLU=FTFL01(2,R_I)*3.93700787402D-5
         IF(sys_units().EQ.2.0D0)&
         &WLU=FTFL01(2,R_I)*1.0D-4
         IF(sys_units().EQ.3.0D0)&
         &WLU=FTFL01(2,R_I)*1.0D-3
         IF(sys_units().EQ.4.0D0)&
         &WLU=FTFL01(2,R_I)*1.0D-6
!
         GRS=WLU/CHINOR
         IF(DABS(FACTOR).GT.1E-6) DSPACE=GRS/FACTOR
         IF(DABS(FACTOR).LE.1E-6) DSPACE=1.0D20
         GRO=FTFL01(1,R_I)
         BLAM=(GRO*WLU)/((DSPACE)*DABS(ldm%getSurfIndex(R_I, INT(WVN))))
      END IF
!     NOW CALC BIG GAMMA AND SMALL MU
      SMU=NUSUBS
!
      BTERM=2.0D0*(SMU*((R_L*LN)+(R_M*MN)+(R_N*NN)))
      CTERM=((SMU**2)+((AX**2)+(AY**2)+(AZ**2)))&
      &+((BLAM**2)*((PX**2)+(PY**2)+(PZ**2)))&
      &-(2.0D0*SMU*((R_L*AX)+(R_M*AY)+(R_N*AZ)))&
      &-(2.0D0*SMU*BLAM*((R_L*PX)+(R_M*PY)+(R_N*PZ)))&
      &+(2.0D0*BLAM*((AX*PX)+(AY*PY)+(AZ*PZ)))&
      &-1.0D0
      DD=(BTERM**2)-(4.0D0*CTERM)
      IF(SIGNNU.LT.0.0D0) THEN
!     WE HAVE A REFLECTION HOE
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE HOE IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=9
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         IF(surf_reflection_mode(R_I) == 1.AND.TIRTESTER.GT.1.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'TIR CONDITION NOT MET'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=20
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0D0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
!     CASE OF A REFLECTION HOE, TAKE THE LARGER ROOT
         BGAM=GAM1
      ELSE
!     WE HAVE A TRANSMISSION HOE
         IF(DD.LT.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE HOE IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=9
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         SGNB=1.0D0
         IF(BTERM.NE.0.0D0) SGNB=BTERM/DABS(BTERM)
         QQUU=-0.5D0*(BTERM+(SGNB*DSQRT(DD)))
         GAM1=QQUU
         IF(GAM1.EQ.0.0D0) THEN
!       RAY ERROR OCCURRED, PRINT MESSAGE AND STOP TRACING
            IF(MSG) THEN
               CALL RAY_FAILURE(R_I)
               OUTLYNE=&
               &'ANGLE OF DIFFRACTION AT THE HOE IS PHYSICALLY UNREALIZABLE'
               CALL SHOWIT(1)
            END IF
            RAYCOD(1)=9
            RAYCOD(2)=R_I
            SPDCD1=RAYCOD(1)
            SPDCD2=R_I
            STOPP=1
            RETURN
!       NO RAY ERROR
         END IF
         BGAM=CTERM/GAM1
!     NOT A HOE, NO PRECALCULATIONS
      END IF
   END IF
!     END OF HOE PRECALCULATOIONS
!     ***********************************************************************
!
!     NOW DEFLECT THE RAY
!
   IF(.NOT.DUM(R_I)) THEN
!       NOT A DUMMY SURFACE
      IF(SIGNNU.LT.0.0D0) THEN
!
!                            REFLECTION
!
         IF(surf_special_type(R_I) /= 12.AND.surf_special_type(R_I) /= 13) THEN
!     NO HOE
!     GRATING
            R_L=(SMU*R_L)-(BLAM*PX)+(BGAM*LN)
            R_M=(SMU*R_M)-(BLAM*PY)+(BGAM*MN)
            R_N=(SMU*R_N)-(BLAM*PZ)+(BGAM*NN)
!       RE-NORMALIZE
            MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
            R_L=R_L/MAG
            R_M=R_M/MAG
            R_N=R_N/MAG
            R_L0=(R_L0-((2.0D0*COSI)*LN))
            R_M0=(R_M0-((2.0D0*COSI)*MN))
            R_N0=(R_N0-((2.0D0*COSI)*NN))
            MAG=DSQRT((R_L0**2)+(R_M0**2)+(R_N0**2))
            R_L0=R_L0/MAG
            R_M0=R_M0/MAG
            R_N0=R_N0/MAG
            COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
            IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
            IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
            IF(surf_diffraction_flag(R_I) == 1.AND.surf_grating_spacing(R_I).NE.0.0D0 &
            &.OR.surf_special_type(R_I) == 12.OR.&
            &surf_special_type(R_I) == 13)&
            &CALL DPHASE
         END IF
         IF(surf_special_type(R_I) == 12.OR.surf_clap_dim(R_I, 4).EQ.13.0D0) THEN
!     HOE
            R_L=((SMU*R_L)-(AX)-(BLAM*PX)+(BGAM*LN))
            R_M=((SMU*R_M)-(AY)-(BLAM*PY)+(BGAM*MN))
            R_N=((SMU*R_N)-(AZ)-(BLAM*PZ)+(BGAM*NN))
!       RE-NORMALIZE
            MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
            R_L=R_L/MAG
            R_M=R_M/MAG
            R_N=R_N/MAG
            R_L0=(R_L0-((2.0D0*COSI)*LN))
            R_M0=(R_M0-((2.0D0*COSI)*MN))
            R_N0=(R_N0-((2.0D0*COSI)*NN))
            MAG=DSQRT((R_L0**2)+(R_M0**2)+(R_N0**2))
            R_L0=R_L0/MAG
            R_M0=R_M0/MAG
            R_N0=R_N0/MAG
            COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
            IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
            IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
            IF(surf_diffraction_flag(R_I) == 1.AND.surf_grating_spacing(R_I).NE.0.0D0 &
            &.OR.surf_special_type(R_I) == 12.OR.&
            &surf_special_type(R_I) == 13)&
            &CALL DPHASE
         END IF
      ELSE
!
!                NOT REFLECTION, PROCEED
!
         IF(surf_special_type(R_I) /= 12.AND.surf_special_type(R_I) /= 13) THEN
!     NO HOE
!     GRATING
            R_L=(SMU*R_L)-(BLAM*PX)+(BGAM*LN)
            R_M=(SMU*R_M)-(BLAM*PY)+(BGAM*MN)
            R_N=(SMU*R_N)-(BLAM*PZ)+(BGAM*NN)
            MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
            R_L=R_L/MAG
            R_M=R_M/MAG
            R_N=R_N/MAG
            ARG=(1.0D0-(NUSUBS**2)*(1.0D0-(COSI**2)))
            IF(ARG.LT.0.0D0) THEN
!       TIR OCCURRED, PRINT MESSAGE AND STOP TRACING
               TIR=.TRUE.
               IF(MSG) THEN
                  CALL RAY_FAILURE(R_I)
                  OUTLYNE='TOTAL INTERNAL REFLECTION'
                  CALL SHOWIT(1)
               END IF
               RAYCOD(1)=4
               RAYCOD(2)=R_I
               SPDCD1=RAYCOD(1)
               SPDCD2=R_I
               STOPP=1
               RETURN
!       NO TIR
            END IF
            IF(COSI.LE.0.0D0) COSIP=-DSQRT(ARG)
            IF(COSI.GT.0.0D0) COSIP=DSQRT(ARG)
            J=((DABS(ldm%getSurfIndex(R_I, INT(WVN)))*COSIP)&
            &-(DABS(ldm%getSurfIndex(R_I-1, INT(WVN)))*COSI))/&
            &(DABS(ldm%getSurfIndex(R_I, INT(WVN))))
            R_L0=(NUSUBS*R_L0)+(J*LN)
            R_M0=(NUSUBS*R_M0)+(J*MN)
            R_N0=(NUSUBS*R_N0)+(J*NN)
!       RE-NORMALIZE
            MAG=DSQRT((R_L0**2)+(R_M0**2)+(R_N0**2))
            R_L0=R_L0/MAG
            R_M0=R_M0/MAG
            R_N0=R_N0/MAG
            COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
            IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
            IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
            IF(surf_diffraction_flag(R_I) == 1.AND.surf_grating_spacing(R_I).NE.0.0D0 &
            &.OR.surf_special_type(R_I) == 12.OR.&
            &surf_special_type(R_I) == 13)&
            &CALL DPHASE
         END IF
         IF(surf_special_type(R_I) == 12.OR.surf_special_type(R_I) == 13) THEN
!     HOE
            R_L=((SMU*R_L)-(AX)-(BLAM*PX)+(BGAM*LN))
            R_M=((SMU*R_M)-(AY)-(BLAM*PY)+(BGAM*MN))
            R_N=((SMU*R_N)-(AZ)-(BLAM*PZ)+(BGAM*NN))
!       RE-NORMALIZE
            MAG=DSQRT((R_L**2)+(R_M**2)+(R_N**2))
            R_L=R_L/MAG
            R_M=R_M/MAG
            R_N=R_N/MAG
            R_L0=(R_L0-((2.0D0*COSI)*LN))
            R_M0=(R_M0-((2.0D0*COSI)*MN))
            R_N0=(R_N0-((2.0D0*COSI)*NN))
            MAG=DSQRT((R_L0**2)+(R_M0**2)+(R_N0**2))
            R_L0=R_L0/MAG
            R_M0=R_M0/MAG
            R_N0=R_N0/MAG
            COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
            IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
            IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
            IF(ARG.LT.0.0D0) THEN
!       TIR OCCURRED, PRINT MESSAGE AND STOP TRACING
               TIR=.TRUE.
               IF(MSG) THEN
                  CALL RAY_FAILURE(R_I)
                  OUTLYNE='TOTAL INTERNAL REFLECTION'
                  CALL SHOWIT(1)
               END IF
               RAYCOD(1)=4
               RAYCOD(2)=R_I
               SPDCD1=RAYCOD(1)
               SPDCD2=R_I
               STOPP=1
               RETURN
!       NO TIR
            END IF
            IF(COSI.LE.0.0D0) COSIP=-DSQRT(ARG)
            IF(COSI.GT.0.0D0) COSIP=DSQRT(ARG)
            J=((DABS(ldm%getSurfIndex(R_I, INT(WVN)))*COSIP)&
            &-(DABS(ldm%getSurfIndex(R_I-1, INT(WVN)))*COSI))/&
            &(DABS(ldm%getSurfIndex(R_I, INT(WVN))))
            R_L0=(NUSUBS*R_L0)+(J*LN)
            R_M0=(NUSUBS*R_M0)+(J*MN)
            R_N0=(NUSUBS*R_N0)+(J*NN)
!       RE-NORMALIZE
            MAG=DSQRT((R_L0**2)+(R_M0**2)+(R_N0**2))
            R_L0=R_L0/MAG
            R_M0=R_M0/MAG
            R_N0=R_N0/MAG
            COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
            IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
            IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
            IF(surf_diffraction_flag(R_I) == 1.AND.surf_grating_spacing(R_I).NE.0.0D0 &
            &.OR.surf_special_type(R_I) == 12.OR.&
            &surf_special_type(R_I) == 13)&
            &CALL DPHASE
         END IF
      END IF
!     DUM
   END IF
!
!     NOW ADD APPROPRIATE PHASE
!
!     IF A PHASE SURFACE, FIX THE PHASE AND DIRCOS
   IF(surf_special_type(R_I) == 6.OR.surf_special_type(R_I) == 7 &
   &.OR.surf_special_type(R_I) == 9.OR.surf_special_type(R_I) == 10 &
   &.OR.surf_special_type(R_I) == 11.OR.surf_special_type(R_I) == 15) THEN
      R_L0=R_L
      R_M0=R_M
      R_N0=R_N
      RLRL=R_L
      RMRM=R_M
      RNRN=R_N
      MYX=R_X
      MYY=R_Y
      MYI=R_I
      PHASE=0.0D0
      INR=surf_inr_value(R_I)
      CALL PHASOR
      R_L=RLRL
      R_M=RMRM
      R_N=RNRN
      COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
      IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
      IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
   END IF
!     IF A TYPE 12 SURFACE, FIX THE PHASE AND DIRCOS
   IF(surf_special_type(R_I) == 12.AND.FTFL01(11,R_I).EQ.1.0D0.OR.&
   &surf_special_type(R_I) == 13.AND.FTFL01(11,R_I).EQ.1.0D0) THEN
      MYX=R_X
      MYY=R_Y
      MYI=R_I
      PHASE=0.0D0
      IF(FTFL01(11,R_I).EQ.1.0D0.OR.FTFL01(11,R_I).EQ.6.0D0)&
      &CALL PHAS12
      IF(FTFL01(11,R_I).EQ.2.0D0.OR.FTFL01(11,R_I).EQ.3.0D0.OR.&
      &FTFL01(11,R_I).EQ.4.0D0.OR.FTFL01(11,R_I).EQ.5.0D0)&
      &CALL PHAS13
      IF(FTFL01(11,R_I).EQ.7.0D0) PHASE=PHASE+PHA11
   END IF
!     IF A PHASE SURFACE 20
   IF(surf_special_type(R_I) == 20) THEN
      RLRL=R_L
      RMRM=R_M
      RNRN=R_N
      MYX=R_X
      MYY=R_Y
      MYI=R_I
      ISURF=MYI
      PHASE=0.0D0
      XPASS=R_X
      YPASS=R_Y
      CALL GRIDS(3,ISURF,GERROR)
      IF(.NOT.GERROR) GRIDSUNLOADED20(ISURF)=.FALSE.
      IF(GERROR) THEN
         IF(MSG) THEN
            CALL RAY_FAILURE(R_I)
            OUTLYNE=&
            &'GRID FILE DOES NOT EXIST FOR THIS SURFACE'
            CALL SHOWIT(1)
            OUTLYNE=&
            &'OR IT HAS INSUFFICIENT DATA FOR THIS SURFACE'
            CALL SHOWIT(1)
         END IF
         STOPP=1
         RAYCOD(1)=15
         RAYCOD(2)=R_I
         RAYEXT=.FALSE.
         POLEXT=.FALSE.
         SPDCD1=RAYCOD(1)
         SPDCD2=R_I
         RETURN
      END IF
      R_L=RLRL
      R_M=RMRM
      R_N=RNRN
      COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
      IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
      IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
   END IF
!     IF AT A DUMMY SURFACE, THE THICKNESS GOES FROM POS TO NEG
!     OR NEG TO POS, THEN IF RV, THEN SET .NOT.RV AND IF
!     NOT RV SET TO RV
!     OR AT A DUMMY SURFACE, N POS INTO NEG TH OR N NEG INTO POS TH
!     THEN IF RV, THEN SET .NOT.RV AND IF
   TESTLEN=R_Z-RR_Z
!     NOT RV SET TO RV
!     IF AT A DUMMY SURFACE, THE THICKNESS CHANGES SIGN, THEN THE
!     RAY GETS "REVERSED"
   IF(surf_thickness(R_I).GT.0.0D0.AND.surf_thickness(R_I-1).LT. &
   &0.0D0.AND.DUM(R_I).OR.surf_thickness(R_I).LT.0.0D0.AND.surf_thickness(R_I-1)&
   &.GT.0.0D0.AND.DUM(R_I)) THEN
      IF(RV) THEN
         RV=.FALSE.
      ELSE
         RV=.TRUE.
      END IF
   END IF
!     IF THE RAY HAS TRAVELED A NEG DIST WITH A POS DIRCOS
!     OR THE RAY HAS TRAVELED A POS DIST WITH A NEG DIR COS
!     AND THE RAY WAS NOT REVERESED, THEN SET IT TO "REVERSED"
!     RAY GETS "REVERSED"
   IF(RR_N.GT.0.0D0.AND.TESTLEN.LT.0.0D0.AND..NOT.REVSTR.OR.&
   &RR_N.LT.0.0D0.AND.TESTLEN.GT.0.0D0.AND..NOT.REVSTR.OR.&
   &RR_N.GT.0.0D0.AND.TESTLEN.GT.0.0D0.AND.REVSTR.OR.&
   &RR_N.LT.0.0D0.AND.TESTLEN.LT.0.0D0.AND.REVSTR.OR.&
   &SNINDX.GT.0.0D0.AND.TESTLEN.LT.0.0D0.AND..NOT.REVSTR.OR.&
   &SNINDX.LT.0.0D0.AND.TESTLEN.GT.0.0D0.AND..NOT.REVSTR.OR.&
   &SNINDX.GT.0.0D0.AND.TESTLEN.GT.0.0D0.AND.REVSTR.OR.&
   &SNINDX.LT.0.0D0.AND.TESTLEN.LT.0.0D0.AND.REVSTR) THEN
      RV=.TRUE.
   ELSE
      RV=.FALSE.
   END IF
   IF(RR_N.GT.0.0D0.AND.TESTLEN.LT.0.0D0.AND.RVSTART.OR.&
   &RR_N.LT.0.0D0.AND.TESTLEN.GT.0.0D0.AND.RVSTART) THEN
      RV=.FALSE.
   END IF
   IF(RR_N.GT.0.0D0.AND.TESTLEN.GT.0.0D0.AND..NOT.REVSTR.OR.&
   &RR_N.LT.0.0D0.AND.TESTLEN.LT.0.0D0.AND..NOT.REVSTR.OR.&
   &RR_N.GT.0.0D0.AND.TESTLEN.LT.0.0D0.AND.REVSTR.OR.&
   &RR_N.LT.0.0D0.AND.TESTLEN.GT.0.0D0.AND.REVSTR) THEN
      RV=.FALSE.
      RVSTART=.FALSE.
   END IF
!       RAYTRACE THROUGH ASPHERE COMPLETED
   IF(R_I.EQ.NEWIMG) THEN
      R_L=OLDL
      R_M=OLDM
      R_N=OLDN
   END IF
   COSIP=(R_L*LN)+(R_M*MN)+(R_N*NN)
   IF(COSIP.LT.-1.0D0) COSIP=-1.0D0
   IF(COSIP.GT.+1.0D0) COSIP=+1.0D0
   STOPP=0
   RAYCOD(1)=0
   RAYCOD(2)=R_I
   SPDCD1=RAYCOD(1)
   SPDCD2=R_I
90 CONTINUE
!     HERE IS WHERE WE ADD A RANDOM RAY ERROR IF IT IS
!     REQUESTED IN THE LENS DATABASE USING THE RAYERROR COMMAND
   IF(surf_ray_error(R_I).NE.0.0D0.AND.FOBRUN) THEN
      FOBRUN=.FALSE.
      RETURN
   END IF
!     NOT A FOB
   IF(surf_ray_error(R_I).NE.0.0D0.AND..NOT.FOBRUN) THEN
!     COMPUTE THE ORIGINAL X AND Y
      ORIG_X=DATAN2(R_L,R_N)
      ORIG_Y=DATAN2(R_M,R_N)
      IF(.NOT.DIFRAYTRACE) THEN
!     LOAD LINEAR AND GAUSSIAN RANDOMS INTO REG 9 AND 10
         DOUB=RANDMM()
         RRR9=REG(9)
         RRR10=REG(10)
      END IF
!     DELTAR IN RADIANS IS:
      DELTAR=(surf_ray_error(R_I)/(3600.0D0))*(PII/180.0D0)*RRR10
      DELTAX=DELTAR*DCOS(2.0D0*PII*RRR9)
      DELTAY=DELTAR*DSIN(2.0D0*PII*RRR9)
      R_L=R_N*DTAN(ORIG_X+DELTAX)
      R_M=R_N*DTAN(ORIG_Y+DELTAY)
      R_N=1.0D0-DSQRT((R_L**2)+(R_M**2))
   END IF
   RETURN
END
