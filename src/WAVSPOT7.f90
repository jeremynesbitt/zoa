...
!
   IF(STI.EQ.1.OR.SQ.EQ.0.AND.SN.EQ.0) THEN
      OUTLYNE='"FLDS" SETS AND QUERRIES THE STATUS OF MULTIPLE FIELDS OF VIEW'
      CALL SHOWIT(1)
      OUTLYNE='ASSOCIATED WITH THE CURRENT LENS'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'THERE ARE CURRENTLY ',CFLDCNT,' FRACTIONAL FIELD POSITIONS DEFINED'
      CALL SHOWIT(1)
...
!
   ...
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'MAX') THEN
         OUTLYNE='"FLDS" ONLY TAKES "MAX" AS VALID'
         CALL SHOWIT(1)
...
!     WQ="MAX"
   ...
   IF(INT(W1).GT.CFLDCNT) THEN
      ...
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'DEFINED MAXIMUM FIELD COUNT SPECIFIER OF ',CFLDCNT
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     SORT BY INCREASING VALUE
      DO I=1,CFLDCNT
         ...
         JK_2(I)=CFLDS(2,I)
      END DO
      CALL SORT_JK1(CFLDCNT,JK_A(1:CFLDCNT),JK_1(1:CFLDCNT),JK_2(1:CFLDCNT))
      DO I=1,CFLDCNT
         CFLDS(1,I)=JK_1(I)
         CFLDS(2,I)=JK_2(I)
      END DO
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"FLDSARE" QUERRIES THE STATUS OF MULTIPLE FIELDS OF VIEW'
      CALL SHOWIT(1)
      OUTLYNE='ASSOCIATED WITH THE CURRENT LENS'
      CALL SHOWIT(1)
      RETURN
   ...
   IF(SN.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
      OUTLYNE='"FLDSARE" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(CFLDCNT.EQ.0) THEN
      WRITE(OUTLYNE,*)'THERE ARE CURRENTLY ',CFLDCNT,' ANALYSIS FIELD POSITIONS DEFINED'
      CALL SHOWIT(0)
   END IF
   IF(CFLDCNT.GT.0) THEN
      WRITE(OUTLYNE,*)'THERE ARE CURRENTLY ',CFLDCNT,' ANALYSIS FIELD POSITIONS DEFINED'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'UNITS ARE DEGREES'
      CALL SHOWIT(0)
      DO I=1,CFLDCNT
         WRITE(OUTLYNE,10)I,CFLDS(1,I),CFLDS(2,I)
         CALL SHOWIT(0)
...
!
   IF(STI.EQ.1.OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
      OUTLYNE='"TFMOTION" SETS UP THE FOCUS MOTION CHARACTERISTICS FOR'
      CALL SHOWIT(1)
      OUTLYNE='"THRU-FOCUS OPTICAL TRANSFER FUNCTION ANALYSIS'
      CALL SHOWIT(1)
...
106 FORMAT('THRU-FOCUS T-MAX   = ',G13.6)
   ...
   IF(DF5.EQ.0) THEN
      OUTLYNE='"TFMOTION" TAKES NO NUMERIC WORD #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SST.EQ.1) THEN
      OUTLYNE='"TFMOTION" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(SQ.EQ.0) THEN
      ...
      WQ='Z       '
   END IF
   IF(WQ.NE.'X       '.AND.WQ.NE.'Y       '.AND.WQ .NE.'Z       ') THEN
      OUTLYNE='"TFMOTION" ONLY TAKES "X", "Y" OR "Z" AS VALID QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(INT(W1).LT.1) THEN
      OUTLYNE='SURFACE NUMBER, NUMERIC WORD #1, MUST BE GREATER THAN 1'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(INT(W1).GT.INT(sys_last_surf())) THEN
      WRITE(OUTLYNE,*)'SURFACE NUMBER, NUMERIC WORD #1, MUST BE LESS THAN ',INT(sys_last_surf())
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(W3.GE.W4) THEN
      WRITE(OUTLYNE,*)'"TMIN" (NUMERIC WORD #3) MUST BE LESS THAN'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'"TMAX" (NUMERIC WORD #4)'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W2.LT.0.0D0) THEN
      WRITE(OUTLYNE,*)'"TDELT" MUST BE GREATER THAN ZERO'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
SUBROUTINE GEOMTF(FREQ,MTF,PPHAS)
   ...
   use DATMAI
!
   REAL*8 AFUNC(-51:51),DELTA,DENOM,FREQ ,SINNOM,COSNOM,MTF,PPHAS,COORD(-51:51),XCOORD(1:110),YCOORD(1:110)
!
...
!
   ...
   AFUNC(51)=0.0D0
   DO I=-50,50
      AFUNC(I)=(1.0D-6*DBLE(NINT(YCOORD(I+51)*1.0D6)))
      COORD(I)=(1.0D-6*DBLE(NINT(XCOORD(I+51)*1.0D6)))
   END DO
!
...
!
   SINNOM=0.0D0
   DO I=-51,50
      SINNOM=SINNOM+(AFUNC(I)*DSIN(TWOPII*FREQ*COORD(I))+AFUNC(I+1)*DSIN(TWOPII*FREQ*COORD(I+1)))
   END DO
...
!
   COSNOM=0.0D0
   DO I=-51,50
      COSNOM=COSNOM+(AFUNC(I)*DCOS(TWOPII*FREQ*COORD(I))+AFUNC(I+1)*DCOS(TWOPII*FREQ*COORD(I+1)))
   END DO
...
!
   ...
   END IF
   IF(DABS(SINNOM).EQ.0.0D0.AND.DABS(COSNOM).EQ.0.0D0) THEN
      PPHAS=0.0D0
   ELSE
...
!
   REAL*8 MINFREQ,ZMIN,DELTAZ ,FREQ,ZMAX,OLDDZ,OLDDF,DELTA,DENOM,MTF,PPHAS,SNUMER,CNUMER,SINNOM,COSNOM
!
   CHARACTER UNIT*11
!
...
!
   COMMON/GMTFPASS/ENN,GDTAF,GDTAM,GDTAP,MTHETA1,DELZ,DELFREQ,MAXFREQ,MTHETA2
!
...
!
   INCORFACT=1.0D0
   IF(SPACEBALL.EQ.1)INCORFACT=(FREQ1/FREQ2)
!
   EXTGMTF1=.FALSE.
   EXTGMTF2=.FALSE.
!
...
!
   ...
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'ACC'.AND.WQ.NE.'TFOCUS'.AND.WQ.NE. 'TFREQ') THEN
...
!     MODE AFOCAL OR UAFOCAL
      IF(WQ.EQ.'TFOCUS') THEN
         OUTLYNE='"GOTF TFOCUS" NOT AVAILABLE IN MODES AFOCAL AND UAFOCAL'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(DELZ.NE.0.0D0) THEN
         OUTLYNE='NON-ZERO DELTA-Z VALUES NOT ALLOWED IN MODES AFOCAL AND UAFOCAL'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(.NOT.SPDEXT) THEN
      OUTLYNE='NO SPOT DIAGRAM EXISTS. GOTF REQUIRES A SPOT DIAGRAM'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
200      FORMAT('THROUGH FOCUS GEOMETRICAL OPTICAL TRANSFER FUNCTION')
...
103      FORMAT('FREQ.-(lp/mrad)',5X,'MODULUS',11X,'PHASE(DEG)')
         ...
         IF(IG.EQ.2) THEN
            ...
            CALL SHOWIT(0)
         END IF
         IF(sys_mode().LE.2.0D0.AND.SPACEBALL.EQ.2.OR.SPACEBALL.EQ.1.AND.NEAR) THEN
            WRITE(OUTLYNE,102)
            CALL SHOWIT(0)
         END IF
         IF(sys_mode().GE.3.0D0.AND.SPACEBALL.EQ.2.OR.SPACEBALL.EQ.1.AND..NOT.NEAR) THEN
            WRITE(OUTLYNE,103)
            CALL SHOWIT(0)
         END IF
!
...
!
   ...
   END IF
   RETURN
10 FORMAT('DELTA FREQUENCY (ADJUSTED FOR INTEGER # OF STEPS) = : ',G14.6)
11 FORMAT('DELTA FREQUENCY WAS ADJUSTED FOR THE MAXIMUN OF 100 STEPS')
20 FORMAT('DELTA Z (ADJUSTED FOR INTEGER # OF STEPS) = : ',G14.6)
END
...
!     ALL THE OTHER BOXES
      DO J=1,21
         IF(REAL(LARGER(I,2)).GT.REAL((XX(J)-DEL2)).AND.REAL(LARGER(I,2)).LT.REAL((XX(J)+DEL2))) THEN
...
!
   REAL*8 L1,L2,ABC,IMAX,THETA,XVAL,YVAL,IL1,IL2,DELSP,XV,YV,OVAL,INTEN,IX1,IX2,IY1,IY2,USDEL,NEWW1,JA,JB,SPDELX,SPDELY,ONLYY,TOPVAL,MTHETA,DELZ ,XCOORD(1:110),YCOORD(1:110),LSF1,LSF2,LSFP1,LSFP2
!
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      OUTLYNE='"LSF", "LSF (ACC)", "LSF (CACC) AND "LSF (CENT)"'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'CENT'.AND.WQ.NE.'ACC'.AND.WQ.NE.'CACC') THEN
         OUTLYNE='"INVALID QUALIFIER USED WITH "LSF"'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     NUMMERIC WORD 1 IS THE DESIRED NUMBER OF POINTS ACROSS THE LSF
      ...
      IF(W1.GT.101.0D0) THEN
         OUTLYNE='"LSF AND "LSF CENT"'
         CALL SHOWIT(1)
         OUTLYNE='REQUIRE NUMERIC WORD 1 INPUT LESS THAN OR EQUAL TO 101.0'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     NUMERIC WORDS 4 AND 5
   IF(WC.EQ.'LSF'.AND.WQ.EQ.'CENT'.OR.WC.EQ.'LSF'.AND.WQ .EQ.'CACC') THEN
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         OUTLYNE='"LSF (CENT)" AND "LSF (CACC)"'
         CALL SHOWIT(1)
   ...
   END IF
!       PROCEED
   IF(WQ.EQ.'CENT'.AND.DF2.EQ.0.AND.W2.NE.0.0D0.OR.WQ.EQ.'CACC'.AND.DF2.EQ.0.AND.W2.NE.0.0D0) THEN
      DELZ=W2
      ABC=W2
      CALL SPMV(ABC)
   ELSE
   END IF
   IF(WQ.NE.'CENT'.AND.DF4.EQ.0.AND.W4.NE.0.0D0.OR.WQ.NE.'CACC'.AND.DF4.EQ.0.AND.W4.NE.0.0D0) THEN
      DELZ=W3
      ABC=W4
      CALL SPMV(ABC)
   ELSE
...
!
   IF(NULL) THEN
      OUTLYNE='"FOB NULL" IS CURRENTLY IN EFFECT'
      CALL SHOWIT(1)
      OUTLYNE='"FOB NULL" PRECLUDES LINE SPREAD FUNCTION CALCULATIONS'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   OPEN(UNIT=66,ACCESS='DIRECT',FILE=trim(LIBSPO)//'LSF.DAT',FORM='UNFORMATTED',RECL=recLen,STATUS='UNKNOWN')
   DO I=2,ITOT
...
!     WQ MUST BE CENT OR CACC
               ...
               END IF
               IF(WQ.NE.'CENT'.OR.WQ.NE.'CACC') THETA=W5*(PII/180.0D0)
               IF(WQ.NE.'CENT'.OR.WQ.NE.'CACC') MTHETA=W5
               IF(WQ.EQ.'CENT'.OR.WQ.EQ.'CACC')THETA=W3*(PII/180.0D0)
               IF(WQ.EQ.'CENT'.OR.WQ.EQ.'CACC') MTHETA=W3
               IF(sys_mode().LT.3.0D0) THEN
                  LSF2=((((XV-REDVLX)/JB)*DCOS(THETA))+(((YV-REDVLY)/JA)*DSIN(THETA)))
               ELSE
                  LSF2=(((XV-REDVLX)*DCOS(THETA))+((YV-REDVLY)*DSIN(THETA)))
               END IF
            ELSE
!     AFOCAL
               IF(WQ.NE.'CENT'.AND.WQ.NE.'CACC') THEN
