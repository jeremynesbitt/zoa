!       SEVENTH FILE OF CAPFN/SPOT ROUTINES
...
!
   IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0) THEN
      ...
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'THERE ARE CURRENTLY ',CFLDCNT,&
      &' FRACTIONAL FIELD POSITIONS DEFINED'
      CALL SHOWIT(1)
...
!
   IF(SST.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"FLDS" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'MAX') THEN
         OUTLYNE=&
         &'"FLDS" ONLY TAKES "MAX" AS VALID'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('QUALIFIER WORD INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
!     WQ="MAX"
   IF(WQ.EQ.'MAX') THEN
      ...
      IF(W1.LT.1.0D0.OR.W1.GT.10.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'MAXIMUM NUMBER OF FIELDS MUST BE BETWEEN 1 AND 10'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      CFLDCNT=INT(W1)
   ...
   IF(INT(W1).GT.CFLDCNT) THEN
      ...
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'DEFINED MAXIMUM FIELD COUNT SPECIFIER OF ',CFLDCNT
      CALL SHOWIT(1)
...
!
   INTEGER I
!
   real(real64) JK_A(1:10),JK_1(1:10),JK_2(1:10)
!
...
!
   ...
   IF(SN.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
      OUTLYNE=&
      &'"FLDSARE" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(CFLDCNT.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'THERE ARE CURRENTLY ',CFLDCNT,' ANALYSIS FIELD POSITIONS DEFINED'
      CALL SHOWIT(0)
   END IF
   IF(CFLDCNT.GT.0) THEN
      WRITE(OUTLYNE,*)&
...
!
   IF(is_command_query().OR.SQ.EQ.0.AND.SN.EQ.0.AND.SST.EQ.0) THEN
      ...
      CALL SHOWIT(1)
      OUTLYNE=&
      &'"THRU-FOCUS OPTICAL TRANSFER FUNCTION ANALYSIS'
      CALL SHOWIT(1)
...
106 FORMAT('THRU-FOCUS T-MAX   = ',G13.6)
   ...
   IF(DF5.EQ.0) THEN
      OUTLYNE=&
      &'"TFMOTION" TAKES NO NUMERIC WORD #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
   ...
   IF(SST.EQ.1) THEN
      OUTLYNE=&
      &'"TFMOTION" TAKES NO STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
   ...
   &.NE.'Z       ') THEN
      OUTLYNE=&
      &'"TFMOTION" ONLY TAKES "X", "Y" OR "Z" AS VALID QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
   ...
   IF(W3.GE.W4) THEN
      WRITE(OUTLYNE,*)&
      &'"TMIN" (NUMERIC WORD #3) MUST BE LESS THAN'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)&
      &'"TMAX" (NUMERIC WORD #4)'
      CALL SHOWIT(1)
   ...
   IF(W2.LT.0.0D0) THEN
      WRITE(OUTLYNE,*)&
      &'"TDELT" MUST BE GREATER THAN ZERO'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'RE-ENTER COMMAND'
      CALL SHOWIT(1)
...
SUBROUTINE GEOMTF(FREQ,MTF,PPHAS)
   ...
   use DATMAI
!
   real(real64) AFUNC(-51:51),DELTA,DENOM,FREQ &
   &,SINNOM,COSNOM,MTF,PPHAS,COORD(-51:51)&
   &,XCOORD(1:110),YCOORD(1:110)
!
...
!
!
!     LINE SPREAD FUNCTION EXISTS NOW. NOW CALCULATE THE MTF FROM
!     FREQUENCY 0 TO THE DIFFRACTION CUT-OFF FREQUENCY
!
!     LSFDAT(I,1) CONTAINS THE SPREAD FUNCTION FUNCTIONAL VALUE
!     LSFDAT(I,2) CONTAINS THE COORDINATE POSITION OF THE SREAD FUNCTION
!     IN EITHER SYSTEM UNITS OF RADIANS(AFOCAL OR UAFOCAL MODES)
!     FREQ IS IN LP/LENS UNIT OR LP/RADIAN ONLY INSIDE THIS SUBROUTINE
!
!     FIRST CALCULATE THE DENOMINATOR WHICH IS THE INTEGRAL OF THE
!     SPREAD FUNCTION OVER ITS RANGE. ASSUME THAT THE LSF HAS ZERO
!     VALUE WHEN THE POSITION IN THE FUNCTION IS BEYOND THE FUNCTION
!     END POINTS BY A DISTANCE EQUAL TO THE SPACING BETWEEN
!     THE 101 TABULATED POINTS.
!
   AFUNC(-51)=0.0D0
   AFUNC(51)=0.0D0
   DO I=-50,50
      AFUNC(I)=&
      &(1.0D-6*DBLE(NINT(YCOORD(I+51)*1.0D6)))
      COORD(I)=&
      &(1.0D-6*DBLE(NINT(XCOORD(I+51)*1.0D6)))
   END DO
!
...
!     NOW THE NUMERATORS
!     SINE TERM NUMERATOR
!     THE SINE TERMS IS JUST DSIN(2*PI*FREQ*COORD)
!      SINTRM=DSIN(TWOPII*FREQ*COORD(I))
!
!     INTEGRATE USING THE TRAPEZOIDAL RULE
!
   SINNOM=0.0D0
   DO I=-51,50
      SINNOM=SINNOM+(&
      &AFUNC(I)*DSIN(TWOPII*FREQ*COORD(I))&
      &+&
      &AFUNC(I+1)*DSIN(TWOPII*FREQ*COORD(I+1)))
   END DO
   SINNOM=0.5D0*DELTA*SINNOM
   SINNOM=SINNOM/DENOM
!     COSINE TERM NUMERATOR
!     THE COSINE TERMS IS JUST DCOS(2*PI*FREQ*COORD)
!      SINTRM=DCOS(TWOPII*FREQ*COORD(I))
!
!     INTEGRATE USING THE TRAPEZOIDAL RULE
!
   COSNOM=0.0D0
   DO I=-51,50
      COSNOM=COSNOM+(&
      &AFUNC(I)*DCOS(TWOPII*FREQ*COORD(I))&
      &+&
      &AFUNC(I+1)*DCOS(TWOPII*FREQ*COORD(I+1)))
   END DO
...
! SUB GOTF.FOR
SUBROUTINE GOTF
   use global_widgets, only: curr_mtf
!
   use DATSP1
   use DATSPD
   use DATLEN
...
!     CALLED BY CMDER FOR COMMAND GOTF
!     THIS DOES GEOMETRICAL OTF CALCULATIONS
!
   real(real64) MINFREQ,ZMIN,DELTAZ &
   &,FREQ,ZMAX,OLDDZ,OLDDF,DELTA,DENOM,MTF,PPHAS,&
   &SNUMER,CNUMER,SINNOM,COSNOM
!
   CHARACTER UNIT*11
!
   REAL GDTAF(1:101),GDTAM(1:101,1:20),GDTAP(1:101,1:20)
   real(real64) MTHETA1,DELFREQ,DELZ,MAXFREQ,MTHETA2,FREQ1,FREQ2
   real(real64) AL,INCORFACT,CUTFR
!
   INTEGER ENN,IG,IIG
   COMMON/GII/IIG
!
...
!     COMMAND STRUCTURE
!     WC IS GOTF
!     WQ MAY BE NONE or FOCUS or FREQ or ACC
!     IF SQ = 0 THEN COMMAND TAKES NW1 THROUGH NW4
!     NW1=MAXFREQ (MAXIMUM FREQUENCY) MINIMUM FREQ=0
!     NW2=DELFREQ (DELTA FREQUENCY)
!     NW3=MTHETA1 OR 2 (IN DEGREES)
!     NW4=DELZEE (FOCUS SHIFT)
!     ALL ARE REQUIRED AND STI IS NOT SUPPORTED
!
   IF(WQ.EQ.'MFREQ') THEN
      MFREQ=.TRUE.
      WQ='        '
...
!
   ...
   IF(SQ.EQ.1) THEN
      ...
      &'TFREQ') THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'INVALID QUALIFIER WORD USED WITH "GOTF"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
!
   IF(WQ.EQ.'ACC') THEN
      IF(DF1.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF ACC" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(SQ.EQ.0) THEN
      ...
      IF(S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF" TAKES NO NUMERIC WORD #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WQ.EQ.'ACC') THEN
      IF(DF1.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF ACC"'//'\n'//&
         & 'REQUIRE EXPLICIT NUMERIC WORD #1 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DF2.EQ.1) THEN
      ...
      IF(S4.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF ACC" TAKE NO NUMERIC WORD #4 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(S5.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF ACC" TAKE NO NUMERIC WORD #5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WQ.EQ.'TFOCUS'.OR.WQ.EQ.'TFREQ') THEN
      ...
      IF(DF2.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF TFOCUS"'//'\n'//&
         & 'REQUIRE EXPLICIT NUMERIC WORD #2 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DF3.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & '"GOTF TFOCUS"'//'\n'//&
         & 'REQUIRE EXPLICIT NUMERIC WORD #3 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DF4.EQ.1) THEN
...
!
!     SANITY CHECK NUMERIC INPUT
!     FREQ
   IF(WQ.EQ.'TFOCUS'.OR.WQ.EQ.'ACC') THEN
      IF(FREQ.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'A NEGATIVE FREQUENCY MAY NOT BE REQUESTED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     MAXFREQ
      IF(MAXFREQ.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'A NEGATIVE MAXIMUM FREQUENCY MAY NOT BE REQUESTED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     MINFREQ
      IF(MINFREQ.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'A NEGATIVE MINIMUM FREQUENCY MAY NOT BE REQUESTED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     DELFREQ
      IF(DELFREQ.LT.0.0D0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'A NEGATIVE DELTA FREQUENCY MAY NOT BE REQUESTED'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
!     MTHETA1
   IF(MTHETA1.LT.0.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL('THETA MAY NOT BE NEGATIVE'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(MTHETA1.GT.360.0D0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & 'THETA MAY NOT BE GREATER THAN 360 DEGREES'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
!
   IF(SQ.EQ.0) THEN
      IF(DELFREQ.GT.MAXFREQ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'THE DELTA FREQUENCY MAY NOT BE GREATER THAN THE'//'\n'//&
         & 'MAXIMUM FREQUENCY'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   ...
   IF(WQ.EQ.'TFOCUS') THEN
      IF(ZMIN.GT.ZMAX) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'ZMIN MAY NOT BE GREATER THAN ZMAX'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DABS(ZMAX-ZMIN).LE.DABS(DELZ)) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'DELTA Z MAY NOT BE GREATER THAN (ZMAX-ZMIN)'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
   IF(WQ.EQ.'TFREQ') THEN
      IF(MINFREQ.GT.MAXFREQ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'THE MINIMUM FREQUENCY MAY NOT BE GREATER THAN'//'\n'//&
         & 'THE MAXIMIM FREQUENCY'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DABS(MAXFREQ-MINFREQ).LT.DELFREQ) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'THE DELTA FREQUENCY MAY NOT BE GREATER THAN'//'\n'//&
         & 'THE (MAXIMIM FREQUENCY-MINIMUM FREQUENCY)'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     MODE AFOCAL OR UAFOCAL
      IF(WQ.EQ.'TFOCUS') THEN
         OUTLYNE=&
         &'"GOTF TFOCUS" NOT AVAILABLE IN MODES AFOCAL AND UAFOCAL'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(DELZ.NE.0.0D0) THEN
         OUTLYNE=&
         &'NON-ZERO DELTA-Z VALUES NOT ALLOWED IN MODES AFOCAL AND UAFOCAL'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     FOCAL OR UFOCAL
   END IF
!     NOW FIX DELZ AND DELFREQ SO THEY ARE EVEN MULTIPLES OF RANGES
!     REQUESTED.
   IF(SQ.EQ.0) THEN
      OLDDF=DELFREQ
...
!
   IF(.NOT.SPDEXT) THEN
      OUTLYNE=&
      &'NO SPOT DIAGRAM EXISTS. GOTF REQUIRES A SPOT DIAGRAM'
      CALL SHOWIT(1)
...
!     SET OUTPUT TO NULL DEVICE
      ...
      CALL CONTRO
!
!     NOW COMPUTE THE APPROPRIATE LINE SPREAD FUNCTION USING THE
!     CENT OPTION TO CENTER THE LSF AT THE SPOT CENTROID. APPLY
!     THE ORIENTATION AND DELTA Z AS APPROPRIATE.
...
!
!     LINE SPREAD FUNCTION EXISTS NOW. NOW CALCULATE THE MTF FROM
!     FREQUENCY 0 TO Fmax FREQUENCY
!
100      FORMAT('GEOMETRICAL OPTICAL TRANSFER FUNCTION')
200      FORMAT(&
         &'THROUGH FOCUS GEOMETRICAL OPTICAL TRANSFER FUNCTION')
...
!
         ...
         ! Store data in mtf
         if(allocated(curr_mtf)) deallocate(curr_mtf)
...
!
!
            ! Where to store data for new plot
            curr_mtf(I+1,1) = FREQ/INCORFACT
            curr_mtf(I+1,2) = MTF
            curr_mtf(I+1,3) = PPHAS
...
!
         END DO
!     THE NEXT END DO IS THE END OF THE DOUBLE LOOP FOR BOTH
!     TARGET  ORIENTATIONS
      END DO
      RETURN
   ELSE
...
!
   ...
   RETURN
10 FORMAT(&
   &'DELTA FREQUENCY (ADJUSTED FOR INTEGER # OF STEPS) = : ',G14.6)
11 FORMAT(&
   &'DELTA FREQUENCY WAS ADJUSTED FOR THE MAXIMUN OF 100 STEPS')
20 FORMAT(&
   &'DELTA Z (ADJUSTED FOR INTEGER # OF STEPS) = : ',G14.6)
END
SUBROUTINE LSFSUM(LARGER,M,N)
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!     N IS ALWAYS 2, M IS SET BY ILSF
   INTEGER I,J,NUM,M,N
   real(real64) DEL2,XX(1:21),YY(1:21),XXX(1:43),YYY(1:43)
   real(real64) POSEXT,NEGEXT,DELEXT,MAXEXT
!
   real(real64) LARGER
!
...
!
   DO I=1,ILSF
!     LOOK AT THE ITH LARGER(,2) VALUE AND SEE WHICH OF THE 21
!     BOXES IT FITS IN
!     ALL THE OTHER BOXES
      DO J=1,21
         IF(REAL(LARGER(I,2)).GT.REAL((XX(J)-DEL2)).AND.&
         &REAL(LARGER(I,2)).LT.REAL((XX(J)+DEL2))) THEN
      ...
      END DO
   END DO
!     THERE ARE NOW 21 DATA POINTS FROM I=1 TO I=21
!     INTERPOLATE TO 43 DATA POINTS
!
!     END POINTS
   YYY(1)=0.0D0
   YYY(43)=0.0D0
...
SUBROUTINE LSFSOR(N,LARGER,M1,N1)
   ...
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) RRA,RRB,RED1,RED2,REDP1,REDP2
...
SUBROUTINE LSFPAK(LARGER,M,N)
   ...
   IMPLICIT NONE
   INTEGER I,K,M,N
   real(real64) RED1,RED2,REDP1,REDP2
!
   real(real64) LARGER
   DIMENSION LARGER(M,N)
!
...
!     COPY EVERY THING UP ONE SPACE
         DO K=I,ILSF
            IF(K.NE.ILSF) THEN
               LARGER(K,1)=LARGER(K+1,1)
               LARGER(K,2)=LARGER(K+1,2)
            ELSE
!     K=ILSF, NOTHING TO DO
            END IF
         END DO
         ILSF=ILSF-1
         GO TO 10
...
!     THESE DO THE GEOMETRICAL LINE SPREAD FUNCTIONS
!
   real(real64) L1,L2,ABC,IMAX,THETA,XVAL,YVAL,IL1,IL2,&
   &DELSP,XV,YV,OVAL,INTEN,IX1,IX2,IY1,IY2,USDEL,NEWW1,&
   &JA,JB,SPDELX,SPDELY,ONLYY,TOPVAL,MTHETA,DELZ &
   &,XCOORD(1:110),YCOORD(1:110),LSF1,LSF2,LSFP1,LSFP2
!
...
!     STRING INPUT
   IF(SST.EQ.1) THEN
      OUTLYNE=&
      &'"LSF", "LSF (ACC)", "LSF (CACC) AND "LSF (CENT)"'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('TAKE NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'CENT'.AND.WQ.NE.'ACC'.AND.WQ.NE.'CACC') THEN
         OUTLYNE=&
         &'"INVALID QUALIFIER USED WITH "LSF"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     NUMMERIC WORD 1 IS THE DESIRED NUMBER OF POINTS ACROSS THE LSF
      ...
      IF(W1.LE.2.0D0) THEN
         ...
         CALL SHOWIT(1)
         OUTLYNE=&
         &'REQUIRE NUMERIC WORD 1 INPUT GREATER THAN 2.0'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
      IF(W1.GT.101.0D0) THEN
         ...
         CALL SHOWIT(1)
         OUTLYNE=&
         &'REQUIRE NUMERIC WORD 1 INPUT LESS THAN OR EQUAL TO 101.0'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL('RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     NUMERIC WORDS 4 AND 5
   ...
   &.EQ.'CACC') THEN
      IF(DF4.EQ.0.OR.DF5.EQ.0) THEN
         OUTLYNE=&
         &'"LSF (CENT)" AND "LSF (CACC)"'
         CALL SHOWIT(1)
         CALL REPORT_ERROR_AND_FAIL(&
         & 'TAKE NO NUMERIC WORD 4 OR 5 INPUT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
!       PROCEED
   ...
   &WQ.EQ.'CACC'.AND.DF2.EQ.0.AND.W2.NE.0.0D0) THEN
      DELZ=W2
      ABC=W2
      CALL SPMV(ABC)
   ELSE
...
!
   IF(.NOT.SPDEXT) THEN
      ...
      RETURN
   END IF
!     HERE IS WHERE THE LINE SPREAD FUNCTION DATA IS PROCESSED.
!     LOAD THE CURRENT SPOT DIAGRAM DATA INTO THE LAS ARRAY
!     THE LSF ARRAY HAS 2 COLUMNS. COLUMN 1 INITIALLY CONTAINS
!     THE RAY INTENSITY TERM. COLUMN 2 EITHER HAS THE OFF-SET
!     COMPONENT MINUS EITHER THE CORRESPONDING CHEIF RAY COMPONENT
!     OR THE CORRESPONDINIG COMPONENT OF THE SPOT CENTROID. IF
!     FOB NULL WAS USED, NO LSF CAN BE DONE.
!
   IF(NULL) THEN
      OUTLYNE=&
      &'"FOB NULL" IS CURRENTLY IN EFFECT'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'"FOB NULL" PRECLUDES LINE SPREAD FUNCTION CALCULATIONS'
      CALL SHOWIT(1)
      CALL REPORT_ERROR_AND_FAIL('NO ACTION TAKEN', 1)
      RETURN
   END IF
...
!
!     OPEN AND CLOSE TO CLEAR
   call clear_file(trim(LIBSPO)//'LSF.DAT')
!
   ILSF=1
!
   inquire(iolength=recLen) LSF1,LSF2
   OPEN(UNIT=66,ACCESS='DIRECT',FILE=trim(LIBSPO)//'LSF.DAT',&
   &FORM='UNFORMATTED',RECL=recLen,STATUS='UNKNOWN')
   DO I=2,ITOT
...
!     WQ MUST BE CENT OR CACC
               ...
               END IF
               IF(WQ.NE.'CENT'&
               &.OR.WQ.NE.'CACC') THETA=W5*(PII/180.0D0)
               IF(WQ.NE.'CENT'&
               &.OR.WQ.NE.'CACC') MTHETA=W5
               IF(WQ.EQ.'CENT'&
...
!
!     THERE ARE NOW ILSF PAIRS OF LSF DATA
!
...
!     THERE ARE NOW ILSF PAIRS OF LSF DATA
   ...
   ILSF=44
   DEALLOCATE(LARGER,STAT=ALLOERR)
!     THERE ARE NOW 43 PAIRS OF LSF DATA
   IMAX=-1.0D10
   IF(ILSF.GT.2) THEN
      DO I=2,ILSF
   ...
   END IF
!     INTERPOLATE AND OUTPUT THE DATA REQUESTED
!
!     NOW DO APPROPRIATE OUTPUT BASED UPON COMMAND
!     FIRST, IF WQ IS ACC OR CACC, NO OUTPUT, JUST CALCULATE THE
!     INTENSITY FOR THE REQUESTED IMAGE POSITION (+ OR -)
   IF(WQ.EQ.'ACC'.OR.WQ.EQ.'CACC') THEN
      IF(ILSF.EQ.2) THEN
!     PERFECT SYSTEM, ONLY ONE NON-ZERO POINT
         READ(UNIT=66,REC=2) LSF1,LSF2
         IF(REAL(W1).EQ.REAL(LSF2)) INTEN=LSF1
         IF(REAL(W1).NE.REAL(LSF2)) INTEN=0.0D0
      ELSE
!     MORE THAN ONE POINT, INTERPOLATE
!
         INTEN=0.0D0
!     W1 IS THE POSITION FOR WHICH WE WISH TO KNOW THE INTENSITY
         DO I=2,ILSF-1
            READ(UNIT=66,REC=I) LSF1,LSF2
            READ(UNIT=66,REC=I+1) LSFP1,LSFP2
            IF(W1.GE.LSF2.AND.W1.LE.LSFP2) THEN
!     DATA IS BETWEEN THESE POINTS, LINEARLY INTERPOLATE IT
               IX1=LSF2
               IX2=LSFP2
               IY1=LSF1
               IY2=LSFP1
...
!
!     PRINT ORIENTATION OF KNIFE EDGE
...
!
!     W1 SPECIFIES THE NUMBER OF DATA POINTS
!     SO THERE WILL BE (ROUNDED TO THE NEAREST INTEGER)
   IF(DF1.EQ.1) THEN
      NOPNTS=21
   ELSE
      NOPNTS=INT(W1)
...
!     SET UP A DELTA FUNCTION LSF FOR USE BY THE GOTF COMMAND
      TOPVAL=-21.0D-20
      ILSF=41
      WRITE(UNIT=66,REC=1) ILSF
      DO I=1,41
...
!
!     INTENSITY CALCULATIONS ARE COMPLETE
!
   RETURN
END
