!       ELEVENTH FILE FOR LENS DATABASE MANAGER FILES
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE AUXCFG. THIS IS THE SUBROUTINE WHICH
!       SETS UP THE FAST ACCESS CFG ARRAYS AT THE END OF THE EOS
!       PROCESS FROM LENS MODIFICATIONS. IT IS CALLED FROM LNSEOS.
!
   CHARACTER AN1*23
!
   INTEGER I,J,K,ALLOERR
!
   real(real64) N1,SURFNM
!
   COMMON/CAUX1/N1,AN1
!
!
...
!       LASCFG IS THE NUMBER OF THE LAST NON ZERO CONFIGURATION
!       IF LASTCFG=1, JUST RETURN, THERE IS NOTHING TO DO
   IF(LASCFG.LE.1) THEN
      DEALLOCATE(SCRATH,STAT=ALLOERR)
      RETURN
   END IF
   IF(LASCFG.GT.1) THEN
!       PROPERLY LOAD THE FAST ARRAYS AND THEN RETURN
      K=0
      AUXMAX=K
      DO I=2,LASCFG
!       SET ULON AND USON BOTH TO FALSE
!       THESE TRACK IF U L OR U SP EXISTS IN CONFIG FILE
         ULON=.FALSE.
         USON=.FALSE.
!       LOAD ARRAYS FOR CFG I
!       THERE IS AN ADDRESS ARRAY AND A VALUE ARRAY
!       CFADD AND CFVAL
!       CFADD(ENTRY#,DATA TYPE#)
!       DATA TYPE = 1 (USE SAME CODE AS IN VARIABLES SUBFILE)
!       SURFACE # = 2
!       CFG #     = 3
!       ADD1      = 4
!       ADD2      = 5
...
!       ADD4      = 7
!       PIKSOL    = 8
!       J(ENTRY NUMBER IN THE ITH CONFIG)=9
!     ENTRY# IS A CHRONOLOGICAL LISTING NUMBER IN THE ARRAY
!     ADD1 AND ADD2 ARE THE STARTING AND ENDING FIELDS FOR
!     NW1 DATA, ADD3 AND ADD4 ARE STARTING AND ENDING FIELDS
!     FOR NW2 DATA.
!     PIKSOL IS SET TO NON-ZERO, IF THE VALUE TYPE IS BEING
!     SET USING A PIKUP OR A SOLVE SINCE IT IS IMPORTANT
!     TO KNOW THAT WHEN TRYING TO OPTIMIZE.
!     SOLVE AND PIKUP TYPES ARE
!               NON-ZERO PIKSOL VALUES REPRESENT:
!     PIKUP RD    = 1
!     PIKUP CV    = 2
!     PIKUP CC    = 3
!     PIKUP TH    = 4
...
!     PIKUP GRT   = 44
!
!     IF AN ITEM IS SET USING A PIKUP OR SOLVE, IT CAN'T BE
!     USED IN THE VARIABLES SUBFILE AND A MESSAGE TO THAT
!     EFFECT WILL BE ISSUED WHEN AN ATTEMP IS MADE TO ADD THAT
!     ITEM TO THE VARIABLES OR TVARIABLES SUBFILE.
!
!     CFVAL(ENTRY#,DATA TYPE#)
!     ENTRY# IS A CRONOLOGICAL LISTING NUMBER IN THE ARRAY (SAME
!     AS FOR CFADD
!     DATA TYPE#=1 FOR NW1 VALUE
!     DATA TYPE#=2 FOR NW2 VALUE
!     CFCHAR(ENTRY#,DATA TYPE#)
!     ENTRY# IS A CRONOLOGICAL LISTING NUMBER IN THE ARRAY (SAME
!     AS FOR CFADD
!     DATA TYPE#=1 > CHARACTER REPRESENTATION OF NW1 VALUE
!     DATA TYPE#=2 > CHARACTER REPRESENTATION OF NW2 VALUE
!     THERE COULD BE A NEED FOR (MAXCFG-1)*198 ENTRY NUMBERS
!     IF ALL ALTERNATE CONFIGS HAD ONLY VARIABLE TYPE CHANGES
!
...
!
!     NO WE ONLY NEED TO DO MORE BEFORE GOING TO THE NEXT LINE IN
!     SCRATH, IF ULON OR USON ARE .TRUE.
            IF(USON.OR.ULON) THEN
!     CONTINUE TO PROCESS
!     FIRST USON, SINCE THAT SEEMS EASIER TO DO
               IF(USON) THEN
!     CHECK FOR C1 THROUGH C96 AND PROCESS APPROPRIATELY
...
!     USON FALSE
               END IF
!     NOW CHECK FOR CASES OF ULON TRUE
               IF(ULON) THEN
!               SET THE CURRENT SURFACE NUMBER IF NECESSARY
                  IF(SCRATH(J)(1:8).EQ.'CHG     ') THEN
                     AN1=SCRATH(J)(10:32)
                     CALL AUXATN
                     SURFNM=N1
                  ELSE
!     PROCEED, SURFNM ALREADY SET
                  END IF
!     THE REGULAR LENS FILE VARIABLES
...
!     ULON FALSE
               END IF
!                       ELSE
!     BOTH ULON AND USON FALSE
            END IF
         END DO
!     WE JUST LEFT CONFIG I, LOOP BACK FOR THE NEXT CONFIG
!
      END DO
!     NEXT LINES WERE USED DURING DEBUGGING OF VARIABLES SUBFILE
!     ARRAYS LOADED, ALL DONE
      DEALLOCATE(SCRATH,STAT=ALLOERR)
      RETURN
   ELSE
   END IF
...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE BLKHEAD WHICH IMPLEMENTS THE
!       FOOTBLOK DEFINITION
!
!
...
!
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      ...
      & '"FOOTBLOK" TAKES NO STRING OR NUMERIC INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(SQ.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"FOOTBLOK" REQUIRES EXPLICIT QUALIFIER WORD INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WQ.NE.'YES'.AND.WQ.NE.'NO'.AND.WQ.NE.'ON'.AND.WQ.NE.'OFF')THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"FOOTBLOK" REQUIRES "ON","OFF","YES" OR "NO"'//'\n'//&
...
24 CONTINUE
!
!     3. APPLY FLIP SURFACES IGNORING ALL TILTS AND DECENTERS
!     SOME THINGS JUST FLIP EXACTLY
   DO K=I,J
      BCNT=I+(J-K)
...
!
   ...
   IMPLICIT NONE
!
!     THIS DOES THE HEXAGON-LIKE CMD LEVEL COMMAND "ROLL"
!
!       FORM OF THE "ROLL" COMMAND IS:
!
!       ROLL, I J DELTAX DELTAY K
!
   INTEGER I,J,SURFN,K,L
!
   COMMON/REMSURF/SURFN
!
   real(real64) D11,D12,D13,D21,D22,D23,D31,D32,D33,DX1,DY1 ,DZ1,ALPHA,BETA,GAMMA,GALPHA,GBETA,GGAMMA,GDECX,GDECY,GDECZ,COSB,SINB,DXER,DYER,NEWALPHA,NEWBETA,TH
!
!
!
...
!
   IF(I.EQ.K) THEN
!     1. GET THE LOCATION OF SURFACE J+1 IN THE COORDINATE SYSTEM OF
!     SURFACE I-1. WRITE IT DOWN
!
...
!     NOW GET ALL THE GOODIES
      ...
      D32=VERTEX(11,J+1)
      D33=VERTEX(12,J+1)
!     CALCULATE GLOBAL DECENTERS AND TILTS
      GDECX=DX1
      GDECY=DY1
      GDECZ=DZ1
!     CALCULATE GLOBAL ALPHA, BETA AND GAMMA
...
!     COSINE OF BETA IS ZERO
      ...
      GGAMMA=GAMMA
!
!     2. APPLY A TILT TO SURFACE I CORRESPONDING TO THE OFFSETS IF
!     THE SURFACE IS NOT FLAT. iF IT IS FLAT, APPLY A DECENTER
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
...
!     DO A PIVOT AT THE CENTER OF CURVATURE ASSUMING NO INITIAL TILTS
      ...
      REST_KDP(1)=RESTINPT(1)
!
!     3. APPLY A RETURN ON SURFACE J+1 TO RETURN TO I-1
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
      ...
      REST_KDP(1)=RESTINPT(1)
!
!     4. ASSIGN GLOBAL DECENTERS AND TILTS TO SURFACE J+1 SO THAT
!     IT RETURNS TO ITS ORIGINAL POSITION WITH RESPECT TO I-1
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
      WQ='        '
...
!
   ...
   IMPLICIT NONE
!
!     THIS DOES THE HEXAGON-LIKE CMD LEVEL COMMAND "STILT"
!
!       FORM OF THE "STILT" COMMAND IS:
!
!       STILT, I ALPHA BETA GAMMA
!C     OR
!        STILT PIVOT X Y Z
!      OR
!        STILT PIVOT AUTO
...
!
   COMMON/REMSURF/SURFN
!
   real(real64) D11,D12,D13,D21,D22,D23,D31,D32,D33,DX1,DY1 ,DZ1,ALPHA,BETA,GAMMA,GALPHA,GBETA,GGAMMA,GDECX,GDECY,GDECZ,COSB,SINB,DALPHA,DBETA,DGAMMA
!
!
!     STILT PIVOT
...
!
   ...
   IF(is_command_query().AND.SQ.EQ.0) THEN
      ...
      RETURN
   END IF
   IF(is_command_query().AND.WQ.EQ.'PIVOT') THEN
      ...
      RETURN
   END IF
   IF(is_command_query().AND.WQ.EQ.'PIVAUTO') THEN
...
!
!
!     1. GET THE LOCATION OF SURFACE I+1 IN THE COORDINATE SYSTEM OF
!     SURFACE I-1. WRITE IT DOWN
!
      SAVE_KDP(1)=SAVEINPT(1)
      WC='GLOBAL'
...
!     COSINE OF BETA IS ZERO
      ...
      GGAMMA=GAMMA
!
!     2. APPLY A TILT TO SURFACE I EQUAL TO THE STILT VALUES
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
      ...
      REST_KDP(1)=RESTINPT(1)
!
!     3. APPLY A RETURN ON SURFACE I+1 TO RETURN TO I-1
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
      ...
      REST_KDP(1)=RESTINPT(1)
!
!     4. ASSIGN GLOBAL DECENTERS AND TILTS TO SURFACE I+1 SO THAT
!     IT RETURNS TO ITS ORIGINAL POSITION WITH RESPECT TO I-1
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
   ...
   END IF
   IF(SQ.EQ.0) THEN
!     NOW DELETE ALL RET AND PIVOTS
!     NOW DELETE ALL RET FROM I+1
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
...
!
   ...
   IMPLICIT NONE
!
!       THIS DOES THE HEXAGON-LIKE CMD LEVEL COMMAND "DISP"
!
...
!
   INTEGER I,J
   real(real64) D11,D12,D13,D21,D22,D23,D31,D32,D33,DX1,DY1 ,DZ1,ALPHA,BETA,GAMMA,GALPHA,GBETA,GGAMMA,GDECX,GDECY,GDECZ,COSB,SINB,XDEC,YDEC,ZDEC
!
!
!
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
...
!     COSINE OF BETA IS ZERO
   ...
   GGAMMA=GAMMA
!
!     2. APPLY A DEC TO SURFACE I EQUAL TO THE DISP VALUES
   SAVE_KDP(1)=SAVEINPT(1)
   INPUT='UPDATE LENS'
   CALL PROCES
   WC='CHG'
...
!
   ...
   IMPLICIT NONE
!
!     THIS DOES THE HEXAGON-LIKE CMD LEVEL COMMAND "BTILT"
!
!       FORM OF THE "BTILT" COMMAND IS:
!
!       BTILT, I J, ALPHA BETA GAMMA
!C     OR
!        BTILT PIVOT X Y Z
!      OR
!        BTILT PIVOT AUTO
...
!     COSINE OF BETA IS ZERO
      ...
      GGAMMA=GAMMA
!
!     2. APPLY A TILT TO SURFACE I EQUAL TO THE BTILT VALUES
      SAVE_KDP(1)=SAVEINPT(1)
      INPUT='UPDATE LENS'
      CALL PROCES
      WC='CHG'
