...
!
   REAL*8 NUMPTS,CAPFNIN,WV,VAL1,VAL2,PTVOPD,MEAN2,ARG,VAL5
!
   DIMENSION CAPFNIN(:,:,:)
!
   ALLOCATABLE CAPFNIN
...
!
   IF(STI.EQ.1) THEN
      IF(WC.EQ.'CAPFNIN') OUTLYNE='"CAPFNIN" INPUTS A CAPFN FROM AN ASCII FILE'
      IF(WC.EQ.'CAPFNIN') OUTLYNE='"CAPFNCLR" DEALLOCATES THE CAPFNIN/CAPFNADD ARRAYS'
      IF(WC.EQ.'CAPFNADD') OUTLYNE='"CAPFNADD" INPUTS AND ADDS A CAPFN FROM AN ASCII FILE'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WC.EQ.'CAPFNCLR') THEN
      IF(SQ.EQ.1.OR.SN.EQ.1.OR.SST.EQ.1) THEN
         WRITE(OUTLYNE,*) WC,' TAKES NO ADDITIONAL INPUT'
         CALL SHOWIT(1)
   ...
   IF(SQ.EQ.1.OR.SN.EQ.1) THEN
      WRITE(OUTLYNE,*) WC,' TAKES NO NUMERIC OR QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(.NOT.EXIS73) THEN
      OUTLYNE='THE REQUESTED ASCII CAPFN FILE DOES NOT EXIST TO INPUT'
      CALL SHOWIT(1)
...
!     OPEN FILE
!
   OPEN(UNIT=73,FILE=CAPOUT,FORM='FORMATTED',STATUS='UNKNOWN')
   OPEN(UNIT=43,ACCESS='DIRECT',FILE=BCAPOUT,FORM='UNFORMATTED',RECL=(100*NRECL),STATUS='UNKNOWN')
   REWIND(UNIT=73)
!
...
!
   IF(WC.EQ.'CAPFNADD'.AND..NOT.ALLO) THEN
      OUTLYNE='"CAPFNIN" MUST HAVE BEEN USED BEFORE "CAPFNADD" MAY BE USED'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
   ...
   IF(WC.EQ.'CAPFNADD'.AND.ALLONUM.NE.INT(NUMPTS)) THEN
      OUTLYNE='"ADDED CAPFN IS NOT THE SAME DIMENSION AS CAPFN IN STORAGE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL CLOSE_FILE(73,1)
...
!     AND WRITING IT OUT AGAIN.
   ...
   ARG=((MEAN2-((MEAN**2)/DBLE(MEANCNT)))/DBLE(MEANCNT-1))
   IF(ARG.LT.0.0D0) THEN
      OUTLYNE='RMSOPD ERROR OCCURED, CAPFNIN RMSOPD CALCULATION HAS FAILED'
      CALL SHOWIT(1)
      CALL MACFAL
      CALL CLOSE_FILE(73,1)
      CALL CLOSE_FILE(43,1)
...
!     SAVE THE SUMMED CAPFN
      ...
      K=1
      DO J=2,ITOT
         READ(UNIT=43,REC=J) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
         IF(DSPOT(12).NE.0.0D0.AND.WC.EQ.'CAPFNADD'.OR.WC.EQ.'CAPFNIN') THEN
!     LOAD DSPOT(*) INTO DSPOTT(*,ID)
      ...
      END DO
      NUMCOL=1
      READ(UNIT=43,REC=ITOT+1) WVSHORT,SHORT,NUMCOL,LFOB(1),LFOB(2),LFOB(3),LFOB(4),LFOB(5),LFOB(6),LFOB(7),IW,DDEELL
      CALL CLOSE_FILE(43,1)
      CPFNEXT=.TRUE.
      PLOTIT(1)=.TRUE.
      PLOTIT(2)=.FALSE.
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"CAPGRID" OUTPUTS THE CURRENT CAPFN TO AN ASCII FILES'
      CALL SHOWIT(1)
      OUTLYNE='"GRIDAPD" AND "GRIDOPD'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
      OUTLYNE='"CAPGRID" TAKES NO QUALIFIER OR STRING INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"CAPGRID" TAKES NO NUMERIC WORD #2 THROUGH #5 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S1.EQ.0) THEN
      OUTLYNE='"CAPGRID" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     DEFAULT WAVELENGTH IS CONTROL WAVELENGTH
   ...
   END IF
!
   OPEN(UNIT=45,FILE='GRIDAPD.DAT',FORM='FORMATTED',STATUS='UNKNOWN')
   CALL CLOSE_FILE(45,0)
   OPEN(UNIT=46,FILE='GRIDOPD.DAT',FORM='FORMATTED',STATUS='UNKNOWN')
...
!
   IF(READOK) THEN
      ...
      WRITE(OUTLYNE,*)'CAPFN DATA FOR WAVELENGTH NUMBER ',INT(W1)
      CALL SHOWIT(1)
      ...
      CALL SHOWIT(0)
20    FORMAT('GRID SPACING AT THE CURRENT REFERENCE SURFACE = ',G13.6,1X,A6)
   ELSE
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"CAPFNOUT" OUTPUTS THE CURRENT CAPFN TO AN ASCII FILE'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SQ.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"CAPFNOUT" TAKES NO STRING OR QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   G=ITOT+1
   WRITE(UNIT=43,REC=G) WVSHORT,SHORT,NUMCOL,LFOB(1),LFOB(2),LFOB(3),LFOB(4),LFOB(5),LFOB(6),LFOB(7),IW,DDEELL
!
   WVN=INT(LFOB(4))
   IF(WVN.EQ.1) WWVN=1
   IF(WVN.EQ.2) WWVN=2
...
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
      ID=I-1
      CALL SPOTIT(4)
      WRITE(UNIT=43,REC=I) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
      WRITE(73,19) DSPOT(12),DSPOT(4),INT(DSPOT(7)),INT(DSPOT(8)),DSPOT(35)
   END DO
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"WAMAP" GENERATES A WAVEFRONT MAP USING CAPFN DATA'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"WAMAP" TAKES NO QUALIFIER OR ALPHANUMERIC INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(S3.EQ.1.OR.S2.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"WAMAP" ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"AMAP" GENERATES AN APERTURE APODIZATION FILE USING CAPFN DATA'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"AMAP" TAKES NO QUALIFIER OR ALPHANUMERIC INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(S3.EQ.1.OR.S2.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"AMAP" ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   REAL*8 F,X,Y,DL1,FV ,REFHT,XMIN,YMIN,ZMIN,XMAX,YMAX,ZMAX,WVAL
!
...
!
   OPEN(UNIT=70,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='OPDMAP.DAT',STATUS='UNKNOWN')
!
   I=1
   J=0
   DO IQ=2,ITOT
...
!             IIVAL LE 64
   ...
   END IF
202 CONTINUE
   WRITE(OUTLYNE,*)'35 LEVELS FROM "Z" (MINIMUM) TO "1" (MAXIMUM)'
   CALL SHOWIT(0)
   WRITE(OUTLYNE,*)'EACH LEVEL REPRESENTS OPD RANGE OF ',DL1, ' WAVES'
   CALL SHOWIT(0)
   IF(DL1.NE.0.0D0) THEN
      WRITE(OUTLYNE,*)'"1" = ',ZMAX-(0.0D0*DL1),' TO ',ZMAX-(1.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"2" = ',ZMAX-(1.0D0*DL1),' TO ',ZMAX-(2.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"3" = ',ZMAX-(2.0D0*DL1),' TO ',ZMAX-(3.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"4" = ',ZMAX-(3.0D0*DL1),' TO ',ZMAX-(4.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"5" = ',ZMAX-(4.0D0*DL1),' TO ',ZMAX-(5.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"6" = ',ZMAX-(5.0D0*DL1),' TO ',ZMAX-(6.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"7" = ',ZMAX-(6.0D0*DL1),' TO ',ZMAX-(7.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"8" = ',ZMAX-(7.0D0*DL1),' TO ',ZMAX-(8.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"9" = ',ZMAX-(8.0D0*DL1),' TO ',ZMAX-(9.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"A" = ',ZMAX-(9.0D0*DL1),' TO ',ZMAX-(10.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"B" = ',ZMAX-(10.0D0*DL1),' TO ',ZMAX-(11.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"C" = ',ZMAX-(11.0D0*DL1),' TO ',ZMAX-(12.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"D" = ',ZMAX-(12.0D0*DL1),' TO ',ZMAX-(13.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"E" = ',ZMAX-(13.0D0*DL1),' TO ',ZMAX-(14.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"F" = ',ZMAX-(14.0D0*DL1),' TO ',ZMAX-(15.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"G" = ',ZMAX-(15.0D0*DL1),' TO ',ZMAX-(16.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"H" = ',ZMAX-(16.0D0*DL1),' TO ',ZMAX-(17.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"I" = ',ZMAX-(17.0D0*DL1),' TO ',ZMAX-(18.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"J" = ',ZMAX-(18.0D0*DL1),' TO ',ZMAX-(19.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"K" = ',ZMAX-(19.0D0*DL1),' TO ',ZMAX-(20.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"L" = ',ZMAX-(20.0D0*DL1),' TO ',ZMAX-(21.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"M" = ',ZMAX-(21.0D0*DL1),' TO ',ZMAX-(22.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"N" = ',ZMAX-(22.0D0*DL1),' TO ',ZMAX-(23.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"O" = ',ZMAX-(23.0D0*DL1),' TO ',ZMAX-(24.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"P" = ',ZMAX-(24.0D0*DL1),' TO ',ZMAX-(25.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Q" = ',ZMAX-(25.0D0*DL1),' TO ',ZMAX-(26.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"R" = ',ZMAX-(26.0D0*DL1),' TO ',ZMAX-(27.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"S" = ',ZMAX-(27.0D0*DL1),' TO ',ZMAX-(28.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"T" = ',ZMAX-(28.0D0*DL1),' TO ',ZMAX-(29.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"U" = ',ZMAX-(29.0D0*DL1),' TO ',ZMAX-(30.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"V" = ',ZMAX-(30.0D0*DL1),' TO ',ZMAX-(31.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"W" = ',ZMAX-(31.0D0*DL1),' TO ',ZMAX-(32.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"X" = ',ZMAX-(32.0D0*DL1),' TO ',ZMAX-(33.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Y" = ',ZMAX-(33.0D0*DL1),' TO ',ZMAX-(34.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Z" = ',ZMAX-(34.0D0*DL1),' TO ',ZMAX-(35.0D0*DL1),' WAVES'
      CALL SHOWIT(0)
!
...
!
   OPEN(UNIT=72,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='APMAP.DAT',STATUS='UNKNOWN')
!
   I=1
   J=0
   DO IQ=2,ITOT
...
202 CONTINUE
   ...
   CALL SHOWIT(0)
   IF(DL1.NE.0.0D0) THEN
      WRITE(OUTLYNE,*)'"1" = ',ZMAX-(0.0D0*DL1),' TO ',ZMAX-(1.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"2" = ',ZMAX-(1.0D0*DL1),' TO ',ZMAX-(2.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"3" = ',ZMAX-(2.0D0*DL1),' TO ',ZMAX-(3.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"4" = ',ZMAX-(3.0D0*DL1),' TO ',ZMAX-(4.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"5" = ',ZMAX-(4.0D0*DL1),' TO ',ZMAX-(5.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"6" = ',ZMAX-(5.0D0*DL1),' TO ',ZMAX-(6.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"7" = ',ZMAX-(6.0D0*DL1),' TO ',ZMAX-(7.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"8" = ',ZMAX-(7.0D0*DL1),' TO ',ZMAX-(8.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"9" = ',ZMAX-(8.0D0*DL1),' TO ',ZMAX-(9.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"A" = ',ZMAX-(9.0D0*DL1),' TO ',ZMAX-(10.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"B" = ',ZMAX-(10.0D0*DL1),' TO ',ZMAX-(11.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"C" = ',ZMAX-(11.0D0*DL1),' TO ',ZMAX-(12.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"D" = ',ZMAX-(12.0D0*DL1),' TO ',ZMAX-(13.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"E" = ',ZMAX-(13.0D0*DL1),' TO ',ZMAX-(14.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"F" = ',ZMAX-(14.0D0*DL1),' TO ',ZMAX-(15.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"G" = ',ZMAX-(15.0D0*DL1),' TO ',ZMAX-(16.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"H" = ',ZMAX-(16.0D0*DL1),' TO ',ZMAX-(17.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"I" = ',ZMAX-(17.0D0*DL1),' TO ',ZMAX-(18.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"J" = ',ZMAX-(18.0D0*DL1),' TO ',ZMAX-(19.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"K" = ',ZMAX-(19.0D0*DL1),' TO ',ZMAX-(20.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"L" = ',ZMAX-(20.0D0*DL1),' TO ',ZMAX-(21.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"M" = ',ZMAX-(21.0D0*DL1),' TO ',ZMAX-(22.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"N" = ',ZMAX-(22.0D0*DL1),' TO ',ZMAX-(23.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"O" = ',ZMAX-(23.0D0*DL1),' TO ',ZMAX-(24.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"P" = ',ZMAX-(24.0D0*DL1),' TO ',ZMAX-(25.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Q" = ',ZMAX-(25.0D0*DL1),' TO ',ZMAX-(26.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"R" = ',ZMAX-(26.0D0*DL1),' TO ',ZMAX-(27.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"S" = ',ZMAX-(27.0D0*DL1),' TO ',ZMAX-(28.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"T" = ',ZMAX-(28.0D0*DL1),' TO ',ZMAX-(29.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"U" = ',ZMAX-(29.0D0*DL1),' TO ',ZMAX-(30.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"V" = ',ZMAX-(30.0D0*DL1),' TO ',ZMAX-(31.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"W" = ',ZMAX-(31.0D0*DL1),' TO ',ZMAX-(32.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"X" = ',ZMAX-(32.0D0*DL1),' TO ',ZMAX-(33.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Y" = ',ZMAX-(33.0D0*DL1),' TO ',ZMAX-(34.0D0*DL1)
      CALL SHOWIT(0)
      WRITE(OUTLYNE,*)'"Z" = ',ZMAX-(34.0D0*DL1),' TO ',ZMAX-(35.0D0*DL1)
      CALL SHOWIT(0)
!
      WRITE(OUTLYNE,*)'UPPER LEFT HAND CORNER OF MAP IS -X, -Y'
      CALL SHOWIT(0)
...
!
   ...
   integer :: rec64
!
   REAL*8 DWW1,DWW2,DWW3,DWW4,TERM,FF3,RHO,THETA
!
!
   REAL*8 W,V,U,B,XXX
!
...
!
   ...
   ALLOCATE (W(1:96),V(1:96,1:96),U(1:96,1:96),B(1:96),XXX(1:96),STAT=ALLOERR)
!
...
!
   IF(SQ.EQ.1.OR.SST.EQ.1) THEN
      OUTLYNE='"FITZERN" TAKES NO QUALIFIER OR ALPHANUMERIC INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(S3.EQ.1.OR.S2.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='"FITZERN" ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     DEFAULT WAVELENGTH IS CONTROL WAVELENGTH
   ...
   IF(.NOT.CPFNEXT) THEN
      ...
      RETURN
   END IF
   OUTLYNE='FITTING THE WAVEFRONT TO A 37 TERM FRINGE ZERNIKE'
   CALL SHOWIT(1)
   CCOL(1:96)=0.0D0
   CFTYPE(1:96)=0.0D0
   X(1:96)=0.0D0
...
!     OPEN OPDDAT.DAT
!
   OPEN(UNIT=64,ACCESS='DIRECT',FILE=trim(LIBSPO)//'OPDDAT.DAT',FORM='UNFORMATTED',RECL=rec64,STATUS='UNKNOWN')
   KK=1
   PRINT *, "ITOT is ", ITOT
   DO IIP=2,ITOT
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
      ...
      CALL SPOTIT(4)
!
      IF(DSPOT(16).EQ.W1.AND.DSPOT(12).NE.0.0D0.OR.DSPOT(16).EQ.W1.AND.DSPOT(12).EQ.0.0D0.AND.DSPOT(7).EQ.7.0D0) KK=KK+1
   END DO
   IF(KK.LT.37.0D0) THEN
!     NOT ENOUGH DATA FOR A FIT
      OUTLYNE='NOT ENOUGH DATA POINTS EXIST FOR A RELIABLE FIT'
      CALL SHOWIT(1)
      OUTLYNE='OR NO DATA WAS AVAILABLE AT THE SPECIFIED WAVELENGTH'
      CALL SHOWIT(1)
      OUTLYNE='TRY TRACING A DENSER GRID OF RAYS'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   EXTERNAL FF3
!
   INTEGER SSN,SM,NP2,MP,JTYPE,IITOT,COUNT,I,J,N,M,II,IIP,IWK,JJJ ,ISS
!
   COMMON/PRSIZE/COUNT
!
!       THIS PASSES THE SOLUTION OF THE LEAST SQUARES PROBLEM
   ...
   LOGICAL ERRR
!
   DOUBLE PRECISION ACCUM(1:96,1:96),REFHT,WW,TERM1,TERM2,WAVEX,CCOL(1:96),DSPOTT(35,IITOT),X(1:96),SUML2,SUML4,WEIS ,DENOM,DZ1,DZ2,SS1,SS2,VALL,VALX,VALY,WEI(1:10),LAM(1:10),PHI,PHIX,PHIY,SSS,COSX,COSY,SINX,SINY,SSSO,DLLZX,DLLZY,DLLZ0 ,PHIO,NRR,ORR,TERM3
!
   COMMON/ACDATA/ACCUM,CCOL
!
   INTEGER III,JJ,ALLOERR
!
   DOUBLE PRECISION DWW1,DWW2,DWW3,DWW4,TERM,FF3,RHO,THETA
!
!
   DOUBLE PRECISION W,V,U,B,XXX,WV
!
   DIMENSION W(:),V(:,:),U(:,:),B(:),XXX(:)
!
   ALLOCATABLE :: W,V,U,B,XXX
...
!     COMPUTE THE EFFECTIVE WAVELENGTH
   ...
   END IF
   DO IIP=1,IITOT
      IF(DSPOTT(16,IIP).EQ.1.0D0)  WV=sys_wavelength(1)
      IF(DSPOTT(16,IIP).EQ.2.0D0)  WV=sys_wavelength(2)
      IF(DSPOTT(16,IIP).EQ.3.0D0)  WV=sys_wavelength(3)
      IF(DSPOTT(16,IIP).EQ.4.0D0)  WV=sys_wavelength(4)
      IF(DSPOTT(16,IIP).EQ.5.0D0)  WV=sys_wavelength(5)
      IF(DSPOTT(16,IIP).EQ.6.0D0)  WV=sys_wavelength(6)
      IF(DSPOTT(16,IIP).EQ.7.0D0)  WV=sys_wavelength(7)
      IF(DSPOTT(16,IIP).EQ.8.0D0)  WV=sys_wavelength(8)
      IF(DSPOTT(16,IIP).EQ.9.0D0)  WV=sys_wavelength(9)
      IF(DSPOTT(16,IIP).EQ.10.0D0) WV=sys_wavelength(10)
      IF(sys_units().EQ.1.0D0) THEN
         WV=WV*3.93700787402D-5
      END IF
      IF(sys_units().EQ.2.0D0) THEN
      ...
      IF(sys_units().EQ.4.0D0) THEN
         WV=WV*1.0D-6
      END IF
      IF(DSPOTT(17,IIP).NE.0.0D0 .AND.DSPOTT(7,IIP).EQ.0.0D0.AND.DSPOTT(12,IIP).NE.0.0D0) THEN
...
!     NOW COMPUTE THE TILTED PART AND SUBTRACT IT OFF
   DO IIP=1,IITOT
      ...
      TERM=0.0D0
      IF(JTYPE.EQ.3) THEN
         TERM1=(X(2)*FF3(RHO,THETA,2))+(X(3)*FF3(RHO,THETA,3))
         TERM2=0.0D0
      END IF
      IF(JTYPE.EQ.4) THEN
         TERM1=(X(2)*FF3(RHO,THETA,2))+(X(3)*FF3(RHO,THETA,3))
         TERM2=(X(1)*FF3(RHO,THETA,1))+(X(4)*FF3(RHO,THETA,4))
      END IF
      TERM3=TERM1+TERM2
      DWW3=DWW3-TERM3
      IF(DSPOTT(12,IIP).NE.0.0D0)DSPOTT(4,IIP)=(DWW3*(TWOPII))/WV
      IF(DSPOTT(12,IIP).EQ.0.0D0) DSPOTT(4,IIP)=0.0D0
   END DO
   IF(WAVEX.EQ.1) THEN
      DLLX=((X(2)*FF3(1.0D0,0.0D0,2))-(X(2)*FF3(1.0D0,(PII),2)))
      DLLY=((X(3)*FF3(1.0D0,(PII/2.0D0),3))-(X(3)*FF3(1.0D0,(3.0D0*PII/2.0D0),3)))
      IF(JTYPE.EQ.3)  DLLZ=0.0D0
      IF(JTYPE.EQ.4)  DLLZ=((X(1)*FF3(1.0D0,0.0D0,1))+(X(4)*FF3(1.0D0,0.0D0,4)))
      ERRR=.FALSE.
      MSG=.FALSE.
...
!
   EXTERNAL FF3
!
   INTEGER DATCNT,TYPE,COUNT,INUMBER,IDATA,I,II,III,IIII,IIIII
!
   LOGICAL OPMAP,OPEN64
!
   COMMON/OPOPMP/OPMAP
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"OPDLIST" TAKES NO EXPLICIT INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(.NOT.OPMAP.OR..NOT.CPFNEXT) THEN
      OUTLYNE='NO OPD MAP FIT COEFFICIENTS EXIST'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   IF(.NOT.OPEN64)OPEN(UNIT=64,ACCESS='DIRECT',FILE=trim(LIBSPO)//'OPDDAT.DAT',FORM='UNFORMATTED',RECL=rec64,STATUS='UNKNOWN')
   READ(UNIT=64,REC=1) INUMBER
!
   DO IIIII=1,INUMBER-1
      READ(UNIT=64,REC=IIIII+1)DDDATA(1),DDDATA(2),DDDATA(3),DDDATA(4)
!       PROCEED WITH EVALUATION
      II=0
      TERM=0.0D0
      RHO=DSQRT((DDDATA(1)**2)+(DDDATA(2)**2))
      IF(DABS(DDDATA(1)).GE.DABS(((1.0D35)*DDDATA(2)))) THEN
         IF(DDDATA(1).GE.0.0D0) THETA=PII/2.0D0
         IF(DDDATA(1).LT.0.0D0) THETA=(3.0D0*PII)/2.0D0
      ELSE
         IF(DABS(DDDATA(1)).EQ.0.0D0.AND.DABS(DDDATA(2)).EQ.0.0D0)THEN
            THETA=0.0D0
         ELSE
...
!       DO A LINE OF OUTPUT
      ...
      WRITE(OUTLYNE,1110) DDDATA(1),DDDATA(2),DDDATA(3),TERM,(DDDATA(3)-TERM)
      CALL SHOWIT(0)
   END DO
...
1106 FORMAT(1X)
1105 FORMAT('   Y-COORD     ',1X,'   X-COORD     ',1X,' INPUT  (OPD)  ',1X,' FITTED  (OPD) ',1X,'FITTING ERROR')
!
END
! SUB WRTCOEFS.FOR
SUBROUTINE WRTCOEFS
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"LISTZERN" TAKES NO EXPLICIT INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   REAL*8 HIGHORD,VCF(1:37),EMN,NV ,VC1,VC2,VC3,VC4,VC5,VC6,VC7,VC8,VC9,X(1:96)
!
   LOGICAL OPMAP
!
   COMMON/OPOPMP/OPMAP
...
!
   IF(STI.EQ.1) THEN
      WRITE(OUTLYNE,*)'"LISTREPT" GENERATES A REPORT FOR 37-TERM FRINGE ZERNIKE'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'WAVEFRONT FITTING RESULTS)'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"LISTREPT" TAKES NO EXPLICIT INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
5  FORMAT('37-TERN FRINGE ZERNIKE REPORT')
   WRITE(OUTLYNE,40)
40 FORMAT ('RMS WAVEFRONT ERROR IN WAVES (FROM THE LAST WAVEFRONT MAP FIT)')
   CALL SHOWIT(0)
   NV=0.0D0
   EMN=2.0D0
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"AWRTSUM" WRITES A SUMMED SPOT DIAGRAM TO AN ASCII FILE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
30 CONTINUE
   ...
   IF(.NOT.EXIS32) THEN
      OUTLYNE='SUMMED SPOT DIAGRAM DOES NOT EXIST TO WRITE TO DISK'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     OVERWRITE THE FILE
   ...
   IF(EXIS65) THEN
      OPEN(UNIT=65,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=NAMER(1:JKNN)//'.ASC',STATUS='UNKNOWN')
      CALL CLOSE_FILE(65,0)
   END IF
!     NOW OPEN FOR READING AND WRITING
   OPEN(UNIT=32,ACCESS='DIRECT',FILE=trim(LIBSPO)//FNAMER(1:FJKNN)//'.DAT',FORM='UNFORMATTED',RECL=(110*NRECL),STATUS='UNKNOWN')
!
   OPEN(UNIT=65,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=NAMER(1:JKNN)//'.ASC',STATUS='UNKNOWN')
!
...
!
   DO I=2,ITOT
      READ(UNIT=32,REC=I) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"BWRTSUM" WRITES A SUMMED SPOT DIAGRAM TO A BINARY FILE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
30 CONTINUE
   ...
   IF(.NOT.EXIS32) THEN
      OUTLYNE='SUMMED SPOT DIAGRAM DOES NOT EXISTS TO WRITE TO DISK'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     OVERWRITE THE FILE
   ...
   IF(EXIS64) THEN
      OPEN(UNIT=64,ACCESS='DIRECT',FILE=NAMER(1:JKNN)//'.SPD',FORM='UNFORMATTED',RECL=(110*NRECL),STATUS='UNKNOWN')
      CALL CLOSE_FILE(64,0)
   END IF
...
!
   DO I=2,ITOT
      READ(UNIT=32,REC=I) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
      WRITE(UNIT=64,REC=I) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
   END DO
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"AWRTSPOT" WRITES THE CURRENT SPOT DIAGRAM TO AN ASCII FILE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"BWRTSPOT" WRITES THE CURRENT SPOT DIAGRAM TO A BINARY FILE'
      CALL SHOWIT(1)
      OUTLYNE='NO ACTION TAKEN'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
      ...
      WRITE(UNIT=64,REC=I) DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
   END DO
   WRITE(UNIT=64,REC=ITOT+1)REFRY(1,NEWOBJ),REFRY(2,NEWOBJ),REFRY(3,NEWOBJ),REFRY(4,NEWOBJ),REFRY(5,NEWOBJ),REFRY(6,NEWOBJ),REFRY(32,NEWOBJ)
!     FINISHED WRITING, CLOSE THE FILE NAMER.SPD
   CALL CLOSE_FILE(64,1)
   RETURN
END
...
!
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      OUTLYNE='"STATS" TAKES NO STRING OR NUMERIC INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WQ.NE.'FULL'.AND.WQ.NE.'MIN') THEN
      OUTLYNE='INVALID QUALIFIER USED WITH "STATS"'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
! SUB SPSAVE.FOR
SUBROUTINE SPSAVE
   use zoa_file_handler, only: clear_file, doesFileExist,isSPOFileOpen, clear_file
!
   use DATSP1
   use DATSPD
   use DATLEN
...
!
   INTEGER I,J,K,N
!
   REAL*8 SPT1,SPT2,SPT3,SPT4,SPT5,SPT6,SPT7,SPT8 ,SPT9,SPT10
!
...
!       DELETE IT
      ...
      END IF
      OPEN(UNIT=32,ACCESS='DIRECT',FILE=trim(LIBSPO)//WQ(1:N)//'.DAT',FORM='UNFORMATTED',RECL=(110*NRECL),STATUS='UNKNOWN')
!
      K=2
      IF(.NOT.SPDEXT) THEN
...
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
            ...
            IF(DSPOT(12).NE.0.0D0) THEN
               WRITE(UNIT=32,REC=K)DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
...
!     OPEN OPDDAT.DAT
!
   OPEN(UNIT=64,ACCESS='DIRECT',FILE=trim(LIBSPO)//'OPDDAT.DAT',FORM='UNFORMATTED',RECL=(10*NRECL),STATUS='UNKNOWN')
   KK=1
   DO IIP=2,ITOT
!     LOAD DSPOT(*) WITH DSPOTT(*,ID)
      ID=IIP-1
      CALL SPOTIT(4)
!
      IF(DSPOT(16).EQ.WVNUMOP.AND.DSPOT(12).NE.0.0D0.OR.DSPOT(16).EQ.WVNUMOP.AND.DSPOT(12).EQ.0.0D0.AND.DSPOT(7).EQ.7.0D0) KK=KK+1
   END DO
   IF(KK.LT.37.0D0) THEN
!     NOT ENOUGH DATA FOR A FIT
      CALL MACFAL
