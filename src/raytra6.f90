!       THIS IS THE SIXTH FILE OF RAYTRACING ROUTINES

! SUB PUPILS
SUBROUTINE FNUMX(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,JAA,JBB,JCC,A1,A2,B1,B2,C1,C2 ,VXHI,VXLO,REFIN
   LOGICAL ERRR
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0
      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET BFNX" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
!     DO VIG CALCS
      VXHI=1.0D0
      VXLO=-1.0D0
      CALL VIGCAL(100,VXLO,VXHI,1)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXHI
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET BFNX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A1=RAYRAY(4,NEWIMG)
      B1=RAYRAY(5,NEWIMG)
      C1=RAYRAY(6,NEWIMG)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXLO
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET BFNX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A2=RAYRAY(4,NEWIMG)
      B2=RAYRAY(5,NEWIMG)
      C2=RAYRAY(6,NEWIMG)
      VALVAL=(A1*A2)+(B1*B2)+(C1*C2)
      JAA=DSQRT((A1*A1)+(B1*B1)+(C1*C1))
      JBB=DSQRT((A2*A2)+(B2*B2)+(C2*C2))
      JCC=JAA*JBB
      IF(JCC.NE.0.0D0) VALVAL=DACOS(VALVAL/JCC)/2.0D0
!     VALVAL IS ANGLE INCLUDED BETWEEN RAYS IN RADIANS
!     F-NUMBER IS JUST:
      REFIN=ldm%getSurfIndex(NEWIMG-1, INT(sys_wl_ref()))
      VALVAL=1.0D0/(2.0D0*DTAN(VALVAL)*REFIN)
      VALVAL=(RBFNX/DABS(RBFNX))*DABS(VALVAL)
      RETURN
   END IF
END
SUBROUTINE FNUMY(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,JAA,JBB,JCC,A1,A2,B1,B2,C1,C2 ,VYHI,VYLO,REFIN
   LOGICAL ERRR
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET BFNY" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VYHI=1.0D0
      VYLO=-1.0D0
      CALL VIGCAL(100,VYLO,VYHI,2)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYHI
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET BFNY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A1=RAYRAY(4,NEWIMG)
      B1=RAYRAY(5,NEWIMG)
      C1=RAYRAY(6,NEWIMG)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYLO
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET BFNY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A2=RAYRAY(4,NEWIMG)
      B2=RAYRAY(5,NEWIMG)
      C2=RAYRAY(6,NEWIMG)
      VALVAL=(A1*A2)+(B1*B2)+(C1*C2)
      JAA=DSQRT((A1*A1)+(B1*B1)+(C1*C1))
      JBB=DSQRT((A2*A2)+(B2*B2)+(C2*C2))
      JCC=JAA*JBB
      IF(JCC.NE.0.0D0) VALVAL=DACOS(VALVAL/JCC)/2.0D0
!     VALVAL IS ANGLE INCLUDED BETWEEN RAYS IN RADIANS
!     F-NUMBER IS JUST:
      REFIN=ldm%getSurfIndex(NEWIMG, INT(sys_wl_ref()))
      VALVAL=1.0D0/(2.0D0*DTAN(VALVAL)*REFIN)
      VALVAL=(RBFNY/DABS(RBFNY))*DABS(VALVAL)
      RETURN
   END IF
END
SUBROUTINE OBFNUMX(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,JAA,JBB,JCC,A1,A2,B1,B2,C1,C2 ,VXHI,VXLO,REFIN
   LOGICAL ERRR
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET FFNX" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VXHI=1.0D0
      VXLO=-1.0D0
      CALL VIGCAL(100,VXLO,VXHI,1)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXHI
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET FFNX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A1=RAYRAY(19,NEWOBJ+1)
      B1=RAYRAY(20,NEWOBJ+1)
      C1=RAYRAY(21,NEWOBJ+1)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXLO
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET FFNX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A2=RAYRAY(19,NEWOBJ+1)
      B2=RAYRAY(20,NEWOBJ+1)
      C2=RAYRAY(21,NEWOBJ+1)
      VALVAL=(A1*A2)+(B1*B2)+(C1*C2)
      JAA=DSQRT((A1*A1)+(B1*B1)+(C1*C1))
      JBB=DSQRT((A2*A2)+(B2*B2)+(C2*C2))
      JCC=JAA*JBB
      IF(JCC.NE.0.0D0) VALVAL=DACOS(VALVAL/JCC)/2.0D0
!     VALVAL IS ANGLE INCLUDED BETWEEN RAYS IN RADIANS
!     F-NUMBER IS JUST:
      REFIN=ldm%getSurfIndex(NEWOBJ, INT(sys_wl_ref()))
      VALVAL=1.0D0/(2.0D0*DTAN(VALVAL)*REFIN)
      VALVAL=(RFFNX/DABS(RFFNX))*DABS(VALVAL)
      RETURN
   END IF
END
SUBROUTINE OBFNUMY(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,JAA,JBB,JCC,A1,A2,B1,B2,C1,C2 ,VYHI,VYLO,REFIN
   LOGICAL ERRR
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET FFNY" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VYHI=1.0D0
      VYLO=-1.0D0
      CALL VIGCAL(100,VYLO,VYHI,2)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYHI
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET FFNY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A1=RAYRAY(19,NEWOBJ+1)
      B1=RAYRAY(20,NEWOBJ+1)
      C1=RAYRAY(21,NEWOBJ+1)
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYLO
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET FFNY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
      A2=RAYRAY(19,NEWOBJ+1)
      B2=RAYRAY(20,NEWOBJ+1)
      C2=RAYRAY(21,NEWOBJ+1)
      VALVAL=(A1*A2)+(B1*B2)+(C1*C2)
      JAA=DSQRT((A1*A1)+(B1*B1)+(C1*C1))
      JBB=DSQRT((A2*A2)+(B2*B2)+(C2*C2))
      JCC=JAA*JBB
      IF(JCC.NE.0.0D0) VALVAL=DACOS(VALVAL/JCC)/2.0D0
!     VALVAL IS ANGLE INCLUDED BETWEEN RAYS IN RADIANS
!     F-NUMBER IS JUST:
      REFIN=ldm%getSurfIndex(NEWOBJ, INT(sys_wl_ref()))
      VALVAL=1.0D0/(2.0D0*DTAN(VALVAL)*REFIN)
      VALVAL=(RFFNY/DABS(RFFNY))*DABS(VALVAL)
      RETURN
   END IF
END
SUBROUTINE ENPDIAY(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,RX0,RY0,RZ0,RLOLD,RMOLD,RNOLD,X1 ,Y1,Z1,L1,M1,N1,XA,XB,YA,YB,ZA,ZB,T ,VYHI,VYLO
   LOGICAL ERRR
!
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET ENDIAY" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VYHI=1.0D0
      VYLO=-1.0D0
      CALL VIGCAL(100,VYLO,VYHI,2)
!     CHIEF RAY DATA AT NEWOBJ+1 IS:
!
      RX0=ENPUX
      RY0=ENPUY
      RZ0=ENPUZ
      RLOLD=REFRY(19,NEWOBJ+1)
      RMOLD=REFRY(20,NEWOBJ+1)
      RNOLD=REFRY(21,NEWOBJ+1)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYHI
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET ENDIAY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     ENTRANCE PUPIL COORDINATES ARE AT:
!     ENPUX,ENPUY AND ENPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWOBJ+1)
      Y1=RAYRAY(2,NEWOBJ+1)
      Z1=RAYRAY(3,NEWOBJ+1)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWOBJ+1)
      M1=RAYRAY(20,NEWOBJ+1)
      N1=RAYRAY(21,NEWOBJ+1)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWOBJ+1 SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XA=X1+(L1*T)
      YA=Y1+(M1*T)
      ZA=Z1+(N1*T)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYLO
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET ENDIAY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     ENTRANCE PUPIL COORDINATES ARE AT:
!     ENPUX,ENPUY AND ENPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWOBJ+1)
      Y1=RAYRAY(2,NEWOBJ+1)
      Z1=RAYRAY(3,NEWOBJ+1)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWOBJ+1)
      M1=RAYRAY(20,NEWOBJ+1)
      N1=RAYRAY(21,NEWOBJ+1)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWOBJ+1 SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XB=X1+(L1*T)
      YB=Y1+(M1*T)
      ZB=Z1+(N1*T)
!
!
!     THE ENTRANCE PUPIL DIAMETER IS JUST
      VALVAL=DSQRT(((XA-XB)**2)+((YA-YB)**2)+((ZA-ZB)**2))
      RETURN
   END IF
END
SUBROUTINE EXPDIAX(VALVAL,ERRR)
!     VIGNETTING IS DONE HERE
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,RX0,RY0,RZ0,RLOLD,RMOLD,RNOLD,X1 ,Y1,Z1,L1,M1,N1,XA,XB,YA,YB,ZA,ZB,T ,VXHI,VXLO
   LOGICAL ERRR
!
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET EXDIAX" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VXHI=1.0D0
      VXLO=-1.0D0
      CALL VIGCAL(100,VXLO,VXHI,1)
!     CHIEF RAY DATA AT NEWIMG IS:
!
      RX0=EXPUX
      RY0=EXPUY
      RZ0=EXPUZ
      RLOLD=REFRY(19,NEWIMG)
      RMOLD=REFRY(20,NEWIMG)
      RNOLD=REFRY(21,NEWIMG)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXHI
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET EXDIAX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     EXIT PUPIL COORDINATES ARE AT:
!     EXPUX,EXPUY AND EXPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWIMG)
      Y1=RAYRAY(2,NEWIMG)
      Z1=RAYRAY(3,NEWIMG)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWIMG)
      M1=RAYRAY(20,NEWIMG)
      N1=RAYRAY(21,NEWIMG)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWIMG SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XA=X1+(L1*T)
      YA=Y1+(M1*T)
      ZA=Z1+(N1*T)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXLO
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET EXDIAX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     EXIT PUPIL COORDINATES ARE AT:
!     EXPUX,EXPUY AND EXPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWIMG)
      Y1=RAYRAY(2,NEWIMG)
      Z1=RAYRAY(3,NEWIMG)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWIMG)
      M1=RAYRAY(20,NEWIMG)
      N1=RAYRAY(21,NEWIMG)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWIMG SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XB=X1+(L1*T)
      YB=Y1+(M1*T)
      ZB=Z1+(N1*T)
!
!
!     THE EXIT PUPIL DIAMETER IS JUST
      VALVAL=DSQRT(((XA-XB)**2)+((YA-YB)**2)+((ZA-ZB)**2))
      RETURN
   END IF
END
SUBROUTINE EXPDIAY(VALVAL,ERRR)
!     VIGNETTING IS DONE HERE
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,RX0,RY0,RZ0,RLOLD,RMOLD,RNOLD,X1 ,Y1,Z1,L1,M1,N1,XA,XB,YA,YB,ZA,ZB,T ,VYHI,VYLO
   LOGICAL ERRR
!
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET EXDIAY" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VYHI=1.0D0
      VYLO=-1.0D0
      CALL VIGCAL(100,VYLO,VYHI,2)
!     CHIEF RAY DATA AT NEWIMG IS:
!
      RX0=EXPUX
      RY0=EXPUY
      RZ0=EXPUZ
      RLOLD=REFRY(19,NEWIMG)
      RMOLD=REFRY(20,NEWIMG)
      RNOLD=REFRY(21,NEWIMG)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYHI
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET EXDIAY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     EXIT PUPIL COORDINATES ARE AT:
!     EXPUX,EXPUY AND EXPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWIMG)
      Y1=RAYRAY(2,NEWIMG)
      Z1=RAYRAY(3,NEWIMG)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWIMG)
      M1=RAYRAY(20,NEWIMG)
      N1=RAYRAY(21,NEWIMG)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWIMG SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XA=X1+(L1*T)
      YA=Y1+(M1*T)
      ZA=Z1+(N1*T)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=VYLO
      W2=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET EXDIAY" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     EXIT PUPIL COORDINATES ARE AT:
!     EXPUX,EXPUY AND EXPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWIMG)
      Y1=RAYRAY(2,NEWIMG)
      Z1=RAYRAY(3,NEWIMG)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWIMG)
      M1=RAYRAY(20,NEWIMG)
      N1=RAYRAY(21,NEWIMG)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWIMG SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XB=X1+(L1*T)
      YB=Y1+(M1*T)
      ZB=Z1+(N1*T)
!
!
!     THE EXIT PUPIL DIAMETER IS JUST
      VALVAL=DSQRT(((XA-XB)**2)+((YA-YB)**2)+((ZA-ZB)**2))
      RETURN
   END IF
END
SUBROUTINE ENPDIAX(VALVAL,ERRR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   real(real64) VALVAL,RX0,RY0,RZ0,RLOLD,RMOLD,RNOLD,X1 ,Y1,Z1,L1,M1,N1,XA,XB,YA,YB,ZA,ZB,T ,VXHI,VXLO
   LOGICAL ERRR
!
!
   IF(.NOT.REFEXT) THEN
      ERRR=.TRUE.
      VALVAL=0.0D0

      IF(F28.EQ.0.AND.F31.EQ.0) THEN
         IF(MSG) THEN
            OUTLYNE='NO REFERENCE RAY EXISTS'
            CALL SHOWIT(1)
            OUTLYNE='AN "FOB" COMMAND IS REQUIRED'
            CALL SHOWIT(1)
            OUTLYNE='A "GET ENDIAX" WILL RESULT'
            CALL SHOWIT(1)
         END IF
      END IF
      RETURN
   ELSE
!     PROCEED
      VXHI=1.0D0
      VXLO=-1.0D0
      CALL VIGCAL(100,VXLO,VXHI,1)
!     CHIEF RAY DATA AT NEWOBJ+1 IS:
!
      RX0=ENPUX
      RY0=ENPUY
      RZ0=ENPUZ
      RLOLD=REFRY(19,NEWOBJ+1)
      RMOLD=REFRY(20,NEWOBJ+1)
      RNOLD=REFRY(21,NEWOBJ+1)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXHI
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET ENDIAX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     ENTRANCE PUPIL COORDINATES ARE AT:
!     ENPUX,ENPUY AND ENPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWOBJ+1)
      Y1=RAYRAY(2,NEWOBJ+1)
      Z1=RAYRAY(3,NEWOBJ+1)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWOBJ+1)
      M1=RAYRAY(20,NEWOBJ+1)
      N1=RAYRAY(21,NEWOBJ+1)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWOBJ+1 SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XA=X1+(L1*T)
      YA=Y1+(M1*T)
      ZA=Z1+(N1*T)
!
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=0
      DF2=0
      DF3=1
      DF4=1
      DF5=1
      S1=1
      S2=1
      S3=0
      S4=0
      S5=0
      SN=1
      W1=0.0D0
      W2=VXLO
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      REST_KDP(1)=RESTINPT(1)
      IF(.NOT.RAYEXT) THEN
         ERRR=.TRUE.
         VALVAL=0.0D0

         IF(F28.EQ.0.AND.F31.EQ.0) THEN
            IF(MSG) THEN
               OUTLYNE='RAY CAN NOT BE TRACED'
               CALL SHOWIT(1)
               OUTLYNE='A "GET ENDIAX" WILL RESULT'
               CALL SHOWIT(1)
            END IF
         END IF
         RETURN
      END IF
!     THE ABOVE DATA IS FOR THE NON-CHIEF RAY
!     ENTRANCE PUPIL COORDINATES ARE AT:
!     ENPUX,ENPUY AND ENPUZ. THE PLANE GOING THROUGH THIS POINT AND
!     PERPENDICULAR TO THE CHIEF RAY IS EASILY DEFINED. THE INTERSECTION POINT
!     OF THIS NON-CHIEF RAY WITH THIS PLANE IS:
!             XX1,YY1,ZZ1
!       THE EQUATION OF THIS PLANE IS:
!       (X-RX0)*RLOLD + (Y-RY0)*RMOLD +(Z-RZ0)*RNOLD = 0
!       WHERE:
      X1=RAYRAY(1,NEWOBJ+1)
      Y1=RAYRAY(2,NEWOBJ+1)
      Z1=RAYRAY(3,NEWOBJ+1)
!       ITS DIRECTION COSINES ARE
      L1=RAYRAY(19,NEWOBJ+1)
      M1=RAYRAY(20,NEWOBJ+1)
      N1=RAYRAY(21,NEWOBJ+1)
!
!       THE OTHER RAY CAN BE REPRESENTED AS:
!
!       X=X1+(L1*T)
!       Y=Y1+(M1*T)
!       Z=Y1+(N1*T)
!       T IS THE DISTANCE FROM THE NEWOBJ+1 SURFACE INTERSECTION TO THE
!       PLANE INTERSECTION AND IS:
!
      T=((RLOLD*(RX0-X1))+(RMOLD*(RY0-Y1))+(RNOLD*(RZ0-Z1)))/((L1*RLOLD)+(M1*RMOLD)+(N1*RNOLD))
!       THE OTHER RAY INTERSECTS THE PLANE AT:
      IF(DABS(T).GE.1.0D20) T=1.0D20
!
      XB=X1+(L1*T)
      YB=Y1+(M1*T)
      ZB=Z1+(N1*T)
!
!
!     THE ENTRANCE PUPIL DIAMETER IS JUST
      VALVAL=DSQRT(((XA-XB)**2)+((YA-YB)**2)+((ZA-ZB)**2))
      RETURN
   END IF
END
! SUB AUXFOB.FOR
SUBROUTINE AUXFOB(ERRFOB)
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     AUXFOB IS CALLED AFTER FFOB TO CALCULATE DIFFERENTIAL RAY BASED
!     FFL,BFL,EFL,FN AND MAG OF THE SYSTEM
!
   real(real64) RPYO,RPXO,RPCYO,RPCXO,RPUYO,RPUXO,RPUCYO,RPUCXO,RPYI,RPXI,RPCYI,RPCXI,RPUYI,RPUXI,RPUCYI,RPUCXI,RPY1,RPX1,RPCY1,RPCX1,RPUY1,RPUX1,RPUCY1,RPUCX1,RPYR,RPXR,RPCYR,RPCXR,RPUYR,RPUXR,RPUCYR,RPUCXR,NWN1,NWN2,NWN3,NWN4,JK20,VALUE,RFX,RFY,RSHIFTX,RSHIFTY
!
   real(real64) VALUE_AUXFOB(1:8)
   COMMON/FOBAUX/VALUE_AUXFOB
!
   INTEGER ERROR,NUM5
!
   COMMON/GV/VALUE,NUM5
!
   LOGICAL ERRFOB
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
!
   JK20=DBLE(NEWIMG-1)
   ERROR=0
   NWN1=0.0D0
   NWN2=0.0D0
   NWN3=0.0D0
   NWN4=sys_wl_ref()
   CALL GNPR12345678(INT(JK20),NWN1,NWN2,NWN3,NWN4,ERROR,0)
   RPYI=VALUE_AUXFOB(1)
   RPXI=VALUE_AUXFOB(2)
   RPUYI=VALUE_AUXFOB(3)
   RPUXI=VALUE_AUXFOB(4)
   RPCYI=VALUE_AUXFOB(5)
   RPCXI=VALUE_AUXFOB(6)
   RPUCYI=VALUE_AUXFOB(7)
   RPUCXI=VALUE_AUXFOB(8)
   JK20=DBLE(NEWOBJ)
   ERROR=0
   NWN1=0.0D0
   NWN2=0.0D0
   NWN3=0.0D0
   NWN4=sys_wl_ref()
   CALL GNPR12345678(INT(JK20),NWN1,NWN2,NWN3,NWN4,ERROR,0)
   RPYO=VALUE_AUXFOB(1)
   RPXO=VALUE_AUXFOB(2)
   RPUYO=VALUE_AUXFOB(3)
   RPUXO=VALUE_AUXFOB(4)
   RPCYO=VALUE_AUXFOB(5)
   RPCXO=VALUE_AUXFOB(6)
   RPUCYO=VALUE_AUXFOB(7)
   RPUCXO=VALUE_AUXFOB(8)
   JK20=1.0
   ERROR=0
   NWN1=0.0D0
   NWN2=0.0D0
   NWN3=0.0D0
   NWN4=sys_wl_ref()
   CALL GNPR12345678(INT(JK20),NWN1,NWN2,NWN3,NWN4,ERROR,0)
   RPY1=VALUE_AUXFOB(1)
   RPX1=VALUE_AUXFOB(2)
   RPUY1=VALUE_AUXFOB(3)
   RPUX1=VALUE_AUXFOB(4)
   RPCY1=VALUE_AUXFOB(5)
   RPCX1=VALUE_AUXFOB(6)
   RPUCY1=VALUE_AUXFOB(7)
   RPUCX1=VALUE_AUXFOB(8)
   JK20=DBLE(NEWREF)
   ERROR=0
   NWN1=0.0D0
   NWN2=0.0D0
   NWN3=0.0D0
   NWN4=sys_wl_ref()
   CALL GNPR12345678(INT(JK20),NWN1,NWN2,NWN3,NWN4,ERROR,0)
   RPYR=VALUE_AUXFOB(1)
   RPXR=VALUE_AUXFOB(2)
   RPUYR=VALUE_AUXFOB(3)
   RPUXR=VALUE_AUXFOB(4)
   RPCYR=VALUE_AUXFOB(5)
   RPCXR=VALUE_AUXFOB(6)
   RPUCYR=VALUE_AUXFOB(7)
   RPUCXR=VALUE_AUXFOB(8)
!       BROUGHT BACK FROM VERSION 2.80 TO RESTORE DIFFERENTIAL
!       RAYTRACE CALCS FOR FIRST ORDER LENS VALUES
!
   IF(ABS(surf_clap_type(NEWREF)).GE.1.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.4.0D0) THEN
!
!       CLAP IS ON REFERENCE SURFACE
!
!       CIRCULAR CLAP
!
      IF(ABS(surf_clap_type(NEWREF)).EQ.1.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 1)
!       NOT CIRCULAR CLAP
      END IF
!        RECT CLAP
!
      IF(ABS(surf_clap_type(NEWREF)).GE.2.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.4.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 2)
      END IF
   ELSE
      RFX=PXTRAX(1,NEWREF)
      RFY=PXTRAY(1,NEWREF)
   END IF
!
!
   IF(RPUCXR.NE.0.0D0) THEN
      MAGXOR=RPUCXO/RPUCXR
   ELSE
      IF(RPUCXO.GE.0.0D0) MAGXOR=1.0D10
      IF(RPUCXO.LT.0.0D0) MAGXOR=-1.0D10
   END IF
   IF(RPUCYR.NE.0.0D0) THEN
      MAGYOR=RPUCYO/RPUCYR
   ELSE
      IF(RPUCYO.GE.0.0D0) MAGYOR=1.0D10
      IF(RPUCYO.LT.0.0D0) MAGYOR=-1.0D10
   END IF
!     MAGX AND MAGY
   IF(RPUCXO.NE.0.0D0) THEN
      RMAGX=RPUCXI/RPUCXO
   ELSE
      IF(RPUCXI.GE.0.0D0) RMAGX=1.0D10
      IF(RPUCXI.LT.0.0D0) RMAGX=-1.0D10
   END IF
   IF(RPUCYO.NE.0.0D0) THEN
      RMAGY=RPUCYI/RPUCYO
   ELSE
      IF(RPUCYI.GE.0.0D0) RMAGY=1.0D10
      IF(RPUCYI.LT.0.0D0) RMAGY=-1.0D10
   END IF
!     EFLX,EFLY,FFLX,FFLY,BFLX,FFLY
!
   IF(((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)REFLY=-(((RPUYO*RPCY1)-(RPY1*RPUCYO ))/((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)REFLX=-(((RPUXO*RPCX1)-(RPX1*RPUCXO ))/((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
   IF(((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)RBFLY=-(((RPUYO*RPCYI)-(RPUCYO*RPYI))/((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)RBFLX=-(((RPUXO*RPCXI)-(RPUCXO*RPXI))/((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
   IF(((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)RFFLY=-(((RPY1*RPUCYI)-(RPUYI*RPCY1 ))/((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)RFFLX=-(((RPX1*RPUCXI)-(RPUXI*RPCX1 ))/((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
!     FFNX,FFNY,BFNX,BFNY
   IF(RPUXO.NE.0.0D0) THEN
      RFFNX=-0.5D0/(RPUXO)
   ELSE
      IF(RFFLX.GE.0.0D0) RFFNX=1.0D10
      IF(RFFLX.LT.0.0D0) RFFNX=-1.0D10
   END IF
   IF(RPUYO.NE.0.0D0) THEN
      RFFNY=-0.5D0/(RPUYO)
   ELSE
      IF(RFFLY.GE.0.0D0) RFFNY=1.0D10
      IF(RFFLY.LT.0.0D0) RFFNY=-1.0D10
   END IF
   IF(RPUXI.NE.0.0D0) THEN
      RBFNX=-0.5D0/(RPUXI)
   ELSE
      IF(RBFLX.GE.0.0D0) RBFNX=1.0D10
      IF(RBFLX.LT.0.0D0) RBFNX=-1.0D10
   END IF
   IF(RPUYI.NE.0.0D0) THEN
      RBFNY=-0.5D0/(RPUYI)
   ELSE
      IF(RBFLY.GE.0.0D0) RBFNY=1.0D10
      IF(RBFLY.LT.0.0D0) RBFNY=-1.0D10
   END IF
   RETURN
END
SUBROUTINE ENERGY_ADJUST(ENERGY_FACTOR,I,J,IA,IAP,RN1,RN2,WA3,POLANG,FACT_PAR,FACT_PER,PHASE_PAR,PHASE_PER,PATHL)
!     THIS ADJUSTS RAY ENERGY DUE TO COATING LOSSES
!     AND SETS UP ABSORBTION COEFFICIENT DATA FOR PATH TO THE NEXT SURFACE.
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   INTEGER I,VALUE,J,WA3,K,ALLOERR,NLAY,L
   CHARACTER CNAME*8,AJ4*8,AB*8
   LOGICAL EXISTCF
   real(real64) ENERGY_FACTOR,FACTOR,IA,IAP,RN1,RN2,EFF,POLANG,PATHL,ACTIVE_WAVE,THI,LA,MGF2,MGIND,ZNSIND,R1,R2,IAM,T
   real(real64) NN1,NN2,NN3,NN4,NN5,NN6,NN7,NN8,NN9,NN10
   real(real64) KK1,KK2,KK3,KK4,KK5,KK6,KK7,KK8,KK9,KK10
   real(real64) FACT_PAR,FACT_PER,PHASE_PAR,PHASE_PER
   COMPLEX*16 CINDEX,SMALLTPAR,SMALLTPER,SMALLRPAR,SMALLRPER
   DIMENSION CINDEX(:,:)
   CHARACTER*13 LAYERNAME
   DIMENSION LAYERNAME(:)
   COMPLEX*16 THICKNESS,THETAI,THETAIP,PHI,INDEXI,INDEXIP,INDEXJ ,THETAJ,INDEXL
   COMPLEX*16 COSTHETAL,ULPAR,ULPER,THICKNESSL ,ROOTONE
   COMPLEX*16 MPAR,MPER,MPARL,MPERL,CPII,CTWO,CACTIVE_WAVE,DELTAL
   real(real64) T1,T2,T3,T4,T5,T6,CTEMPR,CTEMPI
   COMPLEX*16 CTEMP
   DIMENSION MPAR(2,2),MPER(2,2),MPARL(2,2),MPERL(2,2)
   DIMENSION THICKNESS(:)
   ALLOCATABLE :: CINDEX,LAYERNAME,THICKNESS
   LOGICAL TIR
   COMMON/RIT/TIR
   real(real64) BIGTPAR,BIGTPER,BIGRPAR,BIGRPER
   real(real64) PHASETPAR,PHASETPER,PHASERPAR,PHASERPER
   real(real64) R_BIGTPAR,R_BIGRPAR,R_BIGTPER,R_BIGRPER
   real(real64) R_SMALLTPAR,R_SMALLRPAR,R_SMALLTPER,R_SMALLRPER
   FACT_PAR=0.0D0
   FACT_PER=0.0D0
   PHASE_PAR=0.0D0
   PHASE_PER=0.0D0
!
!       COSPOL IS AN IMPORTANT NUMBER. IT IS THE COSINE OF THE ANGLE
!       BETWEEN THE INCIDENT PLANE DIRECTION AND THE Y=PLANE OF THE RAY
!       FOR ON-AXIS IN THE Y-Z PLANE OF A CENTERED SYSTEM, THE ANGLE IS 0
!       FOR ON-AXIS RAYS IN THE X-Z PLANE OF A CENTERED SYSTEM, THE ANGLE IS
!       PII/2
!
!     NOTE THAT IT IS ASSUMED THAT THE IMAGINARY PART OF THE INCIDENT MEDIUM'S
!     REFRACTIVE INDEX IS ALWAYS ZERO.
!     NOTE THAT IT IS ASSUMED THAT THE IMAGINARY PART OF THE SUBSTRATE MEDIUM'S
!     REFRACTIVE INDEX IS ALWAYS ZERO.
!     TIR CARRIES THE TIR STATE OF THE RAY
!     ACTIVE WAVELENGTH IN MICRONS
   ACTIVE_WAVE=sys_wavelength(WA3)
   CACTIVE_WAVE=DCMPLX(ACTIVE_WAVE,0.0D0)
   FACTOR=0.0D0
!       COMPLEX REPRESENTATION OF i, THE ROOT OF -1
   ROOTONE=DCMPLX(0.0D0,1.0D0)
!       COMPLEX REPRESENTATION OF PI
   CPII=DCMPLX(3.14159265358979323846D0,0.0D0)
!       COMPLEX REPRESENTATION OF 2.0
   CTWO=DCMPLX(2.0D0,0.0D0)
!     I IS THE SURFACE NUMBER
!     J IS THE FILE NAME/NUMBER
!       SET COMPLEX INDECIES AND ANGLES
!       INDEX OF THE INCIDENT MEDIUM
   INDEXI=DCMPLX(RN1,0.0D0)
!       INDEX OF THE SUBSTRATE
   INDEXIP=DCMPLX(RN2,0.0D0)
!       ANGLE OF INCIDENCE
   THETAI=DCMPLX(IA,0.0D0)
!       ANGLE OF REFRACTION,REFLECTION OR DIFFRACTION
   THETAIP=DCMPLX(IAP,0.0D0)
!
   IF(J.LT.1.OR.J.GT.1000) THEN
!     COATING TYPE 0, NO LOSSES, NO COATING FILE WAS ASSIGNED
      ENERGY_FACTOR=1.0D0
      POLANG=0.0D0
!     NO ABSORBTION LOSSES
      RETURN
   END IF
!
!     J REPRESENTS A FILE NUMBER. SEE IF IT EXISTS
   CALL ITOA4(AJ4,J)
   CNAME='COAT'//AJ4
   EXISTCF=.FALSE.
   INQUIRE(FILE=CNAME//'.DAT',EXIST=EXISTCF)
   IF(.NOT.EXISTCF) THEN
!     NO COATING FILE FOR THAT COATING NUMBER EXISTS, NO LOSSES
      ENERGY_FACTOR=1.0D0
      POLANG=0.0D0
!     NO ABSORBTION LOSSES
      RETURN
   ELSE
!     COATING FILE EXISTS, CLOSE IT, THEN OPEN IT
      CALL CLOSE_FILE(34,1)
      OPEN(UNIT=34,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=CNAME//'.DAT',STATUS='UNKNOWN')
   END IF
!       READ THE COATING TYPE
   READ(UNIT=34,FMT=*,ERR=999,END=999) K
   IF(K.NE.1.AND.K.NE.2.AND.K.NE.3.AND.K.NE.4) THEN
      CALL CLOSE_FILE(34,1)
!       NO COATING, NO LOSSES
      ENERGY_FACTOR=1.0D0
      POLANG=0.0D0
!     NO ABSORBTION LOSSES
      RETURN
   END IF

   IF(K.EQ.1) THEN
!     COATING TYPE 1
!     NO LOSSES
      ENERGY_FACTOR=1.0D0
      OLDABSCOEF(1:10)=ABSCOEF(1:10)
      ENERGY_FACTOR=ENERGY_FACTOR*DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
!     CHECK FOR ABSORB
!     SET DEFAULT
      ABSCOEF(1:10)=0.0D0
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) AB
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(1)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(2)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(3)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(4)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(5)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(6)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(7)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(8)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(9)
      READ(UNIT=34,FMT=*,ERR=9991,END=9991) ABSCOEF(10)
9991  CALL CLOSE_FILE(34,1)
      RETURN
      POLANG=0.0D0
   END IF

   IF(K.EQ.2) THEN
      K=5
!     NO COATING IS THE SAME AS A HALF-WAVE COATING
   END IF

   IF(K.EQ.3) THEN
!     COATING TYPE 3, FRACTIONAL EFFICIENCY COATING
      READ(UNIT=34,FMT=*,ERR=999,END=999) EFF
      ENERGY_FACTOR=EFF
      OLDABSCOEF(1:10)=ABSCOEF(1:10)
      ENERGY_FACTOR=ENERGY_FACTOR*DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
!     CHECK FOR ABSORB
!     SET DEFAULT
      ABSCOEF(1:10)=0.0D0
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) AB
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(1)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(2)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(3)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(4)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(5)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(6)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(7)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(8)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(9)
      READ(UNIT=34,FMT=*,ERR=9992,END=9992) ABSCOEF(10)
9992  CALL CLOSE_FILE(34,1)
!     FIX FOR TIR
      IF(TIR) ENERGY_FACTOR=1.0D0
      CALL CLOSE_FILE(34,1)
      POLANG=0.0D0
      RETURN
   END IF
!

   IF(K.EQ.4.OR.K.EQ.5) THEN
!     COATING TYPE 4
      ALLOCATE (CINDEX(1:100,1:10),LAYERNAME(1:100),THICKNESS(1:100),STAT=ALLOERR)
!     A MULTILAYER COATING, ASSUMING NO-COHERENCE
!     IF RN1 AND RN2 HAVE OPPOSITE SIGNS THEN WE HAVE A REFLECTION
!     SO WE USE THE R VALUE
!     IF RN1 AND RN2 HAVE THE SAME SIGNS THEN WE HAVE A REFRACTION
!     SO WE USE THE T VALUE
      NLAY=0
      IF(K.EQ.4) THEN
         DO L=1,100
            READ(UNIT=34,FMT=*,ERR=888,END=888)LAYERNAME(L),NN1,NN2,NN3,NN4,NN5,NN6,NN7,NN8,NN9,NN10,KK1,KK2,KK3,KK4,KK5,KK6,KK7,KK8,KK9,KK10,T
            NLAY=NLAY+1
            CINDEX(L,1)=DCMPLX(NN1,KK1)
            CINDEX(L,2)=DCMPLX(NN2,KK2)
            CINDEX(L,3)=DCMPLX(NN3,KK3)
            CINDEX(L,4)=DCMPLX(NN4,KK4)
            CINDEX(L,5)=DCMPLX(NN5,KK5)
            CINDEX(L,6)=DCMPLX(NN6,KK6)
            CINDEX(L,7)=DCMPLX(NN7,KK7)
            CINDEX(L,8)=DCMPLX(NN8,KK8)
            CINDEX(L,9)=DCMPLX(NN9,KK9)
            CINDEX(L,10)=DCMPLX(NN10,KK10)
            THICKNESS(L)=DCMPLX(T,0.0D0)
         END DO
         OLDABSCOEF(1:10)=ABSCOEF(1:10)
         ENERGY_FACTOR=ENERGY_FACTOR*DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
!     CHECK FOR ABSORB
!     SET DEFAULT
         ABSCOEF(1:10)=0.0D0
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) AB
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(1)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(2)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(3)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(4)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(5)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(6)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(7)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(8)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(9)
         READ(UNIT=34,FMT=*,ERR=9993,END=9993) ABSCOEF(10)
9993     CALL CLOSE_FILE(34,1)
      END IF
      IF(K.EQ.5) THEN
         NLAY=1
      END IF
888   CONTINUE
!     NLAY AT LEAST 1, CONTINUE
!
      DO L=1,NLAY
!     CONSTRUCT THE 2X2 L-LAYER MATRIX
!     INDEXL IS THE COMPLEX INDEX AT THE TRACED WAVELENGTH FOR THE L-TH LAYER
         IF(K.EQ.4) INDEXL=CINDEX(L,WA3)
         IF(K.EQ.5) INDEXL=INDEXIP
!       T1 IS THE REAL PART OF THE LAYER INDEX
         T1=DBLE(INDEXL)
!       T2 IS THE IMAGINARY PART OF THE LAYER INDEX
         T2=DIMAG(INDEXL)
!       T3 IS THE REAL PART OF THE LAYER THICKNESS
         IF(K.EQ.4) T3=DBLE(THICKNESSL)
         IF(K.EQ.5) T3=DBLE(0.50D0*CACTIVE_WAVE)
!       T4 IS THE REAL PART OF THE INCIDENT MEDIA INDEX
         T4=DBLE(INDEXI)
!       T5 IS THE REAL PART OF THE SINE OF THE INCIDENT ANGLE
         T5=DSIN(IA)
!
         CTEMPR=(T1**2)-(T2**2)-((T4**2)*(DSIN(IA)**2))
         CTEMPI=2.0D0*T1*T2
         CTEMP=DCMPLX(CTEMPR,CTEMPI)
         CTEMP=CDSQRT(CTEMP)
         COSTHETAL=CTEMP/INDEXL
!     THICKNESS OF L IN MICRONS IS:
         IF(K.EQ.4) THICKNESSL=THICKNESS(L)
         IF(K.EQ.5) THICKNESSL=DCMPLX(T3)
!     COS(PHIL)*THE COMPLEX INDEX OF THE LAYER IS JUST:
!     U SUB L PARALLEL IS:
         ULPAR=INDEXL/COSTHETAL
!     U SUB L PERPENDICULAR IS:
         ULPER=INDEXL*COSTHETAL
!     DELTAL IS:
         DELTAL=(CTWO*CPII/CACTIVE_WAVE)*(INDEXL*THICKNESSL*COSTHETAL)
!     THE LAYER MATRIX FOR PARALLEL POLARIZATION IS:
         MPARL(1,1)=CDCOS(DELTAL)
         MPARL(1,2)=-(ROOTONE/ULPAR)*CDSIN(DELTAL)
         MPARL(2,1)=-ROOTONE*ULPAR*CDSIN(DELTAL)
         MPARL(2,2)=MPARL(1,1)
!     THE LAYER MATRIX FOR PERPENDICULAR POLARIZATION IS:
         MPERL(1,1)=CDCOS(DELTAL)
         MPERL(1,2)=-(ROOTONE/ULPER)*CDSIN(DELTAL)
         MPERL(2,1)=-ROOTONE*ULPER*CDSIN(DELTAL)
         MPERL(2,2)=MPERL(1,1)
!     MULTIPLY THE CURRENT L-LAYER M-MATRIX INTO THE FINAL M-MATRIX
         IF(L.EQ.1) THEN
            MPAR(1,1)=MPARL(1,1)
            MPAR(1,2)=MPARL(1,2)
            MPAR(2,1)=MPARL(2,1)
            MPAR(2,2)=MPARL(2,2)
            MPER(1,1)=MPERL(1,1)
            MPER(1,2)=MPERL(1,2)
            MPER(2,1)=MPERL(2,1)
            MPER(2,2)=MPERL(2,2)
         ELSE
!     L GREATER THAN 1
            MPAR=MATMUL(MPARL,MPAR)
            MPER=MATMUL(MPERL,MPER)
         END IF
      END DO
!
!       NOW COMPUTE THE AMPLITUDE TRANSMITTANCE AND REFLECTANCE
!       COEFFICIENTS SMALLTPAR, SMALLTPER, SMALLRPAR AND SMALLRPER
      SMALLRPAR=((MPAR(1,1)*INDEXI)+(MPAR(1,2)*INDEXI*INDEXIP)-(MPAR(2,1))-(MPAR(2,2)*INDEXIP))/((MPAR(1,1)*INDEXI)+(MPAR(1,2)*INDEXI*INDEXIP)+(MPAR(2,1))+(MPAR(2,2)*INDEXIP))
      SMALLRPER=((MPER(1,1)*INDEXI)+(MPER(1,2)*INDEXI*INDEXIP)-(MPER(2,1))-(MPER(2,2)*INDEXIP))/((MPER(1,1)*INDEXI)+(MPER(1,2)*INDEXI*INDEXIP)+(MPER(2,1))+(MPER(2,2)*INDEXIP))
      SMALLTPAR=(CTWO*INDEXI)/((MPAR(1,1)*INDEXI)+(MPAR(1,2)*INDEXI*INDEXIP)+(MPAR(2,1))+(MPAR(2,2)*INDEXIP))
      SMALLTPER=(CTWO*INDEXI)/((MPER(1,1)*INDEXI)+(MPER(1,2)*INDEXI*INDEXIP)+(MPER(2,1))+(MPER(2,2)*INDEXIP))
!
!       THE INTENSITY TRANSMISSION AND REFLECTION COEFFICIENTS ARE:
      BIGTPAR=CDABS(INDEXIP/INDEXI)*((CDABS(SMALLTPAR))**2)
      BIGTPER=CDABS(INDEXIP/INDEXI)*((CDABS(SMALLTPER))**2)
      BIGRPAR=CDABS(SMALLRPAR)**2
      BIGRPER=CDABS(SMALLRPER)**2
      PHASETPAR=DATAN2(DBLE(SMALLTPAR),DIMAG(SMALLTPAR))
      PHASETPER=DATAN2(DBLE(SMALLTPER),DIMAG(SMALLTPER))
      PHASERPAR=DATAN2(DBLE(SMALLRPAR),DIMAG(SMALLRPAR))
      PHASERPER=DATAN2(DBLE(SMALLRPER),DIMAG(SMALLRPER))
!       NOW POLANG IS ANGLE BETWEEN THE PLANE OF INCIDENCE AND THE
!       PLANE FORMED BY THE "Y" RAY AND THE "Z" RAY VECTORS
!       THE T AND R COMPONENTS IN THE "Y" DIRECTION ARE:
      IF(RN1.GT.0.0D0.AND.RN2.GT.0.0D0.OR.RN1.LT.0.0D0.AND.RN2.LT.0.0D0) THEN
!     REFRACTION
         OLDABSCOEF(1:10)=ABSCOEF(1:10)
         ENERGY_FACTOR=(BIGTPAR+BIGTPER)/2.0D0
         ENERGY_FACTOR=ENERGY_FACTOR*DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
         FACT_PAR=BIGTPAR
         FACT_PER=BIGTPER
         PHASE_PAR=PHASETPAR
         PHASE_PER=PHASETPER
!     CHECK FOR ABSORB
!     SET DEFAULT
         ABSCOEF(1:10)=0.0D0
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) AB
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(1)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(2)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(3)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(4)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(5)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(6)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(7)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(8)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(9)
         READ(UNIT=34,FMT=*,ERR=9994,END=9994) ABSCOEF(10)
9994     CALL CLOSE_FILE(34,1)
      ELSE

!     REFLECTION
         ENERGY_FACTOR=(BIGRPAR+BIGRPER)/2.0D0
         OLDABSCOEF(1:10)=ABSCOEF(1:10)
         ENERGY_FACTOR=ENERGY_FACTOR*DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
         FACT_PAR=BIGRPAR
         FACT_PER=BIGRPER
         PHASE_PAR=PHASERPAR
         PHASE_PER=PHASERPER
!     CHECK FOR ABSORB
!     SET DEFAULT
         ABSCOEF(1:10)=0.0D0
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) AB
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(1)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(2)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(3)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(4)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(5)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(6)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(7)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(8)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(9)
         READ(UNIT=34,FMT=*,ERR=9995,END=9995) ABSCOEF(10)
9995     CALL CLOSE_FILE(34,1)
         IF(TIR) ENERGY_FACTOR=1.0D0
         IF(TIR) THEN
            PHASE_PAR=-PII/2.0D0
            PHASE_PER=-PII/2.0D0
            FACT_PAR=1.0D0
            FACT_PER=1.0D0
            POLANG=0.0D0
         END IF
      END IF
      DEALLOCATE (CINDEX,LAYERNAME,THICKNESS,STAT=ALLOERR)
      POLEXT=.TRUE.
      RETURN
   END IF
!**********************************************************************
999 ENERGY_FACTOR=1.0D0
   POLANG=0.0D0
   CALL CLOSE_FILE(34,1)
   DEALLOCATE (CINDEX,LAYERNAME,THICKNESS,STAT=ALLOERR)
!       FILE READ ERROR
!**********************************************************************
   RETURN
END
SUBROUTINE DIFFRACTION_EFFICIENCY(ENERGY_FACTOR,I,IA,WA3)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   INTEGER I,WA3
   real(real64) ENERGY_FACTOR,IA,A,WAV,WAVB,M,B1,B2,B3
!     M IS ORDER
   IF(M.EQ.0.0D0) THEN
      ENERGY_FACTOR=1.0D0
      RETURN
   END IF
   M=surf_grating_order(I)
!     A IS LINE SPACING IN MICRONS
   IF(sys_units().EQ.1.0D0) A=surf_grating_spacing(I)*25.4D0*1.0D3
   IF(sys_units().EQ.2.0D0) A=surf_grating_spacing(I)*10.0D0*1.0D3
   IF(sys_units().EQ.3.0D0) A=surf_grating_spacing(I)*1.0D3
   IF(sys_units().EQ.4.0D0) A=surf_grating_spacing(I)*1.0D6
!     WAVB IS THE REFERENCE WAVELENGTH (BLAZE WAVELEGTH)
   WAVB=sys_wavelength(INT(sys_wl_ref()))
!     WAV IS THE CURRENT TRACE WAVELENGTH
   WAV=sys_wavelength(WA3)
   B1=DASIN((M*WAV/A)-DSIN(IA))
   B2=DASIN((M*WAVB/A)-DSIN(IA))
   B3=(PII*A/WAV)*(B1-B2)
   IF((B3**2).EQ.0.0D0) THEN
      ENERGY_FACTOR=1.3D0
   ELSE
      ENERGY_FACTOR=1.3D0*(DSIN(B3)**2)/(B3**2)
   END IF
   RETURN
END
! SUB ENPU.FOR
SUBROUTINE ENPU(PX,PY,PZ,DIAX,DIAY,DIST)
!
   use DATLEN
   use mod_surface
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE ENPU.FOR.
!       IT CALCULATES THE X,Y AND Z COORDINIATES
!       OF THE REAL ENTRANCE PUPIL IN THE COORDINATE SYSTEM OF THE
!       NEWOBJ+1 SURFACE
!
   real(real64) XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,LO,LOOX,LOOY,MO,MOOX,MOOY,RSHIFTY,RSHIFTX,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RFX,RFY,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,LOO,MOO,NOO,PX,PY,PZ,DIST,DIAX,DIAY,XRX,XRY,YRX,YRY
!
   real(real64) X11X,Y11X,Z11X,L11X,M11X,N11X,X11Y,Y11Y,Z11Y,L11Y,M11Y,N11Y
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
!
!       DO A REAL RAY INTERNAL EXIT PUPIL ADJUSTMENT
!
!
   IF(ABS(surf_clap_type(NEWREF)).GE.1.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.6.0D0.AND.surf_multi_clap_flag(NEWREF).EQ.0.0D0) THEN
!
!       CLAP IS ON REFERENCE SURFACE
!
!       CIRCULAR CLAP
!
      IF(ABS(surf_clap_type(NEWREF)).EQ.1.0D0) THEN
         IF(surf_clap_dim(NEWREF, 1).LE.surf_clap_dim(NEWREF, 2)) THEN
            RFY=surf_clap_dim(NEWREF, 1)
            RFX=surf_clap_dim(NEWREF, 1)
         ELSE
            RFY=surf_clap_dim(NEWREF, 2)
            RFX=surf_clap_dim(NEWREF, 2)
         END IF
!       NOT CIRCULAR CLAP
      END IF
      IF(ABS(surf_clap_type(NEWREF)).EQ.5.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 1)
!       NOT POLY CLAP
      END IF
      IF(ABS(surf_clap_type(NEWREF)).EQ.6.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 5)
         RFX=surf_clap_dim(NEWREF, 5)
!       NOT IPOLY CLAP
      END IF
!
      IF(ABS(surf_clap_type(NEWREF)).GE.2.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.4.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 2)
      END IF
   ELSE
      RFX=PXTRAX(1,NEWREF)
      RFY=PXTRAY(1,NEWREF)
   END IF
!
!
   XO=REFRY(1,NEWOBJ+1)
   YO=REFRY(2,NEWOBJ+1)
   ZO=REFRY(3,NEWOBJ+1)
   LO=REFRY(19,NEWOBJ+1)
   MO=REFRY(20,NEWOBJ+1)
   NO=REFRY(21,NEWOBJ+1)
!
   XOOX=RFDIFF(1,NEWOBJ+1)
   YOOX=RFDIFF(2,NEWOBJ+1)
   ZOOX=RFDIFF(3,NEWOBJ+1)
   LOOX=RFDIFF(13,NEWOBJ+1)
   MOOX=RFDIFF(14,NEWOBJ+1)
   NOOX=RFDIFF(15,NEWOBJ+1)
!
   XOOY=RFDIFF(7,NEWOBJ+1)
   YOOY=RFDIFF(8,NEWOBJ+1)
   ZOOY=RFDIFF(9,NEWOBJ+1)
   LOOY=RFDIFF(16,NEWOBJ+1)
   MOOY=RFDIFF(17,NEWOBJ+1)
   NOOY=RFDIFF(18,NEWOBJ+1)
!
   X11X=DIFF(1,NEWOBJ+1)
   Y11X=DIFF(2,NEWOBJ+1)
   Z11X=DIFF(3,NEWOBJ+1)
   L11X=DIFF(13,NEWOBJ+1)
   M11X=DIFF(14,NEWOBJ+1)
   N11X=DIFF(15,NEWOBJ+1)
!
   X11Y=DIFF(7,NEWOBJ+1)
   Y11Y=DIFF(8,NEWOBJ+1)
   Z11Y=DIFF(9,NEWOBJ+1)
   L11Y=DIFF(16,NEWOBJ+1)
   M11Y=DIFF(17,NEWOBJ+1)
   N11Y=DIFF(18,NEWOBJ+1)
!
   LOO=REFRY(19,NEWOBJ+1)
   MOO=REFRY(20,NEWOBJ+1)
   NOO=REFRY(21,NEWOBJ+1)
   THXNUM=((XO-XOOX)*(LO-LOOX))+((YO-YOOX)*(MO-MOOX))+((ZO-ZOOX)*(NO-NOOX))
   THYNUM=((XO-XOOY)*(LO-LOOY))+((YO-YOOY)*(MO-MOOY))+((ZO-ZOOY)*(NO-NOOY))
   THXDEN=((LO-LOOX)**2)+((MO-MOOX)**2)+((NO-NOOX)**2)
   THYDEN=((LO-LOOY)**2)+((MO-MOOY)**2)+((NO-NOOY)**2)
   IF(DABS(THXNUM*1.0D-10).LT.DABS(THXDEN).AND.DABS(THYNUM*1.0D-10).LT.DABS(THYDEN)) THEN
      DIST=-((-THXNUM/THXDEN)+(-THYNUM/THYDEN))/2.0D0
!     THE EXIT PUPIL IS LOCATED AT:
!     IN THE COORDINATE SYSTEM OF THE NEWOBJ SURFACE
      PX=XO+(-DIST*LOO)
      PY=YO+(-DIST*MOO)
      PZ=ZO+(-DIST*NOO)
      XRX=X11X-(DIST*L11X)
      YRX=Y11X-(DIST*M11X)
      XRY=X11Y-(DIST*L11Y)
      YRY=Y11Y-(DIST*M11Y)
      DIAX=DABS(((2.0D0*DSQRT((XRX**2)+(YRX**2)))/RSHIFTY)*RFX)
      DIAY=DABS(((2.0D0*DSQRT((XRY**2)+(YRY**2)))/RSHIFTX)*RFY)
      RETURN
   END IF
   IF(DABS(THXNUM*1.0D-10).GE.DABS(THXDEN).OR.DABS(THYNUM*1.0D-10).GE.DABS(THYDEN)) THEN
!       REF RAY IS TELECENTRIC, SET DIST TO INFINITY
      DIST=1.0D10
      PX=XO+(-DIST*LOO)
      PY=YO+(-DIST*MOO)
      PZ=ZO+(-DIST*NOO)
      XRX=X11X-(DIST*L11X)
      YRX=Y11X-(DIST*M11X)
      XRY=X11Y-(DIST*L11Y)
      YRY=Y11Y-(DIST*M11Y)
      DIAX=DABS(((2.0D0*DSQRT((XRX**2)+(YRX**2)))/RSHIFTY)*RFX)
      DIAY=DABS(((2.0D0*DSQRT((XRY**2)+(YRY**2)))/RSHIFTX)*RFY)
      RETURN
   END IF
   RETURN
END
! SUB EXPU.FOR
SUBROUTINE EXPU(PX,PY,PZ,DIAX,DIAY,DIST)
!
   use DATLEN
   use mod_surface
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE EXPU.FOR.
!       IT CALCULATES THE X,Y AND Z COORDINIATES
!       OF THE REAL EXIT PUPIL IN THE COORDINATE SYSTEM OF THE
!       NEWIMG SURFACE
!
   real(real64) XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,LO,LOOX,LOOY,MO,MOOX,MOOY,DIAX,DIAY,RSHIFTY,RSHIFTX,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,LOO,MOO,NOO,PX,PY,PZ,DIST,XRX,XRY,YRX,YRY,RFX,RFY
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
   real(real64) X11X,Y11X,Z11X,L11X,M11X,N11X,X11Y,Y11Y,Z11Y,L11Y,M11Y,N11Y
!
!
!       DO A REAL RAY INTERNAL EXIT PUPIL ADJUSTMENT
!
!
   IF(ABS(surf_clap_type(NEWREF)).GE.1.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.6.0D0.AND.surf_multi_clap_flag(NEWREF).EQ.0.0D0) THEN
!
!       CLAP IS ON REFERENCE SURFACE
!
!       CIRCULAR CLAP
!
      IF(ABS(surf_clap_type(NEWREF)).EQ.1.0D0) THEN
         IF(surf_clap_dim(NEWREF, 1).LE.surf_clap_dim(NEWREF, 2)) THEN
            RFY=surf_clap_dim(NEWREF, 1)
            RFX=surf_clap_dim(NEWREF, 1)
         ELSE
            RFY=surf_clap_dim(NEWREF, 2)
            RFX=surf_clap_dim(NEWREF, 2)
         END IF
!       NOT CIRCULAR CLAP
      END IF
      IF(ABS(surf_clap_type(NEWREF)).EQ.5.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 1)
!       NOT POLY CLAP
      END IF
      IF(ABS(surf_clap_type(NEWREF)).EQ.6.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 5)
         RFX=surf_clap_dim(NEWREF, 5)
!       NOT IPOLY CLAP
      END IF
!
      IF(ABS(surf_clap_type(NEWREF)).GE.2.0D0.AND.ABS(surf_clap_type(NEWREF)).LE.4.0D0) THEN
         RFY=surf_clap_dim(NEWREF, 1)
         RFX=surf_clap_dim(NEWREF, 2)
      END IF
   ELSE
      RFX=PXTRAX(1,NEWREF)
      RFY=PXTRAY(1,NEWREF)
   END IF
!
   XO=REFRY(1,NEWIMG)
   YO=REFRY(2,NEWIMG)
   ZO=REFRY(3,NEWIMG)
   LO=REFRY(4,NEWIMG)
   MO=REFRY(5,NEWIMG)
   NO=REFRY(6,NEWIMG)
   XOOX=RFDIFF(1,NEWIMG)
   YOOX=RFDIFF(2,NEWIMG)
   ZOOX=RFDIFF(3,NEWIMG)
   LOOX=RFDIFF(4,NEWIMG)
   MOOX=RFDIFF(5,NEWIMG)
   NOOX=RFDIFF(6,NEWIMG)
   XOOY=RFDIFF(7,NEWIMG)
   YOOY=RFDIFF(8,NEWIMG)
   ZOOY=RFDIFF(9,NEWIMG)
   LOOY=RFDIFF(10,NEWIMG)
   MOOY=RFDIFF(11,NEWIMG)
   NOOY=RFDIFF(12,NEWIMG)
   X11X=DIFF(1,NEWIMG)
   Y11X=DIFF(2,NEWIMG)
   Z11X=DIFF(3,NEWIMG)
   L11X=DIFF(13,NEWIMG)
   M11X=DIFF(14,NEWIMG)
   N11X=DIFF(15,NEWIMG)
!
   X11Y=DIFF(7,NEWIMG)
   Y11Y=DIFF(8,NEWIMG)
   Z11Y=DIFF(9,NEWIMG)
   L11Y=DIFF(16,NEWIMG)
   M11Y=DIFF(17,NEWIMG)
   N11Y=DIFF(18,NEWIMG)
!
   LOO=REFRY(19,NEWIMG)
   MOO=REFRY(20,NEWIMG)
   NOO=REFRY(21,NEWIMG)
   THXNUM=((XO-XOOX)*(LO-LOOX))+((YO-YOOX)*(MO-MOOX))+((ZO-ZOOX)*(NO-NOOX))
   THYNUM=((XO-XOOY)*(LO-LOOY))+((YO-YOOY)*(MO-MOOY))+((ZO-ZOOY)*(NO-NOOY))
   THXDEN=((LO-LOOX)**2)+((MO-MOOX)**2)+((NO-NOOX)**2)
   THYDEN=((LO-LOOY)**2)+((MO-MOOY)**2)+((NO-NOOY)**2)
   IF(DABS(THXNUM*1.0D-10).LT.DABS(THXDEN).AND.DABS(THYNUM*1.0D-10).LT.DABS(THYDEN)) THEN
      DIST=-((-THXNUM/THXDEN)+(-THYNUM/THYDEN))/2.0D0
!     THE EXIT PUPIL IS LOCATED AT:
!     IN THE COORDINATE SYSTEM OF THE IMAGE SURFACE
      PX=XO+(-DIST*LOO)
      PY=YO+(-DIST*MOO)
      PZ=ZO+(-DIST*NOO)
      XRX=X11X-(DIST*L11X)
      YRX=Y11X-(DIST*M11X)
      XRY=X11Y-(DIST*L11Y)
      YRY=Y11Y-(DIST*M11Y)
      DIAX=DABS(((2.0D0*DSQRT((XRX**2)+(YRX**2)))/RSHIFTY)*RFX)
      DIAY=DABS(((2.0D0*DSQRT((XRY**2)+(YRY**2)))/RSHIFTX)*RFY)
      RETURN
   END IF
   IF(DABS(THXNUM*1.0D-10).GE.DABS(THXDEN).OR.DABS(THYNUM*1.0D-10).GE.DABS(THYDEN)) THEN
!       REF RAY IS TELECENTRIC, SET DIST TO INFINITY
      DIST=1.0D10
      PX=XO+(-DIST*LOO)
      PY=YO+(-DIST*MOO)
      PZ=ZO+(-DIST*NOO)
      XRX=X11X-(DIST*L11X)
      YRX=Y11X-(DIST*M11X)
      XRY=X11Y-(DIST*L11Y)
      YRY=Y11Y-(DIST*M11Y)
      DIAX=DABS(((2.0D0*DSQRT((XRX**2)+(YRX**2)))/RSHIFTY)*RFX)
      DIAY=DABS(((2.0D0*DSQRT((XRY**2)+(YRY**2)))/RSHIFTX)*RFY)
      RETURN
   END IF
   RETURN
END
! SUB EXPUP.FOR
SUBROUTINE EXPUP
!
   use DATLEN
   use mod_surface
   use DATMAI
   use command_utils, only: is_command_query
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!       THIS CONTROLS THE OPERATION OF THE "EXPUP (AUTO or NOAUTO)
!       COMMAND
!
!
   IF(is_command_query()) THEN
      IF(EXPAUT) THEN
!     AUTO IS SET
100      FORMAT('AUTOMATIC EXIT PUPIL LOCATION CALCULATION IS IN EFFECT')
         WRITE(OUTLYNE,100)
         CALL SHOWIT(1)
      ELSE
!     AUTO IS NOT SET
200      FORMAT('AUTOMATIC EXIT PUPIL LOCATION CALCULATION IS NOT IN EFFECT')
         WRITE(OUTLYNE,200)
         CALL SHOWIT(1)
      END IF
      RETURN
   END IF
   IF(SN.EQ.1.OR.SST.EQ.1) THEN
      WRITE(OUTLYNE,*)'"EXPUP" ONLY TAKES QUALIFIER INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SQ.EQ.1) THEN
      IF(WQ.NE.'AUTO'.AND.WQ.NE.'NOAUTO') THEN
         WRITE(OUTLYNE,*)'INVALID QUALIFIER USED WITH "EXPUP"'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
      END IF
   END IF
   IF(SQ.EQ.0.AND..not. is_command_query()) THEN
      WRITE(OUTLYNE,*)'"EXPUP" REQUIRES EXPLICIT QUALIFIER INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WQ.EQ.'AUTO') THEN
      EXPAUT=.TRUE.
300   FORMAT('AUTOMATIC EXIT PUPIL LOCATION CALCULATION HAS BEEN TRUNED "ON"')
      WRITE(OUTLYNE,300)
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WQ.EQ.'NOAUTO') THEN
      EXPAUT=.FALSE.
400   FORMAT('AUTOMATIC EXIT PUPIL LOCATION CALCULATION HAS BEEN TURNED "OFF"')
      WRITE(OUTLYNE,400)
      CALL SHOWIT(1)
      RETURN
   END IF
   RETURN
END
! SUB INSID1.FOR
!
LOGICAL FUNCTION INSID1()
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!  RESULT IS .TRUE. IF (X0,Y0) LIES INSIDE THE NP_GON
!  OR ON ITS BOUNDARY OR ON ITS POINTS
!  THIS ROUTINE IS USED TO CHECK CLEAR APERTURES
!  WHERE TRUE MEANS NO-BLOCKING AND FALSE MEANS BLOCKED RAYS
!  DEFINED BY THE TABLES OF YT VS. XT.
!
   real(real64) TUPI,ANGL(1:200),ARG
!
   INTEGER I
!
!
   TUPI=TWOPII
   IF(REAL(Y0).EQ.REAL(YT(1)).AND.REAL(X0).EQ.REAL(XT(1))) THEN
      INSID1=.TRUE.
      RETURN
   ELSE
      IF(DABS(YT(1)-Y0).LE.1.0D-15.AND.DABS(XT(1)-X0).LE.1.0D-15) THEN
         ANGL(1)=0.0D0
      ELSE
         ANGL(1) = DATAN2( YT(1)-Y0, XT(1)-X0)
      END IF
      IF(ANGL(1).LT.0.0D0) ANGL(1)=TUPI+ANGL(1)
   ENDIF
   DO I = 2, NP
      IF(REAL(Y0).EQ.REAL(YT(I)).AND.REAL(X0).EQ.REAL(XT(I))) THEN
         INSID1=.TRUE.
         RETURN
      ELSE
         IF(DABS(YT(I)-Y0).LE.1.0D-15.AND.DABS(XT(I)-X0).LE.1.0D-15) THEN
            ANGL(I)=0.0D0
         ELSE
            ANGL(I) = DATAN2( YT(I)-Y0, XT(I)-X0)
         END IF
         IF(ANGL(I).LT.0.0D0) ANGL(I)=TUPI+ANGL(I)
      ENDIF
   END DO
   INSID1=.TRUE.
   ARG=ANGL(1)-ANGL(NP)
   IF(ARG.LT.0.0D0) ARG=ARG+TUPI
   IF(ARG.GT.(PII+1.0D-10)) THEN
      INSID1=.FALSE.
      RETURN
   END IF
   DO I=NP,2,-1
      ARG=ANGL(I)-ANGL(I-1)
      IF(ARG.LT.0.0D0) ARG=ARG+TUPI
      IF(ARG.GT.(PII+1.0D-10)) THEN
         INSID1=.FALSE.
         RETURN
      END IF
   END DO
   RETURN
END
! SUB INSID2.FOR
!
LOGICAL FUNCTION INSID2()
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!  RESULT IS .TRUE. IF (X0,Y0) LIES INSIDE THE NP_GON
!  AND FALSE IF ON ITS BOUNDARY OR ON ITS CORNER POINTS
!  THIS ROUTINE IS USED TO CHECK OBSCURATIONS
!  WHERE TRUE MEANS BLOCKING AND FALSE MEANS NO BLOCKED RAYS
!  DEFINED BY THE TABLES OF YT VS. XT.
!
   real(real64) TUPI,ANGL(1:200),ARG
!
   INTEGER I
!
!
   TUPI=TWOPII
   IF(REAL(Y0).EQ.REAL(YT(1)).AND.REAL(X0).EQ.REAL(XT(1))) THEN
      INSID2=.FALSE.
      RETURN
   ELSE
      IF(DABS(YT(1)-Y0).LE.1.0D-15.AND.DABS(XT(1)-X0).LE.1.0D-15) THEN
         ANGL(1)=0.0D0
      ELSE
         ANGL(1) = DATAN2( YT(1)-Y0, XT(1)-X0)
      END IF
      IF(ANGL(1).LT.0.0D0) ANGL(1)=TUPI+ANGL(1)
   ENDIF
   DO I = 2, NP
      IF(REAL(Y0).EQ.REAL(YT(I)).AND.REAL(X0).EQ.REAL(XT(I))) THEN
         INSID2=.FALSE.
         RETURN
      ELSE
         IF(DABS(YT(I)-Y0).LE.1.0D-15.AND.DABS(XT(I)-X0).LE.1.0D-15) THEN
            ANGL(I)=0.0D0
         ELSE
            ANGL(I) = DATAN2( YT(I)-Y0, XT(I)-X0)
         END IF
         IF(ANGL(I).LT.0.0D0) ANGL(I)=TUPI+ANGL(I)
      ENDIF
   END DO
   INSID2=.TRUE.
   ARG=ANGL(1)-ANGL(NP)
   IF(ARG.LT.0.0D0) ARG=ARG+TUPI
   IF(ARG.GT.(PII-1.0D-10)) THEN
      INSID2=.FALSE.
      RETURN
   END IF
   DO I=NP,2,-1
      ARG=ANGL(I)-ANGL(I-1)
      IF(ARG.LT.0.0D0) ARG=ARG+TUPI
      IF(ARG.GT.(PII-1.0D-10)) THEN
         INSID2=.FALSE.
         RETURN
      END IF
   END DO
   RETURN
END
!
!     THIS ROUTINE DOES CROSS TRACK SPECTRAL
!
SUBROUTINE CTS(VALVAL,TYPE,V1,V2,ERROR)
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   INTEGER ERROR,TYPE
   real(real64) VALVAL,V1,V2,YMAX,YMIN,YV1,YV2,YV3
   LOGICAL OLDLDIF,OLDLDIF2
!
!     IF TYPE=1 XFOB
!     IF TYPE=2 YFOB
   YV1=0.0D0
   YV2=0.0D0
   YV3=0.0D0
!     FOCAL AND UFOCAL ONLY
   IF(sys_mode().GE.3.0D0) THEN
      ERROR=1
      RETURN
   END IF
!
!     CHECK WAVELENGTH
   IF(V1.NE.1.0D0.AND.V1.NE.2.0D0.AND.V1.NE.3.0D0.AND.V1.NE.4.0D0.AND.V1.NE.5.0D0.AND.V1.NE.6.0D0.AND.V1.NE.7.0D0.AND.V1.NE.8.0D0.AND.V1.NE.9.0D0.AND.V1.NE.10.0D0) THEN
      ERROR=1
      RETURN
   END IF
   IF(V1.EQ.1.0D0.AND.sys_wavelength(1).EQ.0.0D0.OR.V1.EQ.2.0D0.AND.sys_wavelength(2).EQ.0.0D0.OR.V1.EQ.3.0D0.AND.sys_wavelength(3).EQ.0.0D0.OR.V1.EQ.4.0D0.AND.sys_wavelength(4).EQ.0.0D0.OR.V1.EQ.5.0D0.AND.sys_wavelength(5).EQ.0.0D0.OR.V1.EQ.6.0D0.AND.sys_wavelength(6).EQ.0.0D0.OR.V1.EQ.7.0D0.AND.sys_wavelength(7).EQ.0.0D0.OR.V1.EQ.8.0D0.AND.sys_wavelength(8).EQ.0.0D0.OR.V1.EQ.9.0D0.AND.sys_wavelength(9).EQ.0.0D0.OR.V1.EQ.10.0D0.AND.sys_wavelength(10).EQ.0.0D0) THEN
      ERROR=1
      RETURN
   END IF
!     WAVELENGTHS ARE OK
   IF(TYPE.EQ.1) THEN
!     Y COMPONENTS OF XFOB AT DESIGNATED WAVELENGTH
!
!     DO THE FOB 0 1 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=0.0D0
      W2=1.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE FIRST Y VALUE
      YV1=RAYRAY(2,NEWIMG)
!     DO THE FOB 0 0 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=0.0D0
      W2=0.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE SECOND Y VALUE
      YV2=RAYRAY(2,NEWIMG)
!     DO THE FOB 0 -1 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=0.0D0
      W2=-1.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE THIRD Y VALUE
      YV3=RAYRAY(2,NEWIMG)
      YMAX=-1.0D300
      YMIN=1.0D300
      IF(YV1.GT.YMAX) YMAX=YV1
      IF(YV2.GT.YMAX) YMAX=YV2
      IF(YV3.GT.YMAX) YMAX=YV3
      IF(YV1.LT.YMIN) YMIN=YV1
      IF(YV2.LT.YMIN) YMIN=YV2
      IF(YV3.LT.YMIN) YMIN=YV3
      VALVAL=DABS(YMAX-YMIN)
      IF(V2.NE.0.0D0) VALVAL=(VALVAL/V2)*100.0D0
      ERROR=0
      RETURN
   END IF
   IF(TYPE.EQ.2) THEN
!     XCOMPONENTS OF YFOB AT DESIGNATED WAVELENGTH
!
!     DO THE FOB 1 0 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=1.0D0
      W2=0.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE FIRST X VALUE
      YV1=RAYRAY(1,NEWIMG)
!     DO THE FOB 0 0 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=0.0D0
      W2=0.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE SECOND X VALUE
      YV2=RAYRAY(1,NEWIMG)
!     DO THE FOB -1 0 0 V1
      SAVE_KDP(1)=SAVEINPT(1)
      OLDLDIF2=LDIF2
      OLDLDIF=LDIF
      LDIF2=.FALSE.
      LDIF=.FALSE.
      WQ='        '
      SQ=0
      SST=0
      STI=0
      W1=-1.0D0
      W2=0.0D0
      W3=0.0D0
      W4=V1
      W5=0.0D0
      DF1=0
      DF2=0
      DF3=0
      DF4=0
      DF5=1
      S1=1
      S2=1
      S3=1
      S4=1
      S5=0
      SN=1
!     SET MSG TO FALSE
      MSG=.FALSE.
      WC='FOB     '
      CALL FFOB
      IF(.NOT.REFEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
! NOW THE RAY 0 0
      SAVE_KDP(1)=SAVEINPT(1)
      WQ='        '
      SQ=0
      SST=0
      DF1=1
      DF2=1
      DF3=0
      DF4=1
      DF5=1
      S1=0
      S2=0
      S3=1
      S4=0
      S5=0
      SN=0
      W1=0.0D0
      W2=0.0D0
      W3=V1
      W4=0.0D0
      W5=0.0D0
      WC='RAY     '
      NOCOAT=.TRUE.
      GRASET=.FALSE.
      DXFSET=.FALSE.
      CALL RRAY
      IF(.NOT.RAYEXT) THEN
         ERROR=1
         RETURN
      END IF
      REST_KDP(1)=RESTINPT(1)
!     WRITE DOWN THE THIRD X VALUE
      YV3=RAYRAY(1,NEWIMG)
      YMAX=-1.0D300
      YMIN=1.0D300
      IF(YV1.GT.YMAX) YMAX=YV1
      IF(YV2.GT.YMAX) YMAX=YV2
      IF(YV3.GT.YMAX) YMAX=YV3
      IF(YV1.LT.YMIN) YMIN=YV1
      IF(YV2.LT.YMIN) YMIN=YV2
      IF(YV3.LT.YMIN) YMIN=YV3
      VALVAL=DABS(YMAX-YMIN)
      IF(V2.NE.0.0D0) VALVAL=(VALVAL/V2)*100.0D0
      ERROR=0
      RETURN
   END IF
END
SUBROUTINE COFACTOR
   use DATSPD
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
   COHERENCE_FACTOR=W1
   RETURN
END
! SUB CLPCEN.FOR
SUBROUTINE CLPCEN
!
   use DATLEN
   use mod_surface
   use DATMAI
   use mod_system, only: sys_wl_ref, sys_wavelength, sys_units, sys_mode
   use mod_lens_data_manager, only: ldm
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
!     KEEPS TRACK OF FOB 0 0 RAY LOCATIONS AT EACH SURFACE
!
   INTEGER I
!
   real(real64) CLPLCX(0:499),CLPLCY(0:499)
!
   COMMON/CLPLOC/CLPLCX,CLPLCY

!
!
   CLPLCX(NEWOBJ:NEWIMG)=REFRY(1,NEWOBJ:NEWIMG)
   CLPLCY(NEWOBJ:NEWIMG)=REFRY(2,NEWOBJ:NEWIMG)
   RETURN
END
