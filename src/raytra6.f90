...
SUBROUTINE ENPDIAY(VALVAL,ERRR)
   ...
   &,Y1,Z1,L1,M1,N1,XA,XB,YA,YB,ZA,ZB,T &
   &,VYHI,VYLO
   LOGICAL ERRR
!
!
...
!
   ...
   &RPYI,RPXI,RPCYI,RPCXI,RPUYI,RPUXI,RPUCYI,RPUCXI,&
   &RPY1,RPX1,RPCY1,RPCX1,RPUY1,RPUX1,RPUCY1,RPUCX1,&
   &RPYR,RPXR,RPCYR,RPCXR,RPUYR,RPUXR,RPUCYR,RPUCXR,&
   &NWN1,NWN2,NWN3,NWN4,JK20,VALUE,RFX,RFY,RSHIFTX,RSHIFTY
!
...
!
   ...
   &DABS(ALENS(9,NEWREF)).LE.4.0D0) THEN
!
...
!
   IF(&
   &((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)&
   &REFLY=-(((RPUYO*RPCY1)-(RPY1*RPUCYO &
   &))/((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(&
   &((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)&
   &REFLX=-(((RPUXO*RPCX1)-(RPX1*RPUCXO &
   &))/((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
   IF(&
   &((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)&
   &RBFLY=-(((RPUYO*RPCYI)-(RPUCYO*RPYI))/&
   &((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(&
   &((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)&
   &RBFLX=-(((RPUXO*RPCXI)-(RPUCXO*RPXI))/&
   &((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
   IF(&
   &((RPUYO*RPUCYI)-(RPUCYO*RPUYI)).NE.0.0D0)&
   &RFFLY=-(((RPY1*RPUCYI)-(RPUYI*RPCY1 &
   &))/((RPUYO*RPUCYI)-(RPUCYO*RPUYI)))
!
   IF(&
   &((RPUXO*RPUCXI)-(RPUCXO*RPUXI)).NE.0.0D0)&
   &RFFLX=-(((RPX1*RPUCXI)-(RPUXI*RPCX1 &
   &))/((RPUXO*RPUCXI)-(RPUCXO*RPUXI)))
!
!     FFNX,FFNY,BFNX,BFNY
...
&POLANG,FACT_PAR,FACT_PER,PHASE_PAR,PHASE_PER,PATHL)
...
!     AND SETS UP ABSORBTION COEFFICIENT DATA FOR PATH TO THE NEXT SURFACE.
   ...
   &ACTIVE_WAVE,THI,LA,MGF2,MGIND,ZNSIND,R1,R2,IAM,T
...
!     NO LOSSES
      ...
      &DEXP(-DABS(OLDABSCOEF(INT(WA3))*PATHL))
!     CHECK FOR ABSORB
!     SET DEFAULT
      ABSCOEF(1:10)=0.0D0
...
!     SO WE USE THE T VALUE
      ...
      IF(K.EQ.4) THEN
         DO L=1,100
            ...
            &KK1,KK2,KK3,KK4,KK5,KK6,KK7,KK8,KK9,KK10,T
            NLAY=NLAY+1
...
!
   ...
   &LO,LOOX,LOOY,MO,MOOX,MOOY,RSHIFTY,RSHIFTX,&
   &RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RFX,RFY,&
   &RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,&
   &LOO,MOO,NOO,PX,PY,PZ,DIST,DIAX,DIAY,XRX,XRY,YRX,YRY
!
...
!
   REAL*8 XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,&
   &LO,LOOX,LOOY,MO,MOOX,MOOY,DIAX,DIAY,RSHIFTY,RSHIFTX,&
   &RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,&
   &RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,&
   &LOO,MOO,NOO,PX,PY,PZ,DIST,XRX,XRY,YRX,YRY,RFX,RFY
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
   REAL*8 X11X,Y11X,Z11X,L11X,M11X,N11X,X11Y,Y11Y,Z11Y,&
...
!     AUTO IS SET
100      FORMAT(&
         &'AUTOMATIC EXIT PUPIL LOCATION CALCULATION IS IN EFFECT')
         WRITE(OUTLYNE,100)
         CALL SHOWIT(1)
      ELSE
!     AUTO IS NOT SET
200      FORMAT(&
         &'AUTOMATIC EXIT PUPIL LOCATION CALCULATION IS NOT IN EFFECT')
         WRITE(OUTLYNE,200)
         CALL SHOWIT(1)
      END IF
      RETURN
   ...
   IF(WQ.EQ.'AUTO') THEN
      EXPAUT=.TRUE.
300   FORMAT(&
      &'AUTOMATIC EXIT PUPIL LOCATION CALCULATION HAS BEEN TRUNED "ON"')
      WRITE(OUTLYNE,300)
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(WQ.EQ.'NOAUTO') THEN
      EXPAUT=.FALSE.
400   FORMAT(&
      &'AUTOMATIC EXIT PUPIL LOCATION CALCULATION HAS BEEN TURNED "OFF"')
      WRITE(OUTLYNE,400)
      CALL SHOWIT(1)
      RETURN
   END IF
...
!     CHECK WAVELENGTH
   ...
   &V1.EQ.2.0D0.AND.SYSTEM(2).EQ.0.0D0.OR.&
   &V1.EQ.3.0D0.AND.SYSTEM(3).EQ.0.0D0.OR.&
   &V1.EQ.4.0D0.AND.SYSTEM(4).EQ.0.0D0.OR.&
   &V1.EQ.5.0D0.AND.SYSTEM(5).EQ.0.0D0.OR.&
   &V1.EQ.6.0D0.AND.SYSTEM(71).EQ.0.0D0.OR.&
   &V1.EQ.7.0D0.AND.SYSTEM(72).EQ.0.0D0.OR.&
   &V1.EQ.8.0D0.AND.SYSTEM(73).EQ.0.0D0.OR.&
   &V1.EQ.9.0D0.AND.SYSTEM(74).EQ.0.0D0.OR.&
   &V1.EQ.10.0D0.AND.SYSTEM(75).EQ.0.0D0) THEN
      ERROR=1
      RETURN
   END IF
!     WAVELENGTHS ARE OK
