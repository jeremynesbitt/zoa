...
!
   REAL*8 RPYO,RPXO,RPCYO,RPCXO,RPUYO,RPUXO,RPUCYO,RPUCXO,RPYI,RPXI,RPCYI,RPCXI,RPUYI,RPUXI,RPUCYI,RPUCXI,RPY1,RPX1,RPCY1,RPCX1,RPUY1,RPUX1,RPUCY1,RPUCX1,RPYR,RPXR,RPCYR,RPCXR,RPUYR,RPUXR,RPUCYR,RPUCXR,NWN1,NWN2,NWN3,NWN4,JK20,VALUE,RFX,RFY,RSHIFTX,RSHIFTY
!
...
!     AND SETS UP ABSORBTION COEFFICIENT DATA FOR PATH TO THE NEXT SURFACE.
   ...
   LOGICAL EXISTCF
   REAL*8 ENERGY_FACTOR,FACTOR,IA,IAP,RN1,RN2,EFF,POLANG,PATHL,ACTIVE_WAVE,THI,LA,MGF2,MGIND,ZNSIND,R1,R2,IAM,T
...
!     WAVB IS THE REFERENCE WAVELENGTH (BLAZE WAVELEGTH)
   IF(INT(sys_wl_ref()).EQ.1)  WAVB=sys_wavelength(1)
   IF(INT(sys_wl_ref()).EQ.2)  WAVB=sys_wavelength(2)
   IF(INT(sys_wl_ref()).EQ.3)  WAVB=sys_wavelength(3)
   IF(INT(sys_wl_ref()).EQ.4)  WAVB=sys_wavelength(4)
   IF(INT(sys_wl_ref()).EQ.5)  WAVB=sys_wavelength(5)
   IF(INT(sys_wl_ref()).EQ.6)  WAVB=sys_wavelength(6)
   IF(INT(sys_wl_ref()).EQ.7)  WAVB=sys_wavelength(7)
   IF(INT(sys_wl_ref()).EQ.8)  WAVB=sys_wavelength(8)
   IF(INT(sys_wl_ref()).EQ.9)  WAVB=sys_wavelength(9)
   IF(INT(sys_wl_ref()).EQ.10) WAVB=sys_wavelength(10)
!     WAV IS THE CURRENT TRACE WAVELENGTH
   IF(WA3.EQ.1)   WAV=sys_wavelength(1)
   IF(WA3.EQ.2)   WAV=sys_wavelength(2)
   IF(WA3.EQ.3)   WAV=sys_wavelength(3)
   IF(WA3.EQ.4)   WAV=sys_wavelength(4)
   IF(WA3.EQ.5)   WAV=sys_wavelength(5)
   IF(WA3.EQ.6)   WAV=sys_wavelength(6)
   IF(WA3.EQ.7)   WAV=sys_wavelength(7)
   IF(WA3.EQ.8)   WAV=sys_wavelength(8)
   IF(WA3.EQ.9)   WAV=sys_wavelength(9)
   IF(WA3.EQ.10)  WAV=sys_wavelength(10)
...
!       NEWOBJ+1 SURFACE
!
   REAL*8 XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,LO,LOOX,LOOY,MO,MOOX,MOOY,RSHIFTY,RSHIFTX,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RFX,RFY,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,LOO,MOO,NOO,PX,PY,PZ,DIST,DIAX,DIAY,XRX,XRY,YRX,YRY
!
   REAL*8 X11X,Y11X,Z11X,L11X,M11X,N11X,X11Y,Y11Y,Z11Y,L11Y,M11Y,N11Y
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
!
...
!       NEWIMG SURFACE
!
   REAL*8 XO,XOOY,XOOX,YO,YOOX,YOOY,ZO,ZOOX,ZOOY,LO,LOOX,LOOY,MO,MOOX,MOOY,DIAX,DIAY,RSHIFTY,RSHIFTX,RL0,RM0,RN0,NO,NOOX,NOOY,THYNUM,THXNUM,RRX0,RRZ0,T,RX0,RY0,RZ0,RRY0,M0,L0,N0,THXDEN,THYDEN,LOO,MOO,NOO,PX,PY,PZ,DIST,XRX,XRY,YRX,YRY,RFX,RFY
!
   COMMON/SHIFTY/RSHIFTX,RSHIFTY
!
   REAL*8 X11X,Y11X,Z11X,L11X,M11X,N11X,X11Y,Y11Y,Z11Y,L11Y,M11Y,N11Y
