...
!     OPEN AS SEQUENTIAL, SET APPEND TO TRUE
      APPEND=.TRUE.
      IF(APPEND) OPEN(UNIT=8,ACCESS='APPEND',BLANK='NULL',FORM='FORMATTED',FILE='CARDTEXT.DAT',STATUS='UNKNOWN')
      IF(.NOT.APPEND) OPEN(UNIT=8,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='CARDTEXT.DAT',STATUS='UNKNOWN')
   ELSE
   END IF
   LAMB1=TABLE(1,2)
   LAMB2=TABLE(CTAB,2)
...
200 FORMAT('LISTING OF THE "SPECT" TABLE MEMORY AREA FOR FILE (',A8,' )')
201 FORMAT(2X)
202 FORMAT('ENTRY NUMBER',4X,'WAVLENGTH-(MICRONS)',6X,'FUNCTION VALUE')
203 FORMAT('-------------------------------------------------------')
204 FORMAT('SHORTEST WAVELENGTH VALUE CURRENTLY IN THE FILE = ',G18.10,' MICRONS')
205 FORMAT('LONGEST  WAVELENGTH VALUE CURRENTLY IN THE FILE = ',G18.10,' MICRONS')
...
!
   ...
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      OUTLYNE='NO NUMERIC WORD 2, 3, 4, OR 5 INPUT IS USED WHEN'
      CALL SHOWIT(1)
      OUTLYNE='SETTING THE REGISTER VALUES IN THIS WAY'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
...
!       REAL*8
!
   REAL*8 VALUE,W1A,W1B,V1,V2,SIGN ,EFLY,EFLX
!
   INTEGER CW,I,J,NUM5
...
!
   ...
   END IF
!
   IF(WQ.EQ.'FLCLTH'.OR.WQ.EQ.'PWRY'.OR.WQ.EQ.'PWRX'.OR.WQ.EQ.'FLCLTHY'.OR.WQ.EQ.'FLCLTHX') THEN
...
!
   IMPLICIT NONE
!
   REAL*8 Y,P1,P2,P3,P4,P5,Q1,Q2,Q3,Q4,Q5,R1,R2,R3,R4,R5,R6,S1,S2,S3,S4,S5,S6,X,BESSJ1,XX,Z,AX
   ...
   ELSE
      ...
      Y=Z**2
      XX=AX-2.356194491D0
      BESSJ1=DSQRT(.636619772D0/AX)*(DCOS(XX)*(P1+Y*(P2+Y*(P3+Y*(P4+Y *P5))))-Z*DSIN(XX)*(Q1+Y*(Q2+Y*(Q3+Y*(Q4+Y*Q5)))))*DSIGN(1.0D0,X)
   ENDIF
   RETURN
END
! SUB BLANK.FOR
...
!
   ...
   IF(OUT.EQ.7) THEN
      ...
      IF(.NOT.OPEN7) THEN
         OLDOUT=0
         IF(APPEND)OPEN(UNIT=7,ACCESS='APPEND',BLANK='NULL',FORM='FORMATTED',FILE='PRINTER.TXT',STATUS='UNKNOWN')
         IF(.NOT.APPEND)OPEN(UNIT=7,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='PRINTER.TXT',STATUS='UNKNOWN')
      ELSE
!       UNIT 7 WAS OPEN
         OLDOUT=1
!       PROCEED
...
!
   INTEGER SUR,IEND,OLDOUT,OLDSQ,IPASS1,IPASS2 ,I,J,K
!
   CHARACTER C002*4,WCOLD*8,AI*3,AA1*3,AA2*3,AA3*3,AI4*4
!
   REAL*8 SYS50,SYS56,RPASS1,RPASS2,RPASS3
!
   LOGICAL OPEN7,OPEN8,OPEN9,OPEN10,OPEN30,OPEN63 ,EXIS49,OPEN11,OPEN12,OPEN13,OPEN32,EXIS32,EXIS99,EXIS63 ,EXIS70,EXIS72,EXIS94,EXIS95,EXIS96,LPASS1,LPASS2
!
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
...
!
!       SET THE INTEGER PART OF sys_curr_cfg() EQUAL TO 1
!       SET THE INTEGER PART OF sys_high_cfg() EQUAL TO THE HIGHEST
!       NON-EMPTY CONFIG.
...
!       IEND IS THE NUMBER OF THE HIGHEST NUMBERED NON-EMPTY CONFIG
   ...
   SYS50=1.0
   sys_curr_cfg()=SYS50
   SYSP(50)=sys_curr_cfg()
   call set_sys_high_cfg(SYS56)
...
!
      ...
      OPEN(UNIT=89,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=trim(CURLENS)//'LENSTEXT.DAT',STATUS='UNKNOWN')
!
      OLDOUT=OUT
      OLDSQ=SQ
      OUT=89
...
!     SAVE RAY AND FIELD DATA
   OPEN(UNIT=16,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=trim(basePath)//'FIELDS.DAT', STATUS='UNKNOWN')
100 FORMAT(A4,1X,D15.8,1X,D15.8,1X,D15.8,1X,I2)
   DO I=1,200
      CALL ITOAAA(I,AI4)
      WRITE(UNIT=16,FMT=100) AI4,FIELDY(I),FIELDX(I),FIELDZ(I),INT(FIELDW(I))
   END DO
   CALL CLOSE_FILE(16,1)
   OPEN(UNIT=16,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=trim(basePath)//'RAYS.DAT', STATUS='UNKNOWN')
102 FORMAT(A4,1X,D15.8,1X,D15.8,1X,I2)
   DO I=1,5000
      CALL ITOAAA(I,AI4)
      WRITE(UNIT=16,FMT=102) AI4,RAYY(I),RAYX(I),INT(RAYW(I))
   END DO
   CALL CLOSE_FILE(16,1)
...
!
   IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1.OR.SST.EQ.1) THEN
      WRITE(OUTLYNE,*)'"INCR" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
!
   IF(WQ.NE.'A'.AND.WQ.NE.'B'.AND.WQ.NE.'C'.AND.WQ.NE.'D'.AND.WQ.NE.'E'.AND.WQ.NE.'F'.AND.WQ.NE.'G'.AND.WQ.NE.'H'.AND.WQ.NE.' '.AND.WQ.NE.'ACC'.AND.WQ.NE.'X'.AND.WQ.NE.'Y'.AND.WQ.NE.'Z'.AND.WQ.NE.'IX'.AND.WQ.NE.'IY'.AND.WQ.NE.'IZ'.AND.WQ.NE.'IT'.AND.WQ.NE.'I'.AND.WQ.NE.'ITEST'.AND.WQ.NE.'JTEST'.AND.WQ.NE.'K'.AND.WQ.NE.'L'.AND.WQ.NE.'M'.AND.WQ.NE.'N'.AND.WQ.NE.'KTEST'.AND.WQ.NE.'LTEST'.AND.WQ.NE.'MTEST'.AND.WQ.NE.'NTEST'.AND.WQ.NE.'J') THEN
...
!       DEVICE.
   IF(WQ.NE.'ED'.AND.WQ.NE.'CR'.AND.WQ.NE.'TP'.AND.SQ.NE.0.AND.WQ.NE.'FILE    ') THEN
      OUTLYNE='INVALID INPUT DEVICE NAME'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(WQ.EQ.'FILE    ')LASTFIL=OFILN
   IF(WQ.EQ.'FILE    ')OFILN=WS(1:80)
         ...
         DO I=1,99999
            ...
            IF(INPUT(1:3).NE.'OUT') THEN
               ...
               IF(INPUT(1:8).EQ.'FLDSRAYS') THEN
                  ...
                  DO J=1,200
                     READ(IN,*) AI,FIELDY(I),FIELDX(I),FIELDZ(I),N3
                     FIELDW(I)=DBLE(N3)
...
!
   IF(WQ.EQ.'FILE') THEN
      ...
      CALL CLOSE_FILE(97,1)
      IF(APPEND) OPEN(UNIT=97,ACCESS='APPEND',BLANK='NULL',FORM='FORMATTED',FILE=trim(basePath)//OFILN ,STATUS='UNKNOWN')
      IF(.NOT.APPEND) OPEN(UNIT=97,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE=trim(basePath)//OFILN ,STATUS='UNKNOWN')
      IF(F4.EQ.1) THEN
!     IN A MACRO, READ THE DATA
...
!
   IF(WQ.EQ.'ED') THEN
      ...
      IF(.NOT.OPEN9) THEN
         IF(APPEND) OPEN(UNIT=9,ACCESS='APPEND',BLANK='NULL',FORM='FORMATTED',FILE='EDITTEXT.DAT',STATUS='UNKNOWN')
         IF(.NOT.APPEND) OPEN(UNIT=9,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='EDITTEXT.DAT',STATUS='UNKNOWN')
      ELSE
      END IF
      IF(F4.EQ.1) THEN
!     IN A MACRO, READ THE DATA
...
!
   IF(STI.EQ.1) THEN
      OUTLYNE='"STOAX" PLACES THE LAST PROMPT READ STRING INTO'
      CALL SHOWIT(1)
      OUTLYNE='THE ALPHANUMERIC STORAGE REGISTER DESIGNATED BY'
      CALL SHOWIT(1)
      OUTLYNE='NUMERIC WORD #1. VALID VALUES ARE 1 TO 400'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      OUTLYNE='"STOAX" ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S1.EQ.0) THEN
      OUTLYNE='"STOAX" REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(W1.LT.1.0D0.OR.W1.GT.400.0D0) THEN
      OUTLYNE='"STOAX" REQUIRES NUMERIC WORD #1 INPUT TO BE IN THE RANGE'
      CALL SHOWIT(1)
      OUTLYNE='"1 TO 400'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
...
!                   END IF
   DO I=1,80
      J=ICHAR(ASTRING(I:I))
      IF(J.GE.97.AND.J.LE.122)ASTRING(I:I)=CHAR(J-32)
   END DO
...
!
   REAL*8 DATA1,DATA2,TABLE(1:1001,1:3),VALUE,SLOPE
!
...
!       IS IT AN EXTRAPOLATION AT HIGH END?
   IF(W1.GT.TABLE(POINTS,2)) THEN
      SLOPE=(TABLE(POINTS,3)-TABLE((POINTS-1),3))/(TABLE(POINTS,2)-TABLE((POINTS-1),2))
...
!       INTERPOLATE IN THIS SPACE
         SLOPE=(TABLE((I+1),3)-TABLE(I,3))/(TABLE((I+1),2)-TABLE(I,2))
...
!
   REAL*8 X1,X2,X3,Y1,Y2,Y3,X4,Y4,VALUE,L1,L2,L3,L4,XMIN,XMAX
!
!
!       DATA INPUT
!
   IF(WC.EQ.'X1Y1='.OR.WC.EQ.'X2Y2='.OR.WC.EQ.'X3Y3='.OR.WC.EQ.'X4Y4=') THEN
!
!       CHECK FOR WRONG INPUT
      IF(SQ.EQ.1.OR.SST.EQ.1.OR.DF3.EQ.0.OR.DF4.EQ.4.OR.DF5.EQ.0) THEN
      ...
      IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
         WRITE(OUTLYNE,*)WC(1:5),' REQUIRES EXPLICIT NUMERIC WORDS 1 AND 2'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       INTERPOLATION
      IF(SST.EQ.1.OR.DF2.EQ.2.OR.DF3.EQ.0.OR.DF4.EQ.0 .OR.DF5.EQ.0) THEN
         WRITE(OUTLYNE,*)WC(1:6),' TAKES NO ALPHANUMERIC STRING AND ONLY USES'
         CALL SHOWIT(1)
      ...
      IF(DF1.EQ.1) THEN
         WRITE(OUTLYNE,*)WC(1:6),' REQUIRES EXPLICIT NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       (LAG), (PAR) OR (CUBIC)
      IF(WQ.NE.'LIN'.AND.WQ.NE.'LAG'.AND.WQ.NE.'PAR'.AND.WQ.NE.'CUBIC') THEN
         WRITE(OUTLYNE,*)'INVALID QUALIFIER USED WITH THE "INTERP" COMMAND'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       SLOPE INDETERMINATE, NO INTERP POSSIBLE
            WRITE(OUTLYNE,*)'SLOPE INDETERMINATE, NO LINEAR INTERPOLATION POSSISBLE'
            CALL SHOWIT(1)
         ...
         CALL SHOWIT(0)
         IF(FLAG3.EQ.1) THEN
            WRITE(OUTLYNE,*)'NOTICE: 3 DATA PAIRS WERE ENTERED PRIOR TO INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'ONLY DATA PAIRS 1 AND 2 ARE USED FOR LINEAR INTERPOLATION'
            CALL SHOWIT(1)
            CALL MACFAL
            RETURN
         END IF
!       EXTRAPOLATION WARNING MESSAGE?
         IF(X1.LT.X2.AND.W1.LT.X1.OR.X1.LT.X2.AND.W1.GT.X2 .OR.X2.LT.X1.AND.W1.LT.X2.OR.X2.LT.X1.AND.W1.GT.X1)THEN
!       EXTRAPOLATION
            WRITE(OUTLYNE,*)'WARNING: CURRENT VALUE IS EXTRPOLATED, NOT INTERPOLATED'
            CALL SHOWIT(1)
         END IF
         GO TO 100
      END IF
...
!       ENOUGH POINTS?
         IF(FLAG1.EQ.0) THEN
            WRITE(OUTLYNE,*) 'NO INPUT DATA EXISTS FOR INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'INPUT ADDITIONAL DATA WITH "X1Y1=", "X2Y2=" AND "X3Y3="'
            CALL SHOWIT(1)
         ...
         IF(FLAG1.EQ.1.AND.FLAG2.EQ.0) THEN
            WRITE(OUTLYNE,*)'ONLY 1 DATA PAIR EXISTS FOR INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'INPUT ADDITIONAL DATA WITH "X2Y2=" AND "X3Y3="'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'BEFORE USING "INTERP LAG" OR "INTERP PAR"'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       INTERPOLATION
!       CALCULATE THE LINE SLOPE
         IF(DABS(X2-X1).LE.1.0D-35.OR.DABS(X3-X2).LE.1.0D-35.OR.DABS(X1-X3).LE.1.0D-35) THEN
!       SLOPE INDETERMINATE, NO INTERP POSSIBLE
            WRITE(OUTLYNE,*)'SLOPE INDETERMINATE, NO LAGRANGIAN INTERPOLATION POSSISBLE'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'ENTER NEW DATA TO CONTINUE INTERPOLATING'
            CALL SHOWIT(1)
            CALL MACFAL
         ...
         CALL SHOWIT(0)
         IF(FLAG4.EQ.1) THEN
            WRITE(OUTLYNE,*)'NOTICE: 4 DATA PAIRS WERE ENTERED PRIOR TO INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'ONLY DATA PAIRS 1, 2 AND 3 ARE USED FOR'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'PARABOLIC OR LAGRANGIAN INTERPOLATION'
            CALL SHOWIT(1)
            CALL MACFAL
            RETURN
         END IF
...
!       ENOUGH POINTS?
         IF(FLAG1.EQ.0) THEN
            WRITE(OUTLYNE,*) 'NO INPUT DATA EXISTS FOR INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'INPUT ADDITIONAL DATA WITH "X1Y1=", "X2Y2="'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'"X3Y3=" AND "X4Y4="'
            CALL SHOWIT(1)
         ...
         IF(FLAG1.EQ.1.AND.FLAG2.EQ.0) THEN
            WRITE(OUTLYNE,*) 'ONLY 1 DATA PAIR EXISTS FOR INTERPOLATION'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'INPUT ADDITIONAL DATA WITH "X2Y2=", "X3Y3=" AND "X4Y4="'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*) 'BEFORE USING "INTERP CUBIC"'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
         IF(FLAG1.EQ.1.AND.FLAG2.EQ.1.AND.FLAG3.EQ.0) THEN
            ...
            RETURN
         END IF
         IF(FLAG1.EQ.1.AND.FLAG2.EQ.1.AND.FLAG3.EQ.1.AND.FLAG4.EQ.0) THEN
...
!       INTERPOLATION
!       CALCULATE THE LINE SLOPE
         IF(DABS(X2-X1).LE.1.0D-35.OR.DABS(X3-X2).LE.1.0D-35.OR.DABS(X1-X3).LE.1.0D-35.OR.DABS(X1-X4).LE.1.0D-35.OR.DABS(X2-X4).LE.1.0D-35.OR.DABS(X3-X4).LE.1.0D-35) THEN
!       SLOPE INDETERMINATE, NO INTERP POSSIBLE
            WRITE(OUTLYNE,*)'SLOPE INDETERMINATE, NO CUBIC INTERPOLATION POSSISBLE'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'ENTER NEW DATA TO CONTINUE INTERPOLATING'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       FROM THE CMD LEVEL.
!
   CHARACTER STWORD*80,QWORD*8,CWORD*8 ,QBLANK*8,ANUM*80,AN1*23,AN2*23,AN3*23,AN4*23,AN5*23,BL80*80
!
   INTEGER FLAG1,FLAG2,FLAG3,FLAG4,FLAG5,QFLAG,CFLAG,STFLAG,ACNT
!
   REAL*8 N1WORD,N2WORD,N3WORD,N4WORD,N5WORD
!
!
   QBLANK='        '
   BL80=AA//AA//AA//AA
...
!
      ...
      END IF
      IF(SST.NE.1) THEN
         WRITE(OUTLYNE,*)'"STWORD" REQUIRES EXPLICIT STRING INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      ...
      END IF
      IF(SQ.NE.1) THEN
         WRITE(OUTLYNE,*)'"CWORD" REQUIRES EXPLICIT QUALIFIER INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      ...
      END IF
      IF(SQ.NE.1) THEN
         WRITE(OUTLYNE,*)'"QWORD" REQUIRES EXPLICIT QUALIFIER INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
!       N1WORD TO N5WORD
   IF(WC.EQ.'N1WORD'.OR.WC.EQ.'N2WORD'.OR.WC.EQ.'N3WORD'.OR.WC.EQ.'N4WORD'.OR.WC.EQ.'N5WORD') THEN
!
!       CHECK FOR WRONG INPUT AND SET FLAG
!
      IF(SST.EQ.1.OR.SQ.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"N1WORD" TO "N5WORD" ONLY TAKE NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(S1.NE.1) THEN
         WRITE(OUTLYNE,*)'"N1WORD" TO "N5WORD" REQUIRES EXPLICIT NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       PRIORITY.
      IF(FLAG1.EQ.1.OR.FLAG2.EQ.1.OR.FLAG3.EQ.1.OR.FLAG4.EQ.1 .OR.FLAG5.EQ.1) THEN
         IF(STFLAG.EQ.1) THEN
!       BLANK THE STRING AND DO MESSAGE
            STFLAG=0
            STWORD=AA//AA//AA//AA
            WRITE(OUTLYNE,*)'BOTH NUMERIC AND ALPANUMERIC STRING DATA WAS FOUND'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
!       CONVERT TO ALPHA FROM REAL*8
!
         CALL NTA(N1WORD,N2WORD,N3WORD,N4WORD,N5WORD,AN1,AN2,AN3,AN4,AN5)
!
         IF(FLAG1.EQ.1) THEN
            ANUM=','//AN1
            ACNT=24
...
!       CONVERSIONS.
!
   REAL*8 R,THETA,R2,PHI,X,Y,Z,FAC,H,M,S,BUFFER,COSARG
!
!
   IF(SST.EQ.1.OR.SQ.EQ.1.OR.SN.EQ.1) THEN
!
...
SUBROUTINE GCONVERT
   ...
   IF(STI.EQ.1) THEN
      WRITE(OUTLYNE,*)'"GCONVERT" CONVERTS THE CURRENT LENS INTO A "GLOBAL" LENS WITH'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RESPECT TO THE SURFACE SPECIFIED IN NUMERIC WORD #1'
      CALL SHOWIT(1)
      RETURN
   END IF
   IF(SQ.EQ.1.OR.SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'"GCONVERT TAKES NO QUALIFIER, STRING OR'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NUMERIC WORD #2, #3, #4 OR #5 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*) 'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(S1.EQ.0) THEN
      WRITE(OUTLYNE,*)'"GCONVERT REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*) 'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(W1.LE.1) THEN
      WRITE(OUTLYNE,*)'"NUMERIC WORD #1 NUST BE 2 OR GREATER'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*) 'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     COSINE OF BETA IS NOT ZERO
         ...
         IF((D21/COSB).NE.0.0D0.AND.(D11/COSB).NE.0.0D0)GAMMA=DATAN2((-D21/COSB),(D11/COSB))
      END IF
...
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT CHARACTER REPRESENTATION OF
!       A D23.15 NUMERIC WORD TO AN REAL*8 VALUE
!
   REAL*8 VAL
!
...
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT CHARACTER REPRESENTATION OF
!       A D23.15 NUMERIC WORD TO A REAL*8
!       VALUE. CHARACTER VALUE IS 23 SPACES LONG
!
   REAL*8 VL
!
...
!
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT REAL*8 TO G13.6
!
   REAL*8 VAL
!
...
!
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT REAL*8 TO G23.15
!
   REAL VAL
!
   CHARACTER B*140
...
!
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT 10 DIGIT REAL*8 NUMERIC WORDS
...
!       A REAL*8 NUMERIC WORD
!       INTO A REAL*8 NUMERIC WORD
!
   REAL*8 N1
!
...
!
!       THIS SUBROUTINE USES WRITE AND READS TO AN INTERNAL
!       FILE TO CONVERT A REAL*8 NUMERIC WORD
...
!
   ...
   IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1.OR.SQ.EQ.1) THEN
      WRITE(OUTLYNE,*)'"SAVEREG" ONLY TAKES NUMERIC WORDS #1 AND #2'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
      WRITE(OUTLYNE,*)'"SAVEREG" REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   END IF
   IF(INT(W1).LT.1) THEN
      WRITE(OUTLYNE,*)'NUMERIC WORD #1 MAY NOT BE LESS THAN 1'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(INT(W2).GT.MAXREG) THEN
      WRITE(OUTLYNE,*)'NUMERIC WORD #2 MAY NOT BE GREATER THAN ',MAXREG
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(INT(W1).GT.INT(W2)) THEN
      WRITE(OUTLYNE,*)'NUMERIC WORD #1 MAY NOT BE LARGER THAN NUMERIC WORD #2'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'GENERAL PURPOSE REGISTER COMMANDS ONLY TAKE QUALIFIER AND'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'NUMERIC WORDS #1 AND (OPTIONALLY) #2 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.NE.'CLGREG'.AND.WC.NE.'CLSTREG') THEN
      IF(DF1.EQ.1) THEN
         WRITE(OUTLYNE,*)'GENERAL PURPOSE REGISTER COMMANDS REQUIRE EXPLICIT'
         CALL SHOWIT(1)
...
!
      IF(GPREG(151).LE.1.0) THEN
         WRITE(OUTLYNE,*)'STATISTICAL CALCULATIONS REQUIRE MORE THAN ONE ENTRY'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'IN THE STATISTICAL DATA BASE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)INT(GPREG(151)),' ENTRIES EXISTS IN CURRENT DATA BASE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
