...
!
   ...
   IMPLICIT NONE
!
!       THIS IS SUBROUTINE NSSVERT.INC. THIS SUBROUTINE IMPLEMENTS
!       CREATION OF GLOBAL NSSVERTEX DATA FOR THE NSS DATABASE
!
   INTEGER J,I
!
   real(real64) A,AN,A11,A12,A13,A21,A22,A23,A31,A32,A33 &
   &,B11,B12,B13,B21,B22,B23,B31,B32,B33 &
   &,C11,C12,C13,C21,C22,C23,C31,C32,C33 &
   &,LX,LY,LZ,MX,MY,MZ,NX,NY,NZ,X,Y,Z,TX,TY,TZ &
   &,TLX,TLY,TLZ,TMX,TMY,TMZ,TNX,TNY,TNZ &
   &,TTLX,TTLY,TTLZ,TTMX,TTMY,TTMZ,TTNX,TTNY,TTNZ &
   &,AEEA,BEEB,CEEC,XEEX,YEEY,ZEEZ,JLX1,JMX1,JNX1,JLY1,JMY1,JNY1 &
   &,JLZ1,JMZ1,JNZ1 &
...
!       C33
   ...
   DO J=1,MAXS
      ...
      NSSVERTEX(2,J)=0.0D0
      NSSVERTEX(3,J)=0.0D0
!       THE DIRECTION COSINE OF THE LOCAL X-AXIS IS:
      TX=0.0D0
      TY=0.0D0
      TZ=0.0D0
      LX=1.0D0
      MX=0.0D0
      NX=0.0D0
!       THE DIRECTION COSINE OF THE LOCAL Y-AXIS IS:
      LY=0.0D0
      MY=1.0D0
      NY=0.0D0
!       THE DIRECTION COSINE OF THE LOCAL Z-AXIS IS:
      LZ=0.0D0
      MZ=0.0D0
      NZ=1.0D0
!
...
!
!       IS TRANSFORMATION REFERENCED TO ANOTHER SURFACE
!       INSTEAD OF TO THE GLOBAL ORIGIN
         NSSNEST(0:200)=-1
         INEST=0
         ...
         END IF
         IF(NSSALENS(37,J).NE.-1.0D0) THEN
!       NOW PERFORM THE REFERENCED
!       WE HAVE A VALID SURFACE TO TRANSFORM
            NSSVERTEX(0,J)=1.0D0
            XEEX=NSSALENS(34,INT(NSSALENS(37,J)))
            YEEY=NSSALENS(35,INT(NSSALENS(37,J)))
            ZEEZ=NSSALENS(36,INT(NSSALENS(37,J)))
!
! NEG SIGNS ARE USED BECAUSE THE TRANSFORMS WERE ORIGINALLY DESIGNED
!       FOR CODE-V ANGLE SIGN CONVENTIONS FOR ALPHA AND BETA WHICH ARE
!       LEFT HAND AND NOT RIGHT HAND POSITIVE
...
!     SURFACE I. TRANSFORM THEM TO THE GLOBAL COORDINATE SYSTEM
         ...
         END IF
!       NOW PERFORM THE TRANSFORMATION ATTACHED TO THIS SURFACE
!       WE HAVE A VALID SURFACE TO TRANSFORM
         NSSVERTEX(0,J)=1.0D0
         XEEX=NSSALENS(34,J)
         YEEY=NSSALENS(35,J)
...
!       THE DIRECTION COSINE OF THE LOCAL Z-AXIS IS:
         ...
         NSSVERTEX(12,J)=TNZ
      ELSE
!       NOT A USED SURFACE NUMBER IN THIS OBJECT
      END IF
   END DO
!
   DO J=1,MAXS
...
!
   ...
   &,B11,B12,B13,B21,B22,B23,B31,B32,B33 &
   &,C11,C12,C13,C21,C22,C23,C31,C32,C33 &
   &,LX,LY,LZ,MX,MY,MZ,NX,NY,NZ,TX,TY,TZ &
   &,TLX,TLY,TLZ,TMX,TMY,TMZ,TNX,TNY,TNZ &
   &,TTLX,TTLY,TTLZ,TTMX,TTMY,TTMZ,TTNX,TTNY,TTNZ &
   &,AEEA,BEEB,CEEC,XEEX,YEEY,ZEEZ,JLX1,JMX1,JNX1,JLY1,JMY1,JNY1 &
   &,JLZ1,JMZ1,JNZ1 &
...
SUBROUTINE NSS_TRANSF(J)
   USE NSSMOD
!
!       THIS ROUTINE CONVERTS GLOBAL RAY DATA INTO THE COORDINATE SYSTEM OF
!       SURFACE J
!
!       THIS OPERATES ON RAY DATA BEFORE A SURFACE INTERACTION
!
!
   use DATMAI
   use iso_fortran_env, only: real64
   IMPLICIT NONE
!
   real(real64) JK_AA,A11,A12,A13,A21,A22,A23,A31,A32,A33 &
   &,B11,B12,B13,B21,B22,B23,B31,B32,B33,TDECX,TDECY,TDECZ &
   &,C11,C12,C13,C21,C22,C23,C31,C32,C33,&
   &A,B,C,DX,DY,DZ,AN,X1,Y1,Z1,L1,M1,N1
   real(real64) X,Y,Z,L,M,N,XL,XM,XN,YL,YM,YN
   real(real64) XP,YP,ZP,LP,MP,NP,XLP,XMP,XNP,YLP,YMP,YNP
!
   INTEGER J,I
!
!
...
!       SURFACE J
!
!       THIS OPERATES ON RAY DATA AFTER A SURFACE INTERACTION
!
!
   use DATMAI
   use iso_fortran_env, only: real64
...
SUBROUTINE NSS_TRANSF_BACK(J)
   USE NSSMOD
!
!       THIS ROUTINE CONVERTS LOCAL RAY DATA AT OBJECT I AND SURFACE J INTO
!       GLOBAL COORDINATE RAY DATA.
!
!       THIS OPERATES ON RAY DATA BEFORE A SURFACE INTERACTION
!
!
...
&(GX0,GY0,GZ0,GL0,GM0,GN0,GX,GY,GZ,GL,GM,GN,J)
   USE NSSMOD
!
   use DATMAI
   use iso_fortran_env, only: real64
...
!
   ...
   &A,B,C,DX,DY,DZ,AN,X1,Y1,Z1,L1,M1,N1
   real(real64) X,Y,Z,L,M,N,XL,XM,XN,YL,YM,YN
   real(real64) GX0,GY0,GZ0,GL0,GM0,GN0,GX,GY,GZ,GL,GM,GN
   real(real64) XP,YP,ZP,LP,MP,NP,XLP,XMP,XNP,YLP,YMP,YNP
!
   INTEGER J,I
!
...
!
   ...
   IMPLICIT NONE
!
!       THIS ROUTINE DOES THE NSSORINT COMMAND
!
   INTEGER I
!
   real(real64) LVAL,MVAL,NVAL,MAG,SINA,COSA,SINF,COSF
   real(real64) V1,V2,XVAL,YVAL,ZVAL
!
!
!
...
!
!       THIS ROUTINE CONVERTS GLOBAL RAY DATA INTO THE COORDINATE SYSTEM OF
!       SURFACE J FOR THE SURFACE SEARCH
!
!       THIS OPERATES ON RAY DATA BEFORE A SURFACE INTERACTION
!
!
...
SUBROUTINE OXOYRXRY_LOCAL_TO_GLOBAL(OX,OY,RX,RY,GOX,GOY,GRX,GRY)
   USE NSSMOD
!
!       THIS ROUTINE CONVERTS LOCAL STARTING OBJECT AND REFERENCE X AND Y COORDINATES
!       INTO GLOBAL DATA
!
!       THIS OPERATES ON RAY DATA BEFORE A SURFACE INTERACTION
!
...
!
   ...
   &,B11,B12,B13,B21,B22,B23,B31,B32,B33 &
   &,C11,C12,C13,C21,C22,C23,C31,C32,C33,&
   &A,B,AN,X1,Y1,L1,M1,OX,OY,RX,RY,GOX,GOY,GRX,GRY
   real(real64) X,Y,L,M
   real(real64) XP,YP,LP,MP
!
   INTEGER J,I
!
!
