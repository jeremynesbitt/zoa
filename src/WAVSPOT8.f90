!       EIGHTH FILE OF CAPFN/SPOT ROUTINES
...
SUBROUTINE MAPFIELDOPD
   ...
   IF(WC.NE.'STRLMAP') THEN
      IF(SQ.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &WC,' TAKES NO QUALIFIER, STRING OR NUMERIC WORD #5 INPUT'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   ELSE
      IF(SQ.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &WC,' TAKES NO QUALIFIER, STRING OR NUMERIC WORD #4 OR #5 INPUT'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
   ...
   IF(WC.NE.'STRLMAP') THEN
      ...
      IF(W1.LT.1.0D0.OR.W1.GT.10.0D0) THEN
         WRITE(OUTLYNE,*)&
         &'WAVELENGTH NUMBER MUST BE IN THE RANGE 1 TO 10'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      ...
      IF(W2.LT.16.0D0) THEN
         WRITE(OUTLYNE,*)&
         &'NRD MUST BE 16 OR GREATER'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
      IF(W2.LT.16.0D0.OR.W2.GT.512.0D0.OR.&
      &(W2-DBLE(INT(W2))).NE.0.0D0.OR.W2.GT.512.0D0.OR.&
      ...
      IF(W3.LT.3.0D0) THEN
         WRITE(OUTLYNE,*)&
         &'NUMBER OF FIELD POINT IN ONE DIMENSION MUST BE 3 OR GREATER'
         CALL SHOWIT(1)
         CALL MACFAL
         RETURN
      END IF
...
200 FORMAT(A10)
   IF(K.EQ.1) THEN
      ...
      &'PEAK TO VALLEY OPD MAP AT WAVELENGTH = '&
   ...
   IF(K.EQ.3) THEN
      CALL MY_JUSTSTRING(100,250,&
      &'PSF BASED STREHL RATIO AT WAVELENGTH = '&
      &//TRIM(WAVVAL)//' MICRON(S)',0,1,3,3)
      CALL MY_JUSTSTRING(6550,5400,&
      &'STREHL RATIO CONTOUR LEVELS'&
      &,0,1,3,3)
   END IF
   IU=1
