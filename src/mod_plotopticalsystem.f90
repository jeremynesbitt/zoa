...
type, extends (zoatab) ::  lens_draw_settings
...
subroutine callback_lens_draw_settings (widget, gdata ) bind(c)
  ...
  subroutine lens_draw_new_2(self, parent_window)
    use zoa_tab
    !use ROUTEMOD
    ...
    !lenstab = lens_draw_settings()

    call self%initialize(parent_window, "Lens Draw", ID_NEWPLOT_LENSDRAW)
