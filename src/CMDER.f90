...
SUBROUTINE CMDER
   ...
   use glass_manager
   use kdp_interfaces , only: EDITOR, POWSYM, FIR,SYSCONFIGUI, SPR, plot_seidel, MACROUI, PLT3DTST,PLTIMTST, PTSTUFF, Sandbox
...
!
   CHARACTER DDATE*10,TTIME*8,FILNM*10,NM*8,TTTIM*8,DDDAT*10 ,AI*3,OWC*8,OWQ*8,HNAM*8,AI4*4
!
...
!
   COMMON/MANH/HNAM
!
   LOGICAL ERRFOB,FIELDER,RAYER,IS ,LBT,EXIS51,OPEN51,LERROR,ERRORPSF,OLDLDIF2,OLDLDIF ,REFERR,PDOK
!
   LOGICAL MULTIOTF
...
!
   INTEGER II,I,ITNUM,ITCT,OLDSTOP,OLDSTOP2,KKKEY,CCODE,ERROR ,OREFLOC,IIG,OCOLDEF,OCOLBAC,IU,IPASS2,CACOCHVIE ,MMM,N,ALLOERR
...
!
      IF(F17.EQ.0.AND.WC.EQ.'ITF'.OR.F17.EQ.0.AND.WC.EQ.'ILF'.OR.F17.EQ.0.AND.WC.EQ.'PROCEED'.OR.F17.EQ.0.AND.WC.EQ.'IPF') THEN
!       ILF
         IF(WC.EQ.'ILF') THEN
...
!       IPF
         ...
         END IF
      END IF
      IF(F17.EQ.1.AND.WC.EQ.'ITF'.OR.F17.EQ.1.AND.WC.EQ.'ILF'.OR.F17.EQ.1.AND.WC.EQ.'PROCEED'.OR.F17.EQ.1.AND.WC.EQ.'IPF') THEN
!       WE ARE AT SPECT
         OUTLYNE='"ILF", "ITF" AND "IPF" NOT VALID AT "SPECT" LEVEL'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
...
!       PART DRAWINGS
      ...
      IF(WC.EQ.'FIR') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'SPSRF'.AND.WQ.NE.'ON'.AND.WC.EQ.'SPSRF'.AND.WQ.NE.'OFF')THEN
         IF(RAYCLEAR) THEN
            RAYEXT=.FALSE.
            POLEXT=.FALSE.
            REFEXT=.FALSE.
...
!       CALL AUTOFF
         ...
         RETURN
      END IF
      IF(WC.EQ.'SPSRF'.AND.WQ.EQ.'ON'.OR.WC.EQ.'SPSRF'.AND.WQ.EQ.'OFF')THEN
         IF(RAYCLEAR) THEN
            RAYEXT=.FALSE.
            POLEXT=.FALSE.
            REFEXT=.FALSE.
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'SPSRF'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'SP'.OR.WC.EQ.'U'.AND.WQ.EQ.'SPSRF'.OR.WC.EQ.'U'.AND.WQ.EQ.'SP') THEN
         IF(RAYCLEAR) THEN
            RAYEXT=.FALSE.
            POLEXT=.FALSE.
            REFEXT=.FALSE.
      ...
      IF(WC.EQ.'SPECT')THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'CONFIGS'.OR.WQ.EQ.'UPDATE'.AND.WQ.EQ.'CONFIG'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'CF'.OR.WC.EQ.'U'.AND.WQ.EQ.'CONFIGS'.OR.WC.EQ.'U'.AND.WQ.EQ.'CONFIG'.OR.WC.EQ.'U'.AND.WQ.EQ.'CF') THEN
...
!       CALL TO PSTAT( PLOT LIBRARY STATUS)
      IF(WC.EQ.'PSTAT')  THEN
         IF(SN.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
            OUTLYNE='"PSTAT" TAKES NO EXPLICIT INPUT'
            CALL SHOWIT(1)
            OUTLYNE='RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
      ...
      IF(WC.EQ.'GLASSP') THEN
         IF(WQ.EQ.'GLAK'.AND.SST.EQ.0.OR.WQ.EQ.'GLCAT'.AND.SST.EQ.0) THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'DEG'.OR.WC.EQ.'RAD'.OR.WC.EQ.'TANGENT'.OR.WC.EQ.'ANGMODE') THEN
         CALL DEGRAD
         RETURN
      END IF
      IF(WC.EQ.'ASPH') THEN
...
!       THREEDEESET (PLOTCAD2) removed
      IF(WC.EQ.'X1Y1='.OR.WC.EQ.'X2Y2='.OR.WC.EQ.'X3Y3='.OR.WC.EQ.'X4Y4='.OR.WC.EQ.'INTERP') THEN
         CALL INTRPP
         RETURN
      END IF
      IF(WC.EQ.'TABLE'.OR.WC.EQ.'COLHD'.OR.WC.EQ.'COLHD2'.OR.WC.EQ.'ROWHD'.OR.WC.EQ.'ROWHD2') THEN
         CALL TABLE
         RETURN
      END IF
      IF(WC.EQ.'NEWCMD'.OR.WC.EQ.'STWORD'.OR.WC.EQ.'CWORD'.OR.WC.EQ.'QWORD'.OR.WC.EQ.'N1WORD'.OR.WC.EQ.'N2WORD'.OR.WC.EQ.'N3WORD'.OR.WC.EQ.'N4WORD'.OR.WC .EQ.'N5WORD') THEN
         CALL CWRITE
         RETURN
      END IF
      IF(WC.EQ.'ATAN2') THEN
      ...
      IF(WC.EQ.'LENS') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'LENS'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'L'.OR.WC.EQ.'U'.AND.WQ.EQ.'LENS'.OR.WC.EQ.'U'.AND.WQ.EQ.'L') THEN
         CALL ULENNS
         RETURN
      END IF
! WINTER
...
!
      ...
      IF(WC.EQ.'ENT'.OR.WC.EQ.'ENTI'.OR.WC.EQ.'ENTC'.OR.WC.EQ.'PULL'.OR.WC.EQ.'IPULL'.OR.WC.EQ.'CPULL'.OR.WC.EQ.'RUP'.OR.WC.EQ.'IRUP'.OR.WC .EQ.'CRUP'.OR.WC.EQ.'RDN'.OR.WC.EQ.'IRDN'.OR.WC.EQ.'CRDN'.OR.WC.EQ.'LASTX'.OR.WC.EQ.'X-Y'.OR.WC.EQ.'LASTIX'.OR.WC.EQ.'IX-IY'.OR.WC.EQ.'RE-IM'.OR.WC.EQ.'CLX'.OR.WC.EQ.'CLIX'.OR.WC.EQ.'CLSTK'.OR.WC.EQ.'CLSTKI'.OR.WC.EQ.'CLSTKC'.OR.WC.EQ.'+'.OR.WC.EQ.'-'.OR.WC.EQ.'*'.OR.WC.EQ.'/'.OR.WC.EQ.'C+'.OR.WC.EQ.'C-'.OR.WC.EQ.'C*'.OR.WC.EQ.'C/'.OR.WC.EQ.'I+'.OR.WC.EQ.'I-'.OR.WC.EQ.'I*'.OR.WC.EQ.'I/'.OR.WC.EQ.'Y**X'.OR.WC.EQ.'CY**CX'.OR.WC.EQ.'IY**IX') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'PRSTK'.OR.WC.EQ.'PRSTKC'.OR.WC.EQ.'PRSTKI'.OR.WC.EQ.'PRLSTX'.OR.WC.EQ.'PRLSTIX') THEN
         CALL STACK
         RETURN
      END IF
...
!     IN 3/93
      ...
      IF(WC.EQ.'MOD') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'INTGR'.OR.WC.EQ.'FRAC'.OR.WC.EQ.'FACT'.OR.WC.EQ.'CHS'.OR.WC.EQ.'RTD'.OR.WC.EQ.'DTR'.OR.WC.EQ.'ASIN'.OR.WC.EQ.'ACOS'.OR.WC.EQ.'PLUS'.OR.WC.EQ.'MINUS'.OR.WC.EQ.'DIV'.OR.WC.EQ.'MPY'.OR.WC.EQ.'MOVE'.OR.WC.EQ.'ATAN'.OR.WC.EQ.'PI'.OR.WC.EQ.'RAND'.OR.WC.EQ.'SIN'.OR.WC.EQ.'COS'.OR.WC.EQ.'TAN'.OR.WC.EQ.'TANH'.OR.WC.EQ.'SINH'.OR.WC.EQ.'COSH'.OR.WC.EQ.'SQRT'.OR.WC.EQ.'ABS'.OR.WC.EQ.'EXP'.OR.WC.EQ.'RECIP'.OR.WC.EQ.'POW'.OR.WC.EQ.'LOG10'.OR.WC.EQ.'LN'.OR.WC.EQ.'STORE'.OR.WC.EQ.'SGN'.OR.WC.EQ.'CLREG'.OR.WC.EQ.'PRIREG'.OR.WC.EQ.'WRITE'.OR.WC.EQ.'MAXVAL'.OR.WC.EQ.'MINVAL')THEN
         CALL RGMATH
         RETURN
      END IF
      IF(WC.EQ.'STOREMIN'.OR.WC.EQ.'STOREMAX'.OR.WC.EQ.'RESETMIN'.OR.WC.EQ.'RESETMAX') THEN
         CALL MINMAXREG
         RETURN
      END IF
...
!     IF VIE, MSG OFF
         ...
         IF(EXIS51) THEN
            ...
            IF(.NOT.OPEN51) THEN
!     OPEN FILE
               OPEN(UNIT=51,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='PSF.DAT',STATUS='UNKNOWN')
            END IF
         ...
         IF(EXIS51) THEN
            ...
            IF(.NOT.OPEN51) THEN
!     OPEN FILE
               OPEN(UNIT=51,ACCESS='SEQUENTIAL',BLANK='NULL',FORM='FORMATTED',FILE='SPSF.DAT',STATUS='UNKNOWN')
            END IF
            CALL CLOSE_FILE(51,0)
         END IF
         OLDIF=LDIF
...
!     IF VIE, MSG OFF
      ...
      IF(WC.EQ.'TVB')  THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "TVB" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
      ...
      IF(WC.EQ.'COMPS')  THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "COMPS" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
         ...
         RETURN
      END IF
      IF(WC.EQ.'MR'.OR.WC.EQ.'MRA'.OR.WC.EQ.'OP'.OR.WC.EQ.'OPA')  THEN
!       DO A FORCED RETURN TO CFG1
         GRIDSUNLOADED19(0:MAXSUR)=.TRUE.
         GRIDSUNLOADED20(0:MAXSUR)=.TRUE.
         GRIDSUNLOADED22(0:MAXSUR)=.TRUE.
...
!       PLTCAPFN/PLTCAPCO (PLOTCAD4) removed
      IF(WC.EQ.'USERCONT') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'IOBJECT'.OR.WC.EQ.'IOBJECTN'.OR.WC.EQ.'COLOR'.OR.WC.EQ.'OBJVAL'.OR.WC.EQ.'OTOBMP'.OR.WC.EQ.'ITOBMP'.OR.WC.EQ.'IIMAGE'.OR.WC.EQ.'IIMAGEN'.OR.WC.EQ.'IMTRACE1'.OR.WC.EQ.'IMTRACE2'.OR.WC.EQ.'IMTRACE3'.OR.WC.EQ.'BUILDIMG'.OR.WC.EQ.'PLTOBJ'.OR.WC.EQ.'OBJVAL'.OR.WC.EQ.'OFROMBMP'.OR.WC.EQ.'IFROMBMP'.OR.WC.EQ.'PLTIMG'.OR.WC.EQ.'BMPREADR'.OR.WC.EQ.'PSFTOIMG'.OR.WC.EQ.'IIMAGED'.OR.WC.EQ.'IMGSLICE'.OR.WC.EQ.'LMINUSR'.OR.WC.EQ.'IOBJECTD'.OR.WC.EQ.'IMTRACE4'.OR.WC.EQ.'IMTRACE5'.OR.WC.EQ.'INTTOPSF') THEN
! JRN comment out since FULLIMAGING needs WINTERACTER
      ...
      IF(WC.EQ.'VIRTRAY')  THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'XFAN'.OR.WC.EQ.'YFAN'.OR.WC.EQ.'NFAN'.OR.WC.EQ.'PFAN')  THEN
         CACOCH=1
...
!       AND NO INDEPENDENT ERROR MESSAAGES ARE PRINTED
      ...
      IF(WC.EQ.'VERTS') THEN
         ...
         ZM2=GPREG(23)
         ZN2=GPREG(24)
         CALL VERT123(X1,Y1,Z1,XL1,XM1,XN1,YL1,YM1,YN1,ZL1,ZM1,ZN1,X2,Y2,Z2,XL2,XM2,XN2,YL2,YM2,YN2,ZL2,ZM2,ZN2,X3,Y3,Z3,XL3,XM3,XN3,YL3,YM3,YN3,ZL3,ZM3,ZN3,ALPHA,BETA,GAMMA)
         RETURN
      END IF
      ...
      IF(WC.EQ.'TVAR') THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "TVAR" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
      ...
      IF(WC.EQ.'COMPVAR') THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "COMPVAR" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
      ...
      IF(WC.EQ.'FOCRIT') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'VARIABLE'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'VB'.OR.WC.EQ.'U'.AND.WQ.EQ.'VARIABLE'.OR.WC.EQ.'U'.AND.WQ.EQ.'VB') THEN
         CALL DEROFF
         CALL UVRBL1
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'TVAR'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'TVB'.OR.WC.EQ.'U'.AND.WQ.EQ.'TVAR'.OR.WC.EQ.'U'.AND.WQ.EQ.'TVB') THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "UPDATE TVAR" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'COMPVAR'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'CMP'.OR.WC.EQ.'U'.AND.WQ.EQ.'COMPVAR'.OR.WC.EQ.'U'.AND.WQ.EQ.'CMP') THEN
         IF(F12.NE.1) THEN
            OUTLYNE='CONFIGURATION #1 MUST BE THE CURRENT CONFIGURATION BEFORE'
            CALL SHOWIT(1)
            OUTLYNE='THE "UPDATE COMPVAR" COMMAND MAY BE ISSUED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL MACFAL
            RETURN
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'MERIT'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'M'.OR.WC.EQ.'U'.AND.WQ.EQ.'MERIT'.OR.WC.EQ.'U'.AND.WQ.EQ.'M') THEN
         JK_CHMODE=.FALSE.
         CORMOD=1
         CURFIG=1
         CALL DEROFF
         ...
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'TOPER'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'TOP'.OR.WC.EQ.'U'.AND.WQ.EQ.'TOPER'.OR.WC.EQ.'U'.AND.WQ.EQ.'TOP') THEN
         CALL DEROFF
         CALL UTOPER
         RETURN
      END IF
      IF(WC.EQ.'UPDATE'.AND.WQ.EQ.'FOCRIT'.OR.WC.EQ.'UPDATE'.AND.WQ.EQ.'FC'.OR.WC.EQ.'U'.AND.WQ.EQ.'FOCRIT'.OR.WC.EQ.'U'.AND.WQ.EQ.'FC') THEN
         JK_CHMODE=.FALSE.
         CALL DEROFF
         CALL UFOCRIT
         RETURN
...
!        END IF
      IF(WC.EQ.'PLOT'.AND.WQ.NE.'DEV'.AND.F17.EQ.1) THEN
         OUTLYNE='THIS PLOT COMMAND NOT OPERATIONAL AT THE "SPECT" LEVEL'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
      ...
      IF(WC.EQ.'SAG') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'SPD'.OR.WC.EQ.'FAIL'.OR.WC.EQ.'FAILACC'.OR.WC.EQ.'SPDADD'.OR.WC.EQ.'SPDSAVE'.OR.WC.EQ.'SPDSTATS') THEN
         GRASET=.FALSE.
...
!     DONE, MSG BACK ON
         ...
         RETURN
      END IF
      IF(WC.EQ.'RING'.OR.WC.EQ.'RINGS'.OR.WC.EQ.'RANNUM'.OR.WC.EQ.'SPDRESET'.OR.WC.EQ.'APOD'.OR.WC.EQ.'SPOT'.OR.WC.EQ.'RECT') THEN
         CALL SPOTSET
         RETURN
      END IF
      IF(WC.EQ.'OPRING'.OR.WC.EQ.'OPRINGS'.OR.WC.EQ.'OPRANNUM'.OR.WC.EQ.'OPSPDRST'.OR.WC.EQ.'OPSPOT'.OR.WC.EQ.'OPRECT') THEN
         GRSPT=0
         CALL OPSPOTSET
         RETURN
      END IF
      IF(WC.EQ.'OPNRD'.OR.WC.EQ.'TOLNRD'.OR.WC.EQ.'NRD'.OR.WC.EQ.'TGR'.OR.WC.EQ.'PGR'.OR.WC.EQ.'GRI'.OR.WC.EQ.'CAPFNNRD'.OR.WC.EQ.'EXTENT') THEN
         CALL MERIT2
         RETURN
      END IF
      ...
      IF(WC.EQ.'DRED') THEN
         ...
         IF(ERRORPSF) THEN
            OUTLYNE='NO DIFFRACTION PSF EXISTS, NO PSF SPOT FILE WAS CREATED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
      ...
      IF(WC.EQ.'DREDSQ') THEN
         ...
         CALL PSFSPOT(ERRORPSF,MMM)
         IF(ERRORPSF) THEN
            OUTLYNE='NO DIFFRACTION PSF EXISTS, "DREDSQ" CAN NOT PROCEED'
            CALL SHOWIT(1)
            OUTLYNE='NO ACTION TAKEN'
            CALL SHOWIT(1)
            CALL MACFAL
      ...
      IF(WC.EQ.'PSFSPOT') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'REDK'.OR.WC.EQ.'ESEDX'.OR.WC.EQ.'ESEDY'.OR.WC.EQ.'ESED'.OR.WC.EQ.'REDSUM'.OR.WC.EQ.'SRED'.OR.WC.EQ.'SESED'.OR.WC.EQ.'SESEDX'.OR.WC.EQ.'SESEDY') THEN
      ...
      IF(WC.EQ.'ITER'.OR.WC.EQ.'IT') THEN
         ...
         MSG=.FALSE.
         OPTMES=.FALSE.
         IF(SQ.EQ.0.OR.WQ.EQ.'POWL'.OR.WQ.EQ.'P'.OR.WQ.EQ.'FULL'.OR.WQ.EQ.'F'.OR.WQ.EQ.'DIR'.OR.WQ.EQ.'D'.OR.WQ.EQ.'POWELL') THEN
            ITCT=INT(W1)
            IF(ITCT.EQ.0) ITCT=1
            DO ITNUM=1,ITCT
               DEREXT=.FALSE.
...
!     DO MULTIPLE GOTF FOV'S
            IF(CFLDCNT.EQ.0) THEN
               OUTLYNE='MULTIPLE FIELDS OF VIEW ARE NOT DEFINED'
               CALL SHOWIT(1)
               OUTLYNE='NO ACTION TAKEN'
               CALL SHOWIT(1)
               CALL MACFAL
...
!       CAPFIX (PLOTCAD4) removed
      ...
      IF(WC.EQ.'CAPFN') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'CAPFNIN'.OR.WC.EQ.'CAPFNADD'.OR.WC.EQ.'CAPFNCLR') THEN
         CALL OPDIN
         RETURN
      END IF
      ...
      END IF
      IF(WC.EQ.'THERM') THEN
         IF(WQ.NE.'AIR'.AND.WQ.NE.'OXYGEN'.AND.WQ.NE.'NITROGEN'.AND.WQ.NE.'ETHANE'.AND.WQ.NE.'METHANE'.AND.WQ.NE.'HYDROGEN'.AND.WQ.NE.'ARGON'.AND.WQ.NE.'HELIUM'.AND.WQ.NE.'GAS') THEN
            CALL THERM
         ELSE
            CALL THERMGAS
         END IF
      ...
      IF(WC.EQ.'FOOT'.AND.WQ.EQ.'GRID') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'RADUNITS'.OR.WC.EQ.'PLANK'.OR.WC.EQ.'WIEN'.OR.WC.EQ.'STEFBOLT') THEN
         CALL SCHWARTZ
         RETURN
      END IF
      IF(WC.EQ.'K0'.OR.WC.EQ.'CVG') THEN
         ...
         RETURN
      END IF
      IF(gdb%isNameInCatalog(trim(WC)).OR.WC.EQ.'GLCAT'.OR.WC.EQ.'MATL'.OR.WC.EQ.'USER'.OR.WC.EQ.'GLAK'.OR.WC.EQ.'AIR') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'MAKEAUTO'.OR.WC.EQ.'DFTYPE'.OR.WC.EQ.'DFDEL'.OR.WC.EQ.'FP'.OR.WC.EQ.'DFP'.OR.WC.EQ.'MONO'.OR.WC.EQ.'POLY'.OR.WC.EQ.'DFGRID'.OR.WC.EQ.'DFHEX') THEN
         CALL MAKE_DEF_AUTO
         RETURN
      END IF
      IF(WC.EQ.'LENDIR'.OR.WC.EQ.'MACDIR'.OR.WC.EQ.'TRADIR'.OR.WC.EQ.'PLTDIR'.OR.WC.EQ.'CHGMAC') THEN
         CALL CHADIR
         RETURN
      END IF
      IF(WC.EQ.'PFIND') THEN
...
6662  FORMAT('THE CURRENT CATNUM IS: ',A20)
      IF(WC.EQ.'AST'.OR.WC.EQ.'DIST'.OR.WC.EQ.'FLDCV'.OR.WC.EQ.'FISHDIST') THEN
         CALL FIELDABS
         RETURN
      END IF
...
!       CALCULATES YZ AND XZ PLANE RAY BLOCKAGES
      ...
      IF(WC.EQ.'PSFINT'.OR.WC.EQ.'PSFINTS') THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'GAUSS'.OR.WC.EQ.'NOGAUSS'.OR.WC.EQ.'OPWTA'.OR.WC.EQ.'OPWTAIM') THEN
         RETURN
      END IF
...
!       INITIATE SENSITIVITY ANALYSIS
         ...
         RETURN
      END IF
      IF(WC.EQ.'MXT'.OR.WC.EQ.'MNT'.OR.WC.EQ.'MPR'.OR.WC.EQ.'MNR') THEN
...
!       INITIATE MONTE-CARLO ANALYSIS
         ...
         RETURN
      END IF
      IF(WC.EQ.'AVEC'.OR.WC.EQ.'BVEC'.OR.WC.EQ.'DOT'.OR.WC.EQ.'CROSS') THEN
         CALL VECTOROP
         RETURN
      END IF
      ...
      DO II=1,200
         CALL ITOAAA(II,AI4)
         IF(II.LE.9.AND.WC(1:2).EQ.'F'//AI4(1:1).OR.II.GE.10.AND.I.LE.99.AND.WC(1:3).EQ.'F'//AI4(1:2).OR.II.GE.100.AND.I.LE.999.AND.WC(1:4).EQ.'F'//AI4(1:3)) THEN
            FIELDER =.TRUE.
            GO TO 945
         END IF
      END DO
945   CONTINUE
      ...
      DO II=1,5000
         CALL ITOAAA(II,AI4)
         IF(II.LE.9.AND.WC(1:8).EQ.'R'//AI4(1:1)//'      '.OR.II.GE.10.AND.II.LE.99.AND.WC(1:8).EQ.'R'//AI4(1:2)//'     '.OR.II.GE.100.AND.II.LE.999.AND.WC(1:8).EQ.'R'//AI4(1:3)//'    '.OR.II.GE.1000.AND.II.LE.5000.AND.WC(1:8).EQ.'R'//AI4(1:4)//'   ') THEN
            RAYER=.TRUE.
            GO TO 946
         END IF
      END DO
946   CONTINUE
      IF(RAYER) THEN
         ...
         RETURN
      END IF
      IF(WC.EQ.'NSSDEL'.OR.WC.EQ.'NSSNEW'.OR.WC.EQ.'NSSUNITS'.OR.WC.EQ.'NSSWV'.OR.WC.EQ.'NSSWT'.OR.WC.EQ.'UNIVERSE'.OR.WC.EQ.'OBJECT'.OR.WC.EQ.'ONAME'.OR.WC.EQ.'NSSVERT'.OR.WC.EQ.'NSSDET'.OR.WC.EQ.'SNAME'.OR.WC.EQ.'NSSCOAT1'.OR.WC.EQ.'SGRT'.OR.WC.EQ.'SURFACE'.OR.WC.EQ.'SPROFILE'.OR.WC.EQ.'SGRTD'.OR.WC.EQ.'SPARAM'.OR.WC.EQ.'SPOS'.OR.WC.EQ.'SROT'.OR.WC.EQ.'NSSCOAT2'.OR.WC.EQ.'MEDIA1'.OR.WC.EQ.'MEDIA2'.OR.WC.EQ.'NSSN'.OR.WC.EQ.'NSSSAVE'.OR.WC.EQ.'NSSREST'.OR.WC.EQ.'NSSMINE'.OR.WC.EQ.'NSSMHIT'.OR.WC.EQ.'NSSSPLIT'.OR.WC.EQ.'NSSOBJ'.OR.WC.EQ.'NSSGRIDS'.OR.WC.EQ.'NSSGRIDR'.OR.WC.EQ.'NSSTRACE'.OR.WC.EQ.'SCLAP'.OR.WC.EQ.'SHOLE'.OR.WC.EQ.'NSSLIST'.OR.WC.EQ.'OBJMEDIA'.OR.WC.EQ.'NSSLENO'.OR.WC.EQ.'SCLEAR'.OR.WC.EQ.'SUNCLEAR'.OR.WC.EQ.'SBOUNDX'.OR.WC.EQ.'SBOUNDY'.OR.WC.EQ.'SBOUNDZ'.OR.WC.EQ.'NSSPOL'.OR.WC.EQ.'NSSINTER'.OR.WC.EQ.'NSSAPODR'.OR.WC.EQ.'NSSSPOT'.OR.WC.EQ.'NSSORINT'.OR.WC.EQ.'NSSLINK'.OR.WC.EQ.'NSSEOS'.OR.WC.EQ.'NSSREF') THEN
         CALL NSSCALL
         RETURN
      END IF
