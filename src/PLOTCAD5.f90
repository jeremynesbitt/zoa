!       FIFTH FILE OF PLOT/CAD ROUTINES
...
16       FORMAT(A79)
      ...
      RETURN
   ELSE
!       EXPLICT INPUT EXISTS, PLOT IT
   END IF
   NNTT(1:80)=PLOTNOTE(1:80)
!       DO THE PLOTTING OF THE LENS NOTE AT THE CURRENT
!       PLOT POSITION
   IB=1
   DO I=80,1,-1
      IF(NNTT(I:I).NE.' ') THEN
...
!
!
!                PLOT LIBRARY COMMANDS
   IF(SQ.EQ.1.AND.&
   &WQ.NE.'P'.AND.WQ.NE.'PUT'.AND.WQ.NE.'DELK'.AND.WQ.NE.'GET'&
   &)THEN
      OUTLYNE=&
      &'INVALID QUALIFIER WORD USED WITH THE "PLIB" COMMAND'
      CALL SHOWIT(1)
   ...
   IF(SQ.EQ.0) THEN
      OUTLYNE=&
      &'"PLIB" REQUIRES EXPLICIT QUALIFIER WORD INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      RETURN
   ...
   IF(WQ.EQ.'GET') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE=&
         &'"PLIB GET" TAKES NO STRING OR NUMERIC WORD #2,#3,#4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
   ...
   IF(WQ.EQ.'PUT') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE=&
         &'"PLIB PUT" TAKES NO STRING OR NUMERIC WORD #2,#3,#4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
   ...
   IF(WQ.EQ.'DELK') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE=&
         &'"PLIB DEL" TAKES NO STRING OR NUMERIC WORD #3,#4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
   ...
   IF(WQ.EQ.'P') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         OUTLYNE=&
         &'"PLIB P" TAKES NO STRING OR NUMERIC WORD #3,#4 OR #5 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         RETURN
...
!       THE PLOT COMMANDS PLTYFAN, PLTXFAN, PLTNFAN, PLTPFAN
!       AND PLOTFANS
!       IT IS CALLED BY PPLOTT.FOR.
!
   CHARACTER UNI*11,A1VAL1*5,A2VAL2*3,AWV1*31,AWV2*31 &
   &,AWV3*31,AWV4*31,AWV5*31,AXF1*23,AXF2*23,AXF3*23,AYF1*23,&
   &AYF2*23,AYF3*23,AWV6*31,AWV7*31,AWV8*31,AWV9*31,AWV10*32
!
...
!
   ...
   &SW11,SW12,SW21,SW22,LAX,LAY,DX,DY,DTY,DTX
!
   LOGICAL FOBB0 &
   &,FOBB0X,FOBB0Y,ABORT,FANEXT
!
...
!       CHECK FOR VALID QUALIFIER INPUT
      IF(SQ.EQ.1) THEN
         ...
         &WQ.NE.'OFFSET'.AND.WQ.NE.'WV'.AND.WQ.NE.'GO'.AND.&
         &WQ.NE.'RESET'.AND.WQ.NE.'REFWV'.AND.WQ.NE.'NEWOBJ'.AND.&
         &WQ.NE.'NEWREF'.AND.WQ.NE.'NEWIMG'.AND.WQ.NE.'WV2') THEN
            OUTLYNE=&
...
!     WC NOT PLOTFAN
   END IF
!
!     FOR WC= PLTXFAN,PLTYFAN,PLTPFAN AND PLTNFAN
   IF(WC.EQ.'PLTXFAN'.OR.WC.EQ.'PLTYFAN'.OR.WC.EQ.'PLTPFAN'&
   &.OR.WC.EQ.'PLTNFAN') THEN
!       CHECK FOR VALID QUALIFIER INPUT
      IF(SQ.EQ.1) THEN
...
!       QUALIFIERS ARE VALID, PROCEED
         END IF
      ELSE
!       NO QUALIFIERS, WHICH IN THIS CASE IS OK
      END IF
   ELSE
...
!     NOT PLTPFAN
      END IF
!
!     MOST SYNTAX CHECKING IS DONE, NOW DO INDIVIDUAL COMMANDS
!
!     NOW PLOTFANS RESET
      IF(WC.EQ.'PLOTFANS'.AND.WQ.EQ.'RESET') THEN
!
...
900      CONTINUE
!
!     RESTORE THE OFFSET  (FRACTIONAL APERTURE HT., UNITLESS)
         FANOFF=0.0D0
!
!     RESTORE THE DEFAULT FAN TYPE
         FANTYP=5
!     YFAN FANTYP=1
!     XFAN FANTYP=2
!     NFAN FANTYP=3
...
!     RESTORE FAN YFOB VALUES
         ...
         YFOB3=0.0
!
!     RESTORE FAN XFOB VALUES
         XFOB1=0.0
         XFOB2=0.0
         XFOB3=0.0
!
!     RESTORE SSI AUTO FLAG AND THE SSI DEFAULT VALUE
         SSIFLG=.TRUE.
         SSI=0.0D0
!
!     RESTORE MAXFAN TO THE DEFAULT VALUE
         MAXFAN=20
         STEPJP=125.0D0
!
!     RESTORE THE COUNT FOR THE NUMBER OF FANS ON A PAGE
         FANNUM=1
         FANNM1=3
         FANNM2=3
!
!     (THIS IS INTERNALLY SET BY THE PROGRAM, IT CAN'T BE
!     SET EXPLICITLY BY THE USER)
!
!     ALL DEFAULT VALUES HAVE BEEN RESET
         OUTLYNE=&
         &'DEFAULT FAN PLOTTING VALUES HAVE BEEN RESTORED'
         CALL SHOWIT(1)
!
         RETURN
      ELSE
...
!     VALUE OUT OF RANGE
               OUTLYNE=&
               &'WAVELENGTH NUMBER INPUT VALUES MUST BE:'
               CALL SHOWIT(1)
               OUTLYNE=&
               &'+/-1, +/-2, +/-3, +/-4, +/-5'
               CALL SHOWIT(1)
...
!     PLOTFANS OFFSET
      IF(WC.EQ.'PLOTFANS'.AND.WQ.EQ.'OFFSET') THEN
!     THE OFFSET IS A FRACTIONAL APERTURE DIMMENSION
         FANOFF=W1
         RETURN
      ELSE
      END IF
...
!     PLOTFANS REFWV
      IF(WC.EQ.'PLOTFANS'.AND.WQ.EQ.'REFWV') THEN
         ...
         &.AND.W1.NE.9.0D0.AND.W1.NE.10.0D0) THEN
            OUTLYNE=&
            &'REFERENCE WAVELENGTH NUMBER INPUT VALUE MUST BE:'
            CALL SHOWIT(1)
            OUTLYNE=&
      ...
      ELSE
      END IF
!     PLOTFANS NEWREF,NEWIMG AND NEWOBJ GO HERE
!
!     PLOTFANS NEWOBJ
      IF(WC.EQ.'PLOTFANS'.AND.WQ.EQ.'NEWOBJ') THEN
...
22       FORMAT('CURRENT PLOTTED FAN WILL BE "',A5,1X,A2,'"')
         RETURN
      ELSE
!     NOT PLTYFAN, PLTXFAN, PLTNFAN OR PLTPFAN
      END IF
!
      IF(WC.EQ.'PLOTFANS') THEN
         IF(WQ.EQ.'GO') THEN
            ...
            CALL SHOWIT(3)
23          FORMAT(&
            &'"PLOTFANS GO" CAUSES A FAN TO BE PLOTTED')
            RETURN
         ELSE
         END IF
         IF(WQ.EQ.'RESET') THEN
            ...
            CALL SHOWIT(3)
24          FORMAT(&
            &'"PLOTFANS RESET" RESETS FAN PLOTTING DEFAULT VALUES')
            RETURN
         ELSE
         END IF
         IF(WQ.EQ.'WV') THEN
...
26          FORMAT('CURRENT FAN OFFSET = ',D23.15)
         ...
         IF(WQ.EQ.'SSI') THEN
            IF(SSIFLG) THEN
               ...
               CALL SHOWIT(3)
27             FORMAT(&
               &'SSI WILL BE DETERMINED AUTOMATICALLY DURING FAN PLOTTING')
               RETURN
            ELSE
...
!
      IF(WQ.EQ.'REFWV') THEN
         ...
         CALL SHOWIT(3)
31       FORMAT(&
         &'CURRENT FAN PLOTTING REFERENCE WAVELENGTH NUMBER = ',I3)
         RETURN
      ELSE
      END IF
!
      IF(WQ.EQ.'NEWOBJ') THEN
         ...
         CALL SHOWIT(3)
32       FORMAT(&
         &'CURRENT OBJECT SURFACE NUMBER FOR FAN PLOTTING = ',I3)
         RETURN
      ELSE
      END IF
!
      IF(WQ.EQ.'NEWREF') THEN
         ...
         CALL SHOWIT(3)
33       FORMAT(&
         &'CURRENT REFERENCE SURFACE NUMBER FOR FAN PLOTTING = ',I3)
         RETURN
      ELSE
      END IF
!
      IF(WQ.EQ.'NEWIMG') THEN
         ...
         CALL SHOWIT(3)
34       FORMAT(&
         &'CURRENT IMAGE SURFACE NUMBER FOR FAN PLOTTING = ',I3)
         RETURN
      ELSE
      END IF
!
...
!     NOT QUERY, CONTINUE
   END IF
!
!     HERE IS WHERE THE ACTUAL PLOT GENERATION TAKES PLACE
!
   IF(WC.EQ.'PLOTFANS'.AND.WQ.EQ.'GO') THEN
!
      ...
      END IF
!
!     GENERATE PLOT DATA AND RETURN
!
!     NOW CALCULATE THE FAN DATA AND THEN THE SCALE FACTOR
      ABORT=.FALSE.
!     ZERO THE FFAANN
!     I COUNTS THE NUMBER OF FANS ON THE PLOT. 1, 2 OR 3
!     FAN I=1 IS THE BOTTOM FAN AT FIELD POSITION 1
!     II COUNTS THE 55 RAYS IN THE FAN II=1 IS NEG LIMIT,
!     II = 21 IS TH POS SIDE
!     III COUNTS THE 6 DATA ITEMS IN THE FAN
!     IIII COUNTS THE 10 WAVELENGTH ENTRIES IN THE FAN
!     FFAANN IS PLOT POINTS
!
!     TRANSVERSE FANS THE (QALTYP=0)
!                 III=1 IS TRANSVERS X ABERRATION
!                 III=2 IS TRANSVERS Y ABERRATION
!                 III=3 IS NOT USED
!                 III=4 IS NOT USED
!                 III=5 IS FLAG1 (1=PLOT,0=DON'T PLOT)
!                 III=6 IS FLAG2 (1=PLOT,0=DON'T PLOT)
!     OPD FANS (QALTYP=1)
!                 III=1 OPD IN WAVES AT REF WAVELENGTH
!                 III=2 IS NOT USED
!                 III=3 IS NOT USED
!                 III=4 IS NOT USED
!                 III=5 IS FLAG1 (1=PLOT,0=DON'T PLOT)
!                 III=6 IS FLAG2 (1=PLOT,0=DON'T PLOT)
!     CHROMATIC DIFFERENCE FANS (QALTYP=2)
!                 III=1 XZ PRIMARY CHROMATIC PAIR
!                 III=2 YZ PRIMARY CHROMATIC PAIR
!                 III=3 XZ SECONDARY CHROMATIC PAIR
!                 III=4 YZ SECONDARY CHROMATIC PAIR
!                 III=5 IS FLAG1 (1=PLOT,0=DON'T PLOT)
!                 III=6 IS FLAG2 (1=PLOT,0=DON'T PLOT)
!     CHROMATIC LONGITUDINAL (QALTYP=3)
!                 III=1 LAX
!                 III=2 LAY
!                 III=3 IS NOT USED
!                 III=4 IS NOT USED
...
!                 III=6 IS FLAG2 (1=PLOT,0=DON'T PLOT)
!
!     CALCULATE ALL THE FFAANN
!     OR (PLFAN2 FOR FANTYPs 5 AND 6)
      IF(FANTYP.LE.4) CALL PLFAN1(ABORT,IFA)
      IF(FANTYP.GT.4) CALL PLFAN2(ABORT,IFA)
      IF(ABORT) THEN
!     ALL ERROR MESSAGES DONE IN PLFAN1 OR PLFAN2
         WRITE(OUTLYNE,37)&
         &IFA
37       FORMAT(&
         &'FOR FIELD OF VIEW NUMBER ',I1)
         CALL SHOWIT(1)
         OUTLYNE=&
...
!     PLOT THE FAN DATA AND THE SCALE FACTOR DATA AND CAPTION
      CALL FANDO0
!     PLOT THE BOX AROUND THE PLOT
      CALL FRMBOX
!     PLOT THE LENS IDENTIFIER IF IT IS NOT BLANK
      CALL FANLI
!     PLOT THE WAVELENGTH LEGEND
      IF(QALTYP.NE.2) CALL FANWV
!
!     PLOT THE CHROMATIC DIFFERENCE LEGEND
      IF(QALTYP.EQ.2) CALL FANCD
!
!     PLOT THE REFERENCE WAVELENGTH LEGEND
      CALL FANRWV
!     PLOT THE NAME OF THE TYPE OF PLOT AND THE UNITS OF THE ABERRATION
!     PLOTTED
      CALL FANTP
!     THAT'S IT, WE ARE DONE
!     BEFORE LEAVING, RESTORE THE OLD REF,IMG AND OBJ SURFACES
      CALL OLDSUR
      RETURN
   ELSE
!     NOT 'GO'
...
!
   ...
   &,ICOUNT,KFNN,IIX,L,JK_K,FANWAV,JK_WVN,WWRF
!
   INTEGER JK_WAV(1:10)
!
   COMMON/WAVER/JK_WAV
!
   ...
   &LPWP1,LPWP2,LCW,LSWP1,LSWP2,XXDIF,YYDIF,OOPD,OPDW,&
   &WAV,RRDIF,DIF1,DIF2,DIF3,DIF4,PW11,PW12,PW21,PW22,&
   &SW11,SW12,SW21,SW22,LAX,LAY,DX,DY,DTY,DTX,XX2,YY2 &
   &,JA,JB,XDER1,XDER2,XDER3,XDER4,YDER1,YDER2,YDER3,YDER4
!
...
!     NOT CD FANS
         ...
         &K.EQ.2.AND.JK_WAV(2).EQ.0.OR.&
         &K.EQ.3.AND.JK_WAV(3).EQ.0.OR.&
         &K.EQ.4.AND.JK_WAV(4).EQ.0.OR.&
         &K.EQ.5.AND.JK_WAV(5).EQ.0.OR.&
         &K.EQ.6.AND.JK_WAV(6).EQ.0.OR.&
         &K.EQ.7.AND.JK_WAV(7).EQ.0.OR.&
         &K.EQ.8.AND.JK_WAV(8).EQ.0.OR.&
         &K.EQ.9.AND.JK_WAV(9).EQ.0.OR.&
         &K.EQ.10.AND.JK_WAV(10).EQ.0) THEN
            FFAANN(1:3,1:21,1:4,K)=0.0D0
            FFAANN(1:3,1:21,5:6,K)=1.0D0
!     SPECTRAL WEIGHT IS ZERO, SKIP THE CALCULATIONS
            GO TO 99999
         ELSE
!     WE WANT TO TRACE RAYS
!     SET WW3
            IF(FANEXT) THEN
               WW3=DBLE(JK_WAV(1))
...
!
      DO I=1,FANNUM
         KFNN=I
!     TRACE THE GUT RAY, IF OK KEEP GOING, IF NOT RETURN
!     WITH ABORT SET TO TRUE AND PRINT ERROR MESSAGE
!
         SAVE_KDP(1)=SAVEINPT(1)
!     RESET THE INPUT WORDS AND CALL FFOB
         WC='FOB     '
         WQ='        '
         SQ=0
         S1=1
         ...
         IF(I.EQ.3) THEN
            ...
            W5=0.0D0
         END IF
!     SAVE THE CURRENT OBJ,REF AND IMAGE SURFACE NUMBERS
         CALL RESSUR
!     SET VALUES FOR THE FANS
         NEWOBJ=FANNOB
         NEWREF=FANNRF
         NEWIMG=FANNIM
         CALL FFOB
         ...
         IF(INT(LFOB(4)).EQ.9) WWRF=74
         IF(INT(LFOB(4)).EQ.10) WWRF=75
!     RESTORE THE SAVED OBJ,REF AND IMAGE SURFACE NUMBERS
         CALL OLDSUR
!     RESTORE THE OLD COMMAND LINE
...
!     PROCEED WITH FAN
         END IF
!
!     ALL IS OK WITH THE GUT RAY, TRACE THE FAN.
...
!       CIRCULAR PUPIL COORDINATES ARE RESOLVED IN RAYTRA.FOR.
!
...
!
!
!     FAN LIMITS ARE -1 TO 1 IN 1/(MAXFAN/2.0D0)
!     STEPS FOR MAXFAN+1 TOTAL POINTS
         OFFSET=FANOFF
!
!     NOW DO FAN TRACES
!
...
!     SET THE OFFSET
               OFFSET=FANOFF
!
!     DO THE SETUP FOR THE TYPE 5 6 7 AND 8 FANS
!
...
!       NOT LA
               END IF
!
!     THIS IS THE END OF THE FAN LOOP
            END DO
!     THIS IS THE END OF THE LOOP THAT TRACKS THE LEFT OR RIGHT FAN
!     BY THE ICOUNT VARIABLE
         END DO
!
!     THIS IS THE END OF THE FIELD OF VIEW LOOP
      END DO
!     THIS IS THE END OF THE WAVELENGTH LOOP
99999 CONTINUE
   END DO
   RETURN
END
...
!
   INTEGER J,JJ,IX,OUT1,I,JJJ,K,KKK,IFA &
   &,KFNN,IIX,L,JK_K,FANWAV,JK_WVN,WWRF
!
   INTEGER JK_WAV(1:10)
!
   COMMON/WAVER/JK_WAV
...
!     SET THE OFFSET
            OFFSET=FANOFF
!
!     DO THE SETUP FOR THE EIGHT TYPES OF FANS
!
            IF(FANTYP.EQ.1) THEN
!       YFAN
!       RELATIVE X COORDINATE IS:
...
!     SET THE OFFSET
      OFFSET=FANOFF
!
!     DO THE SETUP FOR TYPES OF FANS
!
      IF(FANTYP.EQ.1) THEN
!       YFAN
!       RELATIVE X COORDINATE IS:
...
!
   ...
   ELSE
      ...
      FAN9=0
      FAN10=0
!     SCAN FOR THE 10 WAVELENGTHS TO USE
      DO I=1,10
...
!
   IF(FAN5.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 5
!     DRAW THE WV5 LINE
      COLPAS=COLR5
      CALL MY_COLTYP(COLPAS)
...
201      FORMAT(A8)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #5 LEGEND
   END IF
   IF(FAN4.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 4
!     DRAW THE WV4 LINE
      COLPAS=COLR4
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #4 LEGEND
   END IF
   IF(FAN3.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 3
!     DRAW THE WV3 LINE
      COLPAS=COLR3
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #3 LEGEND
   END IF
   IF(FAN2.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 2
!     DRAW THE WV2 LINE
      COLPAS=COLR2
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #2 LEGEND
   END IF
   IF(FAN1.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 1
!     DRAW THE WV1 LINE
      COLPAS=COLR1
      CALL MY_COLTYP(COLPAS)
   ...
   END IF
   IF(FAN10.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 10
!     DRAW THE WV10 LINE
      COLPAS=COLR10
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #10 LEGEND
   END IF
   IF(FAN9.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 9
!     DRAW THE WV9 LINE
      COLPAS=COLR9
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #9 LEGEND
   END IF
   IF(FAN8.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 8
!     DRAW THE WV8 LINE
      COLPAS=COLR8
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #8 LEGEND
   END IF
   IF(FAN7.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 7
!     DRAW THE WV7 LINE
      COLPAS=COLR7
      CALL MY_COLTYP(COLPAS)
      ...
      CALL MY_SETCHARASPECT(1.0,1.0)
   ELSE
!     DON'T DO THE PLOTTING OF THE WAVELENGTH #7 LEGEND
   END IF
   IF(FAN6.NE.0) THEN
!     DO THE PLOTTING OF THE LEGEND FOR WAVELENGTH 6
!     DOTTED LINE BUT WITH BIG DOTS
      COLPAS=COLR6
      CALL MY_COLTYP(COLPAS)
