...
!
!
!       SPSRF TAG ON A SURFACE IS AT surf_asi_flag(SURF)
...
!       surf_asi_flag(SURF)=9.0 IF VALUE IS POSITIVE,
...
!
   INTEGER J,I,K,NUMT1,N,NUMT2,NUMT3,NUMT4,NUMT5 ,ALLOERR,NUMT6,NUMT7,NUMT8,NUMT9,NUMT10
!
   REAL*8 TOT,W,SCENTX,SCENTY,APFAC,DELTA,MSS,AMSS,MSSX,MSSY,AMSSX,AMSSY,JA,JB,DS9,DS12
!
!
   JA=COS_A_ANG
   JB=COS_B_ANG
...
!       K IS THE TOTAL NUMBER OF FILE ENTRIES
   ...
   SCENTY=0.0D0
   DO I=2,K
      READ (UNIT=32,REC=I)DSPOT(1),DSPOT(2),DSPOT(3),DSPOT(4),DSPOT(5),DSPOT(6),DSPOT(7),DSPOT(8),DSPOT(9),DSPOT(10),DSPOT(11),DSPOT(12),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(13),DSPOT(14),DSPOT(15),DSPOT(16),DSPOT(17),DSPOT(18),DSPOT(19),DSPOT(20),DSPOT(21),DSPOT(22),DSPOT(23),DSPOT(24),DSPOT(25),DSPOT(26),DSPOT(27),DSPOT(28),DSPOT(29),DSPOT(30),DSPOT(31),DSPOT(32),DSPOT(33),DSPOT(34),DSPOT(35),DSPOT(36),DSPOT(37),DSPOT(38),DSPOT(39),DSPOT(40),DSPOT(41),DSPOT(42),DSPOT(43),DSPOT(44),DSPOT(45),DSPOT(46),DSPOT(47),DSPOT(48),DSPOT(49),DSPOT(50)
      J=I-1
...
!     FOR FOCAL SYSTEMS
!     THE P TERM
         SSSP=SSSP+(DS12*(((LA-SPA)**2)+((LC-SPC)**2))*APFAC)
!     THE Q TERM
         SSSQ=SSSQ+(DS12*((LA*LB)-(LA*SPB)-(LB*SPA)+(SPA*SPB))*APFAC)+(DS12*((LC*LD)-(LC*SPD)-(LD*SPC)+(SPC*SPD))*APFAC)
!     THE R TERM
         SSSR=SSSR+(DS12*(((LB-SPB)**2))*APFAC)+(DS12*(((LD-SPD)**2))*APFAC)
!
!     FOR AFOCAL SYSTEMS
!     THE P TERM
         ASSSP=ASSSP+(DS12*(((AFLB-AFSPB)**2))*APFAC)+(DS12*(((AFLD-AFSPD)**2))*APFAC)
...
!
!       X ONLY
         SSSPX=SSSPX+(DS12*(((LA-SPA)**2))*APFAC)
         SSSQX=SSSQX+(DS12*((LA*LB)-(LA*SPB)-(LB*SPA)+(SPA*SPB))*APFAC)
         SSSRX=SSSRX+(DS12*(((LB-SPB)**2))*APFAC)
!     AFOCAL
         ASSSPX=ASSSPX+(DS12*(((AFLB-AFSPB)**2))*APFAC)
         ASSSQX=0.0D0
         ASSSRX=0.0D0
!       Y ONLY
         SSSPY=SSSPY+(DS12*(((LC-SPC)**2))*APFAC)
         SSSQY=SSSQY+(DS12*((LC*LD)-(LC*SPD)-(LD*SPC)+(SPC*SPD))*APFAC)
         SSSRY=SSSRY+(DS12*(+((LD-SPD)**2))*APFAC)
!     AFOCAL
         ASSSPY=ASSSPY+(DS12*(+((AFLD-AFSPD)**2))*APFAC)
         ASSSQY=0.0D0
         ASSSRY=0.0D0
      ELSE
!       RAY FAILED
...
221 FORMAT('FOCUS SHIFT FOR BEST RMS SPOT LENGTH (X) = ',G17.10,1X,A11)
...
!
   IF(STI.NE.1) THEN
      IF(SST.EQ.1.OR.SQ.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1 .OR.S5.EQ.1) THEN
         OUTLYNE='" SPECIAL " ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
      ...
      IF(DF1.EQ.1.OR.DF2.EQ.1)THEN
         OUTLYNE='" SPECIAL " REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      IF(surf_default_flag(INT(W1)) == 1) THEN
         OUTLYNE='A DEFORMABLE SURFACE MAY NOT BE DEFINED AS A SPECIAL SURFACE'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     ARRAY LENS NOT ALLOWED
      IF(surf_array_parity(INT(W1)) /= 0) THEN
         OUTLYNE='AN ARRAY SURFACE MAY NOT BE DEFINED AS A SPECIAL SURFACE'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       FLAG THE SPECIAL SURFACE TYPE
   IF(INT(DABS(W2)).GT.24) THEN
      ...
      CALL SHOWIT(1)
      OUTLYNE='CURRENTLY SUPPORTED SPECIAL SUFACE TYPES ARE 1 THROUGH 24'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!     CHECK FOR INVALID SURFACE TYPES FOR A TYPE 18 SURFACE
   IF(INT(DABS(W2)).EQ.18) THEN
      IF(surf_toric_flag(INT(W1)) /= 0.OR.surf_is_asphere(INT(W1)) .OR.surf_curvature(INT(W1)).EQ.0.0D0.OR.GLANAM(INT(W1),2).NE.'REFL         '.OR.GLANAM(INT(W1),1).NE.'             '.AND.GLANAM(INT(W1),2).EQ.'REFL         ') THEN
...
!     CHECK FOR INVALID SURFACE TYPES FOR A TYPE 6,7,9,10,11,15,20 SURFACE
   IF(INT(DABS(W2)).EQ.6.OR.INT(DABS(W2)).EQ.7.OR.INT(DABS(W2)).EQ.9.OR.INT(DABS(W2)).EQ.10.OR.INT(DABS(W2)).EQ.11.OR.INT(DABS(W2)).EQ.15.OR.INT(DABS(W2)).EQ.20) THEN
      DM=.FALSE.
      IF(GLANAM(INT(W1),1).EQ.GLANAM((INT(W1)-1),1).AND.GLANAM(INT(W1),2).EQ.GLANAM((INT(W1)-1),2).AND.GLANAM(INT(W1),2).NE.'REFL'.AND.GLANAM(INT(W1),2).NE.'REFLTIR'.AND.GLANAM(INT(W1),2).NE.'REFLTIRO'.AND.GLANAM(INT(W1),2).NE.'IDEAL'.AND.GLANAM(INT(W1),2).NE.'PERFECT      ') DM=.TRUE.
      IF(GLANAM(INT(W1),1).EQ.'MODEL        ') DM=.FALSE.
      IF(GLANAM(INT(W1)-1,1).EQ.'MODEL        ') DM=.FALSE.
      IF(surf_refractive_index(INT(W1), 1).EQ.surf_refractive_index(INT(W1)-1, 1).AND.surf_refractive_index(INT(W1), 2).EQ.surf_refractive_index(INT(W1)-1, 2).AND.surf_refractive_index(INT(W1), 3).EQ.surf_refractive_index(INT(W1)-1, 3).AND.surf_refractive_index(INT(W1), 4).EQ.surf_refractive_index(INT(W1)-1, 4).AND.surf_refractive_index(INT(W1), 6).EQ.surf_refractive_index(INT(W1)-1, 6).AND.surf_refractive_index(INT(W1), 7).EQ.surf_refractive_index(INT(W1)-1, 7).AND.surf_refractive_index(INT(W1), 8).EQ.surf_refractive_index(INT(W1)-1, 8).AND.surf_refractive_index(INT(W1), 9).EQ.surf_refractive_index(INT(W1)-1, 9).AND.surf_refractive_index(INT(W1), 10).EQ.surf_refractive_index(INT(W1)-1, 10).AND.surf_refractive_index(INT(W1), 5).EQ.surf_refractive_index(INT(W1)-1, 5)) DM=.TRUE.
      IF(GLANAM(INT(W1),1).EQ.'MODEL        '.AND.GLANAM(INT(W1)-1,1).EQ.'MODEL        '.AND.GLANAM(INT(W1)-1,2).EQ.GLANAM(INT(W1),2)) THEN
         DM=.FALSE.
         IF(surf_fict_n(INT(W1)).EQ.surf_fict_n(INT(W1)-1).AND.surf_fict_v(INT(W1)).EQ.surf_fict_v(INT(W1)-1).AND.surf_fict_w(INT(W1)).EQ.surf_fict_w(INT(W1)-1)) DM=.TRUE.
      END IF
      IF(surf_dummy_val(INT(W1)) == 1.AND.DM)DM=.FALSE.
      IF(surf_dummy_val(INT(W1)) == 0.AND..NOT.DM)DM=.TRUE.
!
      IF(surf_toric_flag(INT(W1)) /= 0.OR.surf_is_asphere(INT(W1)) .OR.surf_curvature(INT(W1)).NE.0.0D0.OR..NOT.DM) THEN
!     SURFACE NOT A PLANO DUMMY
         WRITE(OUTLYNE,*) 'TYPE ',INT(DABS(W2)),' SURFACE MUST BE A PLANO-DUMMY SURFACE'
         CALL SHOWIT(1)
         OUTLYNE='RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!     SURFACE NOT A ROTATIONALLY SYMMETRIC SURFACE
   ...
   IF(INT(DABS(W2)).EQ.24) THEN
      IF(surf_tilt_flag(INT(W1)) == 2 .OR.surf_tilt_flag(INT(W1)) == 3 ) THEN
...
!
   IF(OPTMES) THEN
      IF(IN.EQ.5) THEN
         WRITE(OUTLYNE,*)'SURFACE ',INT(W1),' NOW DEFINED AS A TYPE ',INT(W2),' SURFACE'
         CALL SHOWIT(1)
      END IF
   END IF
   call set_surf_asi_flag(INT(W1), NINT(W2))
!
...
!
   IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
      WRITE(OUTLYNE,*)'" ',WC,' ',WQ,' " ONLY TAKES NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
   ...
   END IF
   IF(DF1.EQ.1) THEN
      WRITE(OUTLYNE,*)'" ',WC,' ',WQ,' " REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(W1.EQ.0.0) THEN
      ...
      RETURN
   END IF
   IF(surf_asi_flag(INT(W1)).EQ.0.0D0) THEN
      ...
      RETURN
   END IF
   IF(WQ.EQ.'ON'.AND.surf_asi_flag(INT(W1)).LT.0.0D0) THEN
      call set_surf_asi_flag(INT(W1), -surf_asi_flag(INT(W1)))
   END IF
   IF(WQ.EQ.'OFF'.AND.surf_asi_flag(INT(W1)).GT.0.0D0) THEN
      call set_surf_asi_flag(INT(W1), -surf_asi_flag(INT(W1)))
   END IF
   RETURN
END
...
!       CONTRO.FOR)
!
   IF(WC.NE.'EOS'.AND.WC.NE.'EVAL'.AND.WC.NE.'COEFS'.AND.WC.NE.'FIT'.AND.WC.NE.'READ'.AND.WC.NE.'COEF'.AND.WC.NE.'TYPE'.AND.WC.NE.'SURF'.AND.WC.NE.'FITGLASS'.AND.WC.NE.'CK'.AND.WC.NE.'LISTCOEF'.AND.WC.NE.'GDATA'.AND.WC.NE.'M'.AND.WC.NE.'DATA'.AND.WC.NE.'LIST') THEN
...
!     OPEN THE USER.DAT FILE AND ADD AN ENTRY
         OPEN(UNIT=36,ACCESS='APPEND',FILE='USER.DAT',RECL=132,STATUS='UNKNOWN')
         WRITE(UNIT=36,FMT=*)FITGLASS,CFTYPE(9),CFTYPE(11),CFTYPE(7),CFTYPE(5),CFTYPE(3),CFTYPE(1)
...
!
   REAL*8 ACCUM(1:96,1:96),CCOL(1:96),STYPE,SSURF
!
   COMMON/SPIDAT/IDATA
!
   COMMON/ST/STYPE
...
!
   CHARACTER TNAME*8,NM*8,FN*10,TTIME*8,DDATE*10,TTTIM*8,DDDAT*10
!
   LOGICAL TORR,EXISJK
!
   COMMON/TTOORR/TORR
!
   ...
   INTEGER OCC,NUM_JK,POINTS,NF,I,J,CTAB,NTOTAL,CNTR
!
   REAL*8 LAMB1,LAMB2,CUMULT(1:1001,1:3),TABLE(1:1001,1:3),DATA1,DATA2
!
   COMMON/CUM/CUMULT
!
   COMMON/CURNAM/TNAME
...
!       ONLY THE COMMANDS "DATA" AND "ENDTABLE" ARE VALID
      IF(WC.NE.'DATA'.AND.WC.NE.'ENDTABLE') THEN
         WRITE(OUTLYNE,*)'ONLY THE "DATA" AND "ENDTABLE" COMMANDS ARE VALID AT THIS LEVEL'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       IF W3, W4 AND W5 INPUT, PRINT MESSAGE
         IF(SST.EQ.1.OR.SQ.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
            WRITE(OUTLYNE,*)'"DATA" ONLY TAKES NUMERIC WORD #1 AND #2 INPUT'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
         ...
         END IF
         IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
            WRITE(OUTLYNE,*)'"DATA" REQUIRES EXPLICIT NUMERIC WORD 1 AND 2 INPUT'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       "DELETE" COMMAND
   IF(WC.EQ.'DELETE') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"DELETE" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(SQ.EQ.1.AND.DF1.EQ.0) THEN
         WRITE(OUTLYNE,*)'"DELETE" TAKES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
      ...
      IF(SQ.EQ.0.AND.DF1.EQ.1) THEN
         WRITE(OUTLYNE,*)'"DELETE" REQUIRES EITHER QUALIFIER OR NUMERIC WORD #1 INPUT.'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       CHECK THAT W3 GE 2
      IF(W3.LT.2.0) THEN
         WRITE(OUTLYNE,*)'THE NUMBER OF POINTS MUST BE GREATER THAN OR EQUAL TO 2.'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       CHECK THAT NW1 IS LT NW2
      IF(W1.GE.W2) THEN
         WRITE(OUTLYNE,*)'NUMERIC WORD 2 MUST BE GREATER THAN NUMERIC WORD 1'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "GETFILE" COMMAND
   IF(WC.EQ.'GETFILE') THEN
      IF(SST.EQ.1) THEN
         WRITE(OUTLYNE,*)'"GETFILE" ONLY TAKES QUALIFIER AND NUMERIC WORD INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.0) THEN
         WRITE(OUTLYNE,*)'"GETFILE" REQUIRES A SPECT FILE NAME AS THE QUALIFIER WORD'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      IF(W5.NE.DBLE(INT(W5)))THEN
         WRITE(OUTLYNE,*)'ONLY AN INTEGER EXPONENT IS ALLOWED IN "GETFILE"'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "BLACKBDY" COMMAND
   IF(WC.EQ.'BLACKBDY') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"BLACKBDY" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(WQ.NE.' '.AND.WQ.NE.'SRE'.AND.WQ.NE.'SRPE') THEN
         WRITE(OUTLYNE,*)'INVALID QUALIFIER WORD USED WITH "BLACKBDY"'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
      IF(W1.LT.0.0D0) THEN
         WRITE(OUTLYNE,*)'A NEGATIVE TEMPERATURE VALUE IS NOT ALLOWED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "PHOTOPIC" COMMAND
   IF(WC.EQ.'PHOTOPIC') THEN
      IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PHOTOPIC" TAKES NO ADDITIONAL INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "SCOTOPIC" COMMAND
   IF(WC.EQ.'SCOTOPIC') THEN
      IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
         WRITE(OUTLYNE,*)'"SCOTOPIC" TAKES NO ADDITIONAL INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "SPRINT" COMMAND
   IF(WC.EQ.'PRINT') THEN
      IF(SST.EQ.1) THEN
         WRITE(OUTLYNE,*)'"SPRINT" ONLY TAKES QUALIFIER AND NUMERIC WORD INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       WFACTOR COMMAND
   IF(WC.EQ.'WFACTOR') THEN
      IF(SQ.EQ.1.OR.SST.EQ.1) THEN
         WRITE(OUTLYNE,*)'"WFACTOR" TAKES NO QULIFIER OR STRING INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"WFACTOR" ONLY TAKES NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      IF(W1.LT.1.0D0.OR.W1.GT.10.0D0) THEN
         WRITE(OUTLYNE,*)'NUMBER OF WAVELENGTHS MUST BE 1 TO 10'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "LIST" COMMAND
   IF(WC.EQ.'LIST') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"LIST" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.1.AND.DF1.EQ.0) THEN
         WRITE(OUTLYNE,*)'"LIST" TAKES EITHER QUALIFIER OR NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
...
!       ***************************************************************
      ...
      END IF
!       ***************************************************************
      OPEN(UNIT=35,ACCESS='DIRECT',FILE=LIBTRA//'TRA.DAT',FORM='UNFORMATTED',RECL=(50*NRECL),STATUS='UNKNOWN')
!
      DO 201 I=1,999
...
!       OPEN AND READ FROM FILE
      OPEN(UNIT=35,ACCESS='DIRECT',FILE=LIBTRA//FN,FORM='UNFORMATTED',RECL=(80*NRECL),STATUS='UNKNOWN')
!
      CTAB=POINTS
      DO 211 J=1,POINTS
...
!       ***************************************************************
      OPEN(UNIT=35,ACCESS='DIRECT',FILE=LIBTRA//'TRA.DAT',FORM='UNFORMATTED',RECL=(50*NRECL),STATUS='UNKNOWN')
      READ(UNIT=35,REC=INT(W1))NM,DATA1,DATA2,POINTS,OCC,DDATE,TTIME
      DDDAT=DDATE
      TTTIM=TTIME
      IF(OCC.EQ.0) THEN
...
!       "PLOTR AND "PLOTT" COMMAND
   IF(WC.EQ.'PLOTR'.OR.WC.EQ.'PLOTT') THEN
      IF(SST.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PLOTR" AND "PLOTT" TAKE NO STRING INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.1.AND.DF1.EQ.0) THEN
         WRITE(OUTLYNE,*)'"PLOTR" AND "PLOTT" TAKE EITHER QUALIFIER OR ','NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      ...
      END IF
   END IF
   IF(WC.EQ.'PLOTR'.AND.SQ.EQ.0.AND.DF1.EQ.1.OR.WC.EQ.'PLOTT'.AND.SQ.EQ.0.AND.DF1.EQ.1) THEN
      WRITE(OUTLYNE,*)'"PLOTR" AND "PLOTT" REQUIRES EITHER QUALIFIER OR ','NUMERIC WORD 1 INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
      RETURN
   END IF
   IF(WC.EQ.'PLOTT'.AND.SQ.EQ.1.OR.WC.EQ.'PLOTR'.AND.SQ.EQ.1) THEN
!       SEARCH FOR FILE NAMED WQ AND IF FOUND, LOAD IT, THEN
!       PROCEED.
!       DATA STORED IN TRA.DAT IS:
!               FILE NAME (TNAME) CHARACTER *8
...
2112  CONTINUE
      ...
      RETURN
   END IF
   IF(WC.EQ.'PLOTR'.AND.DF1.EQ.0.OR.WC.EQ.'PLOTT'.AND.DF1.EQ.0) THEN
...
!       "PUNCH" COMMAND
   IF(WC.EQ.'PUNCH') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"PUNCH" ONLY TAKES QUALIFIER OR NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.1.AND.DF1.EQ.0) THEN
         WRITE(OUTLYNE,*)'"PUNCH" TAKES EITHER QUALIFIER OR NUMERIC WORD 1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'BUT NOT BOTH'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
...
!       "NAME" COMMAND
   ...
   IF(WC.EQ.'NAME'.AND.SQ.EQ.0) THEN
      ...
      ELSE
         ...
         CALL SHOWIT(0)
1200     FORMAT('THE CURRENT SPECT TABLE FILE IN MEMORY IS ( ',A8,' )')
         RETURN
      END IF
   END IF
!       "FILE" COMMAND
...
!       "RENAME" COMMAND
   IF(WC.EQ.'RENAME') THEN
      ...
      END IF
   END IF
   IF(WC.EQ.'RENAME'.AND.SQ.EQ.0.OR.WC.EQ.'RENAME'.AND.SST.EQ.0) THEN
      WRITE(OUTLYNE,*)'"RENAME" REQUIRES BOTH QUALIFIER AND STRING INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!       "DROP" COMMAND
   IF(WC.EQ.'DROP') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"DROP" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.0) THEN
         WRITE(OUTLYNE,*)'"DROP" REQUIRES AN EXPLICIT FILE NAME QUALIFIER WORD'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(DF1.EQ.1) THEN
         WRITE(OUTLYNE,*)'"DROP" REQUIRES AN EXPLICIT NUMERIC WORD #1 ENTRY TO DELETE'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "INTER" COMMAND
   IF(WC.EQ.'INTER') THEN
      IF(SST.EQ.1.OR.S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"INTER" ONLY TAKES QUALIFIER AND NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.0) THEN
         WRITE(OUTLYNE,*)'"INTER" REQUIRES AN EXPLICIT FILE NAME QUALIFIER WORD'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(W1.LT.0.0D0) THEN
         WRITE(OUTLYNE,*)'INTERPOLATION TO OR BELOW ZERO WAVELENGTH IS NOT ALLOWED'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(DF1.EQ.1) THEN
         WRITE(OUTLYNE,*)'"INTER" REQUIRES AN EXPLICIT NUMERIC WORD #1 WAVELENGTH VALUE.'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!       "INSERT" COMMAND
   IF(WC.EQ.'INSERT') THEN
      IF(SST.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         WRITE(OUTLYNE,*)'"INSERT" ONLY TAKES QULIFIER AND NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(SQ.EQ.0) THEN
         WRITE(OUTLYNE,*)'"INSERT" REQUIRES AN EXPLICIT FILE NAME QULIFIER WORD'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
      IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
         WRITE(OUTLYNE,*)'"INSERT" REQUIRES EXPLICIT NUMERIC WORD #1 AND #2 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   ...
   END IF
   IF(DF1.EQ.1) THEN
      OUTLYNE='" '//WC//' " REQUIRES EXPLICIT NUMERIC WORD #1 INPUT'
      CALL SHOWIT(1)
      OUTLYNE='RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   IF(surf_asi_flag(INT(W1)).EQ.0.0D0) THEN
      ...
      RETURN
   END IF
   IF(surf_asi_flag(INT(W1)).NE.0.0D0) THEN
      call set_surf_asi_flag(INT(W1), 0)
      WRITE(OUTLYNE,*)'SPECIAL SURFACE DEFINITION REMOVED FROM SURFACE ',INT(W1)
      CALL SHOWIT(1)
   END IF
   RETURN
END
