...
!
   IF(SST.EQ.1.OR.SN.EQ.1) THEN
      ...
      & '"UPDATE SPSRF" TAKES NO EXPLICIT INPUT'//'\n'//&
      & 'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   F1=0
...
!       MODE AFOCAL OR UAFOCAL
      IF(SST.EQ.1) THEN
         CALL REPORT_ERROR_AND_FAIL('"SPDSTATS" TAKES NO STRING INPUT'//'\n'//'RE-ENTER COMMAND', 1)
         RETURN
      END IF
      DELTA=0.0D0
   END IF
...
!       NO FILE EXISTS, ERROR AND STOP
      CALL REPORT_ERROR_AND_FAIL(&
      & 'NO '//WQ(1:N)//'.DAT FILE EXISTS TO ANALYZE'//'\n'//&
      & 'NO ACTION TAKEN', 1)
      RETURN
   END IF
   SPDEXT=.TRUE.
...
!
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"SPSRF" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   F1=0
   F7=1
...
!     ARRAY LENS NOT ALLOWED
   ...
   ELSE
!     STI=1
      CALL REPORT_ERROR_AND_FAIL('"SPECIAL ?" HAS NO MEANING'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
...
!     SURFACE NOT A SIMPLE MIRROR AND SPHERICAL OR CONIC
         CALL REPORT_ERROR_AND_FAIL(&
         & 'TYPE 18 SURFACE MUST BE A SIMPLE SPHERE OR CONIC'//'\n'//&
         & 'AND MUST ALSO BE A MIRROR WITH GLASS TYPE "REFL"'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
   END IF
...
!     SURFACE NOT A ROTATIONALLY SYMMETRIC SURFACE
         CALL REPORT_ERROR_AND_FAIL(&
         & 'TYPE 23 SURFACE MUST BE ROTATIONALLY'//'\n'//&
         & 'SYMMETRIC SURFACES'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     SURFACE CAN'T HAVE A TILT AUTO ON IT
         CALL REPORT_ERROR_AND_FAIL(&
         & 'TYPE 24 SURFACE MAY NOT HAVE TILT AUTO OR TILT AUTOM'//'\n'//&
         & 'ASSIGNED TO IT'//'\n'//&
         & 'RE-ENTER COMMAND', 1)
         RETURN
      END IF
...
!     SET DEFAULTS FOR TYPE 12
   IF(INT(W2).EQ.12) THEN
      ...
      FTFL01(2,INT(W1))=sys_wavelength(INT(sys_wl_ref()))
...
!
   ...
   &WC.NE.'M'.AND.WC.NE.'DATA'.AND.WC.NE.'LIST') THEN
      CALL REPORT_ERROR_AND_FAIL('INVALID SPFIT COMMAND'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   IF(WC.EQ.'M'.OR.WC.EQ.'CK') THEN
      CALL MESCOM
...
!       CHECK IF TYPE OR SURF SET
   IF(F23.EQ.0) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '"TYPE" OR "SURF" MUST BE ENTERED BEFORE'//'\n'//&
      & 'DATA MAY BE ENTERED', 1)
      RETURN
   END IF
...
!       COMMAND (FIT)
   IF(WC.EQ.'FIT'.OR.WC.EQ.'FITGLASS') THEN
      IF(F25.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL('NO ACCUMULATED DATA EXISTS FOR FITTING', 1)
         RETURN
      END IF
      CALL FITFIT
...
!       COMMAND (COEFS)
   IF(WC.EQ.'COEFS') THEN
      IF(F24.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL(&
         & 'FITTED DATA DOES NOT EXIST'//'\n'//&
         & 'COEFFICIENTS NOT YET AVAILABLE', 1)
         RETURN
      END IF
      CALL FCOEFS
      RETURN
...
!       COMMAND (EVAL)
   IF(WC.EQ.'EVAL') THEN
      IF(F24.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL('FITTED DATA DOES NOT EXIST FOR EVALUATION', 1)
         RETURN
      END IF
      CALL FTEVAL
      RETURN
...
!       COMMAND (LIST)
   IF(WC.EQ.'LIST') THEN
      IF(F24.EQ.0) THEN
         CALL REPORT_ERROR_AND_FAIL('FITTED DATA DOES NOT EXIST FOR LISTING', 1)
         RETURN
      END IF
      CALL FTLIST
      RETURN
...
!
   IF(SST.EQ.1.OR.SN.EQ.1.OR.SQ.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL('"SPFIT" TAKES NO EXPLICIT INPUT'//'\n'//'RE-ENTER COMMAND', 1)
      RETURN
   END IF
   F1=0
   F9=1
...
!
   ...
   &.OR.S5.EQ.1) THEN
      CALL REPORT_ERROR_AND_FAIL(&
      & '" '//WC//' " ONLY TAKES NUMERIC WORD #1 INPUT'//'\n'//&
      & 'RE-ENTER DATA', 1)
      RETURN
   END IF
   IF(DF1.EQ.1) THEN
