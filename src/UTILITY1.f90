!       FIRST SET OF UTILTIY ROUTINES GO HERE
...
!
!
!       IF SQ.EQ.0, DON'T LOOK FOR A FILE, JUST MOVE THE
!       CUMULATIVE AREA.
!
   IF(SQ.EQ.1) THEN
!
...
210   CONTINUE
      CALL CLOSE_FILE(35,1)
   ELSE
!       QUALIFIER WAS BLANK, NO FILE TO LOAD
!       LOAD CUMULATIVE AREA INTO WORK
      TCOUNT=NTOTAL
      POINTS=NTOTAL
      J=NTOTAL
...
!
   ...
   IF(L1.LE.0.0D0.OR.L2.LE.0.0D0) THEN
      WRITE(OUTLYNE,*)&
      &'A ZERO OR NEGATIVE WAVELENGTH VALUE IS NOT ALLOWED'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
!       SCALE THE CURRENT TEMP DATA BY THE REQUESTED
!       TRANSFORMATION.
!
   DO 505 J=1,TCOUNT
...
!
   IF(TEMP(POINTS,2).LE.TEMP(1,2)) THEN
      WRITE(OUTLYNE,*)&
      &'NEW WAVELENGTHS ARE NOT IN ASCENDING ORDER AND'
      CALL SHOWIT(1)
...
!       LAMB2 RANGE IN WORK, WITH SPACING DELLAM.
!
   DO 11 J=1,NTOTAL
      WORK(J,1)=DBLE(J)
11 CONTINUE
...
10 CONTINUE
!
!       NOW INTERPOLATE THE TEMP AREA AND STORE INTO THE
!       WORK AREA FUNCTIONAL VALUES.
!
   SLOPE=0.0D0
   JK=1
   DO 20 J=1,TCOUNT-1
...
!       VALUE OF TEMP((J+1),2), WE KEEP USING THE CURRENT SLOPE.
!
!       IF THE WAVELENGTH VALUE FROM CUMULATIVE AREA BECOMES
!       GREATER THAN THE CURRENT TEMP((J+1),2), CALCULATE A NEW
!       SLOPE UNLESS J+1=TCOUNT. IF J+1=TCOUNT, KEEP USING
!       THE CURRENT SLOPE UNTIL THE CUMULATIVE AREA IS FILLED.
!
!       IF THE LAST WAVELENGTH VALUE IN WORK IS REACHED(J=TCOUNT),
!
      DO 30 I=KI,NTOTAL
         IF(WORK(I,2).LE.TEMP((J+1),2).AND.(J+1).LE.TCOUNT &
         &.OR.WORK(I,2).GT.TEMP((J+1),2).AND.(J+1).GE.TCOUNT) THEN
...
20 CONTINUE
!
!       WORK AREA HAS HAD STORED IN IT THE TRANSFORMED,
!       INTERPOLATED TEMP AREA. PRINT MESSAGE AND RETURN
   WRITE(OUTLYNE,*)&
   &'THE NEW CONTENTS OF THE "WORK" AREA AFTER ACTION'
   CALL SHOWIT(1)
...
!
   IF(is_command_query()) THEN
      OUTLYNE=&
      &'"NEWSEED" RESETS THE RANDOM NUMBER GENERATOR PROGRAM WIDE'
      CALL SHOWIT(1)
      RETURN
   END IF
   MSEED=1618033.
...
!       AND ALSO SETS THE DEFAULT OPCON OR OPERATING CONDITIONS.
!       USED FOR CHANGING, SETTING, AND QUERRYING
...
!
      ...
      &.AND.WQ.NE.'PFAC'.AND.WQ.NE.'MRAYS'.AND.WQ.NE.'CAIMTOL'&
      &.AND.WQ.NE.'DINMUL'.AND.WQ.NE.'NRAITR'.AND.WQ.NE.'LINTOL'&
      &.AND.WQ.NE.'DAYS'.AND.WQ.NE.'ONTOL'.AND.WQ.NE.'SINGTOL'.AND.&
      &WQ.NE.'DIFTOL'.AND.WQ.NE.'DELSUR'.AND.&
      ...
      &.OR.WQ.EQ.'PFAC'.OR.WQ.EQ.'MRAYS'.OR.WQ.EQ.'CAIMTOL'&
      &.OR.WQ.EQ.'DINMUL'.OR.WQ.EQ.'DIFTOL'.OR.WQ.EQ.'DELSUR'&
      &.OR.WQ.EQ.'ONTOL'.OR.WQ.EQ.'SINGTOL'.OR.WQ.EQ.'LINTOL'.OR.&
      &WQ.EQ.'SERLIM'.OR.WQ.EQ.'SAGDEL'.OR.WQ.EQ.'MAXREG'.OR.&
      &WQ.EQ.'MAXOPT') THEN
...
!       LIMITS FOR MAXREG
         IF(WQ.EQ.'MAXREG') THEN
            IF(INT(W1).LT.4000) THEN
               OUTLYNE=&
               &'"PM MAXREG" REQUIRES NUMERIC WORD #1 INPUT GREATER THAN OR'
               CALL SHOWIT(1)
               OUTLYNE=&
               &'EQUAL TO 4000'
               CALL SHOWIT(1)
...
!       LIMITS FOR MAXOPT
         IF(WQ.EQ.'MAXOPT') THEN
            IF(INT(W1).LT.4000) THEN
               OUTLYNE=&
               &'"PM MAXOPT" REQUIRES NUMERIC WORD #1 INPUT GREATER THAN OR'
               CALL SHOWIT(1)
               OUTLYNE=&
               &'EQUAL TO 4000'
               CALL SHOWIT(1)
...
!       NOT DIFTOL
         ...
         IF(WQ.EQ.'LINTOL') THEN
            IF(W1.LE.0.0D0.OR.W1.GT.1.0D0) THEN
               OUTLYNE=&
               &'"PM LINTOL" REQUIRES POSITIVE NUMERIC WORD #1 INPUT'
               CALL SHOWIT(1)
               OUTLYNE=&
...
!
      IF(WQ.NE.'SURTOL'.AND.WQ.NE.'AIMTOL'.AND.WQ.NE.'SERINC'&
      &.AND.WQ.NE.'PFAC'.AND.WQ.NE.'MRAYS'.AND.WQ.NE.'CAIMTOL'&
      &.AND.WQ.NE.'DINMUL'.AND.WQ.NE.'NRAITR'.AND.WQ.NE.'SHORT'&
      &.AND.WQ.NE.'MEDIUM'.AND.WQ.NE.'LONG'.AND.WQ.NE.'SELDELAY'&
      &.AND.WQ.NE.'DIFTOL'.AND.WQ.NE.'DELSUR'.AND.WQ.NE.'ONTOL'.AND.&
      &WQ.NE.'SINGTOL'.AND.WQ.NE.'LINTOL'&
      ...
      END IF
      IF(WQ.EQ.'SURTOL'.OR.WQ.EQ.'AIMTOL'.OR.WQ.EQ.'SERINC'&
      &.OR.WQ.EQ.'PFAC'.OR.WQ.EQ.'CAIMTOL'&
      &.OR.WQ.EQ.'DINMUL'.OR.WQ.EQ.'DIFTOL'.OR.WQ.EQ.'LINTOL'&
      &.OR.WQ.EQ.'DELSUR'.OR.WQ.EQ.'ONTOL'.OR.WQ.EQ.'SINGTOL'&
      &.OR.WQ.EQ.'SERLIM'.OR.WQ.EQ.'SAGDEL'.OR.WQ.EQ.'MAXREG'.OR.&
      &WQ.EQ.'MAXOPT') THEN
...
!       NOT PMP
   ...
   IF(WC.EQ.'OPCON') THEN
      ...
      LINTOL=0.1D0
      DELSUR=0.0025D0
!       DIFTOL IS THE DEFAULT INCREMENT FOR THE REAL RAYTRACE
!       SIMULATION OF THE DIFFERENTIAL RAY TRACE. THE DEFAULT VALUE
!       IS SET AT 1.0D-3 OR THE REFERENCE APERTURE HT AT THE REFERENCE SURFACE
      DIFTOL=1.0D-3
      ONTOL=1.0D-10
      SINGTOL=1.0D-12
!
...
!
!               T = KEYBOARD + FILE IN STRING (UNIT 97) THIS IS OUT = 69
!               FILE = FILE IN STRING = 97 THIS IS OUT = 97
!
...
!
   ...
   IF(WQ.EQ.'T'.OR.WQ.EQ.'FILE') THEN
      ...
      IF(WQ.EQ.'FILE    '.OR.WQ.EQ.'T       ') THEN
         ...
         IF(APPEND)&
         &OPEN(UNIT=97,ACCESS='APPEND',BLANK='NULL'&
         &,FORM='FORMATTED',FILE=trim(basePath)//OFILN &
         &,STATUS='UNKNOWN',ERR=314)
         IF(.NOT.APPEND)&
...
!
!
   IF(WC.EQ.'REPLACE'.AND.is_command_query()) THEN
      OUTLYNE=&
      &'"REPLACE" MAKES INPUT/OUTPUT ASCII FILES SEQUENTIAL'
      CALL SHOWIT(5)
      IF(APPEND) OUTLYNE=&
      &'"APPEND" IS CURRENTLY IN EFFECT'
      IF(.NOT.APPEND) OUTLYNE=&
      &'"REPLACE" IS CURRENTLY IN EFFECT'
      CALL SHOWIT(5)
      RETURN
   END IF
   IF(WC.EQ.'APPEND'.AND.is_command_query()) THEN
      OUTLYNE=&
      &'"APPEND" MAKES INPUT/OUTPUT ASCII FILES APPENDED'
      CALL SHOWIT(5)
      IF(APPEND) OUTLYNE=&
      &'"APPEND" IS CURRENTLY IN EFFECT'
      IF(.NOT.APPEND) OUTLYNE=&
   ...
   IF(SN.EQ.1.OR.SST.EQ.1.OR.SQ.EQ.1) THEN
      OUTLYNE=&
      &'"APPEND" AND "REPLACE" TAKES NO ADDITIONAL INPUT'
      CALL SHOWIT(5)
      CALL MACFAL
      RETURN
   END IF
...
444   CONTINUE
      ...
      &,FORM='FORMATTED',FILE=trim(basePath)//'EDITTEXT.DAT'&
      &,STATUS='UNKNOWN',ERR=333)
      RETURN
333   IF(APPEND) OPEN(UNIT=9,FILE='EDITTEXT.DAT',STATUS='UNKNOWN'&
      &,ACCESS='APPEND')
      IF(.NOT.APPEND)&
      &OPEN(UNIT=9,FILE='EDITTEXT.DAT',STATUS='UNKNOWN'&
      &,ACCESS='SEQUENTIAL')
...
555   IF(APPEND) OPEN(UNIT=9,FILE='PUNCH.DAT',STATUS='UNKNOWN'&
      &,ACCESS='APPEND')
      IF(.NOT.APPEND)&
      &OPEN(UNIT=9,FILE='PUNCH.DAT',STATUS='UNKNOWN'&
      &,ACCESS='SEQUENTIAL')
...
!******************************************************************************
!     PARAMETERS
!     PULL OUT PARAMETER NUMBER
   TEMPER=TEMPCC(4:1024)//'    '
!
!     STRIP UP TO 10 LEFT BLANKS
   DO J=1,10
      ...
      END IF
   END DO
!     FIND BLANK AT END OF PARAMETER NUMBER
   J=1
   DO WHILE (TEMPER(J:J).NE.' ')
      J=J+1
   END DO
!     J REPRESENTS A BLANK TO THE RIGHT OF THE PARAMETER VALUE
   VALA=AA23
   VALA=TEMPER(1:J-1)
   CALL RIGHTJUST(VALA)
   CALL ATOIZMX(VALA,IPAR,ZMXERROR)
   IF(ZMXERROR) RETURN
!     IVAL IS PARAMETER NUMBER, NOW STRIP OUT PARAMETER VALUE
!
!     STRIP UP TO 10 LEFT BLANKS
   DO J=1,10
      IF(TEMPER(1:1).NE.' ') THEN
   ...
   IF(ZMXERROR) RETURN
!     VALV IS PARAMETER VALUE
!                  FINISHED BREAKING OUT PARAMETER NUMBER AND VALUES
!******************************************************************************
   RETURN
END
! SUB WWORD.FOR
...
!     DISPLAY CURRENT SETTINGS
   ...
   IF(SQ.EQ.1.AND.S1.EQ.1) THEN
      OUTLYNE=&
      &'"'//WC(1:2)//'" TAKES EITHER NUMERIC OR QUALIFIER INPUT'
      CALL SHOWIT(1)
      OUTLYNE=&
      &'BUT NOT BOTH'
      CALL SHOWIT(1)
...
SUBROUTINE UPPRLN
!
!     THIS CAUSES THE CURRENT LENS TO BECOME THE PERMANENT LENS
!     IT IS ONLY CALLED FROM CFG1 JUST BEFORE GOING TO ANOTHER CFG
!
   use DATCFG
   use DATSUB
   use DATLEN
...
!
   IF(F12.EQ.1) THEN
      CALL PTOC
!
!               FINISHED COPYING CFG 1 CURLENS TO PERLENS
!
      F6=1
      F1=0
      F22=1
...
!       COMMANDS FROM THE CMD LEVEL.
!
   CHARACTER*12 HCOL(0:9),HCOL2(0:9),&
   &HROW(0:100),HROW2(0:100)
   CHARACTER AI*3,&
   &BLANK*12,HEADL1*132,&
   &HEADL2*132,HEADL3*80,HEADL4*80,BL12*12,LINE1*132,&
   &LINE3*80,A1*12,A2*12,A3*12,A4*12,&
   &A5*12,A6*12,A7*12,A8*12,A9*12
!
   INTEGER NROW,NCOL,&
   &I,J,FROW,FCOL
!
...
!
   ...
   BL12='            '
!
!       "ROWHD","ROWHD2","COLHD" AND "COLHD2" COMMANDS
!
!       CHECK FOR STRING INPUT
!
   IF(WC.EQ.'COLHD'.OR.WC.EQ.'COLHD2'.OR.WC.EQ.'ROWHD'&
   &.OR.WC.EQ.'ROWHD2') THEN
      IF(SST.EQ.0) THEN
         WRITE(OUTLYNE,*)&
         &WC,' COMMAND REQUIRES EXPLICIT STRING INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
      ...
      END IF
   END IF
!       CHECK FOR QUALIFIER INPUT
!
   IF(WC.EQ.'COLHD'.OR.WC.EQ.'COLHD2'.OR.WC.EQ.'ROWHD'&
   &.OR.WC.EQ.'ROWHD2') THEN
      IF(SQ.EQ.0) THEN
         WRITE(OUTLYNE,*)&
         &WC,' COMMAND REQUIRES EXPLICIT QUALIFIER WORD INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   ...
   &.OR.WC.EQ.'ROWHD2') THEN
      IF(SN.EQ.1) THEN
         WRITE(OUTLYNE,*)&
         &WC,' COMMAND TAKES NO NUMERIC WORD INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   END IF
!       "TABLE" COMMAND
!       VALID QUALIFIERS ARE "CLEAR","SETUP","PUT","GET",
!       "PRINT1", "PRINT2","PRINT3", "SAVE" AND "RELOAD"
!
!       CHECK FOR STRING INPUT
!
   ...
   END IF
!
!       CHECK FOR NO QUALIFIER
!
   IF(WC.EQ.'TABLE'.AND.SQ.EQ.0) THEN
      WRITE(OUTLYNE,*)&
      &'"TABLE" REQUIRES EXPLICIT QUALIFIER WORD INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   IF(WC.EQ.'TABLE'.AND.SQ.EQ.1) THEN
      ...
      &WQ.NE.'GET'.AND.WQ.NE.'PRINT1'.AND.WQ.NE.'SAVE'.AND.WQ.NE.&
      &'RELOAD'.AND.WQ.NE.'PRINT2'.AND.WQ.NE.'PRINT3') THEN
         WRITE(OUTLYNE,*)&
         &'INVALID QUALIFIER WORD USED WITH "TABLE" COMMAND'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
!
   IF(WC.EQ.'TABLE'.AND.SQ.EQ.1) THEN
      ...
      &WQ.EQ.'PRINT1'.OR.WQ.EQ.'PRINT2'.OR.WQ.EQ.'SAVE'.OR.WQ.EQ.&
      &'RELOAD'.OR.WQ.EQ.'PRINT3') THEN
         IF(SN.EQ.1) THEN
            WRITE(OUTLYNE,*)&
            &'"TABLE SETUP", "TABLE CLEAR", "TABLE PRINT1/2/3"'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)&
            &'"TABLE SAVE" AND "TABLE RELOAD" TAKE NO NUMERIC WORD INPUT'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!
   ...
   END IF
!
!       EXPLICIT NW1 AND NW2
!
   IF(WC.EQ.'TABLE'.AND.SQ.EQ.1) THEN
      IF(WQ.EQ.'PUT'.OR.WQ.EQ.'GET') THEN
         IF(DF1.EQ.1.OR.DF2.EQ.1) THEN
            WRITE(OUTLYNE,*)&
            &'"TABLE PUT" AND "TABLE GET"'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)&
            &'REQUIRE EXPLICIT NUMERIC WORD 1 AND 2 INPUT'
            CALL SHOWIT(1)
            WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
            CALL SHOWIT(1)
            CALL MACFAL
...
!       TABLE CLEAR
   IF(WC.EQ.'TABLE'.AND.WQ.EQ.'CLEAR') THEN
      ...
      END DO
      WRITE(OUTLYNE,*)&
      &'THE CURRENT "TABLE" HAS BEEN CLEARED'
      CALL SHOWIT(1)
      RETURN
   END IF
!
!       TABLE PUT
   IF(WC.EQ.'TABLE'.AND.WQ.EQ.'PUT') THEN
      ...
      IF(DF3.EQ.0) THEN
!       EXPLICIT NW3 INPUT, SET TABLE ENTRY TO THAT VALUE
         TABL(INT(W1),INT(W2))=W3
         WRITE(OUTLYNE,*)&
         &'NUMERIC WORD 3 VALUE MOVED TO TABLE ROW ',INT(W1)
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'                              TABLE COL ',INT(W2)
         CALL SHOWIT(1)
      ELSE
!       USE ACCUMULATOR VALUE FOR TABLE ENTRY

         TABL(INT(W1),INT(W2))=REG(9)
         WRITE(OUTLYNE,*)&
         &'ACCUMULATOR CONTENTS MOVED TO TABLE ROW ',INT(W1)
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)&
         &'                              TABLE COL ',INT(W2)
         CALL SHOWIT(1)
...
!       OPEN AND WRITE TO FILE
      ...
      &FORM='UNFORMATTED',RECL=(132*NRECL),STATUS='UNKNOWN')
!
      ...
      DO 210 I=0,100
         ...
         &TABL(I,1),TABL(I,2),TABL(I,3),TABL(I,4),TABL(I,5),&
         &TABL(I,6),TABL(I,7),TABL(I,8),TABL(I,9)
210   CONTINUE
      CALL CLOSE_FILE(36,1)
      WRITE(OUTLYNE,*)&
      &'THE CURRENT "TABLE" HAS BEEN SAVED IN "TAB.DAT"'
      CALL SHOWIT(1)
      RETURN
   END IF
!       TABLE RELOAD
...
310   CONTINUE
      CALL CLOSE_FILE(36,1)
      WRITE(OUTLYNE,*)&
      &'THE CURRENT "TABLE" HAS BEEN RELOADED FROM "TAB.DAT"'
      CALL SHOWIT(1)
      TABEXIST=.TRUE.
      RETURN
   END IF
!       TABLE PRINT
   ...
   &WC.EQ.'TABLE'.AND.WQ.EQ.'PRINT2'.OR.&
   &WC.EQ.'TABLE'.AND.WQ.EQ.'PRINT3') THEN
      IF(.NOT.TABEXIST) THEN
      ...
      END IF
!       PRINT TABLE DATA
!       HERE THE HEADINGS AND VALUES ARE OUTPUT. 3 FORMS ARE USED,
!       ONE FOR 80 COLS(PRINT1) AND ONE FOR 132 COLS(PRINT2) SPACE DELIMITED
!       AND ONE FOR 132 COLS(PRINT3) COMMA DELIMITED
!
      IF(WQ.EQ.'PRINT1') THEN
!       DATA DOES EXIST TO PRINT
!
!       PRINT FIRST 5 COLUMNS OF THE TABLE
!
!       PRINT COLUMN HEADING
         HEADL3='             '//HCOL(1)//' '//HCOL(2)//' '//&
         &HCOL(3)//' '//HCOL(4)//' '//HCOL(5)
         HEADL4='             '//HCOL2(1)//' '//HCOL2(2)//' '//&
         &HCOL2(3)//' '//HCOL2(4)//' '//HCOL2(5)
1002     FORMAT(A79)
!       PRINT HEADING
...
453            FORMAT(A12)
            END IF
         END DO
!       PRINT LAST 4 COLUMNS IF NOT BLANK
         IF(NCOL.GT.5) THEN
!
!       PRINT COLUMN HEADING
            ...
            CALL SHOWIT(0)
            DO I=1,NROW
!       PRINT EACH ROW INCLUDING ROW HEADING. ZERO PRINTS AS BLANK
!       AND A LINE IS SKIPPED IN THE DATA IF A SECOND LINE OF
!       ROW HEADING DESCRIPTION IS USED.
!
!       PRINT THE LINE
...
!       DATA DOES EXIST TO PRINT
         DO I=1,NROW
            ...
            &,TABL(I,3),TABL(I,4),TABL(I,5),TABL(I,6),TABL(I,7)&
            &,TABL(I,8),TABL(I,9)
            CALL SHOWIT(20)
...
!
   IF(&
   &WK.NE.'C1'.AND.WK.NE.'C2'.AND.WK.NE.'C3'.AND.WK.NE.'C4'.AND.&
   &WK.NE.'C5'.AND.WK.NE.'C6'.AND.WK.NE.'C7'.AND.WK.NE.'C8'.AND.&
   &WK.NE.'C9') THEN
      FCOL=0
      WRITE(OUTLYNE,*)&
      &'INVALID QUALIFIER USED WITH "COLHD" OR "COLHD2"'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
...
!
   ...
   &WK.NE.'R5'.AND.WK.NE.'R6'.AND.WK.NE.'R7'.AND.WK.NE.'R8'.AND.&
   &WK.NE.'R9'.AND.WK.NE.'R10'.AND.WK.NE.'R11'.AND.WK.NE.'R12'.AND.&
   &WK.NE.'R13'.AND.WK.NE.'R14'.AND.WK.NE.'R15'.AND.WK.NE.'R16'.AND.&
   &WK.NE.'R17'.AND.WK.NE.'R18'.AND.WK.NE.'R19'.AND.WK.NE.'R16'.AND.&
   &WK.NE.'R21'.AND.WK.NE.'R22'.AND.WK.NE.'R23'.AND.WK.NE.'R24')THEN
      GO TO 4
   END IF
   GO TO 100
4  IF(WK.NE.'R25'.AND.WK.NE.'R26'.AND.WK.NE.'R27'.AND.WK.NE.'R28'&
   &.AND.&
   &WK.NE.'R29'.AND.WK.NE.'R30'.AND.WK.NE.'R31'.AND.WK.NE.'R32'.AND.&
   &WK.NE.'R33'.AND.WK.NE.'R34'.AND.WK.NE.'R35'.AND.WK.NE.'R36'.AND.&
   &WK.NE.'R37'.AND.WK.NE.'R38'.AND.WK.NE.'R39'.AND.WK.NE.'R40')THEN
      GO TO 5
   END IF
   GO TO 100
5  IF(WK.NE.'R41'.AND.WK.NE.'R42'.AND.WK.NE.'R43'.AND.WK.NE.'R44'&
   &.AND.&
   &WK.NE.'R45'.AND.WK.NE.'R46'.AND.WK.NE.'R47'.AND.WK.NE.'R48'.AND.&
   &WK.NE.'R49'.AND.WK.NE.'R50'.AND.WK.NE.'R51'.AND.WK.NE.'R52'.AND.&
   &WK.NE.'R53'.AND.WK.NE.'R54'.AND.WK.NE.'R55'.AND.WK.NE.'R56'.AND.&
   &WK.NE.'R57'.AND.WK.NE.'R58'.AND.WK.NE.'R59'.AND.WK.NE.'R60'.AND.&
   &WK.NE.'R61'.AND.WK.NE.'R62'.AND.WK.NE.'R63'.AND.WK.NE.'R64'.AND.&
   &WK.NE.'R65'.AND.WK.NE.'R66'.AND.WK.NE.'R67'.AND.WK.NE.'R68'.AND.&
   &WK.NE.'R69'.AND.WK.NE.'R70'.AND.WK.NE.'R71'.AND.WK.NE.'R72'.AND.&
   &WK.NE.'R73'.AND.WK.NE.'R74'.AND.WK.NE.'R75'.AND.WK.NE.'R76')THEN
      GO TO 10
   END IF
   GO TO 100
10 IF(&
   &WK.NE.'R77'.AND.WK.NE.'R78'.AND.WK.NE.'R79'.AND.WK.NE.'R80'.AND.&
   &WK.NE.'R81'.AND.WK.NE.'R82'.AND.WK.NE.'R83'.AND.WK.NE.'R84'.AND.&
   &WK.NE.'R85'.AND.WK.NE.'R86'.AND.WK.NE.'R87'.AND.WK.NE.'R88'.AND.&
   &WK.NE.'R89'.AND.WK.NE.'R90'.AND.WK.NE.'R91'.AND.WK.NE.'R92'.AND.&
   &WK.NE.'R93'.AND.WK.NE.'R94'.AND.WK.NE.'R95'.AND.WK.NE.'R96'.AND.&
   &WK.NE.'R97'.AND.WK.NE.'R98'.AND.WK.NE.'R99'.AND.WK.NE.'R100'&
   &)THEN
      GO TO 15
   END IF
   GO TO 100
15 WRITE(OUTLYNE,*)&
   &'INVALID QUALIFIER USED WITH "ROWHD" OR "ROWHD2"'
   CALL SHOWIT(1)
   WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
   CALL SHOWIT(1)
   CALL MACFAL
...
!
   INTEGER I
!
!       X = REG(9),Y=REG(10),Z=REG(11),T=REG(12)
...
!
   ...
   IF(WC.EQ.'C/') THEN
      ...
      ELSE
         ...
         &.OR.REG(9).EQ.0.0D0.AND.REG(13).EQ.0.0D0) THEN
...
!       THIS IS CALLED TO WRITE OUT THE CONTENTS
!       OF REGISTER REG(40). THIS IS THE LASTX REGISTER.
!       PRIOR TO REASSIGNMENT OF THE VALUE OF THE ACCUMULATOR
!       REG(9) THE CURRENT VALUE IS ALWAYS STORED IN REG(40)
      IF(OUT.EQ.6.OR.OUT.EQ.7) THEN
      ...
      ELSE
         ...
         CALL SHOWIT(0)
      END IF
!       THE FOLLOWING ARE THE FORMAT STATEMENTS
101   FORMAT(D23.15)
      RETURN
   END IF
   IF(WC.EQ.'PRLSTIX') THEN
!       THIS IS CALLED TO WRITE OUT THE CONTENTS
!       OF REGISTER REG(30). THIS IS THE LASTIX REGISTER.
!       PRIOR TO REASSIGNMENT OF THE VALUE OF THE IX RERGISTER
!       REG(13) THE CURRENT VALUE IS ALWAYS STORED IN REG(30)
      IF(OUT.EQ.6.OR.OUT.EQ.7) THEN
...
!
   IF(SST.EQ.1.OR.SQ.EQ.1) THEN
      IF(WC.EQ.'STOREMIN')&
      &WRITE(OUTLYNE,*)'"STOREMIN" TAKES NO STRING OR QUALIFIER INPUT'
      IF(WC.EQ.'STOREMAX')&
      &WRITE(OUTLYNE,*)'"STOREMAX" TAKES NO STRING OR QUALIFIER INPUT'
      IF(WC.EQ.'RESETMIN')&
      &WRITE(OUTLYNE,*)'"RESETMIN" TAKES NO STRING OR QUALIFIER INPUT'
      IF(WC.EQ.'RESETMAX')&
      &WRITE(OUTLYNE,*)'"RESETMAX" TAKES NO STRING OR QUALIFIER INPUT'
      CALL SHOWIT(1)
      WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
      CALL SHOWIT(1)
      CALL MACFAL
   ...
   IF(WC.EQ.'RESETMIN'.OR.WC.EQ.'RESETMAX') THEN
      IF(S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         IF(WC.EQ.'RESETMIN')&
         &WRITE(OUTLYNE,*)&
         &'"RESETMIN" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
         IF(WC.EQ.'RESETMAX')&
         &WRITE(OUTLYNE,*)&
         &'"RESETMAX" ONLY TAKES NUMERIC WORDS #1 AND #2 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
   ...
   IF(WC.EQ.'STOREMIN'.OR.WC.EQ.'STOREMAX') THEN
      IF(S2.EQ.1.OR.S3.EQ.1.OR.S4.EQ.1.OR.S5.EQ.1) THEN
         IF(WC.EQ.'STOREMIN')&
         &WRITE(OUTLYNE,*)'"STOREMIN" ONLY TAKES NUMERIC WORD #1 INPUT'
         IF(WC.EQ.'STOREMAX')&
         &WRITE(OUTLYNE,*)'"STOREMAX" ONLY TAKES NUMERIC WORD #1 INPUT'
         CALL SHOWIT(1)
         WRITE(OUTLYNE,*)'RE-ENTER COMMAND'
         CALL SHOWIT(1)
         CALL MACFAL
...
SUBROUTINE SAVEFONT(I)
   ...
   &,FORM='FORMATTED',FILE='FONTSAVE.DAT'&
   &,STATUS='UNKNOWN')
   REWIND(UNIT=110)
